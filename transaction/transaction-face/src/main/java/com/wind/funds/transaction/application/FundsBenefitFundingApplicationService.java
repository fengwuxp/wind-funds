 ...
 * 生成资金底座可理解、可记账、可退款、可撤销、可对账的权益让利资金交易。</p>
 *
...
public interface FundsBenefitFundingApplicationService {
     ...
     * 服务返回资金交易流水号，后续退款、撤销、清结算、对账和重放均以该交易流水号作为稳定引用。</p>
     *
     * @param request  权益让利资金交易请求
     * @param operator 操作者
     * @return 资金交易流水号
     */
    @NonNull String apply(
            @NonNull FundsBenefitFundingApplyRequest request,
            @NonNull WindOperator operator);

    /**
     * 退回已入账的权益让利资金影响。
     *
     * <p>退款必须引用原权益让利资金交易流水号，并以本次退款业务流水号保证幂等、回放和审计追踪。</p>
     *
     ...
     * 撤销尚可反向冲销的权益让利资金影响。
     *
     * <p>撤销用于业务取消、人工纠错或准实时反向冲销场景；若业务语义已经进入退款，应使用 {@link #refund}。</p>
     *
     * @param request  权益让利撤销请求
     * @param operator 操作者
     * @return 资金交易流水号
     */
    @NonNull String reverse(
            @NonNull FundsBenefitFundingReverseRequest request,
            @NonNull WindOperator operator);
}
