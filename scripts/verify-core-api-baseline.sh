...
if [[ $(wc -l < "$actual_types" | tr -d ' ') -ne 102 ]]; then
    echo "Expected 102 public top-level core types; found $(wc -l < "$actual_types" | tr -d ' ')"
    exit 1
fi
...
if [[ $(wc -l < "$stable_types" | tr -d ' ') -ne 94 ]]; then
    echo "Expected 94 stable core types; found $(wc -l < "$stable_types" | tr -d ' ')"
    exit 1
fi
...
if $update_baseline; then
    ...
    echo "Core API baseline updated: 94 stable, 4 experimental, 4 internal public top-level types; public nested signatures included"
    exit 0
fi
...
echo "Core API baseline verified: 94 stable, 4 experimental, 4 internal public top-level types; public nested signatures included"
