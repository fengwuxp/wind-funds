...
| 产品输入 | [支付资金公共能力层产品设计](../产品设计/支付资金公共能力层-产品设计.md) 的已接受产品决策与 Product Context Card |
...
旧文档 [支付资金底座DSL承载层设计.md](支付资金底座DSL承载层设计.md) 继续作为当前实现、账务矩阵和历史方案的证据；其中 API 草图、场景枚举、Benefit 旧模型、账目和治理细节不能反向覆盖本文。
...
5. 本文只把稳定词汇交给后续 DSL、系分和 TDD；实现准入统一由 OpenSpec 管理。

当前结论：本章只定义稳定词汇与事实层级；HOST/E4/E5、Consumer、Java/API、生产/测试源码、Git、发布和生产不由本文授权。

## 10. 指令、事实、route、账务与逆向边界
...
目标态切换不保留 bridge、转发重载、别名、双写双读或兼容窗口。具体删除与 Consumer 同步迁移由系分、清册、垂直切片和 OpenSpec 单独管理。
...
5. 本章只向 Core/Ledger 系分交付稳定边界；HOST/E4/E5、代码、测试、Consumer、Git、发布和生产准入统一见 OpenSpec。

当前结论：上述 DSL 边界可供 Core/Ledger 系分使用，不构成 Java/API、实现、测试、Consumer、Git、发布或生产授权。
...
该物理命名只把稳定 release 语义映射到现有耐久事实，不新增 DSL 词汇、兼容形式或第二事实源。
...
A 的选择不新增 DSL；未来只有项目级重开并证明完整关联合同后，才能产出 10.9 的 canonical refund 事实。不得借模块归属迁移重开 refund DSL。
...
模块搬迁不得改变任何公共事实的 identity、Money、outcome/effect、original refs、route provenance 或 SemanticDigest。不存在兼容 alias、双 DTO、双读/双写或按 Consumer 选择旧/新 Owner；若真实调用清册、MIG-05 原 LedgerTransaction 引用解析合同或目标 Consumer E4 未闭合，对应对象保持未迁移并阻断整个原子切换。`capte-domain` 作为无独立数据库/部署进程的通用模块，其 library test host E4 不替代未来真实部署宿主 L4。
...
物理实现只能把既有 `referenceLedgerTransactionSn` 从 Transaction 自有事实组确定性解析后交给 converter；禁止引入 `LedgerEvidenceResolver`、通用查询 envelope、registry/factory、兼容 facade/V2 或新的 Public ref。缺失或冲突仍表达为 fail-closed/UNKNOWN，不能以 nullable 字段、contextVariables 或查询第一条形成运行时 fallback。

#### 10.12.1 `MIG-05B` profile 引用与受控建账边界
...
本题不新增 `ProfileFact`、`LedgerAdmissionFact`、registry、factory、policy engine 或兼容 alias。Human Owner 已接受 A：复用 `LedgerService` 的受控初始化 surface，catalog/integrity guard 留在 `ledger-impl`，B/C 未选择且不是 fallback。

#### 10.12.2 外部资金腿方向的稳定读法
...
实现和验证阶段不得改变上述词汇、身份、引用或失败关闭合同。

#### 10.12.3 `FUNDING_BALANCE_ADJUST` 的稳定读法
...
该稳定读法不改变 `LIMIT_ADJUST`、外部资金腿、ordinary internal leg 或 reverse replay，也不新增 direction enum、adjustment SPI、registry、factory、V2、兼容 alias、双写或 fallback。Human Owner 已接受方案 A。

#### 10.12.4 余额调账非负 Public surface 的稳定读法
...
本卡不改变 10.12.3 的 signed `ADJUSTMENT`、Money、route、posting、LedgerEntry、Balance、幂等和审计引用。未来真实 Consumer 若需要负余额，必须先提供可查询的 authority、额度窗口、并发累计、撤销/到期和恢复证据，再重开独立 Contract Inquiry；当前不预设授权事实、策略引擎、SPI、DTO、schema 或兼容入口。

实现与 API 治理证据不得产生新 DSL，也不得恢复已删除词汇、旧 profile spec、`getStatus()` 兼容方法或任何双入口。
...
该 Profile 不新增全局 `core` 类型，不冻结 Java enum/DTO/API/table，也不建立兼容 alias、V2 平行契约或通用规则引擎。`ComparisonRuleRef` 是 `reconciliation-face` 语义引用，不批准 registry 或策略执行器。
...
最小公共物理契约可以在 L4 宿主出现前明确，但 E4、L4 和生产准出仍须分别提供证据，互不替代。

### 10.15 Provider-computed strict-exact 物理投影约束
...
Human Owner 明确不考虑任何兼容问题；物理契约一次删除旧 caller assertion 与单 run Gate，替换为 Provider-computed strict-exact 与 mandatory GateRequirement，不保留并行旧面。

### 10.16 Source/Run 物理投影护栏
...
Human Owner 接受的是上述最小物理映射及破坏式替换方向，不是新的 DSL 概念或兼容层；GateRequirement 的稳定边界见 10.17。

### 10.17 GateRequirement 的最小 DSL 投影
...
本节只定义稳定语义；具体实现与执行状态统一见 OpenSpec、系分与 TDD。

### 10.18 `MIG-03` Action、Ledger 与 Balance 闭合的 DSL 护栏
...
上述 DSL 边界不批准 Java service、DTO、枚举、表、缓存或物化视图；真实 Consumer 或 Provider 内部恢复用例必须由 Owner 选择最小承载模块。

### 10.19 `MIG-03` 首个 closure 切片的稳定 DSL 候选
...
### 10.20 `MIG-03` Recorded evidence 精确语义
...
上述语义不保留旧字段、alias、V2、bridge 或 context fallback。

### 10.21 `MIG-03` Ledger Persisted Digest 稳定 DSL
...
本卡选择 `ledger.persisted-transaction.v1 / ledger.persisted-plan.v1 / ledger.persisted-entry.v1` 作为唯一目标 domain/version，不兼容旧 `ledger.transaction.request` 或 `WindObjectDigestUtils` 结果。旧摘要不匹配即 fail-closed，不双验、不回填、不降级为 hasText 校验；exchange-rate scale 由 `CanonicalDecimal` 统一。

### 10.22 `MIG-03` Ledger 完整性与 Clearing 语义分层

`VerifiedLedgerAggregate` 的 DSL 含义必须先于任何下游业务分类：持久化 transaction/plan/entry 的摘要、父引用、数量或唯一性不成立时，不存在可供 Clearing 继续解释的合法 Ledger fact。该顺序不增加新 DSL 类型，也不允许 `legacy digest`、fallback 或调用方声明“可信”。

两类输入必须分开：删除 plan 属于无效 aggregate，必须由 Ledger fail-closed；修改 business identity、subject/direction/role 后仍要判断 Clearing 分类，则输入必须使用同一 persisted v1 重新形成有效 aggregate。重新计算验证载体摘要不是兼容路径，而是明确区分“事实完整性”和“业务适用性”。

### 10.23 `CI-W5-MIG05D-LEDGER-POSTING-COMMAND-001-A` 已接受 DSL 合同

Human Owner 已接受 A。目标高阶命令的精确 core 合同冻结为：
...
旧 `LedgerPostingAssembler` core type 与 `post(LedgerTransactionSpec)` 不属于目标写面，不保留 alias、deprecated、overload、V2、bridge、双端口或 fallback。Ledger 在组装前复验 instruction/route 六字段并要求 tenant/root；`ledger.posting.command.identity` 只吸收 `tenantId + fundsTransactionSn + eventType + businessScene + businessSn`，identity 只定位动作，persisted aggregate digest 继续判定同义/异义，同一 root 的不同 action 由 event/business action 分离。该 DSL 能力不把 Ledger SN 外推为订单成功、外部到账、清算完成、对账通过或生产 finality。
