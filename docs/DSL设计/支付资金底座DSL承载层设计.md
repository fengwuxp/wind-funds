...
当前 `MIG-08 / D-R8B-001` 口径：Benefit 资格、出资性质、原订单和退款资格归 Host/Consumer；Consumer adapter 将已决策责任归一为 generic direct pay/refund，并以 `FundsActionFact` 证明完成和恢复。Provider 不再把 Benefit 场景 service、request 或 funding-nature enum 作为目标 Public API。本文件 7.2.1 中保留的旧 Java 类图、签名和字段表只作迁移 provenance，不构成当前接入合同；当前稳定 DSL 仍是 business identity、Money、账户责任、original refs、route provenance、Ledger/Balance 与 ActionFact。
...
VCC 或父子账户进入 route 时，只固化当前参与方的直接父账户证据：子账户由 `RouteParticipant.subjectRef` 表达，`AccountHierarchySnapshot` 只包含 `relationSn + parentAccountRef`。当前 DSL 不提供 root、完整路径、层级版本或运行时多级树解析；父账户是否产生 `PARENT_CONTROL`、`TRANSFER_OUT` 或 `TRANSFER_IN` 分录，必须由显式 posting plan 决定，不能由层级关系自动推导。
...
| 对账差错和运营处理 | 对账误报关闭、对账补事实、冲正、调账或核销。 | 说明误报不入账，真实差错由上层业务调用受控资金能力完成后再登记动作并重新对账。 |
...
| 授权完成或部分完成 | `AUTHORIZATION_TRANSACTION / COMPLETE`。 |
...
| 对账误报关闭 | 对账差错处理动作。 |
| 对账补事实、冲正、调账或核销 | 由上层业务选择既有明确资金能力，例如 `DIRECT_TRANSACTION / ADJUSTMENT`；对账不自行生成指令。 |
...
| 清算批次确认 / 清算确认 | 同一账务账户的 `CLEARING` 与 `AVAILABLE`。 | 当前固定为同主体账目内释放；费用、准备金、补贴或分润必须在清算前由各自资金事实入账，不在清算确认时临时拆分。 | 借：账务账户 `CLEARING`；贷：同账户 `AVAILABLE`，金额和币种完全一致。 | 待清算减少，可用增加；批次失败不得产生部分余额变化。 | 清分确认和清算候选不入账；退款中、争议中、风控冻结、重大差错、候选未锁定或批次未确认不得进入清算确认。`TDD-CLS-*`、`TDD-RECON-*`。 |
...
| `CREDIT_ACCOUNT` | 承载授信额度、可用额度、授权占用和已用额度的控制账户。 | `LIMIT`、`AVAILABLE`、`AUTHORIZATION`、`OUTSTANDING` |
| VCC 关联子账户 | 发卡场景中的资金子账户或信用子账户；外部别名 Card Account / Financial Account / VCC Account 进入 DSL 后统一映射为 `FUNDING_ACCOUNT` 或 `CREDIT_ACCOUNT` 的具体子账户。 | 资金子账户可使用 `AVAILABLE`、`AUTHORIZATION`、`FROZEN`、`CLEARING`、`SETTLEMENT`、`IN_TRANSIT`、`FEE`、`ADJUSTMENT`；信用子账户可使用 `LIMIT`、`AVAILABLE`、`AUTHORIZATION`、`OUTSTANDING` 及经确认的费用或争议账目。 |
...
| VCC 资金/信用子账户 | VCC 场景中可记账子账户的余额、授权占用、清算、退款、费用和已确认争议资金结果；子账户由参与方主体表达，直接父账户关系由 `AccountHierarchySnapshot` 补充。 | 卡号、PAN、token、Cardholder、issuer 原始账户状态、VCC 事件或主状态、卡账单、发卡处理商协议、独立 `VCC_ACCOUNT` 或卡号账本。 | 预付卡资金入账、共享卡授权、VCC 清算、本金退款和资金对账。 |
...
| 产品对象 | DSL 定性 |
| --- | --- |
| VCC 关联子账户 | 发卡场景中的内部资金子账户或信用子账户，归入 `FUNDING_ACCOUNT` 或 `CREDIT_ACCOUNT`；必须有账目 profile、币种和状态。存在父子结构时固化直接父账户关系；资金责任由独立资金责任关系和显式 route leg 表达。 |
...
| `OUTSTANDING` | 信用账户授权完成后的已用额度；不是现金、交易状态或外部 Network Settlement 完成证明。 |
| `SETTLEMENT` | 资金账户已进入内部结算责任或出款锁定的金额；该桶不证明外部 Network Settlement 已完成。 |
...
本规范不再增加其他与完成金额含义重复的账目。VCC 信用账户完成后的额度责任统一由 `OUTSTANDING` 承接，业务完成金额仍以交易生命周期事实为准；支出控制范围已使用、剩余和占用由 Spend Rule 控制视图、规则快照和报表口径计算，不从 LedgerEntry 反推支出控制范围余额主体。
...
| `exchangeRate` | 是 | 汇率快照。 |
| `eventTime` | 是 | 事实发生时间。 |
| `operator` | 是 | 当前调用链的 `WindOperator` 运行时操作者上下文。 |
...
`operator` 统一复用 `wind-operator` 的 `WindOperator`，只在同步运行链路内表达当前操作者。资金 Core 只能读取操作者标识、主体类型、名称、来源应用和 `isSystem()` 等稳定身份语义；不得调用权限判断方法，不得读取动态 IP 或设备信息，也不得直接序列化、持久化或纳入稳定摘要。需要审计时，由事实写入边界显式投影 `operatorId`、`actorType`、`operatorName`、`appName` 等稳定字段；异步和回放链路不能把运行时权限代理当作历史审计事实。
...
### 7.2.1 让利出资记账交易（历史 Java 承载证据）

本节记录 Benefit 场景 facade 退役前的 Java 承载形态，用于解释迁移来源，不再批准旧 service/request。当前 Consumer adapter 仍把业务侧已决策结果转换成可记账、可退款、可清结算和可对账的标准资金动作；资金底座只回答谁的钱、多少钱、怎么变的，不计算券规则、不维护券包生命周期，也不保存券、活动或规则来源归因。

对象关系：

| 对象 | 用途 | 边界 |
| --- | --- | --- |
| `FundsBenefitContributionTransactionService` | 历史 Java 公共契约中的让利出资记账服务，曾提供 `settle`、`refund` 并返回资金交易流水号。 | 仅作退役 provenance；当前由 Consumer adapter 调用 generic direct action。 |
| `FundsBenefitContributionSettleRequest` | 历史上表达已决策出资账户到让利承接账户的一笔入账交易。 | 仅用于理解字段迁移；不再是目标 Public request。 |
| `FundsBenefitContributionRefundRequest` | 历史上表达原让利出资交易的退款、业务取消、人工纠错或反向冲销。 | 仅用于理解 original-reference 迁移；不再是目标 Public request。 |
...
2. 当时交易层入口使用 `FundsBenefitContributionTransactionService`；当前目标已收敛为 Consumer adapter 调用 generic direct pay/refund，且不得新增任何同义场景 facade 或平行生命周期。
...
| 字段或对象 | 营销/让利账户语义 | 边界 |
| --- | --- | --- |
| `costBearerAccountId` | 指向已解析的成本承担资金账户，例如平台营销资金账户、商户让利责任账户或合作方补贴账户。 | 只接受资金账户或信用账户标识，不能指向支付工具、活动、券实例、营销规则、用户或商户经营主体；单个平台营销账户不能合并商户或合作方出资。 |
| `benefitReceiverAccountId` | 指向本次让利出资交易的承接资金账户，例如商户清结算账户、用户补贴账户或订单维度让利归集账户。 | 是当前资金路径的目标账户，不等同于营销系统中的收券用户；不作为营销规则或券包状态。 |
...
| 让利出资记账交易服务 | 历史 Java 公共契约为 `FundsBenefitContributionTransactionService`；当前目标由 Consumer adapter 调用 generic direct pay/refund。 |
| 不设置独立来源归因解析服务 | 请求已经携带成本承担主体、让利承接账务主体和资金性质；券、活动、规则和分摊来源由上游保留，资金底座不新增来源归因解析服务或 `benefitFundingSources`。 |
| 账户解析在进入本服务前完成 | 调用方必须把活动、券、规则或业务主体解析为可记账 `FundsAccountId`；交易实现再将账户契约转换为内部 route `SubjectRef`。缺成本承担账户、缺承接账户或无真实入账影响时，应在 route leg、posting plan 或 LedgerEntry 生成前失败。 |
...
classDiagram
    ...
    class FundsBenefitContributionSettleRequest {
      ...
      +FundsAccountId costBearerAccountId
...
以下接口草图只记录退役前公共契约骨架，不是当前接入说明；当前调用以 generic direct request 与 ActionFact 合同为准。
...
| `costBearerAccountId` | 是 | 平台、商户或合作方让利责任承担资金账户。 |
| `benefitReceiverAccountId` | 是 | 让利承接资金账户；`settle` 会作为标准直接交易的收款方。 |
...
1. Consumer Benefit adapter 只表达已决策让利出资责任，不承接营销实时规则、券包库存之外的 Funds 事实；Provider 不再保留场景 facade。
...
| 能力范围 | 可声明完成 | 不可声明完成 |
| --- | --- | --- |
| 公共契约 | Consumer adapter 能把成本承担主体、让利承接账务主体、Money 与业务 identity 归一为 generic direct pay/refund，并以 ActionFact 恢复。 | route、posting、replay、清结算或对账已经完整消费权益让利资金事实；或已具备批量结算 API。 |
| route/posting 消费 | 最小交付已支持 `settle` 和退款型逆向处理通过标准直接交易、route、posting、ledger 链路形成独立资金影响；无资金转移解释事实暂 fail-fast 且无资金或账务副作用。 | 补贴、本金、手续费或代金券净额混记；缺资金责任主体仍放行；把 `HOLD_ONLY`、`RELEASE_ONLY`、`REVERSAL_REQUIRED` 或非入账权益解释事实声明为已具备生产资金流可用。 |
...
`contextVariables` 只允许承载非核心的轻量关联引用或稳定摘要，不是公共契约的兼容通道，也不承载完整资金规则。当前 Capte Benefit adapter 不向 generic direct request 写入 context；账户、目标账目、核心金额、出资分摊、券、活动、规则来源、`ledgerEffect` 和 `benefitFundingSources` 均不得借 context 形成旁路。

| 字段类别 | 可否放入 `contextVariables` | 说明 |
| --- | --- | --- |
...
| --- | --- | --- | --- |
| Consumer Benefit adapter + generic direct action | adapter 承接场景决策并归一资金命令。 | generic direct action 生成标准资金交易并委派 route/posting。 | `FundsActionFact` 作为退款、业务取消、清结算、对账和投影的稳定事实入口。 |
| `RouteSnapshotSpec` | 只接受非核心轻量引用或摘要。 | 固化权益资金交易引用或等价不可变摘要。 | 作为退款、业务取消、可信撤销、拒付/争议结果和差错回放来源。 |
...
| C04 | 平台补贴账户是否进入 `PlatformAccountsSnapshotSpec`。 | 使用 `costBearerAccountId` 和账户 profile 表达已解析账户，也可由平台账户快照承接解析结果；生产账务口径必须明确平台成本账户角色，未确认前不得把平台补贴与用户本金净额混记。 |
...
| 对象 | 说明 | 约束 |
| --- | --- | --- |
| `LedgerTransaction` | 一组账务计划成功过账后形成的不可变账务事实聚合。 | 创建即代表完整过账，必须能追溯到资金指令和来源事实，不承载业务 Pending、失败、结算或冲正状态。 |
...
| 账务事实不可变 | `LedgerTransaction`、`PostingPlan` 和 `LedgerEntry` 不通过 update/delete 修正；错账必须追加引用原交易的冲正、补记、调账、追偿或核销事实。 | 用状态更新、管理型 CRUD、投影重放、报表汇总或手工备注直接改原交易、原分录或余额。 |
...
| --- | --- | --- | --- |
| 支付工具 | `PaymentInstrumentRef`、binding snapshot、external reference。 | 工具维度资金流水、VA 入金/出金资金解释、原路径回放索引；上层可据此组合业务账单。 | 工具作为 `LedgerEntry` 主体或余额主体，或 wind-funds 声明生成发卡卡账单。 |
...
Spend Control Movement 是控制额度变动事实 DSL，不是规则准入记录或资金事实 DSL。准入通过和拒绝由 `SpendRuleDecisionRecord` 承载；控制额度变动只允许 `LIMIT_INCREASED`、`LIMIT_DECREASED`、`RESERVED`、`CONSUMED`、`REFUND_COMPENSATED`、`RELEASED`。超时不可信，不生成控制额度变动；交易失败或拒绝导致的同事务回滚也不补写 `RELEASED`。它可以作为 `TransactionView` 的输入并派生预算控制投影，但不能生成 `RouteLeg`、`PostingPlan`、`LedgerEntry`、`LedgerTransaction` 或 `BalanceProjection`。
...
| `transactionRef` / `originalMovementRef` | 交易后控制消耗、可信释放或显式退款控制补偿时回链原资金交易和原控制额度变动。 | 缺原控制额度变动或原交易时补写控制消费事实，或由控制额度变动反向修改交易事实。 |
...
### 7.10 Core DSL 使用者与包边界

Core DSL 是资金主链内部的规范事实和引擎 SPI，不是业务系统直接拼装的通用 SDK。业务调用方优先使用 `*-face` 的场景服务、Request、DTO 和稳定叶子值对象；`FundsInstruction`、`ResolvedRoute`、`RouteSnapshot`、`PostingPlan` 和 `LedgerTransaction` 由各能力实现内部转换和生成。

| 使用者 | 允许入口 | 禁止入口 |
| --- | --- | --- |
| 业务系统 / 宿主应用 | `transaction-face`、`wallet-face`、`ledger-face` 等场景服务及其 Request/DTO；账户参数使用 `FundsAccountId`。 | 直接构造 `FundsInstruction`、route 默认实现、posting plan 或 ledger entry；依赖 `*-impl`。 |
| 资金能力实现 | Core 的 spec、ref、enum、value object 和 port；在 `*-impl` 内构造默认不可变实现。 | 把实现模型重新暴露进 Core 或 face，绕过场景服务写账。 |
| 扩展引擎实现者 | `RouteResolver`、`RouteSnapshotFactory`、`LedgerPostingAssembler` 等显式 SPI。 | 通过 `contextVariables` 旁路新增核心金额、主体、路由或账务语义。 |

Core 包名按能力优先、对象性质其次组织：

```text
com.wind.funds.transaction
  enums/       交易、指令和来源事实枚举
  spec/        FundsInstruction、Reference、Fee 等规范契约
  support/     跨模块共用且具有资金不变量的校验支持

com.wind.funds.route
  enums/       路由枚举
  ref/         SubjectRef、PaymentInstrumentRef、ExternalAccountRef
  spec/        ResolvedRoute、RouteSnapshot、RouteLeg 等规范契约
  support/     敏感字段等路由边界校验

com.wind.funds.ledger
  enums/       账本与 posting 枚举
  spec/        LedgerTransaction、PostingPlan、LedgerEntry、Profile 契约
  support/     账本不变量支持
...
| 清算确认 | `DIRECT_TRANSACTION / CLEARING_CONFIRM`，`FundsTransactionMode.DIRECT`，交易类型 `CLEARING`。 | 同一资金账户 `CLEARING -> AVAILABLE`，`SETTLEMENT + WITHIN_SUBJECT + RELEASE + NON_REPLAYABLE`。 | 只处理上层清算批次完成最终准入复核后的标量金额，不解析候选、清算规则或退款。 | 清算批次生成不直接入账；确认以清算批次号幂等。余额不足在账本写入前确定拒绝时保留 `FAILED` 资金事实但不生成账本事实；技术异常或写账后失败整体回滚。 |
...
| 平台补贴券补足商户 | `DIRECT_TRANSACTION / PAY` + 平台补贴组件。 | Consumer adapter 以平台资金账户向商户 `CLEARING` 账户提交 generic direct pay。 | 生成独立补贴 leg 或明确独立伴随指令；不得和本金净额混记。 | `DSL-BENEFIT-PLATFORM-SUBSIDY-001`、`TDD-BEN-DIR-002`。 |
...
| shared card | `instrumentRef`、binding snapshot、cardholder/department/project 上下文、SpendControlScope 上下文、Spend Rule 快照、`SubjectRef(CREDIT_ACCOUNT)`、参与方 `AccountHierarchySnapshot` | 共享卡是使用和绑定模式；每张卡绑定一个信用子账户，多卡可关联同一父账户。共享卡授权必须用显式 route leg 同时占用父 `FundingAccount`，父账户真实出资也必须由原路径显式表达；这些路径不得从层级关系自动推导。每次授权必须解释谁使用、受哪个预算、规则或资金路径约束。 | 不得新增共享卡主体类型；不得把支出控制范围或 Spend Rule 当成入账主体；不得在逆向事件中按当前绑定或账户层级重选路。 |
...
| 用例 | 覆盖级别 | DSL 语义 | 服务/事件承接 | 测试承接 |
| --- | --- | --- | --- | --- |
| 授权后全额完成 | 必须 | 授权占用一次性全部转为实际消费或清算结果。 | `COMPLETE`，`complete`。 | 授权剩余归零；收款方或清算桶正确增加；不触碰 `LIMIT`。 |
| 授权后部分完成 | 必须 | 只完成部分授权金额，保留剩余可完成或可释放金额。 | `COMPLETE`，`complete`。 | 累计完成金额不超过授权金额；剩余授权正确；每次 posting 独立平衡。 |
| 多次完成或拆单完成 | 必须 | 同一授权可被多笔清算或拆单事件逐步完成。 | 多次 `complete`，同一原授权引用。 | 多次完成累计正确；重复通知幂等；完成明细数量正确；不重复创建授权主记录。 |
| 强制完成 | 必须 | 外部没有前置授权，但确认发生了必须入账的消费结果。 | 使用 `complete` 的强制完成模式承接；初始 FORCE 模式不依赖 `authorizationTransactionSn`，而依赖外部原事实引用和受信策略或审批快照。 | 不伪造授权占用；允许透支必须有受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计；不污染授权占用生命周期。 |

#### 9.2.3 授权链退款用例

| 用例 | 覆盖级别 | DSL 语义 | 服务/事件承接 | 测试承接 |
| --- | --- | --- | --- | --- |
| 已完成授权退款 | 必须 | 基于已完成的授权路径做反向退款。 | `AUTHORIZATION_TRANSACTION / AUTH_REFUND`，`refund`。 | 退款入账；关联原授权和原完成明细；累计退款不超过已完成金额；不按当前绑定重新选路。 |
| 无授权直接退款 | 必须 | 外部没有前置授权，但存在可追溯外部引用，需要按退款事实回补。 | `AUTHORIZATION_TRANSACTION / AUTH_REFUND`，`refund` 的无授权退款模式。 | 不补造授权占用；以空原授权流水进入 no-auth 语义，保留 `externalReferenceSn`、退款原因、操作者和审计上下文；无外部引用、无原因或无审计时失败或进入差错。 |
| 多次退款 | 必须 | 同一授权完成后允许多次退款。 | 多次 `refund`。 | 多次退款累计正确；同一授权可多次退款；累计退款不超过已完成金额。 |
| 已完成授权争议裁决资金结果 | 必须 | 已完成授权发生外部争议或拒付时，先由上层 dispute case 得出裁决；只有用户胜诉或业务决策要求退款时，才按授权链逆向退款事实处理。 | `refund` 携带争议原因、凭证、外部案件引用、裁决结果和审计上下文；用户败诉或无需资金处理时不调用资金交易入口。 | 与授权拒绝严格区分；累计争议退款/退款不超过已完成金额；不要求落到 `FundsAuthorizationTransactionService#chargeback`；即使底层终态复用退款终态，也必须在 reason、external reference、projection 和 audit 中保留争议语义；用户败诉不得生成 route、posting、LedgerEntry 或余额变化。 |
...
| 拒付被强制落到独立 `chargeback` 服务入口 | 必须 | 误把上层案件过程当成资金结果。 | 授权交易层只通过 `refund` 承接上层已确认的原本金退款；无资金影响时不调用 wind-funds，不以 `chargeback` 方法为目标落地。 |
...
| shared card 授权 | `AUTHORIZATION_TRANSACTION / AUTHORIZE`。 | 共享卡只表达工具和使用模式；支出控制范围和 Spend Rule 只表达控制上下文；信用子账户与父 `FundingAccount` 必须分别执行 `AVAILABLE -> AUTHORIZATION`，route 显式承载两个参与主体，父账户不能仅由层级关系隐式推导。 | 固化使用人、绑定版本、信用子账户、父账户快照、支出控制范围、Spend Rule、资金责任解析关系和规则版本；多责任主体不唯一、任一余额或额度不足时整体失败且无副作用。 | `DSL-VCC-HIERARCHY-001`、`DSL-PAYMENT-INSTRUMENT-SHARED-CARD-001`、`TDD-P2-VCC-005`、`TDD-AUTH-008`、`TDD-AUTH-009`。 |
...
| 多次完成或拆单完成 | 多次 `AUTHORIZATION_TRANSACTION / COMPLETE`，同一原授权引用。 | 原授权 `AUTHORIZATION` 分次 -> 收款方或清算桶。 | 相同幂等摘要不得重复入账；不同完成明细累计闭合；不重复创建授权主记录。 |
...
| 无授权强制完成 | `AUTHORIZATION_TRANSACTION / COMPLETE` 的强制完成模式。 | 无前置授权；付款主体 `AVAILABLE` 可按受信策略或审批快照 -> 收款方或清算桶。 | 必须有强制完成模式、受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计；不得伪造授权占用；不得把普通完成的原授权流水当作强制完成凭证；不得污染授权生命周期。 |
...
清结算与对账的 DSL 交付口径是“产品对象与资金事实边界已定义”，不是仅凭 DSL 就声明清结算和对账生产可用。当前已落地的对账批次、来源快照、逐笔匹配、运行结果、对象级 Gate、差错物化和重跑门禁仍是独立的对账证据与运营对象，不进入 route leg、posting plan 或 ledger phase。工程落地必须继续明确哪些经业务确认的资金事实允许进入 DSL，哪些对象只停留在产品或系分层。特别是可清分明细、清分批次、清算候选、对账任务、对账匹配、放行矩阵、差错等级、工作台动作和报表导出都不是资金路径。

| B7 DSL 门禁 | 必须明确 |
| --- | --- |
| 允许生成资金事实的动作 | 清算批次确认、结算锁定、出款成功、失败回退、退回、金额不一致差错、调账、冲正、补事实或追偿中哪些动作进入本批。 |
| 禁止生成资金事实的动作 | 对账排除、恢复、复核、补证据、导出、备注、人工关闭、报表发布或差异报告。 |
| 上层受控资金动作字段 | 命令或等价事件、来源单据、审批号、证据引用、幂等键、原事实引用、操作者、原因、可撤销边界和失败无副作用断言；动作准入由上层业务决定。 |
...
6. 对账模块不判断上层业务准入，也不根据差错自行生成 `FundsInstruction`。补事实、冲正、调账、挂账、追偿或核销由上层业务确认后调用既有资金能力；对账只追加动作类型、来源单据、审批号、证据引用、幂等键、原事实引用、操作者、原因和结果流水，并通过后继批次重新对账。
7. 使用者解释视图、审计导出、告警和 Runbook 信号只能只读引用资金事实、清结算对象、差错对象、审批和脱敏证据引用；不得因解释字段缺失而默认放行，也不得把敏感证据原文作为普通 DSL 字段传播。
8. `contentDigest` 只冻结规范化来源事实的内容身份，不证明外部验签、来源授权或原文真实性；生产宿主必须限制只有可信适配器能够写入来源快照。
9. `SPLIT_READY`、对象级 Gate `PASSED` 和出款 `PREFLIGHT_PASSED` 都不是资金执行授权。最终清分、清算、结算或出款命令必须在自身事务内重新核对当前 Gate、来源版本、退款/逆向事实和业务权威状态，失败时不得生成 route、posting 或 ledger entry。
...
| `DSL-BENEFIT-CONTRIBUTION-SETTLE-001` | 优惠让利出资记账最小契约。 | Consumer adapter 归一的 generic direct pay，包含成本承担主体、让利承接账务主体、目标账目、Money 与稳定业务 identity。 | JSON 可表达谁承担成本、让利结果落到哪个账务承接主体和账目、让了多少钱；成功后以 ActionFact 证明真实入账动作。 | 恢复 `FundsInstruction.benefitSnapshot`、把核心金额和责任塞入 `contextVariables`、补贴与本金净额混记、把用户余额入账或返利分润塞进 generic direct action。 |
...
| `DSL-BENEFIT-PLATFORM-SUBSIDY-001` | 平台补贴券补足商户。 | Consumer adapter 使用 generic direct pay，成本承担主体为平台营销资金账户或等价责任账户。 | 补贴形成独立权益资金交易或伴随资金事实；本金和补贴拆分。 | 补贴与本金净额混记、缺平台资金来源、缺规则版本。 |
...
| `DSL-BENEFIT-PREPAID-VOUCHER-001` | 储值、预付或礼品卡代金券。 | 独立负债、预收待付或用户权益余额口径，不复用当前优惠让利结算路径。 | 按独立能力处理；专业口径未确认前不进入资金流生产完成口径。 | 储值券按普通优惠券处理、缺负债口径仍借 generic direct pay 入账。 |
| `DSL-BENEFIT-MARKETING-ACCOUNT-001` | 营销/让利账户承接有资金影响的优惠让利出资。 | `costBearerAccountId`、`benefitReceiverAccountId`、`fundingNature`、账户 profile 和原订单或原交易引用。 | 平台、商户或合作方出资必须先解析到各自可入账账户；`benefitReceiverAccountId` 表达让利承接账务账户；非入账解释事实不得解析成分录。 | 把营销账户当支付工具、营销规则系统或券库存；用一个全局平台营销账户合并商户或合作方出资；把展示优惠和无真实入账商户让利默认入账；把用户余额入账或返利分润塞进本服务。 |
...
本节是让利出资记账交易 DSL 场景示例的权威入口。样例的资金语义继续有效，但旧 service/request 名只作历史载体：需要真实入账的让利出资由 Consumer adapter 归一为 generic direct pay/refund，并以 ActionFact 证明结果；非入账优惠不进入资金动作，`FundsInstruction.benefitSnapshot`、旧 `FundsBenefitSnapshotSpec` 及其组件、引用、退款策略对象不再作为 core DSL。
...
```json
{
  ...
  "targetTestClass": "CouponImplContractBoundaryTests",
  ...
  "benefitContributionSettleRequest": {
    ...
    "costBearerAccountId": { "type": "FUNDING_ACCOUNT", "id": "fa_platform_marketing_usd" },
    "benefitReceiverAccountId": { "type": "FUNDING_ACCOUNT", "id": "fa_merchant_clearing_usd" },
...
| 让利出资记账交易契约 | 无权益交易遵循空值语义；有资金影响的优惠让利出资必须由 Consumer adapter 归一为 generic direct action，并以 ActionFact 或等价不可变事实保存成本承担主体、让利承接账务主体、Money 和原事实。 |
