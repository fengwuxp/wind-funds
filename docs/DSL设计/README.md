...
DSL 不负责产品策略、外部协议、数据库实现、工程任务状态或生产准出。目标态权威入口是 [支付资金公共能力层-DSL设计.md](支付资金公共能力层-DSL设计.md)。

[支付资金底座DSL承载层设计.md](支付资金底座DSL承载层设计.md) 当前只作为现有 DSL、账务矩阵、API 草图和历史方案的迁移证据；其中内容需在 `W2-02` 后逐项迁移或删除，不得覆盖已通过 `G1` 的产品语义或新 DSL 稳定词汇。
...
本节以下对象、链路和不变量只描述当前实现与历史方案，供 [目标 DSL](支付资金公共能力层-DSL设计.md) 在 `W2-02` 做保留、内收或退役判断；它们不是新的目标态权威。
