...
| P2 业务补充 | VCC、全球账户、收单和 ACH/银行转账只传入归一业务事实、外部引用、状态映射和待确认规则；VCC 卡、prepaid virtual card、shared card 通过 `PaymentInstrumentRef`、绑定快照和 `FundingAllocationDecision` 表达，不新增 DSL 主体。 | 业务能力包通过准入卡接入统一资金内核，不改变 P0/P1 边界；资金账户、信用账户和预算组只按真实资金、授信额度和预算控制职责使用。 | TDD 覆盖业务状态映射、乱序重复、外部引用脱敏、规则待确认、P0/P1 回归、`TDD-RAIL-001A`、`TDD-P2-VCC-004` 至 `TDD-P2-VCC-011`、`TDD-WALLET-015` 至 `TDD-WALLET-017`。 | 业务轨道协议、外部账户、卡组织/银行原始规则、敏感原文沉入资金内核，或把卡工具、预付资金模式、共享卡绑定模式当成资金账户、信用账户或预算组。 |
