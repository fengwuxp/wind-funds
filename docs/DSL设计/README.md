...
| 金融边界是否留痕 | 涉及资质、法域、客户资金、备付金、跨境、外汇、敏感数据或外部规则时，是否有规则来源、确认方、确认状态和证据引用。 |
...
| 金融红线 | 规则来源、版本、适用法域、确认方、敏感信息脱敏和证据引用能在 DSL 中留痕或显式标为待确认。 | DSL 示例保存完整 PAN、CVV、密钥、token secret 或把外部规则写成无版本结论。 |
...
| 在 `tests/src/test/resources/dsl-contract-cases/` 落完整 JSON 夹具并被测试读取 | DSL 契约具备可执行验收入口。 | 已覆盖所有产品验收和生产路径，除非同步覆盖对应 AC/TDD/RED。 |
