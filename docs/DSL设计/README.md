...
[支付资金底座DSL承载层设计.md#51-资金场景借贷平衡与账务期望表](支付资金底座DSL承载层设计.md#51-资金场景借贷平衡与账务期望表) 是各场景 `PostingPlan`、`LedgerEntry` 和余额投影断言的权威输入。PRD 中的账务矩阵用于产品解释，系分和 TDD 必须回到 DSL 权威表承接实现和测试。

| 使用场景 | 必须使用该表回答 |
| --- | --- |
| 产品评审 | 该场景是同主体余额桶转换，还是跨主体价值转移。 |
| DSL 设计 | 借贷方向、账目、币种、账本周期、原事实引用和失败无副作用是否清楚。 |
| 系分设计 | `LedgerPostingAssembler`、账本写入、余额投影和清结算/治理边界如何落地。 |
| TDD 设计 | 每个资金变化测试应断言哪些 route、posting、entry、projection、幂等和 forbidden facts。 |
