...
> 归档说明：本文为早期顶层预研材料，稳定结论已被当前产品设计、DSL 设计、系分设计、TDD 设计和用户接入指南吸收。本文只作为历史背景，不作为当前权威设计、编码准入或生产交付依据。
...
5. 本文不替代法务、合规、财务、税务、审计、银行、ODFI/RDFI、卡组织、PSP、清算网络或持牌机构的最终确认。
...
| 钱包和账户 | 统一管理资金账户、信用账户、支出控制范围、平台账户角色、支付工具引用和外部账户引用。 |
...
| 不做 ACH 或银行转账产品 | ACH 指令、Debit 授权、账户验证、ODFI/RDFI 协议、Nacha 规则、SEC code、文件批次、cut-off、return code、NOC、reversal 和银行账户敏感信息不由资金内核承担。 |
...
ACH 不作为 `wind-funds` 的第四类内建业务，也不作为统一资金内核的一部分。它只能作为全球账户、收单、平台出款、企业付款、钱包充值或费用追偿等上层业务可能使用的外部银行转账轨道。

边界划分如下：

| 层级 | 负责内容 | 与 wind-funds 的关系 |
| --- | --- | --- |
| ACH 业务或银行转账产品 | 用户场景、收付款指令、Debit 授权、账户绑定、账户验证、通知、运营入口和业务单状态。 | 提供已经解释清楚的资金事实、业务引用和证据摘要。 |
| 通道或银行适配层 | Nacha/ODFI/RDFI 规则、SEC code、文件批次、entry、trace number、cut-off、return code、NOC、reversal、银行回单和敏感数据隔离。 | 提供外部状态映射、核验状态、脱敏引用和对账来源。 |
| wind-funds 资金底座 | 资金账户、账本账目、在途、可用、出款中、对账批次、差错单、调账核销、追偿、投影、归档和审计。 | 只消费归一后的资金事实，不解释 ACH 协议和 Nacha 规则。 |

因此，ACH return、NOC、reversal、IAT、Same Day ACH 和资金可用性不能直接落入统一交易状态机。只有当上层业务或适配层把它们解释成明确的资金影响、处理对象、外部引用和规则核验状态后，资金底座才可承接记账、对账、差错、追偿或投影。

产品章节入口见 [产品设计/ACH与银行转账资金底座支持边界.md](../../产品设计/ACH与银行转账资金底座支持边界.md)。该文档用于评审外部银行转账轨道支撑边界，不表示 `wind-funds` 新增 ACH 内建业务或 ACH 通道能力。
...
| 钱包是否统一 | 统一设计、统一实现 | 资金账户、信用账户、支出控制范围、余额桶、支付工具引用和主体余额查询可共享。 |
...
| 分类 | 能力 | 设计结论 |
| --- | --- | --- |
| 统一设计、统一实现 | 钱包、资金账户、信用账户、支出控制范围、账目 bucket、账本、余额投影、交易投影、清算、结算、对账、归档重放。 | 作为 `wind-funds` 核心能力，三类业务共享。 |
...
| 外部系统负责 | 发卡处理、商户入网、收银台、支付网关、银行开户、ACH 指令、Debit 授权、账户验证、通道协议、PCI token vault、KYC/KYB/AML 系统。 | 不进入资金内核；只通过明确结果、证据或状态引用进入资金链路。 |
...
| 外部事件与通道适配层 | 处理外部协议、回调、文件、签名、加密、原始报文、敏感数据隔离和状态映射。 | Authorization Message、Bank Statement、ACH File、ACH Entry、PSP Notification、Scheme File。 | 主要否，可独立包化。 |
...
| `wallet-impl` | 维护资金账户、信用账户、支出控制范围、平台账户角色、工具绑定和查询编排。 | 不表达外部银行账户开户、卡生命周期或商户入网。 |
...
| ACH 规则和银行转账状态如何解释 | 负责业务语义和用户告知 | 负责 return、NOC、reversal、cut-off 和外部状态解释 | 只承接归一资金事实、外部引用和核验状态 | ODFI/RDFI、合作银行、Nacha 规则和合规确认 |
...
| wind-funds 适合承接 | 资金账户、信用账户、支出控制范围、授权占用、授权撤销、授权完成、退款、拒付资金影响、账本、账目、投影、对账和归档。 |
...
| 支出控制范围 | 表达项目、团队或业务预算的控制账户。 | 预算不是资金池；不能表达真实现金沉淀。 |
...
| 从报表导出走向可解释数据面 | 用户账单、商户账单、运营时间线、财务核对视图和指标项输入都必须能回溯事实源；正式报表、看板和监管报送由外部报表指标或财务合规模块承接。 |
...
| 谁是可记账主体 | 资金账户、信用账户、支出控制范围或平台账户角色，不能是卡、VA、银行账户或通道单。 |
...
| `docs/归档/设计预研/资金底座支撑VCC全球账户收付款与收单业务评估.md` | 本文上收该评估中的定位结论，当前仅保留为历史背景。 |
| `docs/归档/设计预研/三类业务支撑能力地图与现状对比.md` | 本文上收该能力地图中的分层结论，当前仅保留为历史背景。 |
