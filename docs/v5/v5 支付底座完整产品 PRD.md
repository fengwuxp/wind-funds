...
| 对账可闭环 | 围绕账实一致、账账一致、单账一致和余额一致，核对平台交易、外部文件、银行流水、账本分录、余额投影、结算出款之间的差异并闭环处理。 |
...
| S-P0-008 | 对账差错 | 交易对账、资金到账对账、账务一致性核对、单据到账务核对、余额一致性核对、结算对账、差错单、挂账和核销。 |
...
对账层负责核对业务事实、资金交易、账本分录、余额投影、外部通道文件、银行流水、清结算批次和出款回单是否能互相解释。对账不是报表，也不是简单标记交易状态，而是面向财务、运营、风控、客服和商户运营的差异发现与处理闭环。对账发现差异后，只能生成差错、补单、冲正、调账或核销流程，不能直接修改历史分录或余额投影。完整概念、场景流程、能力地图和后台视角见 `docs/v5/产品设计/v5 对账差错产品设计.md`。

## 12.1 对账目标

对账核心围绕“单、账、钱、余额、状态”做全链路核对，目标不是生成一张报表，而是让每一类差异都能被识别、处理、追溯和核销。

| 核心目标 | 核对对象 | 一致性标准 | 差异处理方向 |
| --- | --- | --- | --- |
| 账实一致 | 内部资金交易、账本交易、余额投影与银行流水、支付通道清算文件、托管户流水、回单。 | 真实资金动作与内部资金事实在金额、币种、方向、账户、状态和日期口径上可互相解释。 | 外部单边、平台单边、长短款、手续费差、在途未达进入差错、挂账、补单、冲正或调账流程。 |
| 账账一致 | 业务账、支付账、账户账、商户账、总账、明细账、账本分录和余额投影。 | 不同账之间口径一致、金额一致、方向一致，明细可汇总到账户账、商户账和总账。 | 生成账务一致性差错，阻断相关自动清结算、出款或调账自动化。 |
| 单账一致 | 业务单、支付单、退款单、结算单、调账单、资金交易、账本交易和账本分录。 | 每个业务单据都能追溯到对应资金事实和账务流水，单据状态与账务状态不冲突。 | 漏单、漏账、重复单据、状态错配进入补单、补入账、冲正、人工复核或关闭流程。 |
| 余额一致 | 账户余额桶、期初余额、本期入账、本期出账、期末余额和余额投影。 | 满足“期初余额 + 本期入账 - 本期出账 = 期末余额”，并区分可用、冻结、在途、待结算、已结算等余额口径。 | 生成余额投影差错或巡检修复任务；未确认前不得直接改余额或释放风险资金。 |

## 12.2 对账设计要素

| 设计要素 | PRD 要求 | 典型字段或口径 |
| --- | --- | --- |
| 对账数据源 | 明确和谁对，至少覆盖业务系统、支付/交易系统、账务系统、通道文件、银行流水、财务凭证。 | `sourceType`、`sourceSystem`、`sourceFile`、`sourceVersion`、`dataDate`、`importBatchNo`。 |
| 对账口径 | 明确按什么口径对，同一批次必须固定交易日、账务日、清算日、结算日或入账日。 | 订单金额、实付金额、手续费、退款金额、分账金额、账本金额、外部到账金额。 |
| 匹配规则 | 明确怎么匹配，优先按强引用匹配，再按金额、币种、账户、时间窗口、交易类型和状态辅助匹配。 | 业务单号、资金交易号、账本交易号、通道单号、银行流水号、外部 reference。 |
| 差异分类 | 明确差异类型和责任边界，差异不能被一个笼统状态覆盖。 | 漏单、重复、金额不一致、状态不一致、手续费差、退款/冲正错配、长款、短款、在途未达。 |
| 差错处理 | 明确每类差错的默认处理动作、审批要求、影响范围和终态。 | 补单、挂账、调账、冲正、人工复核、核销、关闭、审批和审计。 |
| 批次与任务策略 | 每次对账必须有批次、数据版本、执行时间、重跑机制、处理人、状态、报表和告警。 | `reconcileBatchNo`、`ruleVersion`、`runMode`、`rerunOfBatchNo`、`operator`、`status`、`alertLevel`。 |
...
| 对账差错阻断阈值配置 | 按 12.8 的产品规则细化主体、币种、场景、差错类型、金额、笔数、SLA、阻断范围和审批放行。 | 财务、风控、运营、研发、测试 |
