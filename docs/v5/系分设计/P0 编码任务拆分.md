...
P0-A 已完成，P0-B 已关闭 RouteReplay 和 Fx 汇率方向两类残余，P0-C 已完成首轮入账红线，P0-R 已闭合 FX 决策外置、余额控制无 FX、余额变更日志观察和非 RT 结算策略红线，P0-CTRL 已把信用额度调增/调减、预算调增/调减、受控负 `AVAILABLE` 和 `LIMIT` 红线锁住。下一轮继续 P0-E/P0-G/P0-H。当前剩余风险集中在来源事实边界、冻结后独立资金事实、受控负余额执行策略、命名漂移、测试资产可维护性和 P1 清结算对象落地。
