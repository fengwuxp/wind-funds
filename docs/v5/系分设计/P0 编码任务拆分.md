...
| P0-E | Wallets 账户与余额控制 | 下一批优先。完成 wallet 服务归位、账户 profile、平台角色、冻结订单、余额桶、受控负余额和缺账本语义。 |
...
| P0-R | 产品口径回归闸口 | 下一轮优先。把已对齐的 PRD/TDD 口径转成代码前置红线：FX 决策外置、余额控制无 FX、余额变更日志观察、非 RT 结算策略契约。 |
| P0-G | 命名治理 | 已完成首轮。账本入账结果和生命周期写侧端口已完成兼容命名收敛，剩余项继续分批推进。 |
...
| P0-E-004 | 账户 profile 补齐。 | `LedgerProfileCode`、`DefaultLedgerProfileServiceImpl`。 | 商户资金账户、普通资金账户、信用账户、预算组账目集合符合 PRD。 |
...
| P0-G-003 | 账户类型枚举拆分。 | 将混合账户类型、外部账户、平台角色和账户用途的枚举拆成独立概念。 | `DefaultFundsAccountType` 不再承担多个语义轴。 |
...
```bash
mvn compile
mvn -pl tests -am test -Dtest=WalletLayerBoundaryTests,FundsTransactionServiceApiContractTests
```
...
P0-A 已完成，P0-B 已关闭 RouteReplay 和 Fx 汇率方向两类残余，P0-C 已完成首轮入账红线，P0-R 已闭合 FX 决策外置、余额控制无 FX、余额变更日志观察和非 RT 结算策略红线。下一轮先执行 P0-CTRL 控制账户调额，把信用额度调增/调减、预算调增/调减、受控负 `AVAILABLE` 和 `LIMIT` 红线锁住，再继续 P0-E/P0-G/P0-H。当前剩余风险集中在来源事实边界、冻结后独立资金事实、受控负余额执行策略、命名漂移、控制账户调额校验、测试资产可维护性和 P1 清结算对象落地。
