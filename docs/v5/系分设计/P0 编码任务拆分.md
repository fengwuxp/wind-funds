...
2026-05-17 CAD 测试资产治理四十八轮状态：继续拆分业务组合集成测试，将资金账户调账、信用账户额度调整和预算组预算调整组合迁入 `FundsBalanceControlBusinessFlowTests`；`FundsTransactionBusinessFlowIntegrationTests` 聚焦充值-付款-退款、充值-冻结-提现、多次解冻和转账付款提现等直接交易组合链路。P0-H 仍保留打开状态，后续继续复核余额断言覆盖和其他超大测试类职责拆分。

2026-05-17 CAD 测试资产治理四十九轮状态：继续拆分账本入账服务测试，将 posting plan 内多币种和账本交易金额币种不一致红线迁入 `DefaultLedgerTransactionPostingCurrencyValidationTests`；`DefaultLedgerTransactionPostingServiceImplTests` 聚焦成功入账、幂等、投影服务唯一路由和单 plan 平衡校验。P0-H 仍保留打开状态，后续继续复核余额断言覆盖和其他超大测试类职责拆分。

2026-05-17 CAD 测试资产治理五十轮状态：继续拆分交易编排回放测试，将 `REPLAY_ONCE` 重复消费和缺失原 route snapshot 两条回放策略失败红线迁入 `DefaultRoutedFundsInstructionOrchestratorReplayPolicyTests`；`DefaultRoutedFundsInstructionOrchestratorReplayTests` 聚焦普通退款、授权退款、拒付和解冻的正向回放编排。P0-H 仍保留打开状态，后续继续复核余额断言覆盖和其他超大测试类职责拆分。
...
| P0-H | 测试资产治理 | 当前版本已完成。三类服务测试矩阵、业务组合集成测试、测试包名对齐、余额断言复核和超大测试类拆分均已闭合；后续新增资金变化测试仍按本节标准维护。 |
...
P0-H-005 追加记录：交易编排回放测试已继续按策略红线拆分，`REPLAY_ONCE` 重复消费和缺失原 route snapshot 迁入 `DefaultRoutedFundsInstructionOrchestratorReplayPolicyTests`，主 replay 测试类聚焦正向回放路径。
...
P0-H-005 追加记录：业务组合集成测试已继续按交易/控制语义拆分，资金账户调账、信用账户额度调整和预算组预算调整迁入 `FundsBalanceControlBusinessFlowTests`，主业务流测试聚焦直接交易组合链路。

P0-H-005 追加记录：账本入账服务测试已继续按币种红线拆分，posting plan 内多币种和交易级币种不一致迁入 `DefaultLedgerTransactionPostingCurrencyValidationTests`，主测试类聚焦入账主链路、幂等和投影服务路由。

建议验证命令：

```bash
mvn -pl tests -am test -Dtest=FundsTransactionBusinessFlowIntegrationTests,FundsBalanceControlBusinessFlowTests,FundsTransactionFeeBusinessFlowTests,FundsTransactionLedgerBalanceAssertionsTests
mvn -pl tests -am test -Dtest=DefaultLedgerTransactionPostingServiceImplTests,DefaultLedgerTransactionPostingCurrencyValidationTests
mvn -pl tests -am test -Dtest=DefaultFundsInstructionLifecycleSaverTests,DefaultFundsFrozenOrderLifecycleSaverTests
```

## 4.9 P0-ROUTE：路由层 CR 与场景覆盖

路由层下一轮 CAD 只处理 `RouteResolver`、route replay、route leg 构建和 route 层测试资产，不引入清结算、对账、归档、余额快照或完整 FX 运营对象。路由层职责边界保持为“解析资金路径”，不得直接写交易事实、冻结事实、账本事实或余额投影。

本轮 CR 设计结论：

1. 路由回放仍然需要概念和流程，但不设计新的 `RouteReplayService` face 契约；回放作为一种 `RouteResolver` 场景，由 `supports(FundsInstructionSpec)` 命中带原事实引用的退款、撤销、授权结算、授权退款、拒付、手续费退回和解冻。
2. `DefaultRouteReplayService` 当前承担 replay resolver 的内部实现职责，后续可以继续保留为 impl 内部协作类，但需要治理深层调用、过长参数和 route leg 构造分散问题。
3. `AuthorizationFundsInstructionRouteResolver` 中不带原事实引用的 reversal、settle、auth refund 分支需要先用测试钉住目标口径。默认目标口径是：授权后续事件必须带 `reference` 并走 replay resolver；缺少引用不得 fallback 为普通授权 route。若后续确需兼容历史无引用请求，必须单独标成兼容分支并补兼容测试。
4. 直接交易、余额控制和授权 resolver 中重复的 route builder、校验、主体节点构建和 constraint key 逻辑需要抽取到 impl 内部小对象或 support，不改变 face 契约。
5. 稳定 route code、snapshot schema version、context key 和 replay policy 必须集中在现有常量或领域枚举中；测试里的业务流水、错误消息和场景描述可以保留直写。

| 编号 | 任务 | 修改方向 | 验收 |
| --- | --- | --- | --- |
| P0-ROUTE-001 | `CompositeRouteResolver` 选择语义补测。 | 补齐单命中、多命中优先级、无匹配、`Ordered` 排序、自身排除和 replay resolver 优先级测试。 | resolver 选择稳定，不因注册顺序漂移；无匹配时错误可解释。 |
| P0-ROUTE-002 | Route replay 支持矩阵补测。 | 覆盖 direct refund、fee refund、authorization reversal、authorization settlement、authorization refund、chargeback、unfreeze、缺 reference、缺原 route snapshot、空 legs、部分 legId 缺失和 `EXTERNAL_TRANSACTION` 不回放。 | replay 只依赖原 route snapshot，不重新普通选路；`REPLAY_ONCE` 和原事实缺失失败可回归。 |
| P0-ROUTE-003 | 授权后续事件无引用分支收敛。 | 先补失败用例确认 reversal、settle、auth refund 缺 `reference` 的行为，再删除或收窄非 replay 分支。 | 授权后续事件不能绕开原授权路径；共享卡、预算组和多主体授权路径仍按原快照释放、结算或退款。 |
| P0-ROUTE-004 | route leg 构建重复治理。 | 为 replay service 和三类 resolver 提取 route leg 构建上下文、constraint key 和主体节点构建 support。 | 公共逻辑内聚，私有方法参数明显减少；route snapshot 结构和现有测试语义不变。 |
| P0-ROUTE-005 | 过时类、方法和常量清理。 | 清理仅服务旧测试夹具或已被 replay resolver 替代的常量、私有方法和兼容入口。 | 删除前有引用扫描和聚焦测试保护；不删除仍有业务语义的兼容契约。 |
| P0-ROUTE-006 | route 层架构边界回归。 | 补充或强化 route 层源码边界测试，禁止 route 层依赖 lifecycle saver、ledger posting、DAL mapper、transaction application command service 等写事实实现。 | route 层只产生 `FundsRoutePlan` / route snapshot，不写事实、不写账本、不改余额。 |

场景覆盖矩阵：

| 能力 | 必覆盖场景 | 目标测试资产 |
| --- | --- | --- |
| Composite 选择 | 单 resolver、多个 resolver、排序、无匹配、自身排除、replay 优先级。 | `CompositeRouteResolverTests` |
| Direct route | topup、withdraw、pay、transfer、fee、fee refund、错币种平台账户、缺平台账户、无原交易引用退款边界。 | `TransferFundsInstructionRouteResolverTests`、`TransferFundsInstructionFeeRouteResolverTests`、route replay 测试 |
| Balance route | freeze、unfreeze、资金账户余额调账、受控负余额、信用额度调额、预算组预算调额、unsupported subject/event。 | `BalanceControlFundsInstructionRouteResolverTests`、`BalanceControlFundingAdjustRouteResolverTests`、`BalanceControlLimitAdjustRouteResolverTests` |
| Authorization route | authorize approve/decline、共享卡授权、授权撤销、授权结算、授权退款、拒付、多主体原路回放。 | `AuthorizationFundsInstructionRouteResolverTests`、`AuthorizationSharedCardFundsInstructionRouteResolverTests`、route replay 测试 |
| Replay policy | 缺引用、缺原快照、空快照、部分 legId 缺失、重复消费、外部交易不回放。 | `DefaultRouteReplayServiceTests`、`DefaultRouteReplayAuthorizationTests`、`DefaultRouteReplayDirectRefundTests`、`DefaultRouteReplayPolicyTests` |
| 架构边界 | route 不写交易事实、不写冻结事实、不写账本事实、不反向依赖应用服务或 DAL。 | `RouteLayerBoundaryTests` |

建议验证命令：

```bash
mvn -pl tests -am test -Dtest=CompositeRouteResolverTests,TransferFundsInstructionRouteResolverTests,TransferFundsInstructionFeeRouteResolverTests,BalanceControlFundsInstructionRouteResolverTests,BalanceControlFundingAdjustRouteResolverTests,BalanceControlLimitAdjustRouteResolverTests,AuthorizationFundsInstructionRouteResolverTests,AuthorizationSharedCardFundsInstructionRouteResolverTests,DefaultRouteReplayServiceTests,DefaultRouteReplayAuthorizationTests,DefaultRouteReplayDirectRefundTests,DefaultRouteReplayPolicyTests,RouteLayerBoundaryTests
mvn compile
```

P0-ROUTE-001 追加记录：`CompositeRouteResolverTests` 已补齐无匹配失败、自身排除、按 `Ordered` 顺序评估 `supports`、以及 replay resolver 与普通 resolver 通过 `supports` 互斥保证优先命中的测试。多 resolver 同时命中仍保持失败红线，不按 order 静默选择，以避免掩盖路由歧义。

P0-ROUTE-002 追加记录：已新增 `RouteReplaySupportTests` 钉住 replay 指令识别矩阵：`REVERSAL`、`SETTLE`、`AUTH_REFUND`、`CHARGEBACK`、`REFUND`、`FEE_REFUND`、`UNFREEZE` 携带内部原事实引用时命中 replay；缺 `reference`、空白 `referenceSn`、`EXTERNAL_TRANSACTION` 引用和非 replay 事件均不命中 replay。

P0-ROUTE-002 完成记录：`DefaultRouteReplayServiceTests` 已补齐 `resolve(instruction)` 入口对 direct refund、fee refund、authorization reversal、authorization settlement、authorization refund、chargeback、unfreeze 的原快照回放覆盖，并钉住缺原 `RouteSnapshot` 失败；`DefaultRouteReplayPolicyTests` 已补齐空 legs 与部分指定 `replayLegIds` 缺失失败路径。

P0-ROUTE-003 追加记录：`AuthorizationFundsInstructionRouteResolver#supports` 已收窄为仅支持普通 `AUTHORIZE`；`REVERSAL`、`SETTLE`、`AUTH_REFUND` 缺 `reference` 时组合路由不得 fallback 到普通授权路径，带 `AUTHORIZATION` 引用时由 replay resolver 基于原快照处理。

P0-ROUTE 场景覆盖完成记录：direct route 已补缺平台 `PREPAYMENT` 账户时拒绝生成提现 route snapshot；平台账户错币种防线在 `PlatformFundingAccountServiceImplTests` 钉住，服务即使返回与查询币种不一致的配置也必须失败。余额控制受控负余额、信用额度调额、预算组预算调额已由 `BalanceControlFundingAdjustRouteResolverTests` 和 `BalanceControlLimitAdjustRouteResolverTests` 覆盖；共享卡/多主体授权已由 `AuthorizationSharedCardFundsInstructionRouteResolverTests` 覆盖。由于 `FundsAccountId` 不携带币种，错币种平台账户属于平台账户解析服务边界，不在 route resolver 内重复表达。

P0-ROUTE-004 完成记录：新增 `RouteSpecSupport`，把普通 direct / balance-control / authorization resolver 中重复的 route leg 构建、source/target node 构建和 constraint key 构建收敛到统一 support；三类 resolver 只保留业务分支、平台账户选择、参与方构造和 route 汇总校验。`DefaultRouteReplayService` 的节点复制和双粒度约束 key 保留在 replay 内部，避免把回放特有语义混入普通 route 构建。

P0-ROUTE-005/006 完成记录：普通 resolver 中仅服务旧私有构造逻辑的常量、方法和 import 已清理；`rg` 扫描确认 route 包不再存在普通 resolver 私有 `routeLeg/sourceNode/targetNode/routeNode` 构建方法。route 层边界继续由 `RouteLayerBoundaryTests` 保护，不依赖交易事实写入、冻结事实写入、账本写入端口或 DAL mapper。
...
P0 代码已完成“测试保护、Transaction 门面、helper/route 主链路收敛、Ledger Posting 首轮红线、P0-R 产品口径回归、P0-CTRL 控制账户调额、P0-E 钱包账户归位、P0-G 命名治理、P0-H 测试资产治理和 P0-ROUTE 路由层 CR”等主体任务。

P0-H 与 P0-ROUTE 均已完成当前版本目标：三类服务测试矩阵、业务组合集成测试、测试包名治理、路由层场景 100% 覆盖、replay 语义收敛和过时入口清理均已落地。清分、清算、对账、账本账目归档、余额快照、完整 FX 运营对象和相关运营工作台全部排入下版本。
