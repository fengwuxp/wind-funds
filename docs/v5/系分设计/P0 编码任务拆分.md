...
| P0-H-005 | 巨型测试类拆分。 | 已先拆分账本入账测试：共享 fixture 迁入 `LedgerTransactionPostingTestSupport`，入账前置校验迁入 `DefaultLedgerTransactionPostingValidationTests`；后续继续处理其他超大测试类。 | 单个测试类聚焦一个能力，setup 不再承载过多隐式状态。 |
