...
2026-05-17 CAD 命名治理二次状态：P0-G 已完成账户类型语义轴回写。`DefaultFundsAccountType` 继续作为迁移期兼容枚举，`ExternalFundsAccountType`、`UserWalletFundsAccountType`、`CreditFundsAccountType`、`FundsSubjectType`、`PlatformFundingAccountRole` 分别承接外部账户、用户钱包、信用/VCC、账务主体和平台账户角色语义；`DefaultFundsAccountTypeTests` 与 `PlatformFundingAccountRoleTests` 保护兼容判断、角色命名和边界。包名统一属于独立重构轮次，不在当前 P0-G 中夹带大迁移。
...
| P0-G-003 | 账户类型枚举拆分。 | 已通过 `ExternalFundsAccountType`、`UserWalletFundsAccountType`、`CreditFundsAccountType`、`FundsSubjectType`、`PlatformFundingAccountRole` 拆分外部账户、用户钱包、信用/VCC、账务主体和平台账户角色语义；`DefaultFundsAccountType` 仅作为迁移期兼容枚举保留。 | `DefaultFundsAccountTypeTests` 和 `PlatformFundingAccountRoleTests` 保护语义轴拆分、旧判断入口兼容和平台角色边界。 |
...
| P0-H-005 | 巨型测试类拆分。 | 将交易生命周期、冻结生命周期、冻结引用回放、余额断言等职责从超大测试类中分离。 | 单个测试类聚焦一个能力，setup 不再承载过多隐式状态。 |
