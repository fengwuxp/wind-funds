...
- 当前版本：v1.0
- 文档状态：Review
- owner：收单产品 owner、资金产品 owner、商户运营与财务 owner、风控 owner、测试 owner、架构 owner
- 发布日期：2026-07-17
- 评审结论摘要：本文定义收单支付、清分、清算、结算、出款、退款、拒付和商户账单如何接入统一资金底座。
- 目标读者：收单产品、商户运营、财务、风控、合规、测试、架构师和研发。
- 需要决策：确认收单资金事件、商户责任、逆向处理、清结算和对账的产品边界与验收。
- 当前可信度：内部资金事实和统一账务承接方式已确认；卡组织、收单资质、商户资金、争议运营和生产规则仍待专业确认。

## PRD 模板追踪索引

| PRD 模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 需求概览、背景、目标与范围 | 1、3 | 不实现完整收单受理和卡组织协议。 |
| 核心名相、主体、角色和对象 | 2、5 | 区分商户、支付尝试、清结算对象和资金主体。 |
| 场景、流程、状态和规则 | 6、7 | 覆盖支付、退款、拒付、清结算和异常。 |
| 数据、运营、风险和外部规则 | 8、9 | 保留商户运营、对账、争议和专业确认。 |
| 验收与产品到架构交接 | 10、11 | 业务专项只补场景语义，不新增平行资金内核。 |
...
| --- | --- | --- | --- |
| 收单业务 | 为商户接收消费者付款，并完成资金确认、清分、清算、结算、退款和争议处理的业务集合。 | 不是单一支付成功状态，也不是资金底座内建的商户产品。 | 收单产品系统负责业务对象；资金底座承接已解释的资金事实。 |
| Payment Attempt / Capture | 上层支付尝试及其确认扣款事实，用于关联后续资金动作。 | 不是内部账本状态机，也不等同商户可结算。 | 支付网关或收单产品负责生命周期；资金底座保存业务引用。 |
| 商户资金账户 | 承载商户待清算、可结算、结算锁定、准备金和差错等余额的内部账务主体。 | 不是商户号、通道商户号或外部银行结算账户。 | `wind-funds` 钱包与账本能力负责。 |
| 清分 / 清算 / 结算 | 清分确定归属与金额，清算确认应收应付，结算把已确认结果转为可出款责任。 | 不是三个同义状态，也不由支付成功直接跳转。 | 清结算能力承接；上层业务和外部机构提供规则与结果。 |
| 商户账单 | 基于支付、退款、费用、清结算、出款、争议和账本事实形成的只读解释视图。 | 不是记账依据，也不是对账差错的修改入口。 | 收单产品负责展示；资金底座提供可追溯投影和证据引用。 |
...
本表中的 P0/P1 只表示收单分册内部能力优先级，不改变 01 总览中“收单业务属于全局 P2 业务补充分册”的定位。任何收单能力进入工程落地前，都必须先证明复用统一钱包、账本、账目、清结算、对账和归档能力，并声明独立任务边界。
...
| 02/03/05 资金数据治理拆分承接 | 02 补充商户账单投影重放；03 补充结算单、对账差错和争议追偿的批次视图重放；05 补充治理门禁和指标只读边界。 |
