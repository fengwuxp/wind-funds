...
- 当前版本：v1.0
- 文档状态：Review
- owner：资金产品 owner、银行轨道产品 owner、合规 owner、财务 owner、测试 owner、架构 owner
- 发布日期：2026-07-17
- 评审结论摘要：本文是 ACH 与银行转账的产品能力边界，不是完整 ACH 业务 PRD；资金底座只承接归一后的资金事实和证据引用。
- 目标读者：产品、银行通道、运营、财务、合规、测试、架构师和研发。
- 需要决策：确认哪些银行轨道事实可以进入资金底座，哪些协议、授权、文件和运营职责必须留在上层或适配层。
- 当前可信度：资金底座承接边界已确认；Nacha、银行、法域、资质和生产事件规则仍待外部核验。

## 产品能力边界模板追踪索引

| 模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 目标、范围和非目标 | 1、2、3 | 边界文档不展开完整业务产品。 |
| 能力地图和对象状态 | 3、4、5 | 只映射资金底座可承接能力。 |
| 主流程、异常流程和规则矩阵 | 6、6.1、6.2、9 | 外部协议细节保持非目标。 |
| 风险、外部规则和待确认 | 7、10 | 不把行业规则写成已确认实现。 |
| 验收与工程交接 | 8、9 | 回挂稳定验收 ID 和 TDD 资产。 |
...
| 能力域 | 上层或适配层能力 | 资金底座可复用能力 | 边界结论 |
| --- | --- | --- | --- |
| 指令与授权 | 收付款指令、Debit 授权、账户验证、用户通知和业务状态。 | 接收已解释业务事实、业务流水和幂等信息。 | 不在资金底座判断授权有效性或维护指令状态机。 |
| 外部轨道处理 | 文件生成提交、批次窗口、return/NOC/reversal 解释和敏感数据隔离。 | 保存最小外部引用、摘要与核验状态。 | 不在资金底座内建 ACH 协议或规则引擎。 |
| 账户与账本 | 业务决定资金可用性、责任主体和处理动作。 | 资金账户、账目、路由、账本分录和余额投影。 | 只有归一资金事实才能驱动账务。 |
| 清结算与对账 | 提供银行流水、回单、文件摘要及外部状态。 | 出款、清结算、对账批次、差错、追偿和调账核销。 | 外部 accepted 不等于最终到账。 |
| 运营与审计 | 业务解释、人工复核、专业规则确认和审批。 | 只读证据引用、处理结果、幂等轨迹和归档。 | 资金底座不替代业务决策或合规判断。 |
...
| 规则 ID | 业务表达 | 触发条件 | 资金底座处理 | 验证证据 |
| --- | --- | --- | --- | --- |
| ACH-R-001 | 只有已解释的归一资金事实才能进入资金底座。 | 上层提交 ACH 或银行转账结果。 | 缺少责任主体、金额、币种、动作或业务引用时拒绝处理，不生成账务事实。 | 请求契约校验、无 ledger entry 断言。 |
| ACH-R-002 | submitted、accepted、processing 均不是最终资金完成事实。 | 仅收到外部受理或处理中状态。 | 保持上层状态或显式在途事实，不提前形成 AVAILABLE 或最终完成。 | 状态时间线、余额桶断言、外部证据引用。 |
| ACH-R-003 | return、NOC 和 reversal 必须先由上层解释。 | 收到对应外部事件。 | 只接受上层决定后的追偿、差错、调账核销或无资金影响处理。 | 原事实引用、处理动作、责任方和幂等测试。 |
| ACH-R-004 | 银行账户和规则证据遵循最小化保存。 | 写入引用、日志、投影、归档或测试数据。 | 只保存脱敏引用、token、摘要和核验状态，拒绝完整敏感信息。 | 字段扫描、日志扫描、安全边界测试。 |
| ACH-R-005 | 外部规则缺少版本、适用范围或确认方时不得自动生产处理。 | 处理依赖 Nacha、银行、司法辖区或时效规则。 | 阻断、降级为 contract-only 或转人工；资金底座不自行推断。 | 规则核验状态、审批记录、人工任务引用。 |
