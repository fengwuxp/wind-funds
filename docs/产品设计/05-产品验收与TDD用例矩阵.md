...
拒付/争议准入口径：授权后的争议、拒付或扣回必须可解释、可查询、可投影、可审计。目标态通过 `settleRefund / AUTH_REFUND` 的争议字段、外部引用、凭证、审计上下文和内部 `DISPUTE` 标签区分普通授权链退款、`NO_AUTH` 退款和授权拒绝；既有独立 `chargeback` 入口只作为兼容、显式事件或内部适配资产，不作为目标态主入口。完整争议运营、清结算追偿、外部卡组织规则或旧 `chargeback` 公共入口物理移除需要独立工程任务承接。
