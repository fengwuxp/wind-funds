...
| 清算批次确认 | 只有清算确认才允许 `CLEARING -> AVAILABLE`。 | 重大差错、账期未到、退款/争议/风控阻断、重复入批或候选未锁定必须阻断；批次原子确认，不允许部分成功。 | `AC-REC-008`、`TDD-CLS-004`、`TDD-CLS-006`。 | 已落地清算批次原子锁定、事务内最终复核、受控 `FundsClearingTransactionService`、账务引用回写和候选 `CLEARED/BLOCKED/LOCKED` 分流；争议/风控权威查询、按原资金分配处理部分退款、生产 MySQL 并发与迁移、监控和 Runbook 尚未验收，整体生产准出仍为 `NO-GO`。 |
...
| CLS-GATE-003 | 数据与服务闭环 | 对账批次、可清分明细、清分批次、清算候选、清算批次、结算单、出款单、差错单、追偿单和审计对象的表、状态机、幂等键、DTO、Entity、Mapper、服务和 H2 测试范围已确认。当前仓库已对对账批次/结果/差错/Gate、可清分明细、单主体清分批次、不可变标量金额快照、清算候选和固定清算资金原语形成局部证据；清算批次编排、最终准入复核和候选完成推进仍缺，不能外推为整域数据库级或服务级闭环。 | 不得声明数据库级或服务级完成。 |
