...
| AC-PI-010 | 授权支付工具应用入口 | VCC、共享卡、VA、钱包标识或卡 token 发起授权，调用方只持有支付工具引用和业务上下文。 | 应用层完成工具状态、绑定版本、使用主体、预算组、Spend Rule、资金责任和账户能力校验；批准后委派账户主体型授权内核，拒绝时只返回拒绝事实和原因。 | 支付工具入口不等于账本主体；不得要求外部调用方绕过钱包 / 路由直接选择内部账户，也不得让工具准入失败生成 route、posting 或 LedgerEntry。 |
...
| 验收 ID | 场景 | 输入 | 期望输出 | 核心断言 |
| --- | --- | --- | --- | --- |
| AC-AUTH-000 | 授权入口分层 | 授权由支付工具、VCC、共享卡或钱包标识触发，或由可信内部系统直接传入已解析账户主体。 | 支付工具型入口先解析并固化工具、绑定、规则和资金责任快照；账户主体型入口直接进入授权内核。 | 两类入口不得混用责任：外层不得写账，内核不得按当前工具绑定重算历史路径；新增支付工具型入口必须有独立 Execution Grant。 |
