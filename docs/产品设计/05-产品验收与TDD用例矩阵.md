...
| CLS-GATE-003 | 数据与服务闭环 | 当前声明范围内的对账批次、可清分明细、清分批次、清算候选、清算批次和差错对象已完成表、状态机、幂等键、DTO、Entity、Mapper、服务与 H2 验收，并覆盖批次原子编排、最终准入复核和候选完成推进。结算单、出款单和追偿单不在当前能力范围；操作审计由 Web/宿主承接。 | 未提供能力不得由当前 H2 结果外推。 |
