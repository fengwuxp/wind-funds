...
| `AC-ARCH-*`、`AC-REPLAY-*` | `Replay DSL` 中的投影重放边界、归档 Manifest、Checkpoint、Watermark | `TDD-ARCH-*`、`TDD-REPLAY-*`、`TDD-RED-016` 至 `TDD-RED-019` | 归档只改变冷热位置；余额重建只从检查点和增量分录出发；交易投影重放有范围、dry-run/apply 和差异报告。 |
| `AC-RPT-*` | 指标项来源、口径版本和只读边界 | `TDD-METRIC-*`、`TDD-RED-019` | 资金底座只列产品和业务关心的指标项；指标实现由报表指标模块承接，不反写资金事实。 |
...
| 报表指标模块 | `AC-RPT-*`、`RED-029` | 指标只读、来源可解释、异常不反写资金事实。 |
