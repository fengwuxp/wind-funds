...
- 当前版本：v1.0
- 文档状态：Review
- owner：产品验收 owner、测试 owner、架构 owner、运营与财务确认 owner
- 发布日期：2026-07-17
- 评审结论摘要：本文是产品验收与跨层追踪资产，不是独立 PRD 或产品评审报告；它把已确认产品事实映射为验收种子、红线和 TDD 输入。
- 目标读者：产品、业务、运营、财务、风控、合规、测试、架构师和研发。
- 需要决策：确认验收种子、能力状态、阻断项和工程准入证据是否足够。
- 当前可信度：已有测试资产和未完成交付分开记录；测试数量不替代产品、财务、合规或生产确认。

## 产品验收模板追踪索引

| 模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 目标、范围和验收角色 | 1、2 | 验收矩阵不重复各 PRD 的用户故事。 |
| 场景、对象和证据链 | 3、4、5 | 以验收对象和跨层证据表达。 |
| 规则、异常和红线 | 5、6 | 正向、逆向、异常、权限和资金红线统一编号。 |
| 数据、运营和专业确认 | 5.5-5.9、9-11 | 覆盖报表、运营、外部规则和可解释性。 |
| TDD 与工程交接 | 7、8、9 | 每个进入工程的验收项必须映射验证资产。 |
...
所有验收项先按账务三问收敛，再下钻到 DSL、系分和 TDD：谁的钱、多少钱、怎么变的。无法回答三问的场景，不进入“通过”；只能带条件通过、阻断或转人工确认。

| 三问 | 验收最低证据 |
| --- | --- |
| 谁的钱 | 主体、账户类型、资金责任、平台角色解析和不可入账引用边界清楚。 |
| 多少钱 | 金额、币种、金额组件、余额桶、账本周期和累计处理上限清楚。 |
| 怎么变的 | 业务事实、状态变化、route snapshot、posting plan、ledger entry、投影、幂等和审计清楚。 |
...
2. 为工程任务提供可执行的准入判断，区分设计覆盖、契约承载、服务级落地、数据库级落地和生产完成证据。
...
1. 本矩阵不替代 PRD 正文、DSL 设计、系分详细设计、工程任务边界或测试代码。
2. 本矩阵不把目标测试资产视为已完成测试证据；生产交付仍以代码、DDL/H2、服务级测试、验证命令和审批记录为准。
3. 本矩阵不替代法务、合规、财务、税务、通道、银行、卡组织或持牌机构对外部规则和资金口径的最终确认。

## 1.1 准入评估结论口径

本矩阵是产品、开发和测试做准入评估时的共同证据表。评审结论必须回到 [../README.md#生产准入检查清单](../README.md#生产准入检查清单) 和 [../README.md#阻断和降级口径](../README.md#阻断和降级口径) 的三类口径：通过、带条件通过、阻断。

| 结论 | 本矩阵中的最低证据 | 不允许的误判 |
| --- | --- | --- |
| 通过 | 工程任务范围内的 AC-*、RED-*、DSL 契约、系分落点和 TDD 用例都能互相追溯，且没有未关闭的资金安全或金融红线阻断项。 | 只因为主流程能跑通就判定通过。 |
| 带条件通过 | P0/P1/P2 范围内的 AC-* 有 TDD 承接，待确认项不影响资金不变量，并写清责任方、关闭条件和不覆盖范围。 | 把外部规则、合规确认、争议证据、调账审批或敏感数据边界当成普通建议项。 |
| 阻断 | 存在未覆盖红线、资金路径不可追溯、账务主体不清、投影反写、缺快照逆向、周期混用、敏感信息泄露或外部规则未确认等关键问题。 | 用人工处理、后台改数、报表修正或测试暂缓来替代设计闭环。 |

### 1.2 能力保障优先级和验收顺序

验收顺序按资金底座能力优先级判断，不按 PRD 分册编号或 TDD 编号判断。产品验收必须先证明 P0 资金内核，再证明 P1 交易与读模型，最后按业务模式启用 P2 能力包。
...
| P2 VCC、全球账户和收单业务支持 | 只作为业务补充分册和专项准入输入；不进入默认工程范围。 | 业务专项 PRD、轨道/外部规则确认清单、P0/P1 回归范围、敏感数据边界和专项 TDD 入口分析。 | 三类业务验收 ID、外部规则来源和确认状态、资金底座归一事实、支付工具脱敏、账户层级策略、`B2-ACCOUNT-HIERARCHY` 或等价准入卡、P0/P1 回归清单和未覆盖范围。 | 声明资金底座已经实现卡组织、银行、PSP、跨境、商户经营、风控模型或业务系统完整能力；在账户层级未确认前声明 VCC prepaid/shared card 生产可用。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | 钱包账户和支付工具 | 资金账户、信用账户、VCC 关联资金/信用子账户、支出控制范围、Spend Rule、平台资金账户角色解析、支付工具状态、绑定和资金责任解析。 | `SubjectRef`、`PaymentInstrumentRef`、`AccountHierarchyRelation`、参与方 `AccountHierarchySnapshot`、`RouteParticipant`、`RouteLeg`、Spend Rule 快照。 | 02 分册钱包账户、支付工具、账户层级、资金责任解析关系、支出控制对象和 route 准入。 | `TDD-WALLET-*`、`TDD-ROUTE-006`、`TDD-ROUTE-011` 至 `TDD-ROUTE-013`。 | 支付工具、外部账户、卡号、PAN、token、VA、支出控制范围、Spend Rule 或 `VCC_ACCOUNT` 被设计成 ledger subject，账户层级隐式生成资金路径，或工具换绑导致历史回放漂移。 |
...
| P0 | 清结算与对账 | 清分、清算、结算、出款前准入、外部非终态、出款解释状态、对账差错、调账核销和追偿闭环。 | `SettlementPolicySpec`、`DSL-SETTLEMENT-*`、出款准入、出款结果、差错和调账引用。 | 03 分册可清分明细、批次、候选、结算单、出款单、对账批次、差错单和出款处理台。 | `TDD-CLS-*`、`TDD-SETTLE-*`、`TDD-SETTLE-004`、`TDD-SETTLE-005`、`TDD-RECON-*`、`TDD-OPS-004` 至 `TDD-OPS-006`；宿主操作审计使用 `HOST-OPS-*`。 | 清分候选直接入账、缺前置对账仍清算、出款准入未知仍放行、外部受理展示为成功、差错直接改历史分录。 |
...
| P1 | 权益语义 | 让利出资记账交易、成本承担主体、让利承接账务主体、零实付准入、退款处置和原事实回放。 | Consumer Benefit adapter、generic direct pay/refund、`FundsActionFact`、Capte-owned `fundingNature`、`fixtureLevel` 和权益准入证明。 | 02 分册 Consumer adapter 与通用资金动作、工程落地决策卡；03 分册权益清结算和对账拆分。 | `TDD-BEN-*`、`TDD-BEN-RED-*`、`TDD-BEN-CLS-*`、`TDD-BEN-RECON-*`。 | 恢复请求态权益快照、场景 facade、`CONTRACT_ONLY` 夹具或 `contextVariables` 过渡字段被用来宣称生产完成，或退款/清结算按当前营销规则重算；把非入账优惠塞进 generic direct action。 |
...
| P2 | VCC、全球账户和收单业务支持 | 三类业务模式的资金事实归一、外部规则核验、轨道结果和业务专项边界。 | 业务能力包引用、轨道/外部账户引用、归一资金事实、外部规则核验字段。 | 三类业务补充 PRD 和专项系分；资金底座只承接统一钱包、账本、清结算、对账和归档接口。 | `TDD-RAIL-*`、`TDD-FX-*`、`TDD-OPS-006`、`HOST-OPS-*` 和第 4.2 节业务专项测试入口。 | 把卡组织、银行、PSP、跨境、商户经营、风控模型或合规结论沉入统一资金内核。 |
| 横切 | 运营审计、权限和外部规则 | 高危操作、敏感查询、导出、外部规则确认、合规待确认项。 | `operatorRef`、`auditRef`、`validation.mustFail`、外部规则核验字段。 | 05 分册测试观测安全与金融红线；03/04 分册运营对象。 | Web/宿主使用 `HOST-OPS-*` 验收操作审计；资金红线使用 `TDD-OPS-006`、`TDD-RAIL-*`、`TDD-FX-*`、`TDD-RED-*`。 | 后台直接改账、敏感原文进入日志/导出、公开资料被当成正式规则。 |

### 4.2 P2 场景资金验收追踪矩阵

本矩阵把 06 VCC 场景资金能力边界以及 07、08 场景分册的验收 ID 回挂到产品验收总入口。它只说明上层场景如何验收 wind-funds 提供的资金能力，不改变全局 P0/P1/P2 优先级。VCC 交易主体、外部事件、业务状态、卡账单和 issuer 协议由 Fincone 发卡业务承接；wind-funds 只验收支付工具快照、资金责任、授权占用、清算、退款、账本和对账引用。
...
| 06 VCC 资金边界 | `VCC-RED-001` | 完整 PAN/CVC 不得进入资金底座。 | 支付工具脱敏快照、审计引用和敏感导出边界；05 系分安全红线。 | `TDD-P2-VCC-RED-001`、`HOST-OPS-003`、`TDD-OPS-006`。 | 任何日志、投影、导出或测试夹具出现完整 PAN/CVC 都必须阻断。 |
...
| 08 收单 | `ACQ-RED-001`、`ACQ-RED-002` | 支付成功不得展示可提现；完整 PAN/CVC 不进入资金底座。 | 商户账单展示状态、PCI/tokenization 边界、敏感导出审计；08 分册和 05 安全红线。 | `TDD-P2-ACQ-RED-001`、`TDD-OPS-006`、`HOST-OPS-*`。 | PSP、收单行、卡组织、PCI 和商户结算规则未确认前，不得作为生产放行依据。 |

### 4.2.1 全球账户多币种真实场景模拟验证

本节把外部多币种账户方法论收敛为资金业务专家、产品、架构、TDD 和代码设计共同可检查的场景。它只定义缺口和验证入口，不授权当前代码实现全球账户 capability pack。

| 真实场景 | 资金专家关注点 | 产品/系分应证明 | TDD 入口 | 当前代码设计结论 |
| --- | --- | --- | --- | --- |
| 入金报文 accepted，银行流水尚未匹配 | 外部受理不等于客户可用余额。 | 保持 `IN_TRANSIT` 或挂账；只有到账证据、主体映射、币种和合规状态满足后才能入账。 | `TDD-P2-GA-001`、`TDD-RAIL-008`、`AC-IN-003`。 | 现有 `ExternalFundsEventApplicationService` 只适合 confirmed credit -> funding account；accepted/matching 仍是设计缺口。 |
| 期望 USD VA，实际 EUR 到账 | 错币种不能静默换汇，也不能按 USD 入账。 | 生成错币种差错、挂账或驳回；有 FX quote 和审批后才能形成新资金事实。 | `TDD-P2-GA-RED-001`、`TDD-FX-001`、`TDD-FX-002`。 | 直接交易和 DSL 已有同币种红线；全球账户错币种差错对象仍是 P2 缺口。 |
| 出金已提交，后续退汇并扣银行费 | message sent 不等于付款成功，退汇不是退款。 | 出金保持在途直到终态；退汇、费用和责任方要追加事实并可对账。 | `TDD-P2-GA-002`、`TDD-RAIL-009`、`TDD-SETTLE-*`。 | 当前只具备通用交易、出款和对账门禁设计；缺全球账户退汇 capability pack。 |
| 日切后按法主体、币种核对客户余额与备付/托管账户 | 客户资金和机构自有资金必须隔离，账实要能证明。 | 按法主体、账套、币种、会计日期、余额桶和外部账户引用生成对账范围。 | `AC-REC-009`、`TDD-RECON-*`、`TDD-B7-RED-*`。 | 账本和余额桶具备基础能力；法主体/账套日切、备付账户映射和真实 settlement fact source 仍是缺口。 |
...
| 验收 ID | 场景 | 输入 | 期望输出 | 核心断言 |
| --- | --- | --- | --- | --- |
| AC-BEN-001 | 无权益交易目标态 | 交易指令不携带让利出资记账交易事实。 | 直接交易、授权、退款、route、posting 和投影行为保持主语义。 | `FundsInstruction.amount` 仍表达资金主链路金额；不恢复 `benefitSnapshot` 字段也不影响主流程。 |
...
| AC-BEN-006 | 授权时占券，完成时核销 | 授权请求携带权益占用引用、原权益资金事实引用或等价只读摘要。 | 授权阶段只占用或冻结权益引用，完成阶段核销或入账。 | 授权拒绝、撤销和过期不得核销权益；授权完成不按当前券规则重算。 |
...
| AC-BEN-010 | 缺原权益资金事实的逆向处理 | 原交易被标记为含权益，但退款、清结算或对账时找不到原权益资金事实、历史摘要或经审批补充事实。 | 失败或进入人工处理。 | 不调用当前营销规则重算，不用当前活动状态补造历史权益结果。 |
| AC-BEN-011 | 含权益交易进入清结算和对账 | 商户券、平台补贴、储值券、手续费和退款冲回同时存在。 | 清分、清算、结算、对账明细能拆分各金额项。 | 营销核销金额、订单金额、资金入账、商户应收和补贴冲回可核对；差异进入差错单。 |
| AC-BEN-012 | 含权益生产链路准入 | 真实入账让利出资通过应用服务或等价不可变事实进入资金链路。 | 让利出资交易、route snapshot、交易事实快照、posting context 或等价不可变存储可追溯；非入账优惠只进入订单、清分、对账或投影解释。 | 只在请求、上下文或投影摘要上可见不能声明生产完成；逆向事件、清结算、对账和投影重放必须能读取原让利出资事实、历史摘要或受控补充事实。 |
...
| AC-BEN-019 | 权益视图可理解且不误导 | 含权益交易进入用户账单、商户账单、运营时间线、财务对账视图或审计导出。 | 视图必须说明金额来源、状态含义、不可操作原因、后续处理动作和脱敏证据引用。 | 不得把授权占用展示为最终消费、冻结展示为扣款、待清算展示为可提现、出款受理展示为到账成功；敏感证据只能展示脱敏摘要或外部 reference。 |

权益退款分摊补充验收口径：

1. `AC-BEN-009` 必须同时验收规则版本、分摊依据、稳定组件顺序、舍入模式、尾差归属、组件剩余额度和幂等摘要；不能只验收最终退款总额。
2. 同一原交易、同一原权益资金事实或历史摘要、同一退款请求、同一 `refundDecisionId` 和同一规则版本，重复提交、重试或并发冲突后的可重试结果必须一致。
...
| AC-PI-002 | 支付工具绑定解析 | 工具绑定到资金账户或信用账户；使用主体、支出控制范围和 Spend Rule 作为独立准入上下文提供。 | 路由解析出最终核心资金账务主体，并分别固化工具绑定快照、支出控制范围和 Spend Rule 控制快照。 | `PaymentInstrumentBinding.subjectType` 只允许 `FUNDING_ACCOUNT/CREDIT_ACCOUNT`；最终 route leg 必须落到账户主体，支出控制范围和 Spend Rule 只能作为支出控制上下文。 |
| AC-PI-003 | 工具资金流向、状态和动作能力校验 | 工具资金流向为收款、付款或双向，状态为未验证、暂停、解绑、过期或可用，动作能力为 RECEIVE、PAY、AUTHORIZE、REFUND 或 WITHDRAW。 | 不符合当前动作时失败或授权拒绝。 | 失败不得生成 route、posting plan 或 LedgerEntry；原因和审计可查询。 |
| AC-PI-004 | 资金责任决策 | 支付工具绑定和规则准入已经确定一个资金账户或信用账户主体，调用方按币种和关系类型查询其出资或结算责任目标。 | 资金责任决策保存关系号、源账户、关系类型、币种和唯一目标账户，routing decision 再组合工具、层级和控制快照。 | 0 条或多条关系都失败；源端和目标端都只能是资金账户或信用账户，不在资金责任关系内使用优先级、默认项或兜底链。 |
| AC-PI-005 | 工具换绑后的原路退款 | 原交易后支付工具绑定关系或资金责任关系发生变化。 | 退款、撤销、退费或拒付按原 route snapshot 和原工具快照解释。 | 不按当前绑定重新选路；累计金额和原路径仍可核对。 |
| AC-PI-006 | 支付工具动作和账户能力匹配 | 工具缺少当前动作能力，或工具解析出的内部资金账户缺少收款、付款或提现能力。 | 工具能力失败时直接拒绝或失败；工具能力通过但账户能力不足时路由失败或进入人工处理。 | 支付工具能力只做工具准入；账户无对应能力时不得继续入账，不能用工具能力绕过账户能力。 |
| AC-PI-007 | 绑定历史和审计留痕 | 工具绑定、默认标识、优先级、方向或状态发生变更。 | 系统保留当前有效绑定、历史变更记录、前后值、操作者、原因、时间和版本。 | 当前态用于新交易候选；历史审计用于争议、对账、回放和客服证明。资金责任关系当前无更新入口，不得把绑定变更机制外推为关系变更能力。 |
| AC-PI-008 | 钱包入口作为支付方式引用 | 前台选择内部余额钱包、平台钱包、商户钱包、返利钱包、VCC 卡、第三方钱包端点或通道 token 作为支付方式。 | 内部钱包入口解析为 `SubjectRef`、`BenefitSnapshot` 或资金责任决策；VCC 卡解析到 `PaymentInstrumentRef` + 卡绑定资金/信用子账户；第三方钱包端点和通道 token 可进入工具快照；posting 和 LedgerEntry 主体必须是解析后的资金账户（含平台角色解析后的平台资金账户和 VCC 资金子账户）或信用账户（含 VCC 信用子账户）。 | 钱包入口不等于钱包账户域，也不等于 `FundingAccount` 或 `PaymentInstrument`；缺解析结果时失败，不得用钱包标识、工具号、支出控制范围、Spend Rule、PAN 或 token 入账。 |
| AC-PI-009 | `FundingAccount` 语义不泛化 | 信用账户、支出控制范围、Spend Rule、支付工具或平台账户角色进入路由、授权、余额查询或清结算。 | 信用账户保持独立资金账务主体；支出控制范围和 Spend Rule 保持独立支出控制对象；平台账户角色先解析到平台资金账户；支付工具只做引用。 | `FundingAccount` 只表示真实资金账户或平台责任资金账户；不得把信用额度、预算控制、支出规则或支付工具塞进 `FundingAccount` 统一处理。 |
| AC-PI-010 | 授权支付工具应用入口 | VCC、共享卡、VA、外部钱包端点、通道 token 或卡 token 发起授权，调用方只持有工具引用和业务上下文；内部钱包或信用额度入口先按业务入口解析。 | 应用层完成工具状态、绑定版本、使用主体、支出控制范围、Spend Rule、资金责任和账户能力校验；批准后委派账户主体型授权内核，拒绝时只返回拒绝事实和原因。 | 支付工具入口不等于账本主体；不得要求外部调用方绕过钱包 / 路由直接选择内部账户，也不得让工具准入失败生成 route、posting 或 LedgerEntry。 |
| AC-PI-011 | 共享卡和多卡主账户共享账单归因 | 每张共享卡绑定一个信用子账户，多张共享卡通过同一主信用账户或主资金账户共享额度/资金约束。 | 单卡余额、ledger entry 和余额投影落到各自信用子账户；主账户用于额度/资金约束和汇总展示；卡维度账单通过交易投影按 `PaymentInstrumentRef`、绑定版本、子账户、使用主体、支出控制范围、Spend Rule 和资金责任快照过滤生成。 | 多张共享卡不得绑定同一子账户；不得创建卡号余额投影主体；逆向事件不按当前绑定重算历史卡归因。 |
| AC-PI-012 | VCC 关联子账户作为发卡账务主体 | VCC 卡、预付卡或共享卡需要解释余额、授权、清算、退款、拒付和费用。 | `SubjectRef(FUNDING_ACCOUNT)` / `SubjectRef(CREDIT_ACCOUNT)`、账目 profile、绑定工具、子账户/父账户快照、资金责任来源、route snapshot 和 ledger entry 可追溯。 | VCC 关联子账户可作为资金账户或信用账户入账；卡号、PAN、token、持卡人和卡片凭证不可入账；缺子账户、缺父账户、缺账目 profile 或状态不可用时失败无账务副作用。 |
...
拒付/争议准入口径：授权后的争议、拒付或扣回必须可解释、可查询、可投影、可审计。目标态通过 `refund / AUTH_REFUND` 的争议字段、外部引用、凭证、审计上下文和内部 `DISPUTE` 标签区分普通授权链退款、`NO_AUTH` 退款和授权拒绝；交易层不再暴露独立 `chargeback` 主入口。完整争议运营、清结算追偿或外部卡组织规则由后续独立工程边界承接。

授权过期准入口径：`expire` 是不可信错误状态，不作为资金交易主入口。系统定时到期、等待超时或通道未返回不能单独生成资金释放事实；如需释放剩余授权占用，必须通过外部可信撤销/冲正、清算剩余释放或人工差错补事实进入 `reversal`、调账或差错链路。

| 验收 ID | 场景 | 输入 | 期望输出 | 核心断言 |
| --- | --- | --- | --- | --- |
| AC-AUTH-000 | 授权入口分层 | 授权由外部支付工具、VCC、共享卡、外部钱包端点或通道 token 触发，或由可信内部系统直接传入已解析账户主体。 | 支付工具型入口先解析并固化工具、绑定、规则和资金责任快照；内部钱包、信用额度或权益入口先解析为主体或快照；账户主体型入口直接进入授权内核。 | 三类入口不得混用责任：外层不得写账，内核不得按当前工具绑定重算历史路径；新增支付工具型入口必须由独立工程边界明确契约和验证范围。 |
| AC-AUTH-001 | 授权成功 | 资金账户、信用账户或 VCC 关联子账户可用余额/额度充足，预算型 Spend Rule 和其他支出规则通过。 | 资金或信用主体 AVAILABLE -> AUTHORIZATION，并保存子账户、父账户和规则快照。 | 授权成功不是最终消费；支出控制范围和 Spend Rule 不作为占用主体。 |
...
| AC-AUTH-004 | 授权撤销释放 | 原授权存在且剩余可释放，外部明确撤销或冲正。 | AUTHORIZATION -> AVAILABLE。 | 基于原快照，超剩余释放失败；不得用系统到期或超时替代可信撤销事实。 |
| AC-AUTH-005 | 授权过期不入资金交易 | 原授权超过业务等待窗口，但没有可信撤销、清算剩余释放或人工差错补事实。 | 不生成新的资金交易、route、posting、LedgerEntry 或余额变化。 | 到期只是运营或差错提示，不是可信资金事实；后续释放必须通过 `reversal`、调账或差错链路承接。 |
| AC-AUTH-006 | 授权部分完成 | 原授权存在，完成金额小于剩余授权。 | VCC PREPAID 的 FundingAccount 按完成金额转入 SETTLEMENT；SHARED 的 CreditAccount 按完成金额转入 OUTSTANDING，父 FundingAccount 只向真实收款目标入账一次；剩余授权继续占用，等待可信释放事实。 | 后继请求账户必须与原授权主主体一致；不写 AUTHORIZATION -> LIMIT，不新增含义重复的完成金额账目。 |
...
| AC-CTRL-013 | 外部余额异常纠偏 | 第三方钱包、VCC 发卡行、发卡处理商或外部余额系统已形成终局事实，导致我侧同主体目标账目需要调整，甚至出现负可用。 | 通过受控余额调整或差错闭环调账生成纠偏事实。 | 必须有外部终局事件、外部余额快照、差错单或审批、凭证、责任方、操作者和重新对账引用；不得把外部异常包装成普通消费透支。 |
| AC-CTRL-014 | 资金指令三字段契约 | 调用方提交 `instructionType + eventType + transactionType`。 | 仅允许产品设计 02 中已冻结的 19 组组合进入 route；程序化构造、JSON DSL 和路由入口使用同一白名单。 | `FREEZE/UNFREEZE` 只能归 `BALANCE_CONTROL`，`BALANCE_ADJUST/LIMIT_ADJUST` 只能归 `ADJUSTMENT`；非法组合在路由前失败且无资金副作用，跨主体 `DIRECT_TRANSACTION / ADJUSTMENT` 当前不得准入。 |
| AC-CTRL-009 | 月度预算窗口隔离 | 支出控制范围存在 MONTHLY/2026-04 和 MONTHLY/2026-05 两个预算型 Spend Rule 窗口。 | 2026-05 的授权前预算控制、预留证据、释放证据或规则调整只影响 2026-05 的控制视图。 | 不得消费、释放或抵扣 2026-04 的预算控制额度，也不得把支出控制范围建成账本 bucket。 |
| AC-CTRL-010 | 默认生命周期账本 | 普通资金账户未显式传周期。 | 使用 LIFETIME/LIFETIME 账本 bucket。 | 不得因缺 periodId 错配到天、月或自定义周期 bucket。 |
| AC-CTRL-011 | 自定义周期预算 | 企业预算按合同周期 CUSTOM_CYCLE 管理。 | 周期 ID、起止规则、时区、日切、预算 scope 和规则版本可查询。 | 缺周期 ID、规则版本或预算 scope 不得通过预算控制；不得以支出控制范围入账补救。 |
| AC-CTRL-012 | 业务主体解析为账务主体 | 请求中包含用户、商户、平台角色、信用账户、VCC 关联子账户、支出控制范围、Spend Rule、卡或外部账户引用。 | route snapshot 和 LedgerEntry 只能使用资金账户（含平台角色解析后的平台资金账户和 VCC 资金子账户）或信用账户（含 VCC 信用子账户）作为账务主体；支出控制范围和 Spend Rule 只进控制快照。 | 用户 ID、商户 ID、业务订单、卡号、PAN、token、VA、银行账户、支付工具、通道账户、支出控制范围、Spend Rule 和 `VCC_ACCOUNT` 不得直接作为可记账主体。 |
...
| 信用账户额度调整或预算规则调整，不表达现金流和跨主体资金转移。 | AC-CTRL-005、AC-CTRL-006 | BALANCE_CONTROL / LIMIT_ADJUST；预算型 Spend Rule 版本和控制视图。 | TDD-CTRL-005 至 TDD-CTRL-008。 | 把 LIMIT 当作普通交易 source/target，或把支出控制范围 / Spend Rule 当作资金池或账本主体。 |
...
| 对账差错闭环内需要跨主体补偿、价值转移或明确的资金调账交易。 | AC-ADJ-001 | 批次授权 DIRECT_TRANSACTION / ADJUSTMENT；DSL-SETTLEMENT-RECONCILIATION-ADJUST-001；差错调账账务规则。 | TDD-RECON-001、TDD-RECON-002、HOST-OPS-001。 | 通过余额控制 adjust 表达跨主体价值转移，或直接修改历史分录和投影。 |
...
| AC-CLR-007 | 收益应得项可清分 | 用户代理佣金、平台员工二级分润或 KPI 激励已经由业务侧产出收益应得项、归因快照、规则版本和审批引用。 | 生成收益可清分明细。 | 可追溯到交易利润或 GMV 口径、代理关系、员工关系、规则版本、审批和操作者；不得从报表或人工汇总反推。 |
| AC-CLR-008 | 两级代理收益清分 | 平台员工邀请用户代理，用户代理服务的被邀请用户产生消费利润，平台员工可按配置分润用户代理佣金池。 | 生成用户代理净佣金和平台员工二级分润金额项。 | 只支持两级归因验收；缺归因、缺审批、超两级、规则版本缺失或金额不平时必须排除或阻断。 |
| AC-CLR-009 | 收益分润清算确认 | 收益清分批次已确认，清算前置对账、退款、争议、风控和审批均可放行。 | 收益参与方可用余额、员工应付账户或合作方账户按清算确认进入可结算口径。 | 清分确认和清算候选不入账；清算批次确认才允许生成标准资金事实和账本分录。 |
...
| AC-SET-003 | executor 终态成功与内部资金闭合 | executor 成功回单金额一致，且内部 Funds/Ledger/Balance 合同满足。 | 只关闭 executor result；形成一次内部 payout effect 并关闭 SETTLEMENT 或 IN_TRANSIT。该结果展示为“执行完成/到账待确认”，不单独声明 beneficiary arrival 或 rail finality。 | 外部 reference、内部资金交易、账本和结算单可核对；到账证据和 rail finality 必须由独立 Owner 另行提供。 |
...
| AC-SET-007 | 外部受理和 executor 成功均不等于受益人到账 | 外部返回 accepted、submitted、message sent、processing 或仅有 executor `SUCCEEDED`。 | 出款单进入 ACCEPTED、PROCESSING、IN_TRANSIT 或“执行完成/到账待确认”展示。 | 不关闭为 beneficiary arrival；必须等待独立 bank/beneficiary 到账证据。对账确认只提供核对结果，不替代到账或 rail finality。 |
...
| AC-SET-010 | 收益分润结算单生成 | 用户代理佣金、平台员工二级分润或 KPI 激励已完成清算确认。 | 生成面向收益参与方的结算金额项、扣减项、留置项和可出款解释。 | 结算单必须区分用户代理净佣金、平台员工二级分润、KPI 激励、追偿、留置和税务/会计待确认项，不得混成一笔分润总额。 |
| AC-SET-011 | 成功出款后外部退回 | 原出款已成功，外部随后返回 returned 或退票事实。 | 当前只生成差错并保留原成功事实，不自动动账。 | 不得复用 `PAYOUT_FAILED` 重复释放；新增 `PAYOUT_RETURNED` 前必须闭合原出款引用、外部终局证据、独立 route/effect、部分退回、费用责任、幂等和并发终态门禁。 |

AC-SET-006 至 AC-SET-009 是产品验收口径。出款前准入设计只能作为工程输入和差距复核起点；在未补齐出款单生命周期、回单处理、金额不一致、数据库级服务闭环和目标测试资产前，不得声明清结算、对账或出款生命周期生产完成。
...
| AC-REC-006 | 调账核销 | 差错已确认，审批和凭证齐全。 | 追加调账事实并重新对账。 | 不修改历史分录；核销有状态和审计；关闭依据必须是当前血缘末端已固化的对账运行结果，不能由调用方自报对平。 |
...
| AC-REC-012 | 余额一致验收 | 资金账户（含平台角色解析后的平台资金账户）和信用账户需要按余额桶核对；支出控制范围需要按 Spend Rule 窗口和控制视图核对。 | 输出余额桶公式校验结果和预算控制视图校验结果。 | CLEARING、AVAILABLE、OUTSTANDING、SETTLEMENT、IN_TRANSIT、FROZEN 等桶分别满足期初、本期入出和期末公式；预算视图不得冒充资金余额。 |
...
### 5.6.0 清结算真实场景模拟验证 Loop

本节用于资金业务专家、产品、架构、TDD 和代码设计共同模拟一条商户资金链路。它不新增能力，只把现有 `AC-CLR-*`、`AC-SET-*`、`AC-REC-*`、`CLS-GATE-*` 和当前测试资产串成可阻断的验收路径。

| 真实场景 | 资金业务专家判断 | 产品/系分承接 | TDD 入口 | 当前代码设计结论 |
| --- | --- | --- | --- | --- |
| 支付成功后进入可清分识别 | 支付成功只说明交易事实成立，不代表商户可结算。 | 必须先核对交易、route、ledger entry、CLEARING 桶和基础对账结论。 | `AC-REC-007`、`TDD-CLS-001`、`TDD-B7-RED-001`。 | 已有可清分明细服务冻结交易版本、退款累计、RouteSnapshot 摘要和对账证据；完整策略调度仍属后续能力。 |
| 清分批次确认后准备清算 | 清分是分类和快照，不是释放资金。 | 单主体清分结果快照必须固化来源事实、清分规则版本、标量金额和对账准入；清算账期与清算规则独立决策。 | `TDD-CLS-003`、`TDD-CLS-007`、`CLS-GATE-003`。 | 已有 DDL/H2 和服务测试证明创建、复核、确认、取消、确认前原子复核、快照幂等、不入账，以及候选等待/阻断/排除/锁定；当前候选 `READY` 尚未覆盖退款、争议和风控最终复核。 |
| 清算批次确认 | 只有清算确认才允许 `CLEARING -> AVAILABLE`。 | 重大差错、账期未到、退款/争议/风控阻断、重复入批或候选未锁定必须阻断；批次原子确认，不允许部分成功。 | `AC-REC-008`、`TDD-CLS-004`、`TDD-CLS-006`。 | 已落地清算批次原子锁定、事务内最终复核、受控 `FundsClearingTransactionService`、账务引用回写和候选 `CLEARED/BLOCKED/LOCKED` 分流，并通过 `tests` 模块 H2 仓内本地验收。争议/风控权威查询和按原资金分配处理部分退款不在当前能力范围；监控和 Runbook 属于宿主接入责任。 |
| 结算单锁定并提交出款 | 结算锁定不等于外部到账。 | 出款前必须检查账户、端点、通道额度、cutoff、外部规则、审批、负余额、准备金和对账差错。 | `AC-SET-006`、`TDD-SETTLE-004`、`CLS-GATE-007`。 | `PayoutPreflightServiceTests` 只证明最小门禁；完整出款单、回单和解释视图仍缺。 |
| 外部 accepted 或金额不一致 | accepted/submitted/processing 都不是成功，金额不一致不能按失败重试掩盖。 | 必须保持在途或差错，展示事实状态、展示状态、操作状态和后续动作。 | `AC-SET-007`、`AC-SET-008`、`AC-SET-009`、`TDD-SETTLE-005`。 | 外部非终态和金额不一致仍需 `PayoutReceiptMismatchTests`、`PayoutExplainabilityTests`。 |
| 差错核销后重新对账 | 核销不是改历史账，必须追加事实或处理动作并重新对账。 | 差错单要有来源分段、责任方、审批、凭证、处理动作、重跑记录和审计。 | `AC-REC-006`、`AC-REC-015`、`TDD-RECON-001`、`TDD-B7-RED-009`。 | 已有 difference 首切片；完整对账任务、匹配、重跑并发和关闭规则仍需补齐。 |

### 5.6.1 清结算与对账工程落地验收门禁

以下门禁用于区分 wind-funds 工程落地与宿主接入责任。`CLS-GATE-001` 至 `CLS-GATE-004` 约束本仓工程；`CLS-GATE-005` 至 `CLS-GATE-008` 约束宿主运行、展示、权限和审计，不作为 wind-funds 准出阻塞项。它们不替代 `AC-CLR-*`、`AC-SET-*`、`AC-REC-*`，也不能用出款前准入设计替代未提供的完整结算或外部出款生命周期。

| 门禁 ID | 场景 | 必须满足 | 不满足时结论 |
| --- | --- | --- | --- |
| CLS-GATE-001 | 工程边界准入 | 工程任务已确认设计基线范围、写入范围、禁止范围、验证命令、人工确认点和停止条件。 | 只能继续设计或契约草案，不得改生产代码、测试代码、DDL/H2 schema 或运行配置。 |
| CLS-GATE-002 | Red 准入 | `TDD-B7-RED-001` 至 `TDD-B7-RED-009` 必须先于任何 Green 实现落地，并能证明错误路径会失败。当前首切片可回挂 `ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`，用于证明差错、清算前置对账消费和出款前最小门禁已进入真实测试资产。 | 不得进入清分、清算、结算、出款、差错或追偿 Green 实现。 |
| CLS-GATE-003 | 数据与服务闭环 | 当前声明范围内的对账、清分、内部清算、W1 结算单/金额项、W2 出款单/归一回单和 W3 追偿责任/结果引用已完成表、状态机、幂等键、DTO、Entity、Mapper、服务与 H2 验收。W3 只登记确认责任和已完成资金交易引用，不承接退款、拒付、催收等策略编排或资金执行；操作审计由 Web/宿主承接。 | 未提供能力不得由当前 H2 结果外推。 |
| CLS-GATE-004 | 受控资金事实命令 | 清结算、对账、出款、差错、冲正、调账或追偿若要追加资金事实，必须使用业务语义明确的交易命令，并携带来源单据、审批号、证据引用、幂等键、原事实引用、操作者、原因和可撤销边界。 | 没有对应业务命令和完整证据契约的运营动作只能生成处理单、差异报告或宿主审计记录。 |
| CLS-GATE-005 | 宿主 NFR 与可观测性 | 宿主按自身容量、重跑窗口、对账文件延迟、外部回单乱序、告警指标、Runbook 和恢复要求验收；资金侧只验证声明的容量边界、并发竞争和失败无副作用。 | 由宿主发布制度决定是否上线，不阻塞 wind-funds H2 准出。 |
| CLS-GATE-006 | 宿主专业确认状态 | 税务、会计、合同、银行、通道、卡组织、KYC/KYB/AML、客户资金、跨境或外汇口径由上层保存规则来源、版本、生效日期、适用法域、确认方和确认状态。 | 影响上层业务决策时阻断、降级或转人工，不转化为资金底座自动判断。 |
| CLS-GATE-007 | 宿主使用者解释 | 工作台、商户账单、财务对账视图、审计导出和告警展示事实状态、操作状态、不可操作原因、后续动作、责任方和脱敏证据引用；资金侧只提供只读事实和解释字段。 | 缺解释字段时宿主不得展示为自动放行或商户可见完成态，不反向否定 wind-funds 已声明能力。 |
| CLS-GATE-008 | 宿主职责分离和证据最小化 | Web/宿主对清算确认、结算锁定、出款提交、人工放行、差错核销和敏感导出校验权限、审批、复核、租户/主体边界、脱敏与审计留痕。 | 宿主不得由单一运营动作静默完成高危处理，也不得向无关角色展示敏感原文；本项不纳入 wind-funds H2 验收。 |
| CLS-GATE-009 | 收益分润与激励结算验收 | 两级代理收益分润用例必须具备收益应得项、收益参与方账户解析、归因快照、规则版本、GMV 阶梯或利润口径、审批、退款/争议排除、对账阻断和结算解释。 | 缺任一证据时只能作为设计验证或待确认收益，不得进入可用余额、结算单出款或生产可用声明。 |
...
| GOV-GATE-004 | Manifest 与 H2 范围 | 归档任务、运行记录、Manifest、checkpoint、watermark、冷热对象清单、余额快照、差异报告、人工处理、导出快照和消费方登记的表、状态机、幂等键与 H2 测试范围已确认；操作审计由 Web/宿主承接。 | 不得声明归档、余额快照、交易投影重放、差异报告或大数据消费边界数据库级完成。 |
...
| GOV-GATE-008 | 使用者解释、导出和 Runbook | 差异报告、人工处理、治理导出、Runbook 告警和审计导出必须包含范围、模式、状态、阻断原因、影响范围、责任方、后续处理动作、恢复验收和脱敏证据引用。 | 缺任一解释字段时不得展示为治理完成、归档成功或可覆盖投影。 |
...
清结算与对账、资金数据治理都属于 P0 能力，但 P0 不等于可以直接创建模块或进入编码。进入对应能力工程落地前，必须把模块物理落点、公共契约和测试边界写入独立工程边界；否则只能继续做产品、DSL、系分或 TDD 草案。
...
| AC-REPLAY-005 | 多维交易投影查询 | 同一资金事实携带支付工具快照、子账户/父账户快照、资金账户 / 信用账户账务主体、支出控制范围和 Spend Rule 控制快照。 | 可按支付工具、子账户、父账户、资金账户、信用账户、支出控制范围和 Spend Rule 生成只读账单、控制时间线或差异报告。 | 查询维度不得成为资金事实源；不得把支付工具、支出控制范围、Spend Rule 或 `VCC_ACCOUNT` 写成 LedgerEntry 主体；重放不得反写交易、路由、账本或余额。 |
...
| AC-RPT-003 | 指标边界承接 | 报表指标模块承接资金交易指标项。 | 指标能说明业务问题、使用者、推荐事实来源和只读边界。 | 指标实现不得进入资金主链路，不得复用归档、重建或重放对象。 |
...
| 红线 ID | 场景 | 必须失败的条件 |
| --- | --- | --- |
| RED-001 | 外部账户入账 | 银行账户、VA、卡号、PAN、token、PSP 账户或 `VCC_ACCOUNT` 被创建为内部账本主体；VCC 发卡账务必须先解析为资金子账户或信用子账户。 |
...
| RED-033 | 差错核销未重新对账 | 差错单被人工关闭，但没有补事实、冲正、调账、挂账、追偿或已持久化的重新对账运行结果；或使用非当前线性血缘末端、已被替代的旧结果、对象不一致结果和调用方自报结论关闭。 |
...
| RED-046 | 支付工具直接入账 | 卡号、PAN、token、VA、外部账户、通道 token、钱包标识或支付工具 ID 被写成 LedgerEntry 主体或余额投影主体；VCC 发卡账务必须解析到资金子账户或信用子账户，不能由卡凭证、支付工具或 `VCC_ACCOUNT` 临时替代。 |
| RED-047 | 支付工具敏感信息泄露 | 完整卡号、CVV、密钥、token secret、银行账户敏感号或身份凭证进入普通快照、日志、导出或报表。 |
| RED-048 | 工具换绑导致历史重路由 | 退款、撤销、退费或拒付按当前工具绑定、当前资金责任关系或当前费率重新选路。 |
| RED-049 | 绑定历史被覆盖 | 支付工具绑定、默认关系或优先级变更只覆盖当前记录，缺少历史版本、前后值、操作者、原因和审计证据；或绕过服务直接覆盖当前资金责任关系。 |
...
| RED-052 | 无资金影响权益入账 | 商户让利、展示优惠或其他非入账权益组件生成 LedgerEntry、posting plan 或余额 delta，或误调用让利出资记账交易服务。 |
| RED-053 | 授权拒绝核销权益 | 授权拒绝、支付工具准入失败、风控拒绝、余额不足或预算不足后仍核销、作废或冲减权益。 |
| RED-054 | 权益退款按当前规则重算 | 退款、可信撤销、拒付/争议结果或清结算差错处理时，不读取原权益资金事实、历史摘要或受控补充事实，而按当前营销规则重新判断；授权过期不得触发资金层权益重算。 |
...
| RED-058 | 权益事实只在请求态却声明生产完成 | 没有 route snapshot、交易事实、独立权益资金事实或等价不可变存储，就声明含权益生产链路可回放。 |
| RED-059 | 权益闭合角色混用 | 补贴冲回、不可退权益、负债恢复或展示项被算入原订单正向闭合，导致退款、清结算或对账金额失真。 |
| RED-060 | 权益准入决策缺失仍进入生产资金流 | 未选择能力消费范围、JSON 夹具级别、权益资金事实源、零实付表达、平台补贴表达、独立伴随指令原子性、储值预付口径、退款分摊、历史无权益事实处理策略、补充权益事实模型、专业确认状态、审计证据包、使用者解释视图、证据最小化或外部规则核验状态，就实现含权益 route/posting/replay、清结算、对账、投影、归档、冷热读取或治理重放消费。 |
| RED-061 | `CONTRACT_ONLY` 夹具误作资金流证据 | 只解析契约样例，没有 expectedRoute、expectedPosting、balanceAssertions 或失败无副作用断言，却声明资金流、清结算或对账完成。 |
| RED-062 | 专业确认缺失仍自动入账 | 平台补贴、储值/预付、礼品卡、不返券不冲补贴、负债释放或转损益缺财务、会计、税务、法务、合规或业务确认，或确认已过期、被撤销、适用范围不匹配、证据引用含敏感原文时，仍自动入账、退款或结算。 |
| RED-063 | `contextVariables` 承载核心权益事实 | 组件金额、资金责任、退款处置完整内容或当前营销规则被长期放入 `contextVariables`，而不是进入权益资金事实、route snapshot、交易事实或等价不可变存储。 |
...
| RED-067 | `FundingAccount` 被泛化成万能账户，或支出控制范围 / Spend Rule 被当作账本主体 | 信用账户、VCC 信用子账户、支出控制范围、Spend Rule、支付工具、钱包标识或外部账户被写入普通 `FundingAccount` 口径，或支出控制范围 / Spend Rule 被写成 route leg、LedgerEntry 或清结算主体，导致额度、发卡账务、预算、规则、工具引用和真实资金混账。 |
...
| AC-PI-001 至 AC-PI-012 | PaymentInstrumentRefSpec、ExternalAccountRefSpec、SubjectRef(FUNDING_ACCOUNT/CREDIT_ACCOUNT)、AccountHierarchyRelation、参与方 AccountHierarchySnapshot、RoutingDecisionSpec、RouteParticipantSpec、RouteLegSpec、RouteSnapshotSpec、BindingHistory、PaymentInstrumentCapabilityApplicationService、FundingResponsibilityResolutionApplicationService、ExternalFundsEventApplicationService、FundsAccountCapabilityApplicationService、AccountHierarchyRelationService | 业务入口参数只做产品或应用层选择口径；外部支付工具只做路由输入和快照引用；VCC 发卡账务落到卡绑定资金/信用子账户；工具绑定、方向、状态、账户选择、账户能力、内部钱包入口解析、绑定历史审计、授权准入和收款工具 route snapshot 归因必须可解释；最终资金路径由 participant/leg 显式表达，逆向交易按原快照回放；`FundingAccount` 只表示真实资金账户；AC-PI-010 要求宿主先归一为账户主体型资金动作，Provider internal orchestration 不升级为替代 Public facade。 |
...
| AC-AUTH-012 | AUTHORIZATION_TRANSACTION / AUTH_REFUND 无授权退款模式、DSL-AUTH-REFUND-001 | 无前置授权但有可追溯外部引用时通过 refund 承接；以空原授权流水进入 no-auth 语义，携带 `externalReferenceSn`、退款原因、操作者和审计；不得补造授权占用、携带或查询内部授权流水，也不得静默退款。 |
...
| AC-BEN-*、RED-050 至 RED-066 | Consumer Benefit adapter、generic direct pay/refund、`FundsActionFact`、`DSL-BENEFIT-*`、expectedBenefitContributionTransaction、refund dispositions、fixtureLevel、权益准入证明、伴随权益指令组、补充权益事实、审计证据包、explainableProjection | TDD-BEN-*、TDD-BEN-RED-*、TDD-DIR-*、TDD-AUTH-*、TDD-CLS-*、TDD-RECON-*、TDD-RACE-012、FundsOperationExplainabilityTests；操作权限与审计由 Web/宿主验收 | 权益金额组件只承接业务侧已决策结果；商户让利、平台补贴、储值券、不退券、授权占券、部分退款、零实付权益、伴随指令、补充事实、清结算拆分和营销资金差异都可追踪；无资金影响权益不入账，真实入账让利出资不得和本金或手续费净额混记；含权益生产链路必须能长期回放原 ActionFact、历史摘要或受控补充事实，夹具级别、准入决策、审计证据包、解释视图和外部规则确认不能混用。 |
| AC-ROUTE-*、AC-PI-*、RED-043 至 RED-049、RED-067 | RouteSnapshotSpec、RouteParticipantSpec、RouteLegSpec、PaymentInstrumentRefSpec、ExternalAccountRefSpec、RoutingDecisionSpec、AccountHierarchySnapshotSpec、Route Replay DSL、BindingHistory、Spend Rule 快照、DSL-PAYMENT-INSTRUMENT-ROUTE-001、DSL-PAYMENT-INSTRUMENT-FAIL-001、DSL-PAYMENT-INSTRUMENT-REPLAY-001、expectedRouteCreated=false | TDD-ROUTE-*、TDD-WALLET-*、TDD-RACE-009、TDD-RED-001、TDD-RED-003、TDD-RED-029、TDD-RED-034 至 TDD-RED-037、TDD-RED-043 | 路由只解析路径，不写账；外部支付工具、外部账户、支出控制范围和 Spend Rule 只做引用、scope、规则或快照；内部钱包入口只做主体、权益资金事实或历史摘要解析来源；绑定、方向、状态、账户选择、账户能力、`FundingAccount` 语义边界和历史审计可解释；最终主体和金额由 participant/leg 显式表达；失败不自动换路；普通支付不误入商户清算；工具换绑或账户层级变化不改变历史回放路径。 |
| VCC-AC-007、VCC-AC-008、VCC-AC-009、AC-PI-*、RED-046 至 RED-049、RED-067 | DSL-VCC-HIERARCHY-001、DSL-PAYMENT-INSTRUMENT-PREPAID-CARD-001、DSL-PAYMENT-INSTRUMENT-SHARED-CARD-001、PaymentInstrumentRefSpec、BindingHistory、AccountHierarchyRelation、参与方 AccountHierarchySnapshot、RouteParticipantSpec、RouteLegSpec、Spend Rule 快照 | TDD-P2-VCC-004 至 TDD-P2-VCC-012、TDD-RAIL-001A、TDD-AUTH-008、TDD-AUTH-009、TDD-WALLET-010、TDD-WALLET-015 至 TDD-WALLET-020、TDD-ROUTE-013 | 预付卡是 VCC 资金子账户模式，不是工具余额账户；共享卡是使用和绑定模式，不是卡号资金账户；每张卡最终都必须解析到资金子账户或信用子账户并固化原路径、直接父账户、支出控制范围和 Spend Rule 快照；父账户真实出资必须显式出现在路径中；同名混淆、缺子账户、缺应有的父账户、缺绑定版本、多主体歧义、缺财务确认、工具换绑、应用 facade 缺失和敏感字段都必须有临界测试；不得恢复 `VCC_ACCOUNT` 或把支出控制范围、Spend Rule 作为 LedgerEntry 主体。 |
| AC-AUTH-*、AC-RAIL-001、AC-RAIL-002 | AUTHORIZATION_TRANSACTION、DSL-AUTH-LIFECYCLE-001、DSL-AUTH-FORCE-CAPTURE-001、DSL-AUTH-REFUND-001 | TDD-AUTH-*、TDD-AUTH-FLOW-*、TDD-AUTH-ERR-*、TDD-AUTH-EXT-*、TDD-ROUTE-005、TDD-ROUTE-009、TDD-RACE-001 至 TDD-RACE-003、TDD-RAIL-001、TDD-RED-003、TDD-RED-005、TDD-RED-008、TDD-RED-016、TDD-RED-017、TDD-RED-017A、TDD-RED-017B、TDD-RED-033、TDD-RED-036 | 授权批准、拒绝、撤销、过期、普通完成、强制完成、完成后退款、无授权退款、拒付原因承接、原路径回放和 VCC 授权归一边界一致；预算只保留规则快照、预留和释放证据；完整支付轨道仍按 AC-RAIL-* 专项确认。 |
| AC-RAIL-002A 至 AC-RAIL-007、ACH-BOUNDARY-001 至 ACH-BOUNDARY-006 | P2 业务能力包外部轨道边界、ExternalAccountRefSpec、外部规则核验字段、对账差错和调账引用；不新增 ACH DSL 内核对象。 | TDD-RAIL-002 至 TDD-RAIL-007、TDD-RED-030、TDD-RED-034、TDD-P2-GA-*、TDD-P2-ACQ-*、TDD-RECON-*、TDD-OPS-006、HOST-OPS-* | ACH 或银行转账必须先由上层业务或通道适配层解释为资金事实；资金底座只承接外部引用、核验状态、在途、return 处理对象、差错、追偿、调账核销和投影；操作审计由 Web/宿主承接；不得实现 ACH 协议、Nacha/ODFI/RDFI 规则、Debit 授权、return code/NOC/reversal 解释或保存完整银行账户敏感信息。 |
| AC-CTRL-*、RED-013 | BALANCE_CONTROL、DSL-BALANCE-CONTROL-FREEZE-001、DSL-BALANCE-CONTROL-ADJUST-001、DSL-BALANCE-CONTROL-LIMIT-BUDGET-001、DSL-DIRECT-OVERDRAFT-001、账本周期 DSL、SubjectRef / RouteNode 主体解析、预算型 Spend Rule 控制视图 | TDD-CTRL-*、TDD-CTRL-FLOW-*、TDD-CTRL-ERR-*、TDD-LEDGER-008 至 TDD-LEDGER-011、TDD-WALLET-001 至 TDD-WALLET-004、TDD-ROUTE-004、TDD-RACE-004、TDD-RED-006、TDD-RED-011、TDD-RED-012、TDD-RED-015、TDD-RED-033 | 冻结、解冻、资金账户余额调整、信用账户额度调整、预算规则调整和受控负余额不混用；AC-CTRL-009 至 AC-CTRL-011 必须证明账本周期与预算规则窗口各自隔离；AC-CTRL-012 必须证明用户、商户、卡、VA、银行账户、支付工具、支出控制范围、Spend Rule 和业务单据不能直接作为可记账主体；冻结单状态不得表达消费、扣划或付款完成；LIMIT 只能由调额触碰；余额控制不承接跨主体价值转移。 |
| AC-ADJ-001、RED-010 | BALANCE_CONTROL / BALANCE_ADJUST、批次授权 DIRECT_TRANSACTION / ADJUSTMENT、DSL-SETTLEMENT-RECONCILIATION-ADJUST-001、差错调账账务规则 | TDD-RECON-001、TDD-RECON-002、TDD-CTRL-ERR-005、HOST-OPS-001 | 调账必须有差错或调整来源、审批、凭证、账本交易和重新对账依据；不得绕过差错闭环直接改余额，不得直接修改历史分录或投影。 |
| AC-CLR-*、AC-SET-*、AC-SET-006 至 AC-SET-009、AC-REC-* | DSL-SETTLEMENT-RECONCILIATION-001、DSL-SETTLEMENT-CLEARING-CONFIRM-001、DSL-SETTLEMENT-LOCK-001、DSL-SETTLEMENT-PAYOUT-RESULT-001、DSL-SETTLEMENT-RECONCILIATION-ADJUST-001、DSL-SETTLEMENT-POLICY-001 | TDD-CLS-*、TDD-CLS-FLOW-*、TDD-SETTLE-*、TDD-SETTLE-004、TDD-SETTLE-005、TDD-RECON-*、TDD-RED-020 至 TDD-RED-025、TDD-RED-031、TDD-OPS-004 至 TDD-OPS-006、HOST-OPS-* | 清分、清算、结算、出款和对账对象独立；出款前准入、外部非终态、金额不一致和出款解释状态不误导；清分前基础对账、清算前置对账、差错追加事实、账期临界、退款时序和重跑审计闭环可验证。 |
| AC-VIEW-*、AC-BALLOG-*、AC-REPLAY-005 | Projection DSL、余额日志和交易投影只读边界、Spend Control Activity、规则决策日志、预算控制投影 | TDD-LEDGER-*、TDD-VIEW-*、TDD-REPLAY-*、TDD-RED-010、TDD-RED-013、TDD-RED-014 | 余额投影、余额日志和交易投影都从事实派生；日志失败不回滚事实；投影和日志不得修复余额；支付工具、支出控制范围和 Spend Rule 只能作为查询维度和解释视图，不得成为资金事实源或账本主体。 |
...
| 生产完成断言 | 设计覆盖、局部实现或局部基线不得自动提升为生产完成；必须同时具备代码、DDL/H2、服务级测试、验证命令、审计证据和外部规则核验结论。 |
...
| 权益金额组件 | AC-BEN-*、RED-050 至 RED-066 | 优惠券、代金券、商户让利、平台补贴、储值券和零实付权益只作为已决策权益结果进入资金底座；真实入账让利出资进入让利出资记账交易服务，非入账解释进入订单、清分、对账或投影解释；金额闭合、退款处置、独立伴随指令原子性、补充权益事实、审计证据包、TOCTOU 重校验、使用者解释视图、证据最小化、外部规则核验、清结算拆分、对账差异、生产事实留存、夹具级别、准入决策和人工处理都有验收入口。 |
| 支付工具 | AC-PI-*、RED-001、RED-046、RED-047、RED-048、RED-049 | 支付工具、外部账户和通道 token 只做引用和快照；VCC 发卡账务通过卡绑定资金/信用子账户验收；绑定关系、方向、状态、资金责任决策、账户能力和绑定历史审计可验收；敏感信息不进入普通快照、日志、导出或报表。 |
...
3. 操作状态：当前是否可操作、为什么不可操作、后续处理动作是什么、由谁处理、需要什么审批或证据。

任一视图缺少事实状态、展示状态或操作状态时，只能作为内部排障线索，不得作为用户账单、商户账单、运营台、审计导出或自动放行依据。

| 使用者 | 必须看懂的问题 | 必须提供的证据 | 禁止或阻断 |
| --- | --- | --- | --- |
| 用户或客户服务 | 这笔钱为什么扣了、冻结了、退了或还没释放。 | 交易状态、金额、币种、余额桶变化、原交易引用、授权或冻结引用、失败原因、预计后续处理。 | 不得把授权占用说成最终消费；不得把冻结说成扣款成功；不得展示无法追溯来源的余额变化。 |
