...
| 清算批次确认 | 只有清算确认才允许 `CLEARING -> AVAILABLE`。 | 重大差错、账期未到、退款/争议/风控阻断、重复入批或候选未锁定必须阻断；批次原子确认，不允许部分成功。 | `AC-REC-008`、`TDD-CLS-004`、`TDD-CLS-006`。 | 已落地清算批次原子锁定、事务内最终复核、受控 `FundsClearingTransactionService`、账务引用回写和候选 `CLEARED/BLOCKED/LOCKED` 分流，并通过 `tests` 模块 H2 生产验收。争议/风控权威查询和按原资金分配处理部分退款不在当前能力范围；监控和 Runbook 属于宿主接入责任。 |
