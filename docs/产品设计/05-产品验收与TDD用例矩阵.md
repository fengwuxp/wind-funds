...
| P1 | 权益语义 | 让利出资记账交易、成本承担主体、让利承接账务主体、零实付准入、退款处置和原事实回放。 | `FundsBenefitContributionTransactionService`、让利出资记账交易请求、`fundingNature`、`fixtureLevel` 和权益准入证明。 | 02 分册让利出资记账交易入口、工程落地决策卡；03 分册权益清结算和对账拆分。 | `TDD-BEN-*`、`TDD-BEN-RED-*`、`TDD-BEN-CLS-*`、`TDD-BEN-RECON-*`。 | 恢复请求态权益快照、`CONTRACT_ONLY` 夹具或 `contextVariables` 过渡字段被用来宣称生产完成，或退款/清结算按当前营销规则重算；把非入账优惠塞进让利出资记账交易服务。 |
