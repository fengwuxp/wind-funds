...
- 当前版本：v1.0
- 文档状态：Review
- owner：VCC 产品 owner、资金产品 owner、运营 owner、风控 owner、测试 owner、架构 owner
- 发布日期：2026-07-17
- 评审结论摘要：本文定义 VCC 发卡场景如何消费统一资金底座，不承接发卡处理商协议、卡生命周期内核或独立卡号账本。
- 目标读者：VCC 产品、运营、财务、风控、合规、安全、测试、架构师和研发。
- 需要决策：确认预付卡、共享卡、资金责任、卡账单和逆向场景的产品边界与验收种子。
- 当前可信度：VCC 卡作为支付工具、资金影响落资金或信用子账户的语义已确认；外部 issuer、卡组织、PCI、法域和生产运营规则仍待专业确认。

## PRD 模板追踪索引

| PRD 模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 需求概览、背景、目标与范围 | 1、3 | 本文只定义资金底座支撑范围。 |
| 核心名相、主体、角色和对象 | 2、3.4、5 | 不新增 `VCC_ACCOUNT`。 |
| 场景、流程、状态和规则 | 6、7、13 | 覆盖授权、清算、退款、拒付和卡账单。 |
| 数据、权限、运营和风险 | 8、9、12.4 | 外部规则保持核验状态。 |
| 验收与产品到架构交接 | 10、12、13.7 | 工程准入继续由系分、TDD 和单一任务边界承接。 |
...
| --- | --- | --- | --- |
| VCC | 以虚拟卡凭证访问发卡支付网络的支付工具，以及围绕该工具形成的授权、清算、退款和争议场景集合。 | 不是资金账户、账本主体或独立资金池。 | VCC 产品系统管理支付工具；`wind-funds` 只承接其已解释的资金事实。 |
| VCC 关联子账户 | 每张 VCC 在资金底座中绑定的资金子账户或信用子账户，用于隔离资金责任、额度和账务。 | 不是 PAN、token，也不新增 `VCC_ACCOUNT` 账户类型。 | 钱包账户能力管理绑定关系和账户；VCC 产品系统管理卡生命周期。 |
| 预付卡模式 | VCC 绑定资金子账户，由该子账户及其父资金账户关系表达资金归属和余额约束。 | 不是礼品卡模型，也不是把余额记在卡片对象上。 | 资金账户承载真实资金；Spend Rule 可叠加非现金周期控制。 |
| 共享卡模式 | VCC 绑定信用子账户，以信用账户统一授权记账和额度占用，并由父资金账户承担真实资金责任。 | 不是多张卡共用一个卡号账本，也不是信用账户持有真实现金。 | 信用账户承接额度和授权占用；父资金账户承接授权资金预留与清算扣款。 |
| 卡账单 | 由支付工具、交易、账户、账本和对账事实派生的只读用户视图。 | 不是资金事实源，也不是争议案件管理系统。 | 上层 VCC 产品负责展示；资金底座提供稳定事实引用和投影。 |
...
| 核心资金账务主体 | 资金账户、信用账户及其父子账户 | `wind-funds` 内部可入账主体；平台角色必须先解析到具体平台资金账户。VCC 场景不新增 `VCC_ACCOUNT`，而是把每张 VCC 卡绑定到一个资金子账户或信用子账户。 | 资金账户承载真实资金和平台责任余额，信用账户承载授信额度和授权占用；VCC 预付卡绑定资金子账户，VCC 共享卡绑定信用子账户；支出控制范围和 Spend Rule 只承接支出控制证据。 | 是。 |
...
| VCC 钱包账户 | VCC 关联子账户、企业主账户、支出控制范围、Spend Rule、授权占用和可用余额展示。 | P0 | 钱包、账目和支出控制视图复用 | 资金子账户和信用子账户的 AVAILABLE、AUTHORIZATION、FROZEN、CLEARING 等账目可解释；多张共享卡通过各自子账户受同一主账户约束；支出控制范围和 Spend Rule 的控制视图可解释但不入账。 |
...
5. 支出控制范围和 Spend Rule 只提供使用范围、规则决策、预算控制和审计证据，不成为资金池或 LedgerEntry 主体。
...
| 共享卡 | VCC 卡产品的一种使用和绑定模式。 | 卡绑定信用子账户；多个共享卡子账户可以受同一个主信用账户、企业额度或资金主账户约束；授权时固化工具、使用人、绑定版本、父子账户、支出控制范围、Spend Rule、资金责任关系和规则版本。 | 不把共享卡、卡组、持卡人、支出控制范围或 Spend Rule 当账本主体；多卡共享不得表达为多张卡绑定同一个子账户。 |
...
| shared card | 员工、部门、预算或项目共享主账户约束下的卡额度。 | 交易当时解析出的使用人、部门、支出控制范围、Spend Rule 控制上下文，以及同一主账户下的信用子账户。 | 信用子账户，受同一主账户约束。 | 使用人、卡工具、绑定版本、父子账户、支出控制范围、Spend Rule、资金责任解析结果和授权规则快照。 |
...
| 是否承载真实资金余额、预付责任、平台责任或授信额度。 | 必须解析到资金责任来源。 | 不得入账。 | 由 FundingAccount、CreditAccount、平台责任账户或 product funding / source account 承接；支出控制范围和 Spend Rule 只承接支出控制。 |
...
| 员工、部门、项目或卡组的消费控制。 | 支出控制范围 + 预算型 Spend Rule。 | 预算 scope、规则窗口、预算可用控制视图、预算预留证据和释放证据。 | 支出控制范围控制归属范围，Spend Rule 控制能不能花，二者都不表达真实资金沉淀。 |
...
| VCC Funding Relation | childAccountRef、parentAccountRef、paymentInstrumentRef、targetSubjectType、targetSubjectId、spendControlScopeRef、spendRuleRef、cardholderRef、bindingVersion、fundingDecisionRef、currency、period。 | ACTIVE、SUSPENDED、CLOSED。 | 解释某张卡在某次交易中如何归因到 VCC 关联子账户，并解析到父账户、资金或额度责任来源；支出控制范围和 Spend Rule 只作为控制上下文和审计快照。 |
...
| VCC-R003 | 卡凭证不作为账本主体 | Card / Ledger | 生成资金动作。 | cardRef 只能作为 instrumentRef；VCC 资金影响必须解析到资金子账户或信用子账户。 | 解析到 VCC 关联子账户；背后资金责任再解析到父级 FundingAccount、父级 CreditAccount、平台责任账户或 product funding / source account；支出控制范围和 Spend Rule 只进控制快照。 | P0 | 随 VCC 场景规则确认。 | ledger entry 主体不出现 cardRef、PAN、token、支出控制范围或 Spend Rule；也不出现 `VCC_ACCOUNT`。 |
...
| VCC-R007 | 共享卡绑定快照 | Shared Card | 多使用人、部门、项目或预算使用共享卡，或多张卡共享同一主账户额度/资金约束。 | 每张卡必须绑定一个信用子账户；多张卡共享时共享的是同一个主账户约束，不是同一个子账户。 | 固化使用人、绑定版本、支出控制范围、子账户、父账户、资金责任解析关系和规则版本。 | P0 | 随 VCC 场景规则确认。 | 共享卡换绑后，退款、撤销、过期和拒付仍按原快照解释；多卡通过父账户约束和交易投影区分。 |
...
| 共享卡被误建为卡账本或共用同一子账户 | 卡组、部门、持卡人、支出控制范围、Spend Rule、PAN、token 被写成账本主体，或多张卡绕过子账户直接共用同一个子账户。 | 共享卡只表达工具使用和绑定模式；每张卡绑定一个信用子账户，多张卡通过同一父账户共享额度/资金约束，支出控制范围和 Spend Rule 只保留控制快照。 |
...
| 共享卡的使用人、部门、支出控制范围、信用子账户、父账户、资金责任解析关系和规则版本如何绑定。 | 3、5、7、10 | 中 | 发卡产品、企业管理、风控、财务 | 只保留设计边界，不开放生产自动授权。 |
...
| --- | --- | --- | --- | --- | --- |
| VCC-AC-001 | 授权批准生成占用 | 授权请求、VCC 关联子账户、金额、币种、卡引用。 | 资金子账户或信用子账户 AVAILABLE -> AUTHORIZATION，生成 route、posting、ledger entry；父账户、背后资金责任、支出控制范围和 Spend Rule 只保存快照。 | 多币种、支出控制范围、Spend Rule、父级信用账户或资金账户来源。 | 幂等重复返回原结果。 |
...
| VCC-AC-008 | 共享卡授权绑定快照 | 共享卡由员工、部门、项目或系统代理发起授权。 | 固化使用人、工具、绑定版本、支出控制范围、Spend Rule、信用子账户、父账户、资金责任解析关系和规则版本；账务主体不出现 cardholder、cardRef、PAN、token、支出控制范围或 Spend Rule。 | 工具换绑、支出控制范围变更、Spend Rule 变更、资金责任多命中。 | 缺子账户、缺父账户、缺唯一资金责任来源、缺绑定版本或缺规则版本时失败无账务副作用。 |
...
| B5-SR-CONTROL | Spend Rule 决策和预算预留释放。 | 拒绝是否可审计且无账务副作用，预算控制是否只写控制事实和只读视图。 | 规则决策、控制活动、预算控制投影；需要单独 DDL/H2 授权。 | 支出控制范围账务主体化、资金交易事实反写。 |
...
| 共享卡是绑定和归因模式 | 每张共享卡绑定一个信用子账户；多卡共享时共享同一个主账户额度或资金约束，可按使用人、部门、项目、支出控制范围和 Spend Rule 做授权控制。 | 每次授权必须固化工具、使用人、绑定版本、子账户、父账户、支出控制范围、Spend Rule、资金责任决策和 route snapshot。 |
...
| 资金责任决策 | `FundingResponsibilityResolutionApplicationService` | `com.wind.funds.wallet.application.funding` | 从卡、使用人、支出控制范围、Spend Rule、预付责任和平台角色解析最终资金或额度责任主体。 |
...
交易层目标态必须具备账户主体型授权释放、受控强制完成、无授权退款、争议裁决资金结果承接、余额控制调账审计和原路径回放能力。chargeback / dispute 仍属于案件过程，VCC facade 只在裁决需要资金处理时委派退款、追偿或清结算专项能力；不得把它们包装成统一支付工具交易内核，也不得让卡、卡组、支出控制范围或 Spend Rule 成为账务主体。
...
| 资金责任决策 | 从卡、使用人、部门、项目、支出控制范围、Spend Rule、子账户、父账户、预付责任和托管模式解析唯一资金责任来源。 | 共享卡授权、预付卡授权、充值、退款、退卡。 | `FundingAllocationDecision`、子账户、父账户、责任来源、规则版本和审计原因。 | 不把支出控制范围、卡号、PAN、token、持卡人、issuer 账户或 `VCC_ACCOUNT` 写成账本主体。 |
...
2. 控制证据：Spend Rule、支出控制范围、风控结论、规则版本、拒绝原因和人工复核记录可追溯。
...
| 授权批准 | `AVAILABLE -> AUTHORIZATION`。 | CreditAccount 或 FundingAccount 作为责任来源被快照引用；是否同步占用由切片设计确认。 | 卡、使用人、支出控制范围、Spend Rule、商户和 MCC。 |
...
| --- | --- | --- | --- | --- |
| 一张共享卡绑定一个信用子账户 | 授权时解析到该卡绑定的信用子账户，批准后 `AVAILABLE -> AUTHORIZATION`。 | 信用子账户。 | 按 cardRef、cardholderRef、departmentRef、spendControlScopeRef、spendRuleRef 和 childAccountRef 生成卡维度视图。 | 工具不可用、绑定缺失、规则拒绝或余额不足时无 route、posting、entry。 |
...
| 共享卡叠加支出控制范围 | 支出控制范围只做消费范围和控制视图，资金或额度仍落到信用子账户及其父账户约束。 | 信用子账户。 | 投影保留支出控制范围、预算预留、释放和规则版本。 | 预算不足或规则拒绝不得生成账务副作用。 |
...
| 使用人/部门/项目归因投影 | BindingSnapshot、cardholderRef、departmentRef、projectRef、spendControlScopeRef、Spend Rule 决策。 | 共享卡按使用人、部门、项目和预算拆分。 | 把归因维度建成账本主体。 |
