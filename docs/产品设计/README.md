...
| 业务目标 | 核心目标是让每一笔金额可解释、可核对、可重建。 | 系分通过交易编排、路由快照、账务计划、账本分录、余额投影、对账清算和归档重放承接。 | 每个资金变化测试必须能证明状态、金额、主体、账目、posting、entry、projection、幂等和审计。 |
...
| 资金安全 | 不允许外部账户入账、投影反写事实、冻结表达消费、对账差异直接改账或绕过账本。 | 系分承接为事务边界、唯一约束、幂等键、账本事实只追加、审批审计和失败无副作用。 | TDD 必须包含余额桶、分录平衡、route snapshot 回放、重复请求和失败无副作用断言。 |
