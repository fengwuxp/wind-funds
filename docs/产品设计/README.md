...
按当前设计基线，02 主链路可优先进入编码准入复核；03 清结算与对账、04 归档重放与指标治理具备目标态设计和 TDD 分析基础，但仍需独立工程任务确认首切片、表结构、服务级测试和外部规则状态。
...
| 产品流程 | [big-data-archive-replay-flow.svg](产品流程图/big-data-archive-replay-flow.svg) | 04 归档重放与指标治理 | 核对归档 Manifest、checkpoint、watermark、重放、差异报告和指标只读边界。 |
...
| --- | --- | --- |
| 直接交易、授权交易、余额控制、交易投影 | 对应 `AC-*`、`RED-*`、资金动作、使用者解释、失败原因、退款/撤销/过期/冻结边界和未覆盖范围。 | 清结算、对账、归档、治理、VCC/全球账户/收单特殊规则。 |
| 钱包、账本、账目和余额投影 | 资金主体、账目 bucket、账本周期、余额桶、可用/冻结/结算锁定展示口径和余额不可直接修改红线；资金账户、信用账户、预算组分别承接真实资金、授信额度和预算控制。 | 把支付工具、外部账户、业务订单或报表当内部账本主体，或因 VCC prepaid/shared 卡产品形态自动创建资金账户、信用账户或预算组。 |
| 清结算与对账 | 对账批次、差错单、清分、清算、结算、出款、追偿、运营补事实白名单、审批审计、外部规则确认和 `CLS-GATE-*`。 | 交易主链路特殊规则；未确认外部规则前不得声明生产可出款。 |
| 归档重放与指标治理 | Manifest、checkpoint、watermark、余额快照、交易投影重放、差异报告、人工处理、指标只读边界和 `GOV-GATE-*`。 | 用指标快照替代账本余额确认，或让治理任务反写资金事实。 |
| VCC、全球账户、ACH、收单 | 业务状态映射、外部引用、规则待确认、敏感数据边界、资金事实映射、P0/P1 回归和不新增平行内核承诺；VCC 需额外说明卡工具、预付资金模式、共享卡绑定快照和资金责任主体。 | 平行钱包、平行账本、平行清结算、平行对账、平行归档、外部协议全集，或把 VCC 卡、prepaid virtual card、shared card 建模为新的资金账户/账本主体。 |
...
| 未覆盖范围 | 清结算、对账、归档、治理、P2 业务专项、外部协议全集和生产 NFR 的不覆盖范围清楚。 | 缩小范围或拆独立业务/能力。 |
...
| 不做范围证据 | 本轮不做的清结算深水区、归档治理、P2 业务专项、外部协议全集、运营后台或上线合规结论。 | 防止目标态 PRD 被当成全量工程授权。 |
...
| `productNotDone` | 本轮不能声明的能力，例如完整清结算、归档治理、P2 业务生产流、外部协议全集或上线合规结论。 | 防止目标态 PRD 被误读为全量交付承诺。 |
...
| `productNotDone` | 不声明完整清结算、归档治理、P2 业务生产流或外部通道规则确认。 | 不声明清分、清算、结算、出款、调账核销全部生产完成；不自动改账。 |
...
| MVP 任务裁剪 | 每轮只选择一个最小可用场景，先证明使用者、资金事实、资金动作、失败红线和验收 ID 闭合。 | 系分只承接本轮 MVP 必须的服务入口、状态、表和观测；非必要扩展写入未覆盖范围。 | TDD 只为本轮 MVP 排首批 Red；清结算深水区、归档治理、P2 业务和外部协议拆独立任务。 |
