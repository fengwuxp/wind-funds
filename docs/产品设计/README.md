...
早期三类业务评估、能力现状对比和收单业务预研材料已移至 [../归档/设计预研/README.md](../归档/设计预研/README.md)。这些材料只作为历史背景，不替代本目录 01-08、ACH 边界文档或产品验收矩阵。
...
产品设计可以说明“应该做什么、为什么做、做到什么边界、如何验收”，但不能直接授权研发修改代码、测试、DDL 或运行时配置。进入编码前，产品侧必须把能力优先级和交付边界转成可执行批次。

| 产品能力 | 推荐进入顺序 | 产品侧必须给出的准入材料 | 不应混入 |
| --- | --- | --- | --- |
| 直接交易、授权交易、余额控制、交易投影 | 先于 P2 业务专项，作为首批主线编码候选。 | 对应 `AC-*`、`RED-*`、资金动作、使用者解释、失败原因、退款/撤销/过期/冻结边界和 Not Done。 | 清结算、对账、归档、治理、VCC/全球账户/收单特殊规则。 |
| 钱包、账本、账目和余额投影 | 所有资金动作编码前必须先证明或同步保护。 | 资金主体、账目 bucket、账本周期、余额桶、可用/冻结/结算锁定展示口径和余额不可直接修改红线。 | 把支付工具、外部账户、业务订单或报表当内部账本主体。 |
| 清结算与对账 | 独立产品验收和独立 Execution Grant。 | 对账批次、差错单、清分、清算、结算、出款、追偿、运营补事实白名单、审批审计、外部规则确认和 `CLS-GATE-*`。 | 交易主线首批编码；未确认外部规则前不得声明生产可出款。 |
| 归档重放与指标治理 | 独立产品验收和独立 Execution Grant。 | Manifest、checkpoint、watermark、余额快照、交易投影重放、差异报告、人工处理、指标只读边界和 `GOV-GATE-*`。 | 用指标快照替代账本余额确认，或让治理任务反写资金事实。 |
| VCC、全球账户、ACH、收单 | P0/P1 回归稳定后作为业务能力包进入。 | 业务状态映射、外部引用、规则待确认、敏感数据边界、资金事实映射、P0/P1 回归和不新增平行内核承诺。 | 平行钱包、平行账本、平行清结算、平行对账、平行归档或外部协议全集。 |

产品评审若发现某个能力还不能给出使用者、业务对象、规则边界、验收 ID、失败处理和 Not Done，应先补 PRD 或验收矩阵，不进入 DSL、系分或 TDD 编码准入。

### A0 产品基线核验清单

A0 产品核验只确认“要做什么、为什么做、不能做什么、如何验收”，不授权研发写生产代码、测试代码、DDL/H2 schema 或运行时配置。产品侧不能把需求优先级、设计齐备或宣讲材料直接等同为编码授权。

| 核验项 | 产品侧必须确认 | 未闭合时处理 |
| --- | --- | --- |
| 产品目标 | 本批能力解决的使用者问题、业务收益和资金底座目标一致，不偏离统一资金内核定位。 | 回到 01 或专题 PRD 补目标；不得进入 Execution Grant。 |
...
| 验收和红线 | 本批能映射到 `AC-*`、`RED-*`、`CLS-GATE-*`、`GOV-GATE-*` 或明确不适用原因。 | 补产品验收矩阵；缺红线时不能进入 TDD。 |
| 外部规则 | 轨道、银行、卡组织、PSP、ACH、FX、税务、会计、合规和敏感数据规则状态清楚。 | 写入待确认、不覆盖范围和确认责任；影响资金释放时阻断。 |
| Not Done | 清结算、对账、归档、治理、P2 业务专项、外部协议全集和生产 NFR 的不覆盖范围清楚。 | 缩小范围或拆独立业务/能力批次。 |

产品侧 A0 输出建议作为产品评审页使用，直接交给研发和测试承接：

| 输出项 | 填写口径 |
| --- | --- |
| `productBatchGoal` | 本批要解决的产品问题、目标使用者和成功标准。 |
| `acceptanceMap` | 本批对应的 `AC-*`、`RED-*`、`CLS-GATE-*`、`GOV-GATE-*` 和不适用原因。 |
| `userExplainability` | 用户、商户、运营、财务、风控或审计如何理解成功、失败、冻结、授权、结算、退款或差错。 |
| `ruleStatus` | 外部规则、专业确认、敏感数据和不覆盖范围。 |
| `productNotDone` | 产品侧明确不交付、不验收或需独立授权的范围。 |
...
| 交付边界 | 产品设计证明目标、规则和验收可理解，不证明代码、DDL、测试或外部规则已完成。 | 系分证明系统设计可实施，不替代 Execution Grant、实现证据、验证结果或上线审批。 | 可以进入 TDD 分析；进入编码仍需独立 Execution Grant 声明写入范围、禁止范围、首批 Red 和验证命令。 |
