...
| 预算组和 Spend Rule | 预算组表达预算 scope、owner、展示和审计；Spend Rule 统一表达预算额度、MCC、商户、频控和时间窗口等授权前规则。 | 使用 BudgetGroup 上下文、Spend Rule 快照、规则决策日志和预算控制投影，不新增账本余额桶。 | `BudgetGroupService` 和 Spend Rule 控制视图只管理控制证据；资金责任解析关系解析到资金账户、信用账户、VCC 关联子账户或平台账户角色。 | 覆盖预算创建不初始化 ledger、规则拒绝不生成 route/entry、预算控制占用/释放只更新控制证据。 |
