...
A0 编码准入前，产品侧必须先把一个业务问题收敛成可执行任务封面。该封面不授权编码，只给 DSL、系分、TDD 和 OpenSpec 提供同一份业务输入。当前产品编码准入以确认时 Git HEAD、OpenSpec 和 Harness 最新任务账本为准；`4a7ef12 docs: 固化代码准入 CR 基线`、`f99800b docs: 对齐代码 CR 任务基线`、`9456ab6 docs: 对齐 A0 准入与代码基线` 和 `77bc9f4 fix: 阻断钱包上下文权益核心事实` 只保留为历史准入与局部保护证据；后续编码仍必须由单一 MVP Execution Grant 授权。
