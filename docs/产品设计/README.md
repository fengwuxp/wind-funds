...
共享卡预算授权投影如要进入下一步工程准备，优先使用 [../TDD设计/B2B4-支付工具与SpendRule生产可用性Round0准入卡.md](../TDD设计/B2B4-支付工具与SpendRule生产可用性Round0准入卡.md)。该卡只把产品场景转成 Round 0 和 Execution Grant 输入，不声明预算规则引擎、授权支付工具入口或投影生产链路已完成。
...
| 支付工具入口 | 支付工具是外层业务入口、能力准入、绑定快照、路由快照和投影查询维度；直接交易、授权交易和余额控制内核仍以资金账户、信用账户或平台角色解析后的平台资金账户为 canonical 主体；`authorizeByInstrument` 只表示授权 application facade，不替换现有交易请求。 | 系分 02 承接为 canonical 交易内核 + 支付工具 application facade 两层边界，禁止新增统一 `InstrumentTransactionService` 族覆盖 P1 入口；VCC、VA、全球账户、ACH 和收单由 P2 能力包解释后映射到底座资金事实。 | TDD 需覆盖支付工具准入、绑定快照、资金责任唯一、Spend Rule 决策、拒绝无 route/posting/entry、原路径回放和支付工具不得入账。 |
