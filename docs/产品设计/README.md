...
本目录按最新产品文档模板约规分类治理，不把索引、总览、验收矩阵和专项边界强行改写成同一种 PRD。既有文件默认保留路径；文档顺序、版本和状态由本 README、文档状态头和 Git 历史承接，不通过本轮模板重构批量改名。

| 文档 | 文档类型 | 模板适配要求 | 验证类型 |
| --- | --- | --- | --- |
| 01 PRD 总览 | 产品架构总览 | 目标、能力地图、对象流程、规则、运营数据、风险和验收完整；补文档状态与模板追踪。 | `product-architecture` |
| 02 交易路由钱包账目与投影 | 产品架构分册 | 以能力域、对象、流程、状态、规则和验收组织，不复制系分实现。 | `product-architecture` |
| 03 清结算与对账 | 产品架构分册 | 补四流、清结算对象、规则、运营闭环、风险和验收；外部规则保持待专业确认。 | `product-architecture` |
| 04 资金数据治理拆分索引 | 兼容索引 | 只维护迁移位置、退役条件和权威入口，不恢复为独立 PRD。 | 链接与 Markdown 检查 |
| 05 产品验收与 TDD 用例矩阵 | 产品验收资产 | 承接验收种子、红线、证据和跨层追踪，不写成评审报告。 | `product-architecture` |
| 06 VCC、07 全球账户、08 收单、09 Spend Rule | 专项 PRD | 使用 PRD 主模板，必须有核心名相、范围、角色、对象状态、流程规则、数据审计、风险确认、验收和产品到架构交接。 | `prd` |
| ACH 与银行转账支持边界 | 产品能力边界 | 使用产品架构模板，必须有能力地图、规则矩阵、外部轨道边界和验收。 | `product-architecture` |

正式产品文档统一遵循以下规则：

1. 文档头保留当前版本、状态、owner、发布日期、评审结论、目标读者、需要决策和当前可信度；完整讨论过程和修订流水不进入正文。
2. 模板追踪索引只映射当前正文，不复制一遍正文；缺失信息写“待确认”，同时给出影响、确认方和未确认前处理。
3. 产品正文按“问题证据 -> 目标范围 -> 核心名相和主体 -> 场景流程 -> 状态规则 -> 能力数据 -> 风险 -> 验收交接”组织；章节可以按文档类型裁剪。
4. 支付资金专项必须说明主体、资金归属、四流、账务影响摘要、对账依据、外部规则核验状态和专业确认方；产品文档不替代法律、合规、税务或会计结论。
5. 正文不得保留模板占位符，不以“功能已列出”替代规则、异常、人工处理和可观察验收结果。

## 最小交付约规

产品设计默认先定义最小可交付场景，不提前追求完整资金平台或完整业务产品。最小交付必须选定一个真实使用场景，说明用户是谁、业务事实是什么、资金动作是什么、怎样证明成功、失败后是否无副作用，以及哪些扩展明确不做。

产品 PRD 中属于最小交付范围的能力，必须能被验收矩阵验证；不能验证的能力不得写成默认范围。VCC、全球账户、ACH 或收单等业务补充只允许补充最小交付场景必须的业务语义、外部引用、状态映射和红线，不得把完整发卡、完整银行转账、完整收单、完整风控、完整运营后台或全量报表提前纳入资金底座最小交付范围。

## 能力保障顺序

产品设计、系分设计、TDD 和编码工程拆解均按下列顺序保护资金底座能力，避免三类业务支持过早反向污染统一资金内核。
...
能力优先级只表达资金安全和产品依赖顺序，不表达当前工程成熟度或编码授权。P0 能力也必须按最小交付场景、验收 ID、工程写入范围、数据落点和失败红线逐项确认后，才能进入具体开发任务。

| 成熟度口径 | 产品含义 | 交付动作 |
| --- | --- | --- |
| 产品已定义 | 目标、使用者、对象、流程、规则、红线和验收 ID 已闭合。 | 可以进入系分、DSL 或 TDD 对齐，不代表代码或数据库已完成。 |
| 可进入编码准入 | 已有最小交付场景、写入范围、不做范围、失败红线、外部规则状态和验证命令。 | 可以申请单一工程任务。 |
| 生产可用 | 已有代码、DDL/H2、服务级测试、观测、Runbook、权限审计和外部规则确认。 | 才能声明生产交付或上线启用。 |

02 主链路是资金底座优先落地范围；03 清结算与对账提供目标态设计和 TDD 分析基础；原 04 归档重放与指标治理已拆入 02、03 和 05，04 仅保留兼容索引。涉及 Manifest、checkpoint、watermark、差异报告、指标水位或大数据消费边界时，应通过独立工程边界确认首切片、表结构、服务级测试和外部规则状态。
...
| 6 | [06-VCC场景资金能力边界.md](06-VCC场景资金能力边界.md) | VCC 业务、资金、研发、测试 | 说明 VCC 业务接入时的支付工具、账户、资金责任、canonical 资金动作和原路由回放边界；VCC 主交易与事件设计归 `fincone` 所有。 |
...
01-05 是资金底座主线 PRD，06 是 VCC 场景资金能力边界，07-08 是两类资金业务补充分册，09 是 Spend Rule 支出规则专项分册。06-08 不替代主线，用于说明上层场景如何通过归一资金契约接入统一资金内核；VCC 业务对象、issuer 事件、主状态、卡账单和发卡协议由 Fincone 发卡业务承接。09 不替代交易、钱包或账本主链路，只定义规则配置、准入决策、控制证据、DSL v1.1 可解释性、预算控制视图和只读解释口径。能复用的资金能力回到 01-05，上层业务专属状态机、外部协议、规则待确认项和展示视图不进入 wind-funds。商品订单交易体系属于上层业务系统，不进入本目录最终交付分册；订单、支付意图、履约、售后和退款申请应在上层业务项目承接，`wind-funds` 只提供资金底座能力。ACH 与银行转账边界文档不是业务分册，它只定义外部银行转账轨道如何被资金底座支持以及哪些能力不得沉入 `wind-funds`。
...
| VCC 场景资金边界 | [06-VCC场景资金能力边界.md](06-VCC场景资金能力边界.md) | VCC 场景如何复用资金账户、支付工具、授权、清算、退款、路由和账务能力。 |
...
产品设计进入 DSL、系分和 TDD 拆解前，必须先通过下列产品侧准入检查。完整跨文档门禁见 [../README.md#生产准入检查清单](../README.md#生产准入检查清单) 和 [../README.md#阻断和降级口径](../README.md#阻断和降级口径)。
...
| 资金责任解析关系 | 产品上回答“本次支出可由哪个内部资金或额度主体承担”，可由工具、卡绑定子账户、主账户、使用主体、支出控制范围、Spend Rule 等上下文参与筛选。 | 解析关系只提供候选与选择依据；进入资金路由后，最终主体、账目和金额由 `RouteParticipant`、`RouteLeg` 显式表达，`RoutingDecision` 只记录路径选择原因。兼容名称“资金来源关系”不得扩展为预算资金池。 | `SpendSubjectFundingRelationService` 可保留现有命名，但输出必须是资金账户、信用账户、VCC 关联子账户或平台账户角色解析后的平台资金账户；支出控制范围和 Spend Rule 只作为控制上下文。 | 覆盖缺失、不唯一、错币种、默认关系冲突、支出控制范围或 Spend Rule 被当作资金来源时失败且无账务副作用。 |
...
| 资金责任目标字段 | 已采用 `targetSubjectType + targetSubjectId`；可声明资金账户、信用账户、平台角色责任主体和 VCC 关联资金/信用子账户。 | DSL fixture、Spec 或公共字段变更必须同步摘要、route snapshot 和回放断言，不新增 `VCC_ACCOUNT`。 | DTO、DDL/H2、Entity、Mapper、service 和审计必须同步。 | TDD 必须覆盖缺失/不唯一/错币种/支出控制范围或 Spend Rule 被当最终主体失败；不得恢复旧资金账户镜像字段。 |
...
产品文档可以使用“授权结算”“授权部分结算”等业务表达，但进入 DSL、系分和 TDD 时统一映射为“授权完成”“授权部分完成”以及 AUTHORIZATION_TRANSACTION / COMPLETE。商户结算链路中的“结算锁定”表示 AVAILABLE -> SETTLEMENT，不等同于授权链路的 COMPLETE 动作。完整映射见 [../README.md#统一术语映射](../README.md#统一术语映射)。
...
| 06 VCC 场景资金能力边界 | 明确 Fincone 发卡业务如何通过归一资金契约接入 wind-funds；VCC 卡、虚拟卡和卡 token 只作为支付工具，预付卡和共享卡只在资金底座映射为账户 profile、资金责任与绑定快照。 | Fincone 确认发卡资金模式、Program/BIN/Processor/Card Network 规则、PCI 边界、授权控制规则、clearing、chargeback 证据和卡账单；wind-funds 只确认支付工具、资金责任、资金动作与账务验收。 |
...
产品侧进入生产交付评审前，必须把“设计可理解”升级为“使用者和运营可验收”。缺少下列任一项时，产品结论只能写成带条件通过、设计输入态或 未完成交付，不得声明生产可用。

| 签出项 | 产品侧必须确认 | 生产前证据 |
| --- | --- | --- |
| 使用者完成路径 | 用户、商户、运营、财务、风控、审计或 SRE 能看懂成功、失败、冻结、授权、清算、退款、争议和差错的后续处理动作。 | 页面/接口查询口径、交易投影、运营时间线、错误原因、Runbook 触发和人工处理入口。 |
...
| 未完成交付清单 | 明确哪些业务包、外部协议、运营后台、报表、规则引擎、清结算深水区或生产治理能力不在交付范围内。 | 发布说明、工程计划和残余风险清单。 |
