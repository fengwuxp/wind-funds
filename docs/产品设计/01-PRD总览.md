...
| P1 | 授权交易 | 授权批准、授权拒绝、授权撤销、授权过期、授权完成、强制完成、授权链退款、无授权直接退款和拒付承接。 | 授权成功不等于最终消费；授权拒绝不得生成 route、posting 或 ledger entry；无授权强制完成必须显式 FORCE 模式，无授权退款必须显式 NO_AUTH 或等价模式，且都要有外部原事实引用、原事实类型、凭证、原因、操作者和审计，不得补造授权占用或携带内部授权流水。 |
