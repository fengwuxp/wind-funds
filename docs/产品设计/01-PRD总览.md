...
| P1 | 交易投影 | 用户账单、商户账单、运营时间线和财务基础视图。 | transaction projection、reporting metric input boundary。 | 只读派生和可重放；不能写交易、路由、账本、余额事实或清结算对象。 | `AC-VIEW-*`、`AC-RPT-*`；`TDD-VIEW-*`、`TDD-METRIC-*`。 |
...
| P1 | 交易投影 | 用户账单、商户账单、运营时间线、财务报表基础视图和有界重放。 | 交易投影是只读读模型，不写交易、路由、账本或余额事实。 |
