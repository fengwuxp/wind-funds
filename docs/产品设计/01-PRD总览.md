...
优先级不等于编码授权。P0 表示资金底座必须优先保护的产品不变量，不表示本轮可以一次性实现全部钱包、清结算、对账、归档和余额快照。进入工程任务前，必须把目标能力拆成单一 MVP 场景，并明确产品验收 ID、写入范围、不做范围、外部规则状态、TDD Red 和验证命令。

| 成熟度层级 | 判断口径 | 当前设计承接 |
| --- | --- | --- |
| P0/P1/P2 优先级 | 产品依赖和资金安全顺序。 | 本 PRD 定义能力保障顺序和风险边界。 |
| 编码准入 | 单一场景、资金事实、服务入口、数据落点、失败红线和测试入口闭合。 | 02 主链路优先进入编码准入复核；03、04 先进入独立任务的 TDD 和系统落点复核。 |
...
| P0 | 大数据归档 | 归档 Manifest、checkpoint、watermark、余额重建、冷热查询和差异报告。 | `governance-face`、`governance-impl` 或确认后的等价治理模块；余额检查点和水位归属账本模块协作。 | 归档只改变冷热位置；余额重建只从账本事实恢复；资金冷归档不是在线报表库。 | `AC-ARCH-*`、`AC-ARCH-009`、`RED-016` 至 `RED-019`、`RED-029`；`GOV-GATE-*`、`TDD-GOV-*`、`TDD-ARCH-*`、`TDD-ARCH-010`。 |
...
| --- | --- | --- | --- | --- | --- |
| 钱包账户、支付工具、资金账户、信用账户、预算组和账本账目 | P0 | `AC-PI-*`、`AC-CTRL-*`、`AC-BALLOG-*`、`RED-043` 至 `RED-049`、`RED-067` | DSL 主体、支付工具、资金来源、账本周期；02 系分；`TDD-WALLET-*`、`TDD-LEDGER-*`、`TDD-ROUTE-*`。 | 资金主体、工具绑定、账本初始化、余额断言和模块边界必须可解释、可测试、可审计。 | 不得把 `FundingAccount` 扩大为信用账户、预算组、支付工具或钱包标识的统一父类；不得让外部账户或工具入账。 |
...
| 权益金额组件 | P1 | `AC-BEN-001` 至 `AC-BEN-019`、`RED-050` 至 `RED-066` | `FundsBenefitSnapshotSpec`、`DSL-BENEFIT-*`、02/03 系分；`TDD-BEN-*`、`TDD-BEN-RED-*`。 | 权益准入证明、原权益快照事实源、闭合角色、伴随指令、补充事实、审计证据包、解释视图和外部规则核验状态必须可追溯。 | 不得在资金底座重算券、重算当前营销规则、静默补平权益差异，或用请求样例替代生产可回放事实。 |
...
| 归档、余额快照、交易重放、大数据归档和指标边界 | P0/P1 | `AC-ARCH-*`、`AC-REPLAY-*`、`AC-RPT-*`、`RED-016` 至 `RED-019`、`RED-034`、`RED-040` 至 `RED-042` | `DSL-GOVERNANCE-*`、04 系分；`TDD-GOV-*`、`TDD-ARCH-*`、`TDD-REPLAY-*`、`TDD-METRIC-*`、`TDD-B8-RED-*`。 | Manifest、checkpoint、watermark、coverage mode、differenceReport、manualResolutionRef、冷热读取、脱敏和审计必须可验证。 | 不得缺 Manifest 推进水位；不得用普通指标快照替代余额确认；不得让治理任务反写资金事实或让数仓绕过治理读冷归档。 |
