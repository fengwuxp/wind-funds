...
| 核心能力 | 优先保障钱包账户、账本账目、余额投影、对账、清分、清算、结算、大数据归档和账本余额快照；其次保障直接交易、授权交易、余额控制和交易投影；再次保障 VCC、全球账户收付款和收单业务补充接入。 |
...
运营闭环进入交易层也不是开放通用改账入口。后续 Execution Grant 必须列出“运营补事实命令白名单”，至少说明允许的清算确认、结算锁定、出款结果、差错调账、冲正、追偿或等价命令，以及来源单据、审批号、证据引用、幂等键、原事实引用、操作者、原因和可撤销边界。未列入白名单的运营动作只能停留在处理单、差异报告或人工复核记录，不能生成资金事实。

含权益退款不是简单按比例拆金额。只要部分退款可能影响现金、商户让利、平台补贴、储值代金券、不可退权益或商户应收，Execution Grant 就必须明确退款分摊确定性规则：规则版本、分摊依据、稳定组件顺序、舍入模式、尾差归属、组件剩余额度版本、幂等摘要字段和并发保护。缺少这些规则时，含权益部分退款只能失败、转人工或停留在 contract-only 验证，不能声明生产资金流完成。

### 1.2 能力保障优先级

支付资金底座的产品落地顺序不以业务入口数量为先，而以资金事实是否可解释、可核对、可重建为先。后续任务拆解、系分落地、TDD 矩阵和编码准入均按下列优先级判断。

| 优先级 | 能力域 | 必须优先保障的原因 | 不做稳的风险 |
| --- | --- | --- | --- |
| P0 | 钱包、账本、账目、余额投影、对账、清分、清算、结算、大数据归档、账本余额快照。 | 这些能力决定资金归属、余额来源、账务事实、运营清账和历史可证明性，是所有业务共用的资金底座。 | 余额不可解释，账务不可核对，清结算只能人工修正，历史数据归档后无法证明。 |
...
产品设计目录采用“主线 PRD + 业务补充分册”的结构，而不是把所有业务细节都并入 01-05。原因是三类业务的交易层语义并不完全通用：VCC 有授权、clearing、chargeback 和卡账单语义；全球账户有外部受理、在途、退汇、FX 引用和多币种对账语义；收单有 payment attempt、capture、refund、dispute、准备金和商户结算语义。这些业务语义应通过场景交易 capability pack 接入资金底座，不应反向改变钱包、账本、账目、清结算、对账和归档的统一内核。

| 文档组 | 设计定位 | 合并口径 |
| --- | --- | --- |
| 01-05 主线 PRD | 支付资金底座的权威产品主线。 | 承载产品定位、统一能力地图、职责边界、交易/钱包/账本/投影、清结算、对账、归档、验收和生产门禁。 |
| 06 VCC 发卡业务分册 | VCC 场景资金语义补充。 | 保留 Program、Card、Cardholder、授权控制、卡组织清算和拒付语义；把授权不等于入账、卡不入账、敏感数据最小化和原路径回放抽象回 01-05。 |
...
| 钱包账户层 | 钱包账户域、资金账户、信用账户、预算组、平台资金账户角色、支付工具引用 | 表达主体、账户、余额桶、额度、预算、绑定关系和展示口径。 | 账户是事实，余额来自账本 |
...
| 钱包 | 面向用户、商户、企业和运营展示资金、额度、预算和余额状态的产品层。 | 不替代账本，不允许业务侧绕过交易层直接改余额；钱包余额可以作为前台支付方式展示，但资金域内必须解析为资金账户、信用账户或预算组后才能入账。 |
| 钱包账户域 | 钱包产品层的上位账户能力域，统一管理真实资金账户、信用账户、预算组、平台账户角色、支付工具绑定、资金来源关系和余额查询。 | 这是产品域和服务域的聚合边界，不是单一账户表；不能用 `FundingAccount` 代替信用账户、预算组或支付工具。 |
...
| 资金账户 / FundingAccount | 承载真实资金余额的账户主体，也是最常见的账务主体。 | 可以有可用、冻结、授权、待清算、出款中等余额桶；外部银行账户不是内部资金账户，不能直接作为账务主体；`FundingAccount` 只表示真实资金账户，不是所有钱包账户的统一父类。 |
...
| --- | --- | --- | --- | --- | --- |
| P0 | 钱包账户 | 账户、余额桶、信用、预算、平台账户角色和支付工具引用。 | `wallet-face`、`wallet-impl`、`ledger-face` 查询、`core` 主体和值对象。 | 管理账户能力、工具绑定、显式建账和余额查询；不直接写交易事实、账本事实或绕过交易编排。 | `AC-PI-*`、`AC-CTRL-*`、`RED-046` 至 `RED-049`；`TDD-WALLET-*`、`TDD-ROUTE-011` 至 `TDD-ROUTE-013`。 |
...
| P0 | 清分、清算与对账 | 运营闭环中的对账、清分、内部清算、差错、核销和追偿。 | `reconciliation-face`、`reconciliation-impl` 或后续确认模块；通过 transaction 和 ledger 引用资金事实。进入编码前，Execution Grant 必须明确 `CLS-GATE-*`、复用/新增/暂不落物理模块、公共契约、DDL/H2、状态机、服务级测试和验证命令。 | 清分明细和清算候选不入账；差错不直接改历史分录或余额；对账是清分、清算、结算和出款的基础准入。 | `AC-CLR-*`、`AC-SET-*`、`AC-REC-*`、`RED-030` 至 `RED-039`；`CLS-GATE-*`、`TDD-CLS-*`、`TDD-SETTLE-*`、`TDD-RECON-*`。 |
| P0 | 大数据归档 | 归档 Manifest、checkpoint、watermark、余额重建、冷热查询和差异报告。 | `governance-face`、`governance-impl` 或后续确认模块；余额检查点和水位归属账本模块协作。进入编码前，Execution Grant 必须明确 `GOV-GATE-*`、治理物理落点、依赖方向、公共契约、DDL/H2、Mapper/Entity 归属、指标水位隔离测试和边界验证。 | 归档只改变冷热位置；余额重建只从账本事实恢复；资金冷归档不是在线报表库。 | `AC-ARCH-*`、`AC-ARCH-009`、`RED-016` 至 `RED-019`、`RED-029`；`GOV-GATE-*`、`TDD-GOV-*`、`TDD-ARCH-*`、`TDD-ARCH-010`。 |
...
| P1 | 权益语义 | 交易入口上的已决策权益结果，解释订单金额、用户实付、商户应收、零实付准入、退款处置、使用者解释视图和证据边界。 | `core` 的 `FundsBenefitSnapshotSpec` 及权益组件；后续由 transaction、route、ledger、清结算、对账、投影和归档消费。 | 只承接订单、业务系统或营销权益系统已决策结果；不计算券规则、不维护券包生命周期、不用当前规则重算历史权益；含权益生产批次必须先完成权益准入证明、使用者解释视图、证据最小化和外部规则核验状态。 | `AC-BEN-*`、`RED-050` 至 `RED-066`；`DSL-BENEFIT-*`；`TDD-BEN-*`、`TDD-BEN-RED-*`。 |
...
| P1 | 指标项输入 | 指标项、业务问题、使用者、建议事实来源和只读边界。 | reporting metric input boundary；报表指标模块另行承接。 | 指标只读观察，不推进余额水位或替代 Manifest，不反写资金事实。 | `AC-RPT-*`、`TDD-METRIC-*`。 |
| P2 | 三类业务补充 | VCC 发卡、全球账户收付款和收单业务的场景语义补充。 | 场景交易 capability pack、业务 PRD 06/07/08、后续 OpenSpec 批次。 | 复用 P0/P1 能力，不改变钱包、账本、账目、清结算、对账和归档统一实现边界。 | 06/07/08 的业务验收、对应 TDD 扩展用例和外部规则待确认项。 |
...
| --- | --- | --- | --- | --- |
| 钱包账户域 | 账户、工具、资金来源和余额查询的上位能力域。 | `wallet-face`、`wallet-impl`、`FundingAccountService`、`CreditAccountService`、`BudgetGroupService`、`PaymentInstrumentService`、`SpendSubjectFundingRelationService`、`FundsSubjectBalanceQueryService`。 | 否。 | 能力域不是单表；不能被 `FundingAccount` 替代。 |
...
### 6.1.2 目标优先级与当前编码准入状态矩阵

本矩阵把产品目标优先级和当前工程准入状态拆开。P0 表示资金底座生产交付时必须优先保障的能力，不等于当前已经获得生产代码、测试代码、DDL/H2 schema 或运行时配置写入授权。进入编码时，仍以 OpenSpec、Harness Plan、Git 基线和本批次 Execution Grant 为准。

| 能力域 | 产品优先级 | 当前准入状态 | 可做事项 | 不可做事项 |
| --- | --- | --- | --- | --- |
| 钱包账户、账本、账目、余额投影 | P0 | 可进入 B2 Round 0 和独立 Execution Grant。 | 做编码前核验、补首批 Red、明确允许写入的契约、表结构、测试和验证命令。 | 未授权时不得修改生产代码、测试代码、DDL/H2 schema 或运行时配置。 |
| 直接交易、授权交易、余额控制、资金路由和交易投影 | P1 | B2 基础事实闭合后，可按 B3 至 B6 独立 Execution Grant 进入。 | 基于钱包账户、账本账目和余额断言推进交易主链路、逆向回放、投影重放和权益资金流消费。 | 不得抢跑 P0 基础事实，不得反向改变钱包、账本、账目和余额投影口径。 |
| 清分、清算、结算和对账 | P0 | 只进入 TDD 分析、契约草案和实现差距复核；整体编码准入未打开。 | 补 `CLS-GATE-*`、`TDD-B7-RED-*`、物理模块选择、DDL/H2 范围、服务级 H2 流程、权限审计和运营补事实白名单。 | 不得把模块骨架、出款前准入候选或设计矩阵声明为清结算、对账或出款生命周期 Done。 |
| 归档重放、余额快照和指标边界 | P0/P1 | 只进入 TDD 分析、契约草案和实现差距复核；整体编码准入未打开。 | 补 `GOV-GATE-*`、`TDD-B8-RED-*`、Manifest、checkpoint、watermark、差异报告、人工处理、冷热边界和指标水位隔离测试。 | 不得把交易投影重放骨架声明为归档治理、余额快照或大数据归档 Done。 |
| VCC、全球账户和收单业务能力包 | P2 | 不开放默认编码准入。 | 先完成业务分册、场景 pack、外部规则核验、归一资金动作和 P0/P1 回归范围。 | 不得新增平行钱包、平行账本、平行清结算、平行对账或平行归档链路。 |

### 6.1.3 编码准入决策卡

本决策卡用于把产品判断转成后续 Execution Grant 的必填输入。任何编码批次都必须先选定一个能力域，不允许把 B2 基础事实、B3 至 B6 交易读模型、B7 清结算对账、B8 归档治理或 P2 业务能力包混成一个大批次。

| 决策项 | 必填内容 | 未填时的处理 |
| --- | --- | --- |
| 能力域和批次 | 明确 B1、B2、B3 至 B6、B7、B8 或 P2 业务专项，并列出对应 PRD 验收 ID、DSL caseId、系分章节和 TDD Red。 | 只能继续设计、CR 或 Round 0 核验，不得进入代码写入。 |
| 基线附件 | 明确本批引用的 docs、OpenSpec、Harness Plan、Git 提交点和允许读取的未提交文档。 | 未提交设计只能作为草稿输入，不得作为 Done 证据。 |
| 物理落点 | 明确复用既有模块、新增 face/impl、暂不落物理模块或只做 contract-only；说明依赖方向和事实端口边界。 | 不得新增空壳模块，不得把对象散落到 transaction、ledger 或 wallet 中。 |
| 写入范围 | 明确是否允许改生产代码、测试代码、公共契约、枚举、Request/DTO、状态机、表结构、H2 schema、运行时配置。 | 默认不允许写入。 |
| 禁止范围 | 明确不得触碰的模块、对象、资金语义、外部规则和 Not Done 结论。 | 任一越界修改都不得进入 Done。 |
| 首批 Red | 明确必须先失败的 Red ID、目标测试资产、失败断言和验证命令。 | 只能做设计、契约草案或 dry-run，不进入 Green 实现。 |
| 资金不变量 | 明确金额闭合、主体、账目、周期、route、posting、entry、projection、幂等和失败无副作用断言。 | 资金变化场景不得声明完成。 |
| 运营和治理门禁 | 清结算、对账、归档、重放或指标边界必须列出 `CLS-GATE-*`、`GOV-GATE-*`、白名单、Manifest、checkpoint、watermark、差异报告和人工处理入口。 | 对应能力只能停留在 TDD 分析和差距复核。 |
| 外部和专业确认 | 涉及权益、客户资金、商户待结算、税务、会计、合同、卡组织、银行、通道、KYC/KYB/AML、跨境或外汇时，列出规则来源、版本、生效日期、适用法域、核验日期、确认方和确认状态。 | 不得进入生产资金流 Done 结论。 |
| 停止条件 | 明确发现基线冲突、授权不足、测试无法闭合、外部规则缺失、数据模型越界或工作树冲突时如何停止。 | 不得用继续编码替代重新确认。 |

### 6.1.4 产品准入到 Execution Grant 映射

本表用于把产品能力准入结论翻译为编码授权清单。Execution Grant 不能只写“实现某能力”，必须选择下表中的能力域，并把产品验收、DSL、系分、TDD、允许写入范围和 Not Done 红线一并列出。

| 产品能力域 | 产品验收入口 | DSL / 系分 / TDD 回挂 | Execution Grant 必填 | Not Done 红线 |
| --- | --- | --- | --- | --- |
| 钱包账户、支付工具、资金账户、信用账户、预算组和账本账目 | `AC-PI-*`、`AC-CTRL-*`、`AC-BALLOG-*`、`RED-043` 至 `RED-049`、`RED-067` | DSL 主体、支付工具、资金来源、账本周期；02 系分；`TDD-WALLET-*`、`TDD-LEDGER-*`、`TDD-ROUTE-*`。 | 允许改的账户契约、工具绑定、账本初始化、H2 schema、首批 Red、余额断言和模块边界测试。 | 不得把 `FundingAccount` 扩大为信用账户、预算组、支付工具或钱包标识的统一父类；不得让外部账户或工具入账。 |
| 直接交易、授权交易、余额控制、资金路由和交易投影 | `AC-IN-*`、`AC-PAY-*`、`AC-MER-*`、`AC-AUTH-*`、`AC-CTRL-*`、`AC-ROUTE-*`、`AC-VIEW-*` | `DSL-DIRECT-*`、`DSL-AUTH-*`、`DSL-BALANCE-CONTROL-*`、route replay、02 系分；`TDD-DIR-*`、`TDD-AUTH-*`、`TDD-CTRL-*`、`TDD-REPLAY-*`。 | 交易入口、Request/DTO、状态机、route snapshot、posting plan、幂等、原路径回放、并发和失败无副作用验证。 | 不得抢跑 P0 主体和账本不变量；不得在缺 route snapshot 时重新选路；不得让投影或报表反写事实。 |
| 权益金额组件 | `AC-BEN-001` 至 `AC-BEN-019`、`RED-050` 至 `RED-066` | `FundsBenefitSnapshotSpec`、`DSL-BENEFIT-*`、02/03 系分；`TDD-BEN-*`、`TDD-BEN-RED-*`。 | 权益准入证明、fixtureLevel、原权益快照事实源、闭合角色、伴随指令、补充事实、审计证据包、解释视图和外部规则核验状态。 | 不得用 contract-only 样例声明生产资金流 Done；不得在资金底座重算券、重算当前营销规则或把权益差异静默补平。 |
| 清分、清算、结算、出款和对账 | `AC-CLR-*`、`AC-SET-*`、`AC-REC-*`、`AC-ADJ-*`、`RED-030` 至 `RED-039` | `DSL-SETTLEMENT-*`、03 系分；`TDD-CLS-*`、`TDD-SETTLE-*`、`TDD-RECON-*`、`TDD-B7-RED-*`。 | `CLS-GATE-*`、首批 Red、物理模块、DDL/H2、服务级 H2 测试、出款前准入、金额不一致、差错核销、运营补事实白名单和职责分离。 | 不得把清分候选直接入账；不得把外部 accepted/submitted/processing 展示为到账；不得直接改历史分录或绕过重新对账关闭差错。 |
| 归档、余额快照、交易重放、大数据归档和指标边界 | `AC-ARCH-*`、`AC-REPLAY-*`、`AC-RPT-*`、`RED-016` 至 `RED-019`、`RED-034`、`RED-040` 至 `RED-042` | `DSL-GOVERNANCE-*`、04 系分；`TDD-GOV-*`、`TDD-ARCH-*`、`TDD-REPLAY-*`、`TDD-METRIC-*`、`TDD-B8-RED-*`。 | `GOV-GATE-*`、Manifest、checkpoint、watermark、coverage mode、differenceReport、manualResolutionRef、治理读取或导出快照、脱敏和审计。 | 不得缺 Manifest 推进水位；不得用普通指标快照替代余额确认；不得让治理任务反写资金事实或让数仓绕过治理读冷归档。 |
| VCC、全球账户和收单业务能力包 | `VCC-AC-*`、`GA-AC-*`、`ACQ-AC-*` 及对应 `*-RED-*` | 06/07/08 分册、P2 业务能力包 DSL 准入卡、业务系分补充；`TDD-P2-*` 和 P0/P1 回归。 | 业务分册、场景 pack、归一资金动作、外部引用脱敏字段、P0/P1 回归范围、外部规则核验状态和 Not Done 红线。 | 不得新增平行钱包、平行账本、平行清结算、平行对账或平行归档链路；不得把卡、VA、PSP、银行账户或通道对象当账本主体。 |
| ACH 和银行转账支撑边界 | `ACH-BOUNDARY-001` 至 `ACH-BOUNDARY-006`、`AC-RAIL-002A` 至 `AC-RAIL-007` | ACH 边界文档、DSL P2 业务能力包准入卡、TDD `TDD-RAIL-002` 至 `TDD-RAIL-007`、`TDD-RED-030`、`TDD-RED-034`。 | 上层业务归属、上层解释后的归一资金事实、return/NOC/reversal 责任方、submitted/accepted/processing 展示隔离、externalAccountRef/token/摘要白名单、外部规则核验状态和敏感数据最小化验证。 | 不得新增 ACH 内核模块、ACH 产品 PRD、ACH 通道能力、ACH 交易主状态机；不得解释 Nacha/ODFI/RDFI 规则、Debit 授权或 return code；不得保存完整银行账户敏感信息。 |
...
| --- | --- | --- | --- |
| VCC 发卡 | 授权不等于入账、拒绝无账务副作用、卡不作为账本主体、原路径回放、敏感数据最小化、clearing 和 dispute 可对账。 | Program、Card、Cardholder、授权控制规则、卡组织 clearing、forced post、chargeback 证据和企业卡账单解释。 | PAN/CVC、HSM、token vault、卡组织原始报文、发卡处理商协议、完整企业卡产品生命周期。 |
...
| 钱包账户层 | 管理资金账户、信用账户、预算组、支付工具引用和余额查询展示。 | 不承载交易命令，不绕过账本修余额。 | 账户、账目、余额桶、支付工具绑定快照。 |
| 路由和账务层 | 解析参与方、账户、账目、账本周期，生成 route snapshot、posting plan、ledger entry。 | 不重新计算业务规则，不把外部账户、卡、VA 或支付工具当账本主体。 | 路由快照、账务计划、账本交易、账本分录。 |
...
| 优先级 | 能力域 | 产品能力 | 能力边界 |
| --- | --- | --- | --- |
| P0 | 钱包账户 | 资金账户、信用账户、预算组、平台资金账户角色、支付工具引用、余额查询。 | 钱包表达账户能力和查询入口，不承载交易命令，不绕过账本改余额。 |
...
| P0 | 大数据归档 | 归档 Manifest、余额检查点、水位、归档清单、余额重建、冷热查询和差异报告。 | 归档只改变冷热位置，余额重建只从账本事实恢复，报表数仓不得绕过治理读取或反写资金事实。 |
| P1 | 直接交易 | 入金、出金、支付、转账、退款、费用、批次授权调账。 | 直接交易记录已确认资金事实，不替代业务订单、外部通道状态机或运营工单。 |
| P1 | 授权交易 | 授权批准、授权拒绝、授权撤销、授权完成、授权链退款和拒付承接。 | 授权成功不等于最终消费；授权拒绝不得生成 route、posting 或 ledger entry。 |
| P1 | 余额控制 | 冻结、解冻、资金账户余额调整、信用账户额度调整、预算组额度调整。 | 冻结只表达同主体可用性控制；余额调整只做同主体受控修正，不承接跨主体价值转移。 |
...
| P1 | 报表数据面 | 指标项、业务问题、使用者、建议事实来源和只读边界。 | 指标只读消费事实和投影，不反写资金事实；具体实现由报表指标模块承接。 |
| P2 | VCC 发卡业务支持 | 授权资金链、清算入账、退款、拒付、卡账单解释和发卡授权控制扩展。 | 只作为业务补充分册和场景 pack，Program、Card、PAN/CVC、HSM 和处理商协议不进入资金内核。 |
...
资金底座的发展路径按能力依赖和风险闭环排序，不按业务线先后排序。任何业务接入都必须先证明它能复用 P0/P1 能力，并且不会绕过统一账户、账本、清结算、对账和归档边界。

| 阶段 | 产品目标 | 必须稳定的能力 | 可接入的业务形态 | 进入下一阶段的门禁 |
| --- | --- | --- | --- | --- |
| 基础可信 | 每一笔余额变化都能被账本、投影和审计解释。 | 钱包账户、账本账目、余额投影、资金路由、直接交易、授权交易、余额控制、幂等和审计。 | 平台内部交易、钱包余额查询、直接交易和授权交易基础场景。 | 状态、金额、route、posting、ledger entry、余额投影、幂等和失败无副作用均有验收证据。 |
...
| 业务扩展 | VCC、全球账户收付款和收单通过业务分册和场景 pack 接入。 | 场景事件归一、业务状态映射、外部引用、规则确认、敏感数据最小化和分册验收。 | VCC 发卡、全球账户收付款、收单支付和商户结算。 | 每个业务 pack 都有独立 Execution Grant、外部规则确认台账、业务验收矩阵和生产回滚方案。 |
...
| --- | --- | --- | --- |
| 场景执行 | 目标态已覆盖直接交易、授权、冻结、退款、清算、结算、出款前准入、出款、对账、归档和重放。 | 每个场景都有触发条件、期望结果、异常分支和验收入口，出款必须先过准入门禁。 | 可用。 |
...
| 验收执行 | 已有产品验收用例矩阵和 TDD 转化路径。 | 验收覆盖正常路径、异常路径、红线失败、权限审计和数据质量；清结算、对账、归档和重放仍需在编码批次补齐服务级测试证据。 | 具备进入批次准入评审的产品输入条件，不等同于生产 Done。 |

总体判断：PRD 已经能支撑产品评审、运营评审、财务风控评审以及研发和测试理解产品边界。它不承担代码设计职责，也不替代 DDL、H2 schema、服务契约、测试资产、验证命令和合规规则核验；这些证据需要在系分、DSL、TDD 和编码批次继续闭环。
