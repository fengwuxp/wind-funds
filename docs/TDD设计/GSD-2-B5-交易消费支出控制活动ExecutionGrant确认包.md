...
| 所属阶段 | GSD-2 / B5 Spend Rule / Transaction consumption admission / idempotency semantic guard green verified。 |
...
| 写入范围 | `SpendControlActivityApplicationServiceImpl` 控制活动幂等语义一致性校验、`SpendControlTransactionConsumptionApplicationServiceTests` 与 `SpendControlActivityApplicationServiceTests` 目标回归，以及本文、LWT Goal、W5 推进计划和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `wallet/wallet-impl/src/main/java/com/wind/funds/wallet/application/spend/impl/SpendControlActivityApplicationServiceImpl.java`、`tests/src/test/java/com/wind/funds/wallet/application/spend/SpendControlTransactionConsumptionApplicationServiceTests.java`、`tests/src/test/java/com/wind/funds/wallet/application/spend/SpendControlActivityApplicationServiceTests.java`、`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、core、wallet、transaction、ledger、tests、Justfile、AGENTS.md 和最近 Git 提交。 |
| Git 策略 | 本确认包已被多轮服务层补片消费；退款引用已消费一致性守卫和交易事实累计解释金额守卫已形成服务层验证证据，本轮控制活动幂等语义一致性守卫随本次提交固化。后续若继续新 Grant，需重新确认写入范围和 Git 策略。 |
