...
| 当前状态 | `SPEND_CONTROL_ADMISSION_GREEN_VERIFIED` |
| Git / code baseline | `a1397ddf feat: 补齐对账差异报告只读查询` 是上一已提交基线；B7 对象级 Gate、清算 / 结算 consumer 和 B7 对账差异报告均已作为能力证据固化。本轮新增 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 支出控制准入快照服务层能力，验证通过后随本轮提交固化为新基线。`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据。 |
...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `SPEND_CONTROL_ADMISSION_GREEN_VERIFIED` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、账户能力来源准入、支付工具预交易快照、支出控制准入快照、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座、B7 清算 / 结算 consumer 已提交，以及 B7 差异报告 Green / Verify 已提交。 |
| 当前可执行状态 | `SPEND_CONTROL_ADMISSION_GREEN_VERIFIED` | wallet application facade 已能只读组合支付工具预交易快照和外部 Spend Rule 决策证据，形成支出控制准入结论；该结论不计算规则、不写 route、不写账本事实、不更新预算投影。 | 当前 Grant 已消费；下一步需要重新选择完整 Spend Rule 控制活动、预算控制投影、B7 报告扩展、VCC facade 等新的单一 Grant。 |
...
恢复规则：下一轮必须先读取本文、GSD-2 工作流、W5 推进计划、B5-003 余额调账审计扩展确认包和 OpenSpec tasks；若运行时 Goal objective、旧对话摘要或历史任务卡仍指向 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 或 `GSD2-B5-BALANCE-ADJUST-AUDIT-003`，以当前工作树、本文和 OpenSpec tasks 为准，将这些切片视为已消费证据，不得回退重做或沿用为新授权。若用户没有确认新的单一 Grant，只能继续低风险状态维护或只读 Gap Audit。若用户改选 B7 清算/结算 gate、B7 报告扩展、完整 Spend Rule 控制活动 / 预算控制投影、VCC facade 或 P2 业务能力，必须先同步本映射和第 8.1 节 Grant 决策账本。
...
影响范围：本轮影响 `wallet-face` 支出控制准入契约、`wallet-impl` 支出控制准入只读实现、`core` 决策结果枚举、`tests` 目标服务流测试，以及 PRD、系分、LWT Goal、GSD-2 总入口、W5、README 和 OpenSpec tasks 的状态同步；不新增 DDL/H2 schema、资金写入链路、交易内核支付工具入参、Controller、HTTP/RPC、运行时配置、外部通道、生产迁移或 Git 历史重写。后续任一候选进入编码前，必须重新确认单一 Execution Grant。
...
| 15 | Spend Rule / P2 VCC / 全球账户 | `contract-only` 起步 | 业务价值高，但必须消费账户、钱包 facade、交易投影、ledger guard、余额调账审计回链、对象级 gate、清算 / 结算 consumer、必要报告解释、账户能力来源、预交易快照和支出控制准入证据。 | 不直接写 P2 生产代码、外部轨道或通道规则。 |
...
| --- | --- | --- | --- | --- |
| 默认推荐 | B7 对账差异报告已提交，账户能力来源准入、支付工具预交易快照和支出控制准入快照已完成首轮 Green，下一轮从完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或 P2 业务中重新选择单一 Grant。 | 新的编码切片需重新确认单一 Execution Grant、首个 Red、写入范围、验证命令和停止条件。 | 若优先完整清结算生命周期、运营审批或 P2 业务，则另起专项 Grant，不复用已消费 consumer、report、account capability、pre-transaction snapshot 或 spend control admission Grant。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B7 对账差异报告确认包和 OpenSpec tasks。 |
...
| Wallet facade 补强 | 目标是补账户能力来源、预交易快照、支出控制准入或后续业务准入。 | `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已完成账户能力来源准入首轮 Green，`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已完成支付工具预交易快照首轮 Green，`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已完成外部 Spend Rule 决策证据准入首轮 Green；后续 wallet 子切片需重新确认 Grant。 | 需要完整 Spend Rule 控制活动、预算控制投影、VCC facade、外部规则最终结论或替换交易 canonical 入参时停止。 | 回写第 5.2 Wallet 矩阵和 Evidence Anchor Matrix。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费为本地 Green / Verify，当前状态 `SPEND_CONTROL_ADMISSION_GREEN_VERIFIED`。B7 对象级 Gate 基座和清算 / 结算只读 consumer 已在 `0d3f68dc` 提交，B7 差异报告已在 `a1397ddf` 提交。 |
| 写入范围 | 本轮已写入 wallet 支出控制准入 face 契约、request、DTO、core 决策枚举、impl 只读聚合、目标服务流测试和状态文档。未授权 DDL/H2 schema、资金写入链路、交易内核支付工具入参、运营后台、导出、完整清结算消费方、补事实执行、运行时配置或 Git 历史重写。 |
| 服务层边界 | 本轮只交付 wallet application service 能力；不新增 Controller、HTTP/RPC、页面、导出端点或外部适配入口。 |
| 验证命令 | `just test-one SpendControlAdmissionApplicationServiceTests tests` 已在非沙箱环境通过，3 tests / 0 failures / 0 errors；本轮收口继续运行 `just test-one SpendControlAdmissionApplicationServiceTests,PaymentInstrumentPreTransactionSnapshotApplicationServiceTests,PaymentInstrumentCapabilityApplicationServiceTests,FundingResponsibilityResolutionApplicationServiceTests,FundsAccountCapabilityApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests`、`just compile`、`just pmd` 和 `git diff --check`。 |
| 编译说明 | 本轮已改 Java、测试和文档；目标服务流测试在沙箱内触发 Spring/Redis 端口类 ApplicationContext 失败，已按项目约规在非 sandbox 复跑确认。 |
| 下一 owner | 继续编码前需重新确认完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或 P2 业务中的单一 Grant。若要扩完整清结算生命周期、补事实、运营审批或生产迁移，必须另起专项 Grant。 |
...
| 对齐当前 Git/code baseline | 本文、W5、GSD-2 工作流和 OpenSpec tasks 均校准到 `a1397ddf`；`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化，`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化，`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已在 `0d3f68dc` 提交固化，`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已在 `a1397ddf` 提交固化，本轮补齐 `GSD2-B2-SPEND-CONTROL-ADMISSION-001`；保留 `0b251593`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6`、`e81a8a25` 作为已消费证据。 | `DONE_CURRENT_BASELINE_PENDING_COMMIT`。 | 本轮提交后形成新基线；后续选择新的单一 Grant。 |
...
| 按依赖顺序推进低风险文档/计划 | AUTH 兼容、B4 投影解释 remaining、wallet 授权准入、route snapshot 回链、账户能力来源、预交易快照、ledger guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate、B7 consumer 和 B7 report 均已消费；本轮补齐支出控制准入 service、Request、DTO、enum、impl、目标测试、验证证据和 Not Done。 | `CONSUMED_GREEN`。 | 下一轮不得复用 B5-003、B7 consumer、B7 report、account capability、pre-transaction snapshot 或 spend control admission Grant；需重新确认完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或 wallet 后续单一 Grant。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费并通过目标测试；B7 报告已提交到 `a1397ddf`。 | 下一轮仍缺新的单一 Execution Grant；若改选后续能力，默认从完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展或 VCC facade 中选择；对象级 Gate、consumer、report、account capability、pre-transaction snapshot 和 spend control admission Grant 均不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；B7 差异报告已提交到 `a1397ddf`。后续不得复用已消费 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、统一支付工具交易内核、独立审计表、运营审批流、补事实执行、完整清结算生命周期或清结算补事实；若继续推进，默认重新选择完整 Spend Rule 控制活动 / 预算控制投影、VCC facade、B7 报告扩展或其他单一 Grant。
