...
| 写入文件 | `docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
...
| `B7-GATE-SCOPE-018` | 2026-06-19 | 对 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 做只读准入审计。 | 源码确认 `CheckReconciliationGateRequest.gateObjectSn` 已作为请求和决策解释字段存在，出款 preflight 也会传入 `payoutSn` 或 `settlementSn`；但 `ReconciliationDifference` 当前只保存 `blockingScope`，`ReconciliationDifferenceMapper.selectByBlockingScope` 只按租户和阻断范围查询，尚未按具体清算候选、结算单或出款单流水命中。 | 0 | 下一轮若选择清算/结算 gate，必须先确认类型级阻断还是对象级阻断；若选择对象级阻断，需要新的单一 Grant 明确公共契约、DDL/H2 schema、Entity、Mapper、Red 和兼容口径。 |
| `B7-CLSSET-GATE-GRANT-PRECHECK-019` | 2026-06-19 | 为 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 建立下一轮确认包。 | 新增 [GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)，把进入前决策收敛为 `scopeDecision=type-scope-no-schema` 和 `scopeDecision=object-scope-schema-backed` 两个可确认选项；产品建议生产可用优先对象级阻断，架构上需显式授权公共契约、DDL/H2、Entity、Mapper 和 no-false-block Red。 | 0 | 当前仍是 `READY_TO_CONFIRM_NOT_CODE_AUTHORIZED`；未确认 scopeDecision 和单一 Execution Grant 前，不写 Java、测试、DDL/H2、公共契约、Entity、Mapper、运行时配置或 Git。 |
| `B7-GATE-SCOPE-SOURCE-AUDIT-020` | 2026-06-19 | 对 B7 清算 / 结算 gate 的阻断粒度做源码级 Gap Audit。 | 源码确认 `gateObjectSn` 已在 gate request / decision 中作为消费对象字段存在，但差错登记请求、差错 DTO、阻断差错 DTO、`ReconciliationDifference`、H2 schema 和 `selectByBlockingScope` 仍只承载 `blockingScope`；对象级阻断不能靠现有类型级查询隐式实现。 | 0 | 下一轮生产可用方向优先 `scopeDecision=object-scope-schema-backed`；`type-scope-no-schema` 只能作为保守 MVP，交付时必须标为 `CONSERVATIVE_MVP_NOT_FULLY_USABLE`。 |
...
| `B7-GATE-AUTHORIZATION-WAIT-022` | 2026-06-19 | 对未确认 `scopeDecision` 的连续续跑做停止点收敛。 | B7 对象级阻断交接包已通过 Harness、产品、架构结构检查和 `git diff --check`；当前不缺更多文档细节，缺的是用户确认 `scopeDecision` 和单一 Execution Grant。 | 1 | 下一轮若仍未确认 Grant，不继续扩写同一确认包；只允许回答状态、提交用户确认后的文档，或在用户改选 B7 差异报告、wallet 预交易快照等新候选时重新开只读审计。 |
...
| Conditional Ready | 清算、结算、出款消费方接入 | 出款 preflight 已接入，清算和结算消费方仍未接入；当前 gate 底层查询按 `tenantId + blockingScope` 类型级消费，`gateObjectSn` 暂只承载消费对象解释，尚未作为差错命中键。 | `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 进入前先确认类型级阻断或对象级阻断。 |
...
| 9 | `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` | `service-flow-backed` | 出款 preflight 已消费 gate，清算/结算消费方仍未接入；进入前需确认 `gateObjectSn` 是解释字段还是对象级阻断命中键。 | 不一次性做完整清分、清算、结算、出款和补事实；未确认对象粒度前不新增 DDL/H2 或公共契约。 |
...
| --- | --- | --- | --- | --- |
| 默认推荐 | B5-003 已消费；下一轮默认重新确认 B7 清算/结算 gate 消费或 B7 差异报告。 | 用户确认新的单一 Execution Grant；资深架构师先确认消费方、只读/写侧边界、差错状态和账务副作用断言，再写首个目标 Red。 | 若用户优先 wallet，改选完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环；若优先运营审批，则另起 B5 审批闭环专项 Grant。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B5-003 确认包、B7 清算结算 Gate 确认包和 OpenSpec tasks。 |
...
| B7 清算/结算 gate 消费 | 目标是把对账 gate 从出款 preflight 扩展到清算/结算消费方。 | 用户确认 [GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md) 中的 `scopeDecision` 和单一 Execution Grant；源码 Gap Audit 已确认生产可用方向应优先对象级消费流水命中，类型级 `blockingScope` 只能作为保守 MVP。 | 需要完整清分、清算、结算、补事实执行、运营审批、生产迁移，或对象级阻断需要新增公共契约 / DDL/H2 schema / Entity / Mapper 时停止并扩大 Grant。 | 回写第 5.4 Reconciliation 矩阵、W5 Grant 队列、B7 确认包和 OpenSpec tasks。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 和 `GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已消费并通过目标测试；B5-003 确认包第 13 节已记录 Red/Green、验证、Not Done 和下一候选。 | 下一轮仍缺新的单一 Execution Grant；默认建议从 B7 清算/结算 gate 消费或 B7 差异报告重新选择，本 Grant 不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 和 `GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已消费，后续不得复用这些 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、完整预交易快照、统一支付工具交易内核、独立审计表、运营审批流、补事实执行或清结算补事实；若继续推进，默认重新确认 B7 清算/结算 gate、B7 差异报告或其他单一 Grant。
