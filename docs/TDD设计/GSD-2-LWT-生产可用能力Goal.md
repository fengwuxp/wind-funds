...
本文不是新的 PRD，不替代产品设计、DSL 设计、系分设计或 TDD 设计；也不单独替代 DDL/H2 schema 授权、公共契约变更授权、生产发布授权或外部规则专业确认。`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化为能力证据；`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化余额调账独立审计查询 Red / Green / Verify 结果；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 与 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已在 `0d3f68dc` 提交固化对象级 Gate 和清算 / 结算只读 consumer 服务。`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已在 `a1397ddf` 提交固化对账差异报告最小只读查询能力；`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已在 `021ee2ce` 提交固化支出控制准入快照；`GSD2-B5-SR-CONTROL-ACTIVITY-001` 已在 `78f7f008` 提交固化 Spend Rule 控制活动与预算控制投影首轮服务层能力。当前不再复用 B5-003、B7 对象级阻断、B7 consumer、B7 差异报告、支出控制准入或 B5 控制活动授权扩范围，后续若扩展能力必须另行确认新的单一 Grant。

| 字段 | 内容 |
| --- | --- |
| Task ID | `GSD2-LWT-PRODUCTION-CAPABILITY-GOAL-001` |
| 原子任务 | 建立 LWT 生产可用能力 Goal，聚合当前能力完备性、下一候选优先级、验证矩阵和 handoff。 |
| 所属阶段 | GSD-2 / LWT capability goal / B5 Spend Rule control activity green verified committed。 |
...
| Git / code baseline | 当前已提交代码基线为 `78f7f008 feat: 补齐支出控制活动与预算投影`；`021ee2ce feat: 补齐支出控制准入快照`、B7 对象级 Gate、清算 / 结算 consumer、B7 对账差异报告和支出控制准入快照均已作为能力证据固化。本轮新增 Spend Rule 控制活动服务层实现、H2 schema 和目标服务流测试，已通过目标测试、支出控制准入回归、compile、PMD 和 `git diff --check`，并随 `78f7f008` 提交固化。`a1397ddf feat: 补齐对账差异报告只读查询`、`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据。 |
...
| Git 策略 | B5-003 已提交到 `4ef64275`；B7 对象级阻断和清算 / 结算 consumer 已提交到 `0d3f68dc`；B7 差异报告已提交到 `a1397ddf`；支出控制准入快照已提交到 `021ee2ce`；Spend Rule 控制活动与预算控制投影已提交到 `78f7f008`。后续若继续新 Grant，需重新确认写入范围和 Git 策略。 |
...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、账户能力来源准入、支付工具预交易快照、支出控制准入快照、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座、B7 清算 / 结算 consumer、B7 差异报告、支出控制准入快照和 Spend Rule 控制活动均已提交。 |
| 当前可执行状态 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED` | wallet application facade 已新增 Spend Rule 控制活动持久化、查询和预算控制投影的首轮实现，并随 `78f7f008` 提交。 | 当前停止在新 Grant 选择门禁；若扩完整规则引擎、交易消费控制活动、VCC facade、Controller 或生产迁移，必须另起单一 Grant。 |
...
恢复规则：下一轮必须先读取本文、GSD-2 工作流、W5 推进计划、[GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md](GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md) 和 OpenSpec tasks；若运行时 Goal objective、旧对话摘要或历史任务卡仍指向 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 或 `GSD2-B5-SR-CONTROL-ACTIVITY-001`，以当前工作树、本文和 OpenSpec tasks 为准，将这些切片视为已消费证据，不得回退重做或沿用为新授权。若用户没有确认新的单一 Grant，只能继续低风险状态维护或只读 Gap Audit。若用户改选 B7 报告扩展、VCC facade、交易消费控制活动、完整 Spend Rule 规则引擎或 P2 业务能力，必须先同步本映射和第 8.1 节 Grant 决策账本。
...
| --- | --- | --- | --- | --- |
| 默认推荐 | B7 对账差异报告、账户能力来源准入、支付工具预交易快照、支出控制准入快照和 B5 Spend Rule 控制活动均已提交固化。下一轮不再默认确认已消费的 B5 控制活动 Grant。 | 重新选择一个新的单一 Grant，可在交易消费控制活动、完整 Spend Rule 规则引擎、VCC facade、B7 报告扩展、完整清结算生命周期或生产迁移中择一。 | 不复用已消费 consumer、report、account capability、pre-transaction snapshot、spend control admission 或 B5 控制活动确认包。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B5 Spend Rule 控制活动确认包和 OpenSpec tasks。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费为本地 Green / Verify 并提交到 `021ee2ce`；当前状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`。B7 对象级 Gate 基座和清算 / 结算只读 consumer 已在 `0d3f68dc` 提交，B7 差异报告已在 `a1397ddf` 提交；`GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 已完成本地 Green / Verify 并提交到 `78f7f008`。 |
...
| 验证命令 | 已执行 `just test-one SpendControlActivityApplicationServiceTests tests`、`just test-one SpendControlAdmissionApplicationServiceTests tests`、`just compile`、`just pmd` 和 `git diff --check`，均通过。 |
| 编译说明 | 本轮已改 Java、测试、公共契约和 H2 schema；目标测试、相邻回归、编译和 PMD 均已通过。 |
| 下一 owner | 若要扩完整清结算生命周期、补事实、运营审批、VCC facade、交易消费控制活动、完整 Spend Rule 规则引擎或生产迁移，必须另起专项 Grant。 |
...
| 按依赖顺序推进低风险文档/计划 | AUTH 兼容、B4 投影解释 remaining、wallet 授权准入、route snapshot 回链、账户能力来源、预交易快照、支出控制准入、ledger guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate、B7 consumer、B7 report 和 B5 Spend Rule 控制活动均已消费。 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`。 | 下一轮不得复用 B5-003、B7 consumer、B7 report、account capability、pre-transaction snapshot、spend control admission 或 B5 控制活动 Grant；若继续扩完整 Spend Rule、VCC facade、交易消费控制活动或生产迁移，必须另起单一 Grant。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 和 `GSD2-B5-SR-CONTROL-ACTIVITY-001` 已消费并通过目标测试；B7 报告已提交到 `a1397ddf`，支出控制准入快照已提交到 `021ee2ce`，Spend Rule 控制活动已提交到 `78f7f008`。 | 下一轮需重新选择单一 Grant；对象级 Gate、consumer、report、account capability、pre-transaction snapshot、spend control admission 和 B5 控制活动 Grant 均不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 和 `GSD2-B5-SR-CONTROL-ACTIVITY-001` 已消费；B7 差异报告已提交到 `a1397ddf`，支出控制准入快照已提交到 `021ee2ce`，Spend Rule 控制活动已提交到 `78f7f008`。后续不得复用已消费 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、统一支付工具交易内核、独立审计表、运营审批流、补事实执行、完整清结算生命周期或清结算补事实；若继续推进，必须重新确认新的单一 Grant。
