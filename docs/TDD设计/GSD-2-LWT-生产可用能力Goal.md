...
| B7 清算/结算 gate 消费 | 目标是把对账 gate 从出款 preflight 扩展到清算/结算消费方。 | 用户确认 [GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md) 中的 `scopeDecision` 和单一 Execution Grant；可选继续类型级 `blockingScope`，或引入对象级消费流水命中。 | 需要完整清分、清算、结算、补事实执行、运营审批、生产迁移，或对象级阻断需要新增公共契约 / DDL/H2 schema / Entity / Mapper 时停止并扩大 Grant。 | 回写第 5.4 Reconciliation 矩阵、W5 Grant 队列、B7 确认包和 OpenSpec tasks。 |
