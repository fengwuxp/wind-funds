...
本文不是新的 PRD，不替代产品设计、DSL 设计、系分设计或 TDD 设计；也不单独替代 DDL/H2 schema 授权、公共契约变更授权、生产发布授权或外部规则专业确认。`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化为能力证据；`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化余额调账独立审计查询 Red / Green / Verify 结果；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 已按 `scopeDecision=object-scope-schema-backed` 完成本地 Green；`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已完成 Red / Green / Verify，新增清算 / 结算只读 gate consumer 服务，但尚未提交。当前不再复用 B5-003、B7 对象级阻断或 B7 consumer 授权扩范围，下一轮需在 B7 差异报告、wallet 完整预交易快照、账户能力来源组合或其他单一 Grant 中重新确认。

| 字段 | 内容 |
| --- | --- |
| Task ID | `GSD2-LWT-PRODUCTION-CAPABILITY-GOAL-001` |
| 原子任务 | 建立 LWT 生产可用能力 Goal，聚合当前能力完备性、下一候选优先级、验证矩阵和 handoff。 |
| 所属阶段 | GSD-2 / LWT capability goal / B7 clearing-settlement consumer verified。 |
...
| Git / code baseline | `8ae0f90f docs: 收敛B7对象级阻断交接包` 是当前已提交基线；本轮 B7 对象级阻断和清算 / 结算 consumer 代码、测试、文档状态仍未提交。`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据。 |
| 关联入口 | [GSD-2-新基线工作流规划.md](GSD-2-新基线工作流规划.md)、[GSD-2-P0P1-LedgerWalletTransaction推进计划.md](GSD-2-P0P1-LedgerWalletTransaction推进计划.md)、[GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)、[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md)、[GSD-2-AUTH-Chargeback目标语义对齐任务卡.md](GSD-2-AUTH-Chargeback目标语义对齐任务卡.md)、[GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md](GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md)、OpenSpec `tdd-baseline-reset` tasks。 |
| Owner | AI Native 流程编排负责 Goal、Loop、状态和停止条件；产品架构专家负责业务价值、验收和 Not Done；资深架构师负责模块边界、契约、测试、验证和编码准入；用户确认单一 Grant。 |
| 写入范围 | 本文、B5-003 确认包、B7 清算/结算 Gate 确认包、B7 清算/结算真实消费方确认包、GSD-2 入口、P0/P1 LWT 推进计划、TDD README、docs README 和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
...
| Git 策略 | B5-003 已按用户提交授权在验证通过后提交到 `4ef64275`；本轮 B7 对象级阻断和清算 / 结算 consumer 仍是 `summary_only`，未获得提交授权。后续新 Grant 默认重新确认写入范围和 Git 策略。 |
...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `B7_RECON_CLSSET_CONSUMER_GREEN_VERIFIED_SUMMARY_ONLY` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座和 B7 清算 / 结算 consumer 已消费。 |
| 当前可执行状态 | `B7_RECON_CLSSET_CONSUMER_GREEN_VERIFIED_SUMMARY_ONLY` | 对账差错已能登记对象级阻断字段，gate 查询可按对象精确命中并兼容历史类型级差错；清算 / 结算 consumer 已能只读消费对象级 gate，覆盖 CLEARING 阻断、SETTLEMENT 阻断、同类型不同对象不误阻断、处理后重跑对平条件放行、非法或不完整请求拒绝和无账本副作用。 | 下一轮不得沿用本 Grant 扩完整清分、清算、结算、出款、补事实执行或生产迁移；若继续 LWT，需从 B7 差异报告、wallet 完整预交易快照、账户能力来源组合或其他单一 Grant 重新确认。 |
...
| 9 | `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` | `consumed-green-summary-only` | 已按 `scopeDecision=object-scope-schema-backed` 完成对象级阻断基座：差错事实、DTO、H2 schema、Mapper 和 gate 查询支持对象精确命中，兼容历史类型级差错。 | 本 Grant 不再复用；不一次性做完整清分、清算、结算、出款和补事实，不做生产迁移或运营后台。 |
| 10 | `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` | `consumed-green-verified-summary-only` | 已新增清算 / 结算只读 gate consumer，证明 CLEARING / SETTLEMENT 对象可真实消费对象级 gate，且目标测试、reconciliation 分组、compile 和 PMD 通过。 | 不继续沿用本 Grant 扩完整清分分佣、出款生命周期、补事实执行、运营审批、生产迁移或报表大包。 |
| 11 | B7 差异报告 / Spend Rule / P2 VCC / 全球账户 | `contract-only` 起步 | 业务价值高，但必须消费账户、钱包 facade、交易投影、ledger guard、余额调账审计回链、对象级 gate 和清算 / 结算 consumer 证据；其中 B7 差异报告作为 consumer 后的优先备选。 | 不直接写 P2 生产代码、外部轨道或通道规则。 |
...
| --- | --- | --- | --- | --- |
| 默认推荐 | `B7 差异报告`，承接已完成的对象级 Gate 基座和清算 / 结算 consumer，形成可供运营、财务和研发核对的差错解释视图。 | 用户确认新的单一 Execution Grant；资深架构师先收敛差异报告的查询对象、过滤条件、权限边界、敏感字段过滤、无资金副作用和目标 Red。 | 若用户优先 wallet，改选完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环；若优先完整清结算生命周期、运营审批或 P2 业务，则另起专项 Grant，不复用已消费 consumer Grant。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B7 清算结算 Gate 确认包、B7 清算结算真实消费方确认包和 OpenSpec tasks。 |
...
| 状态一致性 | `rg "GSD2-LWT|LWT-PRODUCTION|CONDITIONAL_DELIVERABLE_BASELINE|PARTIAL_BASELINE|Completion Audit|Evidence Anchor Matrix|单一 Grant 决策账本|Loop Progress Ledger|AuthorizationAdmissionApplicationServiceTests|LedgerNormalBalanceGuard|B7_RECON_CLSSET_CONSUMER_GREEN_VERIFIED_SUMMARY_ONLY|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001|blockingObjectType|blockingObjectSn" docs openspec` | GSD2 入口、W5、生产可用基线裁决、完成度审计、证据锚点、单一 Grant 决策账本、Loop Progress Ledger、README、OpenSpec tasks、wallet 授权准入、ledger guard、B5 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座和 B7 清算/结算 consumer 已消费证据能追踪到本文和当前基线。 |
...
| 编译和测试 | B5-003 涉及 transaction-face 查询契约、transaction-impl 只读聚合、余额调账目标服务流测试和文档状态回写，已运行目标 B5 测试、余额控制分组、边界分组、交易分组、`just compile` 和 `just pmd`；收口执行 `git diff --check`。 | Java、测试和公共契约变更均有本地验证证据。 |

## 10. 当前执行交接和 handoff

| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 对象级 Gate 基座和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 清算 / 结算只读 consumer 已完成本地 Red / Green / Verify。下一轮默认重新确认 B7 差异报告，或改选 wallet 完整预交易快照、账户能力来源组合等单一 Grant。 |
| 写入范围 | 本轮已写入差错对象级阻断字段、H2 schema、Mapper 查询、gate 决策返回字段、目标测试、本文、GSD-2 入口、P0/P1 LWT 推进计划、TDD README、docs README 和 OpenSpec tasks 状态同步。未授权生产 DDL、迁移、完整清结算消费方、补事实执行、运行时配置或 Git。 |
| 验证命令 | `just test-one ReconciliationGateApplicationServiceTests,ReconciliationDifferenceApplicationServiceTests tests`、`just test-reconciliation`、`just compile`、`just pmd` 已通过；收口执行 `git diff --check`。 |
| 编译说明 | 本轮已改 Java、测试和文档，并完成目标测试、reconciliation 分组、编译和 PMD；Spring/Redis 端口类 sandbox 失败已按环境问题在非 sandbox 复跑通过。 |
| 下一 owner | 用户确认下一单一 Grant；建议优先在 B7 差异报告、完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环中重新选择一个。若要扩完整清结算生命周期、补事实、运营审批或生产迁移，必须另起专项 Grant。 |
...
| 对齐当前 Git/code baseline | 本文、W5、GSD-2 工作流和 OpenSpec tasks 均校准到 `8ae0f90f`；`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化，`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化，`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已完成本地 Green 但尚未提交，并保留 `0b251593`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6`、`e81a8a25` 作为已消费证据。 | `DONE_CURRENT_BASELINE`。 | 后续基线从 `8ae0f90f` 加本轮未提交 B7 对象级 Gate 和 consumer 变更继续演进。 |
...
| 消费 B7 清算 / 结算真实消费方服务 | `GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md` 已从确认包演进为消费记录；新增 `ClearingSettlementGateConsumerService`、请求、结果 DTO、只读实现和目标服务流测试。Red 首次证明服务缺失；Green 后清算和结算对象能真实消费对象级 gate，且无账本副作用。 | `CONSUMED_GREEN_VERIFIED_SUMMARY_ONLY`。 | 本 Grant 已消费但未提交；下一轮不得沿用它扩完整清分、清算、结算、出款、补事实、运营审批或生产迁移。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已消费并通过目标测试。 | 下一轮仍缺用户确认单一 Execution Grant；默认建议 B7 差异报告，或改选 wallet 完整预交易快照、账户能力来源组合、Spend Rule 控制闭环；对象级 Gate 和 consumer Grant 均不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已消费，后续不得复用这些 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、完整预交易快照、统一支付工具交易内核、独立审计表、运营审批流、补事实执行、完整清结算生命周期或清结算补事实；若继续推进，默认重新确认 B7 差异报告、wallet 后续能力或其他单一 Grant。
