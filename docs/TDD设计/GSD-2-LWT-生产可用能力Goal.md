...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、账户能力来源准入、支付工具预交易快照、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座、B7 清算 / 结算 consumer 已提交，以及 B7 差异报告 Green / Verify 已提交。 |
| 当前可执行状态 | `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` | wallet application facade 已能只读组合支付工具能力、资金责任和账户能力，解析最终资金账户或信用账户目标主体快照；该快照不执行交易、不写 route、不写账本事实。 | 当前 Grant 已消费；下一步需要重新选择 B7 报告扩展、Spend Rule 控制闭环、VCC facade 等新的单一 Grant。 |
...
恢复规则：下一轮必须先读取本文、GSD-2 工作流、W5 推进计划、B5-003 余额调账审计扩展确认包和 OpenSpec tasks；若运行时 Goal objective、旧对话摘要或历史任务卡仍指向 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 或 `GSD2-B5-BALANCE-ADJUST-AUDIT-003`，以当前工作树、本文和 OpenSpec tasks 为准，将这些切片视为已消费证据，不得回退重做或沿用为新授权。若用户没有确认新的单一 Grant，只能继续低风险状态维护或只读 Gap Audit。若用户改选 B7 清算/结算 gate、B7 报告扩展、Spend Rule 控制闭环、VCC facade 或 P2 业务能力，必须先同步本映射和第 8.1 节 Grant 决策账本。
...
现状：`GSD2-B7-RECON-GATE-CONSUME-002` 已在 `a38776c5` 提交，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 和 `GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING` 已推进到 `ca603eab`；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 与 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已在 `dd442888` 前完成本地 Green，wallet 授权准入能从支付工具解析到账户主体、委派授权内核，并把支付工具引用、绑定版本和准入决策固化到 route snapshot。`GSD2-LD-LEDGER-GUARD-REGRESSION-001` 已在 `0b251593` 固化固定账目类别正常余额方向 guard 首轮。`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 完成 route snapshot 审计回链子切片，`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交余额调账独立审计查询最小服务流。B7 对象级 Gate 基座和清算 / 结算只读 consumer 已在 `0d3f68dc` 提交固化。`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已在 `a1397ddf` 提交单笔只读差异报告查询。`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已完成账户能力来源准入首轮 Green，`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已完成支付工具预交易快照首轮 Green：wallet application facade 能只读组合支付工具能力、资金责任和账户能力，并输出最终资金账户或信用账户目标主体快照。ledger、wallet、transaction 和 reconciliation 已完成多个 service-flow-backed 首轮 Green，但仍存在 Spend Rule 控制闭环、VCC facade、B7 批次报告 / 导出 / 运营后台、完整清分 / 清算 / 结算生命周期、运营审批闭环和生产权限模型等生产可用缺口。

影响范围：本轮影响 `wallet-face` 预交易快照契约、`wallet-impl` 预交易快照只读聚合实现、`tests` 目标服务流测试，以及 LWT Goal、GSD-2 总入口、W5、README 和 OpenSpec tasks 的状态同步；不新增 DDL/H2 schema、资金写入链路、交易内核支付工具入参、Controller、HTTP/RPC、运行时配置、外部通道、生产迁移或 Git 历史重写。后续任一候选进入编码前，必须重新确认单一 Execution Grant。
...
| Conditional Ready | wallet application facade | 已能完成授权准入最小服务流、route snapshot 支付工具引用回链、账户能力来源准入和支付工具预交易快照首轮 Green；预交易快照仍是只读准入快照，不执行交易。 | 新的 Spend Rule 控制、VCC facade 或业务场景接入 Grant。 |
| Not Done | 钱包账户聚合、Spend Rule 控制闭环、VCC facade、完整业务策略准入 | 仍不能声明 wallet 全量生产 Done。 | 按业务价值拆成 contract-only 或 service-flow-backed 小切片。 |
...
| 钱包准入基线 | wallet application facade 至少能解析账户层级、资金责任、账户能力和支付工具能力，并把能力快照交给交易内核。 | 资金责任解析 facade、支付工具能力准入 facade、账户能力来源准入 facade、授权准入 facade、授权 route snapshot 支付工具回链和支付工具预交易快照已完成首轮 Green。 | `CONDITIONAL_DELIVERABLE_BASELINE`。 | 进入 VCC、全球账户或 Spend Rule 控制前，必须消费预交易快照并补业务策略准入或说明为何本切片不需要。 |
| 交易内核基线 | 直接交易、授权交易、余额控制和原路径回放继续以账户主体作为 canonical 入参，支付工具只在外层解析。 | B4、B5、B7、wallet 授权准入、预交易快照和本轮 ledger guard 切片已证明 route snapshot、失败无副作用、只读解释、支付工具快照回链、账户主体委派边界、账目方向护栏和余额调账审计查询。 | `CONDITIONAL_DELIVERABLE_BASELINE`。 | 下一步优先补 Spend Rule 控制闭环、B7 报告扩展或 VCC facade，继续保持 canonical 请求不替换为支付工具引用。 |
...
| 14 | Spend Rule / P2 VCC / 全球账户 | `contract-only` 起步 | 业务价值高，但必须消费账户、钱包 facade、交易投影、ledger guard、余额调账审计回链、对象级 gate、清算 / 结算 consumer、必要报告解释、账户能力来源和预交易快照证据。 | 不直接写 P2 生产代码、外部轨道或通道规则。 |
...
| --- | --- | --- | --- | --- |
| 默认推荐 | B7 对账差异报告已提交，账户能力来源准入和支付工具预交易快照已完成首轮 Green，下一轮从 Spend Rule 控制闭环、B7 报告扩展、VCC facade 或 P2 业务中重新选择单一 Grant。 | 新的编码切片需重新确认单一 Execution Grant、首个 Red、写入范围、验证命令和停止条件。 | 若优先完整清结算生命周期、运营审批或 P2 业务，则另起专项 Grant，不复用已消费 consumer、report、account capability 或 pre-transaction snapshot Grant。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B7 对账差异报告确认包和 OpenSpec tasks。 |
...
| Wallet facade 补强 | 目标是补预交易快照、Spend Rule 控制闭环或后续业务准入。 | `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已完成账户能力来源准入首轮 Green，`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已完成支付工具预交易快照首轮 Green；后续 wallet 子切片需重新确认 Grant。 | 需要 VCC facade、外部规则最终结论或替换交易 canonical 入参时停止。 | 回写第 5.2 Wallet 矩阵和 Evidence Anchor Matrix。 |
...
| 状态一致性 | `rg "GSD2-LWT|LWT-PRODUCTION|CONDITIONAL_DELIVERABLE_BASELINE|PARTIAL_BASELINE|Completion Audit|Evidence Anchor Matrix|单一 Grant 决策账本|Loop Progress Ledger|AuthorizationAdmissionApplicationServiceTests|FundsAccountCapabilityApplicationServiceTests|PaymentInstrumentPreTransactionSnapshotApplicationServiceTests|GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001|GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001|LedgerNormalBalanceGuard|B7_RECON_DIFFERENCE_REPORT_GREEN_VERIFIED_COMMITTED|GSD2-B7-RECON-DIFFERENCE-REPORT-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001|blockingObjectType|blockingObjectSn" docs openspec` | GSD2 入口、W5、生产可用基线裁决、完成度审计、证据锚点、单一 Grant 决策账本、Loop Progress Ledger、README、OpenSpec tasks、wallet 授权准入、账户能力来源准入、支付工具预交易快照、ledger guard、B5 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座、B7 清算/结算 consumer 和 B7 差异报告 Green 结果能追踪到本文和当前基线。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已消费为本地 Green / Verify，当前状态 `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED`。B7 对象级 Gate 基座和清算 / 结算只读 consumer 已在 `0d3f68dc` 提交，B7 差异报告已在 `a1397ddf` 提交。 |
| 写入范围 | 本轮已写入 wallet 支付工具预交易快照 face 契约、request、DTO、impl 只读聚合、目标服务流测试和状态文档。未授权 DDL/H2 schema、资金写入链路、交易内核支付工具入参、运营后台、导出、完整清结算消费方、补事实执行、运行时配置或 Git 历史重写。 |
| 服务层边界 | 本轮只交付 wallet application service 能力；不新增 Controller、HTTP/RPC、页面、导出端点或外部适配入口。 |
| 验证命令 | `just test-one PaymentInstrumentPreTransactionSnapshotApplicationServiceTests tests` 已在非沙箱环境通过，2 tests / 0 failures / 0 errors；`just test-one PaymentInstrumentPreTransactionSnapshotApplicationServiceTests,PaymentInstrumentCapabilityApplicationServiceTests,FundingResponsibilityResolutionApplicationServiceTests,FundsAccountCapabilityApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 已在非沙箱环境通过，14 tests / 0 failures / 0 errors；`just compile`、`just pmd` 和 `git diff --check` 通过。 |
| 编译说明 | 本轮已改 Java、测试和文档；同一组合回归在沙箱内触发 Spring/Redis 端口类 ApplicationContext 失败，已按项目约规在非 sandbox 复跑确认。 |
| 下一 owner | 继续编码前需重新确认 Spend Rule 控制闭环、B7 报告扩展、VCC facade 或 P2 业务中的单一 Grant。若要扩完整清结算生命周期、补事实、运营审批或生产迁移，必须另起专项 Grant。 |
...
| 对齐当前 Git/code baseline | 本文、W5、GSD-2 工作流和 OpenSpec tasks 均校准到 `a1397ddf`；`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化，`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化，`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已在 `0d3f68dc` 提交固化，`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已在 `a1397ddf` 提交固化，本轮补齐 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`；保留 `0b251593`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6`、`e81a8a25` 作为已消费证据。 | `DONE_CURRENT_BASELINE_COMMITTED`。 | 后续选择新的单一 Grant。 |
...
| 按依赖顺序推进低风险文档/计划 | AUTH 兼容、B4 投影解释 remaining、wallet 授权准入、route snapshot 回链、账户能力来源、预交易快照、ledger guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate、B7 consumer 和 B7 report 均已消费；本轮补齐支付工具预交易快照 service、Request、DTO、impl、目标测试、验证证据和 Not Done。 | `CONSUMED_GREEN`。 | 下一轮不得复用 B5-003、B7 consumer、B7 report、account capability 或 pre-transaction snapshot Grant；需重新确认 Spend Rule 控制闭环、B7 报告扩展、VCC facade 或 wallet 后续单一 Grant。 |
...
| 证明三模块全量生产可用 | 当前只具备条件可交付基线，仍缺 Spend Rule 控制闭环、VCC facade、失败态全量解释、projection store、治理重放、B7 批次报告 / 导出、完整清分/清算/结算/出款生命周期、补事实、运营审批、生产迁移、灰度和告警。 | `NOT_DONE`。 | 不得标记 Goal complete；继续按单一 Grant 收敛缺口。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费并通过目标测试；B7 报告已提交到 `a1397ddf`。 | 下一轮仍缺新的单一 Execution Grant；若改选后续能力，默认从 Spend Rule 控制闭环、B7 报告扩展或 VCC facade 中选择；对象级 Gate、consumer、report、account capability 和 pre-transaction snapshot Grant 均不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；B7 差异报告已提交到 `a1397ddf`。后续不得复用已消费 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、统一支付工具交易内核、独立审计表、运营审批流、补事实执行、完整清结算生命周期或清结算补事实；若继续推进，默认重新选择 Spend Rule 控制闭环、VCC facade、B7 报告扩展或其他单一 Grant。
