...
本文不是新的 PRD，不替代产品设计、DSL 设计、系分设计或 TDD 设计；也不单独替代 DDL/H2 schema 授权、公共契约变更授权、生产发布授权或外部规则专业确认。`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已在 `da3b4f19` 提交固化为能力证据；`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已在 `4ef64275` 提交固化余额调账独立审计查询 Red / Green / Verify 结果；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 与 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已在 `0d3f68dc` 提交固化对象级 Gate 和清算 / 结算只读 consumer 服务。`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已在 `a1397ddf` 提交固化对账差异报告最小只读查询能力；`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已在 `021ee2ce` 提交固化支出控制准入快照。本轮已按用户确认进入 `GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 首轮实现并完成目标测试、支出控制准入回归、compile 和 PMD 验证；当前不再复用 B5-003、B7 对象级阻断、B7 consumer、B7 差异报告或支出控制准入授权扩范围，后续若提交或扩展能力必须另行确认 Git 策略或新的单一 Grant。

| 字段 | 内容 |
| --- | --- |
| Task ID | `GSD2-LWT-PRODUCTION-CAPABILITY-GOAL-001` |
| 原子任务 | 建立 LWT 生产可用能力 Goal，聚合当前能力完备性、下一候选优先级、验证矩阵和 handoff。 |
| 所属阶段 | GSD-2 / LWT capability goal / B5 Spend Rule control activity green verified。 |
...
| Git / code baseline | 当前已提交代码基线为 `021ee2ce feat: 补齐支出控制准入快照`；B7 对象级 Gate、清算 / 结算 consumer、B7 对账差异报告和支出控制准入快照均已作为能力证据固化。本轮工作树新增 Spend Rule 控制活动服务层实现、H2 schema 和目标服务流测试，已通过目标测试、支出控制准入回归、compile 和 PMD，但尚未提交。`a1397ddf feat: 补齐对账差异报告只读查询`、`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据。 |
...
| 写入范围 | 本文、B5 Spend Rule 控制活动确认包、GSD-2 入口、P0/P1 LWT 推进计划、TDD README、docs README 和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
...
| Git 策略 | B5-003 已提交到 `4ef64275`；B7 对象级阻断和清算 / 结算 consumer 已提交到 `0d3f68dc`；B7 差异报告已提交到 `a1397ddf`；支出控制准入快照已提交到 `021ee2ce`。后续若继续新 Grant，需重新确认写入范围和 Git 策略。 |
...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、账户能力来源准入、支付工具预交易快照、支出控制准入快照、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座、B7 清算 / 结算 consumer、B7 差异报告和支出控制准入快照均已提交，以及本轮 Spend Rule 控制活动已完成本地 Green / Verify。 |
| 当前可执行状态 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` | wallet application facade 已新增 Spend Rule 控制活动持久化、查询和预算控制投影的首轮实现。 | 当前停止在提交或新 Grant 选择门禁：若提交，只提交本 Task 独立切片；若扩完整规则引擎、交易消费控制活动、VCC facade、Controller 或生产迁移，必须另起单一 Grant。 |
...
恢复规则：下一轮必须先读取本文、GSD-2 工作流、W5 推进计划、[GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md](GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md) 和 OpenSpec tasks；若运行时 Goal objective、旧对话摘要或历史任务卡仍指向 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 或 `GSD2-B7-RECON-DIFFERENCE-REPORT-001`，以当前工作树、本文和 OpenSpec tasks 为准，将这些切片视为已消费证据，不得回退重做或沿用为新授权。若用户没有确认新的单一 Grant，只能继续低风险状态维护或只读 Gap Audit。若用户改选 B7 报告扩展、`GSD2-B5-SR-CONTROL-ACTIVITY-001`、VCC facade 或 P2 业务能力，必须先同步本映射和第 8.1 节 Grant 决策账本。
...
| 状态 | 能力 | 当前结论 | 下一动作 |
| --- | --- | --- | --- |
| Ready | funding account / credit account、账户层级来源、资金责任目标主体、资金责任解析 facade、支付工具能力准入 facade、授权准入 facade、预交易快照 facade 和支出控制准入 facade 最小服务流 | 已完成首轮服务流 Green，支付工具仍是能力入口和快照维度，不是账务主体。 | 后续作为 VCC、全球账户和完整 Spend Rule 控制活动的被依赖能力。 |
| Conditional Ready | wallet application facade | 已能完成授权准入最小服务流、route snapshot 支付工具引用回链、账户能力来源准入、支付工具预交易快照和支出控制准入快照首轮 Green；这些快照均为只读准入证据，不执行交易。 | 新的完整 Spend Rule 控制活动、预算控制投影、VCC facade 或业务场景接入 Grant。 |
| Not Done | 钱包账户聚合、完整 Spend Rule 控制活动、预算控制投影、VCC facade、完整业务策略准入 | 仍不能声明 wallet 全量生产 Done。 | 按业务价值拆成 contract-only 或 service-flow-backed 小切片。 |
...
| 16 | P2 VCC / 全球账户 | `contract-only` 起步 | 业务价值高，但必须消费账户、钱包 facade、交易投影、ledger guard、余额调账审计回链、对象级 gate、清算 / 结算 consumer、必要报告解释、账户能力来源、预交易快照、支出控制准入和后续控制活动证据。 | 不直接写 P2 生产代码、外部轨道或通道规则。 |
...
| --- | --- | --- | --- | --- |
| 默认推荐 | B7 对账差异报告已提交，账户能力来源准入、支付工具预交易快照和支出控制准入快照已完成首轮 Green；下一轮默认确认 [GSD2-B5-SR-CONTROL-ACTIVITY-001](GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md)，补 Spend Rule 控制活动与预算控制投影最小服务层能力。 | 用户确认 `Execution Grant：GSD2-B5-SR-CONTROL-ACTIVITY-001`，并选择 `schemaDecision=ddl-backed` 或 `contract-only`。 | 若优先 B7 报告扩展、完整清结算生命周期、运营审批、VCC facade 或 P2 业务，则另起专项 Grant，不复用已消费 consumer、report、account capability、pre-transaction snapshot、spend control admission 或本确认包。 | 本文第 8 节、第 10 节、第 11 节、W5 推进计划、B5 Spend Rule 控制活动确认包和 OpenSpec tasks。 |
...
| Wallet facade 补强 | 目标是从账户能力来源、预交易快照和支出控制准入继续推进到控制活动和预算控制投影。 | `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已完成账户能力来源准入首轮 Green，`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已完成支付工具预交易快照首轮 Green，`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已完成外部 Spend Rule 决策证据准入首轮 Green；`GSD2-B5-SR-CONTROL-ACTIVITY-001` 确认包已准备。 | 需要规则引擎、VCC facade、外部规则最终结论、替换交易 canonical 入参或生产迁移时停止。 | 回写第 5.2 Wallet 矩阵、Evidence Anchor Matrix 和 B5 Spend Rule 控制活动确认包。 |
...
| 编译和测试 | 支出控制准入涉及 wallet-face application 契约、Request/DTO、core 决策枚举、wallet-impl 只读聚合和目标服务流测试；本轮收口运行目标 wallet application 组合回归、`just compile`、`just pmd` 和 `git diff --check`。 | Java、测试和公共契约变更均有本地验证证据。 |

## 10. 当前执行交接和 handoff

| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费为本地 Green / Verify 并提交到 `021ee2ce`；当前状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED`。B7 对象级 Gate 基座和清算 / 结算只读 consumer 已在 `0d3f68dc` 提交，B7 差异报告已在 `a1397ddf` 提交；本轮 `GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 已完成本地 Green / Verify，尚未提交。 |
| 写入范围 | 本轮新增 Spend Rule 控制活动 application 契约、Request / Query / DTO、活动类型枚举、wallet-impl Entity / Mapper / Service、H2 `t_spend_control_activity` 和目标服务流测试，并同步状态文档。未授权交易内核支付工具入参、运营后台、导出、完整清结算消费方、补事实执行、运行时配置或 Git 历史重写。 |
| 服务层边界 | 本轮只交付 wallet application service 能力；不新增 Controller、HTTP/RPC、页面、导出端点或外部适配入口。 |
| 验证命令 | 已执行 `just test-one SpendControlActivityApplicationServiceTests tests`、`just test-one SpendControlAdmissionApplicationServiceTests tests`、`just compile` 和 `just pmd`，均通过；收口前仍需保留 `git diff --check` 作为最终空白和 patch 检查。 |
| 编译说明 | 本轮已改 Java、测试、公共契约和 H2 schema；目标测试、相邻回归、编译和 PMD 均已通过。 |
| 下一 owner | 若用户要求提交，先执行 `git diff --check` 后提交本 Task 独立切片；若要扩完整清结算生命周期、补事实、运营审批、VCC facade 或生产迁移，必须另起专项 Grant。 |
| 交接要求 | 后续每轮必须先读取本文、B5 Spend Rule 控制活动确认包、GSD-2 工作流、W5 推进计划和 OpenSpec tasks，再确认新的单一 Execution Grant、写入范围、禁止范围、验证命令和回滚提示。 |
...
| 按依赖顺序推进低风险文档/计划 | AUTH 兼容、B4 投影解释 remaining、wallet 授权准入、route snapshot 回链、账户能力来源、预交易快照、支出控制准入、ledger guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate、B7 consumer 和 B7 report 均已消费；本轮已执行 `GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 首轮实现。 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED`。 | 下一轮不得复用 B5-003、B7 consumer、B7 report、account capability、pre-transaction snapshot 或 spend control admission Grant；若继续扩完整 Spend Rule、VCC facade、交易消费控制活动或生产迁移，必须另起单一 Grant。 |
...
| 准备 Spend Rule 控制活动 Grant | [GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md](GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md) 已补 `GSD2-B5-SR-CONTROL-ACTIVITY-001` 的目标、能力地图、对象、规则矩阵、候选服务、Red、验证命令、停止条件和可复制授权文本。 | `CONSUMED_BY_SR_CONTROL_ACTIVITY_GREEN_VERIFIED`。 | 用户已确认 Execution Grant 与 `schemaDecision=ddl-backed`，本轮已完成首轮实现和本地验证；提交或扩展能力需遵循新的 Git 策略或单一 Grant。 |
| 执行 Spend Rule 控制活动 Grant | 本轮已按用户确认进入 `GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 首轮实现，新增 application 契约、请求、查询、DTO、活动类型、Entity、Mapper、Service、H2 `t_spend_control_activity` 和目标服务流测试。 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED`。 | 目标测试 6 tests、支出控制准入回归 3 tests、`just compile` 和 `just pmd` 已通过；本切片只声明服务层最小能力 Green，不外推为完整 Spend Rule 引擎、交易消费控制活动、VCC facade、Controller 或生产迁移 Done。 |
...
| 证明三模块全量生产可用 | 当前只具备条件可交付基线，仍缺完整 Spend Rule 控制活动、预算控制投影、VCC facade、失败态全量解释、projection store、治理重放、B7 批次报告 / 导出、完整清分/清算/结算/出款生命周期、补事实、运营审批、生产迁移、灰度和告警。 | `NOT_DONE`。 | 不得标记 Goal complete；继续按单一 Grant 收敛缺口。 |
...
| Engineering Handoff Card | `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费并通过目标测试；B7 报告已提交到 `a1397ddf`，支出控制准入快照已提交到 `021ee2ce`。 | 下一轮默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`；对象级 Gate、consumer、report、account capability、pre-transaction snapshot 和 spend control admission Grant 均不得复用。 | 资深架构师 + 用户。 |
...
三卡裁决：当前三卡可用于下一轮编码准入交接，但不能替代新的 Execution Grant、测试通过、Git 授权或上线审批。`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；B7 差异报告已提交到 `a1397ddf`，支出控制准入快照已提交到 `021ee2ce`。后续不得复用已消费 Grant 继续扩完整 dispute case、projection store、治理重放、VCC facade、Spend Rule 策略引擎、统一支付工具交易内核、独立审计表、运营审批流、补事实执行、完整清结算生命周期或清结算补事实；若继续推进，默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`。
...
验证边界：本轮 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 为 Java、测试和文档状态回写切片，已运行目标支出控制准入测试，并收口运行 wallet application 组合回归、LWT Goal / W5 / GSD-2 状态一致性检索、`compile`、`pmd` 和 `git diff --check`。

## 12. 停止条件

1. 需要越出本轮 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 写入范围，继续写 DDL/H2 schema、Entity、Mapper、状态机、Controller、HTTP/RPC、完整 Spend Rule 控制活动、预算控制投影、VCC facade 或运行时配置。
