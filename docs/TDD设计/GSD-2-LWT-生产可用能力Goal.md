...
| 所属阶段 | GSD-2 / LWT capability goal / BudgetGroup non-ledger subject green verified committed。 |
| Goal ID | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` |
| Loop ID | `GSD2-LWT-PRODUCTION-CAPABILITY-LOOP-2026-06-18` |
| 当前状态 | `BUDGET_GROUP_NON_LEDGER_SUBJECT_GREEN_VERIFIED_COMMITTED` |
| Git / code baseline | 当前已提交代码基线以 Git HEAD `a5b12a3f feat: 收敛资金服务层交付基线` 为准；本提交固化权益让利 `settle/refund` 服务层收敛、支付工具授权入口投影解释和预算组创建非建账服务层切片。预算组切片证明 `BudgetGroupService#createBudgetGroup` 不再调用 `SubjectLedgerInitializer`，LIFETIME、MONTHLY 和 CUSTOM_CYCLE 预算组创建不产生 ledger bucket、LedgerTransaction、LedgerEntry 或余额投影；目标测试、相邻回归、`compile`、`pmd` 和 `git diff --check` 已通过。`78f7f008 feat: 补齐支出控制活动与预算投影`、`021ee2ce feat: 补齐支出控制准入快照`、`a1397ddf feat: 补齐对账差异报告只读查询`、`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据。 |
...
| LWT 子 Goal | `GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` / `BUDGET_GROUP_NON_LEDGER_SUBJECT_GREEN_VERIFIED_COMMITTED` | 聚合 ledger、wallet、transaction 和 reconciliation 的条件生产基线、完备性矩阵、三卡交接和下一 Grant 队列。 | 继续作为当前 LWT 状态载体，记录 wallet 授权准入、授权 route snapshot 回链、账户能力来源准入、支付工具预交易快照、支出控制准入快照、ledger normal balance guard、AUTH 兼容 guard、B4 投影解释矩阵、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 gate 基座、B7 清算 / 结算 consumer、B7 差异报告、Spend Rule 控制活动和预算组创建非建账均已形成证据；本轮预算组非建账切片已随 `a5b12a3f` 提交。 |
| 当前可执行状态 | `BUDGET_GROUP_NON_LEDGER_SUBJECT_GREEN_VERIFIED_COMMITTED` | `BudgetGroupService#createBudgetGroup` 已收敛为预算控制对象创建，不再隐式调用 `SubjectLedgerInitializer` 自动初始化 ledger bucket；需要预算组控制账本的兼容测试显式调用初始化器。 | 当前停止在新 Grant 选择门禁；若扩预算组 route/posting/余额查询/余额控制兼容退出、完整规则引擎、交易消费控制活动、VCC facade、Controller 或生产迁移，必须另起单一 Grant。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前可执行任务 | `GSD2-B2-BUDGET-GROUP-NON-LEDGER-SUBJECT-001` 已随 `a5b12a3f` 完成服务层 Red / Green / Verify 并提交。当前没有可继续复用的 active Grant；下一轮应重新选择单一 Grant，例如交易消费控制活动、完整 Spend Rule 规则引擎、预算组兼容退出、VCC facade、B7 报告扩展或完整清结算生命周期中的一个。 |
