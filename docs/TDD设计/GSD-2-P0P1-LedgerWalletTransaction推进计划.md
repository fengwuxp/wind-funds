...
| 当前状态 | `ACCOUNT_CAPABILITY_SOURCE_GREEN_VERIFIED` |
| 上游输入 | GSD-2 W1 基线差距审计、W2 单一 Grant 选择卡、W3 B2 账户层级 CAD 准入草案、W4 B2 Execution Grant 确认包、当前 PRD/DSL/系分/TDD/OpenSpec、本轮支付工具授权准入和 route snapshot 回链提交、`GSD2-B7-RECON-GATE-CONSUME-002` 本地 Green 证据、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费证据、`a1397ddf feat: 补齐对账差异报告只读查询` 当前已提交代码基线、[GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)、[GSD-2-B7-对账差异报告ExecutionGrant确认包.md](GSD-2-B7-对账差异报告ExecutionGrant确认包.md)、[GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)、[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md)、[GSD-2-AUTH-Chargeback目标语义对齐任务卡.md](GSD-2-AUTH-Chargeback目标语义对齐任务卡.md)、[GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md](GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md) 和 [GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md](GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md)。 |
...
状态层级：`GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` 是 GSD-2 父 Goal，`GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` 是当前 LWT 子 Goal，当前可执行状态以 `ACCOUNT_CAPABILITY_SOURCE_GREEN_VERIFIED` 为准。完整映射见 [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md) 第 1.1 节；历史 B7、B2、AUTH 兼容 adapter、B4 remaining、wallet route snapshot、账户能力来源、ledger guard、B5-002、B5-003、B7 对象级 Gate、B7 consumer 或 B7 差异报告状态只作为已消费证据，不作为下一步授权。
...
运行时恢复口径：若当前会话 Goal objective、旧摘要或历史任务卡仍写着优先补齐 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、B7 `scopeDecision` 待确认或清算 / 结算真实消费方待确认，以当前 Git/code baseline、LWT Goal 和 OpenSpec tasks 为准；这些切片已经完成 Green 并成为已消费证据，不得作为下一轮默认任务或授权依据。2026-06-19 已按恢复请求重跑授权准入目标测试 3 tests 和授权准入组合回归 40 tests，均在沙箱外通过，B2 授权准入只作为被依赖基线继续维护；后续 wallet 方向应改选完整预交易快照或 Spend Rule 控制闭环。
...
| 当前状态 | `ACCOUNT_CAPABILITY_SOURCE_GREEN_VERIFIED` |
| 下一建议 Grant | B7 对象级 Gate 基座、清算 / 结算只读 consumer、B7 差异报告最小查询和账户能力来源准入均已完成，不再需要确认 `scopeDecision`、复用 consumer Grant、复用 report Grant 或复用 account capability Grant。若继续编码，则重新选择 wallet 完整预交易快照、Spend Rule 控制闭环或 B7 报告扩展的新单一 Grant。 |
| 下一 Red | 若改选 B7 报告扩展，首个 Red 应证明批次范围、导出、运营审批或更完整证据链中的一个最小缺口；若改选 wallet，则按完整预交易快照或 Spend Rule 控制闭环的候选重建首个 Red。 |
...
| 状态一致性 | `rg "GSD2-W5|P0P1-LWT|GSD2-AUTH-CHARGEBACK-TARGET-ALIGN|GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER|FundsAccountCapabilityApplicationServiceTests|GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001|LedgerNormalBalanceGuard|B7_RECON_DIFFERENCE_REPORT_GREEN_VERIFIED_COMMITTED|GSD2-B7-RECON-DIFFERENCE-REPORT-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001|blockingObjectType|blockingObjectSn" docs openspec` | GSD2 入口、AUTH Chargeback 目标语义任务卡、兼容入口确认包、B7 清算结算 Gate 确认包、B7 清算结算真实消费方记录、B7 对账差异报告 Green 结果、账户能力来源准入、README 和 OpenSpec tasks 能追踪到本文、ledger guard Green 结果、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座和清算 / 结算 consumer。 |
...
| Not Done | 完整清分清算结算出款、追偿、账龄升级、运营后台、生产迁移脚本、B7 批次报告 / 导出、补事实命令执行服务、运营审批流、余额调账独立审计表/审批流、退款/no-auth refund/释放/拒付全量投影解释矩阵、projection store、治理重放、钱包账户聚合、完整预交易快照、Spend Rule 控制闭环、VCC / 全球账户 / 清结算 P2 场景。 |
...
| Grant Decision Ledger | `GSD-2-LWT-生产可用能力Goal.md` 第 8.1 节已补单一 Grant 决策账本：`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 和 `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已消费；当前默认从 wallet 预交易快照、Spend Rule 控制闭环、B7 报告扩展或 P2 业务能力中重新确认进入条件、停止条件和状态回写位置。 |
...
| 下一候选顺序 | 若继续 wallet，则只能在完整预交易快照、Spend Rule 控制闭环或 VCC facade 中重新确认单一 Grant；若继续 B7，则选择 B7 报告扩展或完整清结算生命周期中的单一切片。 |
