...
| 所属阶段 | GSD-2 Wave 5 / P0-P1 dependency planning / B7 difference report verified。 |
| 关联 Goal | `GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` |
| 当前状态 | `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` |
| 上游输入 | GSD-2 W1 基线差距审计、W2 单一 Grant 选择卡、W3 B2 账户层级 CAD 准入草案、W4 B2 Execution Grant 确认包、当前 PRD/DSL/系分/TDD/OpenSpec、本轮支付工具授权准入和 route snapshot 回链提交、`GSD2-B7-RECON-GATE-CONSUME-002` 本地 Green 证据、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费证据、`a1397ddf feat: 补齐对账差异报告只读查询` 当前已提交代码基线、[GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)、[GSD-2-B7-对账差异报告ExecutionGrant确认包.md](GSD-2-B7-对账差异报告ExecutionGrant确认包.md)、[GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)、[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md)、[GSD-2-AUTH-Chargeback目标语义对齐任务卡.md](GSD-2-AUTH-Chargeback目标语义对齐任务卡.md)、[GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md](GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md) 和 [GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md](GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md)。 |
| Owner | AI Native 流程编排负责顺序和门禁；产品架构专家确认业务价值、验收和 Not Done；资深架构师确认源码锚点、写入范围、Red、验证命令和 CAD 停止条件；用户确认单一 Grant。 |
| 写入范围 | 本文、LWT 生产可用能力 Goal、B7 对账差异报告确认包、GSD-2 入口、TDD README、docs README 和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B7-对账差异报告ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、ledger、wallet、transaction、core、tests、Justfile、AGENTS.md、最近 Git 提交和旧 GSD/Grant 历史材料。 |
| Git 策略 | B7 差异报告已提交到 `a1397ddf`；后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
| 服务层边界 | 当前 Goal 只推进服务层能力；本轮 B7 差异报告不新增 Controller、HTTP/RPC、页面、导出端点或外部适配入口。 |

状态层级：`GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` 是 GSD-2 父 Goal，`GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` 是当前 LWT 子 Goal，当前可执行状态以 `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` 为准。完整映射见 [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md) 第 1.1 节；历史 B7、B2、AUTH 兼容 adapter、B4 remaining、wallet route snapshot、账户能力来源、预交易快照、ledger guard、B5-002、B5-003、B7 对象级 Gate、B7 consumer 或 B7 差异报告状态只作为已消费证据，不作为下一步授权。

Wave 边界：本文最初只做任务规划和设计落地；用户后续已确认 `Execution Grant：GSD2-B2-ACCOUNT-HIERARCHY-CONTRACT-001` 并进入首个 Red。Red 已证明当前授权服务流缺少合法账户层级来源；随后 `GSD2-B2-ACCOUNT-HIERARCHY-SOURCE-CONTRACT-002` 补齐来源契约，`GSD2-B2-FR-TARGET-001` 采用 `ddl-backed targetSubjectType + targetSubjectId` 补齐资金责任目标主体，`GSD2-B2-WALLET-APPLICATION-FACADE-001` 补齐资金责任解析 facade，`GSD2-B2-WALLET-APPLICATION-FACADE-002 / B2-PI-CAP-CAD-001` 补齐支付工具能力准入 facade，`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-001` 补齐交易投影解释首轮只读查询能力，`GSD2-B5-BALANCE-ADJUST-AUDIT-001` 补齐外部余额异常纠偏和受控负可用调账的首轮审计准入，`GSD2-B7-RECON-DIFFERENCE-MVP-001` 补齐对账差错登记、处理回链、重新对账幂等和无资金副作用首轮闭环，`GSD2-B7-RECON-DIFFERENCE-MVP-002` 补齐差错处理动作白名单上下文守卫，`GSD2-B7-RECON-GATE-CONSUME-001` 补齐清算、结算和出款准入的只读差错消费决策，`GSD2-B7-RECON-GATE-CONSUME-002` 已把 gate 决策接入出款前准入消费方。上述已完成切片均保留 Not Done 边界；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 已补 contract/design-only 任务卡，确认 dispute / chargeback 是案件过程，资金结果默认由 `settleRefund` 争议字段承接，现有 `chargeback` 仅作为兼容资产；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成兼容说明、最小审计 guard、兼容测试和状态回写；`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 已完成授权争议退款和 remaining 矩阵；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 已完成支付工具授权准入最小服务流；`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已完成授权准入后的支付工具引用、绑定版本和准入决策 route snapshot 回链；`GSD2-LD-LEDGER-GUARD-REGRESSION-001` 已补 ledger 固定账目类别正常余额方向 guard 首轮；`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已补余额调账 route snapshot 审计回链；`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已补余额调账独立审计查询最小服务流；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 已按对象级 schema-backed 完成差错阻断基座；`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已完成清算 / 结算只读 consumer 服务流并提交到 `0d3f68dc`；`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已完成本地 Red / Green / Verify。下一步不得复用 B5-003、B7 对象级 Gate、B7 consumer 或 B7 差异报告 Grant 扩审批、补事实、批次报告、导出或完整清结算，若继续推进需先确认提交或从 wallet 后续能力中重新选择单一 Grant。

运行时恢复口径：若当前会话 Goal objective、旧摘要或历史任务卡仍写着优先补齐 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、B7 `scopeDecision` 待确认或清算 / 结算真实消费方待确认，以当前 Git/code baseline、LWT Goal 和 OpenSpec tasks 为准；这些切片已经完成 Green 并成为已消费证据，不得作为下一轮默认任务或授权依据。2026-06-19 已按恢复请求重跑授权准入目标测试 3 tests 和授权准入组合回归 40 tests，均在沙箱外通过，B2 授权准入只作为被依赖基线继续维护；后续 wallet 方向应改选 Spend Rule 控制闭环或 VCC facade。
...
| 当前状态 | `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` |
| 状态载体 | 本文、GSD-2 新基线工作流规划、LWT 生产可用能力 Goal、B7 对账差异报告确认包、AUTH Chargeback 目标语义任务卡、AUTH Chargeback 兼容入口确认包、W2/W3/W4 B2 账户层级文档、TDD README、docs README、OpenSpec tasks。 |
| 反馈源 | checker、`rg`、`git status --short`、`git diff --check`、目标测试、compile、pmd、用户确认。 |
| 验证者 | 产品语义由产品架构专家确认；工程边界由资深架构师确认；优先级和 Grant 由用户确认。 |
| 默认推进 | 当前来源契约、资金责任目标主体、资金责任解析 facade、支付工具能力准入 facade、账户能力来源准入 facade、支付工具预交易快照 facade、授权准入 facade、授权 route snapshot 回链、交易投影解释、余额调账审计、对账差错闭环、差错处理动作上下文守卫、B7 准入消费、出款 preflight 首个消费方接入、ledger 正常余额方向 guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer 和 B7 差异报告均已完成目标测试、分组回归、规约扫描、状态回写；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 已完成 contract/design-only 任务卡，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成确认包；`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING` 已完成解释矩阵；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 与 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已完成支付工具授权准入和快照回链。下一轮若继续编码，应重新确认 Spend Rule 控制闭环、VCC facade 或 B7 报告扩展。 |
...
| 交接 | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；B7 报告已提交到 `a1397ddf`。若继续推进，改选 wallet 后续能力 / B7 报告扩展的新单一 Grant。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前 Wave / Task | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、B7 对象级 Gate 基座和 B7 清算 / 结算 consumer 均已消费；`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已提交到 `a1397ddf`。 |
| 当前状态 | `WALLET_PRE_TRANSACTION_SNAPSHOT_GREEN_VERIFIED` |
| 下一建议 Grant | B7 对象级 Gate 基座、清算 / 结算只读 consumer、B7 差异报告最小查询、账户能力来源准入和支付工具预交易快照均已完成，不再需要确认 `scopeDecision`、复用 consumer Grant、复用 report Grant、复用 account capability Grant 或复用 pre-transaction snapshot Grant。若继续编码，则重新选择 Spend Rule 控制闭环、VCC facade 或 B7 报告扩展的新单一 Grant。 |
| 下一 Red | 若改选 B7 报告扩展，首个 Red 应证明批次范围、导出、运营审批或更完整证据链中的一个最小缺口；若改选 wallet，则按 Spend Rule 控制闭环或 VCC facade 的候选重建首个 Red。 |
| 写入范围 | 来源契约已写入 core port、wallet-face/impl、H2 schema、授权路由接入和目标测试；资金责任目标主体已写入 wallet Request/DTO/Query、Entity、H2 schema、资源服务校验和目标测试；wallet application facade 首轮已写入资金责任解析契约、支付工具能力准入契约、支付工具授权准入契约、Request/DTO、实现、route snapshot 回链和目标测试；ledger guard 首轮已写入 `LedgerNormalBalanceGuard`、账本创建、入账、余额投影入口和目标测试；B4 投影解释已写入 transaction-face projection 查询契约、解释来源、transaction-impl 查询实现和服务流测试；B5 余额调账审计已写入 balance adjust 请求审计字段、外部异常来源类型、instruction context keys、转换器校验/透传、route snapshot 审计摘要和服务流测试；B7 差错闭环已写入 reconciliation application 契约、Request/DTO、差错枚举、Entity/Mapper、H2 schema 和服务流测试；B7 准入消费已写入 reconciliation gate application 契约、Request/DTO、决策枚举、Mapper 只读查询、实现和服务测试；B7 出款准入消费已写入 `PayoutOrderServiceImpl` gate 调用、阻断原因映射、证据合并和服务流测试；B7 对象级 Gate 已写入差错阻断对象字段、H2 schema、Mapper 对象级查询、gate 返回字段和目标测试；B7 清算 / 结算 consumer 已写入 reconciliation-face 请求/结果/服务契约、reconciliation-impl 只读实现和目标服务流测试。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、ledger、wallet、transaction、core、tests、Justfile、AGENTS.md、最近 Git 提交和历史准入卡。 |
| 验证命令 | wallet 授权准入、route snapshot 回链、账户能力来源和预交易快照已执行目标服务流、组合回归、`just compile`、`just pmd`、`git diff --check` 和状态结构检查；ledger guard 已执行目标 ledger 测试、`just test-ledger`、`just compile`、`just pmd` 和结构检查；B5-002 已执行目标 Red/Green、`just test-one FundsBalanceAdjustAuditFlowTests tests`、`just test-balance-control`、`just compile`、`just pmd` 和 `git diff --check`；B5-003 已执行 `just test-one LedgerDtoContextVariablesContractTests tests`、`just test-one FundsBalanceAdjustAuditFlowTests tests`、`just test-balance-control`、`just test-boundary`、`just test-transaction`、`just compile` 和 `just pmd`；B7 对象级 Gate 和清算 / 结算 consumer 已执行目标测试、`test-reconciliation`、`compile` 和 `pmd`。B4 remaining、B5-001、B7 和 `e81a8a25 verify-cad` 保留为历史已消费验证证据；下一 Grant 后运行对应目标测试、`just compile`、`git diff --check`，必要时补分组测试和 `pmd`。 |
| Git 策略 | 当前 B5-002 已验证 Green 并提交固化到 `da3b4f19`；B5-003 已验证 Green 并提交固化到 `4ef64275`；B7 对象级 Gate 和清算 / 结算 consumer 已提交固化到 `0d3f68dc`；B7 差异报告已提交固化到 `a1397ddf`。后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
| 交接要求 | 若用户确认下一 Grant，先读取对应确认包、消费第 11 节预检清单和第 12 节运行卡，再写最小 Red、最小 Green；完成后回写本文第 4 节 Grant 队列、OpenSpec tasks、验证证据、Not Done 和残余风险。 |
...
| 状态一致性 | `rg "GSD2-W5|P0P1-LWT|GSD2-AUTH-CHARGEBACK-TARGET-ALIGN|GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER|FundsAccountCapabilityApplicationServiceTests|PaymentInstrumentPreTransactionSnapshotApplicationServiceTests|GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001|GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001|LedgerNormalBalanceGuard|B7_RECON_DIFFERENCE_REPORT_GREEN_VERIFIED_COMMITTED|GSD2-B7-RECON-DIFFERENCE-REPORT-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001|blockingObjectType|blockingObjectSn" docs openspec` | GSD2 入口、AUTH Chargeback 目标语义任务卡、兼容入口确认包、B7 清算结算 Gate 确认包、B7 清算结算真实消费方记录、B7 对账差异报告 Green 结果、账户能力来源准入、预交易快照、README 和 OpenSpec tasks 能追踪到本文、ledger guard Green 结果、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座和清算 / 结算 consumer。 |
| 空白和 Markdown | `git diff --check` | 无行尾空白或 patch 格式问题。 |
| 编译、测试、PMD | `WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-one ReconciliationDifferenceReportApplicationServiceTests tests`、`just test-reconciliation`、`just compile`、`just pmd`、`git diff --check`。 | B7 差异报告目标测试、对账分组、编译和 PMD 已通过；沙箱内 embedded Redis 端口限制已在非沙箱复跑确认。 |
...
| Not Done | 完整清分清算结算出款、追偿、账龄升级、运营后台、生产迁移脚本、B7 批次报告 / 导出、补事实命令执行服务、运营审批流、余额调账独立审计表/审批流、退款/no-auth refund/释放/拒付全量投影解释矩阵、projection store、治理重放、钱包账户聚合、Spend Rule 控制闭环、VCC / 全球账户 / 清结算 P2 场景。 |
...
| Git / code baseline | 当前已提交代码基线为 `a1397ddf feat: 补齐对账差异报告只读查询`；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已完成 Red / Green / Verify 并提交固化。`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链` 和 `0b251593 feat: 补齐账本正常余额方向护栏` 保留为已消费能力证据，`dd442888` 和 `ca603eab` 保留为上一已消费证据。 |
| 新增状态载体 | [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)。 |
| 裁决 | 当前不再把 `GSD2-B7-RECON-GATE-CONSUME-002`、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 或 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 作为下一候选；这些切片均已完成并成为已消费证据。下一轮默认按用户优先级切 B7 差异报告或 wallet 后续能力。 |
...
| 不可外推范围 | Spend Rule 控制闭环、VCC facade、失败态全量解释、projection store、独立 ledger guard 回归包、差异报告、完整清分 / 清算 / 结算生命周期、补事实命令执行、运营审批、生产迁移、灰度、告警、VCC / 全球账户 / 收单 P2 业务。 |
...
| Grant Decision Ledger | `GSD-2-LWT-生产可用能力Goal.md` 第 8.1 节已补单一 Grant 决策账本：`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 和 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已消费；当前默认从 Spend Rule 控制闭环、B7 报告扩展、VCC facade 或 P2 业务能力中重新确认进入条件、停止条件和状态回写位置。 |
...
| 下一候选顺序 | 若继续 wallet，则只能在 Spend Rule 控制闭环或 VCC facade 中重新确认单一 Grant；若继续 B7，则选择 B7 报告扩展或完整清结算生命周期中的单一切片。 |
