...
| 所属阶段 | GSD-2 Wave 5 / P0-P1 dependency planning / B5 transaction spend control activity refund reference guard green verified。 |
...
| 12 | `GSD2-B5-SR-TRANSACTION-CONSUME-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004` | wallet / transaction 服务层控制活动消费 | 已完成本地 Green / Verify / Commit；依赖支出控制准入、支付工具预交易快照、Spend Control Activity、预算控制投影和账户主体型交易内核。 | 成功交易记录 `CONSUMED` 控制消耗；失败 / 拒绝 / 过期的非退款交易记录 `RELEASED`；已有退款资金事实记录 `REFUND_COMPENSATED` 控制补偿；`REFUND` 交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放；非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿；退款交易引用原资金交易但该原交易没有对应 `CONSUMED` 控制活动时不得生成退款控制补偿；缺原控制活动、金额超占用、币种 / 主体不一致或幂等摘要冲突 fail-fast；并发同流水同摘要唯一键冲突回读既有活动。 | 已写 wallet-face application/spend 契约、Request、必要活动类型、wallet-impl 服务实现、H2 schema / Entity 字段和目标服务流测试；并发硬化、退款释放守卫、退款事实守卫、退款补偿入口守卫和退款已消费引用守卫只改服务层边界，不改支付工具 `REFUND` 能力方向。 | 本轮目标复跑 `SpendControlTransactionConsumptionApplicationServiceTests` 10 tests 通过，`SpendControlActivityApplicationServiceTests` 6 tests、`SpendControlAdmissionApplicationServiceTests` 3 tests、compile、pmd、diff 和边界扫描保留为历史已消费证据。 | 不复用本 Grant 扩交易 canonical 入参、统一支付工具交易内核、Controller、HTTP/RPC、事件消费者、完整规则引擎、VCC facade、生产迁移、ledger posting 或支付工具 `REFUND` 方向规则。 |
...
| 验证命令 | wallet 授权准入、route snapshot 回链、账户能力来源、预交易快照和支出控制准入已执行目标服务流、组合回归、`compile`、`pmd`、`git diff --check` 和状态结构检查；ledger guard 已执行目标 ledger 测试、`test-ledger`、`compile`、`pmd` 和结构检查；B5-002 已执行目标 Red/Green、`FundsBalanceAdjustAuditFlowTests`、`test-balance-control`、`compile`、`pmd` 和 `git diff --check`；B5-003 已执行 `LedgerDtoContextVariablesContractTests`、`FundsBalanceAdjustAuditFlowTests`、`test-balance-control`、`test-boundary`、`test-transaction`、`compile` 和 `pmd`；B5 控制活动已执行 `SpendControlActivityApplicationServiceTests`、`SpendControlAdmissionApplicationServiceTests`、`compile`、`pmd` 和 `git diff --check`；B5 交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫和退款已消费引用守卫已执行 `SpendControlTransactionConsumptionApplicationServiceTests` 10 tests、历史服务层组合回归 16 tests、`compile`、`pmd`、`git diff --check` 和边界扫描；B7 对象级 Gate 和清算 / 结算 consumer 已执行目标测试、`test-reconciliation`、`compile` 和 `pmd`。B4 remaining、B5-001、B7 和 `e81a8a25 verify-cad` 保留为历史已消费验证证据；下一 Grant 后运行对应目标测试、`just compile`、`git diff --check`，必要时补分组测试和 `pmd`。 |
| Git 策略 | 当前 B5-002 已验证 Green 并提交固化到 `da3b4f19`；B5-003 已验证 Green 并提交固化到 `4ef64275`；B7 对象级 Gate 和清算 / 结算 consumer 已提交固化到 `0d3f68dc`；B7 差异报告已提交固化到 `a1397ddf`；支出控制准入快照已提交固化到 `021ee2ce`；B5 控制活动已提交固化到 `78f7f008`；B5 交易消费支出控制活动已提交固化到 `b29a5df`；本轮退款已消费引用守卫随本提交固化。后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
...
| 验证证据 | `SpendControlTransactionConsumptionApplicationServiceTests` 5 tests、`SpendControlActivityApplicationServiceTests` 6 tests、`SpendControlAdmissionApplicationServiceTests` 3 tests、`just compile`、`just pmd` 和 `git diff --check` 均通过；后续退款释放守卫、退款事实守卫、退款补偿入口守卫和退款已消费引用守卫补片已将交易消费目标测试扩展到 10 tests。 |
