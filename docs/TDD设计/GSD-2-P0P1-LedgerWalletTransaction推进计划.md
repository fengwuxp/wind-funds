...
| 所属阶段 | GSD-2 Wave 5 / P0-P1 dependency planning / B7 consumer verified。 |
| 关联 Goal | `GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` |
| 当前状态 | `B7_RECON_DIFFERENCE_REPORT_GRANT_READY_TO_CONFIRM` |
...
| 默认推进 | 当前来源契约、资金责任目标主体、资金责任解析 facade、支付工具能力准入 facade、授权准入 facade、授权 route snapshot 回链、交易投影解释、余额调账审计、对账差错闭环、差错处理动作上下文守卫、B7 准入消费、出款 preflight 首个消费方接入、ledger 正常余额方向 guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate 基座和 B7 清算 / 结算 consumer 已完成目标测试、分组回归、规约扫描、状态回写并提交到 `0d3f68dc`；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 已完成 contract/design-only 任务卡，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成确认包；`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING` 已完成解释矩阵；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 与 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已完成支付工具授权准入和快照回链。下一轮默认确认 B7 差异报告；若用户优先 wallet，再确认完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环。 |
...
| 交接 | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已消费并提交；`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 确认包已准备。若继续推进，默认确认 B7 差异报告或改选 wallet 后续能力；未确认时继续低风险状态维护。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前 Wave / Task | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、B7 对象级 Gate 基座和 B7 清算 / 结算 consumer 均已消费并提交到 `0d3f68dc`；`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 确认包已准备。 |
| 当前状态 | `B7_RECON_DIFFERENCE_REPORT_GRANT_READY_TO_CONFIRM` |
| 下一建议 Grant | B7 对象级 Gate 基座和清算 / 结算只读 consumer 已完成，不再需要确认 `scopeDecision` 或复用 consumer Grant。下一轮优先确认 `GSD2-B7-RECON-DIFFERENCE-REPORT-001`；若用户优先 wallet，再选完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环。 |
| 下一 Red | 若改选 B7 差异报告，首个 Red 应证明报告能解释差错状态、阻断对象、处理动作、重跑结果和准入消费证据；若改选 wallet，则按完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环的候选重建首个 Red。 |
...
| Git 策略 | 当前 B5-002 已验证 Green 并提交固化到 `da3b4f19`；B5-003 已验证 Green 并提交固化到 `4ef64275`；B7 对象级 Gate 和清算 / 结算 consumer 已提交固化到 `0d3f68dc`。下一 Grant 未确认提交授权前继续保持 `summary_only`。 |
...
| 状态一致性 | `rg "GSD2-W5|P0P1-LWT|GSD2-AUTH-CHARGEBACK-TARGET-ALIGN|GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER|LedgerNormalBalanceGuard|B7_RECON_DIFFERENCE_REPORT_GRANT_READY_TO_CONFIRM|GSD2-B7-RECON-DIFFERENCE-REPORT-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001|blockingObjectType|blockingObjectSn" docs openspec` | GSD2 入口、AUTH Chargeback 目标语义任务卡、兼容入口确认包、B7 清算结算 Gate 确认包、B7 清算结算真实消费方记录、B7 对账差异报告确认包、README 和 OpenSpec tasks 能追踪到本文、ledger guard Green 结果、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座和清算 / 结算 consumer。 |
| 空白和 Markdown | `git diff --check` | 无行尾空白或 patch 格式问题。 |
| 编译、测试、PMD | `WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-one ClearingSettlementGateConsumerServiceTests tests`、`just test-reconciliation`、`just compile`、`just pmd`、`git diff --check`。 | B7 清算 / 结算 consumer 目标测试、对账分组、编译和 PMD 已通过；收口执行 diff 证据。 |
...
| Grant Decision Ledger | `GSD-2-LWT-生产可用能力Goal.md` 第 8.1 节已补单一 Grant 决策账本：`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已消费，`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已准备确认包；当前默认确认 B7 差异报告，或改选 wallet 预交易快照、账户能力来源组合、Spend Rule 控制闭环或 P2 业务能力的进入条件、停止条件和状态回写位置。 |
