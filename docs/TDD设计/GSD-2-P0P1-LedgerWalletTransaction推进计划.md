...
| 所属阶段 | GSD-2 Wave 5 / P0-P1 dependency planning / B5 Spend Rule control activity green verified。 |
| 关联 Goal | `GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` |
| 当前状态 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` |
| 上游输入 | GSD-2 W1 基线差距审计、W2 单一 Grant 选择卡、W3 B2 账户层级 CAD 准入草案、W4 B2 Execution Grant 确认包、当前 PRD/DSL/系分/TDD/OpenSpec、支出控制准入快照已提交基线、B5 Spend Rule 控制活动确认包、支付工具授权准入和 route snapshot 回链提交、`GSD2-B7-RECON-GATE-CONSUME-002` 本地 Green 证据、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费证据、`021ee2ce feat: 补齐支出控制准入快照` 当前已提交代码基线、[GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)、[GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md](GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md)、[GSD-2-B7-对账差异报告ExecutionGrant确认包.md](GSD-2-B7-对账差异报告ExecutionGrant确认包.md)、[GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)、[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md)、[GSD-2-AUTH-Chargeback目标语义对齐任务卡.md](GSD-2-AUTH-Chargeback目标语义对齐任务卡.md)、[GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md](GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md) 和 [GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md](GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md)。 |
| Owner | AI Native 流程编排负责顺序和门禁；产品架构专家确认业务价值、验收和 Not Done；资深架构师确认源码锚点、写入范围、Red、验证命令和 CAD 停止条件；用户确认单一 Grant。 |
| 写入范围 | 本文、LWT 生产可用能力 Goal、B5 Spend Rule 控制活动确认包、GSD-2 入口、TDD README、docs README 和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、ledger、wallet、transaction、core、tests、Justfile、AGENTS.md、最近 Git 提交和旧 GSD/Grant 历史材料。 |
| Git 策略 | B7 差异报告已提交到 `a1397ddf`；支出控制准入快照已提交到 `021ee2ce`。后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
| 服务层边界 | 当前 Goal 只推进服务层能力；本轮 B5 确认包只准备 Spend Rule 控制活动与预算控制投影的服务层候选，不新增 Controller、HTTP/RPC、页面、导出端点或外部适配入口。 |

状态层级：`GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` 是 GSD-2 父 Goal，`GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` 是当前 LWT 子 Goal，当前可执行状态以 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` 为准。完整映射见 [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md) 第 1.1 节；历史 B7、B2、AUTH 兼容 adapter、B4 remaining、wallet route snapshot、账户能力来源、预交易快照、支出控制准入快照、ledger guard、B5-002、B5-003、B7 对象级 Gate、B7 consumer 或 B7 差异报告状态只作为已消费证据，不作为下一步授权。
...
运行时恢复口径：若当前会话 Goal objective、旧摘要或历史任务卡仍写着优先补齐 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、B7 `scopeDecision` 待确认或清算 / 结算真实消费方待确认，以当前 Git/code baseline、LWT Goal 和 OpenSpec tasks 为准；这些切片已经完成 Green 并成为已消费证据，不得作为下一轮默认任务或授权依据。2026-06-19 已按恢复请求重跑授权准入目标测试 3 tests 和授权准入组合回归 40 tests，均在沙箱外通过，B2 授权准入只作为被依赖基线继续维护；后续 wallet 方向默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`，进入编码前仍需确认 Execution Grant 与 `schemaDecision`。
...
| 当前状态 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` |
| 状态载体 | 本文、GSD-2 新基线工作流规划、LWT 生产可用能力 Goal、B5 Spend Rule 控制活动确认包、AUTH Chargeback 目标语义任务卡、AUTH Chargeback 兼容入口确认包、W2/W3/W4 B2 账户层级文档、TDD README、docs README、OpenSpec tasks。 |
| 反馈源 | checker、`rg`、`git status --short`、`git diff --check`、目标测试、compile、pmd、用户确认。 |
| 验证者 | 产品语义由产品架构专家确认；工程边界由资深架构师确认；优先级和 Grant 由用户确认。 |
| 默认推进 | 当前来源契约、资金责任目标主体、资金责任解析 facade、支付工具能力准入 facade、账户能力来源准入 facade、支付工具预交易快照 facade、支出控制准入 facade、授权准入 facade、授权 route snapshot 回链、交易投影解释、余额调账审计、对账差错闭环、差错处理动作上下文守卫、B7 准入消费、出款 preflight 首个消费方接入、ledger 正常余额方向 guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer、B7 差异报告和本轮 Spend Rule 控制活动均已完成目标测试、相关回归、规约扫描和状态回写；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 已完成 contract/design-only 任务卡，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成确认包；`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING` 已完成解释矩阵；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 与 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已完成支付工具授权准入和快照回链。下一轮若继续编码，需重新选择新的单一 Grant；若改选 VCC facade、完整 Spend Rule 规则引擎、交易消费控制活动或 B7 报告扩展，需另起单一 Grant。 |
...
| 交接 | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；B7 报告已提交到 `a1397ddf`，支出控制准入快照已提交到 `021ee2ce`。若继续 wallet，默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`。 |
...
| 字段 | 内容 |
| --- | --- |
| 当前 Wave / Task | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、B7 对象级 Gate 基座和 B7 清算 / 结算 consumer 均已消费；`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已提交到 `a1397ddf`，`GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已提交到 `021ee2ce`。 |
| 当前状态 | `SR_CONTROL_ACTIVITY_GREEN_VERIFIED` |
| 下一建议 Grant | B7 对象级 Gate 基座、清算 / 结算只读 consumer、B7 差异报告最小查询、账户能力来源准入、支付工具预交易快照和支出控制准入快照均已完成，不再需要确认 `scopeDecision`、复用 consumer Grant、复用 report Grant、复用 account capability Grant、复用 pre-transaction snapshot Grant 或 spend control admission Grant。本轮已执行 `GSD2-B5-SR-CONTROL-ACTIVITY-001` 并完成本地 Green / Verify；下一步如要提交，先提交本 Task 独立切片，如要扩能力需另起单一 Grant。 |
| 下一 Red | 当前 B5 控制活动首轮 Red / Green 已消费；若改选 B7 报告扩展、VCC facade、完整 Spend Rule 规则引擎或交易消费控制活动，需另起对应单一 Grant 和新的 Red。 |
...
| Git / code baseline | 当前已提交代码基线为 `021ee2ce feat: 补齐支出控制准入快照`；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed` 和 `GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 已完成 Red / Green / Verify 并提交固化。`a1397ddf feat: 补齐对账差异报告只读查询`、`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链` 和 `0b251593 feat: 补齐账本正常余额方向护栏` 保留为已消费能力证据，`dd442888` 和 `ca603eab` 保留为上一已消费证据。 |
| 新增状态载体 | [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)。 |
| 裁决 | 当前不再把 `GSD2-B7-RECON-GATE-CONSUME-002`、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 或 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 作为下一候选；这些切片均已完成并成为已消费证据。下一轮默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`，进入条件是用户同时确认 Execution Grant 与 `schemaDecision`。 |
...
| 不可外推范围 | 完整 Spend Rule 控制活动、预算控制投影、VCC facade、失败态全量解释、projection store、独立 ledger guard 回归包、差异报告、完整清分 / 清算 / 结算生命周期、补事实命令执行、运营审批、生产迁移、灰度、告警、VCC / 全球账户 / 收单 P2 业务。 |
...
| Grant Decision Ledger | `GSD-2-LWT-生产可用能力Goal.md` 第 8.1 节已补单一 Grant 决策账本：`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 和 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费；当前默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001` 的 Spend Rule 控制活动与预算投影确认包，进入条件是用户同时确认 Execution Grant 与 `schemaDecision`。 |
...
| 下一候选顺序 | 若继续 wallet，默认确认 `GSD2-B5-SR-CONTROL-ACTIVITY-001`；若改选 VCC facade 或 B7 报告扩展，则重新建立对应单一 Grant。 |
