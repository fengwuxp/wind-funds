...
| 所属阶段 | GSD-2 Wave 5 / P0-P1 dependency planning / wallet instrument receive binding version guard verified。 |
| 关联 Goal | `GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` |
| 当前状态 | `GA_PAYOUT_RAIL_KERNEL_GREEN_VERIFIED` |
...
| 写入范围 | 本文、LWT 生产可用能力 Goal、Wallet 特殊业务能力确认包、TDD 清单、docs 索引和 OpenSpec tasks 的状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-Wallet特殊业务能力地图ExecutionGrant确认包.md`、`docs/TDD设计/支付资金底座测试驱动设计.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、ledger、wallet、transaction、core、tests、Justfile、AGENTS.md、最近 Git 提交和旧 GSD/Grant 历史材料。 |
| Git 策略 | B7 差异报告已提交到 `a1397ddf`；支出控制准入快照已提交到 `021ee2ce`；Spend Rule 控制活动与预算控制投影已提交到 `78f7f008`；预算组非建账和资金服务层收敛已提交到 `a5b12a3f`；交易消费支出控制服务已提交到 `b29a5df`；交易消费并发幂等硬化、退款释放守卫、退款事实消费守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫和本轮退款引用原交易业务一致性守卫已形成服务层验证证据。后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
| 服务层边界 | 当前 Goal 只推进服务层能力；wallet 支付工具生命周期授权入口只做服务层 facade 委派，不新增 Controller、HTTP/RPC、页面、导出端点、外部适配入口、统一支付工具交易内核或交易 canonical 入参变更。 |

状态层级：`GSD2-GOAL-PRODUCTION-FUNDS-BASELINE-2026-06-12` 是 GSD-2 父 Goal，`GSD2-GOAL-LWT-PRODUCTION-CAPABILITY-2026-06-18` 是当前 LWT 子 Goal，当前代码基线以 `GA_PAYOUT_RAIL_KERNEL_GREEN_VERIFIED` 为准。`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit，`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` 已补齐并发唯一键冲突同摘要回读既有活动的硬化证据，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` 已补齐失败退款交易不得被 `release` 降级为普通失败释放的守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` 已补齐已关闭退款交易不得被 `consume` 降级为普通成功消费的守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003` 已补齐非退款交易不得被 `refund` 降级为退款控制补偿的守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004` 已补齐退款交易必须关联已消费控制活动的守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005` 已补齐交易消费业务场景一致性守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006` 已补齐成功交易业务流水一致性守卫证据，`GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` 已补齐失败释放业务流水一致性守卫证据，`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008` 已补齐退款引用净消费上限守卫证据，`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009` 已补齐退款引用已消费一致性守卫证据，`GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010` 已补齐交易事实累计解释金额守卫证据，`GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011` 已补齐控制活动幂等语义一致性守卫证据，`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012` 已补齐退款引用原消费资金交易事实存在性和基础一致性守卫证据，`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013` 已补齐退款引用原消费资金交易业务场景和业务流水一致性守卫证据，`GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` 已补齐预算控制投影按目标资金账户或信用账户隔离的守卫证据；`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001` 已补齐支付工具生命周期授权入口委派切片；`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 已补齐收款工具 route snapshot 支付工具归因回链；`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 已补齐收款工具 expectedBindingVersion 必填护栏；`GSD2-GA-PAYOUT-RAIL-KERNEL-INTEGRATION-001` 已补齐出款支付工具到账户主体型提现内核的服务层委派；完整映射见 [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md) 第 1.1 节。历史 B7、B2、AUTH 兼容 adapter、B4 remaining、wallet route snapshot、账户能力来源、预交易快照、支出控制准入快照、ledger guard、B5-002、B5-003、B7 对象级 Gate、B7 consumer、B7 差异报告、B5 控制活动、B5 交易消费、B5 交易消费并发幂等、B5 交易消费退款事实、补偿、引用守卫、业务场景守卫、成功消费业务流水守卫、失败释放业务流水守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离、释放上限目标账户隔离或预算组非建账状态只作为已消费证据，不作为下一步授权。
...
| 12 | `GSD2-B5-SR-TRANSACTION-CONSUME-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003` / `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004` / `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005` / `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006` / `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` / `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008` / `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009` / `GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010` / `GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011` / `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012` / `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013` / `GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` / `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` | wallet / transaction 服务层控制活动消费 | 已完成本地 Green / Verify / Commit；依赖支出控制准入、支付工具预交易快照、Spend Control Activity、预算控制投影和账户主体型交易内核。 | 成功交易记录 `CONSUMED` 控制消耗；失败 / 拒绝 / 过期的非退款交易记录 `RELEASED`；已有退款资金事实记录 `REFUND_COMPENSATED` 控制补偿；`REFUND` 交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放；非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿；退款交易引用原资金交易但该原交易没有对应 `CONSUMED` 控制活动时不得生成退款控制补偿；跨业务场景交易不得消费当前 Spend Rule 控制占用；`consume` 成功交易不得串用同场景其他业务流水；`release` 失败、拒绝或过期交易不得串用同场景其他业务流水；退款补偿金额不得超过退款交易引用的那笔已消费控制活动净额；被引用 `CONSUMED` 控制活动必须与原 `RESERVED` 控制活动在业务场景、业务流水、目标账户和币种上同源一致；同一原控制活动下同一资金交易流水的 `CONSUMED`、`RELEASED` 或 `REFUND_COMPENSATED` 累计解释金额不得超过资金交易金额；同一控制活动流水和摘要不得被不同活动类型或不同业务语义复用；退款交易引用的原消费资金交易必须存在，且租户一致、不是另一笔退款、状态已关闭、币种一致，且业务场景和业务流水与当前控制补偿请求一致；缺原控制活动、缺原消费交易事实、金额超占用、币种 / 主体 / 业务场景 / 业务流水不一致或幂等摘要冲突 fail-fast；并发同流水同摘要唯一键冲突回读既有活动；预算控制投影和释放写入上限传入目标账户时不得混入同预算组或同 Spend Rule 下其他账户或其他卡的控制活动。 | 已写 wallet-face application/spend 契约、Request、必要活动类型、wallet-impl 服务实现、H2 schema / Entity 字段和目标服务流测试；并发硬化、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易事实守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫只改服务层边界，不改支付工具 `REFUND` 能力方向。 | 本轮组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlMovementApplicationServiceTests` 33 tests 通过；收口执行 compile、pmd、边界扫描和 `git diff --check`。 | 不复用本 Grant 扩交易 canonical 入参、统一支付工具交易内核、Controller、HTTP/RPC、事件消费者、完整规则引擎、VCC facade、生产迁移、ledger posting 或支付工具 `REFUND` 方向规则。 |
...
| 状态载体 | 本文、GSD-2 新基线工作流规划、LWT 生产可用能力 Goal、Wallet 特殊业务能力地图确认包、B5 交易消费支出控制活动确认包、AUTH Chargeback 目标语义任务卡、AUTH Chargeback 兼容入口确认包、W2/W3/W4 B2 账户层级文档、TDD README、docs README、OpenSpec tasks。 |
| 反馈源 | checker、`rg`、`git status --short`、`git diff --check`、目标测试、compile、pmd、用户确认。 |
| 验证者 | 产品语义由产品架构专家确认；工程边界由资深架构师确认；优先级和 Grant 由用户确认。 |
| 默认推进 | 当前来源契约、资金责任目标主体、资金责任解析 facade、支付工具能力准入 facade、账户能力来源准入 facade、支付工具预交易快照 facade、支出控制准入 facade、授权准入 facade、支付工具生命周期授权入口、授权 route snapshot 回链、交易投影解释、余额调账审计、对账差错闭环、差错处理动作上下文守卫、B7 准入消费、出款 preflight 首个消费方接入、ledger 正常余额方向 guard、B5 route snapshot 审计回链、B5 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer、B7 差异报告、Spend Rule 控制活动、预算组非建账、交易消费支出控制活动、交易消费并发幂等硬化、退款释放 / 事实 / 补偿 / 引用 / 净额 / 同源一致性 / 原交易事实守卫、交易事实累计解释金额守卫和控制活动幂等语义一致性守卫均已完成目标测试、相关回归、规约扫描和状态回写。当前没有可复用默认编码 Grant；下一轮需重新选择单一 Grant。 |
...
| 交接 | `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B5-SR-CONTROL-ACTIVITY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006`、`GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009`、`GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010`、`GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012` 和 `GSD2-B2-BUDGET-GROUP-NON-LEDGER-SUBJECT-001` 已消费；交易消费切片仍保留 Not Done：完整规则引擎、事件消费、VCC facade、生产迁移、自动告警、补偿重试和支付工具 `REFUND` 方向裁决。若继续 wallet，需重新选择新的单一 Grant。 |
...
| 当前状态 | `GA_PAYOUT_RAIL_KERNEL_GREEN_VERIFIED` |
| 下一建议 Grant | 本轮已消费 `GSD2-WALLET-INSTRUMENT-BINDING-UNIQUENESS-001` 的绑定幂等和默认绑定 DB guard，并已消费 `GSD2-GA-PAYOUT-RAIL-KERNEL-INTEGRATION-001`：`payOutByRail` 可从支付工具、付款绑定和资金责任解析到出款资金账户，凭 `referenceFreezeSn` 与外部 `payeeAccountId` 委派标准提现内核，未知 rail 失败无资金和账本副作用。若继续 wallet，下一推荐在出款费用 / 在途 / 清结算 / 对账生命周期、外部扣账 / return / NOC / reversal、信用账户事件、VCC / VA 单场景、事件消费者 / outbox 或生产迁移执行评审之间重新选择单一 Grant。 |
| 下一 Red | 若继续全球账户出款生命周期，首批 Red 推荐证明：`payOutByRail` 在已完成提现内核委派基础上，能表达费用、在途、清结算 gate 或对账引用中的一个最小场景，并在缺费用口径、缺清结算 gate、账户能力不足或外部协议状态不可信时失败无资金事实扩张。不得混入外部扣账、return/NOC/reversal、信用账户还款 / 调额、事件消费者、outbox、Controller、HTTP/RPC、交易 canonical 入参或 ledger posting 扩张。 |
...
| 不可外推范围 | 完整 Spend Rule 生产规则引擎、VCC facade、失败态全量解释、projection store、独立 ledger guard 回归包、B7 报告扩展、完整清分 / 清算 / 结算生命周期、补事实命令执行、运营审批、生产迁移、灰度、告警、VCC / 全球账户 / 收单 P2 业务。 |
...
| 下一候选顺序 | 若继续 wallet，可在 Spend Rule 规则定义契约、Spend Rule 决策消费、Spend Rule 投影解释或 VCC facade 中择一建立新的单一 Grant；若改选 B7 报告扩展，也必须重新建立对应单一 Grant。 |
...
| 验证证据 | `SpendControlTransactionConsumptionApplicationServiceTests` 5 tests、`SpendControlMovementApplicationServiceTests` 6 tests、`SpendControlAdmissionApplicationServiceTests` 3 tests、`just compile`、`just pmd` 和 `git diff --check` 均通过；后续退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫和业务场景一致性守卫补片已将交易消费目标测试扩展到 11 tests。 |
...
| 已完成能力 | 两个线程并发消费同一 `tenantId + movementSn` 且摘要相同时，唯一键冲突方读取既有活动并比对摘要，同摘要返回同一条 `CONSUMED` 活动，不生成重复活动或资金事实。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、生产迁移或 Git push。 |
| 验证证据 | `SpendControlTransactionConsumptionApplicationServiceTests`、`SpendControlMovementApplicationServiceTests`、`SpendControlAdmissionApplicationServiceTests` 服务层组合回归 15 tests 通过；`just compile`、`just pmd`、`git diff --check` 和边界扫描通过。其中并发用例先在非沙箱环境 Red 为 `DuplicateKeyException`，Green 后同摘要回读同一活动事实。 |
...
## 14. B5 交易消费退款释放守卫记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` 的实际执行结果。该切片只硬化交易消费控制活动的退款边界，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| Execution Grant | `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001`。 |
| 写入范围 | `wallet-impl` 交易消费支出控制服务、交易消费目标服务流测试、TDD 清单、本文、LWT Goal 和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `release` 只允许失败、拒绝或过期的非退款交易事实释放控制占用；失败的 `REFUND` 交易事实不得被降级为普通失败释放，退款相关控制影响只能通过 `refund` 记录 `REFUND_COMPENSATED`。 |
...
| 验证证据 | 新增目标 Red 在非沙箱环境失败为 `Expecting code to raise a throwable`，证明旧服务会把失败 `REFUND` 交易写成 `RELEASED` 控制活动；Green 后 `SpendControlTransactionConsumptionApplicationServiceTests` 7 tests 通过，失败路径断言不写控制活动、route、posting、LedgerEntry、账本交易或余额投影。 |
| 该切片状态 | `SR_TRANSACTION_CONSUME_REFUND_RELEASE_GUARD_GREEN_VERIFIED`。 |
| Not Done | 完整 Spend Rule 规则引擎、事件消费 / outbox、自动告警、补偿重试、运营后台、生产 DDL、历史补数、VCC facade 和清结算补事实。 |

## 15. B5 交易消费退款事实守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` 的实际执行结果。该补片只补 `consume` 侧目标服务流测试，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `consume` 拒绝已关闭 `REFUND` 交易事实，避免退款交易被降级为普通成功消费；结合第 14 节 release 守卫，退款相关控制影响只能通过 `refund` 记录 `REFUND_COMPENSATED`。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 16. B5 交易消费退款补偿入口守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003` 的实际执行结果。该补片只补 `refund` 侧目标服务流测试，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `refund` 只接受已关闭的 `REFUND` 交易事实；非退款交易事实不得被降级为 `REFUND_COMPENSATED` 控制补偿。失败路径不新增控制活动、不修改资金事实、不写 route、posting、LedgerEntry、账本交易或余额投影。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 17. B5 交易消费退款已消费引用守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004` 的实际执行结果。该补片只补 `refund` 侧目标服务流测试，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `refund` 不仅要求资金交易类型为 `REFUND`，还要求退款交易引用的原资金交易已经存在对应 `CONSUMED` 控制活动；只有退款交易事实但没有已消费控制事实时，不得记录 `REFUND_COMPENSATED`。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 18. B5 交易消费业务场景一致性守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005` 的实际执行结果。该补片只补交易消费服务层的业务场景一致性守卫，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。
...
| 已完成能力 | `consume / release / refund` 均要求原控制活动与请求的业务场景和业务流水一致；交易事实必须与请求业务场景一致，避免跨业务域串用资金交易流水消费 Spend Rule 控制占用。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 19. B5 交易消费业务流水一致性守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006` 的实际执行结果。该补片只补 `consume` 成功交易业务流水一致性守卫，不重新打开支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `consume` 要求成功资金交易事实的业务流水与请求和原控制活动一致；同业务场景但不同业务流水的成功交易不得消费当前 Spend Rule 控制占用。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 20. B5 交易释放业务流水一致性守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` 的实际执行结果。该补片只补 `release` 失败、拒绝或过期交易业务流水一致性守卫，不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | `release` 要求失败、拒绝或过期资金交易事实的业务流水与请求和原控制活动一致；同业务场景但不同业务流水的失败交易不得释放当前 Spend Rule 控制占用。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 21. B5 退款引用净消费上限守卫补充记录（2026-06-20）
...
## 22. B5 退款引用已消费一致性守卫补充记录（2026-06-20）
...
## 23. B5 交易事实累计解释金额守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010` 的实际执行结果。该补片只补同一原控制活动下同一资金交易流水被多个控制活动重复解释时的累计金额守卫，不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | 同一原控制活动下，同一资金交易流水的 `CONSUMED`、`RELEASED` 或 `REFUND_COMPENSATED` 累计解释金额不得超过资金交易金额，避免一笔交易被多个 Spend Rule 控制活动重复消耗、释放或补偿。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 24. B5 控制活动幂等语义一致性守卫补充记录（2026-06-20）
...
## 25. B5 退款引用原交易事实守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012` 的实际执行结果。该补片只补退款控制补偿对退款资金交易引用原消费交易事实的存在性和基础一致性守卫，不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | 退款控制补偿不能只凭历史 `CONSUMED` 控制活动回链；退款交易的 `referenceTransactionSn` 必须能查询到真实原消费资金交易，且原交易租户一致、不是另一笔 `REFUND`、状态为 `CLOSED`、币种与请求一致。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
| 验证证据 | 非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后复跑 `SpendControlTransactionConsumptionApplicationServiceTests` 20 tests 通过；组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlMovementApplicationServiceTests` 27 tests 通过；收口执行 `just compile`、`just pmd`、`git diff --check` 和边界关键词扫描。 |
| 当前状态 | `SR_TRANSACTION_REFUND_REFERENCE_FACT_GUARD_GREEN_VERIFIED`。 |
| Not Done | 完整 Spend Rule 规则引擎、事件消费 / outbox、自动告警、补偿重试、运营后台、生产 DDL、历史补数、VCC facade、清结算补事实和支付工具 `REFUND` 方向裁决。 |

## 26. B5 退款引用原交易业务一致性守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013` 的实际执行结果。该补片只补退款控制补偿对退款资金交易引用原消费交易事实的业务场景和业务流水一致性守卫，不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013`。 |
| 写入范围 | `SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | 退款控制补偿引用的原消费资金交易必须与当前请求的 `businessScene` 和 `businessSn` 一致，避免跨订单或历史脏交易事实被控制补偿复用。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
| 验证证据 | 沙箱内目标测试因 embedded Redis 端口绑定限制失败；非沙箱复跑 `SpendControlTransactionConsumptionApplicationServiceTests` 21 tests 通过；收口执行 `just compile`、`just pmd`、`git diff --check` 和边界关键词扫描。 |
| 当前状态 | `SR_TRANSACTION_REFUND_REFERENCE_FACT_BUSINESS_GUARD_GREEN_VERIFIED`。 |
| Not Done | 完整 Spend Rule 规则引擎、事件消费 / outbox、自动告警、补偿重试、运营后台、生产 DDL、历史补数、VCC facade、清结算补事实和支付工具 `REFUND` 方向裁决。 |

## 27. B5 预算控制投影目标账户隔离守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` 的实际执行结果。该补片只补预算控制投影的目标账户隔离，不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014`。 |
| 写入范围 | `BudgetControlProjectionQuery`、`BudgetControlProjectionDTO`、`SpendControlMovementApplicationServiceImpl`、`SpendControlMovementApplicationServiceTests`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | 预算控制投影可按目标资金账户或信用账户过滤，交易消费后的账户级投影不混入同预算组、同 Spend Rule 下其他账户或其他卡的控制活动。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
...
## 28. B5 释放上限目标账户隔离守卫补充记录（2026-06-20）

本节记录 `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` 的实际执行结果。该补片只补释放类控制活动写入上限的目标账户隔离，以及交易消费服务对历史关联活动的同源守卫；不重新打开 Controller、HTTP/RPC、支付工具 `REFUND` 能力方向、交易 canonical 入参或 ledger posting。

| 字段 | 内容 |
| --- | --- |
| 补充任务 | `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015`。 |
| 写入范围 | `SpendControlMovementApplicationServiceImpl`、`SpendControlTransactionConsumptionApplicationServiceImpl`、`SpendControlMovementApplicationServiceTests`、`SpendControlTransactionConsumptionApplicationServiceTests`、本文、LWT Goal、TDD 清单和 OpenSpec tasks 状态同步。 |
| 已完成能力 | 释放类控制活动写入上限按目标资金账户或信用账户过滤；同一原控制活动下的历史关联活动必须与原 `RESERVED` 在业务场景、业务流水、目标账户、币种、Spend Rule、规则版本和预算组上一致，跨账户、跨规则或跨预算组历史脏活动不得参与当前金额计算。 |
| 禁止范围确认 | 未写 Controller、HTTP/RPC、统一支付工具交易内核、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper、生产迁移或 Git push。 |
| 验证证据 | 非沙箱复跑 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlMovementApplicationServiceTests` 33 tests 通过；收口执行 `just compile`、`just pmd`、边界关键词扫描和 `git diff --check`。 |
...
| 已完成能力 | 将 wallet application 能力面拆成业务 lifecycle facade、外部资金事件 facade、只读解析 / 准入服务、交易后控制消费服务和基础服务；后续已补 rail / channel 语义解析、事件映射解释和 `payOutByRail` 到账户主体型提现内核的委派，避免业务 rail 直接透传为交易层 enum 实现细节，也避免支付工具下沉为交易 canonical 入参。 |
| 下一推荐编码 Grant | 出款费用 / 在途 / 清结算 / 对账生命周期、外部扣账 / return / NOC / reversal、信用账户事件、VCC / VA 单场景、事件消费者 / outbox 或生产迁移执行评审，需重新确认单一 Grant。 |
| 推荐 Red | 出款费用 / 在途 / 清结算 / 对账生命周期、外部逆向事件、信用账户事件或生产迁移任选其一，不得复用本轮默认绑定 guard 或出款内核委派 Grant 扩张。 |
| 禁止范围确认 | 本轮未写 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、ledger posting、外部通道出款协议、费用、在途、清结算、对账、外部扣账、return/NOC/reversal、信用账户还款 / 调额、事件消费者、outbox、生产数据库执行或 Git。 |
| 当前状态 | `GA_PAYOUT_RAIL_KERNEL_GREEN_VERIFIED`。 |
...
| 下一候选 | 若要真正执行生产迁移，必须另起 DBA / 发布 Grant，补备份、执行窗口、回滚、环境验证和应用版本发布顺序；若继续业务能力，优先从出款费用 / 在途 / 清结算 / 对账生命周期、外部逆向事件、信用账户事件或 VCC / VA 单场景中择一。 |
