...
| 上游输入 | GSD-2 W1 基线差距审计、W2 单一 Grant 选择卡、W3 B2 账户层级 CAD 准入草案、W4 B2 Execution Grant 确认包、当前 PRD/DSL/系分/TDD/OpenSpec、本轮支出控制准入快照实现、支付工具授权准入和 route snapshot 回链提交、`GSD2-B7-RECON-GATE-CONSUME-002` 本地 Green 证据、`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费证据、`a1397ddf feat: 补齐对账差异报告只读查询` 当前已提交代码基线、[GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md)、[GSD-2-B7-对账差异报告ExecutionGrant确认包.md](GSD-2-B7-对账差异报告ExecutionGrant确认包.md)、[GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md)、[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md)、[GSD-2-AUTH-Chargeback目标语义对齐任务卡.md](GSD-2-AUTH-Chargeback目标语义对齐任务卡.md)、[GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md](GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md) 和 [GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md](GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md)。 |
...
运行时恢复口径：若当前会话 Goal objective、旧摘要或历史任务卡仍写着优先补齐 `GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、B7 `scopeDecision` 待确认或清算 / 结算真实消费方待确认，以当前 Git/code baseline、LWT Goal 和 OpenSpec tasks 为准；这些切片已经完成 Green 并成为已消费证据，不得作为下一轮默认任务或授权依据。2026-06-19 已按恢复请求重跑授权准入目标测试 3 tests 和授权准入组合回归 40 tests，均在沙箱外通过，B2 授权准入只作为被依赖基线继续维护；后续 wallet 方向应改选完整 Spend Rule 控制活动 / 预算控制投影或 VCC facade。
...
| 当前状态 | `SPEND_CONTROL_ADMISSION_GREEN_VERIFIED` |
| 下一建议 Grant | B7 对象级 Gate 基座、清算 / 结算只读 consumer、B7 差异报告最小查询、账户能力来源准入、支付工具预交易快照和支出控制准入快照均已完成，不再需要确认 `scopeDecision`、复用 consumer Grant、复用 report Grant、复用 account capability Grant 或复用 pre-transaction snapshot Grant 或 spend control admission Grant。若继续编码，则重新选择完整 Spend Rule 控制活动 / 预算控制投影、VCC facade 或 B7 报告扩展的新单一 Grant。 |
| 下一 Red | 若改选 B7 报告扩展，首个 Red 应证明批次范围、导出、运营审批或更完整证据链中的一个最小缺口；若改选 wallet，则按完整 Spend Rule 控制活动 / 预算控制投影或 VCC facade 的候选重建首个 Red。 |
...
| Grant Decision Ledger | `GSD-2-LWT-生产可用能力Goal.md` 第 8.1 节已补单一 Grant 决策账本：`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 和 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费；当前默认从完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或 P2 业务能力中重新确认进入条件、停止条件和状态回写位置。 |
...
| 下一候选顺序 | 若继续 wallet，则只能在完整 Spend Rule 控制活动 / 预算控制投影或 VCC facade 中重新确认单一 Grant；若继续 B7，则选择 B7 报告扩展或完整清结算生命周期中的单一切片。 |
