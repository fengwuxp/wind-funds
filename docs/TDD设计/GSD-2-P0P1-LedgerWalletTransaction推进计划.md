...
| 当前状态 | `B5_BALANCE_ADJUST_AUDIT_QUERY_COMMITTED` |
| 下一建议 Grant | 优先在 [GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md](GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md) 中确认 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 的 `scopeDecision`，或另选 B7 差异报告；若用户优先 wallet，再选完整预交易快照、账户能力来源组合或 Spend Rule 控制闭环。最新源码 Gap Audit 已确认 `gateObjectSn` 目前只是消费对象请求 / 解释字段，差错事实和 Mapper 尚未按对象命中；生产可用方向推荐 `scopeDecision=object-scope-schema-backed`，类型级方案只作为保守 MVP。 |
| 下一 Red | 若改选 B7，首个 Red 应证明清算/结算消费方或差异报告能正确消费差错状态、阻断范围、处理动作和重跑结果，且不直接生成资金事实；若要求对象级阻断，还需增加“某一清算候选或结算单差错不得误阻断另一个消费对象”的 Red；若改选 wallet，则按对应候选重建首个 Red。 |
