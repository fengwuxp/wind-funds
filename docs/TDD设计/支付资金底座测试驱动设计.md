...
当前=`ENTRY_CARD_REWORK_INDEPENDENT_CHECKER_PASS / CAPTE_AUTHORITY_DOC_RESTORE_GRANT_NO / CONSUMER_GREEN_EXECUTION_GRANT_NO / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

本轮只写 wind-funds 三份权威文档；不修改 Capte authority/code/test，不执行 Maven/Git/install/联网/发布。下一 Human Gate 只能是单文件 `MIG09_CAPTE_R8B_AUTHORITY_DOC_RESTORE_GRANT`，不是八文件 Green。
