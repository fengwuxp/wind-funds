...
| TDD-BEN-ENTRY-004 | 授权后继事件权益回放 | `reversal` / `expire` / `settle` / `settleRefund` | 后继事件接收当前重新计算的权益结果，或为完成、撤销、过期、退款分别新增权益专用方法。 | 仍使用授权生命周期入口；后继事件读取原授权事实、原 route snapshot 和原权益快照；`expire` 只作为授权生命周期既有缺口新增，不是权益专用入口。 |
