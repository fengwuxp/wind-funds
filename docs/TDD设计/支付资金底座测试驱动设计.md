...
| 权益快照 DSL | 是否已有可执行资产，以当前工作区 diff、`FundsBenefitSnapshotSpec` 契约测试、`DSL-BENEFIT-*` JSON 夹具和验证命令为准。 | 金额闭合测试、退款处置测试、无权益空值语义测试、稳定摘要测试、缺原权益快照失败测试。 | Phase 1 编码只承诺契约层：无权益空值语义、最小合法快照、金额闭合、组件唯一性、退款处置表达和 JSON 夹具解析；商户券 no-ledger、平台补贴 posting-required、授权占券和不退券资金影响进入 Phase 2 对应批次验收。若只改文档，不宣称机器验收已完成。 |
...
| 账务与投影 | `LedgerBalanceProjectionServiceImplTests`、`DefaultRoutedFundsInstructionOrchestratorProjectionTests`、`PostingLedgerDslContractTests`。 | posting assembler、ledger transaction posting、余额断言工具和账本周期隔离专项。 | 有资金变化的测试不得只断言交易状态；必须断言 route、posting、entry、projection 和幂等。 |
...
| 账本账目和余额投影 | L1 posting 契约 + L2/L4 账务和投影。 | posting plan 独立平衡、ledger transaction、entry digest、余额投影 delta、账本周期隔离。 | 不平衡仍写账、缺账本自动建账、负金额表达方向、投影反写事实。 | Existing Needs Update。已有 posting 和投影测试，账本周期专项仍需补齐。 | `PostingLedgerDslContractTests`、`LedgerBalanceProjectionServiceImplTests`，目标补 posting assembler 和账本周期专项。 |
