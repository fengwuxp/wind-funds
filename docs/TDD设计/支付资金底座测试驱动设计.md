...
| 营销/让利账户 | 平台营销资金账户、商户让利责任账户、合作方补贴账户、平台补贴用户账户和补贴冲回。 | 营销/让利账户是优惠让利成本责任的账户化承接，必须解析到可入账主体；不是营销规则、券库存、支付工具或多方出资汇总池。 | 首个 MVP 建议从 `TDD-BEN-MKT-001`、`TDD-BEN-MKT-002` 和 `TDD-BEN-MKT-005` 开始，先证明平台、商户、合作方成本责任不混。 |
...
| TDD-WALLET-019 | 钱包应用服务边界 | `InstrumentTransactionLifecycleApplicationService` / `ExternalFundsEventApplicationService` / `FundsAccountCapabilityApplicationService` / boundary tests | 支付工具交易生命周期、外部确认资金事件、账户能力查询、业务入口参数解析和工具准入应通过 application/use-case facade 表达；账户聚合型 `WalletAccountApplicationService` 仍是目标态候选，未编码落地前不得把它写成当前公共契约；调用方不得绕过 facade 拼接 `PaymentInstrumentService`、`SpendSubjectFundingRelationService` 或账户资源服务完成授权准入、资金责任解析或交易委派；内部余额钱包、信用额度账户、返利钱包和商户钱包不得被强制包装成 `PaymentInstrument`。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期不入资金交易、强制完成、授权链退款、无授权退款、争议裁决退款可区分性、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 首轮已把 `B4-CB-RED-001A / TDD-RED-017B` 回归化，`B4-AUTH-RACE` 首轮已把 `B4-RACE-RED-001` 回归化。 | 授权拒绝、完成、可信撤销、退款、授权过期不入账或争议裁决资金结果任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/退款并发红线；争议案件不得强制落独立 `chargeback` 公共入口，用户败诉不得产生资金副作用，用户胜诉退款不得被压缩为不可区分的普通退款；完整争议运营、旧 `chargeback` API 物理移除或更深跨节点并发控制仍需新 Grant。 |
