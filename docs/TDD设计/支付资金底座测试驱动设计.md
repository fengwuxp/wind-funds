...
| 3 | VCC-PACK-01 授权场景包 | transaction owner、wallet owner、测试 owner | VCC 授权批准、拒绝、支付工具准入、资金责任解析和拒绝无账务副作用的场景包。 | 目标测试 `VccAuthorizationPackContractTests` 或等价验证；需要 issuer、processor 或 card lifecycle 能力时停止。 |
| 4 | VCC-PACK-02 清算与逆向场景包 | transaction owner、ledger owner、测试 owner | clearing、partial clearing、refund、reversal 按原 route snapshot、binding version 和规则快照回放。 | 目标测试 `VccClearingReplayTests` 或等价验证；缺外部 clearing / chargeback 事实模型时停止。 |
| 5 | VCC-PACK-03 争议和敏感边界 | 产品 owner、安全 owner、测试 owner | chargeback outcome 资金准入、敏感数据阻断和资金底座不得承接 PCI / dispute case 管理的边界。 | 目标测试 `VccChargebackBoundaryTests`、`VccSensitiveDataBoundaryTests` 或等价验证；需要声明 PCI / 卡组织 / dispute 运营完成时停止。 |
...
| P2 VCC 业务专项 | 现有授权、路由和支付工具测试可复用为边界基线。 | 待补目标资产：`VccAuthorizationPackContractTests`、`VccClearingReplayTests`、`VccChargebackBoundaryTests`、`VccSensitiveDataBoundaryTests`。 | 只验证 VCC 场景 pack 归一和资金底座边界；未完成卡组织、Issuer、Processor、Program 和 PCI 确认前，不得声明 VCC 生产资金流完成。 |
