...
| 用例 ID | 场景 | 必须证明 |
| --- | --- | --- |
| TDD-CLS-001 | 交易进入可清分 | 只来自成功交易，或聚合仍 `OPEN` 但当前明细已成功的 `PAY` 交易事实；明细和账本事件必须一致为 `PAY/COMPLETE`，来源分录必须是 `CLEARING + CLEARING category + CREDIT + INCREASE + DETAIL`，并存在可解析 route snapshot、posting plan、账本交易和清分前对账 gate。生成 SPLIT_READY 必须冻结来源交易版本与退款累计，且不改变资金、route、posting、ledger transaction、entry 或余额；SPLIT_READY 只是候选，不是清分执行授权。当前由 `ClearingSplittableDetailApplicationServiceTests` 证明单笔准入、方向排除、重跑幂等和来源版本推进后拒绝复用。最终清分命令仍须在自身事务内复核版本、退款累计和 Gate。 |
...
| TDD-SETTLE-004 | 出款前准入门禁 | 目标态必须基于权威事实校验出款账户、收款端点、外部通道、额度、cutoff、名单、负余额、准备金、对账差错、幂等和审批。当前 `PayoutPreflightServiceTests` 只覆盖引用缺失、调用方证据预检与对象级对账阻断：无 payoutSn 时 Gate 对象是 `SETTLEMENT + settlementSn`，有 payoutSn 时是 `PAYOUT + payoutSn`；不读取金额、币种、幂等或上述权威事实，不能作为提交授权。 |
| TDD-SETTLE-005 | 出款解释状态防误导 | 出款处于门禁待补、外部受理、处理中、在途、金额不一致或规则待确认时，不得展示为到账成功或可自动放行；命中对象级差错时返回 `RECONCILIATION_REQUIRED`，逐条区分平台内部 `confirmationOwner` 与外部 `responsiblePartyRef`，仅需平台运营确认的阻断原因设置 `manualReviewRequired=true`。预检通过固定返回 `PREFLIGHT_PASSED + SUBMISSION_REVALIDATION_REQUIRED`，明确要求真实提交命令重验，不等于 `PayoutExplainabilityTests` 或完整出款状态机已完成。 |
...
| TDD-RECON-007 | 对账任务生命周期 | 批次以必填 `reconciliationScopeRef` 固化业务范围，Gate 对象必须同时存在或同时为空；批次表达 `CREATED -> DATA_COLLECTING -> DATA_READY -> COMPLETED`，并允许当前血缘头进入审计终态 `ABORTED`。终止必须固化操作人、时间和原因，不删除历史或自动重试；完成批次存在差错状态锚点时拒绝终止。替代批次显式引用已终止批次并保持范围、Gate 对象和窗口一致。运行结果只表达 `BALANCED / DIFFERENCE_FOUND`，准入决策只表达通过或阻断；`ABORTED` 当前头必须失败关闭。当前由 Batch、RunResult 与 Gate 测试覆盖。条件放行、任务等待、解析失败、匹配执行失败和调度仍由上层负责。 |
...
| TDD-RECON-020 | 对账批次重跑与替代 | 绑定 Gate 的同一租户、范围和准入对象只有一个当前血缘头；正常重跑引用已完成当前头并校验差错物化和处理动作，替代批次引用已终止当前头。两者都生成新批次、原子推进当前头，保持 `reconciliationScopeRef`、准入对象和窗口一致，允许调整规则版本，不覆盖旧批次、来源和运行结果。同父唯一键禁止分叉；当前头缺失、漂移或为 `ABORTED` 时 Gate 失败关闭。当前由 Batch、Difference 与 Gate 测试覆盖。 |
| TDD-RECON-021 | Gate 权威检查与只读解释 | `checkGate` 必须在最终资金命令事务内对对象血缘头、当前批次和命中的阻断差错执行 `FOR UPDATE` current read，并以 `Propagation.MANDATORY` 拒绝事务外调用，不得自行创建检查后立即释放锁的短事务；`inspectGate` 与其共享判定规则但不加锁，只用于报表和运营时点解释。差错报告和 `ClearingSettlementGateConsumerService#inspectGate` 使用只读事务；消费结果不暴露可被误解为授权的 `operationStatus`，任何 inspect 结果都不得缓存或作为清分、清算、结算、出款授权。当前由 Gate、DifferenceReport 与 ClearingSettlementGateConsumer 测试覆盖事务准入和判定结果，生产同版本 MySQL `REPEATABLE READ` 双事务锁竞争仍需专项验证。 |
| TDD-RECON-022 | 对账原子封版容量 | 当前单侧来源快照最多 1000 个成员，来源证据最多 100 项；单次运行结果最多 2000 个逐笔结论。来源稳定引用和逐笔来源引用最长 128 字符，来源证据和逐笔证据引用最长 256 字符；公共请求 Bean Validation 与服务层直调校验必须一致。超限必须在批次查询、加锁、流水号生成和任何写库前失败，来源快照、来源成员、运行结果、逐笔结果和批次状态均不得出现部分变化。当前由 Batch/RunResult 容量与字段边界测试覆盖；边界值吞吐、生产同版本 MySQL 容量、分段导入和最终封版协议仍是生产门禁。 |
...
| TDD-RECON-024 | Gate 有界失败关闭 | `checkGate` 与 `inspectGate` 共享判定规则；单次最多遍历 100 层批次血缘并读取 2000 条仍可能阻断的活跃差错。血缘循环、断裂、超深必须失败；活跃差错超过容量必须返回 `BLOCKED`，不得因截断误判通过，已闭环历史不得永久占用此容量。当前由 `ReconciliationGateApplicationServiceTests` 覆盖循环、断裂、超深、2001 条活跃差错阻断和 2001 条已闭环历史放行；生产同版本 MySQL 锁竞争与峰值耗时仍需专项验证。 |
| TDD-RECON-025 | MySQL DDL 交付与漂移守卫 | 对账首次建表正向迁移必须覆盖九张当前表，不得包含 `DROP TABLE` 或 `IF NOT EXISTS` 掩盖结构漂移；每张生产表统一使用 `utf8mb4_bin`。部署回读必须精确校验表、列和索引结构；仓库不得发布删表 rollback。执行会加载含 `DROP TABLE` 测试 schema 的 `test-mysql-reconciliation` 只允许连接固定数据库名 `wind_funds_reconciliation_test`，即使已设置破坏性开关也必须拒绝其他库名。`ReconciliationMysqlDdlContractTests` 比较生产 DDL、测试 schema、验证签名和命令保护；它不证明 SQL 已在生产同版本 MySQL 执行，也不替代宿主迁移 checksum、历史数据迁移、真实锁竞争、备份恢复和预发演练。 |
...
| TDD-RACE-007 | 对账重跑、差错物化和差错核销同时执行。 | Gate 批次的逐笔差异必须全部物化后才能创建直接重跑；写路径遵循 `BatchLineage -> Batch -> Difference` 的固定锁序。血缘头推进后拒绝对旧批次新增差错，已存在的同一匹配差错仍可幂等回读；重跑不得覆盖审批、凭证、核销结果，已关闭差错不得无新证据自动重开。H2 双事务用例已验证锁等待后的迟到物化拒绝，生产准出仍需在目标 MySQL 隔离级别下执行并发压测和死锁观测。 |
...
| 对账清算 | 已有 H2 局部切片：`ReconciliationBatchApplicationServiceTests`、`ReconciliationRunResultApplicationServiceTests`、`ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ReconciliationDifferenceReportApplicationServiceTests`、`ClearingSplittableDetailApplicationServiceTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`；清算策略可参考 `SettlementPolicySpecTests`。 | `ReconciliationTaskServiceTests`、`ReconciliationMatchingRuleTests`、显式聚合匹配组、清分批次/快照测试、`SettlementOrderServiceTests`、`PayoutReceiptMismatchTests`、`PayoutExplainabilityTests`；版本化生产迁移、生产同版本 MySQL 并发、可信来源适配器 IAM、最终命令原子复核、观测告警、Runbook、故障/备份恢复、容量与数据保留专项。 | 当前证明批次范围冻结、同一 Gate 对象唯一当前血缘头、两侧来源快照、一对一完整覆盖、不可变完成态结果、持久化逐笔匹配及派生摘要、逐笔结果唯一差错物化、append-only 多动作事实、对象级精确 Gate、Gate 重跑前差异完整物化、非金额差异不伪造金额、只读报告、冻结交易版本的单笔可清分候选、clearing/settlement gate consumer 和 payout preflight；不得外推为来源真实性、外部文件采集/解析、业务归一化、匹配规则执行、条件放行、SLA/任务调度、生产迁移、清分批次、清算、结算或出款生命周期完整实现，整体生产准出仍为 `NO-GO`。 |
