...
| --- | --- | --- |
| 支付工具或业务入口 | 外部工具状态、方向、币种、绑定版本、使用主体、预算组、Spend Rule 和资金责任解析关系均被读取并进入快照；内部钱包入口解析为主体或权益快照。 | 拒绝原因、规则版本、工具、绑定或入口解析失败原因可查询；敏感工具信息不进入普通日志、投影或导出。 |
...
| `DSL-BALANCE-CONTROL-EXTERNAL-DEFICIT-ADJUST-001` | 外部余额终局事实导致我侧同主体余额纠偏。 | `TDD-CTRL-012`、`TDD-CTRL-ERR-007`、`TDD-RECON-016`。 | balance control service、reconciliation difference、negative balance policy、external evidence guard、ledger projection。 | 上游未终局、无外部余额快照、无差错单或审批凭证、跨主体转移损失、纠偏后绕过负余额策略继续交易。 |
...
| `core` | `FundsBenefitSnapshotSpec`、`FundsBenefitComponentSpec`、`FundsBenefitReferenceSpec`、`FundsBenefitRefundPolicySpec`、权益枚举 | 权益快照结构、金额闭合、组件唯一性、账务效果、资金性质、退款处置和无权益空值语义。 | 必须补 DSL/契约测试；如只落设计不落代码，必须声明未执行机器契约验收。 |
...
| TDD-BEN-009 | `contextVariables` 过渡白名单 | 短期过渡只允许放 `benefitSnapshotId`、`stableDigest`、`benefitGroupSn`、`componentSn` 摘要、`ruleVersion`、`refundDecisionId`、`externalDecisionId`；组件金额、资金责任、退款处置完整内容和当前营销规则必须失败。 |

### 7A.1A 服务入口与请求契约模拟

本节用于把“服务入口不新增，只在明确工程任务下扩展 Request/指令转换/原快照引用”的设计结论转成可执行或可转化为可执行的 Red 用例。它不替代 7A.2 至 7A.4 的资金流断言，而是防止编码时把优惠券、代金券或平台补贴误拆成新的产品服务方法。

| 用例 ID | 模拟场景 | 服务入口 | Red 条件 | Green 设计校准 |
| --- | --- | --- | --- | --- |
| TDD-BEN-ENTRY-001 | 含权益直接支付 | `FundsDirectTransactionService#pay` | 新增 benefitPay、couponPay、voucherPay 等权益专用服务方法，或把权益核心字段塞入 `contextVariables`。 | 仍使用 `pay`；工程任务明确允许后，为 `FundsTransactionPayRequest` 增加可选 `benefitSnapshot` 或等价一等字段，并纳入请求摘要。 |
| TDD-BEN-ENTRY-002 | 含权益直接退款 | `FundsDirectTransactionService#refund` | 新增 benefitRefund，让退款请求携带当前重新计算的权益结果作为资金事实来源，或由资金底座自行判断券能不能退。 | 仍使用 `refund`；工程任务明确允许后，补原资金交易、原 route snapshot、原权益快照、业务层本次退款决策引用或 replay 策略引用；缺原权益快照失败或进入人工处理。 |
| TDD-BEN-ENTRY-003 | 授权占券 | `FundsAuthorizationTransactionService#authorize` | 新增 authorizeWithBenefit、couponAuthorize、voucherAuthorize，或授权拒绝仍核销、作废、冲减权益。 | 仍使用 `authorize`；工程任务明确允许后，为 `FundsAuthorizationTransactionAuthorizeRequest` 增加可选 `benefitSnapshot` 或等价字段；拒绝和准入失败无 route/posting/entry 和无权益副作用。 |
| TDD-BEN-ENTRY-004 | 授权后继事件权益回放 | `reversal` / `expire` / `settle` / `settleRefund` | 后继事件接收当前重新计算的权益结果，或为完成、撤销、过期、退款分别新增权益专用方法。 | 仍使用授权生命周期入口；后继事件读取原授权事实、原 route snapshot 和原权益快照；`expire` 已作为授权生命周期既有入口补齐，不是权益专用入口。 |
| TDD-BEN-ENTRY-005 | 仅 DSL 契约范围 | 无交易服务入口改动 | 在仅契约范围内修改直接交易、授权交易服务方法、Request/DTO、route/posting/replay 或交易主链路。 | 仅 DSL 契约范围只验证 `core` DSL 契约、权益 Spec、枚举和 JSON 空值语义；服务入口和 Request/DTO 变更必须进入对应工程任务。 |
| TDD-BEN-ENTRY-006 | 营销账户应用门面 | `FundsBenefitFundingApplicationService` + 既有交易服务 | 新增 `FundsMarketingTransactionService`，或门面直接写 route/posting/LedgerEntry/余额投影，或拥有独立交易状态机。 | `FundsBenefitFundingApplicationService` 是保留命名；门面只做权益资金责任解析、营销账户 guard、快照固化和委派；直接交易、授权、退款、撤销、清结算和对账生命周期仍由既有服务承接。 |
...
| TDD-BEN-MKT-003 | 商户让利和展示优惠默认不生成营销账户分录。 | `NO_LEDGER` 的商户让利、展示优惠和平台券不补足商户只进入权益快照、清分展示、商户账单或对账解释，不写营销账户 LedgerEntry。 |
...
| TDD-BEN-MKT-006 | 退款、撤销或对账差错回放原营销账户。 | 补贴冲回、保留补贴、负债恢复和不可退权益沿原权益快照、原 route snapshot 和原营销账户引用处理，不读取当前营销账户绑定或当前营销规则。 |
...
| TDD-BEN-REPLAY-003 | 权益快照事实源一致性 | `refund` / `reversal` / `expire` / `settleRefund` | 被设计选定的事实源必须能读取 `benefitSnapshotId`、组件摘要、规则版本、退款策略和决策流水；同一交易不能在不同事实源读到冲突摘要。 |
...
| TDD-BEN-RED-018 | 只有指令请求携带权益快照，但没有 route snapshot、交易事实快照、独立权益快照表或等价不可变存储，就声明含权益生产链路完成。 | route replay tests、immutable fact source review。 |
...
| TDD-BEN-RED-021 | 工程任务未选择零实付表达、平台补贴表达或权益快照事实源，却进入含权益生产资金流实现。 | service readiness review、service flow tests。 |
...
| --- | --- | --- | --- |
| 契约空值语义 | 无权益空值语义、最小合法快照、金额闭合、组件唯一、闭合角色、稳定摘要。 | 核心字段一等化；`contextVariables` 只放扩展上下文；同业务键不同权益摘要必须冲突。 | 只能证明契约承载和 JSON 空值语义，不能外推为资金流完成。 |
...
| 授权权益 | 授权占券、完成核销、撤销或过期释放、授权拒绝不处理权益。 | 授权阶段只固化 hold 引用；完成、撤销、过期读取原授权权益快照；拒绝无 route、posting、entry 和权益副作用。 | 授权资金流落地必须补授权生命周期和并发竞争测试。 |
| 退款和回放 | 不退券但冲补贴、不退券且不冲补贴、部分退款、业务层本次退券决策、缺原权益快照。 | 逆向事件引用原资金交易、原 route snapshot、原权益快照和本次业务决策；不得按当前营销规则重算。 | 退款和回放落地必须补 replay、累计上限和缺快照失败测试。 |
| 清结算和对账 | 含权益清分、平台补贴拆分、清算后退款、营销核销与资金入账不一致、不退券客服争议。 | 权益金额项必须可追溯到组件、核销引用、规则版本和退款处置；差异进入差错单，不静默补平。 | DDL/H2 和服务级测试完成前只能声明设计覆盖。 |
| 累计和并发 | 多笔部分退款并发、授权完成和过期并发、营销补偿和资金退款并发、同业务流水不同权益快照并发。 | 幂等键、稳定摘要、组件剩余额度、确定性尾差规则和状态版本必须共同约束；失败方无资金副作用。 | 必须补 race tests，不能只用顺序用例证明安全。 |
...
| 生产门禁 | 请求态权益快照、route snapshot、交易事实快照、独立权益快照表或等价不可变存储、清结算和对账消费。 | 请求态可承载不等于生产可回放；缺不可变快照来源不得声明含权益生产链路完成；`CONTRACT_ONLY` 夹具不得作为资金流完成证据。 | `RED-058` 是生产完成门禁，契约承载不得越权声明资金流完成；资金流、清结算、对账、投影和治理消费必须填写权益准入证明。 |
...
| 权益快照 DSL | `FundsBenefitSnapshotSpecTests`、`FundsDslJsonContractTests`、`tests/src/test/resources/dsl-contract-cases/DSL-BENEFIT-SNAPSHOT-001.json` 已形成契约承载基线，覆盖无权益空值语义、最小合法快照、金额闭合、组件唯一性、退款处置表达、稳定摘要和 `CONTRACT_ONLY` JSON 夹具解析。 | `fixtureLevel` 边界、`contextVariables` 白名单、零实付平台补贴或储值权益、权益快照事实源一致性、缺原权益快照失败、route/posting/replay 消费、独立伴随指令原子性、补充权益事实、审计证据包 TOCTOU、平台补贴 posting-required、授权占券、不退券资金影响、清结算、对账、投影、归档、冷热读取、治理重放、解释视图、证据最小化和外部规则核验拆分专项。 | 契约承载只能声明字段和 JSON 语义；商户券 no-ledger、平台补贴 posting-required、授权占券、零实付权益资金影响、伴随指令、补充事实和不退券资金影响必须进入资金流、清结算、对账、投影或治理专项验收。若只改文档，不宣称机器验收已完成。 |
...
| 权益语义 | L1 契约测试起步；进入资金影响后补 L2/L3/L4。 | 无权益空值语义、最小合法快照、金额闭合、组件唯一性、稳定摘要、退款处置表达、伴随指令组、补充权益事实、审计证据包和可理解视图。 | 核心语义藏 context、NO_LEDGER 入账、缺原快照重算当前规则、请求态快照冒充生产完成、伴随指令部分成功静默成功、补充事实覆盖原事实、证据包失效仍 apply、视图误导使用者、证据越界、外部规则未核验仍放行。 | 已有契约承载证据；route/posting/replay、清结算、对账、投影、归档、冷热读取和治理重放仍为未覆盖范围。 | `FundsBenefitSnapshotSpecTests`、`FundsDslJsonContractTests`、`DSL-BENEFIT-*` 夹具，目标补 route/posting/replay/清结算/对账/投影/归档/冷热读取/治理重放、解释视图、证据最小化和外部规则核验专项。 |
...
| 权益金额组件 | 权益快照契约、商户券、平台补贴、储值券、授权占券、不退券、部分退款、伴随指令原子性、补充权益事实、审计证据包、可解释视图和证据最小化单独验证。 | 含权益直接交易、含权益授权、含权益退款、含权益清结算、营销资金对账差错、交易投影、归档读取和治理重放组合链路。 | 契约承载只证明字段和 JSON 语义；route/posting/replay、清结算、对账、投影、归档、冷热读取和治理重放必须分别具备服务级或治理级证据。 | 资金底座重算券、NO_LEDGER 入账、补贴净额混记、授权拒绝核销权益、储值券误作普通券、缺快照重算当前规则、伴随指令部分成功静默成功、补充事实覆盖原事实、证据包失效仍 apply、使用者视图误导、敏感证据越界、未核验规则放行。 |
