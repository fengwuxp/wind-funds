...
| `DSL-BENEFIT-MERCHANT-DISCOUNT-001` | 商户优惠券不入账。 | `TDD-BEN-DIR-001`。 | direct converter、route resolver、posting assembler、clearing projection。 | `TDD-BEN-RED-003`。 |
| `DSL-BENEFIT-PLATFORM-SUBSIDY-001` | 平台补贴券补足商户。 | `TDD-BEN-DIR-002`。 | route resolver、platform account resolver、posting assembler。 | `TDD-BEN-RED-004`、`TDD-BEN-RED-008`。 |
| `DSL-BENEFIT-PLATFORM-NO-SETTLEMENT-001` | 平台券不补足商户。 | `TDD-BEN-DIR-003`。 | direct route、clearing amount projection。 | `TDD-BEN-RED-004`。 |
| `DSL-BENEFIT-PREPAID-VOUCHER-001` | 储值、预付或礼品卡代金券。 | `TDD-BEN-DIR-004`、`TDD-BEN-RED-006`。 | voucher liability route、platform/prepaid account role、finance confirmation guard。 | `TDD-BEN-RED-006`。 |
| `DSL-BENEFIT-AUTH-HOLD-001` | 授权时占券、完成时核销。 | `TDD-BEN-AUTH-001`、`TDD-BEN-AUTH-002`、`TDD-BEN-AUTH-003`。 | authorization service、authorization route resolver、route replay。 | `TDD-BEN-RED-005`。 |
| `DSL-BENEFIT-REFUND-NO-COUPON-001` | 不退券但冲补贴。 | `TDD-BEN-REFUND-001`。 | route replay、refund policy resolver、posting assembler。 | `TDD-BEN-RED-007`。 |
| `DSL-BENEFIT-REFUND-RETAIN-SUBSIDY-001` | 不退券且不冲补贴。 | `TDD-BEN-REFUND-002`。 | refund policy resolver、audit context、finance confirmation guard。 | `TDD-BEN-RED-007`。 |
| `DSL-BENEFIT-PARTIAL-REFUND-001` | 部分退款权益分摊。 | `TDD-BEN-REFUND-003`、`TDD-BEN-RACE-001`。 | refund amount allocator、route replay、lifecycle saver。 | `TDD-BEN-RED-008`、`TDD-BEN-RED-009`。 |
| `DSL-BENEFIT-MISSING-SNAPSHOT-REPLAY-001` | 缺原权益快照的逆向处理。 | `TDD-BEN-REPLAY-001`。 | route replay、manual exception boundary。 | `TDD-BEN-RED-002`、`TDD-BEN-RED-009`。 |
...
| `AC-BEN-001` 至 `AC-BEN-011`、`RED-050` 至 `RED-057` | `TDD-BEN-*`、`TDD-BEN-RED-*`、`TDD-BEN-RACE-*`，并按资金影响映射到 `TDD-DIR-*`、`TDD-AUTH-*`、`TDD-CLS-*`、`TDD-RECON-*`。 | 02-交易路由钱包账目与投影系分设计的权益快照契约、权益路由账务消费、授权占券和退款回放；03-清结算与对账系分设计的权益金额项和权益差错。 | 权益快照专项；Phase 1 只做契约承载时不宣称 route/posting/replay 完整闭合，Phase 2 后才要求资金影响组件进入完整证据链。 |
...
| TDD-BEN-003 | 权益金额闭合 | `userPayAmount + components.amount = orderAmount`，币种一致；组件金额合计超过订单金额必须失败或由业务侧裁剪后提交。 |
...
| --- | --- | --- | --- |
| DSL 契约 | `FundsDslJsonContractTests`、`FundsInstructionDslContractTests`、`FundsAmountBoundaryContractTests`、`RouteDslContractTests`、`PaymentInstrumentRouteDslContractTests`、`PostingLedgerDslContractTests`、`SettlementPolicySpecTests`。 | `tests/src/test/resources/dsl-contract-cases/` 完整夹具读取器；新增或变更 caseId 的完整 JSON。 | 新增 DSL caseId 时，必须说明复用现有样例还是新增完整夹具；只引用正文样例不得宣称完整夹具已落地。 |
| 权益快照 DSL | 当前为设计目标资产。 | `FundsBenefitSnapshotSpec` 契约测试、`DSL-BENEFIT-*` JSON 夹具、金额闭合测试、退款处置测试、无权益兼容测试。 | Phase 1 编码只承诺契约层：无权益兼容、最小合法快照、金额闭合、组件唯一性、退款处置表达和 JSON 夹具解析；商户券 no-ledger、平台补贴 posting-required、授权占券和不退券资金影响进入 Phase 2 对应批次验收。若只改文档，不宣称机器验收已完成。 |
...
| 指标和运营边界 | 暂无专项资产。 | `FundsMetricBoundaryTests`、`FundsOperationPermissionBoundaryTests`、`FundsRailFxBoundaryTests`。 | 指标实现由报表指标模块承接；资金底座只验证只读边界、权限审计和轨道/FX 未启用边界。 |
