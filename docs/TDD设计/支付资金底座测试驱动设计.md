...
背景：支付资金底座的产品设计、DSL 设计和系统分析设计已经把钱包、账本、账目、交易路由、清结算、对账、归档重放、余额快照和业务能力包拆成多层设计。团队需要一份稳定的 TDD 入口，把“设计已覆盖”转成“测试应证明什么、Red 用例先拦什么、哪些范围不能用测试缺口掩盖”。
...
3. 清结算、对账、归档重放、指标治理、P2 业务能力包和 ACH/银行转账支撑，必须先声明是否只进入 TDD 分析，以及工程写入范围、外部规则和敏感数据边界。

### 1.0.1 当前代码基线与 A0 Red 裁剪

当前代码能力基线截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`。TDD 可以把该提交之前已验证的交易 Request 上下文只读、敏感上下文阻断、权益核心字段不得旁路、防御性拷贝和局部服务测试作为回归资产，但不能把这些资产外推为后续资金流、清结算、对账、归档或 P2 业务专项已完成。
...
| P0 归档、重放、大数据归档、余额快照和指标治理 | 可进入 TDD 分析；工程落地必须单独声明边界。 | 治理物理落点、Manifest/H2、范围锁、checkpoint、watermark、dry-run/apply、冷热读取、指标水位隔离和导出审计。 | `TDD-B8-RED-001` 至 `TDD-B8-RED-005`，以及 `GOV-GATE-*` 对应的只读、差异和脱敏边界断言。 | 无范围 apply、先推水位后计算、用报表或指标反推余额、指标任务反写资金事实。 |
...
| TDD-WALLET-015 | VCC 预付卡不创建资金账户 | wallet boundary tests / VCC contract tests | prepaid virtual card 只能创建或引用 `PaymentInstrumentRef`；若需要资金责任，必须通过资金来源关系解析到已存在或已确认的内部责任主体；不得因卡产品类型自动创建 `FundingAccount`。 |
| TDD-WALLET-016 | VCC 共享卡不创建信用账户 | wallet boundary tests / VCC contract tests | shared card 只能创建或引用支付工具、绑定快照、预算组和资金来源关系；不得因共享卡产品类型自动创建 `CreditAccount`；缺预算组或资金来源时授权失败无账务副作用。 |
| TDD-WALLET-017 | 内部主体能力选择 | wallet capability decision tests | 使用者按真实资金、授信额度、预算控制、支付工具四类需求选择 `FundingAccount`、`CreditAccount`、`BudgetGroup` 或 `PaymentInstrument`；任一场景不能用卡产品形态反推账户类型。 |
...
清结算与对账、归档重放与指标治理在工程落地前必须先落 Red，用失败用例固定对象边界、准入门禁、只读边界和人工处理边界；只有 Red 能证明错误行为会被阻断时，才允许进入 Green 设计和最小实现。
...
| 归档重放与治理 | TDD-B8-RED-001 | 缺 Manifest 或 checkpoint 仍归档成功 | 归档申请缺范围、检查点、水位、Manifest、对象清单或摘要校验，仍标记成功或推进水位。 | `TDD-ARCH-001`、`TDD-ARCH-005`、`TDD-RED-026`。 |
| 归档重放与治理 | TDD-B8-RED-002 | 治理任务反写资金事实 | 归档、余额重建、交易投影重放或差异报告任务尝试创建 route、posting、entry、交易明细、清结算对象或余额修复事实。 | `TDD-GOV-004`、`TDD-RED-010`、`TDD-RED-028`。 |
| 归档重放与治理 | TDD-B8-RED-003 | 普通指标快照替代余额确认 | 普通指标快照成功后推进余额水位、归档 Manifest、交易投影 checkpoint，或被当作账本余额正确证据。 | `TDD-METRIC-003`、`TDD-METRIC-004`、`TDD-RED-019`。 |
| 归档重放与治理 | TDD-B8-RED-004 | 无范围或无差异报告的正式重放 | 交易投影重放缺范围、模式、checkpoint、Manifest 或差异报告，仍进入 apply 或覆盖正式投影。 | `TDD-REPLAY-001` 至 `TDD-REPLAY-004`、`TDD-RED-027`。 |
| 归档重放与治理 | TDD-B8-RED-005 | 大数据归档绕过治理读取 | 报表数仓、离线指标或经营分析直接扫描在线资金冷归档，绕过治理导出快照、脱敏、digest、消费方登记和审计。 | `TDD-ARCH-010`、`TDD-RED-018`、`TDD-RED-019`。 |
...
#### 13.0.3 归档重放与指标治理 TDD 交付判定

归档重放与指标治理的 PRD、DSL、系分和 TDD 需要共同证明测试资产设计、Red 顺序、治理物理落点候选、Manifest/H2 范围、dry-run/apply 边界、指标水位隔离、治理导出和大数据归档边界测试矩阵。工程落地前必须完成以下判定，并把结论写入对应工程任务。

| 判定项 | 必须证明 | 失败处理 |
| --- | --- | --- |
| 设计基线 | PRD、DSL、系分、TDD、任务基线和代码基线均已引用到同一个交付点。 | 只允许继续文档对齐，不写生产代码或测试代码。 |
| Red 闭环 | `TDD-B8-RED-001` 至 `TDD-B8-RED-005` 先失败，覆盖缺 Manifest/checkpoint 仍归档成功、治理任务反写资金事实、普通指标快照替代余额确认、无范围或无差异报告正式重放、大数据归档绕过治理读取。 | Red 未落地时，不得进入归档重放与指标治理 Green 实现。 |
...
#### 13.0.4 归档重放与指标治理 TDD 输出矩阵

TDD 交付完成时，必须形成下列输出。缺任一项时，只能继续 TDD 分析，不能进入归档重放与指标治理工程落地。
...
| TDD-RAIL-001A | VCC 支付工具定性边界 | VCC 卡、虚拟卡、卡 token、prepaid virtual card 和 shared card 都不得作为 ledger subject、余额投影主体或 `FundingAccount`；必须先进入 `PaymentInstrumentRef`、绑定快照和资金来源决策，再解析到内部资金账户、信用账户、预算组或平台账户角色。 |
...
| TDD-P2-VCC-004 | `VCC-AC-007` | prepaid virtual card 授权资金来源。 | 预付卡只作为支付工具快照；预付资金来源必须解析为内部责任主体；缺资金来源、缺财务确认引用或资金模式待确认时失败无 route、posting、entry。 |
| TDD-P2-VCC-005 | `VCC-AC-008` | shared card 授权绑定快照。 | 共享卡只作为工具和使用模式；必须固化使用人、绑定版本、预算组、资金来源关系和规则版本；多资金来源不唯一或缺绑定版本时失败无账务副作用。 |
...
| TDD-P2-VCC-008 | shared card 多资金来源同时命中。 | 授权失败且无账务副作用；不能随机选择资金账户、信用账户或预算组；审计记录命中冲突。 |
| TDD-P2-VCC-009 | prepaid virtual card 缺财务确认引用。 | 即使工具和余额看似可用，也不得生产入账；只允许待确认、人工复核或 contract-only。 |
| TDD-P2-VCC-010 | 工具换绑后 VCC 退款或撤销。 | 必须沿原 route snapshot 和原 binding version；不得读取当前绑定、当前预算组或当前资金来源重新解释。 |
...
| 钱包账户和支付工具 | L2 服务测试 + L5 架构边界。 | 账户初始化、平台角色解析、支付工具绑定、方向/状态/资金来源准入、余额查询只读。 | 工具不可用仍入账、资金来源不唯一、敏感原文泄露、wallet 直接写交易或账本。 | 已有服务测试；仍需补 wallet 边界和最终红线回归。 | `FundingAccountServiceImplTests`、`PaymentInstrumentServiceImplTests`、`SpendSubjectFundingRelationServiceImplTests`，目标补 wallet 边界测试。 |
...
| 能力域准入 | P0/P1/P2 能力必须按工程任务独立声明边界；钱包账本余额、清结算对账、归档治理、交易、授权、余额控制和交易投影分别声明写入范围、测试范围和未覆盖条件。 |
...
| 能力域 Red | 清结算与对账必须覆盖 `TDD-B7-RED-001` 至 `TDD-B7-RED-007`；归档重放与指标治理必须覆盖 `TDD-B8-RED-001` 至 `TDD-B8-RED-005`；Red 未证明错误行为会失败前不得进入对应 Green 实现。 |
