...
| SR-CONTROL-002 | 支出控制准入决策记录 | 既有支出控制准入结论无法记录为决策证据，或记录后不能按业务流水、目标账户、规则版本和预算 scope 查询。 | 复用 `SpendControlAdmissionDecisionDTO` 或等价准入事实生成 `SpendRuleDecisionRecord`，查询只读且无资金事实副作用。 | 不重新实现 Spend Rule 规则引擎，不把准入快照当交易事实或控制额度变动。 |
