...
| P2 VCC、全球账户支持与收单设计 | VCC 和全球账户只作为后置业务专项 TDD 输入，不进入默认工程范围；收单当前只做设计和边界复核。 | 外部规则确认状态、敏感数据边界、P0/P1 回归范围、专项验收 ID 和未覆盖条件；收单额外确认 PCI/商户清结算设计边界。 | `TDD-P2-*`、`TDD-RAIL-*`、`TDD-FX-*`、`TDD-OPS-006` 与对应资金底座红线；宿主运营审计使用 `HOST-OPS-*` 自行验收，收单只登记 design-only 差距。 | 把资金底座测试通过声明为 VCC、全球账户或收单业务系统整体交付完成，或把收单设计项解释成实现授权。 |
...
| 数据和 H2 范围 | 当前声明范围包含对账来源/结果/差错、可清分明细、清分批次、清算候选和清算批次；完整结算单、出款单和追偿单仅在未来纳入时补表与 H2 schema。 | 每个已声明资金对象都有状态机、唯一约束、幂等约束、操作人/证据字段和表结构测试；不新增宿主操作审计表。 |
| 服务级流程测试 | 当前声明范围覆盖对账、清分、内部清算和人工候选状态动作；完整结算、外部出款和追偿仅在未来纳入时补 H2 流程。 | 使用真实内部 Spring Bean、Repository/Mapper、事务和 H2 表结构；操作审计由宿主另行验收。 |
...
| 结算策略断言 | 仅在未来纳入完整结算能力时验证 settlementMode、settlementDestination、triggerMode、cutoff、时区、节假日、准备金和审批引用。 | 未纳入前保持目标态，不影响当前对账、清分和内部清算准出。 |
| 对账数据分段断言 | 交易数据、账务数据、清分清算数据和外部资金数据的来源记录、匹配结果、差错类型和责任方。 | 差错关闭、对账报告和阻断判断不能只依赖备注或人工说明。 |
| 并发和重跑测试 | 当前范围验证清算候选、退款、对账重跑、差错物化与核销竞争；结算锁定、出款回单和重复出款仅在未来纳入对应能力时验证。 | 以对象级锁、状态版本、唯一键、幂等键或运行记录证明当前声明范围不会重复入账或覆盖证据。 |
...
本轮已落地候选准入、清算批次编排和受控清算资金原语：`ClearingCandidateApplicationService` 承载候选生成与状态管理，`ClearingBatchApplicationService` 原子锁定候选、执行最终复核并编排 `FundsClearingTransactionService` 完成单账户 `CLEARING -> AVAILABLE`。`ClearingBatchApplicationServiceTests` 已覆盖成功、确定性失败和技术异常三类结果，上述声明范围以 `tests` 模块 H2 全量集成测试完成项目生产验收。争议/风控权威查询、按原资金分配处理部分退款不在当前能力范围；监控和 Runbook 属于宿主接入责任，不影响已声明能力准出。
...
| TDD-CLS-012 | 可选 MySQL 迁移与回读 | 项目生产验收统一使用 `tests` 模块 H2 集成测试。`ReconciliationMysqlMigrationIntegrationTests` 和 `just test-mysql-reconciliation` 仅用于接入方需要 MySQL 兼容验证时，在显式授权的 `wind_funds_reconciliation_test` 上执行前向 DDL、部署回读和对账回归；缺少该环境不阻塞 wind-funds 准出。 |
...
| TDD-RECON-022 | 对账原子封版容量 | 当前单侧来源快照最多 1000 个成员，来源证据最多 100 项；单次运行结果最多 2000 个逐笔结论。来源稳定引用和逐笔来源引用最长 128 字符，来源证据和逐笔证据引用最长 256 字符；公共请求 Bean Validation 与服务层直调校验必须一致。超限必须在批次查询、加锁、流水号生成和任何写库前失败，来源快照、来源成员、运行结果、逐笔结果和批次状态均不得出现部分变化。当前由 Batch/RunResult H2 容量与字段边界测试完成生产验收；分段导入和更大容量属于后续扩展。 |
...
| TDD-RECON-025 | MySQL DDL 静态契约与可选兼容验证 | 对账首次建表正向迁移必须覆盖当前表及批次替代证据字段，不得包含 `DROP TABLE` 或 `IF NOT EXISTS` 掩盖结构漂移；每张生产表统一使用 `utf8mb4_bin`。`ReconciliationMysqlDdlContractTests` 比较生产 DDL、测试 schema、验证签名和命令保护，属于仓内生产验收。`test-mysql-reconciliation` 只允许连接固定数据库名 `wind_funds_reconciliation_test`，用于接入方选择 MySQL 时的可选兼容验证，不作为 wind-funds 准出门禁。 |
...
| TDD-RACE-007 | 对账重跑、差错物化和差错核销同时执行。 | Gate 批次的逐笔差异必须全部物化后才能创建直接重跑；写路径遵循 `BatchLineage -> Batch -> Difference` 的固定锁序。血缘头推进后拒绝对旧批次新增差错，已存在的同一匹配差错仍可幂等回读；重跑不得覆盖审批、凭证、核销结果，已关闭差错不得无新证据自动重开。H2 双事务用例已验证锁等待后的迟到物化拒绝并作为项目生产验收；宿主数据库压测和死锁观测属于接入方运行责任。 |
...
| 对账清算 | H2 生产验收已覆盖：`ReconciliationBatchApplicationServiceTests`、`ReconciliationRunResultApplicationServiceTests`、`ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ReconciliationDifferenceReportApplicationServiceTests`、`ClearingSplittableDetailApplicationServiceTests`、`ClearingSplitBatchApplicationServiceTests`、`ClearingBatchApplicationServiceTests`、`FundsClearingTransactionFlowTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`；清算策略可参考 `SettlementPolicySpecTests`。 | 当前未提供且不得外推：对账任务调度、业务匹配规则、退款/争议/风控权威查询、原分配退款、完整结算单和外部出款生命周期。可信来源接入、观测告警、Runbook、操作审计和数据治理属于宿主责任。 | 当前已声明的批次范围冻结、对账证据、差错和 Gate、清分识别与确认、清算候选、批次原子锁定，以及成功、确定性失败、技术异常三类清算结果通过生产验收；未提供能力按范围边界阻断，不影响已声明能力准出。 |
...
补充：清算候选准入与状态管理由 `ClearingCandidateApplicationService` 及 `ClearingSplitBatchApplicationServiceTests` 覆盖；资金原语由 `FundsClearingTransactionFlowTests` 覆盖；批次原子编排与候选 `CLEARED/BLOCKED/LOCKED` 结果由 `ClearingBatchApplicationServiceTests` 覆盖。H2 全量集成测试是本项目生产验收；争议/风控权威查询和按原分配退款属于未提供能力，不得从现有结果外推。
...
| 清结算与对账 | 已声明切片已进入 L2/L3 并通过 H2 项目生产验收。 | 当前已有对账差错、Gate、单笔可清分明细、单主体清分批次、不可变标量金额快照、清算候选、清算批次原子编排和固定清算资金原语；争议/风控权威准入、按原分配退款、完整结算和外部出款不在当前能力范围。 | 清分对象直接入账、清分规则冒充清算规则、跨主体或跨币种混批、候选 `READY` 绕过已声明准入事实、清算部分成功、出款受理当成功、差错直接改历史分录。 | 清分识别至 `CLEARING -> AVAILABLE` 已由 H2 组合测试完成项目生产验收；未提供能力不得由现有结果外推。 | 已有 `ClearingSplittableDetailApplicationServiceTests`、`ClearingSplitBatchApplicationServiceTests`、`ClearingBatchApplicationServiceTests`、`FundsClearingTransactionFlowTests`；仅在未来纳入争议/风控、原分配退款、完整结算或外部出款时补对应 H2 服务测试。 |
