...
| L5 目标 MySQL 专项测试 | 验证生产 DDL、前向迁移、部署回读、锁等待、唯一键和 `REPEATABLE_READ` 隔离语义。 | `tests` 模块专用测试库与 `just test-mysql-reconciliation`。 | H2 无法等价证明的 DDL、锁、并发裁决和隔离级别行为。 |
...
| TDD-BEN-CONTR-002 | 平台直接补贴用户 | 平台营销资金账户 -> 用户可记账补贴账户，`PLATFORM_OWN_FUNDS`，`benefitReceiverLedgerSubjectCode=SETTLEMENT`。 | `benefitReceiverSubjectRef` 被解释为让利承接账务主体，不是营销用户；余额和账本事实独立于券来源。 | 已由 `testSettleScenarioMatrixShouldRecordConcreteContributionPairs` 覆盖。 |
...
| TDD-WALLET-019 | 钱包应用服务边界 | `PaymentInstrumentTransactionApplicationService` / `FundsAccountCapabilityApplicationService` / boundary tests | 支付工具授权、带控制预留的可信授权完成/撤销与收款、账户能力查询、业务入口参数解析和工具准入应通过 wallet application/use-case facade 表达；可信完成/撤销从原授权快照恢复账务主体和控制预留，不重新读取当前绑定；外部确认资金事件一旦消费为交易事实，应通过 `transaction.application.ExternalFundsEventApplicationService` 表达；账户聚合型 `WalletAccountApplicationService` 仍是目标态候选，未编码落地前不得把它写成当前公共契约；调用方不得绕过 facade 拼接 `PaymentInstrumentService`、`SpendSubjectFundingRelationService` 或账户资源服务完成授权准入、资金责任解析或交易委派；内部余额钱包、信用额度账户、返利钱包和商户钱包不得被强制包装成 `PaymentInstrument`。 |
...
| 跨场景可靠性门禁 | 并发、幂等、重放和失败无副作用已由交易、外部事件、账本、清结算、对账、出款、追偿和 Spend Control 组合切片验证。 | 目标 MySQL 运行证据仍为 `PENDING`：当前环境未配置专用数据库运行时；H2 和 DDL 静态契约不能替代 `REPEATABLE-READ` 验证。 | `just test-mysql-reconciliation` 仅在显式授权的 `wind_funds_reconciliation_test` 专用库执行。 |
...
| TDD-CLS-012 | 目标 MySQL 迁移与回读门禁 | `ReconciliationMysqlMigrationIntegrationTests` 和 `just test-mysql-reconciliation` 必须在显式授权的专用 `wind_funds_reconciliation_test` 库执行前向 DDL、部署回读和对账回归。缺少该证据时 H2 功能结论仍有效，但不得声明 MySQL 生产可交付。 |
...
| TDD-RECON-025 | MySQL DDL 静态契约与目标库验证 | 对账首次建表正向迁移必须覆盖当前表及批次替代证据字段，不得包含 `DROP TABLE` 或 `IF NOT EXISTS` 掩盖结构漂移；每张生产表统一使用 `utf8mb4_bin`。`ReconciliationMysqlDdlContractTests` 比较生产 DDL、测试 schema、验证签名和命令保护；`test-mysql-reconciliation` 只允许连接固定专用库，并验证迁移、回读、锁和 RR 并发语义。两类证据共同构成 MySQL 准出门禁。 |
...
补充：清算候选准入与状态管理由 `ClearingCandidateApplicationService` 及 `ClearingSplitBatchApplicationServiceTests` 覆盖；资金原语由 `FundsClearingTransactionFlowTests` 覆盖；批次原子编排与候选 `CLEARED/BLOCKED/LOCKED` 结果由 `ClearingBatchApplicationServiceTests` 覆盖。H2 全量集成测试是本地功能与事务语义基线，不能替代目标 MySQL 验证；争议/风控权威查询和按原分配退款属于未提供能力，不得从现有结果外推。
...
| 清结算与对账 | 对账至内部清算、W1 结算锁定、W2 出款与 W3 追偿责任/结果引用均已进入 L2-L4 本地验收范围，MySQL 由 L5 专项收口。 | 当前已有对账差错、Gate、清分/清算批次、SettlementOrder/Item 与 `AVAILABLE -> SETTLEMENT`，PayoutOrder/Receipt、`SETTLEMENT -> PREPAYMENT -> CASH` 和 `SETTLEMENT -> AVAILABLE`，以及 RecoveryOrder/Result、`CREATED -> PARTIALLY_RECOVERED -> RECOVERED`。W2 不启用 `IN_TRANSIT`，不做失败后重试；W3 不执行追偿资金策略。 | 清分对象直接入账、清算部分成功、来源跨结算单重复占用、Gate 证据覆盖、非终态误作成功、重复回单/外部 reference 造成二次资金动作、RETURNED/MISMATCHED 自动动账、追偿超额或重复认领已完成资金交易、差错直接改历史分录。 | W1/W2/W3 源码、H2、资金红线和 41 张表联合 MySQL 迁移测试已落地；目标 MySQL 的实际迁移、回读、锁和 RR 运行仍是仓库级待补门禁，部署和真实生产运行不在 Goal 证据范围。 | `SettlementOrderApplicationServiceTests`、`FundsSettlementTransactionFlowTests`、`SettlementPublicContractTests`、`PayoutOrderApplicationServiceTests`、`PayoutPublicContractTests`、`RecoveryOrderApplicationServiceTests`、`ReconciliationMysqlDdlContractTests`、`ReconciliationMysqlMigrationIntegrationTests` 和边界测试。 |
...
| 目标 MySQL 专项 | `just test-mysql-reconciliation` |
