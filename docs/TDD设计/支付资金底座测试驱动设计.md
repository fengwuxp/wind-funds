...
| 用例 ID | 场景 | 必须证明 |
| --- | --- | --- |
| TDD-CLS-001 | 交易进入可清分 | 只来自成功交易，或聚合仍 `OPEN` 但当前明细已成功的 `PAY` 交易事实；明细和账本事件必须一致为 `PAY/COMPLETE`，来源分录必须是 `CLEARING + CLEARING category + CREDIT + INCREASE + DETAIL`，并存在可解析 route snapshot、posting plan、账本交易和清分前对账 gate。生成 SPLIT_READY 必须冻结不可变交易明细、账本交易、posting plan、LedgerEntry、RouteSnapshot 摘要与退款累计，且不改变任何资金或账本事实；资金交易聚合的乐观锁版本不属于清分事实。当前测试证明单笔准入、方向排除、重跑幂等，以及聚合因其他合法明细推进版本时当前明细仍可确认。最终清分命令仍须在自身事务内复核退款累计、RouteSnapshot、批次摘要和 Gate。 |
...
| TDD-CLS-012 | MySQL 迁移与回读 | `ReconciliationMysqlMigrationIntegrationTests` 只在显式授权的 `wind_funds_reconciliation_test` 上运行；必须执行发布前向 DDL 和部署回读 SQL，所有漂移结果集为空，且 MySQL 版本前缀与 `REPEATABLE-READ` 隔离级别符合目标环境。`just test-mysql-reconciliation` 随后在同一实例上执行对账回归；未提供真实 MySQL 环境时不得将 H2 结果冒充为该证据。 |
...
补充：清算候选准入与状态管理由 `ClearingCandidateApplicationService` 及 `ClearingSplitBatchApplicationServiceTests` 覆盖；资金原语由 `FundsClearingTransactionFlowTests` 覆盖；批次原子编排与候选 `CLEARED/BLOCKED/LOCKED` 结果由 `ClearingBatchApplicationServiceTests` 覆盖。待补范围是争议/风控权威查询、按原分配退款和生产环境工程证据，不能把 H2 局部闭环当成完整生产准出。
...
| 清结算与对账 | 局部切片已进入 L2/L3，整域仍待补生产工程证据。 | 当前已有对账差错、Gate、单笔可清分明细、单主体清分批次、不可变标量金额快照、清算候选、清算批次原子编排和固定清算资金原语；目标仍含争议/风控权威准入、按原分配退款、结算锁定和出款生命周期。 | 清分对象直接入账、清分规则冒充清算规则、跨主体或跨币种混批、候选 `READY` 绕过退款/争议/风控、清算部分成功、出款受理当成功、差错直接改历史分录。 | 清分识别至 `CLEARING -> AVAILABLE` 已由 H2 组合测试闭环；尚不等于可生产清算或可结算。 | 已有 `ClearingSplittableDetailApplicationServiceTests`、`ClearingSplitBatchApplicationServiceTests`、`ClearingBatchApplicationServiceTests`、`FundsClearingTransactionFlowTests`；目标补原分配退款、争议/风控端口、settlement/payout 完整 H2 服务测试和生产 MySQL 证据。 |
