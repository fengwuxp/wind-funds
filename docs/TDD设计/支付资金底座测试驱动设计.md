...
| TDD-ROUTE-012 | 支付工具路由准入失败 | route resolver / command guard | 工具状态不可用、方向不匹配、币种不一致、资金责任关系缺失或不唯一、账户能力不足时失败；不得生成 route、posting 或 entry。 |
...
| TDD-WALLET-018 | 支付工具能力应用服务 | `PaymentInstrumentCapabilityApplicationService` 或等价 facade | 工具状态、方向、币种、敏感字段、绑定版本和动作能力由应用层一次性校验并输出快照；服务不返回完整敏感凭证，不表达余额，不直接写 route、posting 或 ledger entry。 |
