...
TDD 主文档只定义目标测试资产、Red 裁剪标准和生产交付断言。局部测试资产可以作为任务计划和交付说明中的回归证据，但不能被外推为后续完整 dispute/chargeback case、授权支付工具应用入口、权益生产资金流、清结算、对账、资金数据治理或 P2 业务专项已完成。
...
| TDD-WALLET-012 | 支付工具绑定历史审计 | `PaymentInstrumentService` / binding history | 绑定、默认标识、优先级、方向、状态或资金责任解析关系变更必须保留历史版本、前后值、操作者、原因和时间；当前态可更新，但历史证据不得被覆盖。 |
...
| TDD-WALLET-025 | 支付工具收款入口 | `InstrumentTransactionLifecycleApplicationService#receiveByInstrument` | VA、ACH 或外部钱包端点收款必须先完成支付工具状态、方向、币种、调用方期望绑定版本、资金责任和账户能力准入，再委派账户主体型 `FundsDirectTransactionService#topup`；成功后资金账户 `AVAILABLE` 增加并生成标准 TOPUP 交易、route、posting plan、账本交易和分录；付款-only 工具、缺绑定、缺 `expectedBindingVersion` 或能力不匹配时必须在进入交易内核前失败，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用；该入口不改变交易 canonical 入参，不给 VA、卡或外部账户建账。 |
| TDD-WALLET-026 | 外部资金事件消费入口 | `ExternalFundsEventApplicationService#consume` | ACH、银行文件、渠道回调或第三方钱包回调进入 funds 前必须先归一为外部事件流水、事件类型、目标资金/信用账户、金额、币种、原交易引用、对账差异引用和业务流水；`ACH_CREDIT_CONFIRMED`、`BANK_CREDIT_CONFIRMED`、`EXTERNAL_CREDIT_CONFIRMED` 到资金账户时必须委派标准 `topup` 内核，证明目标资金账户 `AVAILABLE` 增加并生成 TOPUP 交易、route snapshot、posting plan、LedgerTransaction 和 LedgerEntry；未授权的 debit、return、NOC、reversal、信用账户目标或其他事件类型必须在交易内核前 fail-fast，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用；后续 Grant 必须分别证明扣账、信用账户还款或调额、return/NOC/reversal、补事实、差异单和幂等回放边界。 |
...
| TDD-P2-VCC-012 | `VCC-AC-009` | 多张共享卡受同一主账户约束。 | 每张卡绑定一个信用子账户；余额、授权占用、清算、退款和拒付按各自子账户记录；主账户用于额度/资金池约束和汇总展示；每张卡的账单、预算视图和使用人视图由交易投影按 `PaymentInstrumentRef`、绑定版本、子账户和使用人过滤。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期、强制完成、授权链退款、无授权退款、争议裁决退款可区分性、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 首轮已把 `B4-CB-RED-001A / TDD-RED-017B` 回归化，`B4-AUTH-RACE` 首轮已把 `B4-RACE-RED-001` 回归化。 | 授权拒绝、完成、释放、退款、过期或争议裁决资金结果任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/过期/退款并发红线；争议案件不得强制落独立 `chargeback` 公共入口，用户败诉不得产生资金副作用，用户胜诉退款不得被压缩为不可区分的普通退款；完整 dispute/chargeback case、独立 `chargeback` API 移除或更深跨节点并发控制仍需新 Grant。 |
