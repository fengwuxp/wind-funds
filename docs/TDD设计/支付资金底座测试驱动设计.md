...
当测试覆盖卡、VCC、VA、钱包标识、共享卡或外部支付工具触发授权时，测试必须把入口断言和内核断言分开，不能只断言授权服务返回成功或失败。

| 断言面 | 授权批准最低断言 | 授权拒绝最低断言 |
| --- | --- | --- |
| 支付工具入口 | 工具状态、方向、币种、绑定版本、使用主体、预算组、Spend Rule 和资金责任解析关系均被读取并进入快照。 | 拒绝原因、规则版本、工具或绑定失败原因可查询；敏感工具信息不进入普通日志、投影或导出。 |
...
| `AC-PI-001` 至 `AC-PI-010`、`RED-046` 至 `RED-049`、`RED-067` | `TDD-WALLET-005`、`TDD-WALLET-006`、`TDD-WALLET-009` 至 `TDD-WALLET-019`、`TDD-ROUTE-006`、`TDD-ROUTE-011` 至 `TDD-ROUTE-013`、`TDD-RACE-009`、`TDD-RED-001`、`TDD-RED-034` 至 `TDD-RED-037`、`TDD-RED-043`，以及授权支付工具应用入口的 `TDD-AUTH-*` 准入和失败无副作用用例。 | 02-交易路由钱包账目与投影系分设计的 PaymentInstrumentCapabilityApplicationService、FundingResponsibilityResolutionApplicationService、AuthorizationAdmissionApplicationService、PaymentInstrumentService、SpendSubjectFundingRelationService、RouteSnapshot、RouteReplay、FundingAccountService、CreditAccountService、BudgetGroupService 和 wallet 边界。 | 支付工具、绑定、钱包标识、资金责任解析和账户语义专项；作为钱包账户与路由能力的一部分，不单独表达余额；`FundingAccount` 只表达真实资金账户，不作为信用、预算或支付工具的统一父类；`AC-PI-010` 只授权外层应用入口，不改变交易内核账户主体入参。 |
...
| TDD-WALLET-013 | 钱包标识支付方式解析 | `PaymentInstrumentService` / route resolver | 钱包标识只进入 `PaymentInstrumentRef` 或外部端点引用；route leg、posting plan 和 LedgerEntry 必须使用解析后的资金账户、信用账户或平台账户角色解析后的平台资金账户；预算组和 Spend Rule 只能作为控制上下文；缺解析结果时失败无副作用。 |
...
| TDD-WALLET-019 | 钱包应用服务边界 | `WalletAccountApplicationService` / boundary tests | 对外开户、账户能力查询、余额能力聚合和工具准入应通过 application/use-case facade 表达；调用方不得绕过 facade 拼接资源服务完成授权准入或资金责任解析。 |
