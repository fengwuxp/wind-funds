...
### 1.0.1 当前代码基线与 A0 Red 裁剪

当前代码能力基线以确认时 Git HEAD 和 OpenSpec/Harness 最新任务账本为准。TDD 可以把已验证的交易 Request 上下文只读、敏感上下文阻断、权益核心字段不得旁路、防御性拷贝、授权过期释放、授权强制完成、无授权退款、争议退款可区分性和授权后继并发竞争首轮闭环作为回归资产，但不能把这些资产外推为后续完整 dispute/chargeback case、授权支付工具应用入口、权益生产资金流、清结算、对账、资金数据治理或 P2 业务专项已完成。

A0 基线核验阶段不写测试代码，只输出 `redCandidateSet`、`targetAssets`、`schemaNeed`、`minimumAssertions`、验证命令和停止条件。只有用户确认单一 Execution Grant 后，才把候选 Red 转成实际测试写入；触碰 `contextVariables`、Request/DTO、route snapshot、交易事实快照或上下文导出时，必须回归 `ReadonlyContextVariables`、敏感字段阻断和权益核心字段不得旁路。
...
| 资金责任解析 | `FundingAllocationDecision` 唯一，最终主体只能是资金账户、信用账户或平台账户角色解析后的平台资金账户；若 B2-FR 选择 `funding-account-only`，本轮最终主体只能是资金账户。 | 资金责任缺失、不唯一、错币种、账户能力不足、预算组或 Spend Rule 被当作最终主体时失败；不得在字段仍只表达 `fundingAccountId` 时声明信用账户或平台角色责任主体生产可用。 |
...
授权交易不设计单独的“预算组授权”。预算组是共享卡场景下的预算范围、归属和审计维度，Spend Rule 承接预算额度、MCC、时间、商户、频控等授权前控制；真实资金责任仍需解析到资金账户、信用账户或平台角色解析后的平台资金账户。授权单场景按资金责任组合覆盖：共享卡 + 资金账户、共享卡 + 预算组 / Spend Rule + 资金账户、资金账户。
...
| TDD-AUTH-008 | 共享卡 + 资金账户授权 | `authorize` | 共享卡只作为支付工具和授权快照引用；资金账户 `AVAILABLE -> AUTHORIZATION`；不经过预算组 / Spend Rule 控制。 |
| TDD-AUTH-009 | 共享卡 + 预算组 / Spend Rule + 资金账户授权 | `authorize` | 共享卡挂在预算组下；Spend Rule 至少校验并记录预算额度控制证据；资金账户承担真实资金责任和账务占用；任一约束失败整体失败。 |
...
8.2 的授权组合场景必须按三种账户模型参数化验证：资金账户、共享卡 + 资金账户、共享卡 + 预算组 / Spend Rule + 资金账户。除非某条用例明确写出“不适用”及原因，否则不得只用单一资金账户模型替代组合场景；三种模型都要断言真实资金账户账务占用，涉及共享卡时断言工具快照，涉及预算组时断言预算范围和 Spend Rule 控制占用 / 释放证据。

| 用例 ID | 场景 | 账户模型覆盖 | 步骤断言 |
| --- | --- | --- | --- |
| TDD-AUTH-FLOW-001 | 授权批准 -> 部分撤销 -> 剩余完成 -> 完成后退款 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 授权占用、部分撤销释放、剩余完成消费、退款回补逐步断言；累计金额不超过原授权和已完成金额；预算控制证据随原快照回放。 |
| TDD-AUTH-FLOW-002 | 授权批准 -> 部分撤销 -> 剩余过期 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 部分撤销后仅剩余授权继续占用；过期只释放剩余授权；已撤销金额不得再次释放；预算控制释放只更新控制证据。 |
| TDD-AUTH-FLOW-003 | 授权批准 -> 全额完成 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 授权占用后全额完成；授权剩余归零；收款方或清算桶正确增加；预算组不生成 LedgerEntry。 |
| TDD-AUTH-FLOW-004 | 授权 -> 部分完成 -> 剩余撤销 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 资金账户模型断言 `AVAILABLE/AUTHORIZATION` 变化；共享卡模型额外断言工具快照；共享卡 + 预算控制模型额外断言 Spend Rule 控制占用、释放和资金账户账务占用同步；预算组不是独立授权入口；不生成预算组额度分录。 |
| TDD-AUTH-FLOW-005 | 授权完成后退款 + 费用处理 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 退款本金通过 `settleRefund` 承接；普通退款、争议退款和扣回类退款通过 reason、外部引用、投影展示和审计链路区分；费用退回走 `refundFee` 或独立退费；退款和退费必须回放原授权模型快照。 |
| TDD-AUTH-FLOW-006 | 授权部分完成 -> 剩余过期 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 完成金额进入收款方或清算桶；仅剩余授权释放回 `AVAILABLE`；共享卡和预算控制模型必须同步释放工具/预算控制占用证据；生命周期关闭。 |
| TDD-AUTH-FLOW-007 | 同一授权多次完成 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 多次完成累计正确；重复完成幂等；同一授权主记录不重复创建；预算控制模型累计完成不得超过 Spend Rule 控制占用和资金账户授权占用。 |
| TDD-AUTH-FLOW-008 | 授权完成后多次退款 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 多次退款累计正确；累计退款不超过已完成金额；部分退款后授权状态不得误收口为 `REFUNDED`，仍需保留可继续退款或争议处理的状态和剩余可退金额；三种模型都不得按当前绑定重新选路。 |
| TDD-AUTH-FLOW-009 | 无前置授权强制完成 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 通过 `settle` 强制完成模式按受控策略入账；必须记录 FORCE 模式、受信策略或审批快照、上限、外部原事实引用、凭证、原因、操作者和审计；不得补造授权占用；共享卡和预算控制模型只有在强制完成策略显式允许时才可解析到资金账户、信用账户或平台资金账户并入账，否则必须失败且无账务副作用；预算组和 Spend Rule 不入账。 |
| TDD-AUTH-FLOW-010 | 完成后过期释放请求 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 已完成金额不得被过期释放；只处理剩余授权，剩余为 0 时不得生成释放分录；共享卡和预算控制模型必须证明工具/Spend Rule 剩余占用不会被重复释放。 |
...
| TDD-VIEW-005 | 多维交易投影查询 | transaction view query / view index | 同一资金事实可按支付工具、资金账户、信用账户、预算组和 Spend Rule 查询只读账单、控制时间线或差异报告；查询结果引用原交易、route snapshot、LedgerEntry、BudgetGroup scope、Spend Rule 快照和规则决策日志，但不得把这些查询维度写成资金事实源或 LedgerEntry 主体。 |
...
| TDD-WALLET-013 | 钱包入口支付方式解析 | application facade / route resolver | 内部余额钱包、平台钱包和商户钱包入口必须解析为 `SubjectRef`，返利钱包或权益入口必须解析为 `BenefitSnapshot` 或等价快照；第三方钱包端点或通道 token 才进入 `PaymentInstrumentRef` / `ExternalAccountRef`。route leg、posting plan 和 LedgerEntry 必须使用解析后的资金账户、信用账户或平台账户角色解析后的平台资金账户；预算组和 Spend Rule 只能作为控制上下文；缺解析结果时失败无副作用。 |
...
| TDD-WALLET-015 | VCC 预付卡不创建资金账户 | wallet boundary tests / VCC contract tests | prepaid virtual card 只能创建或引用 `PaymentInstrumentRef`；若需要资金责任，必须通过资金责任解析关系解析到已存在或已确认的内部责任主体；不得因卡产品类型自动创建 `FundingAccount`。 |
| TDD-WALLET-016 | VCC 共享卡不创建信用账户 | wallet boundary tests / VCC contract tests | shared card 只能创建或引用支付工具、绑定快照、预算组、Spend Rule 快照和资金责任解析关系；不得因共享卡产品类型自动创建 `CreditAccount`；缺预算组、规则快照或资金责任主体时授权失败无账务副作用。 |
...
支付工具能力控制首批测试只证明工具准入和路由失败边界，不在未授权前扩展生产枚举、DTO、表结构或交易入口。进入编码前由 B2 或 B4 Execution Grant 明确写入范围和验证命令。
...
### 12.2 支付工具与 Spend Rule 生产可用性 Round 0
...
| Round 0 ID | 目标能力 | 首批 Red | Green 目标 | 不允许声明 |
| --- | --- | --- | --- | --- |
| R0-PI-001 | 支付工具能力准入 facade | 工具非 ACTIVE、方向不匹配、缺 RECEIVE/PAY/AUTHORIZE/REFUND/WITHDRAW、过期、错币种、敏感原文或绑定版本失效仍被放行。 | application facade 返回可解释失败或授权拒绝，不生成 route、posting、LedgerEntry。 | 不声明直接交易、授权、提现或收款生产可用。 |
| R0-PI-002 | 支付工具绑定版本快照 | 工具换绑、解绑、暂停或能力变化后，历史退款、撤销、退费或拒付按当前绑定重选路。 | 原路径回放使用原 route snapshot、原工具快照和原绑定版本；缺快照失败或人工处理。 | 不声明 route replay 全链路完成。 |
| R0-FR-001 | 资金责任目标主体 | 缺资金责任、多个责任命中、错币种、预算组或 Spend Rule 被作为最终责任主体时仍生成 route；B2-FR 未选择 `funding-account-only` 或 `targetSubjectType + targetSubjectId` 策略时直接进入实现。 | 输出唯一 `FundingAllocationDecision` 或等价不可变决策；选择 `funding-account-only` 时仅声明资金账户责任解析，选择目标主体迁移时同步 DTO、DDL/H2、摘要、fixture、route snapshot 和回放断言。 | 不声明 `fundingAccountId` 已支持信用账户和平台角色目标态；不混用资金责任字段策略。 |
| R0-SR-001 | Spend Rule 决策日志 | MCC、商户、时间窗、频控、单笔限额、累计限额或规则版本拒绝时仍继续授权或付款。 | 生成可审计规则决策和拒绝原因；拒绝无 route、posting、LedgerEntry。 | 不声明规则引擎或风控策略生产完备。 |
| R0-SR-002 | 预算预留和释放控制活动 | 授权预留、撤销、过期、部分完成或退款释放缺幂等、并发保护或控制活动仍更新预算可用。 | 只写 Spend Control Activity 和预算控制投影；不写资金交易、route leg 或 ledger entry。 | 不声明预算组 ledger 或预算余额桶生产可用。 |
| R0-AUTH-001 | 支付工具授权准入组合 | `authorizeByInstrument` 或等价入口绕过工具准入、Spend Rule、资金责任或账户能力，直接调用授权内核。 | 组合准入通过后构造账户主体型 canonical 授权请求；拒绝只留下授权拒绝和审计，不产生资金事实。 | 不替换 `FundsAuthorizationTransactionAuthorizeRequest.accountId`。 |

Round 0 通过只表示可以准备单一 Execution Grant；仍不得在缺少授权时写生产代码、测试代码、DDL/H2 schema 或运行时配置。若 Execution Grant 选择 B2，则优先处理支付工具能力准入和资金责任解析；若选择 B4，则优先处理授权准入组合；若选择 B5/B6/B8，则必须单独说明预算控制、交易投影和治理重放范围。

B6/B8 进入交易投影或重放验证时，投影输入必须限定为交易事实、冻结单、route snapshot、`paymentInstrumentRef`、`FundingAllocationDecision`、`SpendRuleDecisionLog`、`SpendControlActivity`、账本摘要、授权拒绝事实、清结算和对账差错；不得把交易投影通过写成账务事实、余额事实或生产 Done。
...
| TDD-RAIL-001A | VCC 支付工具定性边界 | VCC 卡、虚拟卡、卡 token、prepaid virtual card、shared card、预算组和 Spend Rule 都不得作为 ledger subject、余额投影主体或 `FundingAccount`；必须先进入 `PaymentInstrumentRef`、绑定快照、预算控制上下文和资金责任决策，再解析到内部资金账户、信用账户或平台账户角色解析后的平台资金账户。 |
...
| TDD-P2-VCC-004 | `VCC-AC-007` | prepaid virtual card 授权资金责任解析。 | 预付卡只作为支付工具快照；预付资金责任必须解析为内部责任主体；缺资金责任、缺财务确认引用或资金模式待确认时失败无 route、posting、entry。 |
| TDD-P2-VCC-005 | `VCC-AC-008` | shared card 授权绑定快照。 | 共享卡只作为工具和使用模式；必须固化使用人、绑定版本、预算组、资金责任解析关系和规则版本；多责任主体不唯一或缺绑定版本时失败无账务副作用。 |
...
| TDD-P2-VCC-008 | shared card 多资金责任主体同时命中。 | 授权失败且无账务副作用；不能随机选择资金账户、信用账户、平台资金账户或把预算组 / Spend Rule 当作资金来源；审计记录命中冲突。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期、强制完成、授权链退款、无授权退款、争议退款可区分性、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 首轮已把 `B4-CB-RED-001A / TDD-RED-017B` 回归化，`B4-AUTH-RACE` 首轮已把 `B4-RACE-RED-001` 回归化。 | 授权拒绝、完成、释放、退款、过期或拒付承接任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/过期/退款并发红线；拒付不得强制落独立 `chargeback` 公共入口，也不得被压缩为不可区分的普通退款；完整 dispute/chargeback case、独立 `chargeback` 一等目标 API 或更深跨节点并发控制仍需新 Grant。 |
...
| 授权交易服务 | authorize、reversal、settle、settleRefund、expire 单独验证；`settle` 同时覆盖普通完成和强制完成模式；`settleRefund` 同时覆盖完成后退款、无授权直接退款和争议类退款原因；授权资金责任覆盖资金账户、共享卡 + 资金账户、共享卡 + 预算组 / Spend Rule + 资金账户。 | 授权-撤销-完成-退款、授权部分完成后过期、授权-完成-剩余撤销、完成后退款和费用处理、多次完成、多次退款，均按 8.2 三种账户模型参数化覆盖。 | 授权拒绝无账务、普通完成不触碰 `LIMIT`、预算组只作为共享卡下的预算范围和 Spend Rule 控制维度、预算和信用账户不表达真实资金、过期和撤销终态可区分、强制完成不伪造授权占用；普通授权链退款继续引用原完成事实，无授权退款以空原授权流水进入 no-auth 语义，并保留 `externalReferenceSn`、退款原因、操作者和审计。 | 撤销超额、完成超额、授权退款超额、授权拒绝误写争议退款、缺原快照、完成后过期不得释放已完成金额、无原消费退款、无授权退款缺外部引用/原因/审计、无授权退款携带内部授权流水、强制完成缺策略。 |
