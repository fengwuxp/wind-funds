...
| P2 全球账户专项 | 现有出款、路由、FX 边界和对账设计可复用为边界基线；`P2-BOUNDARY-01` 已给出入金、出金、退汇和 FX 引用验证路径草案。 | 待补目标资产：`GlobalAccountInboundPostingTests`、`GlobalAccountOutboundTransitTests`、`GlobalAccountReturnFlowTests`、`GlobalAccountFxBoundaryTests`。 | 只验证外部受理、在途、退汇、费用和 FX 引用边界；未完成银行协议、跨境、法域和资金归属确认前，不得声明全球账户生产资金流完成。 |
