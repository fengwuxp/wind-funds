...
| TDD-WALLET-025 | 支付工具收款入口 | `PaymentInstrumentTransactionApplicationService#receiveByInstrument` | VA、ACH 或外部钱包端点收款必须先完成支付工具状态、方向、币种、调用方期望绑定版本、资金责任和账户能力准入，再委派账户主体型 `FundsDirectTransactionService#topup`；`channelCode` 可以是钱包支持的业务 rail / channel 别名，例如 `BANK_RAIL`、`ACH`、`wire-transfer` 或交易层稳定渠道，wallet application 必须先归一为交易层 `FundsTransactionChannel`，非法值必须友好拒绝且不得暴露 `valueOf` 实现细节；成功后资金账户 `AVAILABLE` 增加并生成标准 TOPUP 交易、route、posting plan、账本交易和分录，route snapshot 应保留支付工具快照和外部账户 providerCode；付款-only 工具、缺绑定、缺 `expectedBindingVersion`、未知 rail 或能力不匹配时必须在进入交易内核前失败，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用；该入口不改变交易 canonical 入参，不给 VA、卡或外部账户建账。 |
