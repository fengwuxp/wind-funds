...
| `DSL-BENEFIT-FUNDING-APPLY-001` | 权益让利资金事实最小契约、无权益空值语义、Phase/Batch 边界和旧 `benefitSnapshot` 拒绝。 | `TDD-BEN-001` 至 `TDD-BEN-009`、`TDD-BEN-ENTRY-005`、`TDD-BEN-ENTRY-006`。 | `FundsBenefitFundingApplicationService`、权益资金请求模型、DSL JSON parser、JSON fixture verifier。 | `TDD-BEN-RED-001`、`TDD-BEN-RED-002`、`TDD-BEN-RED-020`、`TDD-BEN-RED-023`。 |
...
| `AC-VIEW-*`、`AC-BALLOG-001`、`AC-REPLAY-005`、`RED-012`、`RED-036` | `TDD-LEDGER-007`、`TDD-LEDGER-012`、`TDD-VIEW-001` 至 `TDD-VIEW-005`、`TDD-REPLAY-005`、`TDD-RED-010`、`TDD-RED-013`、`TDD-RED-014`。 | 02-交易路由钱包账目与投影系分设计的余额投影、余额日志、交易投影、多维交易投影查询和预算/规则控制视图。 | P0 余额投影和 P1 交易投影共同承接；任一投影都不得反写事实；支付工具、预算组和 Spend Rule 只能作为查询维度和解释视图，不得成为资金事实源或账本主体。 |
...
| `transaction-face` | `FundsBenefitFundingApplicationService`、权益资金交易请求、权益来源 DTO、权益资金来源枚举 | 权益让利资金事实、来源引用、账务效果、资金性质、退款和撤销入口，以及旧 `benefitSnapshot` DSL 拒绝。 | 必须补 DSL/契约测试；如只落设计不落代码，必须声明未执行机器契约验收。 |
...
| --- | --- | --- | --- | --- |
| TDD-BEN-ENTRY-001 | 含权益直接支付 | `FundsBenefitFundingApplicationService#apply` + `FundsDirectTransactionService#pay` | 新增 benefitPay、couponPay、voucherPay 等直接交易专用服务方法，或把权益核心字段塞入 `contextVariables`。 | 先由权益资金应用服务生成权益资金交易或等价不可变事实；主支付仍使用 `pay`，并通过原交易、route snapshot、posting context 或投影引用权益资金事实。 |
...
| TDD-BEN-ENTRY-003 | 授权占券 | `FundsBenefitFundingApplicationService#apply` + `FundsAuthorizationTransactionService#authorize` | 新增 authorizeWithBenefit、couponAuthorize、voucherAuthorize，或授权拒绝仍核销、作废、冲减权益。 | 授权阶段只固化权益占用或资金事实引用；拒绝和准入失败无 route/posting/entry 和无权益副作用。 |
| TDD-BEN-ENTRY-004 | 授权后继事件权益回放 | `reverse` / `expire` / `settle` / `settleRefund` + `FundsBenefitFundingApplicationService#refund/reverse` | 后继事件接收当前重新计算的权益结果，或为完成、撤销、过期、退款分别新增权益专用方法。 | 仍使用授权生命周期入口；后继事件读取原授权事实、原 route snapshot 和原权益资金交易；`expire` 已作为授权生命周期既有入口补齐，不是权益专用入口。 |
...
| TDD-BEN-ENTRY-006 | 权益让利资金交易应用服务 | `FundsBenefitFundingApplicationService` + 交易路由 + 账本链路 | 新增 `FundsMarketingTransactionService`，或 application service 直接写 route/posting/LedgerEntry/余额投影，或拥有独立营销交易状态机，或停留在 prepare/decision DTO 不生成资金交易流水号，或要求调用方传入专业确认状态、事实摘要等过重字段。 | `FundsBenefitFundingApplicationService` 是交易级 application 服务；必须暴露 `apply`、`refund`、`reverse`，请求模型只表达让利方、受益方、金额、来源/规则引用、原订单或原交易引用等最小资金事实，返回资金交易流水号；实现层委派标准 route、transaction fact 和 ledger，专业确认和摘要由准入/证据链或实现侧处理。首个实现资产为 `FundsBenefitFundingApplicationServiceFlowTests`，覆盖 `POSTING_REQUIRED` 正向、退款、撤销、route/ledger 对齐、余额变化和 `NO_LEDGER` 失败无副作用。 |
...
| 补充事实和审计证据 | 历史缺快照补录、补充事实撤销、专业确认 TOCTOU、证据包过期、范围不匹配或敏感原文。 | 补充事实只追加不覆盖；最终写入前重校验专业确认和审计证据包；dry-run 不替代 apply；失败有人工处理入口。 | 补充事实进入退款、清结算、对账、归档或治理重放前必须补 `TDD-BEN-REPLAY-004`、`TDD-BEN-RED-026`、`TDD-BEN-RED-027` 和 `TDD-RACE-012`。 |
...
| TDD-WALLET-023 | 预算控制投影只读 | `SpendControlActivityApplicationService` / budget control projection query | 预算控制投影只能从 Spend Control Activity 聚合 reservedAmount、releasedAmount、remainingControlAmount、lastActivitySn 和 lastActivityAt；不得包含或反写 `AVAILABLE`、`FROZEN`、`AUTHORIZATION` 等账本余额桶，不得声明预算组 ledger 或预算余额桶生产可用。 |
