...
| TDD-AUTH-FLOW-009 | 无前置授权强制完成 | 资金账户；共享卡 + 资金账户；共享卡 + 预算组 / Spend Rule + 资金账户 | 通过 `settle` 强制完成模式按受控策略入账；必须记录策略、原因、操作者和审计；不得补造授权占用；共享卡和预算控制模型只有在强制完成策略显式允许时才可解析到资金账户、信用账户或平台资金账户并入账，否则必须失败且无账务副作用；预算组和 Spend Rule 不入账。 |
