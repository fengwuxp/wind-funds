...
当前代码能力基线以确认时 Git HEAD 和 OpenSpec/Harness 最新任务账本为准。TDD 可以把已验证的交易 Request 上下文只读、敏感上下文阻断、权益核心字段不得旁路、防御性拷贝、授权过期释放、授权强制完成、无授权退款和争议退款可区分性作为回归资产，但不能把这些资产外推为后续完整 dispute/chargeback case、授权并发、授权支付工具应用入口、权益生产资金流、清结算、对账、资金数据治理或 P2 业务专项已完成。
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期、强制完成、授权链退款、无授权退款、争议退款可区分性、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 首轮已把 `B4-CB-RED-001A / TDD-RED-017B` 回归化。 | 授权拒绝、完成、释放、退款、过期或拒付承接任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/过期/退款并发红线；拒付不得强制落独立 `chargeback` 公共入口，也不得被压缩为不可区分的普通退款；完整 dispute/chargeback case 或独立 `chargeback` 一等目标 API 仍需新 Grant。 |
