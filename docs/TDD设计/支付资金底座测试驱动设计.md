...
| `DSL-BENEFIT-SNAPSHOT-001` | 权益快照最小合法契约和无权益空值语义。 | `TDD-BEN-001`、`TDD-BEN-002`、`TDD-BEN-003`。 | `FundsInstructionSpec`、`FundsBenefitSnapshotSpec`、DSL JSON parser。 | `TDD-BEN-RED-001`、`TDD-BEN-RED-002`。 |
...
| `AC-BEN-001` 至 `AC-BEN-011`、`RED-050` 至 `RED-057` | `TDD-BEN-*`、`TDD-BEN-ENTRY-*`、`TDD-BEN-RED-*`、`TDD-BEN-RACE-*`，并按资金影响映射到 `TDD-DIR-*`、`TDD-AUTH-*`、`TDD-CLS-*`、`TDD-RECON-*`。 | 02-交易路由钱包账目与投影系分设计的权益快照契约、权益路由账务消费、授权占券和退款回放；03-清结算与对账系分设计的权益金额项和权益差错。 | 权益快照专项；Phase 1 只做契约承载时不宣称 route/posting/replay 完整闭合，Phase 2 后才要求资金影响组件进入完整证据链。 |
...
| TDD-BEN-005 | 零实付边界 | 当前主资金指令 `amount > 0` 时，券覆盖全额不能静默作为用户付款主指令；必须拆为补贴或代金券资金事实，或等待编码授权明确零金额规则。 |
...
| 生产门禁 | 请求态权益快照、route snapshot、交易事实快照、独立权益快照表或等价不可变存储、清结算和对账消费。 | 请求态可承载不等于生产可回放；缺不可变快照来源不得声明含权益生产链路 Done。 | `RED-058` 是生产 Done 门禁，B1/B1-10 不得越权声明。 |
...
| 权益快照 DSL | `FundsBenefitSnapshotSpecTests`、`FundsDslJsonContractTests`、`tests/src/test/resources/dsl-contract-cases/DSL-BENEFIT-SNAPSHOT-001.json` 已形成 B1-10 契约承载基线，覆盖无权益空值语义、最小合法快照、金额闭合、组件唯一性、退款处置表达、稳定摘要和 `CONTRACT_ONLY` JSON 夹具解析。 | 缺原权益快照失败、route/posting/replay 消费、平台补贴 posting-required、授权占券、不退券资金影响、清结算和对账拆分专项。 | Phase 1 只能声明契约承载；商户券 no-ledger、平台补贴 posting-required、授权占券和不退券资金影响进入 Phase 2/3 对应批次验收。若只改文档，不宣称机器验收已完成。 |
