...
| `DSL-BENEFIT-FUNDING-SETTLE-001` | 权益让利资金事实最小契约、无权益空值语义、Phase/Batch 边界和旧 `benefitSnapshot` 拒绝。 | `TDD-BEN-001` 至 `TDD-BEN-009`、`TDD-BEN-ENTRY-005`、`TDD-BEN-ENTRY-006`。 | `FundsBenefitFundingApplicationService`、权益资金请求模型、DSL JSON parser、JSON fixture verifier。 | `TDD-BEN-RED-001`、`TDD-BEN-RED-002`、`TDD-BEN-RED-020`、`TDD-BEN-RED-023`。 |
...
| `AC-BEN-001` 至 `AC-BEN-019`、`RED-050` 至 `RED-066` | `TDD-BEN-*`、`TDD-BEN-ENTRY-*`、`TDD-BEN-RED-*`、`TDD-BEN-RACE-*`，并按资金影响映射到 `TDD-DIR-*`、`TDD-AUTH-*`、`TDD-CLS-*`、`TDD-RECON-*`、`TDD-OPS-*`。 | 02-交易路由钱包账目与投影系分设计的权益资金交易契约、权益路由账务消费、授权占券、退款回放、消费范围、`fixtureLevel`、零实付表达、不可变事实源、补充权益事实、专业确认状态、审计证据包、使用者解释视图、证据最小化和外部规则核验；03-清结算与对账系分设计的权益金额项和权益差错。 | 权益资金事实专项；只做契约承载时不宣称 route/posting/replay 完整闭合，进入资金流消费后才要求资金影响组件进入完整证据链；含权益生产资金流必须填写权益准入证明，并证明使用者视图不误导、证据不过界、外部规则已核验。 |
...
| `transaction-face` | `FundsBenefitFundingApplicationService`、权益资金交易请求、权益来源 DTO、权益资金来源枚举 | 权益让利资金事实、来源引用、账务效果、资金性质、结算和退款型逆向入口，以及旧 `benefitSnapshot` DSL 拒绝。 | 必须补 DSL/契约测试；如只落设计不落代码，必须声明未执行机器契约验收。 |
...
| TDD-BEN-002 | 最小合法权益资金事实 | `businessSn`、成本承担主体、受益主体、金额、`ledgerEffect`、`fundingNature` 和 `benefitFundingSources` 可构造；来源引用具备 `sourceType`、`sourceId`、`ruleId` 或规则版本。外部决策通过 `sourceType=EXTERNAL_DECISION` 或 `sourceId` 表达，不再单设 `externalDecisionId`。 |
| TDD-BEN-003 | 权益金额闭合 | 权益资金事实只承接业务侧已决策金额；用户实付、商户应收、平台补贴和商户让利不得被净额混记。 |
| TDD-BEN-004 | 来源唯一性和引用完整 | 同一权益资金请求内来源引用应可稳定排序和去重；有核销、占用、退款或规则版本要求的来源必须保留对应引用。 |
...
| TDD-BEN-006 | 资金效果边界 | `NO_LEDGER`、`POSTING_REQUIRED`、`HOLD_ONLY`、`RELEASE_ONLY` 和 `REVERSAL_REQUIRED` 的处理边界必须明确；非入账权益事实不得生成 route、posting 或 LedgerEntry。 |
| TDD-BEN-007 | 权益资金事实稳定摘要 | 同一业务流水下，承担方、受益方、金额、账务效果、资金性质、来源引用或规则版本变化时，请求摘要必须变化；重复请求摘要冲突必须拒绝。 |
...
| TDD-BEN-009 | `contextVariables` 过渡白名单 | 只允许历史 `benefitSnapshotId`、`stableDigest`、`ruleVersion`、`refundDecisionId`、历史 `externalDecisionId` 等只读摘要；组件金额、资金责任、退款处置完整内容和当前营销规则必须失败。 |
...
| TDD-BEN-ENTRY-002 | 含权益直接退款 | `FundsBenefitFundingApplicationService#refund` + `FundsDirectTransactionService#refund` | 新增 benefitRefund，让退款请求携带当前重新计算的权益结果作为资金事实来源，或由资金底座自行判断券能不能退。 | 权益逆向走 `FundsBenefitFundingApplicationService#refund` 并引用原权益资金交易；主退款仍使用 `refund` 引用原资金交易和原 route snapshot；缺原权益资金事实失败或进入人工处理。 |
...
| TDD-BEN-ENTRY-006 | 权益让利资金交易应用服务 | `FundsBenefitFundingApplicationService` + 交易路由 + 账本链路 | 新增 `FundsMarketingTransactionService`，或 application service 直接写 route/posting/LedgerEntry/余额投影，或拥有独立营销交易状态机，或停留在 prepare/decision DTO 不生成资金交易流水号，或要求调用方传入专业确认状态、事实摘要等过重字段。 | `FundsBenefitFundingApplicationService` 是交易级 application 服务；必须暴露 `settle`、`refund`，请求模型只表达让利方、受益方、金额、来源/规则引用、原订单或原交易引用等最小资金事实，返回资金交易流水号；实现层委派标准 route、transaction fact 和 ledger，专业确认和摘要由准入/证据链或实现侧处理。首个实现资产为 `FundsBenefitFundingApplicationServiceFlowTests`，覆盖 `POSTING_REQUIRED` 结算、退款、业务取消冲回、route/ledger 对齐、余额变化和 `NO_LEDGER` 失败无副作用。 |
...
| TDD-BEN-MKT-004 | 储值、预付或礼品卡负债释放。 | 权益负债账户、预收待付或用户权益余额账户必须可追溯；专业口径未确认时阻断；退款时按原组件恢复责任或执行确认后的资金侧处置。 |
...
| TDD-BEN-REPLAY-003 | 权益资金事实源一致性 | `refund` / `reversal` / `expire` / `settleRefund` | 被设计选定的事实源必须能读取原权益资金交易流水、历史摘要、来源引用、规则版本、退款策略和决策流水；同一交易不能在不同事实源读到冲突摘要。 |
...
| --- | --- | --- | --- |
| 契约空值语义 | 无权益空值语义、最小合法权益资金事实、来源引用、账务效果、资金性质和旧 `benefitSnapshot` 拒绝。 | 核心字段一等化；`contextVariables` 只放扩展上下文或历史摘要；同业务键不同权益摘要必须冲突。 | 只能证明契约承载和 JSON 空值语义，不能外推为资金流完成。 |
...
| TDD-WALLET-022 | Spend Control Activity 记录和查询 | `SpendControlActivityApplicationService` | 通过准入、拒绝准入、预算预留、释放、过期、撤销或退款释放可以记录为控制活动；同一 `tenantId + activitySn` 同摘要重放返回既有活动，不同摘要拒绝；所有路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
| TDD-WALLET-023 | 预算控制投影只读 | `SpendControlActivityApplicationService` / budget control projection query | 预算控制投影只能从 Spend Control Activity 聚合 reservedAmount、consumedAmount、releasedAmount、remainingControlAmount、lastActivitySn 和 lastActivityAt；按预算组或 Spend Rule 查询时可以展示控制范围聚合，传入目标资金账户或信用账户时必须只聚合该账户控制活动，不得混入同预算组、同 Spend Rule 下其他账户或其他卡的活动；不得包含或反写 `AVAILABLE`、`FROZEN`、`AUTHORIZATION` 等账本余额桶，不得声明预算组 ledger 或预算余额桶生产可用。 |
...
| SR-CONTROL-003 | 控制活动幂等和摘要冲突 | 同一活动流水重复提交时覆盖旧事实、重复增加预算占用或允许摘要漂移。 | 同一 `tenantId + activitySn` 同摘要返回既有活动，不同摘要 fail-fast；预算控制投影不得重复累计。 | 不声明并发预留释放完整生产闭环，除非后续 Grant 覆盖并发测试。 |
...
B6/B8 进入交易投影或重放验证时，投影输入必须限定为交易事实、冻结单、route snapshot、`paymentInstrumentRef`、`AccountHierarchySnapshot`、`FundingAllocationDecision`、`SpendRuleDecisionLog`、`SpendControlActivity`、账本摘要、授权拒绝事实、清结算和对账差错；不得把交易投影通过写成账务事实、余额事实或生产 Done。
...
| 权益资金事实 DSL | `FundsBenefitFundingApplicationServiceContractTests`、`FundsBenefitFundingApplicationServiceFlowTests`、`FundsDslJsonContractTests` 已形成权益资金交易契约和首个 `POSTING_REQUIRED` 资金流基线，旧 `FundsBenefitSnapshotSpecTests` 与 `DSL-BENEFIT-SNAPSHOT-001.json` 已移出目标资产。 | `fixtureLevel` 边界、`contextVariables` 白名单、零实付平台补贴或储值权益、权益资金事实源一致性、缺原权益事实失败、route/posting/replay 消费、独立伴随指令原子性、补充权益事实、审计证据包 TOCTOU、非入账权益事实、授权占券、不退券资金影响、清结算、对账、投影、归档、冷热读取、治理重放、解释视图、证据最小化和外部规则核验拆分专项。 | 契约承载只能声明字段和 JSON 语义；商户券 no-ledger、平台补贴 posting-required、授权占券、零实付权益资金影响、伴随指令、补充事实和不退券资金影响必须进入资金流、清结算、对账、投影或治理专项验收。若只改文档，不宣称机器验收已完成。 |
...
| 权益语义 | L1 契约测试起步；进入资金影响后补 L2/L3/L4。 | 无权益空值语义、最小合法权益资金事实、来源引用、稳定摘要、退款处置表达、伴随指令组、补充权益事实、审计证据包和可理解视图。 | 核心语义藏 context、NO_LEDGER 入账、缺原权益事实重算当前规则、请求态或摘要冒充生产完成、伴随指令部分成功静默成功、补充事实覆盖原事实、证据包失效仍自动处理、视图误导使用者、证据越界、外部规则未核验仍放行。 | 已有契约承载和首个 `POSTING_REQUIRED` 资金流证据；非入账权益事实、授权占券、清结算、对账、投影、归档、冷热读取和治理重放仍为未覆盖范围。 | `FundsBenefitFundingApplicationServiceContractTests`、`FundsBenefitFundingApplicationServiceFlowTests`、`FundsDslJsonContractTests`、`DSL-BENEFIT-*` 夹具，目标补 route/posting/replay/清结算/对账/投影/归档/冷热读取/治理重放、解释视图、证据最小化和外部规则核验专项。 |
