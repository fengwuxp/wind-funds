...
| TDD-WALLET-024 | 交易消费支出控制活动 | `SpendControlTransactionConsumptionApplicationService` | 交易成功、授权完成、退款或争议退款后，可以回链原控制活动和原资金交易记录消费或补偿型控制活动；交易失败或拒绝不是业务释放事实，控制预留应随同一资金事务回滚，不通过本应用服务写 `RELEASED` 补偿；超时不是可信完成事实，不写控制流水；退款交易事实不得被 `consume` 降级为普通成功消费，非退款交易事实不得被 `refund` 降级为退款控制补偿；退款交易即使引用原资金交易，也必须能找到对应 `CONSUMED` 控制活动后才能记录补偿，只能走对应控制语义；缺原控制活动、缺已消费控制活动、金额超占用、目标账户或币种不一致时 fail-fast；同一活动流水同摘要并发重复写入必须回读既有活动，不得抛出数据库唯一键异常或重复累计投影；交易消费后的预算控制投影按目标账户查询时不得混入其他账户、其他卡或其他周期控制活动；全路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
