...
| 6 | P2-BOUNDARY-01 全球账户 / 收单 / FX 规划 | 产品 owner、架构 owner | 业务路径、证据入口、外部规则确认清单和停止条件。 | 文档切片 `git diff --check`；无明确 owner 和外部规则确认前不写代码。 |
...
| P2 全球账户专项 | 现有出款、路由、FX 边界和对账设计可复用为边界基线。 | 待补目标资产：`GlobalAccountInboundPostingTests`、`GlobalAccountOutboundTransitTests`、`GlobalAccountReturnFlowTests`、`GlobalAccountFxBoundaryTests`。 | 只验证外部受理、在途、退汇、费用和 FX 引用边界；未完成银行协议、跨境、法域和资金归属确认前，不得声明全球账户生产资金流完成。 |
| P2 收单业务专项 | 现有商户收款、清结算和对账设计可复用为边界基线。 | 待补目标资产：`AcquiringCaptureClearingTests`、`AcquiringSettlementPayoutTests`、`AcquiringRefundChargebackCollisionTests`、`AcquiringSensitiveDataBoundaryTests`。 | 只验证 capture/refund/dispute 归一、商户清结算和敏感数据边界；未完成 PSP、收单行、卡组织、商户入网/KYB 和 PCI 确认前，不得声明收单生产资金流完成。 |
