...
| `AC-PI-001` 至 `AC-PI-007`、`RED-046` 至 `RED-049` | `TDD-WALLET-005`、`TDD-WALLET-006`、`TDD-WALLET-009`、`TDD-WALLET-010`、`TDD-WALLET-011`、`TDD-WALLET-012`、`TDD-ROUTE-006`、`TDD-ROUTE-011` 至 `TDD-ROUTE-013`、`TDD-RACE-009`、`TDD-RED-001`、`TDD-RED-034` 至 `TDD-RED-037`。 | `02-交易路由钱包账目与投影系分设计` 的 PaymentInstrumentService、SpendSubjectFundingRelationService、RouteSnapshot 和 RouteReplay。 | 支付工具、绑定和资金来源专项；作为钱包账户与路由能力的一部分，不单独表达余额。 |
| `RED-020` | `SettlementPolicySpecTests`、`TDD-RED-031`。 | `DSL 设计` 的 `SettlementPolicy DSL` 和 `03-清结算与对账系分设计` 的结算策略边界。 | DSL 契约层；策略解析失败必须显式失败，不得静默按实时结算处理。 |
...
| TDD-WALLET-011 | 支付工具敏感数据治理 | `PaymentInstrumentService` / snapshot / log boundary | `instrumentNo`、binding snapshot、日志、导出和普通快照只能保存掩码号、别名或安全 token reference；不得落完整 PAN、CVV、密钥、token secret 或银行账户敏感号。 |
...
| TDD-RACE-004 | 冻结解冻和提现确认同时到达。 | 同一冻结来源只能被消费或释放一次；`FROZEN` 不为负；提现成功后不得再简单解冻。 |
| TDD-RACE-005 | 清算候选锁定和退款同时到达。 | 退款要么在锁定前排除候选，要么由已锁定批次的退款时序规则承接；不得重复扣减、重复候选或重复结算。 |
| TDD-RACE-006 | 结算锁定、出款回单和退款同时到达。 | `SETTLEMENT` 只能被消耗、释放或转差错一次；出款成功后退款进入追偿或后续抵扣，不得回滚已完成出款。 |
