...
| `AC-CLR-*`、`AC-SET-*`、`AC-SET-009`、`AC-REC-*`、`RED-030` 至 `RED-033`、`RED-037` 至 `RED-039` | `TDD-CLS-*`、`TDD-CLS-FLOW-*`、`TDD-SETTLE-*`、`TDD-SETTLE-005`、`TDD-RECON-*`、`TDD-RED-020` 至 `TDD-RED-025`。 | 03-清结算与对账系分设计的可清分明细、清分批次、清算候选、清算批次、结算单、出款单、出款解释状态、对账批次、差错单和追偿单。 | 清结算与对账专项；不混入交易主链路。 |
