...
| 预算控制范围和 Spend Rule | 预算控制范围创建不初始化 ledger；Spend Rule 拒绝不生成账务 route leg/posting/entry；预算预留、释放和规则命中只更新控制证据或只读视图。 | PRD 的预算控制口径、DSL 的 BudgetGroup 上下文和 Spend Rule 快照、系分的预算控制视图和规则决策日志。 |
...
| 授权权益 | 授权占券、完成核销、撤销或过期释放、授权拒绝不处理权益。 | 授权阶段只固化 hold 引用、权益资金事实引用或历史摘要；完成、撤销、过期读取原授权权益资金事实；拒绝无账务 route leg、posting、entry 和权益副作用。 | 授权资金流落地必须补授权生命周期和并发竞争测试。 |
...
| --- | --- | --- | --- | --- |
| PI-CAP-001 | 支付工具能力准入 facade | 工具非 ACTIVE、方向不匹配、缺 RECEIVE/PAY/AUTHORIZE/REFUND/WITHDRAW、过期、错币种、敏感原文或绑定版本失效仍被放行。 | application facade 返回可解释失败或授权拒绝，不生成账务 route leg、posting、LedgerEntry。 | 不声明直接交易、授权、提现或收款生产可用。 |
...
| SC-LOOP-04 准入和授权组合回归 | transaction owner、测试 owner | 外部 approve / decline 只作为决策证据；拒绝无账务 route leg、posting、LedgerEntry，通过仍继续支付工具、账户能力、资金责任和余额校验。 | `just verify-slice SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests tests`。 | 需要把规则通过等同于授权成功或资金可用。 |
...
| 用例 ID | 场景 | 必须断言 |
| --- | --- | --- |
| TDD-P2-VCC-006 | prepaid virtual card 与储值券同名混淆。 | 名称包含 prepaid 或 card 不足以进入 VCC；未确认发卡体系和预付资金责任时，只能拒绝或 contract-only；不得生成账务 route leg、posting、entry。 |
...
| 钱包账户 | `FundingAccountServiceImplTests`、`PlatformFundingAccountServiceImplTests`、`PaymentInstrumentServiceImplTests`、`SpendSubjectFundingRelationServiceImplTests`、`ControlAccountLedgerInitializationTests`、`WalletLayerBoundaryTests`。 | `CreditAccountServiceImplTests`、`BudgetGroupServiceImplTests`。 | wallet 只能作为产品门面和账户管理层；支付工具生命周期 facade 可作为 `wallet-face` 契约暴露，但实现应由 `transaction-impl` 消费 `wallet-face` 落地，`wallet-impl` 不得直接写交易事实、账本事实或绕过交易编排。 |
...
| 钱包账户和支付工具 | L2 服务测试 + L5 架构边界。 | 账户初始化、平台角色解析、支付工具绑定、方向/状态/资金责任准入、余额查询只读。 | 工具不可用仍入账、资金责任不唯一、敏感原文泄露、wallet 直接写交易或账本。 | 已有服务测试和 wallet 边界测试；信用账户、预算组基础服务专项仍需补齐。 | `FundingAccountServiceImplTests`、`PlatformFundingAccountServiceImplTests`、`PaymentInstrumentServiceImplTests`、`SpendSubjectFundingRelationServiceImplTests`、`ControlAccountLedgerInitializationTests`、`WalletLayerBoundaryTests`，目标补 `CreditAccountServiceImplTests`、`BudgetGroupServiceImplTests`。 |
