...
| SR-CONTROL-001 | 预算预留和释放控制活动 | 授权预留、撤销、过期、部分完成或退款释放缺幂等、并发保护或控制活动仍更新预算可用。 | 只写 Spend Control Activity 和预算控制投影；不写资金交易、route leg 或 ledger entry。 | 不声明支出控制范围 ledger 或预算余额桶生产可用。 |
