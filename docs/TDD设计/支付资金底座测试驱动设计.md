...
| 资金责任解析 | `targetSubjectType + targetSubjectId` 解析结果唯一；最终主体只能是资金账户、信用账户、VCC 关联资金/信用子账户或平台账户角色解析后的平台资金账户，并由 route participant/leg 显式承载。 | 资金责任缺失、不唯一、错币种、账户能力不足、缺应有的参与方账户层级快照、支出控制范围或 Spend Rule 被当作最终主体时失败；不得恢复旧资金账户镜像字段或独立资金分配决策对象。 |
