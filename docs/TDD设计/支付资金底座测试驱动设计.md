...
| TDD-AUTH-FLOW-004 | 共享卡授权 -> 部分完成 -> 剩余撤销 | 信用、预算、资金账户占用和释放同步；不回写 `LIMIT`。 |
...
| 授权交易服务 | authorize、reversal、settle、settleRefund、expire 单独验证；`settle` 同时覆盖普通完成和强制完成模式；拒付通过 `settleRefund` 的原因、凭证和审计上下文承接。 | 授权-撤销-完成-退款、授权部分完成后过期、共享卡授权-完成-剩余撤销、完成后拒付承接和费用、多次完成、多次退款。 | 授权拒绝无账务、普通完成不触碰 `LIMIT`、预算和信用账户不表达真实资金、过期和撤销终态可区分、强制完成不伪造授权占用。 | 撤销超额、完成超额、授权退款/拒付承接超额、授权拒绝误写拒付、缺原快照、完成后过期不得释放已完成金额、无原消费退款或拒付承接、强制完成缺策略。 |
