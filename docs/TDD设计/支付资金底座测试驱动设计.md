...
| 外部/场景 facade | `ExternalFundsEventApplicationServiceTests`、历史 Benefit facade/tests | 仅作当前行为与迁移 provenance；raw rail allowlist 和 Provider Benefit 场景 facade 不进入目标 Public API，后者按 R8B absence/zero-call 退役。 |
...
本节承接系分第十一章和 OpenSpec 迁移计划，只冻结剩余迁移的测试目标与编码准入条件。MIG-02B、MIG-03、MIG-04、MIG-05 与 MIG-07 Provider 当前范围均已 Green；MIG-02C、MIG-06/08、MIG-09 保持各自 blocker。当前只进入本文顶部活动入口。
...
| Authorization Complete | ordinary partial/multiple complete、SHARED replay、累计和后继不变。 | 逐 complete refund、force/no-auth complete、失败 proven-zero。 |
| Authorization Release | ordinary 未完成授权范围 release、Funding/SHARED Balance、原 HOLD replay、原/本次 Ledger、稳定 identity/digest 和后继累计不变。 | 逐 complete refund、settlement release、外部 payout failure/finality。 |
...
| --- | --- | --- | --- |
| authorization `release` | `authorize100 -> complete30 -> release20`，形成独立 release20，authorization remaining=50；原 authorize/complete DTO 不变。 | 同摘要重放、异摘要冲突、并发竞争、超剩余范围、错 tenant/币种/原 authorization/责任/route、PROCESSING/FAILED/混合、Ledger/Balance 失败、timeout 同 identity 查询；release 不撤销 completed。 | `PROVIDER_GREEN_CHECKER_PASS / CURRENT_SCOPE_COMPLETE` |
...
当前没有满足上述条件的下一代码切片：MIG-02C 缺权威逐 complete allocation；MIG-06/08 缺真实 Adapter/Consumer E4；MIG-09 缺 Consumer cutover 与 zero-call 证据。MIG-03/04/05/07 Provider 当前范围和 MIG-02B release 已完成，不得因旧矩阵重复建立 RED。
...
#### 20.13.4 Release ActionFact Contract/Entry Card

当前 `FundsAuthorizationTransactionFlowTests` fresh XML=`53/0F/0E/0S`，XML SHA-256=`d8e5fea6df0b243423699c4364c0ba164b026a27690872355c68b0449fa1c01d`。RED 仅修改该文件并新增以下 7 个方法；每个方法先建立合法 release 正例，再验证其对应边界，确保 projector 缺失时各形成一个稳定 `AssertionError`，不得出现 compile error 或第二根因：

1. `testSuccessfulAuthorizationReleaseShouldExposeStableActionFact`；
2. `testMultipleAuthorizationReleasesShouldExposeIndependentStableActionFacts`；
3. `testSharedAuthorizationReleaseShouldExposeOneActionFactWithoutDoublingMoney`；
4. `testAuthorizationReleaseActionFactShouldRemainStableAcrossReplayConflictAndLifecycleProgress`；同时覆盖 Base64URL v1 delimiter collision，以及主表/detail、action family、authorization root 同业务键冲突时列表与 identity 双查询 fail-closed；
5. `testAuthorizationReleaseActionFactShouldFailClosedForDurableFactTamper`；
6. `testAuthorizationReleaseActionFactShouldRejectRouteAndCumulativeTamper`；同时覆盖 root `completedAmount` 与全部 verified COMPLETE groups 合计不一致、complete sibling/route 损坏、root `reversedAmount` 与 verified REVERSAL groups 合计不一致；
7. `testConcurrentAuthorizationCompleteAndReleaseShouldExposeOnlyWinnerActionFact`。

RED 目标固定为 focused=`60/7F/0E/0S`、transaction=`185/7F/0E/0S`；原 53 项和 transaction 原 178 项全部保持通过。7 项 failure 只能是缺少已接受 release ActionFact 的断言，不得来自既有 REVERSAL、route、Ledger、Balance、fixture 或环境。Green 目标固定为 focused=`60/0F/0E/0S`、transaction=`185/0F/0E/0S`。

Green 只修改 `DefaultFundsTransactionQueryService.java`，复用现有 face/API/DTO。验收必须同时证明：UTF-8 Base64URL 无 padding 的 v1 三层身份无歧义且可反查原耐久字段；列表查询同时检查主表与 successor detail 候选，跨 table/family/root 冲突双查询 fail-closed；ordinary/multiple/SHARED Money 不重复；原 authorization ref 与逐 route provenance 完整；同 identity 重放稳定、异语义冲突；complete/release 并发只有合法赢家；全部 verified COMPLETE/REVERSAL groups 分别闭合 root completed/reversed 累计；details、references、route 或累计篡改双查询 fail-closed；后继生命周期和新进程查询不改变历史 DTO/digest。

后续验证固定为 Java 21 `just mvn-version`、写前/后 `just compile`、focused 与 `just test-transaction`、`just verify-public-contracts` 保持 `307/181/42`、`just verify-core-api` 保持 `104/96/4/4 / 1039 lines`、双 manifest/staged/scope fingerprint、`git diff --check` 和独立 Checker。真实 MySQL host、PMD、Mockito/JDK self-attach、Consumer E4/L4 不作为本公共库卡硬门，也不得冒充通过。

任何新 Public API/DTO/字段、DDL/Mapper、独立 ActionFact 表、第二写链、registry/factory、兼容层、Consumer 或白名单外文件需求均立即停止并返回 Human Owner。Contract/Entry Card 首轮两个 P1 的验收已并入原 7 个 RED 方法，不增加测试方法或文件；文件卡返工后的独立 Checker 已判定 `PASS / 0 P0 / 0 P1 / 0 P2`。

首轮 RED Execution fresh focused=`60/7F/0E/0S`、transaction=`185/7F/0E/0S`，7F 均精准缺 release ActionFact；但 RED Checker=`NOT PASS / 0 P0 / 2 P1 / 0 P2`。P1-1 为多个目标用例在首次查询前只证明 detail/Ledger 和 replay key 存在，没有逐 release 证明 Balance 与原 HOLD leg ID/amount 的精确映射；P1-2 为首笔 release DTO 未在同 root 后继 complete/release 改变 mutable cumulative 后回查，错误 digest 仍可能过测。该结果只形成受控 RED，不准出 Green。

Human Owner 随后授权同文件最小 RED rework。`assertAuthorizationReleasePhysicalFacts` 现按 authorization root + business identity + REVERSAL 精确筛选 detail/Ledger，逐 sibling 绑定原 RouteSnapshot 同主体 HOLD leg，核验 replay consumed IDs/amounts、`RELEASE_<originalLegId>` posting、Ledger ref、Money/currency、intent/scope/phase 与 entries；7 个目标用例在首次 ActionFact 查询前均完成 Funding 或 SHARED Balance delta。multiple 用例在首笔 release 后保存 DTO，再以第二笔 release 改变 `reversedAmount`；lifecycle 用例以后继 complete 改变 `completedAmount`；两者随后双查询原 DTO，历史 digest 不吸收 mutable root cumulative 成为可达断言。

最终 RED fresh focused=`60/7F/0E/0S`、transaction=`185/7F/0E/0S`，原 53 项与 Transaction 其余 178 项全绿；7F 仍全部为 `release action fact missing ... Expected size: 1 but was: 0`。Java 21 compile/clean compile=`21/21`，Public Contract=`307/181/42`，Core API=`96 stable / 4 experimental / 4 internal`。test SHA=`3d292c4b200383c5511743348a10291013078b2f682f8ef18e9714f554055db9`，authorization XML SHA=`bc0179ec0477ca817f632b19fe23c88c03ec86af5f1d4a5954e2693c342bd358`，query impl SHA 保持 `32e4123b906a0355f328c666074f2d74448aafd88a1df4dc0f5d230e6f547a57`，两文件有序 scope fingerprint=`94d2125cd2f2bb3ce59a495ca5a220cf0ea3e01bb329f96c309ad888ff5240da`；HEAD=`fc6b6e004a32eb1813534de05df7f844cc6edf95`，双 manifest=`68/7a68c0ebaba16d49ea2f684062605ed09da5bd7f273f47074a882f81e5503a5d`，staged empty，`git diff --check` PASS。最终 RED Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。

Human Owner 随后授权唯一单文件 Green，7 个 RED 测试保持 immutable。`DefaultFundsTransactionQueryService` 增加显式 release projector：列表、release identity 与普通 main identity 共享业务键歧义门；Base64URL v1 身份做 canonical 重编码校验；release group 校验原授权、本次/原 Ledger、责任 sibling 与逐 HOLD replay；全部 verified COMPLETE/REVERSAL 分别闭合 root completed/reversed。projection digest 覆盖本次动作、原 authorization fact digest、allocation、排序 sibling、Ledger、route 与 replay，明确排除 root 当前累计、余额、描述和时间。

最终 Green fresh focused=`60/0F/0E/0S`、transaction=`185/0F/0E/0S`；compile=`21/21`、Public Contract=`307/181/42`、Java 21 Core API=`96 stable / 4 experimental / 4 internal`。query impl SHA=`acd8ad0203ae0334b1431b8fba8667edeffba733b49546b19a86da4c032938c7`，immutable test SHA=`3d292c4b200383c5511743348a10291013078b2f682f8ef18e9714f554055db9`，authorization XML SHA=`6d19702fdca8ed8b3a15bdaa175e43f311867b76f04cec381254515dffa823f7`，两文件有序 scope fingerprint=`7d8a4e387accb63a8e2a1382ef49a777cabeaaba6b0cd40265e2f0cf5795f701`；HEAD 与双 manifest 保持 `fc6b6e... / 68 / 7a68c0eb...`，staged empty，`git diff --check` PASS。独立 Green Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。MIG-02B 当前范围关闭；下一切片必须重新形成 Entry Card，`ENTRY_CARD_REQUIRED / EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`。
...
| Benefit facade 退出 Provider | `D-R8B-001` 已以唯一 Provider 制品、真实 Bean、联合 schema/tx 和两 JVM restart 关闭 Capte 通用模块 library test host E4；Authority 对齐后再独立 RED/Green。 | Consumer 回归、同 identity 恢复、三处负向 allowlist、旧 Provider facade/serialized surface 零正向引用；未来部署宿主 L4 独立。 | 未闭合 Authority/RED/Green 就删除 facade，或把 Benefit 规则泛化进 Funds。 |
...
| 项目 | 冻结内容 |
| --- | --- |
| `slice_id / status` | `W5-MIG07-BREAKING-BEHAVIORAL-RED-ENTRY-CARD-REWORK-001 / plan-r2.147` 为历史状态。当前活动状态只见本文顶部。 |
...
Human Owner 已接受候选 A。未来 RED Entry Card 至少冻结以下可观察行为：
...
首轮 Checker 因 A 未明确完整 matched sibling set 判定 `NOT PASS / 0 P0 / 1 P1 / 0 P2`；C02/C03/C05 补齐 principal/PAYEE/可选 FEE_RECEIVER、唯一 distinct LedgerTransaction 和 fee 不误选约束后，最终 Entry Card Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。Human Owner 已接受 A，但当前仍不冻结测试文件 ADD/MODIFY、方法名、精确计数或验证命令；只进入独立 Contract Acceptance Checker，PASS 不构成 RED/Green Grant。

### 20.25 `MIG-03` Contract RED Entry Card

Contract Acceptance Checker 已判定 A `PASS / 0 P0 / 0 P1 / 0 P2`。本卡只冻结未来一个 RED 测试文件，不写生产或既有测试：

| Operation | 完整路径 | 方法与失败责任 |
| --- | --- | --- |
| `ADD` | `tests/src/test/java/com/wind/funds/reconciliation/contract/FundsActionLedgerClosurePublicContractTests.java` | `testRecordedEvidenceContractShouldExposeCompleteMatchedSiblingSet` 聚合缺失 service/DTO/method/sibling/digest 和禁止泄露项；`testClearingSourceRequestShouldAcceptOnlyStableActionRef` 聚合新 source ref 缺失与旧 tuple 仍存在。 |

测试必须使用反射字符串读取尚不存在的目标类型和成员；缺类/缺方法须进入 assertion failure，不得冒充 error。fresh RED 固定为 `2/2F/0E/0S`，且两项 failure message 分别包含 `recorded evidence contract missing` 与 `clearing source request contract mismatch`；不允许一项短路另一项。

RED 前后冻结检查：五个候选生产文件、四个既有 clearing caller测试、`FundsDirectTransactionFlowTests`、`FundsActionFactDTO/Ref`、Ledger face/impl、schema、POM 与 API baseline 哈希均不得变化。当前 `FundsActionFactContractTests` 继续 immutable，不把两个合同塞入旧 ActionFact contract 后隐藏独立失败。

RED 验证顺序固定为：Java 21 `just mvn-version`、`just compile`、`just verify-public-contracts`、`just test-one FundsActionLedgerClosurePublicContractTests tests`、fresh XML 复算和独立 Checker。`git diff --check` 仅在另获 Git 授权后执行；本卡不设置 MySQL 或 PMD 门禁。

Human Owner 已授权并消耗 RED Grant。唯一新增测试源码 SHA-256=`91ccb56ae80446f637b9e3500dc571507e3371001ce5ff33c1de90c10ab3e254`；Java 21 compile=`21/21`，Public Contract=`types 313 / models 186 / enums 42`。`just test-one` 在父级 Surefire HTML report 的私有 site descriptor / `~/.m2` 写锁处先行阻断；仅增加 `-DskipSurefireReport=true` 的等价 Maven 命令保留 test compilation、Surefire 和 XML，fresh=`2/2F/0E/0S`，XML SHA-256=`1dbcb0de75da6c22687a5d23c561d0ffb306a47e5db0fe9e29b19d80ab72f46d`。两项均为目标 `AssertionError`，独立 RED Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；后续 Green 文件卡见 20.26。

### 20.26 `MIG-03` Green Entry Card

#### 20.26.1 RED 到 Green 映射

| RED / 验收 | Green 生产责任 | 测试责任 | 通过条件 |
| --- | --- | --- | --- |
| RED-1 / C02-C05/C07 | 新增窄 service/DTO；`DefaultFundsTransactionQueryService` 复用 verified PAY projector，输出 principal proven-full 的完整 sibling recorded evidence 与 digest。 | immutable Contract test + `FundsDirectTransactionFlowTests`。 | Contract 第 1 项通过；有/无 fee group 完整，fee action/proven-zero/incomplete 返回 empty，后续余额变化不改 evidence。 |
| RED-2 / C01 | Request hard break 为 tenant + `sourceActionFactRef` + policy fields；旧三个字段/getter/setter 删除。 | immutable Contract test + 七个 caller/fixture机械迁移。 | Contract 第 2 项通过，仓内旧 request setter 与旧 Gate owner 生产/测试引用归零。 |
| C02/C03/C05 | Clearing 使用 recorded group + Ledger facts，验证全 sibling、唯一 LedgerTransaction、全部 posting plan和唯一 PAYEE/CLEARING credit。 | Clearing 新增 fee、missing sibling、different Ledger ref 三项；既有 mismatch/incomplete 测试迁移。 | 任一缺失/重复/多命中 fail-closed，fee 不误选，零新资金事实。 |
| C04 | unsupported/proven-zero/UNKNOWN/non-principal 无 recorded evidence。 | Direct fail-closed test + Clearing existing failed/unsupported safety assertions。 | 零 clearing candidate、零 Ledger/Balance 副作用。 |
| C06 | 复用既有 Ledger posting + balance projection 本地事务不变量，不改 Ledger 生产代码。 | 既有 direct flow、clearing ledger snapshot 与 rollback assertions。 | 不出现半套 LedgerTransaction/Plan/Entry/Balance。 |
| C08 | Clearing source digest 纳入 recorded-reference digest、Ledger digests、Gate evidence和 rule version。 | 既有 same-source replay、rule drift、Gate transaction tests。 | 同 digest 复用；异事实/规则冲突；rollback 不消费 Gate evidence。 |

#### 20.26.2 精确测试白名单与增量

Immutable：`tests/src/test/java/com/wind/funds/reconciliation/contract/FundsActionLedgerClosurePublicContractTests.java`，源码 SHA-256 必须保持 `91ccb56ae80446f637b9e3500dc571507e3371001ce5ff33c1de90c10ab3e254`。

允许 `MODIFY` 的测试/fixture 仅七个：

1. `FundsDirectTransactionFlowTests.java`：新增 `testRecordedEvidenceShouldExposeCompletePayGroupAndRemainStableAfterLaterBalanceChange`、`testRecordedEvidenceShouldFailClosedForFeeActionProvenZeroAndIncompleteGroup`，class `85 -> 87`。
2. `ClearingSplittableDetailApplicationServiceTests.java`：迁移既有 21 tests，新增 `testIdentifyShouldRejectNonFundsActionIdentityWithoutSideEffects`、`testIdentifyShouldSelectPayeeFromCompleteFeeBearingGroup`、`testIdentifyShouldFailClosedWhenRecordedSiblingIsMissing`、`testIdentifyShouldFailClosedWhenSiblingLedgerReferencesDiffer`，class `21 -> 25`。
3. `AcquiringSettlementBusinessFlowTests.java`：仅机械迁移 request/Gate identity，保持 `4` tests。
4. `AgentCommissionSettlementBusinessFlowTests.java`：仅机械迁移 request/Gate identity，保持 `4` tests。
5. `ReconciliationGateRequirementBehaviorTests.java`：迁移 ActionFact stage 与完整 source group，保持 `10` tests。
6. `ReconciliationTestFixture.java`：`CLEARING_SPLITTABLE_IDENTIFY` stage owner 从 `funds-transaction-detail` 改为 `funds`，无测试增量。
7. `ClearingSplitBatchApplicationServiceTests.java`：相关 fixture stage value 改为 principal ActionFact identity，保持 `16` tests。

任何其他测试、fixture、schema 或 Justfile 修改都停止重冻。

#### 20.26.3 Green 验证门

所有测试命令使用已证明只跳过 HTML report、不跳过 Surefire 的前缀：

```bash
MAVEN_OPTS=-DskipSurefireReport=true WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-one FundsActionLedgerClosurePublicContractTests tests
MAVEN_OPTS=-DskipSurefireReport=true WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just verify-slice FundsActionLedgerClosurePublicContractTests,FundsDirectTransactionFlowTests,ClearingSplittableDetailApplicationServiceTests,ClearingSplitBatchApplicationServiceTests,ReconciliationGateRequirementBehaviorTests,AcquiringSettlementBusinessFlowTests,AgentCommissionSettlementBusinessFlowTests tests
MAVEN_OPTS=-DskipSurefireReport=true WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-transaction
MAVEN_OPTS=-DskipSurefireReport=true WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-reconciliation
MAVEN_OPTS=-DskipSurefireReport=true WIND_FUNDS_JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home just test-business-flow
```

精确结果：Contract=`2/0F/0E/0S`；七类聚焦=`148/0F/0E/0S`（`2+87+25+16+10+4+4`）；transaction=`178/0F/0E/0S`；reconciliation=`242/0F/0E/0S`（当前 18 类 XML 基线 `238 + Clearing 4`）；business-flow=`206/0F/0E/0S`；Java 21 compile=`21/21`；Public Contract=`types 315 / models 187 / enums 42`。每项必须读取 fresh Surefire XML；没有 fresh XML、实际执行数为零或计数漂移均停止。

本公共库卡不设置真实 MySQL host 或 PMD 门禁；Git、Consumer E4/L4、发布与生产仍需独立授权。独立 Green Entry Card Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；后续 Green Execution 未通过 Checker，当前由 20.27 接替。

### 20.27 `MIG-03` Ledger Persisted Digest Contract Entry Card

#### 20.27.1 价值到可证伪行为

本卡价值只有在“Ledger 自己能从持久化事实重建摘要，并在 read boundary 阻断篡改”时成立；仅让 hash 非空、仅让 H2 测试变绿或由 Reconciliation 复制算法均不算完成。

| ID | 业务/资金场景 | 必须证明 | 失败或停止条件 |
| --- | --- | --- | --- |
| `MIG03-LD-001` | nanos transaction time 经不同数据库精度持久化 | writer 先归一到秒，persisted transaction/entry 时间与摘要重建输入完全相同 | 回读后摘要不同；依赖 DATETIME(6) 才通过 |
| `MIG03-LD-002` | fresh balanced posting | transaction、plan、entry 三层摘要均能从持久化 entities 重建；借贷、余额投影和原行为不变 | 写后自校验失败仍提交任一事实或余额 |
| `MIG03-LD-003` | same-key replay | 同 normalized facts 返回既有 transaction，零新 plan/entry/balance | 换 time precision 或 fallback digest 才复用 |
| `MIG03-LD-004` | legacy digest | 当前唯一 v1 不匹配即拒绝且不重写历史 | 接受 legacy golden、双验、回填或静默放行 |
| `MIG03-LD-005` | transaction/plan/entry 分别被改写 | stable-SN get/query 在 Ledger 内部 fail-closed，错误能定位层级与 SN | 只比较 stored sha 非空；调用方自行声明 verified |
| `MIG03-LD-006` | direct PAY / acquiring / commission clearing | Ledger integrity error 发生在 candidate 持久化与 Gate evidence 消费前 | 形成 EXCLUDED 持久行、消费 Gate 或自动修复 Ledger |
| `MIG03-LD-007` | 后续合法余额动作 | 历史 aggregate digest 不依赖 current balance，后续动作不使原事实失效 | 把 current balance 纳入 persisted digest |
| `MIG03-LD-008` | child 缺失、重复、父引用漂移 | aggregate 数量、稳定顺序、父子引用和每层 digest 一起验证 | first/latest、固定页截断或只验被选 PAYEE entry |
| `MIG03-LD-009` | request `exchangeRate=1`，持久化回读 `1.00000000` | 两者经去尾零 plain decimal 归一后生成同一 canonical digest，且非等值汇率仍冲突 | 直接使用 BigDecimal scale/toString；通过忽略 exchangeRate 变绿 |

#### 20.27.2 后续 RED 候选与 immutable 边界

未来 RED Entry Card 只能在以下四个现有测试文件内选择最小用例，精确执行数和 expected failures 必须基于当时 fresh XML 重新冻结：

1. `LedgerTransactionServiceImplTests.java`：删除 legacy compatibility 预期，增加 nanos round-trip、exchange-rate scale-drift round-trip、no-compat replay 和三层 tamper。
2. `LedgerTransactionServiceFactQueryTests.java`：保持 face 方法 shape，不新增 verifier API；证明 stable-SN read 的 fail-closed。
3. `ClearingSplittableDetailApplicationServiceTests.java`：三层 tamper 均零 candidate、零 Gate consumption、零 Ledger/Balance 副作用。
4. `ReconciliationGateRequirementBehaviorTests.java`：只迁移手工 Ledger fixture 的摘要生成，测试数量和 Gate 行为不变。

`FundsActionLedgerClosurePublicContractTests.java`、`FundsDirectTransactionFlowTests.java`、Acquiring/Agent flow、其余 reconciliation tests、`jdbc-schema.sql`、Justfile 和全部生产源码在 RED 阶段 immutable。RED 不允许通过预改 writer、弱化 digest 字段、把异常改成 EXCLUDED 或写测试专用 bypass 形成。

#### 20.27.3 未来 Green 验证门

只有独立 RED Checker PASS 并取得 Green Grant 后，才允许单个 Ledger impl 生产候选闭合。Green 至少执行：

```bash
just mvn-version
just compile
just verify-public-contracts
just test-one LedgerTransactionServiceImplTests tests
just test-one LedgerTransactionServiceFactQueryTests tests
just test-one ClearingSplittableDetailApplicationServiceTests tests
just test-one ReconciliationGateRequirementBehaviorTests tests
just test-ledger
...
测试命令继续使用已证明只跳过 HTML report 的 `MAVEN_OPTS=-DskipSurefireReport=true` 环境前缀，并逐类读取 fresh Surefire XML。最终还必须证明 immutable Contract SHA、staged empty、稳定 workspace manifest 与 `git diff --check`；本公共库不设置真实 MySQL host 或 PMD 门禁。

本轮是文档设计，不运行测试、不修改 fixture 或生产源码。最终 Entry Card Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；当前只进入 Human Owner 的 Ledger Digest RED Entry Card 文档授权决策，未授权 RED、测试执行或 Green。

### 20.28 `MIG-03` Ledger Persisted Digest RED Entry Card

#### 20.28.1 基线与目标计数

当前一手 XML/源码基线为：

| Test class | 当前执行数 | RED 后执行数 | Expected failures |
| --- | ---: | ---: | ---: |
| `LedgerTransactionServiceImplTests` | 9 | 10 | 2 |
| `LedgerTransactionServiceFactQueryTests` | 5 | 8 | 3 |
| `ClearingSplittableDetailApplicationServiceTests` | 25 | 28 | 3 |
| `ReconciliationGateRequirementBehaviorTests` | 10 | 10 | 0 |
| **合计** | **49** | **56** | **8** |

RED 精确结果固定为 `56 tests / 8 failures / 0 errors / 0 skipped`。Ledger impl 通过 8、Ledger query 通过 5、Clearing 通过 25、Gate 通过 10；八个 failure 之外的 `48` 个既有行为必须保持 Green。

#### 20.28.2 测试设计

`LedgerTransactionServiceImplTests`：

1. 将 `testPersistedLegacyLedgerDigestShouldReplayAndRejectConflictWithoutChangingFacts` 重命名为 `testPersistedLegacyLedgerDigestShouldFailClosedWithoutChangingFacts`，保留事实快照，改为断言 same-key replay 因 legacy digest 抛错，label=`ledger legacy digest must be rejected`。
2. 新增 `testPersistedDigestShouldRoundTripCanonicalTimeAndDecimalScale`：使用非零 nanos transaction time 与 `BigDecimal.ONE` 写入；从 persisted transaction/plan/entry 机械重建 10.23 canonical digest，断言 stored digest 等于 expected，并验证 `1 / 1.00000000` 同 token、非等值 rate 冲突、same-key replay 零新增。当前 writer 产生不同 digest，形成 1 failure。

`LedgerTransactionServiceFactQueryTests`：

3. 新增 parameterized `testStableSnQueriesShouldRejectTamperedPersistedLedgerAggregate`，参数固定 `TRANSACTION / POSTING_PLAN / LEDGER_ENTRY`。每次先用真实 posting gateway 形成完整 facts，只 UPDATE 目标层的 `sha256`，再调用既有 `getLedgerTransactionBySn` 或 `getLedgerEntryBySn/queryLedgerEntries`，断言 integrity error 包含 layer/SN，且 Ledger/Balance snapshot 不变。当前 reader 不验证，形成 3 failures。

`ClearingSplittableDetailApplicationServiceTests`：

4. 新增 parameterized `testIdentifyShouldFailClosedWhenPersistedLedgerDigestIsTampered`，参数同上。每次使用完整 ActionFact sibling 与 passed Gate fixture，只改目标 Ledger digest；断言 identify 抛 integrity error、`detailCount=0`、Gate consumed evidence=0、Ledger/Balance snapshot 不变。当前 identify 会继续 admission，形成 3 failures。
5. 将本类手工 transaction/plan/entry fixture 改为按 10.23 生成目标 canonical values；这只是让非 tamper 用例继续表达有效 Ledger facts，不允许硬编码绕过 production verifier。

`ReconciliationGateRequirementBehaviorTests`：

6. 只迁移 `prepareStageSourceFacts/insertLedgerFacts` 的手工摘要为目标 canonical values；10 个 Gate 行为、断言和执行数完全不变。

测试内 canonical helper 只允许私有、确定性、无 Spring Bean/生产依赖地投影冻结字段；不得新建共享 helper 文件、Fake Ledger service 或接受 stored digest 作为 expected。

#### 20.28.3 RED 执行与验证

RED 已执行原 focused/test-ledger/reconciliation/transaction/business 主链。返工后的可执行顺序如下；其中 non-assembler 当前结果来自同一源码状态下五份 fresh XML 合计，Green 前必须用新增的精确组合命令重新 fresh 执行：
...
MAVEN_OPTS=-DskipSurefireReport=true just verify-slice LedgerTransactionServiceImplTests,LedgerTransactionServiceFactQueryTests,ClearingSplittableDetailApplicationServiceTests,ReconciliationGateRequirementBehaviorTests tests

# 本切片 Ledger owned behavior 必过门；精确排除与候选生产文件无交集的 assembler 环境类。
MAVEN_OPTS=-DskipSurefireReport=true just verify-slice DefaultLedgerTransactionPostingServiceImplTests,LedgerBalanceProjectionServiceImplTests,LedgerServiceImplTests,LedgerTransactionServiceFactQueryTests,LedgerTransactionServiceImplTests tests

# 只作完整 Ledger 诊断，不作为当前环境下的本切片准出门。
MAVEN_OPTS=-DskipSurefireReport=true just test-ledger
MAVEN_OPTS=-DskipSurefireReport=true just test-reconciliation
MAVEN_OPTS=-DskipSurefireReport=true just test-transaction
MAVEN_OPTS=-DskipSurefireReport=true just test-business-flow
```

目标：

- focused=`56/8F/0E/0S`；
- non-assembler Ledger 当前五份 fresh XML 合计=`50/5F/0E/0S`；Green 前精确组合命令必须 fresh 得到同一结果；
- assembler environment observation 当前=`15/0F/15E/0S`，15E 必须全部为 `MockitoInitializationException -> ByteBuddyAgent -> Could not self-attach`；该结果不是行为 PASS；
- 完整 `test-ledger` 当前只记诊断=`65/5F/15E/0S`；仅当 assembler 环境可用并达到 `15/0F/0E/0S` 后，才可声明完整 Ledger=`65/5F/0E/0S`；
- test-reconciliation=`245/3F/0E/0S`；
- test-transaction=`178/0F/0E/0S`；
- test-business-flow=`206/0F/0E/0S`；
- compile=`21/21`，Public Contract=`315/187/42`；
- `FundsActionLedgerClosurePublicContractTests` SHA 保持 `91ccb56ae80446f637b9e3500dc571507e3371001ce5ff33c1de90c10ab3e254`；
- staged empty、workspace manifest stable、`git diff --check` PASS。

non-assembler 五类出现 5 个目标 failure 之外的任何 failure/error、Reconciliation 出现 3 个目标 failure 之外的任何 failure/error，或 transaction/business-flow 出现任何 failure/error，RED 均不可准出。assembler 若能够进入行为测试则必须 `15/0F/0E/0S`；若仍启动失败，15E 必须逐项同根于冻结的 Mockito/ByteBuddy self-attach，任何不同 error 都阻断并重冻。不得用 assembler 环境失败宣称完整 Ledger 通过，也不得让该白名单外环境失败覆盖 focused/non-assembler 的已观察行为证据。

实际 RED 执行结果为 focused=`56/8F/0E/0S`、Reconciliation=`245/3F/0E/0S`、Transaction=`178/0F/0E/0S`、Business=`206/0F/0E/0S`、compile=`21/21`、Public Contract=`315/187/42`；同一源码状态下 non-assembler 五份 fresh XML 合计=`50/5F/0E/0S`。原扩大门禁 Checker 因完整 `test-ledger=65/5F/15E/0S` 与错误冻结的 `0E` 目标冲突而判定 `NOT PASS / 0 P0 / 1 P1 / 0 P2`；本轮只返工证据层，不修改或运行测试。

门禁分层经独立 Checker 判定 `PASS / 0 P0 / 0 P1 / 0 P2`，并以 `plan-r2.220` 收口；当前由 20.29 Green Entry Card 接替。

### 20.29 `MIG-03` Ledger Persisted Digest Green Entry Card

#### 20.29.1 Immutable acceptance evidence

Green 不得修改以下文件，SHA-256 必须保持：

| 文件 | SHA-256 |
| --- | --- |
| `LedgerTransactionServiceImplTests.java` | `a897a56579acc1ac48c2fb11bba721b89305ed95b1da274594db91ccde6b25fe` |
| `LedgerTransactionServiceFactQueryTests.java` | `eed2f4f58d188f20ce1cc1e6c77b9599e2e73981785b3b42e0e1c8e61da886a9` |
| `ClearingSplittableDetailApplicationServiceTests.java` | `07555f3d6f81a700f7439d8e7b54d4e41566248ffc1d7f003069f1f13093747e` |
| `ReconciliationGateRequirementBehaviorTests.java` | `95d6126c4d3a2b73b60903ce4d997d36f69b72eed7c2bc9ffccce2648ea3f524` |
| `FundsActionLedgerClosurePublicContractTests.java` | `91ccb56ae80446f637b9e3500dc571507e3371001ce5ff33c1de90c10ab3e254` |

生产唯一候选 `LedgerTransactionServiceImpl.java` 的执行前 SHA-256 为 `5acbbdb67273c419095b0eb79e845bba97a8286eda638b2d0b19dd4055f7a866`。

#### 20.29.2 Green 精确结果

- focused 四类=`56/0F/0E/0S`；
- non-assembler Ledger 五类=`50/0F/0E/0S`；
- test-reconciliation=`245/0F/0E/0S`；
- test-transaction=`178/0F/0E/0S`；
- test-business-flow=`206/0F/0E/0S`；
- compile=`21/21`，Public Contract=`315/187/42`；
- 完整 `test-ledger` 当前环境只作诊断，目标为 `65/0F/15E/0S` 且 15E 必须仍全部同根于 assembler Mockito/ByteBuddy self-attach；环境可用时必须 `65/0F/0E/0S`。任何生产 failure 或不同 error 都阻断。
...
Green 前四测试的 8 个 RED 必须 fresh 可复现；Green 后只改生产文件让其自然转绿，不得调整 expected builder、failure label、fixture 或 allowlist。focused、non-assembler、Reconciliation、Transaction、Business 任一 failure/error，immutable SHA 漂移、生产白名单扩大、legacy/dual digest、API/DDL/Mapper/Reconciliation 修改或 `git diff --check` 失败都立即停止。assembler 15E 只保留环境 observation，不是 Green PASS。

Green Entry Card Checker 判定 `NOT PASS / 0 P0 / 1 P1 / 0 P2`：当前 3 个 tamper invocation 只能证明 transaction/plan 的 by-SN aggregate read 与 entry by-SN，不足以证明 by-id、entry query 和 `existsPostingPlan`。下一候选只重开 `LedgerTransactionServiceFactQueryTests.java`，不增加 invocation/failure 数：每个 layer 聚合调用全部相关 exact read，将未拒绝入口名称收集后用原 stable label 断言集合为空；仍断言零事实副作用。

Human Owner 已授权 `W5-MIG03-LEDGER-PERSISTED-DIGEST-EXACT-READ-RED-COVERAGE-REWORK-001`。Maker 仅重开 `LedgerTransactionServiceFactQueryTests.java`，保持 `8 tests / 3 failures`，并在三个 stable-label failure 中聚合暴露全部 exact read guard 缺口；fresh focused=`56/8F/0E/0S`、non-assembler Ledger=`50/5F/0E/0S`、Reconciliation=`245/3F/0E/0S`、Transaction=`178/0F/0E/0S`、Business=`206/0F/0E/0S`、compile=`21/21`、Public Contract 清册=`315/187/42`。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`，原 Green Entry Card 已重新 PASS。该阶段以 `LEDGER_DIGEST_EXACT_READ_RED_COVERAGE_REWORK_INDEPENDENT_CHECKER_PASS / LEDGER_DIGEST_GREEN_ENTRY_CARD_INDEPENDENT_CHECKER_PASS / GREEN_EXECUTION_GRANT_NO / CODE_FREEZE / plan-r2.223` 收口，当前由 20.30 接替。

### 20.30 `MIG-03` Ledger Digest Green 结果与 Legacy Clearing 测试卡候选

#### 20.30.1 Green 一手结果

Green 前已 fresh 复现 `56/8F/0E/0S`。只修改 `LedgerTransactionServiceImpl.java` 后，fresh focused 为：

| Test class | Tests | Failures | Errors | Skipped |
| --- | ---: | ---: | ---: | ---: |
| `LedgerTransactionServiceImplTests` | 10 | 0 | 0 | 0 |
| `LedgerTransactionServiceFactQueryTests` | 8 | 0 | 0 | 0 |
| `ReconciliationGateRequirementBehaviorTests` | 10 | 0 | 0 | 0 |
| `ClearingSplittableDetailApplicationServiceTests` | 28 | 3 | 1 | 0 |
| **合计** | **56** | **3** | **1** | **0** |

原 8 个 expected failures 已全部关闭；compile=`21/21`。5 个 immutable acceptance 文件 SHA-256 均保持 20.29.1 值，生产文件 SHA-256=`d5b8fd6993913b66e0bfd24dc38aed04b2b90c11d101fd17167e936b219affa4`。独立 Checker=`NOT PASS / 0 P0 / 1 P1 / 0 P2`：唯一 P1 是原卡 `focused=56/0` 不可达，未发现已审生产范围内的 P0/P1/P2 实现缺陷。Green 停在 focused，未继续运行 non-assembler、reconciliation、transaction 或 business 扩大门禁。

#### 20.30.2 四个合同冲突

| 旧测试 | 冲突 | 候选迁移责任 |
| --- | --- | --- |
| `testIdentifyShouldExcludeWhenPostingPlanIsMissing` | 删除 plan 后仍要求 Ledger 返回 aggregate。 | 断言 Ledger parent-integrity fail-closed，保留 candidate/Gate/Ledger/Balance 零副作用。 |
| `testIdentifyShouldExcludeWhenLedgerTransactionBusinessIdentityDiffers` | 修改 transaction 业务身份后未重算 aggregate digest。 | 重算合法 transaction aggregate v1，再保留原 Clearing exclusion 断言。 |
| `testIdentifyShouldExcludeClearingDebitDecreaseEntry` | 修改 entry direction/effect 后未重算 entry/plan/transaction digest。 | 重算三层合法 v1，再保留原 Clearing exclusion 断言。 |
| `testIdentifyShouldExcludeNonClearingEntry` | 修改 entry subject/role 后未重算 entry/plan/transaction digest。 | 重算三层合法 v1，再保留原 Clearing exclusion 断言。 |

#### 20.30.3 下一授权卡边界

下一候选任务为 `W5-MIG03-LEDGER-PERSISTED-DIGEST-GREEN-LEGACY-CLEARING-TEST-ENTRY-CARD-REWORK-001`，只允许先在五份权威文档冻结上述测试迁移；未来测试执行候选白名单仅 `MODIFY tests/src/test/java/com/wind/funds/reconciliation/application/clearing/impl/ClearingSplittableDetailApplicationServiceTests.java`，`ADD=0 / DELETE=0`。该文件已有 persisted v1 测试 builder，必须复用；不新建 helper、兼容层或第二摘要算法。

返工卡必须冻结迁移后的精确测试计数、断言和执行命令，并继续以 `56/0F/0E/0S` 为 focused Green 目标。任何 Clearing production、Ledger production、共享 fixture、API/DTO/schema/Mapper/POM、其他测试或 Consumer 修改需求都立即停止，返回 Human Owner 决策。该阶段以 `LEGACY_CLEARING_TEST_ENTRY_CARD_REWORK_GRANT_NO / GREEN_EXECUTION_PAUSED / DOCUMENTATION_ONLY / CODE_FREEZE / plan-r2.224` 收口，当前由 20.31 接替。

### 20.31 `MIG-03` Legacy Clearing 测试迁移 Entry Card

#### 20.31.1 价值到可证伪行为

| 场景 | Ledger 输入 | Owner 与预期 |
| --- | --- | --- |
| posting plan 缺失 | `InvalidLedgerAggregate` | Ledger exact read 必须先拒绝；Clearing candidate、成功 Gate consumption 与额外 Ledger/Balance 变化均为零。 |
| transaction business identity 不同 | 重算后为 `VerifiedLedgerAggregate` | Clearing 返回 `EXCLUDED / SOURCE_FACT_MISMATCH`，证明动作归属不一致不会进入清分。 |
| payee entry 为 AVAILABLE | 重算后为 `VerifiedLedgerAggregate` | Clearing 仍拒绝“缺少唯一 PAYEE/CLEARING credit”，证明其他余额桶不能冒充待清分来源。 |
| payee entry 为 DEBIT/DECREASE/REFUND | 重算后为 `VerifiedLedgerAggregate` | Clearing 仍拒绝“无唯一账本分录”，证明逆向/扣减不能冒充正向待清分本金。 |

三个业务过滤场景的 persisted v1 重算只用于构造可验证输入，其中 AVAILABLE 场景同时使用匹配的 `ASSET` category；不接受 legacy、空摘要或摘要不一致。missing plan 不重算，专门证明 Ledger fail-closed。测试仍观察 ApplicationService 的 DTO、异常和资金副作用，不断言私有生产方法或内部调用顺序。

#### 20.31.2 精确测试写入卡

- 唯一未来写入文件：`MODIFY tests/src/test/java/com/wind/funds/reconciliation/application/clearing/impl/ClearingSplittableDetailApplicationServiceTests.java`。
- 执行前 SHA-256：`07555f3d6f81a700f7439d8e7b54d4e41566248ffc1d7f003069f1f13093747e`。
- 只迁移四个既有测试方法；不增加、删除或重命名测试，`ClearingSplittableDetailApplicationServiceTests` 保持 `28 tests`。
- 复用同文件 `ledgerTransactionDigest`、`ledgerPostingPlanDigestFacts`、`ledgerEntryDigestFacts`、`postingPlanAggregate`；只允许让 transaction builder 显式接收 `businessSn` 并迁移同文件 call sites，不新增共享 helper 文件、通用 mutation abstraction 或 production test hook。
- 其他四个 immutable acceptance 文件保持 20.29.1 SHA；`LedgerTransactionServiceImpl.java` 保持 `d5b8fd6993913b66e0bfd24dc38aed04b2b90c11d101fd17167e936b219affa4`。

#### 20.31.3 执行与验证顺序

未来 Test Rework 获独立授权后按以下顺序执行：
...
# 只作当前环境诊断，不作为本卡独立准出证据。
MAVEN_OPTS=-DskipSurefireReport=true just test-ledger
```

写入前目标固定为 focused=`56/3F/1E/0S`；迁移后必须 focused=`56/0F/0E/0S`、non-assembler Ledger=`50/0F/0E/0S`、Reconciliation=`245/0F/0E/0S`、Transaction=`178/0F/0E/0S`、Business=`206/0F/0E/0S`、compile=`21/21`、Public Contract=`315/187/42`。完整 Ledger 在当前 sandbox 只允许记录 `65/0F/15E/0S` 且 15E 全部同根于 assembler Mockito/ByteBuddy self-attach；环境可用时必须 `65/0F/0E/0S`，不得把 15E 计为 PASS。

任何非目标 failure/error、测试计数漂移、immutable SHA 漂移、生产或第二测试文件需求、断言弱化、兼容/dual digest 或 `git diff --check` 失败都立即停止。该阶段只是 Entry Card 文档与 Checker，已以 `TEST_REWORK_EXECUTION_GRANT_NO / GREEN_EXECUTION_PAUSED / CODE_FREEZE / plan-r2.226` 收口。

独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`，确认单文件、四方法、既有 builder、精确计数与无兼容边界可执行。该阶段以 `TEST_REWORK_EXECUTION_GRANT_NO / plan-r2.226` 收口，当前由 20.32 接替。

### 20.32 `MIG-03` Legacy Clearing Test Rework 执行结果

执行前 Java 21、compile=`21/21`，fresh focused=`56/3F/1E/0S`。只修改 `ClearingSplittableDetailApplicationServiceTests.java` 后，四个目标方法均按 20.31 分层转绿，测试总数保持 28；目标文件 SHA-256=`1c54a19dd7ebe553253cb7a01f1ed952f906feb15e5c91aa8c22d40fc0ae786a`。

后置 fresh 证据为 focused=`56/0F/0E/0S`、non-assembler Ledger=`50/0F/0E/0S`、Reconciliation=`245/0F/0E/0S`、Transaction=`178/0F/0E/0S`、Business=`206/0F/0E/0S`、compile=`21/21`、Public Contract=`315/187/42`。完整 Ledger 诊断=`65/0F/15E/0S`：一个 assembler ApplicationContext 根错误的 cause 为 `MockitoInitializationException -> ByteBuddyAgent -> Could not self-attach`，其余 14 个为 failure-threshold 派生；不得计为行为 PASS，也不属于本切片源码缺陷。

独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。生产与另四个 acceptance SHA 未漂移，staged empty、manifest stable、`git diff --check` PASS；没有运行 MySQL host 或 PMD，也没有生产、兼容、第二测试、共享 fixture、API/DDL/Mapper/POM/Consumer 写入。MIG-03 Ledger Digest Green 已完成且授权耗尽；该阶段当时进入 `W5-MIG03-NEXT-SLICE / ENTRY_CARD_REQUIRED / EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`，当前由 20.33 接替。

### 20.33 `MIG-04` Transaction orchestration surface internalization Entry Card

#### 20.33.1 RED

唯一 RED 文件为 `FundsModuleDependencyBoundaryTests.java`。新增一个测试，以源码路径和 import/implements token 聚合证明：两个 Wallet facade 与六个 Wallet action/consumption request 当前仍存在，六个 Transaction internal command 尚不存在，两个 concrete service 仍绑定旧 interface。当前类基线来自既有 XML=`26/0F/0E/0S`；未来 RED 执行前必须 fresh 复验，新增后固定 `27/1F/0E/0S`，不得出现 compile/error 或第二 failure。

#### 20.33.2 Green 行为保护

Green 只按系分 11.27.3 的 `6 ADD / 14 MODIFY / 8 DELETE` 同批 hard break。RED 测试 immutable；四个行为测试只迁移 package/注入类型并保持各自执行数与断言：

| Test class | Tests | 必须保持的行为 |
| --- | ---: | --- |
| `PaymentInstrumentTransactionApplicationServiceTests` | 9 | 收款工具/rail/binding 准入、重放、失败零 Funds/Ledger/Balance。 |
| `PaymentInstrumentTransactionAuthorizationTests` | 26 | authorize/complete/reversal 原快照、控制预留、原子回滚和幂等。 |
| `SpendControlTransactionConsumptionApplicationServiceTests` | 25 | consume/release/refund/业务确认补偿的可信交易引用、累计和审计。 |
| `WalletSpendControlsAcceptanceFlowTests` | 1 | Wallet movement fact 与 Transaction 编排组合行为。 |
...
just mvn-version
just compile
MAVEN_OPTS=-DskipSurefireReport=true just test-one FundsModuleDependencyBoundaryTests tests
# 修改唯一 RED 文件后重复，目标 27/1F/0E/0S。
MAVEN_OPTS=-DskipSurefireReport=true just test-one FundsModuleDependencyBoundaryTests tests
```
...
```bash
just clean-compile
MAVEN_OPTS=-DskipSurefireReport=true just verify-slice FundsModuleDependencyBoundaryTests,PaymentInstrumentTransactionApplicationServiceTests,PaymentInstrumentTransactionAuthorizationTests,SpendControlTransactionConsumptionApplicationServiceTests,WalletSpendControlsAcceptanceFlowTests tests
MAVEN_OPTS=-DskipSurefireReport=true just test-transaction
just verify-public-contracts
git diff --check
```

目标固定为 clean compile=`21/21`、focused=`88/0F/0E/0S`、transaction=`178/0F/0E/0S`、Public Contract=`307/181/42`。删除源码必须通过 clean compile 证明无 stale class；旧 8 路径、旧 import 和旧 interface 类型引用必须归零。完整 test-boundary、MySQL host 与 PMD 按系分 11.27.4 分层，不作为当前卡硬门。

该阶段只形成 Entry Card，不运行测试、不修改生产/测试/接入文档；当时为 `RED_EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`。

执行前 live HEAD 固定为 `fc6b6e004a32eb1813534de05df7f844cc6edf95`，23 个既有 RED/Green 输入文件的有序 scope fingerprint 固定为 `07b05881cd05f0068624ba099350227a735e6135839f33fd83f8528b82a88dc2`；新增 6 个 internal command 的父目录已经存在。任何 HEAD、scope fingerprint、调用清册或仓外 Consumer 证据漂移都必须先回到 Entry Card。

独立 Checker 首轮唯一 P1 为缺 live HEAD；r2.229 最小返工后最终判定 `PASS / 0 P0 / 0 P1 / 0 P2`。该阶段当时只进入 Human Owner RED Execution Grant 决策，不授权测试写入。

### 20.34 `MIG-04` Transaction orchestration surface internalization RED 执行结果

执行前 Java 21、compile=`21/21`、fresh Boundary=`26/0F/0E/0S`。只修改 `FundsModuleDependencyBoundaryTests.java` 并新增一个聚合测试后，fresh Boundary=`27/1F/0E/0S`；唯一 failure 为 `testTransactionOrchestrationShouldNotRemainInWalletFace`，精确列出 8 个旧 wallet-face 路径、6 个缺失 transaction-impl internal command，以及两个 concrete service 的 5 条旧 interface import/implements 绑定。目标测试 SHA-256=`38e42fe318436f1816a6f164b977128ffe2cd80049c3cf3920e388b35394a015`。

22 个非 RED 文件有序指纹执行前后均为 `227a539d7d3610d782e76ee7323e648fba96bfb75b986452c7c74b4c536aea63`；后置 compile=`21/21`、staged empty、`git diff --check` PASS。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`，确认无生产、第二测试、Green、兼容或白名单外写入。RED 文件自此 immutable；当前只进入 20.33.3 Green 命令与 11.27.3 文件卡的 Human Owner Grant 决策，`GREEN_EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`。

### 20.35 `MIG-04` Transaction orchestration surface internalization Green 执行结果

Green 按 20.33.2 一次性 hard break：六个 action/consumption request 迁入 transaction-impl，两个 Wallet facade 与六个旧请求删除，三个生产实现和四个行为测试完成机械迁移，七份接入/专题文档删除旧 Public facade 承诺。业务断言、字段、validation、Money/identity、幂等摘要和 serial shape 均保持；没有替代 facade 或兼容路径。

fresh 证据为 clean compile=`21/21`、focused=`27+9+26+25+1=88/0F/0E/0S`、transaction=`178/0F/0E/0S`、Public Contract=`307/181/42`。RED 测试 SHA 保持 `38e42fe318436f1816a6f164b977128ffe2cd80049c3cf3920e388b35394a015`；旧路径、旧 import 和旧 implements 生产引用归零。独立 Checker 与格式增量 Checker 均=`PASS / 0 P0 / 0 P1 / 0 P2`。MIG-04 已完成且授权耗尽；当前只进入下一 W5 Entry Card Human Owner 决策，`ENTRY_CARD_REQUIRED / EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`。

### 20.36 `MIG-05C` Ledger extension surface collapse Entry Card

#### 20.36.1 RED 与行为保护

唯一 RED 文件为 `FundsModuleDependencyBoundaryTests.java`。新增一个源码路径/token 聚合测试，固定当前 core projection interface、composite、projection list/router、旧 implements 与 assembler `Ordered` 残留为唯一失败；执行前 `27/0`，执行后 `28/1F/0E/0S`。测试不编译引用目标删除类型，Green immutable。

Green 行为保护固定为：

| Test class / support | 当前 tests | 必须保持 |
| --- | ---: | --- |
| `FundsModuleDependencyBoundaryTests` | 27，RED 后 28 | core/impl 路径与 token 边界；Green 只关闭新增 failure。 |
| `RouteResolverFactBoundaryTests` | 1 | Route 不声明 Ledger/Transaction fact write dependency；只移除已删除 interface 类型。 |
| `PaymentInstrumentTransactionApplicationServiceTests` | 9 | 收款 route/posting/Ledger/Balance、幂等和失败零副作用。 |
| `LedgerTransactionServiceFactQueryTests` | 8 | Ledger exact read、完整性、Balance snapshot 与稳定引用。 |
| `ExternalFundsEventApplicationServiceTests` | 12 | normalized external topup、外部事实幂等和余额投影。 |
| `DefaultLedgerTransactionPostingServiceImplTests` | 18 | posting 校验、entry/plan 平衡、投影原子性与重复入账。 |
| `LedgerBalanceProjectionServiceImplTests` | 9 | normal side、周期、负余额准入、事件和失败不污染事实。 |
| `LedgerPostingJdkProxyContextTests` | 1 | Spring/JDK 事务代理装配；当前 Mockito self-attach 环境 failure 单列，不弱化。 |
| `FundsTransactionFlowTestSupport` | support | transaction 组合流 concrete injection，只做类型迁移。 |
| `FundsAuthorizationTransactionFlowTests` | `test-transaction` 组成类 | 一处继承 projection 字段的单参数调用改为显式 `NORMAL`；授权、完成、撤销、退款、余额与账务断言不变。 |

当前无 Mockito 集群=`84/0F/0E/0S`；RED 后=`85/1F/0E/0S`；Green=`85/0F/0E/0S`。JDK proxy 当前=`1/1F/0E/0S`，仅 `ByteBuddyAgent self-attach`，环境可用目标=`1/0F/0E/0S`。

#### 20.36.2 Green 精确文件与验收

Green 文件卡严格复用系分 11.30.3：`DELETE=2 / MODIFY=19 / ADD=0`。新增第 19 个 MODIFY 仅为 `core/src/main/java/com/wind/funds/ledger/LedgerPostingAssembler.java` comment-only Javadoc 校正；接口签名、实现行为与测试 immutable。九个测试/support 只迁移 concrete type、显式 `LedgerPostingAccessType.NORMAL` 和必要 Spring bean type；不得删测试、改测试数、改 Money/余额/事务/事件断言或新增 Mock 业务实现。Core API 只删除四条 `LedgerBalanceProjectionService` stable signature，目标 `104/96/4/4 / 1039 lines`；Public Contract 只扫描 face，目标保持 `307/181/42`。
...
just compile
MAVEN_OPTS=-DskipSurefireReport=true just test-one FundsModuleDependencyBoundaryTests tests
# RED 后仍只运行同类，目标 28/1F/0E/0S。
MAVEN_OPTS=-DskipSurefireReport=true just test-one FundsModuleDependencyBoundaryTests tests

# Green 后。
just clean-compile
MAVEN_OPTS=-DskipSurefireReport=true just verify-slice FundsModuleDependencyBoundaryTests,RouteResolverFactBoundaryTests,PaymentInstrumentTransactionApplicationServiceTests,LedgerTransactionServiceFactQueryTests,ExternalFundsEventApplicationServiceTests,DefaultLedgerTransactionPostingServiceImplTests,LedgerBalanceProjectionServiceImplTests tests
MAVEN_OPTS=-DskipSurefireReport=true just test-one LedgerPostingJdkProxyContextTests tests
MAVEN_OPTS=-DskipSurefireReport=true just test-transaction
just verify-core-api
just verify-public-contracts
git diff --check

# 当前环境只作诊断；环境可用时必须全绿。
MAVEN_OPTS=-DskipSurefireReport=true just test-one DefaultLedgerPostingAssemblerTests tests
MAVEN_OPTS=-DskipSurefireReport=true just test-ledger
```

`just verify-core-api` 会执行 clean compile；若执行顺序引入 stale class、cardinality 不是 `104/96`、baseline 不是 1039 行、出现四条目标以外 signature diff 或 `api-policy.tsv` 漂移，立即停止。JDK/assembler Mockito 环境 observation 不替代无 Mockito 集群、compile、Core API、Public Contract 和 transaction 硬门。

#### 20.36.3 不变量与停止线

Funding、Credit、VCC shared、refund/reversal、external topup 与 balance adjust 的 posting plans、LedgerTransaction/Entries、余额 delta、normal side、period、minimum balance、`allowNegative`、optimistic version、事件和幂等行为必须逐项保持。删除 router 不允许把投影移出 Ledger 本地事务，也不允许在 Transaction 选择账目或 projector。

`LedgerPostingAssembler`、`LedgerTransactionPostingService`、全部 posting specs、`DefaultRoutedFundsInstructionOrchestrator`、POM、DDL/schema、Entity/Mapper/Converter、Ledger face、其他生产模块、Consumer、兼容层与其他 MIG immutable。发现第二真实实现或高阶 posting port 需求时停止重冻。

RED SHA 保持 `93bb5146966a34338ba55ec95a846c57cd013167258c664c37a1de9d95df7caa`。Green rework 只校正 `LedgerPostingAssembler.java` 一行 Javadoc，SHA 从 `c37149b77af6208939506db4e0d6f020b1535df2dae6e4f78b9c64b624039943` 变为 `4d97fdf3effcf6921f76e2e02040f7231c6b9f9bcc112a4d0e66e2a97e4ee045`，21 路径 post fingerprint=`714fb0528b52d85fa5ea3d70bea446c5d1394f959ffe5f4e071f9dfa56d00366`。clean compile=`21/21`、Boundary=`28/0`、focused=`85/0`、transaction=`178/0`、Core API=`104/96/4/4 / 1039 lines`、Public Contract=`307/181/42`；JDK proxy=`1/1F` 仍唯一为 ByteBuddy self-attach 环境 observation。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。MIG-05C 当前范围关闭，下一 W5 切片必须重新形成 Entry Card，`EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE`。

### 20.37 `CI-W5-MIG05D-LEDGER-POSTING-COMMAND-001` 决策验证卡

#### 20.37.1 当前可执行事实

当前生产只有 `DefaultRoutedFundsInstructionOrchestrator` 同时调用 `LedgerPostingAssembler` 与 `LedgerTransactionPostingService`；`FundsTransactionProjectionPublishContext` 公开持有完整 `LedgerTransactionSpec`，但生产只读取 LedgerTransaction SN。Ledger 内部 tests 已证明任意外部 spec 中的不平衡、敏感字段、权益字段、错账目/period、重复入账、摘要和余额约束会 fail-closed；因此本卡测试目标不是制造“当前可绕过”的虚假 RED，而是验证公共写能力能否无行为损失地收窄。

#### 20.37.2 三候选验收差异

| 候选 | 未来最小 RED/Contract 责任 | Green 必须保持 | 不得用什么变绿 |
| --- | --- | --- | --- |
| `A / SINGLE_HIGH_LEVEL_CORE_POST_COMMAND`（推荐） | 架构/Public API 测试证明 `LedgerPostingAssembler` stable type 和 `post(LedgerTransactionSpec)` 消失；唯一高阶命令只接收 instruction + 非空 fundsTransactionSn + resolvedRoute 并返回 ledgerTransactionSn；Ledger 独立拒绝六个 identity 字段任一错配；同命令身份同 digest 返回同一 persisted SN，异 digest 冲突且零新增；Projection context 不再暴露 spec。 | 现有 route、posting plan/phase/entry、Funding/Credit/VCC、正逆、external、adjust、persisted digest、余额、失败归纳和投影 explanation 全部逐项相同。 | 不修改业务断言，不依赖 `CompositeRouteResolver` 已校验，不把 internal spec 校验删除，不用 Mock Ledger、测试直写 SN、兼容重载、bridge 或双端口。 |
| `B / KEEP_DUAL_LOW_LEVEL_CORE_PORTS` | 不产生代码 RED；必须记录真实第二 grammar/production Consumer、owner、验收和复审时间。 | 现有全部测试。 | 不以测试可构造 spec、fincone 文档或 capte test composition 冒充生产复用。 |
| `C / MOVE_LOW_LEVEL_PORTS_TO_LEDGER_FACE` | 不进入测试设计。 | 不适用。 | 不得修改模块依赖守卫为 Transaction -> ledger-face 放行。 |

#### 20.37.3 A 被接受后的未来测试层级

1. Contract/architecture RED：在既有 `FundsModuleDependencyBoundaryTests` 或独立冻结的 Public Contract test 中聚合检查旧两个写端口、旧 `post(spec)` 和 projection spec field 仍存在，新高阶命令缺失；精确方法和文件数只能在 Entry Card 回读完整 caller 后冻结。
2. Transaction orchestration：`DefaultRoutedFundsInstructionOrchestratorProjectionTests` 证明无账务动作返回 null SN、有账务动作发布唯一 LedgerTransaction SN、projection publisher failure 不回滚资金与账务事实。
3. Ledger command：新增或复用冻结测试逐项覆盖 instruction/route 的 `tenantId/businessScene/businessSn/instructionType/eventType/transactionType` 六种错配、空 fundsTransactionSn、同 root + action identity 同摘要重放返回同一 persisted SN、异摘要冲突、同 authorization root 不同 action 生成不同 SN，以及每个拒绝路径零 LedgerTransaction/plan/entry/Balance；不得只从 orchestrator 间接证明。
4. Ledger internal：现有 `DefaultLedgerTransactionPostingServiceImplTests`、`LedgerTransactionServiceImplTests`、`LedgerTransactionServiceFactQueryTests` 继续验证低阶 materialization/持久化 guard，但调用迁移到 Ledger impl 内部 command，不删除 arbitrary-spec 负例。
5. 资金组合流：transaction、business-flow、balance-control、reconciliation 和 capte-domain 集中 tests 保持真实 Bean/H2 装配；只机械迁移 concrete Bean，不改 Money、route、posting、LedgerEntry、Balance 或幂等断言。
6. API/依赖：Core API baseline 只发生 Owner 接受后的目标 signature delta；Public Contract、模块依赖与旧 production reference 扫描同时通过。

#### 20.37.4 当前证据、停止线与下一门

当前十三文件 ordered fingerprint=`f01da0518faddc44cced8befb6c476c11d06f615a63d1bf54d29de22a828a404`。精确仓库相对路径和 1~13 顺序以系分 11.31.3 的代码块为唯一来源，不按本文列举顺序推断；它覆盖 instruction、resolved route、四个 posting specs、两个稳定端口、orchestrator、projection context、posting impl、default assembler 与 Core API baseline。HEAD=`fc6b6e004a32eb1813534de05df7f844cc6edf95`，default/`-uall` manifest=`68/7a68c0ebaba16d49ea2f684062605ed09da5bd7f273f47074a882f81e5503a5d`，staged empty，`git diff --check` PASS。

本轮不运行 Java 测试，不预写 RED/Green 数量，不冻结 ADD/MODIFY/DELETE。A 只有在 Human Owner 接受且 Acceptance Checker PASS 后才能形成文件级 Entry Card；B 只关闭为显式保留决策；C 已拒绝。发现生产外部 caller、完整 projection spec Consumer、跨库事务、兼容/V2/bridge、schema 或新业务规则时停止，不进入实现。

### 20.38 `CI-W5-MIG05D-LEDGER-POSTING-COMMAND-001-A` Acceptance 与分阶段 TDD 卡

Human Owner 已接受 A；本卡以无兼容 breaking release 分成 `CONTRACT_SURFACE_RED -> CONTRACT_SURFACE_GREEN -> BEHAVIORAL_RED -> BEHAVIORAL_GREEN`。任何阶段的 PASS 都不能跳过下一阶段，也不构成 Git、发布或生产授权。

#### 20.38.1 Contract Surface RED

只允许 `MODIFY tests/src/test/java/com/wind/funds/architecture/FundsModuleDependencyBoundaryTests.java`，新增一个顶层测试方法：

```text
testLedgerPostingCommandShouldOwnAssemblyAndStableIdentity
```

该方法不编译引用待删除或待新增 signature，而是聚合检查：

1. `LedgerPostingAssembler.java` 仍存在，stable baseline 仍有其三条 signature。
2. `LedgerTransactionPostingService` 仍是 `void post(LedgerTransactionSpec)`，尚未形成精确三输入/String 返回。
3. `DefaultRoutedFundsInstructionOrchestrator` 仍注入 assembler、构造 raw spec 再 post。
4. `FundsTransactionProjectionPublishContext` 仍公开 `LedgerTransactionSpec` component。
5. core route/orchestrator/spec Javadoc 仍把 assembler 写成公共职责。

当前 Boundary=`28/0F/0E/0S`；RED 后固定为 `29/1F/0E/0S`。唯一 failure label 必须指向 `ledger posting command surface`，不能出现 testCompile、路径、fixture、Spring 或 H2 error。RED 观察后该文件 immutable。

#### 20.38.2 Contract Surface Green

Green 严格使用系分 11.32.3 的 surface 文件：删除旧 interface、替换 core signature、迁移唯一 production caller、去除 projection raw spec、更新 Javadoc/API baseline/script，并机械迁移四个 legacy test 文件。此阶段只建立可编译高阶入口，允许继续复用当前时序 Ledger SN，禁止提前写 Behavioral RED 或为幂等加兼容路径。

Surface Green 验证：

```text
just compile                                      -> 21/21
focused 4-class cluster                           -> 61/0F/0E/0S
FundsModuleDependencyBoundaryTests                -> 29/0F/0E/0S
DefaultLedgerTransactionPostingServiceImplTests   -> 18/0F/0E/0S
LedgerTransactionServiceFactQueryTests            -> 8/0F/0E/0S
DefaultRoutedFundsInstructionOrchestratorProjectionTests -> 6/0F/0E/0S
just verify-core-api                              -> 103 public / 95 stable / 4 experimental / 4 internal / 1036 lines
just verify-public-contracts                      -> 307 types / 181 models / 42 enums
```

`LedgerPostingJdkProxyContextTests=1` 与 `DefaultLedgerPostingAssemblerTests=15` 继续单列 Mockito/ByteBuddy self-attach 环境 observation；环境可执行时必须 `16/0F/0E/0S`，当前沙箱 attach 失败不得冒充 PASS，也不得阻断已独立执行的无 Mockito contract/behavior proof。

#### 20.38.3 Behavioral RED

Surface Green 独立 Checker PASS 后，才允许在已迁移的 `DefaultLedgerTransactionPostingServiceImplTests` 新增以下 7 个方法、12 个 Surefire invocations：

| 方法 | invocations | 必须观察的缺口 |
| --- | ---: | --- |
| `testPostShouldRejectInstructionRouteIdentityMismatch` | 6 | 逐项错配 tenantId、businessScene、businessSn、instructionType、eventType、transactionType；每项在 Ledger/Balance 前拒绝。 |
| `testPostShouldRejectMissingTenantIdBeforeLedgerFacts` | 1 | identity 缺 tenant 不得到达持久化。 |
| `testPostShouldRejectBlankFundsTransactionSnBeforeLedgerFacts` | 1 | 空白 Funds identity 不得产生 Ledger facts。 |
| `testPostShouldReturnSamePersistedLedgerTransactionSnForSameCommand` | 1 | 顺序重放必须返回同一 SN，事实和余额只变化一次。 |
| `testPostShouldRejectSameCommandIdentityWithDifferentFacts` | 1 | 同 identity 改 Money 或 route 后摘要冲突，原 facts/Balance 不变。 |
| `testPostShouldPostOnceForConcurrentSameCommand` | 1 | 两线程同 identity 同义并发均返回同一 SN，只有一套 transaction/plan/entry 和一次余额投影。 |
| `testPostShouldSeparateAndReplayDifferentActionsOnSameFundsTransactionRoot` | 1 | 同一 authorization root 的两个不同 event/business action 必须生成不同 SN；各自同义重放返回自己的原 SN 且零重复余额变化。 |

Behavioral RED 前该类为 `18/0`；RED 后固定 `30/12F/0E/0S`。四类 focused 从 Surface Green `61/0` 变为 `73/12F/0E/0S`，12F 必须一一对应表中 invocation；其他 61 项全部通过。RED 后 posting service test immutable。

#### 20.38.4 Behavioral Green 与扩大门禁

Behavioral Green 只允许修改：

- `ledger/impl/src/main/java/com/wind/funds/ledger/DefaultLedgerTransactionPostingServiceImpl.java`：独立输入校验并从 `postAssembled` 返回稳定 SN。
- `ledger/impl/src/main/java/com/wind/funds/ledger/posting/LedgerTransactionSpecFactory.java`：用 `ledger.posting.command.identity` canonical v1 和 `tenantId + fundsTransactionSn + eventType + businessScene + businessSn` 生成 `LE + 48 hex`，删除时序 SN。

Green 目标为 posting service=`30/0`、focused=`73/0`。随后执行 `just compile`、`just test-core`、non-assembler Ledger 五类、`just test-transaction`、`just test-reconciliation`、`just test-business-flow`、`just verify-core-api`、`just verify-public-contracts`、`git diff --check` 与独立 Checker。目标基线：compile=`21/21`、core=`104/0`、non-assembler Ledger=`62/0`、transaction=`185/0`、reconciliation=`245/0`、business-flow=`206/0`、Core API=`103/95/4/4 / 1036 lines`、Public Contract=`307/181/42`。真实 fresh 计数若与冻结基线不同，先解释来源；不得刷新数字掩盖额外测试或失败。

失败状态必须保持可归因：identity/route 输入错误在任何 Ledger fact 前拒绝；persisted same-SN different aggregate 继续使用现有“账本交易已存在但摘要不一致”语义；并发唯一键 loser 回读并验证 winner。任何错误不得改写 Funds lifecycle 为成功，也不得新增或回滚外部事实。

Entry Card 冻结时 19 文件 ordered fingerprint=`25c12e2dae73722cc7155f458350e2ec4fa362b234d36b54a99970a88a4762a4`；精确路径、操作、阶段与 SHA 以系分 11.32.3 为唯一来源。Entry Card Checker=`PASS / 0 P0-P2`；该阶段已由 20.38.5 的 Surface RED 执行接替。

#### 20.38.5 Contract Surface RED 实际证据

唯一 RED 方法已加入 `FundsModuleDependencyBoundaryTests` 并保持单一聚合 failure。首轮 Checker 的 P1/P2 通过 reflection rework 关闭：interface 只允许一个三参数/String `post`；concrete impl 只允许一个 package-private `postAssembled(LedgerTransactionSpec)`，任何 public/protected raw-spec 方法都违规；private validators 不受影响；orchestrator 不再绑定局部变量或内联参数写法。

fresh 结果：

```text
mvn-version                                         -> Maven 3.6.3 / Java 21.0.11
pre/post compile                                    -> 21/21
FundsModuleDependencyBoundaryTests                  -> 29/1F/0E/0S
four-class focused                                  -> 61/1F/0E/0S
DefaultLedgerTransactionPostingServiceImplTests     -> 18/0F/0E/0S
LedgerTransactionServiceFactQueryTests              -> 8/0F/0E/0S
DefaultRoutedFundsInstructionOrchestratorProjectionTests -> 6/0F/0E/0S
```

唯一 failure=`testLedgerPostingCommandShouldOwnAssemblyAndStableIdentity`；test SHA=`b05c78e7a0542b1302062f81481136948b12b229fadf530a1d00e5dd31f4e31d`，XML SHA=`e063473d228b6ee16a3f594bb6901c4e1976ae684b863db96266a603a1bbaf1e`，19 文件 post fingerprint=`f57b9a0a5f22ad4e2e175d4a98c9dca0f7ae23bd91594de5cb34a854e7674f0b`。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。该测试自此 immutable；当时状态为 `CONTRACT_SURFACE_GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`。

#### 20.38.6 Contract Surface Green 实际证据

Human Owner 授权后仅执行表 11.32.3 的 Surface 子集：`1 DELETE + 16 MODIFY`，Boundary RED 与 Behavioral-only `LedgerTransactionSpecFactory` 保持 immutable。实现删除 core assembler type，以唯一高阶 `post(instruction, fundsTransactionSn, resolvedRoute) -> ledgerTransactionSn` 取代 raw-spec 写入，只保留 concrete package-private `postAssembled`；orchestrator 与 projection context 不再持有 raw spec。当前仍是时序 Ledger SN，因而 Behavioral RED 的缺口未被提前填充。
...
just verify-core-api                              -> 103 public / 95 stable / 4 experimental / 4 internal / 1036 lines
just verify-public-contracts                      -> 307 types / 181 models / 42 enums
just test-core                                    -> 106/0F/0E/0S
just test-transaction                             -> 185/0F/0E/0S
just test-reconciliation                          -> 245/0F/0E/0S
just test-business-flow                           -> 213/0F/0E/0S
```

`core=106` 不来自本 Surface，因为本轮没有修改 core test 类；`business-flow 206 -> 213` 来自先前 MIG-02B 已新增的 7 个 authorization release 测试，不通过刷新旧基线掩盖。Mockito/ByteBuddy 专项为 JDK proxy=`1/1F/0E/0S`、assembler=`15/0F/15E/0S`，均是 self-attach 环境边界，不冒充 PASS。Boundary SHA=`b05c78e7a0542b1302062f81481136948b12b229fadf530a1d00e5dd31f4e31d`，factory SHA=`4335ab19447a26ffd27f29e6a07e23d55005bd1f1676c20e04aa6bc982b84ed4`，19 路径 post-Surface fingerprint=`314d7a3d9e4f29bca80bd073511f737ea135bba73935f48dc08b43c06c470f0b`。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；当时只进入 `BEHAVIORAL_RED_EXECUTION_GRANT_NO / CODE_FREEZE`，随后已由本文顶部 `plan-r2.254` 接替。

#### 20.38.7 Behavioral RED 实际证据与 Checker 收口

唯一写入文件为 `DefaultLedgerTransactionPostingServiceImplTests`，新增 7 方法/12 invocations。修改前该类=`18/0`；修改后 fresh class=`30/12F/0E/0S`、focused=`73/12F/0E/0S`，Boundary=`29/0`、fact query=`8/0`、projection=`6/0`，其他原有 18 项保持通过。12F 只对应 instruction/route 六字段复验、missing tenant、blank root、same-command replay、same-identity conflict、concurrent winner readback 和 same root different action replay；没有 testCompile、Spring context、fixture 或 unexpected error。test SHA=`144fe653951b513dd74f5a1b8d25b3881d0d37d0ee634b240c67669f64ba4324`，XML SHA=`dcebac107a06dbfea74832f892830ffee3d4392b43c6f692174d655098b00381`，19 路径 fingerprint=`95c1135a0b09add4e8683b304d7a1bfbc8da28339e1f7f36088a12f97f10a144`。Boundary SHA 与 factory SHA 保持冻结值，未实现 Green。

首轮 Checker 判定技术 RED PASS，但因未单独授权的 Maven Snapshot 联网尝试给出 `NOT PASS / P1=1`。命令已中断，后续离线 compile=`21/21`、classfile guard PASS，技术 RED 无需重做。Human Owner 已接受并授权记录该偏差；未另获联网授权前，Maven 命令必须带 `-o`，任何在线解析立即停止。独立 recheck 最终=`PASS / 0 P0 / 0 P1 / 0 P2`，确认未追认原联网动作、无双活动权威；当时进入 `BEHAVIORAL_RED_INDEPENDENT_CHECKER_PASS / BEHAVIORAL_GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`，随后已由 20.38.8 接替。

#### 20.38.8 Behavioral Green fresh 证据

Green 只修改两个冻结生产文件，RED 与 Boundary 测试 SHA 保持 `144fe653951b513dd74f5a1b8d25b3881d0d37d0ee634b240c67669f64ba4324 / b05c78e7a0542b1302062f81481136948b12b229fadf530a1d00e5dd31f4e31d`。posting service 在任何组装/事实访问前拒绝 missing tenant、blank root 与六字段 mismatch；factory 用 canonical action identity 生成 50 字符 SN，并删除无 root 重载。12 个 RED 全部自然转绿，未修改断言或 fixture。

所有 Maven 验证均带 `-o`：compile=`21/21`、posting=`30/0F/0E/0S`、focused=`73/0F/0E/0S`、去重扩大=`638/0F/0E/0S`；分组 core=`106/0`、non-assembler Ledger=`62/0`、transaction=`185/0`、reconciliation=`245/0`、business-flow=`213/0`。Core API=`103/95/4/4 / 1036 lines`、Public Contract=`307/181/42`、classfile 与 `git diff --check` PASS。Core API 首次未固定 Java 时被本机 Java 8 `javap` 把 default method 误读为普通 public method，使用冻结 Java 21 重新执行后 PASS；不构成 API 差异。Wind 离线结构守卫的 `3 error / 4 warn` 均为本轮前已存在的 `@Component/@Slf4j/JSpecify` 规则项，独立 Checker 确认本轮未新增偏差。最终 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。

### 20.39 `W5-REFACTORING-PROGRESS-REBASE-002`（历史）

本轮不新增测试，只按已存在的 fresh XML、源码 SHA、API baseline 和独立 Checker 记录校准测试 backlog：authorization release、MIG-03、MIG-04、MIG-05 与 MIG-07 Provider 不再列为未来 RED；refund、normalized external fact、Consumer cutover 与 global legacy retirement 继续按原 blocker 保持未准入。任何未来 Entry Card 必须提供触发 blocker 关闭的新证据，不能只引用本次重基线。

该重基线不复跑 Maven/PMD，也不修改 Java、测试、fixture、schema 或 Consumer。验证仅限五文档状态一致性、历史/当前分层、stale-active 扫描、`git diff --check` 与独立 Checker；最终 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`，当时=`PROGRESS_REBASE_INDEPENDENT_CHECKER_PASS / NEXT_SLICE_EVIDENCE_REQUIRED / EXECUTION_GRANT_NO / CODE_FREEZE`，现已由 20.40 承接。

### 20.40 `W5-MIG08-CAPTE-BENEFIT-ACTIONFACT-CUTOVER-ENTRY-CARD-001`（历史 Entry Card）

#### 20.40.1 当前证据与价值假设

`capte-domain@ce3c69467745b181f128561a887519bcba2950c7` 当前 production POM 使用 `wind-funds.version=1.0.1-SNAPSHOT`，coupon-impl 已依赖 core/transaction-face/wallet-face，集中 tests 以 test scope 装配三个 Provider impl。当前 Capte default/`-uall` manifest=`35 / 877689322b181484735c3b503eff0048e6d63bbcfce31deb448f7088047d2b2a`，staged empty、`git diff --check` PASS。

本机已解析制品存在：core=`dbc36d9e61df78de40ac4c09b47de73423275e68913539f8cae11b114de30be6`、transaction-face=`8df1ced2fa32166ca629c70fa71858a9263b5d5bf13366f952cfa52a4d581dcf`、transaction-impl=`813da0f1134a6c47704b4cc7071bd89edc810ff43438b60fbc9a89a98477747a`。这些 SHA 只证明本机制品身份，不证明本轮 runtime 已执行或源码 commit lineage。

价值假设可证伪：当主交易或本地 reference 存在但 ActionFact 缺失、歧义、失败、`proven-zero`、Money/intentRef/original ref 冲突时，Consumer 不保存/复用完成引用、不跳过退款，也不产生新的 Coupon 成功事实；当 Provider 已完成而 Consumer 在保存 reference 前中断时，只有 Coupon Owner 已成立且仍有效的业务意图和冻结请求才能授权同 identity 恢复，同一 ActionFact/transactionSn 恢复后零新资金与账务事实。ActionFact 空结果只表示 UNKNOWN，本身不授权重试、逆向或补单。

#### 20.40.2 未来 RED 白名单与预期失败

未来 RED 只允许在 `capte-domain` 修改以下三个测试文件：

1. `tests/src/test/java/com/capte/marketing/coupon/CouponRedemptionApplicationServiceImplTests.java`
2. `tests/src/test/java/com/capte/marketing/coupon/CouponImplContractBoundaryTests.java`
3. `tests/src/test/java/com/capte/order/transaction/OrderCouponRedemptionIntegrationTests.java`

生产文件、POM、schema、Provider 与其他 Consumer 在 RED 阶段 immutable。RED 必须至少形成以下可观察失败：

- 当前 `CouponBenefitFundingSettlement` 没有调用 `queryFundsActionFacts`，source boundary 精准失败。
- settlement 返回交易号但 primary ActionFact 为空/歧义/语义冲突时，当前实现仍保存 reference，行为测试精准失败。
- 已存在 `benefitTransactionSn` 但 primary ActionFact 缺失或与 `intentRef` 不一致时，当前 `findBenefitFundingTransactionSn != null -> continue` 仍绕过验证，行为测试精准失败。
- 只有 refund 主交易记录、没有匹配 recovery ActionFact 时，当前 `hasRefunded` 仍跳过 Provider 恢复，行为测试精准失败。
- Provider 返回 refund 交易号但 recovery ActionFact 未成立或 `recovery.intentRef != returned transactionSn` 时，当前实现仍结束，行为测试精准失败。

现有 Coupon 规则、金额拆分、退款资格、reference 集合、幂等和异常断言不得放宽。`CouponRedemptionApplicationServiceImplTests` 当前 dirty 包含既有 `LedgerProfileCatalog` 迁移和 ActionFact query stubs；`OrderCouponRedemptionIntegrationTests` 当前 dirty 包含既有 `LedgerProfileCatalog` 测试装配迁移。未来 RED 可以在前者既有 stub 上形成事实 fixture，但必须保留两份 dirty，不得回退；写前按 live state 重算 SHA 和范围指纹。

#### 20.40.3 未来 Green 与验证矩阵

未来 Green 唯一生产白名单为：

`marketing/coupon-impl/src/main/java/com/capte/marketing/coupon/integration/funds/CouponBenefitFundingSettlement.java` `MODIFY`。

Green 只能复用既有 `FundsTransactionQueryService#queryFundsActionFacts`、`FundsActionFactQuery` 与 `FundsActionFactDTO`，关闭 RED；测试在 RED 独立 Checker PASS 后 immutable。禁止新增 Provider API/DTO、service/facade、POM、schema、Mapper、缓存、锁、第二完成状态、主交易 fallback 或兼容层。

未来验证顺序由 Capte Owner 在同仓重冻：

1. `mvn -pl tests -am test -Dtest=CouponRedemptionApplicationServiceImplTests,CouponImplContractBoundaryTests`，记录 fresh Surefire counts/XML。
2. 使用已发布 `1.0.1-SNAPSHOT` 与显式 artifact path/version 运行 `OrderCouponRedemptionIntegrationTests` 的 lineage、seed、recover phase；证明唯一 primary ActionFact、`intentRef == benefitTransactionSn`、refund returned transactionSn 与 recovery `intentRef` 对齐、重建零新 transaction/detail/ledger facts。
3. 运行 Capte 相关扩大回归、compile、规约扫描和 `git diff --check`；私有 Maven、PMD 或运行环境问题必须与行为证据分层。
4. 扫描 `CouponBenefitFundingSettlement` 对 `queryFundsTransaction` 与 `findFundsTransactionByBusiness` 的零调用，只证明本 Consumer 完成判定切换，不外推全仓 MIG-09。

该卡当时没有 fresh Surefire XML，两份目标测试文件有既有 dirty，JAR manifest 也没有 Provider commit lineage，因此只能是 `ENTRY_CARD`，不能记为 E4 PASS。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；当时=`ENTRY_CARD_INDEPENDENT_CHECKER_PASS / CAPTE_CONSUMER_EXECUTION_GRANT_NO / CODE_FREEZE`，后续 RED 由 20.41 承接。

### 20.41 `W5-MIG08-CAPTE-BENEFIT-ACTIONFACT-CONSUMER-RED-CLOSEOUT-AND-GREEN-REFREEZE-001`（历史 Entry Card）

#### 20.41.1 RED 执行证据

Capte 使用 Java 21、Maven offline 和显式 `-Dmaven.test.skip=false -DskipTests=false` 执行聚焦 RED。修改前 offline compile=`42/42`，修改后 `testCompile` PASS；fresh application=`57/8F/0E/0S`、boundary=`31/2F/0E/0S`，合计=`88/10F/0E/0S`，原 78 项通过。Surefire XML SHA 分别为 `b7c8cedbb71046a64cc9d8d13e42684a7326f1bfbdc5fa27ef2ad7c752c52611` 与 `b533b35bbead903958130cde9ccb6d4fc3aa90bb02d51e4d5537c4c40bbacf58`。

十个失败只覆盖冻结缺口：五种无效 primary ActionFact、existing reference 无 ActionFact、refund root-only、recovery `intentRef` 冲突，以及 ActionFact 使用/旧查询零调用两个 source boundary；无 testCompile、Spring/H2、fixture 或额外业务 error。当前 Maven/Surefire 配置在 XML 有失败时仍可能返回 lifecycle `BUILD SUCCESS`，因此 RED 只以 fresh XML 计数、失败类和零 error 裁决，不能以进程退出码自证。

集成主类最终=`25/0F/0E/3S`；lineage、seed、recover method-only 阶段分别=`1/0F/0E/0S`。当前仓内保留的 recover XML=`1/0F/0E/0S`，SHA=`a15f4f39129d71f63df66eb1d860811986ee1e3622027c27dd87dcb7bde8c427`；该证据只证明已发布 `1.0.1-SNAPSHOT` 下的当前恢复路径，不外推 Provider source commit lineage、L4 或生产。独立 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`。

#### 20.41.2 Green Entry Card 重冻

Green 只允许 `MODIFY marketing/coupon-impl/src/main/java/com/capte/marketing/coupon/integration/funds/CouponBenefitFundingSettlement.java`，写前 SHA=`c32e6f1df72551ae7a62039362b1d6a62209344e3517ed67ce65473f1f9264b5`；三个 RED 测试 SHA=`d78277172a2d76982d5b197d6bfaa9c7cd0f35f902e4833d18c7cc4e55629043 / f3558962efbd9ef73fd649e581c18b3eb8a5f7059b4de9d1775af837775fa614 / 6b1cd39a4852729cd88498232e91795d7be92f3bd97168b665b7a71fea4ed4ca`，Green 中 immutable。

Green 必须把聚焦结果转为 `88/0F/0E/0S` 且保持原 78 项通过；集成主类保持 `25/0F/0E/3S`，lineage/seed/recover 各 `1/0F/0E/0S`；offline compile=`42/42`，目标组件对 `queryFundsTransaction` 与 `findFundsTransactionByBusiness` 的完成判断零调用。任何非目标 failure/error、第二生产文件、测试改动、Provider/POM/schema 变化、新 service/facade、主交易 fallback、兼容层或无法以既有 ActionFact 表达的行为都必须停止并重冻。

独立 Entry Card 重冻 Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；该卡当时只进入 Human Owner Green Gate，后续执行由 20.42 承接。

### 20.42 `W5-MIG08-CAPTE-BENEFIT-ACTIONFACT-CONSUMER-GREEN-CLOSEOUT-001`

Green 唯一生产文件 SHA=`8b28a049aa44b08fd5d2e99ce25d6ca0c2af43f49ee8927babe5081b76c0f1f3`；三个 immutable tests SHA 保持 `d78277172a2d76982d5b197d6bfaa9c7cd0f35f902e4833d18c7cc4e55629043 / f3558962efbd9ef73fd649e581c18b3eb8a5f7059b4de9d1775af837775fa614 / 6b1cd39a4852729cd88498232e91795d7be92f3bd97168b665b7a71fea4ed4ca`。根 POM、coupon POM、tests POM、schema SHA 继续为 `363e8678... / 5ea3322e... / b3ecc50f... / 91a4244e...`。

final fresh application=`57/0F/0E/0S`、boundary=`31/0F/0E/0S`，focused=`88/0F/0E/0S`；XML SHA=`bb0b8d8c3ba3d270cc631022d290314c67c6217be1b097382707ad1cbb318971 / 2fe600a53adb68dff9073fae39a8ba34b683be70f4383825a6ccdf1bcc9134a2`。main integration=`25/0F/0E/3S`，lineage/seed/recover 各=`1/0F/0E/0S`；XML SHA=`4445377b... / b012f63f... / 68c15304... / b0cd04d5...`。Java 21 offline compile=`42/42`。

Green 中间两次单一 failure 都是既有异常诊断片段未保留：unit 要求“原交易业务流水”，main tampered-reference 要求“原交易不存在”。只在同一生产失败分支补齐稳定诊断文本，测试、查询边界、资金行为和零副作用断言均未修改；最终聚焦和主矩阵重新 fresh 全绿。独立 Green Checker 校正事实后最终=`PASS / 0 P0 / 0 P1 / 0 P2`。

该组测试只证明 Capte library Consumer 能在已解析制品下使用 ActionFact，并以 seed/recover 两 JVM context 证明零新 transaction/detail/ledger facts；它不证明 Provider source revision 到 Consumer 实际加载制品的可审计 lineage，因此不能单独作为 E4 PASS。后续执行状态和验证明细统一见 [重设计执行规格](../../openspec/changes/funds-public-capability-redesign/spec.md)。

### 20.43 `MIG-08` Benefit facade R8B Authority / RED / Green 当前门禁

`D-R8B-001` 已以唯一 Provider r9、真实 Bean、联合 schema/事务、source-to-loaded lineage 和两 JVM file-H2 seed/recover 关闭 `capte-domain` 通用模块的 Benefit library test host E4；真实部署宿主 L4 仍独立等待。本节只冻结后续测试合同，不授权测试或代码写入。

1. Authority 对齐后，RED 只允许修改 wind-funds `FundsBenefitContributionTransactionServiceContractTests` 和 Capte `CouponImplContractBoundaryTests`。前者由三个旧正向反射合同收成一个 absence contract，预期=`1/1F/0E/0S`；后者更新文档 required terms 并新增一个 Consumer source boundary，预期=`32/1F/0E/0S`。两个 failure 必须只指向旧 Provider surface/Consumer 调用仍存在，任何额外 failure/error 阻断。
2. Green 中两个 RED 测试 immutable；Consumer 只迁既有 production/test caller，Provider 只删除已冻结 service、两 request、funding enum、impl 和旧 flow test，并机械更新 test support、Justfile 与 core API baseline。首次 14 文件 Green 后因删除 stable core enum 触发治理脚本 cardinality，最终可执行闭包为 15 文件，仅新增 `scripts/verify-core-api-baseline.sh` 的机械 `103/95 -> 102/94`；`ADD=0`，不得修改 generic direct command、ActionFact、schema/POM、Wallet/Reconciliation 或建立兼容。
3. Green fresh 门槛：wind-funds compile=`21/21`、legacy absence=`1/0F/0E/0S`、Core API=`102 public / 94 stable / 4 experimental / 4 internal / 1025 lines`；Capte compile=`45/45`、application=`57/0F/0E/0S`、activity=`37/0F/0E/0S`、boundary=`32/0F/0E/0S`、focused=`89/0F/0E/0S`、只读 `CouponFaceContractShapeTests=61/0F/0E/0S`、main=`25/0F/0E/3S`、lineage/seed/recover 各=`1/0F/0E/0S`。wind-funds `test-business-flow` 只允许相对 fresh 写前基线减少明确退役的 13 个 facade flow tests，其余全绿。
4. 资金不变量必须继续由 Consumer/E4 与 generic direct 测试共同证明：tenant、Money/币种、账户/账目、逐出资方独立事实、同键重放/冲突、原 ActionFact/route 退款、累计上限、posting 平衡、余额变化、UNKNOWN/restart 和零新增事实。funding nature 三值封闭、无资金 `NONE` 不调用 Funds、adapter 不写 context 属于结构性边界；旧 Provider 专属扩展面不迁移。
5. 新 Provider 必须使用唯一非 Snapshot 版本重新证明 source -> binary -> resolved/loaded；r9 只能作为旧 E4 provenance。三处合法负向字符串仅限两个 absence/source boundary 和 `CouponFaceContractShapeTests`，其他当前权威、production/test、反射、Spring 装配、Justfile、baseline 与序列化配置必须零正向引用。
6. `CouponActivityServiceImplTests=37/0F/0E/0S` 已证明 readiness 错误文案随 generic direct service 迁移；`CouponFaceContractShapeTests=61/0F/0E/0S` 是未改源码的真实计数，旧 `63` 仅为权威计数错误，不代表删除测试。两项只作当前 Green 门禁校准，不放宽业务断言。
7. 最终独立 Checker=`PASS / P0=0 / P1=0 / P2=0`，确认 15 文件闭包、Core API、Activity/Face XML、immutable RED、零兼容和工作区范围成立。该 PASS 只关闭 R8B Green；MIG-09 必须先另建 documentation-only Entry Card，不得直接执行 legacy 全仓删除。

### 20.44 `W5-MIG09-FROZEN-ORDER-PUBLIC-CRUD-RETIREMENT-ENTRY-CARD-001`

#### 20.44.1 测试价值与准入结论

本卡只证明 FrozenOrder Public CRUD 可以退出，不删除冻结事实。四仓 production scan 对 service/DTO/query/request/impl/converter 为零 Consumer；内部生命周期仍由余额控制命令、冻结单 Entity/Mapper、原 route、request digest、累计释放、Ledger/Balance 与稳定查询承重。raw rail、Ledger 宽查询和整体 `FundsTransactionQueryService` 均不在本卡。

当前 `FundsFrozenOrderServiceImplTests` 的两个测试只保护宿主手工创建冻结单时的 context 拒绝；该入口本身没有 Consumer 且允许制造与真实 freeze command 平行的持久事实。入口删除后，敏感上下文仍由 `FundsInstructionContextValidator`、`FundsBalanceControlInstructionConverter`、`SensitiveContextVariablesValidatorTests` 和真实资金 flow 保护，不保留 CRUD 只为运行两条安全测试。
...
```text
MODIFY tests/src/test/java/com/wind/funds/transaction/services/impl/FundsFrozenOrderServiceImplTests.java
```

保留现有两个 `@Test` 数量，改为：

1. `face absence`：`FundsFrozenOrderService`、`FundsFrozenOrderDTO`、`FundsFrozenOrderQuery`、`CreateFundsFrozenOrderRequest` 必须不存在。
2. `impl absence + survivor`：`FundsFrozenOrderServiceImpl`、`FundsFrozenOrderConverter` 必须不存在；`FundsBalanceControlService`、`DefaultFundsFrozenOrderLifecycleSaver`、`FundsTransactionQueryService` 必须存在。

RED 预期=`2/2F/0E/0S`，只允许 assertion failure 列出六个当前仍存在类型；class loading、Spring、H2、fixture、编译或非目标失败均阻断。RED Checker PASS 后该文件 immutable，Green 不得修改或删除。

#### 20.44.3 Green 文件、测试映射与精确门禁

Green 只执行系分 11.37.2 的 `DELETE=6 / MODIFY=3 / ADD=0`。两个测试 MODIFY 各只删除一条退役类型清册，接入指南 MODIFY 只替换验证锚点；不得修改业务断言。删除类型到证据映射：

| 退役责任 | Green 文件 | 保留证据 |
| --- | --- | --- |
| face CRUD 与专属模型 | service + DTO + query + request 四个 DELETE | immutable absence contract 2/0 |
| 平行 CRUD 实现/转换器 | impl + converter 两个 DELETE | internal lifecycle 3；balance-control 44；Public Contract 清册 |
| route 写事实禁止清册 | `RouteResolverFactBoundaryTests` MODIFY | 原测试 `1/0` |
| Public request context 清册 | `FundsTransactionRequestContextVariablesContractTests` MODIFY | 原测试 `6/0`；其余 14 个 context case 不变 |
| 余额控制接入验证锚点 | `docs/用户接入指南/README.md` MODIFY | 两处只以 `DefaultFundsFrozenOrderLifecycleSaverTests` 替换退役 CRUD 测试名；保留 balance-control/withdraw 行为锚点 |
| 冻结/解冻事实、累计与提现 | 不写 production survivor | `DefaultFundsFrozenOrderLifecycleSaverTests=3`、balance failure=`19`、withdraw=`19`、partial unfreeze=`4` |

最小聚焦命令：

```bash
just verify-slice FundsFrozenOrderServiceImplTests,DefaultFundsFrozenOrderLifecycleSaverTests,FundsBalanceControlFailureFlowTests,FundsWithdrawalSuccessFlowTests,FundsWithdrawalAfterPartialUnfreezeFlowTests,RouteResolverFactBoundaryTests,FundsTransactionRequestContextVariablesContractTests tests
```

目标聚焦=`54/0F/0E/0S`；`just test-balance-control=44/0F/0E/0S`。随后执行 `just clean-compile`、`just verify-public-contracts`、`just test-transaction`、`just test-business-flow`、`just test-boundary`、`git diff --check` 和独立 Checker。Public Contract 从当前 fresh `304/179/42` 精确变为 `300/176/42`；Core API 保持 `102/94/4/4 / 1025 lines`。扩大计数若因白名单外并发测试变化而不同，必须先解释并重冻，不得刷新门禁掩盖下降。

#### 20.44.4 冻结基线与停止条件

当前 HEAD=`ed4eaeddbfa3eb529847f3d14b23cf2c360c2337`，live manifest 在盘点期间由 `70/106b29f4...` 并发漂移后稳定为 `102/02cb4c18...`，staged empty、`git diff --check` PASS。六个 DELETE SHA 依次为 `c988f5a3... / 8332cd5a... / fe7b5212... / 4e3ce898... / fd5b15f4... / 545a4847...`；RED test=`645ef419...`；两个 Green test MODIFY=`8735a0c0... / 3c367afe...`；接入指南=`ff2d9e55...`、既有 diff=`541b4506...`。converter 是唯一生产目标 dirty overlap，当前内容 SHA=`545a4847...`、diff SHA=`f92a30b7...`；未来删除必须精确命中该版本，不得先覆盖或回退其 strict MapStruct hunk。接入指南只允许在既有 R8B dirty 内容上替换两处测试锚点，不得覆盖其他段落。

Entity、Mapper、lifecycle saver 当前虽有并发非目标修改，但全部 immutable；任一 target/immutable SHA、manifest、staged、caller 或 authority 变化使本卡 stale。未获后续 RED Grant 前不运行测试、不修改 Java/测试/Justfile/接入指南；MIG-09 其他候选、Git、联网、安装、HOST/L4、enable/release/production 均未授权。

独立 Entry Card Checker 最终=`PASS / P0=0 / P1=0 / P2=0`，确认 RED `1 MODIFY`、Green `6 DELETE + 3 MODIFY`、两个 contract 方法、Public Contract/54/44 计数、dirty overlap 与 immutable survivor 成立。下一唯一 Human gate 为 `MIG09_FROZEN_ORDER_PUBLIC_CRUD_RED_EXECUTION_GRANT`，只允许修改 `FundsFrozenOrderServiceImplTests` 形成 `2/2F/0E/0S`；不授权 Green、其他测试/生产、其他 MIG-09、Git 或发布。

#### 20.44.5 RED Entry Card re-freeze

首次获准 RED 已把目标测试改为两个 absence contract；Java 21 写前/后 compile=`21/21`，两次 fresh 技术 RED 均为 `2/2F/0E/0S`，face failure 只列四个旧 Public 类型，impl failure 只列 impl/converter，三个 survivor 先通过。当前测试 SHA=`ac4b60ca...`；固化诊断 XML SHA=`f0349e81...`。但原卡 manifest=`102/02cb4c18...` 在执行中变为 `107/436200c2...`，故独立 RED Checker 分层裁决为 `技术 RED PASS / 整体 NOT PASS / P0=0 / P1=1 / P2=0`，不得回写 RED PASS 或进入 Green。

manifest 五项增量中，目标 RED test 是授权内变化；非目标只有以下四项，全部 read-only：

| 非目标 governance replay 差异 | 当前 SHA / diff SHA | 本卡处置 |
| --- | --- | --- |
| `governance/face/src/main/java/com/wind/funds/governance/projection/FundsProjectionReplayTaskDTO.java` | `50bd5e63... / f6898e55...` | 冻结现状，不读取为 FrozenOrder 依据，不修改 |
| `governance/impl/src/main/java/com/wind/funds/governance/dal/entities/ProjectionReplayTask.java` | `cf7bcd10... / 5e88a93c...` | 同上 |
| `governance/impl/src/main/java/com/wind/funds/governance/projection/FundsProjectionReplayService.java` | `ffb13b3c... / 1223a15b...` | 同上 |
| `tests/src/test/java/com/wind/funds/transaction/application/flow/FundsHostCompositionContractTests.java` | `270dad6d... / 58303f5d...` | 同上 |

重冻后下一 RED Grant 的 source baseline 是 test SHA=`ac4b60ca...`，默认动作仅为 fresh 重跑，不再改测试；结果仍须 `2/2F/0E/0S` 且 XML 可回读。六生产候选、converter dirty、Entity/Mapper/lifecycle/query、Public Contract 与 Green `6 DELETE + 3 MODIFY` 均保持 20.44.3/4 原值。任何目标/immutable/non-target SHA、manifest 或 staged 变化使卡再次 stale。

独立 re-freeze Checker 最终=`PASS / P0=0 / P1=0 / P2=0`。下一 `MIG09_FROZEN_ORDER_PUBLIC_CRUD_RED_EXECUTION_GRANT` 只允许在 test SHA=`ac4b60ca...`上 fresh 重跑并固化 `2/2F/0E/0S`；不允许再改测试，不授权 Green、其他 MIG-09、Git、L4 或生产。

#### 20.44.6 RED fresh-rerun 与 Green 结果

获准后的 RED 只在 immutable test SHA=`ac4b60ca...` 上 fresh 重跑，没有再次修改测试；结果=`2/2F/0E/0S`，failure class 精准覆盖六个待删除类型，固化 XML SHA=`51647480...`，独立 Checker=`PASS / P0=0 / P1=0 / P2=0`。Green 随后只执行 `DELETE=6 / MODIFY=3 / ADD=0`，absence test 保持 immutable，未写 Entity、Mapper、schema、lifecycle、Balance/Ledger 或 query，也未提供兼容路径。

最终验证：Core API clean compile=`21/21`、`102/94/4/4 / 1025 lines`；Public Contract=`300/176/42`；absence=`2/0F/0E/0S`；focused=`54/0F/0E/0S`；balance-control=`44/0F/0E/0S`；transaction=`185/0F/0E/0S`；business-flow=`200/0F/0E/0S`；boundary=`212/0F/0E/0S`。45 份固化 Surefire XML 全绿；四仓 Java 正向引用和 clean classpath 旧类型均为 0。boundary 新增四项来自执行前已存在的非目标架构测试，未改本卡测试数或业务断言。独立 Checker 对技术 Green=`PASS / P0=0 / P1=0 / P2=0`。

#### 20.44.7 Green closeout manifest re-freeze

技术 Green Checker 完成后，manifest 从其复核的 `114/bfa74000...` 变为当前稳定 HEAD=`ed4eaeddbfa3eb529847f3d14b23cf2c360c2337`、default=`115/063a1de8fb1fd01f227e07dbabd3d7421726a03b973adc8488fb2b4e10601370`、`-uall=115/22dfd572039037c422d2b5029fef7c350e110752236c7884165261e98ea7b896`。唯一新增路径是只读 `openspec/changes/funds-state-naming-normalization/spec.md`，当前 SHA=`7eb5839e42f400bed876febb955fbbc29506bf5a0e8658564853aee2a7ba6dc6`；该规格明确排除 MIG-09 Green，不进入本卡 whitelist，也不改变任何测试、生产目标或 survivor。

RED test 与五个 survivor SHA 保持，staged empty、`git diff --check` PASS。由于 drift 发生在技术验证和 Checker 后，本重冻复用既有 Green/XML 证据，不重复运行技术测试；当前只等待独立 re-freeze Checker。Checker PASS 前不得把技术 Green 写成当前 manifest 的治理准出，也不得修改 state-naming 规格、Java、测试、Git 或其他 MIG-09。

#### 20.44.8 state-naming rebase 后 fresh Green closeout

旧 MIG-09 固化快照的 source 仍包含六个退役 CRUD 类型，但同一快照的 absence XML 为 `2/0F/0E/0S`，不能证明同一 source/classpath 下的 Green，故本轮不复用该快照。当前 HEAD=`ce360b039940aa951588bbe4c1bfdc43df181315` 上先由 `just verify-core-api` clean 编译全部 21 个 reactor module，再以 current checkout fresh 执行合同与扩大回归。

| 验证 | 当前 fresh 结果 | 证明边界 |
| --- | --- | --- |
| `just verify-core-api` | compile=`21/21`；Core API=`102/94/4/4 / 1025 lines` | clean classpath 与当前源码一致，六个旧类型为 0 |
| `just verify-public-contracts` | `300 types / 176 models / 42 enums` | 四个 Public CRUD 类型已从清册退出，其余合同未扩张 |
| immutable absence contract | `2/0F/0E/0S` | face 四类型、impl/converter 不存在，三个 survivor 存在 |
| 七类冻结/边界 focused | `54/0F/0E/0S` | lifecycle、余额控制、提现、route 与 context 清册保持 |
| `just test-balance-control` | `44/0F/0E/0S` | 冻结/解冻余额控制不变量保持 |
| `just test-transaction` | `185/0F/0E/0S` | transaction 回归保持 |
| `just test-business-flow` | `200/0F/0E/0S` | 资金业务流回归保持 |
| `just test-boundary` | `212/0F/0E/0S` | 模块与事实写入边界保持 |

state-naming 先行切片在当前 HEAD 已由独立 Checker 判定 `PASS / P0=0 / P1=0 / P2=0`，因此 survivor 的 state 持久化映射不再是 MIG-09 closeout 阻断。本轮只回写三份权威文档，Java、测试、schema、构建与 Consumer 均未写；当前 manifest=`95/307f22161003c5d4a7df61230aff55927297e7b9bd123096c3b1a13b3686eb74`、staged empty、`git diff --check` PASS。

独立 closeout Checker 最终=`PASS / P0=0 / P1=0 / P2=0`，复算 45 份 fresh XML 后确认上述五组测试矩阵全绿，并核对六旧类型源码/clean classpath 为 0、survivor/test SHA 无漂移、三文档写入后无 Java/测试/构建/schema 写入。该 PASS 只关闭当前 FrozenOrder Public CRUD retirement；下一 MIG-09 候选仍须先有新的 documentation-only Entry Card 和 Human Owner 授权。

### 20.45 `W5-MIG09-LEDGER-TRANSACTION-QUERY-SURFACE-NARROWING-ENTRY-CARD-001`

#### 20.45.1 RED 合同

RED 白名单只有 `MODIFY tests/src/test/java/com/wind/funds/ledger/service/LedgerTransactionServiceFactQueryTests.java`。现有首个契约测试同时证明保留的 `getLedgerTransactionBySn/getLedgerEntryBySn` 返回 DTO、非 Optional、非 DAL，并聚合要求以下旧 surface 缺失：

```text
getLedgerTransactionById(Long)
queryAccountLedgerTransactions(LedgerTransactionQuery, WindQuery)
getLedgerEntryById(Long)
com.wind.funds.ledger.query.LedgerTransactionQuery
```

同文件 tampered digest helper 只机械移除两个待退役 by-id 调用，transaction/plan/entry 三个参数化 invocation、stable failure label、零事实副作用以及 by-SN/entry-query/plan-membership 完整性断言保持。测试数量不变，旧生产源码精确 RED=`8/1F/0E/0S`；任何 compile error、第二 failure、Spring/H2 error 或 retained method 失败均阻断。RED 独立 Checker PASS 后该文件 immutable。

#### 20.45.2 Green 映射与门禁

Green 只执行系分 11.38.2 的 `MODIFY=2 / DELETE=1 / ADD=0`，映射如下：

| RED 缺口 | Green 文件 | 保持证据 |
| --- | --- | --- |
| 两个无 tenant 的 by-id 方法仍存在 | interface + impl MODIFY | stable-SN 反射、tampered digest 与 Reconciliation 回归 |
| 无 Consumer 的交易宽分页仍存在 | interface + impl MODIFY | transaction/entry/plan retained method 清册与 Public Contract |
| 专属 `LedgerTransactionQuery` 仍存在 | Query DELETE + 两个 import 清理 | Public Contract `299/175/42`，不新增 replacement Query |
...
just mvn-version
just compile
just verify-slice LedgerTransactionServiceFactQueryTests tests
just verify-public-contracts
just test-ledger
just test-reconciliation
just test-transaction
...
目标 Green：contract=`8/0F/0E/0S`、Ledger=`77/0F/0E/0S`、Reconciliation=`247/0F/0E/0S`、Transaction=`186/0F/0E/0S`、Business=`200/0F/0E/0S`、Boundary=`211/0F/0E/0S`、compile=`21/21`、Public Contract=`299/175/42`、Core API=`102/94/4/4 / 1025 lines`。本 Entry Card 只设计测试与未来代码闭包，不运行 Maven，不授权 RED 或 Green。

#### 20.45.3 Immutable 与停止条件

Green 中 RED test immutable；`getLedgerTransactionBySn`、`getLedgerEntryBySn`、`existsPostingPlan`、`queryLedgerEntries` 及其 digest fail-closed、Clearing 真实 caller、DTO、`LedgerEntryQuery`、Entity/Mapper/schema、posting/command/Balance 均不可写。interface 当前已有两行 Javadoc dirty，只能随未来同文件 MODIFY 保留，不得回退。任何新 caller、测试 invocation 数下降、需要新增 API/文件、需要兼容、或扩大到 DTO 字段和 entry query 的需求，都必须返回 Human Owner 另建卡。

独立 Entry Card Checker 最终=`PASS / P0=0 / P1=0 / P2=0`，独立复核目标/retained caller、8 invocation RED 可达性、三个 Green 生产文件、interface 既有 Javadoc dirty、Public Contract `299/175/42` 目标和 Ledger/Transaction/Reconciliation/Business/Boundary fresh XML。该 PASS 只冻结 RED/Green 文件卡；当前仍为 `RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

#### 20.45.4 RED 结果

RED 只修改 `LedgerTransactionServiceFactQueryTests.java`，test SHA 从 `91c55f47...` 变为 `956da091...`。写前/后 compile=`21/21`；fresh XML mtime=`2026-08-26 13:44:55`、SHA=`d2614a35...`、结果=`8/1F/0E/0S`。唯一 failure 为 `testLedgerTransactionServiceShouldExposeStableSnGetQueriesWithoutOptional`，实际列表精确等于三个旧方法和 `com.wind.funds.ledger.query.LedgerTransactionQuery`；其余 7 invocation 通过，无 Spring/H2/error/skip。

独立 RED Checker=`PASS / P0=0 / P1=0 / P2=0`，确认 tampered digest 仍由 stable-SN、entry query、`existsPostingPlan` 三层读取保护，零事实副作用断言保持。Green 中该 test 现为 immutable；当前 `GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`。

#### 20.45.5 Green 结果

Green 只执行 `MODIFY=2 / DELETE=1 / ADD=0`，immutable contract test SHA=`956da091...` 未变。clean compile 后 `LedgerTransactionQuery` source/class 均不存在，三个旧方法不再进入 face/impl；保留方法、Clearing caller 和账务完整性实现未改。

fresh 结果：contract=`8/0F/0E/0S`、Ledger=`77/0F/0E/0S`、Reconciliation=`247/0F/0E/0S`、Transaction=`186/0F/0E/0S`、Business=`200/0F/0E/0S`、Boundary=`211/0F/0E/0S`、Public Contract=`299/175/42`。显式 Java 21 Core API baseline PASS；系统 Java 8 `javap` 对 default method 的额外误报不计为源码或合同 failure。独立 Checker=`PASS / P0=0 / P1=0 / P2=0`。

### 20.46 `W5-MIG09-TRANSACTION-QUERY-TENANT-SCOPE-BREAKING-CLOSEOUT-001`

本切片保护的是交易、明细、RouteSnapshot 和 replay 消费金额的租户数据边界，不把全局唯一 sn 当作授权。无兼容 breaking contract 要求六类持久化查询均以 `Long tenantId` 为首参，旧 `queryFundsTransaction` 和无 tenant 重载必须不存在；实现中主交易、明细、replay 明细、冻结单与排除释放记录均必须带 tenant 谓词。

| 验证层 | 当前 fresh 结果 | 保护事实 |
| --- | --- | --- |
| `FundsTransactionQueryServiceContractTests` | `2/0F/0E/0S` | 方法首参为 `tenantId`，旧无 tenant 方法和旧名称不存在 |
| `FundsTransactionQueryTenantIsolationTests` | `3/0F/0E/0S` | 真实 Spring/H2 下外租户对主交易、明细、交易/冻结 RouteSnapshot 得到 empty，对 replay 消费得到 false/zero；本租户仍可正常读取 |
| 生产 caller/SQL 只读闭包 | `45 callers / 0 legacy overload` | caller tenant 来自指令或已持有事实；所有目标 MyBatis-Flex 查询带 tenant 谓词，RouteSnapshot 不反向授权 |
| 扩大回归 | Transaction=`186/0/0/0`；Reconciliation=`247/0/0/0`；Business=`200/0/0/0`；Boundary=`211/0/0/0` | 交易、route replay、清算/追偿、支出控制和模块边界不回退 |
| 全仓 current checkout | compile=`21/21`；Surefire=`1198/0F/0E/1S`；Public Contract=`299/175/42`；PMD 13 份无违规 | 最终源码早于最终 XML；唯一 skip 为无真实 MySQL host 的 migration integration |

跨租户断言不泄露存在性：单对象返回 `empty`，明细返回空列表，判定返回 `false`，金额返回同币种零值；Recovery 的 foreign-tenant case 必须在查询边界报“交易不存在”，不先读取后比较 tenant。独立 Checker=`PASS / P0=0 / P1=0 / P2=0`。

当前只准出 `RUNTIME_D4`，不把本地测试写成生产 D5。仓外 Consumer 无兼容迁移、lifecycle saver 私有 sn-only 读取、raw-id、字段命名和 projection/context policy 都不属于本测试准出；任一项需要新源码或新契约时，必须停止并建立独立 Entry Card。

### 20.47 `W5-MIG09-LIFECYCLE-TENANT-INVARIANT-ENTRY-CARD-001`

本卡保护 lifecycle 写边界，不再把稳定 transaction/detail/freeze sn 当作跨租户授权。被保护事实为：`FundsInstructionSpec.tenantId` 是两个 saver 全部读取和更新的唯一 tenant 来源；result、reference、RouteSnapshot 和 context 只提供事实标识。外租户标识必须在 lifecycle 查询边界表现为不存在，零交易/明细/冻结单状态变化，零授权/退款/释放累计变化，零 Ledger/Balance 新增或变化。

#### 20.47.1 RED 场景与不可变断言
...
| testcase | Given | When | Then / 红线 |
| --- | --- | --- | --- |
| `testMarkFailedShouldRejectForeignTenantTransactionLifecycleResult` | 真实 Spring/H2、当前 tenant 的 saver `beforePosting` 已建立 PROCESSING transaction/detail；外租户 instruction 持有同一 result | 直接调用真实 `DefaultFundsInstructionLifecycleSaver.markFailed` | 必须抛不存在类异常；transaction/detail 仍为原状态，错误码/信息不变，Ledger/Balance/其他租户事实零变化 |
| `testMarkSucceededShouldRejectForeignTenantFrozenOrderLifecycleResult` | 真实 Spring/H2、当前 tenant 的 frozen saver `beforePosting` 已建立 CREATED order；外租户 instruction 持有同一 result | 直接调用真实 `DefaultFundsFrozenOrderLifecycleSaver.markSucceeded` | 必须抛不存在类异常；order state/releasedAmount/freezeLedgerTransactionSn 不变，原冻结、Ledger/Balance 零变化 |

测试继承既有 `FundsTransactionFlowTestSupport`，真实使用 Spring Bean、H2、Mapper、Route resolver/factory 和 lifecycle，不 mock saver、Mapper、事务或资金内部链路；只复用既有账户/账本测试基座。测试方法使用 `testXxx`，场景、输入、输出和红线写在方法级 Javadoc。不得修改 `FundsTransactionFlowTestSupport`、现有 tenant query 测试、schema 或 fixture 来制造 RED。

旧实现预期 fresh=`2/2F/0E/0S`。如果失败来自缺 Bean、账户/route fixture、SQL/schema、编译或测试污染，属于 Harness 失败，必须先修正测试本身并重新取得精准 RED；不得进入 Green。RED Checker PASS 后新测试 SHA immutable。

#### 20.47.2 Green 断言与验证矩阵

Green 只修改两个 saver。每条 `findTransactionDetails/findTransactionBySnNullable/findTransactionBySn/findDetailBySn/findDetailsBySn/findFrozenOrderBySn/findReferencedFrozenOrder` 及其 sibling 调用必须显式接收当前 instruction tenant，MyBatis-Flex 继续使用对应 NameRefs，不增加裸字符串、LambdaQueryWrapper、TenantContext fallback 或新 helper 层。

| 验证层 | 命令 / 目标 | 保护事实 |
| --- | --- | --- |
| 编译与 RED/Green | `just compile`；`just verify-slice FundsInstructionLifecycleTenantIsolationTests tests` | RED 精准 2F；Green=`2/0F/0E/0S`，且目标 XML 为本轮 fresh |
| lifecycle + tenant 联合 | `just verify-slice FundsInstructionLifecycleTenantIsolationTests,DefaultFundsFrozenOrderLifecycleSaverTests,FundsTransactionQueryTenantIsolationTests tests` | 新写边界、既有冻结摘要/状态和 Public Query tenant 边界同时成立 |
| 资金扩大回归 | `just test-balance-control`、`just test-transaction`、`just test-business-flow` | 冻结/解冻、退款、回放、Ledger/Balance、幂等和失败无副作用不回退 |
| 架构与契约 | `just test-boundary`、`just verify-public-contracts` | face/core/internalization/raw-id 等非目标保持，Public Contract=`299/175/42` |
| 规约与全量 | `just pmd`、`just verify-cad`、`git diff --check` | 两个 saver 无旧 sn-only query；全量从当前 `1198` 只新增 2 个 testcase，既有唯一 MySQL skip 可披露但不得新增 skip |

Checker 必须独立复核：两个 result RED 是否真实触达旧缺口；所有 sn/transactionSn sibling query 是否带 tenant；tenant 是否始终来自 instruction，而非 result/RouteSnapshot/context；外租户异常是否不泄露目标事实状态；当前 naming dirty 是否完整保留；无 face/DTO/internalization/Wallet/Ledger/schema/兼容扩面。Green 通过后只准出 current checkout `RUNTIME_D4`，不代表生产 D5。

独立 Entry Card Checker=`PASS / P0=0 / P1=0 / P2=0`。Checker 确认旧实现的 transaction `markFailed` 和 frozen `markSucceeded` 会分别产生可观察外租户状态修改，两个 testcase 可精准形成 `2/2F/0E/0S`；Green 后共享 helper 首次 tenant-scoped read fail-fast，全部 reference/context/concurrent-replay caller 由无 tenant helper 签名清零、编译闭包和源码 100% scan 强制迁移。该 PASS 不授权 RED；下一唯一 Gate 为 `MIG09_LIFECYCLE_TENANT_INVARIANT_RED_EXECUTION_GRANT`。

#### 20.47.3 RED 实际结果与 Harness 返工

首轮 RED 以真实 Spring/H2 和 saver 得到 `2/2F/0E/0S`，但 route/snapshot 为手工 fixture，独立 Checker 因未命中冻结 route/factory 判定 `NOT PASS / P1=1`。同一文件返工后，两场景都真实调用 `RouteResolver.resolve -> RouteSnapshotFactory.createSnapshot -> saver.beforePosting`；TOPUP 三条 detail 逐条冻结，FREEZE 的 setup Ledger/plan/entry 和 Balance 使用 before/after 集合比较。

返工后 test SHA=`39ed7c98...`，fresh XML=`717fc287...`，仍为 `2/2F/0E/0S`。Transaction case 实际将主交易和三条 detail 从 `PROCESSING` 改为 `FAILED` 并写 error；Frozen case 实际将 order 从 `CREATED` 改为 `FROZEN` 并写 ledger reference。零 Ledger/Balance 增量成立，无 error/skip；独立 RED Checker=`PASS / P0=0 / P1=0 / P2=0`，随后 test immutable。

#### 20.47.4 Green 实际结果

Green 只修改两个 saver，全部目标 helper 首参为 `Long tenantId`，四个真实 SN/transactionSn QueryWrapper 同时使用 NameRefs tenant 与 stable SN 谓词，tenant 只来自当前 instruction。源码和 Java 21 classfile 中旧无 tenant 签名为 0；immutable RED SHA=`39ed7c98...` 保持。

| 验证层 | fresh 结果 |
| --- | --- |
| lifecycle | `2/0F/0E/0S` |
| lifecycle + frozen + query tenant | `8/0F/0E/0S` |
| balance-control / transaction / business-flow / boundary | `44/0 / 186/0 / 200/0 / 211/0` |
| Public Contract / PMD | `299/175/42` / 13 份 PMD XML 无 violation/error |
| CAD / Surefire | `verify-cad exit 0` / 119 份 XML=`1200/0F/0E/1S` |

唯一 skip 为既有无真实 MySQL host migration integration；lifecycle XML SHA=`ee25682f...` 晚于源码/classfile。独立 Green Checker=`PASS / P0=0 / P1=0 / P2=0`。当前只准出 current checkout `RUNTIME_D4`，不代表生产 D5、不授权其他 MIG-09。

### 20.48 `W5-MIG09-WALLET-TENANT-RAW-ID-SURFACE-INQUIRY-001`

#### 20.48.1 当前测试缺口与验证分层

当前账户正向测试能证明开户、ledger 初始化、账户能力和余额投影，但未把 tenant 作为 core account query 的 public contract：`FundsAccountQueryService#getAccount/getLedgerProfileCode/getBalance/supports` 均只接收 `FundsAccountId`。Funding/Credit 的显式 `tenantId + accountSn` 生产路径已有正向证据，12 个 raw Long getter 则只有 5 个 Wallet 内部 caller、7 类 wind-funds 测试调用或 Capte unsupported stub，没有仓外生产价值。

本 Inquiry 不创建 RED，只冻结后继分层：

| 候选卡 | 保护事实 | RED 行为 | Green 目标 |
| --- | --- | --- | --- |
| Core query tenant scope | 账户存在、状态、币种、能力、profile、余额 | 同租户四方法正向；外租户三个强查询不存在、`supports=false`；反射断言新签名存在且旧签名不存在 | 四方法首参 tenant；Default impl 所有 Funding/Credit query 含 tenant；24 production caller closure |
| Funding query narrowing | FundingAccount 主数据与受控检索 | raw-id/FundsAccountId overload absence；缺 tenant query fail-closed；Capte tenant query 保持 | 删除两个旧 getter，query tenant 必填，tenant+sn 保留 |
| Credit query narrowing | CreditAccount 主数据与受控检索 | 与 Funding 同形，但独立测试和文件卡 | 删除两个旧 getter，query tenant 必填，tenant+sn 保留 |
| Secondary account query | 平台账户解析、账户层级 | implicit tenant overload absence；hierarchy wide query absence；explicit tenant find 正向 | 删除 implicit overload 与零 Consumer 宽分页，不改稳定 find |
| 其余 raw-id | PaymentInstrument、Spend、Rule 事实 | 分组 absence + 现有业务行为保持 | 每组删除无公共价值 getter；内部 caller 不新增替代 Public API |

#### 20.48.2 推荐首卡的 RED 候选

推荐未来只新增：

```text
tests/src/test/java/com/wind/funds/wallet/services/impl/FundsAccountQueryTenantIsolationTests.java
```

测试必须使用真实 Spring Bean、H2 Funding/Credit/Ledger 表和真实 `DefaultFundsAccountQueryServiceImpl`，不得 Mock Mapper 或断言私有 helper。建议保持 3 个外显 testcase：

1. public contract：用反射断言 `getAccount/getLedgerProfileCode/getBalance/supports` 均为 `(Long tenantId, FundsAccountId)`，旧单参签名不存在；反射只守 public breaking contract。
2. cross-tenant：先建 tenant A/B 的真实账户和 ledger，在 A 授权上下文下用 B 的 locator 调用未来 public surface；测试 helper 在新签名尚不存在时调用旧 surface，从而证明旧实现会返回 B 的账户/profile/balance 或 `supports=true`，Green 后统一 fail-closed。
3. same-tenant：A 的账户、profile、余额桶、状态、币种、capability 和 `supports=true` 保持，证明租户约束不改变正常资金语义。

RED failure 必须来自新签名缺失或外租户事实被读取，不能来自 Bean、schema、ledger fixture、反射异常未归纳、编译或环境。Entry Card 冻结后再确定精确 invocation 计数；本 Inquiry 不预写 `2F/3F` 数字冒充 fresh RED。

#### 20.48.3 推荐首卡 Green 闭包与测试迁移

生产 Green 白名单严格沿系分 `11.41.5` 的 24 文件。tenant 来源只允许：`FundsInstructionSpec#getTenantId`、已持有 request/order 的 `tenantId`、`LedgerTransactionSpec#getTenantId`，或当前 ApplicationService 已使用的 `TenantContextHolder.requireTenantId()`；不得从查询返回的 `FundsAccount#getTenantId`、RouteSnapshot、LedgerEntry context、账户 locator 或异常信息反向决定 tenant。

新增 RED 在 Green 阶段 immutable。为适配 breaking signature，只有以下 6 个既有测试文件可机械迁移 public 调用、stub 或匿名实现，不得放宽资金断言：
...
tests/src/test/java/com/wind/funds/wallet/application/spend/SpendRuleEvaluationApplicationServiceTests.java
tests/src/test/java/com/wind/funds/wallet/services/impl/ControlAccountLedgerInitializationTests.java
tests/src/test/java/com/wind/funds/wallet/services/impl/FundingAccountServiceImplTests.java
```

`DefaultLedgerTransactionPostingServiceImplTests`、`LedgerTransactionServiceFactQueryTests`、`ExternalFundsEventApplicationServiceTests`、`FundsTransactionFlowTestSupport`、`PaymentInstrumentTransactionApplicationServiceTests` 没有旧方法调用、stub、匿名实现或需传 tenant 的测试 helper，只作为扩大回归 verification-only/non-write，不进入未来 MODIFY 白名单。

未来最小验证矩阵：

| 验证层 | 命令 / 目标 |
| --- | --- |
| compile | Java 21 `just mvn-version`、写前/后 `just compile`，旧 core query 签名源码/classfile 为 0 |
| tenant RED/Green | `just verify-slice FundsAccountQueryTenantIsolationTests tests`，跨租户无账户/profile/balance/存在性泄露 |
| Wallet/Ledger | `FundingAccountServiceImplTests,ControlAccountLedgerInitializationTests,FundsSubjectBalanceQueryServiceImplTests,LedgerBalanceProjectionServiceImplTests,DefaultLedgerTransactionPostingServiceImplTests` |
| Transaction/Reconciliation | `just test-balance-control`、`just test-transaction`、`just test-business-flow`、`just test-reconciliation` |
| 契约与边界 | `just test-boundary`、`just verify-public-contracts`、Java 21 Core API baseline |
| 规约与全量 | `just pmd`、`just verify-cad`、`git diff --check` |

Capte production 无需为 core 首卡修改，但未来 Funding/Credit 子卡必须重跑 `CouponActivityServiceImplTests`、`CouponRedemptionApplicationServiceImplTests` 和 `OrderCouponRedemptionIntegrationTests`，证明 tenant+sn/query、真实 Bean 与联合 H2 保持。Fincone/fincone-issuing 当前无 Java，不能用文档引用替代 Consumer 测试。

当前只冻结 Inquiry 与候选，不授权创建 RED。下一代码 Gate 前保持 `ENTRY_CARD_REQUIRED / RED_EXECUTION_GRANT_NO / CODE_FREEZE`；`InternalAccountRef` 物理类型、create 返回 Long、DTO/Request id、lifecycle internalization、entrySide、digest/context、projection、并发、DDL 与其他 MIG-09 均排除。

独立 Inquiry Checker 首轮因测试写入白名单过宽判定 `NOT PASS / P1=1`。三文档只把 11 个候选收紧为 6 个真实旧签名调用/stub/匿名实现，另 5 个扩大测试保持 verification-only/non-write；返工 Checker=`PASS / P0=0 / P1=0 / P2=0`。本 PASS 不授权创建 `FundsAccountQueryTenantIsolationTests`，下一 Gate 仍是 Core query Entry Card 的 documentation grant。

### 20.49 `W5-MIG09-CORE-FUNDS-ACCOUNT-QUERY-TENANT-SCOPE-ENTRY-CARD-001`

#### 20.49.1 immutable RED 精确合同
...
测试使用 `AbstractFundsServiceTest.TestInfrastructureConfig` 与本文件内部 `@Configuration/@Import`，真实装配 `FundingAccountServiceImpl`、`CreditAccountServiceImpl`、`DefaultFundsAccountQueryServiceImpl`、`LedgerServiceImpl` 和 `LedgerProfileCatalog`；使用 H2 Funding/Credit/Ledger 表，不 Mock Service/Mapper，不探测生产私有 helper 或 QueryWrapper。

冻结为 3 个 `@Test`、3 次 invocation：

1. `testFundsAccountQueryServiceShouldRequireTenantScopedPublicContract`：public reflection 同时断言四个 `(Long, FundsAccountId)` 方法存在，四个旧 `(FundsAccountId)` 方法不存在。测试通过 `hasPublicMethod` 返回布尔值并用 SoftAssertions 聚合，旧实现只形成一个 test failure，不产生 `NoSuchMethodException` error。
2. `testFundingAccountQueriesShouldNotCrossTenantBoundary`：真实创建 tenant A/B FundingAccount 与 required ledger。adaptive public invoker 优先调用新双参方法；不存在时回退调用旧单参方法。先断言 A 同租户 account/profile/balance/supports 正向，再以 A tenant 调用 B locator，断言三个强查询失败且 value 为空、`supports=false`。
3. `testCreditAccountQueriesShouldNotCrossTenantBoundary`：与 Funding 同形，覆盖 CreditAccount/CREDIT_BASIC 与控制余额桶。

旧实现预期 fresh RED 精确=`3 tests / 3 failures / 0 errors / 0 skipped`：第 1 项因新签名缺失且旧签名存在失败；第 2、3 项因 foreign tenant 返回账户/profile/balance 且 `supports=true` 失败。任何 compile、Spring、H2、schema、reflection unwrap 或 fixture error 都属于 Harness 失败，不是有效 RED。

adaptive invoker 只调用 `FundsAccountQueryService` 的 public 方法：用 `Method#getParameterTypes` 选择双参或单参，`InvocationTargetException` 必须解包真实业务 cause；Green 后同一测试源码不修改并直接验证新 contract。RED Checker PASS 后 test SHA immutable。

#### 20.49.2 H2 事实与断言矩阵

| 事实 | Funding testcase | Credit testcase | foreign tenant 不可观察结果 |
| --- | --- | --- | --- |
| account | ACTIVE、USD、owner、version、FUNDING capability | ACTIVE、USD、owner、version、CREDIT capability | value 为空，异常只表达当前 tenant 不存在 |
| profile | `FUNDING_BASIC` | `CREDIT_BASIC` | 不返回 profile |
| balance | AVAILABLE/FROZEN/AUTHORIZATION 与明确非零 fixture | LIMIT/AVAILABLE/AUTHORIZATION/OUTSTANDING 与明确非零 fixture | 不返回 bucket、金额、币种或 period |
| supports | 同租户 `true` | 同租户 `true` | `false` |
| 零副作用 | account/ledger count、state、currency、owner、profile、normalBalance before/after 相等 | 同左 | 查询不得写 transaction/detail/ledger/posting/entry 或账户事实 |

tenant A/B 的 accountSn 仍使用各自全局唯一 locator，不人为制造违反 UK 的同 SN fixture。测试要证明的是“持有 B locator 的 A 调用方仍无权读 B”，不是验证 locator 唯一性。

#### 20.49.3 6 个 existing test MODIFY 精确闭包

| 文件 | 当前适配点 | Green 机械修改 | 写前 SHA |
| --- | --- | --- | --- |
| `tests/src/test/java/com/wind/funds/route/AuthorizationFundsInstructionRouteResolverTests.java` | mock `getLedgerProfileCode(USER_ACCOUNT)` | 增加 `TENANT_ID` 首参 | `5d85183d...` |
| `tests/src/test/java/com/wind/funds/route/CompositeRouteResolverTests.java` | anonymous interface 四个单参 override | 四方法增加 `Long tenantId` | `13e8ee5b...` |
| `tests/src/test/java/com/wind/funds/transaction/application/flow/FundsAuthorizationTransactionFlowTests.java` | 3 处 `getBalance(accountId)` | 增加继承的 `TENANT_ID` | `b25a91c3...` |
| `tests/src/test/java/com/wind/funds/wallet/application/spend/SpendRuleEvaluationApplicationServiceTests.java` | test bean 四个单参 override | 四方法增加 `Long tenantId` | `f24c284a...` |
| `tests/src/test/java/com/wind/funds/wallet/services/impl/ControlAccountLedgerInitializationTests.java` | 1 处 `getAccount(creditAccountId)` | 增加 `TENANT_ID` | `9c5e79e1...` |
| `tests/src/test/java/com/wind/funds/wallet/services/impl/FundingAccountServiceImplTests.java` | 1 处 `getAccount`、1 处 `getBalance` | 增加 `TENANT_ID` | `4ad7bbf1...` |

6 文件 manifest=`71585768e8ace612120ac3a3630ba77c37914a587e7866dcdcdbb06606e7061b`。只允许签名/实参机械迁移，不改测试场景、金额、断言、Spring 配置或 fixture。

以下 5 文件保持 verification-only/non-write：
...
Ledger projection 保留 `project(entries, postingAccessType)` 签名，因而上述文件不需迁移；它们只参与扩大回归。另有装配 Default impl 的测试也通过模块级验证覆盖，不因此获得写权限。

#### 20.49.4 Core API baseline 与 Green 门禁

`FundsAccountQueryService` 是 stable core type，`stable-api.txt` 当前第 800-803 行固定四个单参签名。baseline 每行格式是左侧 type prefix、literal TAB、右侧 javap signature。breaking Green 必须保持左侧 type prefix 与 TAB，只把右侧 signature column 替换为下列完整目标行；代码块中的 type 与 `public` 之间是 literal TAB：

```text
com.wind.funds.wallet.FundsAccountQueryService	public abstract boolean supports(java.lang.Long, com.wind.funds.wallet.FundsAccountId);
com.wind.funds.wallet.FundsAccountQueryService	public abstract com.wind.funds.ledger.enums.LedgerProfileCode getLedgerProfileCode(java.lang.Long, com.wind.funds.wallet.FundsAccountId);
com.wind.funds.wallet.FundsAccountQueryService	public abstract com.wind.funds.wallet.FundsAccount getAccount(java.lang.Long, com.wind.funds.wallet.FundsAccountId);
com.wind.funds.wallet.FundsAccountQueryService	public abstract com.wind.funds.wallet.FundsAccountBalanceView getBalance(java.lang.Long, com.wind.funds.wallet.FundsAccountId);
```

只授权 `MODIFY core/api-baseline/stable-api.txt`，保持 `1025 lines / 94 stable / 4 experimental / 4 internal`。`scripts/verify-core-api-baseline.sh` 使用 Java 21 `javap` 动态比较全部 stable signature，已有逻辑无需改；禁止用 `--update` 接受四行以外变化。Public Contract type/model/enum 数量保持 `299/175/42`。

#### 20.49.5 验证矩阵与预计结果

| 层级 | 命令 / 资产 | Green 目标 |
| --- | --- | --- |
| 环境与编译 | `just mvn-version`、写前/后 `just compile` | Java 21；旧四个单参签名源码/classfile=0 |
| RED/Green | `just verify-slice FundsAccountQueryTenantIsolationTests tests` | RED=`3/3F/0E/0S`；Green=`3/0F/0E/0S` |
| Wallet focused | tenant test + `FundingAccountServiceImplTests,ControlAccountLedgerInitializationTests,FundsAccountCapabilityApplicationServiceTests,SpendControlMovementServiceFlowTests` | account/profile/balance/capability 与零副作用保持 |
| Ledger focused | `LedgerBalanceProjectionServiceImplTests,DefaultLedgerTransactionPostingServiceImplTests,LedgerTransactionServiceFactQueryTests` | transaction tenant 准入、projection 与 posting 平衡保持 |
| Route/Transaction | 6 可写测试 + `just test-balance-control`、`just test-transaction`、`just test-business-flow` | route/profile/amount/capability 与资金事实保持 |
| Reconciliation | `PayoutOrderApplicationServiceTests`、`just test-reconciliation` | order tenant 传递与 payout gate 保持 |
| 架构/契约 | `just test-boundary`、`just verify-public-contracts`、Java 21 `just verify-core-api` | `299/175/42`；Core=`94/4/4/1025` |
| 规约/全量 | `just pmd`、`just verify-cad`、`git diff --check`，复算 fresh XML | 13 PMD XML 0 finding；全量预计 `1203/0F/0E/1S` |

唯一允许 skip 仍是无真实 MySQL host 的 migration integration；新增 tenant test 不得 skip。Checker 必须复核 tenant 来源、24 production、6 test、baseline 四行、5 non-write、RED adaptive 行为与源码/classfile absence。

Entry Card 冻结阶段只授权文档，不授权创建 RED。任何第 25 个 production、第 7 个 existing test、仓外 Core caller、缺 tenant 来源、新类型、Funding/Credit face/query、raw-id、Entity/Mapper/schema/build/compatibility 或 baseline 四行外变化都必须停止。当时=`ENTRY_CARD_MAKER_COMPLETE / ENTRY_CARD_INDEPENDENT_CHECKER_PENDING / RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

独立 Entry Card Checker 首轮因白名单省略路径和 baseline 行格式不完整判定 `NOT PASS / P1=2`。返工仅展开 24+6 完整路径，并把四条 baseline 目标写成完整 type prefix + literal TAB + 双参 signature；最终 Checker=`PASS / P0=0 / P1=0 / P2=0`。该 PASS 当时只准出一个 RED 文件，不授权 Green 或修改任何既有源码/测试/baseline；后续 RED 事实由 `20.49.6` 接替。

#### 20.49.6 RED 结果与 authority alignment

Human Owner 只授权新增 `FundsAccountQueryTenantIsolationTests.java`。最终测试严格保持 `3 @Test / 3 invocations`，test SHA=`300c4901e42da31e566e163ce1460a17df76809afec299571473c59edc20eda2`；fresh XML SHA=`d1edf42a89a934f71abf4a83dbd8236e51513eb831f50173c085f69b1de61903`，结果=`3/3F/0E/0S`。三项失败精确为：Public contract 聚合的 8 个新旧签名差异；Funding 外租户 account/profile/balance 非空、未抛不存在且 `supports=true`；Credit 同形泄露。无 compile、Spring、H2、schema、reflection unwrap、fixture、skip 或清理错误。

Funding/Credit 场景均使用真实 Service、`LedgerServiceImpl` 与 `LedgerProfileCatalog` 建账，同租户 ACTIVE/USD/owner/version/capability/profile/非零 balance/supports 正向全部通过。外租户失败断言覆盖 value 为空、当前 tenant 不存在、`supports=false`，并禁止异常携带 owner、state、currency、account type、capability、profile、bucket、金额、period 或 foreign tenant 关系。账户、ledger、funds transaction/detail、ledger transaction/posting/entry 的 before/after 快照在预期 RED 差异后仍实际执行并通过。

首轮 RED Checker 因泄露 token 未覆盖 account type、bucket、金额和 tenant 关系判定 `NOT PASS / P1=1`；同文件返工后最终 Checker=`PASS / P0=0 / P1=0 / P2=0`，测试自此 immutable。Checker 另发现本节原 Credit 矩阵把第四个控制桶误写为 `SETTLEMENT`；当前已按 `LedgerProfileCatalog`、`TDD-WALLET-002` 与 `TDD-CTRL-FLOW-003` 机械纠正为 `OUTSTANDING`。这只修正文档 authority drift，不改变其他 `SETTLEMENT` 场景、Green 文件卡、baseline 四行或验证矩阵。

独立文档 Checker 以三文档 SHA=`4486c86e... / cd13f8d0... / 2f7d8176...` 复核本节与 `11.42.6`、OpenSpec `plan-r2.299` 的 authority、RED 证据、白名单和恢复入口，结论=`PASS / P0=0 / P1=0 / P2=0`。当前=`RED_EXECUTION_COMPLETE / RED_INDEPENDENT_CHECKER_PASS / RED_TEST_IMMUTABLE / DOCUMENTATION_AUTHORITY_ALIGNMENT_INDEPENDENT_CHECKER_PASS / DOCUMENTATION_AUTHORITY_ALIGNMENT_COMPLETE / GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`；不得由本 RED 或文档纠正自动进入 Green，下一 Gate 只能由 Human Owner 单独裁决。

#### 20.49.7 Green 实际结果与当前范围完成

Green 未修改 immutable RED，`FundsAccountQueryTenantIsolationTests.java` SHA 仍为 `300c4901e42da31e566e163ce1460a17df76809afec299571473c59edc20eda2`。四个 public 方法只剩 `(Long tenantId, FundsAccountId)`，旧单参源码/classfile 为 0；Funding/Credit 同租户账户、profile、非零余额与 `supports=true` 保持，foreign tenant 三个强查询统一不存在且 `supports=false`，测试 fresh=`3/0F/0E/0S`。

| 验证层 | fresh 结果 |
| --- | --- |
| immutable tenant Green | `3/0F/0E/0S`；RED SHA=`300c4901...` 不变 |
| Wallet/Ledger/route/caller focused | 两组=`145/0F/0E/0S`、`47/0F/0E/0S` |
| balance-control / transaction / business-flow | `44/0 / 186/0 / 200/0` |
| reconciliation / boundary | `247/0 / 211/0` |
| Public Contract | `299 types / 175 models / 42 enums` |
| Java 21 Core API | `94 stable / 4 experimental / 4 internal / 1025 lines`；baseline SHA=`812e5328...` |
| PMD | 13 份 XML，`0 violation / 0 error` |
| 完整离线 CAD / Surefire | `verify-cad exit 0`；120 份 XML=`1203/0F/0E/1S` |

首次完整 CAD 为 `1203/1F/0E/1S`，唯一失败 `LedgerPostingJdkProxyContextTests`：`DefaultLedgerTransactionPostingServiceImpl` 新增 `FundsAccountQueryService` 构造依赖后，该最小 Context 未提供对应 Bean。Human Owner 单独授权只修改此 Harness 文件，在既有 `Config` 增加无业务 stub 的默认 mock Bean；test method、JDK proxy 断言、transaction manager、其他 Bean 和测试语义均未改变。最终 test SHA=`9ba73590ded304e92ce171b8b039a795028e167e888ae3a08554d9bbc5c349d9`、XML SHA=`92d5809c5ff4f44588a5b8aee39e36e04e2f85b7bf0134608642913ac6dc156e`，单类=`1/0F/0E/0S`；随后完整 CAD 达到上表全绿，唯一 skip 仍为无真实 MySQL host 的 migration integration。

独立技术 Green Checker=`PASS / P0=0 / P1=0 / P2=0`，复核双参 contract、tenant 首次读取、24 production、6 breaking-signature test、baseline 四行、immutable RED 和第 7 个 Harness 装配闭包均成立。独立文档 closeout Checker 以 Maker 三文档 SHA=`948dc2f6... / 5d2703ee... / 1a336d0c...` 复核本节全部 fresh 证据、首次 CAD 失败分类、D4 边界与状态一致性，结论=`PASS / P0=0 / P1=0 / P2=0`。该结果只证明 current checkout `RUNTIME_D4 / ENGINEERING_READY`；真实 MySQL、HOST/L4、发布与生产 D5 未验证。当前=`GREEN_CLOSEOUT_INDEPENDENT_CHECKER_PASS / CURRENT_SCOPE_COMPLETE / MIG09_EXECUTION_GRANT_NO / CODE_FREEZE`。

### 20.50 `W5-MIG09-FUNDING-ACCOUNT-QUERY-SURFACE-NARROWING-ENTRY-CARD-001`

#### 20.50.1 immutable RED 精确合同
...
测试继承 `AbstractFundsServiceTest`，使用本文件内部 `@Configuration/@Import` 真实装配 `FundingAccountServiceImpl`、`LedgerServiceImpl` 和 `LedgerProfileCatalog`，使用 H2 Funding/Ledger/Transaction 表；不 Mock Service/Mapper，不检查私有 helper、QueryWrapper 或 Mapper interaction。固定 3 个 `@Test`：

| testcase | Given / When | Green 断言 | 旧实现结果 |
| --- | --- | --- | --- |
| `testFundingAccountServiceShouldExposeOnlyTenantScopedQueryContract` | public reflection 检查 face 方法和 `FundingAccountQuery.tenantId` field annotation | 两个旧 getter absence；tenant+sn get/query retained；Jakarta `@NotNull` 存在 | SoftAssertions 聚合旧方法存在和 annotation 缺失为 1F |
| `testQueryFundingAccountsShouldRejectMissingTenantWithoutFactsMutation` | 真实建立 tenant A/B account+required ledgers，直接以无 tenant Query 调用 Service | 拒绝且只表达 tenant 必填；账户/Ledger/Transaction/Posting/Entry before/after 相等 | 无异常，形成 1F；零副作用断言仍执行 |
| `testTenantScopedFundingAccountQueriesShouldNotCrossTenantBoundary` | 同租户 get/query；tenant A + tenant B accountSn 的 get/query | 同租户返回 A；foreign get 当前 tenant 不存在，foreign query empty；无敏感 token/事实变化 | 旧 tenant+sn/query 已通过，不增加 failure |

旧实现 fresh 必须精确=`3 tests / 2 failures / 0 errors / 0 skipped`。任何 compile、Spring、H2、schema、fixture、清理或 reflection error 都是 Harness 失败；若第三项产生失败，先核对当前 tenant-scoped retained path，不得扩大 Green。RED Checker PASS 后 test SHA immutable，Green 不修改。

#### 20.50.2 H2 事实、不可观察结果与零副作用

两租户使用不同且全局唯一 accountSn，均通过真实 `createFundingAccount` 初始化 `FUNDING_BASIC` required ledgers。setup 后再冻结：FundingAccount 全行与数量、tenant/sn/owner/ownerType/accountType/platform/role/currency/state/profile/version/context，Ledger 全行与余额桶，以及 funds transaction/detail、ledger transaction/posting plan/entry 集合。

| 调用 | 同租户 | foreign / 缺 tenant | 不可泄露 |
| --- | --- | --- | --- |
| `getFundingAccount(tenantId, accountSn)` | 返回当前 tenant DTO | foreign 表现为当前 tenant 账户不存在 | state、currency、accountType、能力派生、profile、owner、context |
| `queryFundingAccounts(query, options)` | 只返回 query.tenantId 范围 | foreign accountSn 空页；tenantId=null 在首次 guard 被拒绝 | 其他 tenant 的存在性、数量、排序位置和全部 DTO 字段 |
| 所有 query | 只读 | fail-closed | account/Ledger/Transaction/Posting/Entry 零新增、零更新 |

异常可以回显调用方已提供的 current tenant/accountSn，但不得包含 foreign tenantId、owner、state、currency、accountType、profile、context 或能力名称。分页必须保持统一 `WindPagination.empty` 语义，不返回 null。

#### 20.50.3 Provider Green 与 Capte Consumer 阶段

Provider Green 精确 `MODIFY=4 / ADD=0 / DELETE=0`：3 production 为 `FundingAccountService.java`、`FundingAccountQuery.java`、`FundingAccountServiceImpl.java`，1 existing test 为 `FundingAccountServiceImplTests.java`。production 只删除旧方法/import、添加 Jakarta `@NotNull` 和 Service 首次 tenant guard；MyBatis-Flex 继续使用 `FundingAccountNameRefs.tenantId`，不新增裸字符串或 `LambdaQueryWrapper`。既有测试两处 raw-id 回读机械切换到 `(TENANT_ID, ACCOUNT_SN)`，其余 10 个 testcase、fixture、断言和金额事实不变。

Capte 必须在 Provider hard cut 后以独立 Consumer Grant 修改 `tests/src/test/java/com/capte/marketing/coupon/CouponActivityServiceImplTests.java`（plan-r2.317 重冻 SHA=`d263b5fbad5344f43f82a3295d8d0911c0e268835c765bf3fdefef0fe9604283`），只删除两个旧 interface override；保留真实生产的 tenant+sn/query contract 和 37 个测试语义。`CouponRedemptionApplicationServiceImplTests` 49 项、`OrderCouponRedemptionIntegrationTests` 25 项只读扩大验证。未完成 exact Provider artifact -> Capte resolved artifact 谱系与上述三类测试前，不形成跨仓 E4 或发布准出。旧 `4d3e7239...` 只作历史输入。

#### 20.50.4 验证矩阵、约规与停止线

| 层级 | 未来命令 / 资产 | 目标 |
| --- | --- | --- |
| 环境与编译 | Java 21 `just mvn-version`、`just compile` | Provider 旧两方法源码/classfile=0；3 production 编译闭包成立 |
| RED / Green | `just verify-slice FundingAccountServiceTenantIsolationTests tests` | RED=`3/2F/0E/0S`；Green=`3/0F/0E/0S` |
| Provider focused | 新 tenant test + `FundingAccountServiceImplTests,FundsAccountQueryTenantIsolationTests,PlatformFundingAccountServiceImplTests,SpendSubjectFundingRelationServiceImplTests,AccountHierarchyRelationServiceImplTests` | get/query、null tenant、平台/关系/核心账户查询保持；零资金副作用 |
| 扩大回归 | balance-control / transaction / business-flow / reconciliation / boundary | `44/0 / 186/0 / 200/0 / 247/0 / 211/0` |
| 公共约规 | Public Contract、Wind 目标规则源码检查、Java 21 Core API | `299/175/42` 不变；Jakarta `@NotNull` + Service 显式 guard + NameRefs 成立；Core=`94/4/4/1025` 不变 |
| 规约与全量 | PMD、CAD、fresh XML | 13 份 PMD XML 0 finding；120 份基线加 1 新 XML，总计 `1206/0F/0E/1S` |
| Capte Consumer | compile + `CouponActivityServiceImplTests,CouponRedemptionApplicationServiceImplTests,OrderCouponRedemptionIntegrationTests` | 只删除匿名旧 override；37/49/25 source testcase 均零 failure/error/skip，真实 tenant 使用不变 |

唯一既有 skip 仍只允许无真实 MySQL host 的 migration integration。Wind 约规结论：`FundingAccountQuery` 位于 wallet-face query 包，`tenantId` 使用 `jakarta.validation.constraints.NotNull`；ServiceImpl 显式 guard 承担直接调用的运行时失败语义；无 Entity/Mapper/Page 泄露，无新类型、浅服务或 admin surface。`verify-public-contract-conventions.sh` 当前 SHA=`33579205...` 且保持只读。

本轮全仓 Wind profile 守卫在 `check_redundant_jspecify_checks / METHOD_WITH_BODY.finditer` 长时间无输出后以 exit 130 中断，不计作 PASS，也不作为 Entry Card 阻断；未来按上表对目标三文件做确定性源码检查，并由 RED、Public Contract、PMD/CAD 与独立 CR 交叉验证。若执行轮要把全仓守卫升级为硬门，必须先单独修复或限界该工具，不得修改业务代码迎合非收敛扫描。

本 Entry Card 不授权 RED、Provider Green、Capte Consumer、Git、Maven、联网或发布。第 4 个 Provider production、第二个 wind existing test、第二个 Capte write、生产旧 caller、无可信 tenant、DDL/Mapper/schema/build、Core/Credit/Platform/Hierarchy/其他 raw-id/create/DTO id、兼容层或 RED 非精准 `3/2F/0E/0S` 都必须停止并重冻。

独立 Entry Card Checker 以三文档 Maker SHA=`4d4f26a1... / d706247e... / 929b796d...` 独立复算旧 caller=0、RED=`3/2F/0E/0S` 可达、Provider `3 production + 1 test`、Capte 单一 test write、1206/契约/Core/PMD 基线与 Wind 确定性规则，结论=`PASS / P0=0 / P1=0 / P2=0`。下一 Gate 只能由 Human Owner 单独授予 Provider RED；当前=`ENTRY_CARD_INDEPENDENT_CHECKER_PASS / RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

#### 20.50.5 `plan-r2.319` Funding RED 实证

唯一新增 RED=`FundingAccountServiceTenantIsolationTests.java`，source SHA=`4a2373bd0157f501ec94d521af47b8274eb17d85f3a4f215274a920aed909102`。测试固定 3 个方法并使用真实 Spring/H2/Funding/Ledger/Profile 链路：

| testcase | fresh 结果 | 精确缺口 |
| --- | --- | --- |
| `testFundingAccountServiceShouldExposeOnlyTenantScopedQueryContract` | FAIL | raw-id 与 `FundsAccountId` getter 仍存在，query tenant 缺 `@NotNull`；SoftAssertions 聚合为 1F |
| `testQueryFundingAccountsShouldRejectMissingTenantWithoutFactsMutation` | FAIL | null tenant 未在读取前拒绝；全部账户/资金交易/账务事实保持不变，聚合为 1F |
| `testTenantScopedFundingAccountQueriesShouldNotCrossTenantBoundary` | PASS | 既有 tenant+sn get/query 已同租户正向、foreign fail-closed、不泄露且零写入 |

fresh XML SHA=`3639b99225f459dff06668dd80800b54f925066e2b8a8b86d23a045beb85475e`、mtime=`2026-08-27 12:54:33 +0800`、结果=`3/2F/0E/0S`。`verify-slice` 的 site descriptor/`~/.m2` 写锁和首次 testCompile checked-exception 均没有形成业务 RED；前者无 XML，后者仅在同一 RED 文件把反射结果移出 lambda。最终 Java 21 offline 命令以 `-DskipSurefireReport=true` 跳过 HTML report，JUnit 与 XML 真实执行；production compile 前后=`21/21`。

独立 Checker=`PASS / P0=0 / P1=0 / P2=0`。RED 文件从此 immutable，Green 不得修改测试或放宽断言。当前=`RED_INDEPENDENT_CHECKER_PASS / GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`；下一 Human Gate=`MIG09_FUNDING_ACCOUNT_QUERY_SURFACE_NARROWING_GREEN_EXECUTION_GRANT`，精确 `MODIFY=4 / ADD=0 / DELETE=0`，只允许 `FundingAccountService.java`、`FundingAccountQuery.java`、`FundingAccountServiceImpl.java`、`FundingAccountServiceImplTests.java`。Capte Consumer 与所有扩大回归只读，Core/Credit/Platform/Hierarchy、其他 raw-id、Git、联网、发布和生产继续 `NO`。

#### 20.50.6 `plan-r2.320` Provider Green 验证与价值确认

Green 未修改 immutable `FundingAccountServiceTenantIsolationTests.java`，SHA 仍为 `4a2373bd0157f501ec94d521af47b8274eb17d85f3a4f215274a920aed909102`。两个旧 getter 在 public interface、实现、源码 caller 和 Java 21 classfile 中均为 0；`FundingAccountQuery.tenantId` 使用 Jakarta `@NotNull`，Service 在首次读取前显式拒绝 null tenant，tenant+sn 与 query 都继续使用 `FundingAccountNameRefs`。四个 Green 文件最终 SHA=`f0711e17... / 87aaa1b8... / 320347d4... / 0b084d77...`，与冻结责任一一对应。

| 验证层 | fresh 结果 |
| --- | --- |
| compile / Funding tenant | `21/21` / `3/0F/0E/0S` |
| Funding 六类 focused | `33/0F/0E/0S` |
| balance-control / transaction / business-flow | `44/0 / 186/0 / 200/0` |
| reconciliation / boundary | `247/0 / 211/0` |
| Public Contract / Core API | `299/175/42` / `94 stable / 4 experimental / 4 internal / 1025 lines` |
| PMD / CAD / Surefire | 13 份 PMD XML 无 finding / `verify-cad exit 0` / 121 份 XML=`1206/0F/0E/1S` |

Funding Green XML SHA=`87216d2816726b627b7e947f38b777fd4946485576f8f141cd69da3fccf10019`，mtime 晚于四个 Green 源码；唯一 skip 仍是没有真实 MySQL host 的 `ReconciliationMysqlMigrationIntegrationTests`。独立技术 Checker=`PASS / P0=0 / P1=0 / P2=0`，独立代码 Review=`Ready`，确认 RED 未弱化、无旧 caller、无兼容、无白名单外实现扩张。

业务验收意义是：同租户仍可以按 accountSn 和已受控 query 读取 FundingAccount；缺 tenant 不再可能退化为宽扫描；错 tenant 无法观察账户存在性、owner、state、currency、profile、capability 和相关账务事实；全部拒绝路径零资金、Ledger、Posting 和 Entry 副作用。它证明 Provider 边界自身履行 tenant 授权，不依赖 Consumer 调用自律；不新增产品行为，不上推真实 HOST/L4 或生产 D5。

本次 documentation-only closeout 只回写三份权威文档，不重跑 Maven，不修改 Java、测试、Capte、schema 或 build，不执行 Git 或联网。独立 closeout Checker 以三文档 Maker SHA=`c1277616... / 6e4242bb... / 73e18476...` 复核全部 Green 证据、业务断言、D4 边界、结构校验与下一 Gate，结论=`PASS / P0=0 / P1=0 / P2=0`。当前=`PROVIDER_GREEN_CLOSEOUT_INDEPENDENT_CHECKER_PASS / PROVIDER_CURRENT_SCOPE_COMPLETE / CAPTE_CONSUMER_EXECUTION_GRANT_NO / CODE_FREEZE`；下一 Human Gate=`MIG09_FUNDING_ACCOUNT_QUERY_SURFACE_NARROWING_CAPTE_CONSUMER_EXECUTION_GRANT`，只允许 Capte `CouponActivityServiceImplTests.java` 删除两个旧 interface override，并以 exact Provider artifact 完成 `37/49/25` 测试闭环；不授权 Credit、Platform/Hierarchy、其他 raw-id、Git、联网、发布或生产。

#### 20.50.7 `plan-r2.321` Capte Consumer Green 验证与价值确认

Consumer Green 唯一修改为 Capte `CouponActivityServiceImplTests.java`，SHA=`d263b5fb... -> abee9390...`，diff 仅删除 Funding 匿名 Bean 的两个旧 getter override。保留 create、tenant+sn、tenant-scoped query、Credit Bean 和所有 fixture/断言；source 仍为 37 个 `@Test`，Capte Java 中旧 Funding getter 引用为 0。

Provider 构建输入=`674 files / 07f11958917046275728de48c1968858a36db5c2556d9f4431dd44cce784f7a8`，唯一验证版本=`1.0.1-mig09-funding-20260827-07f119589170`，Java 21/Maven 3.6.3 offline `flatten + install=21/21`。隔离仓为 `/private/tmp/wind-funds-r8b-e4.VTAMGS/repository`，主 `~/.m2` 未写入该版本。旧 Consumer 在显式开启测试编译后精准 RED：169 个 test source 真实进入 javac，只有目标文件两个旧 `@Override` compile error；Green 后相关 reactor test compilation=`42/42`。

| 证据 | fresh 结果 | XML SHA |
| --- | --- | --- |
| Activity | `37/0F/0E/0S` | `3a23f8a1...` |
| Redemption | `49/0F/0E/0S` | `7d01ae2d...` |
| Integration main | `25/0F/0E/3S` | `1fc739bc...` |
| Artifact lineage | `1/0F/0E/0S` | `d4499747...` |

Integration main 的 3S 只是 lineage/seed/recover 分阶段方法，lineage 已单独执行并通过。七个 built/resolved SHA 成对相等：`b6f8a3e9... / 6e15287a... / 51e7ded2... / 72feb259... / 76cb2733... / 804e0d3... / 79218b79...`；lineage 用 `CodeSource` 复核 runtime loaded 路径和版本。生产 Consumer/adapter、两个扩大测试和 root/tests POM 指纹均未漂移。

Harness 诊断与业务证据严格分层：`testCompile` 不是 Maven phase；默认测试跳过会导致 test compilation 假绿；raw installed POM 的 `${revision}` 会阻断 Consumer 解析。最终命令使用 `test-compile`、`-Dmaven.test.skip=false -DskipTests=false`、同版本 flatten install 和 fresh XML，因而没有把旧 class、reactor 摘要或 Harness failure 冒充 Green。

业务价值是关闭 Provider -> artifact -> Consumer 的真实交付链：Capte 生产代码早已使用 tenant-scoped Funding 查询，本轮删除的仅是无调用编译 stub；它证明不需恢复 Provider raw-id/无 tenant 旁路也能保持真实业务行为。唯一版本与 loaded lineage 同时防止用可变 Snapshot 或旧 target class 伪造跨仓兼容。独立代码/证据 Checker=`PASS / P0=0 / P1=0 / P2=0`，结论仅覆盖 library test host `D4/E4`，不表示可部署 HOST/L4、真实 MySQL 或生产 D5。

本 closeout 只修改主系分、主 TDD 与 OpenSpec，不重跑 Maven，不修改 wind-funds/Capte Java、测试、POM、schema 或 build，不执行 Git 或联网。独立 closeout Checker 以 Maker 三文档 SHA=`c25ac83c... / 8e404ed0... / f7af9480...` 复核单文件 diff、Consumer/artifact/test 证据、Harness 诊断、D4/E4 边界与下一 Credit Gate，结论=`PASS / P0=0 / P1=0 / P2=0`。当前=`FUNDING_ACCOUNT_QUERY_SURFACE_GREEN_CLOSEOUT_INDEPENDENT_CHECKER_PASS / CURRENT_SCOPE_COMPLETE / CREDIT_ENTRY_CARD_GRANT_NO / CODE_FREEZE`；下一 Human Gate=`MIG09_CREDIT_ACCOUNT_QUERY_SURFACE_NARROWING_ENTRY_CARD_GRANT`，只允许三份权威文档建立 Credit documentation-only Entry Card，不授权 RED、Java、测试、Consumer、Git、联网、发布或生产。

### 20.51 `W5-MIG09-PUBLIC-READ-SURFACE-CONSUMER-ARTIFACT-CLOSURE-REBASE-001`

#### 20.51.1 验证目标与基线证据

本卡把两项已接受但 current Consumer 未保留的 hard cut 合并成一个可执行闭环：R8B 删除 Benefit facade/request/enum/impl，Transaction query 增加 tenant 并删除旧方法。Provider current source/class SHA=`899a202f... / f5525754...`；Maven-local `1.0.1-SNAPSHOT` face JAR SHA=`8df1ced2...` 仍含两组旧 surface。Capte current source/target 同时依赖两组旧契约，当前没有 fresh Surefire XML。

本卡保护：Benefit 只经 generic direct action + ActionFact；交易查询显式 tenant；Provider source、unique built、Consumer resolved/loaded artifact 同一；既有 Money、原事实、route、Ledger/Balance、幂等与失败无副作用不变。它不新增资金行为或兼容层。

#### 20.51.2 未来七文件写入与 immutable 范围

未来 Consumer Green 精确 `MODIFY=7 / ADD=0 / DELETE=0`：
...
除两个 Wallet 文件外的五个 Coupon production/test 路径复用已接受 `D-R8B-001` 责任：settlement 直接调用 generic pay/referenced refund 并验证 ActionFact；activity readiness 改用 direct service；三个 Coupon 测试恢复 direct/ActionFact/source-boundary 行为。Transaction query 责任叠加在 settlement/current recording/integration 以及 Wallet production/test 上：只使用业务已持有 tenant，不从返回事实或全局唯一 SN 反推。

`CapteOrderConfiguration` verification-only/non-write；`CouponImplContractBoundaryTests`、`CouponFaceContractShapeTests` immutable。Capte POM/schema/config、wind-funds Java/test/POM/schema/build、主 Maven-local JAR 均不可写。`CouponActivityServiceImplTests` 本轮只迁 R8B readiness，不提前删除 Funding/Credit 旧 override；执行 closeout 必须把新 SHA回写 20.50 后才恢复 Funding RED。

#### 20.51.3 可执行 compile RED 与 Green 判据

未来 RED 顺序：

1. 双读冻结 wind-funds source manifest、Capte 七文件/POM/verification-only SHA，并生成由 source fingerprint 派生的唯一 non-Snapshot validation version。
2. Java 21 offline 构建 core、transaction-face/impl、wallet-face/impl、ledger-face/impl 到隔离 Maven local，记录七制品 built SHA；不得复用 r9、旧 Snapshot 或覆盖主 `~/.m2`。
3. Capte 对 exact artifact 执行 compile/testCompile。允许失败类别只有已删除 Benefit facade/request/enum/impl和旧 Transaction query/recording signature；javac 级联诊断不冻结固定数量，也不报告 Surefire RED。
4. 其他源码、POM、schema、Bean、依赖解析或 Harness failure 立即停止，不增加第 8 个写文件。

Green 只迁移七文件，随后验证：

| 层级 | 未来验证 | 必须证明 |
| --- | --- | --- |
| Source contract | 四仓旧 Benefit surface 扫描；Capte 旧 query/invocation 扫描；Provider/Consumer `javap` | 旧正向引用=0；合法 absence/source-boundary token 保留；tenant 首参存在 |
| 编译闭包 | Capte Java 21 offline full reactor compile + testCompile | 七文件对 exact Provider artifact 编译；POM/schema 不改 |
| Application focused | `CouponRedemptionApplicationServiceImplTests,WalletPaymentParticipantTests` fresh | `57/0F/0E/0S`；direct/ActionFact、tenant query、正逆异常保持 |
| Activity | `CouponActivityServiceImplTests` fresh | `37/0F/0E/0S`；generic readiness 与 Funding/Credit fixture 保持 |
| R8B boundary | `CouponImplContractBoundaryTests` fresh | `32/0F/0E/0S`；旧 facade/request/query completion fallback 不回归 |
| Real integration main | `OrderCouponRedemptionIntegrationTests` 默认阶段 fresh | `25/0F/0E/3S`；3S 只允许 lineage/seed/recover 条件方法 |
| Restart phases | 同 integration 的 lineage、seed、recover method-only 分阶段 fresh | 各 `1/0F/0E/0S`；两 JVM/file-H2 恢复与零新增事实保持 |
| Face boundary | `CouponFaceContractShapeTests` fresh | 当前 source testcase 全部 0F/0E；旧 Funds Benefit enum 仅保留负向禁止 token |
| Artifact lineage | built/resolved/loaded 七制品 SHA 对账 | 每项相等；runtime code source 指向隔离仓；version 为唯一 non-Snapshot validation version |
| Provider baseline | source manifest 未漂移时回读 CAD/PMD/Public/Core；漂移时重跑 | 不复用旧 Provider 绿灯或旧 Maven-local JAR |

当前已接受 source test 基线为 application=`57`、activity=`37`、boundary=`32`、main=`25/3S`、lineage/seed/recover 各 `1`；执行前须重新扫描方法数并解释任何漂移，不得机械刷新下降。所有目标 XML 必须 fresh；除 main 既有 3S 外，其他目标不得 skip 或零执行。

#### 20.51.4 状态、顺序与停止条件

20.51 supersede 20.50 的下一执行顺序和 Capte 单文件 SHA，不改变 Funding Provider `3/2F`、3 production + 1 wind test 文件卡。七文件 Consumer/Artifact Closure 与独立 Checker PASS 后，closeout 机械重冻 `CouponActivityServiceImplTests` 新 SHA，再恢复 Funding RED；Credit、Platform/Hierarchy、其他 raw-id、`entrySide`、projection/context、lifecycle internalization 和 `InternalAccountRef` 继续分卡。

首轮独立 Checker=`NOT PASS / P0=0 / P1=3 / P2=0`：发现遗漏 Activity production/test、query-only RED 无法覆盖 current Capte 仍依赖的已删除 R8B surface，以及 OpenSpec MIG-09 当前表陈旧。本返工只把白名单重冻为七文件并集、恢复两组已接受行为和同步权威表；未改代码、测试或 Funding 合同。

最终独立 recheck=`PASS / P0=0 / P1=0 / P2=0`，确认七文件 union、两组 compile failure family、tenant 来源、application=`57`、activity=`37`、boundary=`32`、main=`25/3S`、lineage/seed/recover 各 `1`、unique non-Snapshot 七制品和 Funding 新 SHA 重冻顺序一致。

当前=`DOCUMENTATION_REBASE_INDEPENDENT_CHECKER_PASS / CAPTE_CONSUMER_EXECUTION_GRANT_NO / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`。本 PASS 不授权 Maven/install、Capte/Provider 写入、Git、联网、远端发布、部署或生产；第 8 个写文件、compatibility、缺可信 tenant、新业务语义或无法建立七制品谱系均立即停止。

### 20.52 `W5-MIG09-CAPTE-R8B-TRANSACTION-QUERY-CONSUMER-ARTIFACT-CLOSURE-ENTRY-CARD-REWORK-001`

#### 20.52.1 为什么七文件 Gate 不可执行

执行前只读复核确认两项白名单外 drift：Capte 正式优惠券系分仍要求旧 Benefit service，current `CouponImplContractBoundaryTests` 也把旧 settle/refund 当 required terms。原七文件即使编译转绿，也会留下错误接入权威，并使目标 boundary=`32/0` 不可达。独立 Checker=`NOT PASS / P0=0 / P1=2 / P2=0`；当前 Execution Gate 未消费，未修改文件、未运行 Maven/Git。

整体最小闭包为 9 文件，但 TDD 顺序固定为 `1 authority doc -> 1 boundary RED -> 8 code/test Green`，不把设计、守卫和实现同时改绿。

#### 20.52.2 Authority Doc Restore 与 boundary RED

第一 Gate 只允许修改：

```text
docs/系分设计/通用优惠券-系分设计.md
```

当前 SHA=`8576f7d6760600b92c1ced59fdeac58f6566d2e5270da5c9668dfa89eaf1e666`。完整目标 hunk 固定为：

- 准入/参数/业务键：成本与受益从 `costBearerSubjectRef/benefitReceiverSubjectRef` 上送改为 `FundsAccountId + 显式账目 + Money + business identity/original ref`；多出资方仍按 `activityFundingId` 独立业务键。
- 责任边界：`fundingNature`、营销主体、适用商户、原订单、退款资格和 context 留在 Capte，不进入 Funds request/digest。
- 执行/完成：Consumer adapter 调用 `FundsDirectTransactionService.pay/referenced refund`，只以 `FundsActionFact` 判定 primary/recovery 完成，不用 `queryFundsTransaction/findFundsTransactionByBusiness` root presence fallback。
- 文档位置：同步当前系分 315-333、418/421、814-817、823-828 的准入表、对象表、交易事实边界和生产编码要求；不改其他产品/数据/状态设计。

验证矩阵：

| 层级 | 未来验证 | 目标 |
| --- | --- | --- |
| 文档结构 | Capte system-design validator、链接、围栏、尾空白 | PASS；只一份正式系分写入 |
| Authority scan | Capte 当前产品/系分/任务文档扫描 | 正向旧 facade=0；产品设计和 AI 基线 non-write |
| Focused RED | `CouponImplContractBoundaryTests`，仅在该 Gate 明确允许 offline Maven 后执行 | `32` 个 source tests 中只允许 `testBenefitFundingDeliveryDocsKeepSubjectMappingAndImpactModeGates`、`testBenefitFundingDeliveryDocsKeepSubjectMappingDecisionPackage` 失败；实际 failure count 取 fresh XML，必须 `0E/0S` |
| Immutable scope | 九文件其余八项、POM/schema/config、Face boundary | SHA 不变 |

若 focused 命中上述两方法以外的 failure，出现 compile/error/skip，或扫描发现另一份当前 authority 需要修改，停止并重冻；不得放宽 boundary 或把旧文案写回。Gate A closeout 必须记录实际 failure methods/count，并据此重冻 Gate B 的精确 RED 基线。

#### 20.52.3 八文件 Green 与回归

Authority Doc Checker PASS、boundary RED Checker PASS 且实际 failure 基线已回写后，第二 Gate 只允许 8 个 Java/test MODIFY：20.51 的七文件，加 `CouponImplContractBoundaryTests.java`（current SHA=`e7c495031f93a876b248b632bfeb0893f0522070827ba064d209b81281ffe8fb`）。boundary test Green 责任是将两组 required/source assertions迁移到 direct pay/referenced refund + ActionFact + tenant query，并禁止已删除 Benefit surface 与旧主交易完成 fallback；全部非目标方法保持。

| 层级 | Green 目标 |
| --- | --- |
| Source/contract | 九文件整体旧 Benefit 正向引用=0；Capte 旧 query invocation=0；合法 Face negative token 保留 |
| Compile | Java 21 offline full reactor compile/testCompile PASS；不改 POM/schema |
| Application | `57/0F/0E/0S` |
| Activity | `37/0F/0E/0S` |
| Boundary | `32/0F/0E/0S`，其中 Authority Doc RED 转绿 |
| Integration | main=`25/0F/0E/3S`；lineage/seed/recover method-only 各=`1/0F/0E/0S` |
| Face | 当前 source test count 全部 0F/0E；旧 Funds Benefit token 只作负向断言 |
| Artifact | unique non-Snapshot version；core、transaction-face/impl、wallet-face/impl、ledger-face/impl built/resolved/loaded SHA 逐项一致 |

Provider source manifest 未漂移可复核既有 `1203/0F/0E/1S`、PMD/Core/Public Contract；漂移则重跑对应门禁。公共库唯一 MySQL skip 不阻断 Consumer closure。所有目标 XML 必须 fresh，除 main 既有 3S 外不得 skip。

#### 20.52.4 状态与下一 Gate

整体闭包完成后先机械重冻 `CouponActivityServiceImplTests` 新 SHA，再恢复 Funding `3/2F` RED；不提前删除其 Funding/Credit override。最终独立 recheck=`PASS / P0=0 / P1=0 / P2=0`，确认完整 authority hunk、动态 RED、`1+8` 文件和验证矩阵成立。

当前=`AUTHORITY_DOC_RESTORE_AND_BOUNDARY_RED_COMPLETE / AUTHORITY_BOUNDARY_RED_INDEPENDENT_CHECKER_PASS / AUTHORITY_CLOSEOUT_INDEPENDENT_CHECKER_PASS / SCHEMA_ENTRY_CARD_REWORK_INDEPENDENT_CHECKER_PASS / GREEN_REWORK_EXECUTION_GRANT_NO / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

历史记录：第一 Gate 当时按 Human Owner 授权写入 Capte 单文件 authority 并执行 focused offline Maven；当时下一 Human Gate 为独立八文件 Consumer/Artifact Closure Green Execution Grant。该 Gate 已消费并由 20.52.5 至 20.52.7 的后继证据接替。

#### 20.52.5 Authority boundary RED 实际基线

| 证据 | 实际结果 | TDD 定性 |
| --- | --- | --- |
| Capte authority doc | post SHA=`9ba8749e79ce74139cb8bf2484ad70f9b15aa32c97eddf06d6718a0b54bef740`；system-design、链接、围栏、尾空白 PASS | 设计先行完成；业务责任仍由 Capte 持有，Funds 只执行 generic direct action 并返回 ActionFact |
| Immutable test/build | boundary test=`e7c495031f93a876b248b632bfeb0893f0522070827ba064d209b81281ffe8fb`；root/tests POM=`6655fa65... / b3ecc50f...` | 未通过改测试、POM 或跳过用例制造 RED |
| Fresh XML | mtime=`2026-08-27 09:07:48 +0800`；SHA=`0e35facd0ed9db0a8497aa0b8b1b53d6473bde33b747efdb4b9d15a894b1fa6d`；`29/2F/0E/0S` | 实际运行方法数是 29，不沿用 source 静态计数 32 |
| Failure closure | 仅 `testBenefitFundingDeliveryDocsKeepSubjectMappingAndImpactModeGates`、`testBenefitFundingDeliveryDocsKeepSubjectMappingDecisionPackage` | 两项均因旧 `costBearerSubjectRef` required-term 失败；其余 27 项通过，形成精确受控 RED |
| Runner caveat | 42-module Maven reactor 报 `BUILD SUCCESS` | 生命周期未把 focused failure 传播为 reactor failure；准出只认 fresh Surefire XML |
| Independent Checker | `AUTHORITY_BOUNDARY_RED PASS / P0=0 / P1=0 / P2=0` | 可冻结为未来八文件 Green 的 immutable acceptance evidence；不授权 Green |

历史八文件卡当时不得修改 authority doc，只允许 20.52.3 的八个 Java/test 文件；该白名单已因 schema drift 停止，并由 20.52.7 supersede，不再构成当前 Green 权限或计数来源。

三文档 closeout 首轮只发现两处现行 MIG-09 汇总仍停在 Authority Restore 前，最小机械修正后独立 recheck=`PASS / P0=0 / P1=0 / P2=0`。本 PASS 只准出下一 Human Gate=`MIG09_CAPTE_R8B_AND_TRANSACTION_QUERY_CONSUMER_ARTIFACT_CLOSURE_EXECUTION_GRANT`，不构成该 Gate 的执行授权。

Human Owner 现已单独授权该八文件 Gate。执行版本=`1.0.1-mig09-capte-20260827-298111e715f8`、Provider source fingerprint=`298111e715f8c7bdef45a12a475590ab7abfba8461b3c7e9f27807d9c09093d1`；Java 21 offline Provider flatten+install=`21/21`。Capte `clean compile` 的真实 RED 只命中两组冻结 failure family：Benefit facade/request/enum absence 与 Transaction query tenant hard cut。非 clean compile 的旧 class 假绿和 raw installed POM `${revision}` 解析失败均只作 Harness 诊断，不计 RED/Green。下一步只迁移 20.52.3 八文件并按 20.51.3/20.52.5 读取 fresh XML；测试、制品谱系与独立 Checker 未完成前仍不得写 `GREEN_PASS`。

#### 20.52.6 八文件执行证据与 10 文件 Entry Card 重做要求

八文件已完成最小实现且未越界；实际验证如下：

| 验证层 | 结果 | 定性 |
| --- | --- | --- |
| Provider artifact | Java 21 offline flatten+install `21/21`；version=`1.0.1-mig09-capte-20260827-298111e715f8` | 唯一制品构建有效；不等于 Consumer Green |
| Capte production compile | coupon/order 聚焦 `10/10` | direct service、ActionFact、显式 tenant 查询可编译 |
| Focused tests | Wallet `8` + Activity `37` + Boundary `29`=`74/0F/0E/0S` | 旧 Benefit surface 与无 tenant query 未回归；原 Entry Card 的 boundary `32` 已陈旧 |
| Coupon application | fresh XML `49/0F/14E/0S`，SHA=`1bb2b04d...` | 1 项 tenant fixture 已在授权测试内修复但未 fresh 重跑；13 项先命中 `t_funding_account.STATE` 缺失 |
| Independent Checker | `NOT PASS / P0=0 / P1=2 / P2=1` | Green 不准出；必须先重做 Entry Card |

阻断是测试宿主的生命周期列契约漂移：Provider 六张实体/查询已使用 `state`，Capte H2 schema 仍用 `status`。禁止增加双列、alias、兼容视图、测试内 `ALTER`、mock Provider 或跳过 Spring/H2；这些方案会让宿主在错误 schema 上假绿。精确重冻结候选为原八文件加：
...
未来 Green 必须在 `MODIFY=10 / ADD=0 / DELETE=0` 内完成：schema 六表生命周期列及索引 `status -> state`；host schema expected column `STATUS -> STATE`；既有 `OrderCouponRedemptionIntegrationTests` 的 5 处 raw SQL 同步为 `state`。前八文件当前修改保持，不重写业务设计；新增两文件无其他表结构扩张。重冻结时同时以实际 source/fresh XML 校准 application=`49`、activity=`37`、boundary=`29`、Wallet=`8`，再冻结 integration main/lineage/seed/recover 与七制品验证，禁止机械沿用陈旧 `57/32`。

plan-r2.312 历史停止记录：当时下一 Gate=`MIG09_CAPTE_R8B_AND_TRANSACTION_QUERY_CONSUMER_ARTIFACT_CLOSURE_ENTRY_CARD_REWORK_GRANT`。该 Gate 已消费并由 20.52.7 supersede；Funding RED、Git、联网、远端发布、部署和生产继续 `NO`。plan-r2.312 状态回写独立 Checker=`PASS / P0=0 / P1=0 / P2=0`，只确认当时 `49/14E` 分类、10 文件最小闭包、无兼容和授权状态准确。

#### 20.52.7 `plan-r2.313` 可执行 Green 卡

本轮 Human Grant 只允许重做 Entry Card，不运行 Maven，不修改 Capte 文件。新卡把“累计闭包”和“剩余写入”分离：累计 `10` 文件，下一 Green `MODIFY=3 / IMMUTABLE=7 / ADD=0 / DELETE=0`，路径、SHA 与职责以 11.45.8 为准。任何 immutable SHA 漂移都使 Green Gate 失效。

三个剩余文件的行为验收固定如下：

| 文件 | 精确 Green | 可执行守卫 |
| --- | --- | --- |
| `jdbc-schema.sql` | 六表列 `status -> state`；`idx_funding_account_status`、`idx_credit_account_status`、`idx_funds_transaction_status`、`idx_funds_transaction_detail_status`、`idx_funds_frozen_order_status` 改为对应 `*_state`；expire index 第二列改为 `state` | 六表声明中旧生命周期列和上述旧索引名为 0；不改其他表/列/default/comment/UK。 |
| `FundsProviderHostSchemaTests.java` | Ledger expected `STATUS -> STATE`；在现有 `testHostSchemaSupportsCurrentFundsProviderPersistence` 内检查六表含 `STATE` 且不含 `STATUS` | class 仍 `1/0F/0E/0S`；不新增测试类、依赖或测试内 DDL。 |
| `OrderCouponRedemptionIntegrationTests.java` | 第 1060、1091、1098、1140、1262 行附近 5 处 raw SQL 使用 `state` | source 仍 25 个 `@Test`；lineage/seed/recover 三方法和所有 Money、ActionFact、transaction/detail/Ledger/posting/entry/Balance/零副作用断言不变。 |

验证使用 Java 21、offline、现有隔离仓 `/private/tmp/wind-funds-r8b-e4.VTAMGS/repository` 与唯一版本 `1.0.1-mig09-capte-20260827-298111e715f8`。正式命令在 `capte-domain` 根执行：

```bash
JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home mvn -o -Dmaven.repo.local=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dwind-funds.version=1.0.1-mig09-capte-20260827-298111e715f8 clean compile testCompile
JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home mvn -o -Dmaven.repo.local=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dwind-funds.version=1.0.1-mig09-capte-20260827-298111e715f8 -Dwind-funds.repository=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -pl tests -am test -Dtest=FundsProviderHostSchemaTests,CouponRedemptionApplicationServiceImplTests,CouponActivityServiceImplTests,WalletPaymentParticipantTests,CouponImplContractBoundaryTests,OrderCouponRedemptionIntegrationTests -Dsurefire.failIfNoSpecifiedTests=false
JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home mvn -o -Dmaven.repo.local=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dwind-funds.version=1.0.1-mig09-capte-20260827-298111e715f8 -Dwind-funds.repository=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dcapte.l3.phase=lineage -pl tests -am test '-Dtest=OrderCouponRedemptionIntegrationTests#testCurrentProviderArtifactLineageAndAssembly' -Dsurefire.failIfNoSpecifiedTests=false
JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home mvn -o -Dmaven.repo.local=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dwind-funds.version=1.0.1-mig09-capte-20260827-298111e715f8 -Dwind-funds.repository=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dcapte.l3.phase=seed '-Dcapte.l3.h2.url=jdbc:h2:file:/private/tmp/capte-r8b-state-recovery;MODE=MySQL' -Dcapte.l3.schema-init=always -pl tests -am test '-Dtest=OrderCouponRedemptionIntegrationTests#testSeedTrustedGoodsOrderSpecifiedCouponForContextRebuild' -Dsurefire.failIfNoSpecifiedTests=false
JAVA_HOME=/Users/wuxp/Library/Java/JavaVirtualMachines/corretto-21.0.11/Contents/Home mvn -o -Dmaven.repo.local=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dwind-funds.version=1.0.1-mig09-capte-20260827-298111e715f8 -Dwind-funds.repository=/private/tmp/wind-funds-r8b-e4.VTAMGS/repository -Dcapte.l3.phase=recover '-Dcapte.l3.h2.url=jdbc:h2:file:/private/tmp/capte-r8b-state-recovery;MODE=MySQL' -Dcapte.l3.schema-init=never -pl tests -am test '-Dtest=OrderCouponRedemptionIntegrationTests#testRebuildQueriesOriginalBenefitActionWithoutNewAttempt' -Dsurefire.failIfNoSpecifiedTests=false
```

fresh XML 必须逐类为：host schema=`1/0F/0E/0S`、application=`49/0F/0E/0S`、activity=`37/0F/0E/0S`、wallet=`8/0F/0E/0S`、boundary=`29/0F/0E/0S`、integration main=`25/0F/0E/3S`，合计=`149/0F/0E/3S`。3S 只允许 `testCurrentProviderArtifactLineageAndAssembly`、`testSeedTrustedGoodsOrderSpecifiedCouponForContextRebuild`、`testRebuildQueriesOriginalBenefitActionWithoutNewAttempt`；上面三条 method-only 命令必须分别 fresh=`1/0F/0E/0S`。seed/recover 使用同一 file-H2、seed `schema-init=always`、recover `schema-init=never`，证明跨进程恢复零新增 transaction/detail/Ledger/posting/entry/refund facts。

在 `wind-funds` 根执行以下精确 target/resolved 对账；每行两个 SHA 必须相等，并分别保持当前已观察值。lineage method-only 再证明 runtime loaded JAR 路径/SHA 等于 resolved JAR：

```bash
shasum -a 256 core/target/wind-funds-core-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-core/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-core-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 transaction/face/target/wind-funds-transaction-face-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-transaction-face/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-transaction-face-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 transaction/impl/target/wind-funds-transaction-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-transaction-impl/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-transaction-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 wallet/face/target/wind-funds-wallet-face-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-wallet-face/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-wallet-face-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 wallet/impl/target/wind-funds-wallet-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-wallet-impl/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-wallet-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 ledger/face/target/wind-funds-ledger-face-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-ledger-face/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-ledger-face-1.0.1-mig09-capte-20260827-298111e715f8.jar
shasum -a 256 ledger/impl/target/wind-funds-ledger-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar /private/tmp/wind-funds-r8b-e4.VTAMGS/repository/com/wind/funds/wind-funds-ledger-impl/1.0.1-mig09-capte-20260827-298111e715f8/wind-funds-ledger-impl-1.0.1-mig09-capte-20260827-298111e715f8.jar
```

当前七组相等 SHA 依次为 `887fc17d... / 6ee0399f... / 79e7c3ee... / 56dd9c17... / 2a7c83c8... / 0fbdb614... / 774a061f...`；未来任一变化都必须由同一 Provider source fingerprint 的重新构建解释，不得机械刷新。

验证还必须证明：七个 Provider artifact built/resolved/loaded SHA 逐项相等；旧 Benefit 正向引用和无 tenant query invocation 为 0；六表旧 `status` 生命周期列、host `STATUS` 和 integration 五处旧 SQL 为 0；PMD/文档 Harness/尾空白通过。Maven reactor 的摘要不能覆盖 fresh Surefire XML；任何非目标 failure/error、test count 下降、3S 扩大、制品路径/SHA 不等、隔离仓缺失或 Provider source fingerprint 漂移都停止并返回 Owner。

20.52.7 supersede 20.52.3 至 20.52.6 的当前白名单、计数与下一 Gate；这些旧段只保留历史。首轮 Checker=`NOT PASS / P0=0 / P1=3 / P2=1`，返工只关闭计数、精确 phase/artifact 命令、陈旧 Gate 和 manifest 顺序四项；最终 recheck=`PASS / P0=0 / P1=0 / P2=0`。下一 `MIG09_CAPTE_R8B_AND_TRANSACTION_QUERY_CONSUMER_ARTIFACT_CLOSURE_GREEN_REWORK_EXECUTION_GRANT` 保持 `NO`，只允许未来由 Human Owner 单独授权上述 3 文件，不授权 7 个 immutable 文件、Funding RED、Git、联网、远端发布、部署或生产。当前=`ENTRY_CARD_REWORK_INDEPENDENT_CHECKER_PASS / GREEN_REWORK_EXECUTION_GRANT_NO / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`。

#### 20.52.8 `plan-r2.314` Green 证据与 PMD Gate 返工入口

Human Owner 已授权 20.52.7 的三文件 Green。实际 post SHA 为：schema=`1ba05e12ced857ec4ea97f2c42240f56b0778c85d2e7007ae6e117dd860fa107`、host contract=`9cdaa54b4fa875fe9ce1b93bfd2cb692791cc1578c9006bea5ad537684da6e28`、integration=`02a69b1a2a888081555484da6a958ee5796f126c831c2e7108db1f35277e37de`；7 个 immutable SHA 保持且没有白名单外写入。旧 schema + 新 host contract 的 fresh RED=`1/1F/0E/0S`，唯一 failure 是首张目标表缺 `STATE`，不是 Spring/H2 Harness error。

Green fresh 证据：full reactor compile/test compile=`45/45`；host/application/activity/wallet/boundary/integration default 分别=`1/0/0/0`、`49/0/0/0`、`37/0/0/0`、`8/0/0/0`、`29/0/0/0`、`25/0/0/3`，合计=`149/0F/0E/3S`；lineage、file-H2 seed、file-H2 recover 分别=`1/0F/0E/0S`。integration 的 capability-rejection fixture 只把 payer account 对齐到既有 request helper 实际使用的 `FA-BENEFIT-COST-L3`，仍真实到达 RECEIVE capability reject，并保留 failed ActionFact、零 transaction/detail 成功效果、零 Ledger/posting/entry 和零 Balance delta。七制品 target/resolved SHA 逐项相等，lineage 进一步证明 runtime loaded SHA 一致。

20.52.7 的 Maven 命令有两个已证实缺陷：phase `testCompile` 不存在，必须使用 `test-compile`；父 POM 默认跳过测试，所有真实 test 命令必须显式加入 `-Dmaven.test.skip=false`。上述计数和 XML 都来自纠正后的命令；没有 fresh XML 的 reactor `BUILD SUCCESS` 不计测试证据。

PMD 准出未成立。全仓 `pmd:check` 在本切片未修改的 `web-security` 四文件报告 6 个既有 violation，PMD XML SHA=`3690f4b9f315f1b505a79ab0256667cf066bbbfab465e617c7ef2f14edb57a33`；tests module 的 PMD 运行提示 `No files to analyze`，不能证明三个已改测试宿主文件通过 PMD。独立 Green Checker 因此裁决=`NOT PASS / P0=0 / P1=1 / P2=0`，其余白名单、immutable、RED、Green、资金断言、重启恢复和制品谱系均通过。

当前不得写 `GREEN_PASS` 或恢复 Funding RED。下一文档 Gate 只能校准 PMD 验收：冻结当前无关全仓 baseline red 及报告 SHA，定义对已改文件可执行的确定性源码守卫、`test-compile`、fresh XML 与 no-new-violation 证据；不得借此修改 `web-security`、降低资金行为断言或扩展 Capte 白名单。当前=`GREEN_REWORK_EXECUTION_COMPLETE / GREEN_INDEPENDENT_CHECKER_NOT_PASS / PMD_GATE_ENTRY_CARD_REWORK_REQUIRED / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`；下一 Human Gate=`MIG09_CAPTE_R8B_AND_TRANSACTION_QUERY_CONSUMER_ARTIFACT_CLOSURE_PMD_GATE_REWORK_GRANT`。

#### 20.52.9 `plan-r2.315` PMD Gate 可执行验收

本轮为 documentation-only PMD Gate rework，不重新执行 Maven，也不修改 Capte。PMD 验收按实际覆盖面分为三层：

| 层级 | 可执行证据 | 准出条件 |
| --- | --- | --- |
| 本切片 production | `marketing/coupon-impl/target/pmd.xml`、`order/order-impl/target/pmd.xml` | 报告晚于 `CouponBenefitFundingSettlement`、`CouponActivityServiceImpl`、`WalletPaymentParticipant`；两个 XML 的 violation count 都是 0 |
| 全仓无关基线 | `web-security/target/pmd.xml` | 根 `pmd:check` 允许非零退出，但只能得到 20.52.9 冻结的 4 文件/6 finding；不得写全仓 PMD PASS |
| test/schema 写入 | compile/test-compile、fresh Surefire、host schema contract、source guard、独立 Checker | PMD=`NOT_APPLICABLE_TO_CHANGED_TEST_FILES`；tests XML 的 `0 file/0 violation` 不计 PASS |

结构化检查固定使用 `xmllint`，不按报告文本猜测：

```bash
xmllint --xpath 'count(/*[local-name()="pmd"]/*[local-name()="file"]/*[local-name()="violation"])' marketing/coupon-impl/target/pmd.xml
xmllint --xpath 'count(/*[local-name()="pmd"]/*[local-name()="file"]/*[local-name()="violation"])' order/order-impl/target/pmd.xml
xmllint --xpath 'count(/*[local-name()="pmd"]/*[local-name()="file"])' web-security/target/pmd.xml
xmllint --xpath 'count(/*[local-name()="pmd"]/*[local-name()="file"]/*[local-name()="violation"])' web-security/target/pmd.xml
xmllint --xpath 'count(/*[local-name()="pmd"]/*[local-name()="file"])' tests/target/pmd.xml
```

当前目标依次为 `0 / 0 / 4 / 6 / 0`。`web-security` 六个 semantic tuple 精确为：`ApplicationJwtSecurityContextRepository` 的 line 1 `TooManyStaticImports` 与 line 29/30 `UnnecessaryImport`，以及 `MultiFactorAuthenticationFilter`、`TenantContextFilter`、`AuthenticationUserUtils` 各一个 line 1 `TooManyStaticImports`。当前报告 SHA=`3690f4b9...` 只作本次 artifact identity；未来重跑按 tuple 比较，避免 timestamp 造成字节 SHA 假漂移。

test/schema 的替代证据不新增门禁，只复用本轮已 fresh 的完整闭包：

1. Java 21/Maven 3.6.3 使用有效 phase `test-compile` 并显式 `-Dmaven.test.skip=false`，full reactor=`45/45`。
2. 六类 default=`149/0F/0E/3S`，且 3S 只为三个条件阶段；lineage/seed/recover method-only 各=`1/0F/0E/0S`。
3. host contract 证明六表有 `STATE`、无 `STATUS`；integration 真实进入 RECEIVE capability reject、failed ActionFact 与 transaction/detail/Ledger/posting/entry/Balance 零成功副作用。
4. source guard 证明六表旧 lifecycle `status`、五个旧 status index、host `STATUS`、integration 五处旧 SQL、旧 Benefit 正向调用和无 tenant query invocation 均为 0。
5. 三个 post SHA 与七个 immutable SHA 必须保持 plan-r2.314 的值；任一漂移使本卡 stale。

本卡 supersede 20.52.7 对“PMD/文档 Harness/尾空白通过”的宽泛表述：只有相关 production PMD 为 0；全仓为 `FULL_REPO_BASELINE_RED_UNRELATED`；test/schema 为 `NOT_APPLICABLE_TO_CHANGED_TEST_FILES`。这不是豁免新问题，而是把每种文件交给实际能观察它的验证器。当前=`PMD_GATE_REWORK_MAKER_COMPLETE / INDEPENDENT_CHECKER_PENDING / GREEN_CLOSEOUT_PENDING / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`；Checker PASS 前不得把既有行为 Green 升级为最终 Green PASS。

独立 Checker=`PASS / P0=0 / P1=0 / P2=0`，确认 changed-production PMD、full-repo baseline red 和 test/schema 替代证据三层门禁完整且可执行。该结论足以进入后续机械 Green closeout，但本轮不直接写 `CURRENT_SCOPE_COMPLETE`，也不恢复 Funding RED。当前=`PMD_GATE_REWORK_INDEPENDENT_CHECKER_PASS / GREEN_CLOSEOUT_GRANT_NO / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`；下一 Human Gate=`MIG09_CAPTE_R8B_AND_TRANSACTION_QUERY_CONSUMER_ARTIFACT_CLOSURE_GREEN_CLOSEOUT_GRANT`，只允许主系分、主 TDD、主 OpenSpec 状态收口，并把 Funding Consumer 的 `CouponActivityServiceImplTests.java` 重冻为当前 SHA=`d263b5fbad5344f43f82a3295d8d0911c0e268835c765bf3fdefef0fe9604283`。Closeout Checker PASS 前不得执行 Funding RED。

#### 20.52.10 `plan-r2.317` Green closeout 验收

本轮不重新运行 Maven，不修改 Capte 或 Funding；只把已经 fresh 且独立核验的 Consumer closure 证据机械收入口径。最终证据矩阵保持：compile/test-compile=`45/45`；default 六类=`149/0F/0E/3S`；lineage/seed/recover 各=`1/0F/0E/0S`；七制品 target/resolved/loaded 一致；相关 production PMD=`0/0`；全仓 PMD=`FULL_REPO_BASELINE_RED_UNRELATED / 4 files / 6 findings`；test/schema PMD=`NOT_APPLICABLE_TO_CHANGED_TEST_FILES`；三个 Green post SHA、七个 immutable SHA 和资金零副作用断言均未漂移。

Funding Entry Card 恢复检查：

| 输入 | 当前结果 | closeout 处理 |
| --- | --- | --- |
| 3 production + 1 wind existing test | SHA=`76ae32ac... / 3d3a4706... / 302b2f57... / 460b42c5...` | 与 plan-r2.301 一致，保持原卡 |
| Provider RED file | `FundingAccountServiceTenantIsolationTests.java` absent | 仍待未来 RED Grant 新增，不在本轮创建 |
| Capte read-only production caller | `CouponActivityServiceImpl.java` SHA=`8ad126aad5d2550953a0cf98e0a691588749743e66df4db12909fa7f3bbce68a` | 替换旧 `f46294bc...`；`CouponBenefitReceiverAccountAdapter.java=d068ae90...` 未漂移 |
| Capte Consumer test | SHA=`d263b5fbad5344f43f82a3295d8d0911c0e268835c765bf3fdefef0fe9604283` | 替换旧 `4d3e7239...`，成为未来独立 Consumer Gate 唯一写前基线 |
| Funding RED contract | `3/2F/0E/0S` | 不变；本轮未运行、未授权 |

closeout 的成功标准不是增加一次测试，而是证明现有 Green 证据、当前 source SHA 与下一 Funding 卡之间无断链。当前=`CONSUMER_ARTIFACT_CLOSURE_GREEN_CLOSEOUT_MAKER_COMPLETE / INDEPENDENT_CHECKER_PENDING / FUNDING_RED_EXECUTION_GRANT_NO / CODE_FREEZE`。独立 Checker 必须确认没有陈旧 active SHA、没有误写 PMD PASS、没有把 closeout 外推为 Funding RED、Git、发布或生产授权；PASS 前不写 `CURRENT_SCOPE_COMPLETE`。

独立 Checker 首轮=`NOT PASS / P0=0 / P1=1 / P2=0`，唯一 P1 是 active read-only `CouponActivityServiceImpl` 仍引用历史 SHA。最小返工将其重冻为 `8ad126aa...`，保留 adapter=`d068ae90...`、Consumer test=`d263b5fb...`；最终 recheck=`PASS / P0=0 / P1=0 / P2=0`。本 Consumer closure 当前=`GREEN_CLOSEOUT_INDEPENDENT_CHECKER_PASS / CURRENT_SCOPE_COMPLETE`。

下一 Human Gate=`MIG09_FUNDING_ACCOUNT_QUERY_SURFACE_NARROWING_RED_EXECUTION_GRANT`。权限只覆盖 `ADD tests/src/test/java/com/wind/funds/wallet/services/impl/FundingAccountServiceTenantIsolationTests.java`，目标 fresh=`3/2F/0E/0S`：contract/annotation 聚合 1F、缺 tenant 未拒绝 1F、既有 tenant-scoped 正向/跨租户隔离 1P，且 `0E/0S`。RED 后必须独立 Checker；四个 Provider Green 文件、Capte Consumer test、其他 MIG-09、Git、联网、发布和生产继续 `NO`。

### 20.53 `W5-MIG09-CREDIT-ACCOUNT-QUERY-SURFACE-NARROWING-ENTRY-CARD-001`

#### 20.53.1 要证明的能力与失败合同

本卡验证的不是“查询方法改名”，而是 Credit Provider 必须把 tenant 作为对象授权不变量：`Long id`、accountSn 和 `FundsAccountId` 只定位账户，不能授权读取 owner、state、currency、account type、账期、ledger profile、context 或由此派生的额度事实。Credit 是额度控制账户而非现金账户，但它仍是可记账资金主体；跨租户读取会暴露信用责任与控制信息，缺 tenant 的宽查询必须在首次 read 前 fail-closed。

目标 Public contract 只保留：
...
旧 `getCreditAccountById(Long)` 与 `getCreditAccount(FundsAccountId)` 必须从 interface、impl、源码调用和 Java 21 classfile 同时归零。`CreditAccountQuery.tenantId` 使用 Jakarta `@NotNull`，ServiceImpl 仍须显式校验 null；不得以 JSpecify、Bean Validation、`TenantContextHolder`、全局唯一 sn 或事后 DTO tenant 比较替代首次读取前的授权约束。

#### 20.53.2 单一 RED 文件与精确用例

未来 RED Grant 只允许：

```text
ADD tests/src/test/java/com/wind/funds/wallet/services/impl/CreditAccountServiceTenantIsolationTests.java
```

固定 `3 @Test / 3 invocations / old fresh=3/2F/0E/0S`：

| Test | 真实输入与路径 | 目标断言 | 旧实现预期 |
| --- | --- | --- | --- |
| Public contract | 反射 `CreditAccountService` 与 `CreditAccountQuery.tenantId` | 两个旧方法 absent；tenant+sn/query present；tenant 有 Jakarta `@NotNull` | 1F：旧方法仍在且 annotation 缺失 |
| missing tenant | 真实 Spring/H2 创建 tenant 1/2 CreditAccount，并由 `LedgerServiceImpl + LedgerProfileCatalog` 初始化 `CREDIT_BASIC` | public query 在读取前拒绝；异常含“租户 ID 不能为空”；全部事实 before/after 相等 | 1F：当前 query 未拒绝 |
| tenant isolation | 同一真实 fixture，以 tenant 1 get/query 本地账户，再查询 tenant 2 accountSn | 同租户正向；foreign get 不存在、query 空；异常不泄露外租户敏感事实；零写副作用 | 1P：既有 tenant 谓词应成立 |

事实快照至少包含 `t_credit_account`、`t_funds_transaction`、`t_funds_transaction_detail` 与 `LedgerFactSnapshot` 覆盖的 Ledger、LedgerTransaction、PostingPlan、Entry；不硬编码只有一个 ledger，必须按真实 `CREDIT_BASIC` 四个 required bucket 建账。异常负断言覆盖 foreign owner、state、currency、accountType、periodType/periodId、profile code/version、context 和 foreign tenant；调用方自己提交的 accountSn 不作为泄露判据。

测试继承 `AbstractFundsServiceTest`，真实装配 `CreditAccountServiceImpl`、`LedgerServiceImpl`、`LedgerProfileCatalog` 和 H2，不 Mock/Fake Service、Mapper 或 tenant。测试只观察 public service contract 与持久事实，不窥探私有 helper、QueryWrapper 或 Mapper invocation。RED Checker PASS 后该文件 immutable，Green 不得修改它。

#### 20.53.3 Green 文件卡与调用迁移

Provider Green 精确为 `3 production + 2 existing test MODIFY / ADD=0 / DELETE=0`：

```text
wallet/face/src/main/java/com/wind/funds/wallet/service/CreditAccountService.java
wallet/face/src/main/java/com/wind/funds/wallet/model/query/CreditAccountQuery.java
wallet/impl/src/main/java/com/wind/funds/wallet/services/impl/CreditAccountServiceImpl.java
tests/src/test/java/com/wind/funds/wallet/services/impl/ControlAccountLedgerInitializationTests.java
tests/src/test/java/com/wind/funds/transaction/application/flow/FundsTransactionFlowTestSupport.java
```
...
| 文件角色 | 允许的最小变化 | 必须保持 |
| --- | --- | --- |
| face service | 删除两个旧 getter 和孤立 import | create、tenant+sn get、query 的名称与返回模型 |
| face query | `tenantId` 加 Jakarta `@NotNull` | 其他筛选字段和 DTO 结构 |
| impl | 删除两个旧实现/import；query 在 `QueryWrapper` 前显式 tenant guard | `CreditAccountNameRefs` tenant+条件查询、转换器、开户与 Ledger 初始化 |
| Control test | 两处 by-id 读取改为 `(TENANT_ID, 既有 accountSn)`，不再为读取保留返回 id | 15 个 testcase 的四 bucket、profile、账期、回滚、并发和零副作用断言 |
| Flow support | `getCreditAccount(accountId)` 改为 `getCreditAccount(TENANT_ID, accountId.id())` | 既有 Credit fixture 到 `InitializeSubjectLedgerRequest` 的 tenant/sn/currency/profile/period 映射及所有子类行为 |

当前 SHA 依次为 `15f59ffe... / 89dfd903... / 424525fe... / 46b53da9... / 235681a2...`；production manifest=`2b4dc5e8...`，existing-test manifest=`c302a920...`。`FundsAccountQueryTenantIsolationTests=300c4901...` 与 `FundingAccountServiceTenantIsolationTests=4a2373bd...` 只读；其他仅装配 Credit Bean 的测试都是 verification-only/non-write。

Capte 在 Provider Green 后另取 Consumer Grant，只修改 `tests/src/test/java/com/capte/marketing/coupon/CouponActivityServiceImplTests.java` 当前 SHA=`abee9390...`，删除两个旧 Credit override；保留 Funding hard cut、Credit tenant+sn/query 和 37 个现有测试。`CouponActivityServiceImpl` 已用 `(tenantId, subjectId)`，不修改生产代码。Consumer 不得通过兼容层或 Snapshot 旧制品先行变绿。

#### 20.53.4 验证矩阵、计数与准出边界

| 层级 | 未来命令 / 证据 | 精确准出 |
| --- | --- | --- |
| preflight | `just mvn-version`、`just compile` | Java 21/Maven 基线与 21/21 compile |
| RED / Green | `just verify-slice CreditAccountServiceTenantIsolationTests tests` | RED=`3/2F/0E/0S`；Green=`3/0F/0E/0S` |
| direct caller | `ControlAccountLedgerInitializationTests` + test compilation | `15/0F/0E/0S`，两个旧调用和 FlowSupport 单参调用归零 |
| tenant regression | `FundsAccountQueryTenantIsolationTests,FundingAccountServiceTenantIsolationTests` | `6/0F/0E/0S`，Core/Funding 已闭合能力不回退 |
| shared support | `just test-transaction` | `186/0F/0E/0S`，覆盖 FlowSupport 子类编译与交易行为 |
| expanded funds | balance-control/business/reconciliation/boundary | `44/200/247/211`，全部 0F/0E |
| contract/static | Public Contract、Core API、PMD、CAD | `299/175/42`、`94/4/4/1025` 不变；13 PMD XML 无 finding；CAD PASS |
| full wind | fresh Surefire XML | 最后接受基线 121 XML=`1206/0F/0E/1S`；新增 Credit test 后 122 XML Green=`1209/0F/0E/1S` |
| Capte Consumer | unique artifact + test-compile + Activity/Redemption/Integration + lineage | `37/49/25`，只保留 integration 既有 3S；method-only lineage 非零通过；七制品 built/resolved/loaded SHA 一致 |

唯一 wind skip 仍只允许无真实 MySQL host 的 migration integration；公共库不自建真实 MySQL host。Capte 默认跳过测试、`testCompile` 错误 phase、raw `${revision}` POM 或旧 target class 都不能作为成功证据；沿用已证明的 `test-compile + -Dmaven.test.skip=false + flatten install + 隔离 Maven local + fresh XML`。

本卡不授权 RED、Green、Capte Consumer、Maven、Git、联网、发布或生产。若出现第 4 个 production、第 3 个 wind existing test、第 2 个 Capte write、第三类 RED failure/error/skip、旧生产 caller、缺 tenant 来源、兼容需求或 Entity/Mapper/schema/POM/build/Core/Platform/Hierarchy/create/DTO id/其他 raw-id 改动，立即返回 Human Owner。

独立 Checker 除主系分一处 stale current-state P2 外，确认调用闭包、真实 H2 RED 可达性、`3/2F/0E/0S`、`3 production + 2 wind tests`、单一 Capte Consumer、计数和无兼容边界均成立；最小状态返工后 recheck=`PASS / P0=0 / P1=0 / P2=0`。当前=`ENTRY_CARD_INDEPENDENT_CHECKER_PASS / RED_EXECUTION_GRANT_NO / GREEN_EXECUTION_GRANT_NO / CAPTE_CONSUMER_EXECUTION_GRANT_NO / CODE_FREEZE`；下一 Human Gate 仅为 `MIG09_CREDIT_ACCOUNT_QUERY_SURFACE_NARROWING_RED_EXECUTION_GRANT`，权限只覆盖 `ADD CreditAccountServiceTenantIsolationTests.java`。

#### 20.53.5 `plan-r2.323` RED 结果与下一 Green 门禁

唯一新增 `CreditAccountServiceTenantIsolationTests.java` SHA=`8c2fe2e6ecad58678afbe09e8728898ad74e5425a3d13c6c2a725963d97c2585`，精确保持 `3 @Test`。写前/后 compile=`21/21`；fresh XML SHA=`74ea8ddbf2610cfff556f61fc325ff7f99c7c702e8ef3c0c8ac86dbb0d43a98d`、mtime=`2026-08-27 15:12:55 +0800`、结果=`3/2F/0E/0S`。

| 用例 | fresh 结果 | 已证明事实 |
| --- | --- | --- |
| Public contract | `FAIL` | 两个无 tenant getter 仍存在，`CreditAccountQuery.tenantId` 尚无 Jakarta `@NotNull`；tenant+sn/query 目标方法存在 |
| missing tenant | `FAIL` | 真实 query 未在 read 前拒绝 null tenant；CreditAccount、Transaction/Detail 与全部 Ledger facts 前后相等 |
| tenant isolation | `PASS` | 同租户 get/query 正向；foreign get 不存在、query 空；owner/description/context/state/currency/type/period/profile/tenant 不泄露；全事实零变化 |

真实装配实际创建两租户 CreditAccount 与每户四个 `CREDIT_BASIC` required Ledger；没有 Mock/Fake、私有 helper 反射、QueryWrapper 或 Mapper 调用次数断言。独立 RED Checker=`PASS / P0=0 / P1=0 / P2=0`，该测试自此 immutable。

下一 Human Gate=`MIG09_CREDIT_ACCOUNT_QUERY_SURFACE_NARROWING_GREEN_EXECUTION_GRANT`，只允许 20.53.3 的 5 个 `MODIFY`：`CreditAccountService.java`、`CreditAccountQuery.java`、`CreditAccountServiceImpl.java`、`ControlAccountLedgerInitializationTests.java`、`FundsTransactionFlowTestSupport.java`。Green 必须把本 RED 转为 `3/0F/0E/0S`，并按 20.53.4 执行扩大回归；不得修改 immutable RED、Capte Consumer、Entity/Mapper/schema/POM/build 或恢复任何兼容入口。当前=`RED_INDEPENDENT_CHECKER_PASS / RED_TEST_IMMUTABLE / GREEN_EXECUTION_GRANT_NO / CODE_FREEZE`。
