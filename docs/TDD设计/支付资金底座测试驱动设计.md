...
当前代码能力基线截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`。TDD 可以把该提交之前已验证的交易 Request 上下文只读、敏感上下文阻断、权益核心字段不得旁路、防御性拷贝和局部服务测试作为回归资产，但不能把这些资产外推为后续资金流、清结算、对账、资金数据治理或 P2 业务专项已完成。
...
| TDD-AUTH-006 | 无授权直接退款 | `settleRefund` 无授权退款模式 | 无前置授权但存在外部原消费、原完成或差错凭证时可直接退款；不得补造授权占用；必须显式 `NO_AUTH` 或等价模式，保留外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者和审计上下文；不得携带或查询内部授权流水。 |
...
| TDD-RED-017A | `settleRefund` 无授权退款缺 `NO_AUTH` 或等价模式、外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者或审计仍入账，或无授权退款携带内部授权流水、查询原授权账本交易、补造授权占用。 | authorization no-auth refund tests。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期、强制完成、授权链退款、无授权退款、拒付原因承接、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 若获授权，首批补 `B4-CB-RED-001A/B`。 | 授权拒绝、完成、释放、退款、过期或拒付承接任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/过期/退款并发红线；拒付不得强制落独立 `chargeback` 公共入口，也不得被压缩为不可区分的普通退款。 |
...
| 授权交易服务 | authorize、reversal、settle、settleRefund、expire 单独验证；`settle` 同时覆盖普通完成和强制完成模式；`settleRefund` 同时覆盖完成后退款、无授权直接退款和争议类退款原因；授权资金责任覆盖资金账户、共享卡 + 资金账户、共享卡 + 预算组 / Spend Rule + 资金账户。 | 授权-撤销-完成-退款、授权部分完成后过期、授权-完成-剩余撤销、完成后退款和费用处理、多次完成、多次退款，均按 8.2 三种账户模型参数化覆盖。 | 授权拒绝无账务、普通完成不触碰 `LIMIT`、预算组只作为共享卡下的预算范围和 Spend Rule 控制维度、预算和信用账户不表达真实资金、过期和撤销终态可区分、强制完成不伪造授权占用；普通授权链退款继续引用原完成事实，无授权退款必须显式 `NO_AUTH` 或等价模式、外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者和审计。 | 撤销超额、完成超额、授权退款超额、授权拒绝误写争议退款、缺原快照、完成后过期不得释放已完成金额、无原消费退款、无授权退款缺模式/原事实类型/原因/凭证/审计、无授权退款携带内部授权流水、强制完成缺策略。 |
