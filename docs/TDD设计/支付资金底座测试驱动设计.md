...
| 清结算与对账 | 设计阶段先 L1/L5；数据库落地后补 L2/L4/L3。 | 可清分明细、清分批次、清分结果快照、清算候选、清算确认、结算锁定、出款前准入、出款结果、出款解释状态、差错核销。 | 清分对象直接入账、缺前置对账清算、出款受理当成功、差错直接改历史分录、缺解释状态仍展示可操作。 | 未覆盖服务级闭环。现有设计矩阵和局部实现不能替代完整清结算、对账或出款生命周期完成证据，仍缺完整出款生命周期、表结构和数据库闭环。 | 目标补 reconciliation/clearing/settlement/payout H2 服务测试、`PayoutPreflightTests`、`PayoutReceiptMismatchTests` 和 `PayoutExplainabilityTests`。 |
