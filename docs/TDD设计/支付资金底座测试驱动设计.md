...
| `DSL-BENEFIT-SNAPSHOT-001` | 权益快照最小合法契约和无权益兼容。 | `TDD-BEN-001`、`TDD-BEN-002`、`TDD-BEN-003`。 | `FundsInstructionSpec`、`FundsBenefitSnapshotSpec`、DSL JSON parser。 | `TDD-BEN-RED-001`、`TDD-BEN-RED-002`。 |
...
| `core` | `FundsBenefitSnapshotSpec`、`FundsBenefitComponentSpec`、`FundsBenefitReferenceSpec`、`FundsBenefitRefundPolicySpec`、权益枚举 | 权益快照结构、金额闭合、组件唯一性、账务效果、资金性质、退款处置和无权益兼容。 | Phase 1 必须补 DSL/契约测试；如只落设计不落代码，必须声明未执行机器契约验收。 |
...
| 用例 ID | 场景 | 核心断言 |
| --- | --- | --- |
| TDD-BEN-001 | 无权益交易兼容 | 不携带 `benefitSnapshot` 的既有直接交易、授权和退款 JSON 仍可解析；`FundsInstruction.amount`、`originalAmount`、`exchangeRate` 语义不变。 |
...
| TDD-BEN-ENTRY-005 | 当前 B1-10 DSL 契约批次 | 无交易服务入口改动 | 在 B1-10 中修改直接交易、授权交易服务方法、Request/DTO、route/posting/replay 或交易主链路。 | B1-10 只验证 `core` DSL 契约、权益 Spec、枚举和 JSON 兼容；服务入口和 Request/DTO 变更必须等待后续批次授权。 |
...
| --- | --- | --- | --- |
| 契约兼容 | 无权益兼容、最小合法快照、金额闭合、组件唯一、闭合角色、稳定摘要。 | 核心字段一等化；`contextVariables` 只放扩展上下文；同业务键不同权益摘要必须冲突。 | B1/B1-10 只证明契约承载和 JSON 兼容。 |
...
| 权益快照 DSL | 是否已有可执行资产，以当前工作区 diff、`FundsBenefitSnapshotSpec` 契约测试、`DSL-BENEFIT-*` JSON 夹具和验证命令为准。 | 金额闭合测试、退款处置测试、无权益兼容测试、稳定摘要测试、缺原权益快照失败测试。 | Phase 1 编码只承诺契约层：无权益兼容、最小合法快照、金额闭合、组件唯一性、退款处置表达和 JSON 夹具解析；商户券 no-ledger、平台补贴 posting-required、授权占券和不退券资金影响进入 Phase 2 对应批次验收。若只改文档，不宣称机器验收已完成。 |
