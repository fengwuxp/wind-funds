...
| `AC-CTRL-*`、`AC-ADJ-001`、`RED-010`、`RED-013` | `TDD-CTRL-*`、`TDD-CTRL-FLOW-*`、`TDD-CTRL-ERR-*`、`TDD-LEDGER-006`、`TDD-RED-006`、`TDD-RED-012`。 | 02-交易路由钱包账目与投影系分设计的余额控制、信用账户、预算组、调账和受控负余额。 | 余额控制与账务主链路。 |
...
| --- | --- | --- | --- |
| `core` | `FundsInstructionSpec`、route spec、ledger spec、`SettlementPolicySpec` | 指令结构、route snapshot、replay、posting、金额临界值、FX 快照和 JSON 契约可解析。 | `FundsInstructionSpecContractTests`、`FundsAmountBoundaryContractTests`、`RouteDslContractTests`、`TransactionServiceAbilityDslJsonContractTests`、`SettlementPolicySpecTests`。 |
...
| TDD-SETTLE-001 | 结算锁定 | `AVAILABLE -> SETTLEMENT`；锁定不等于出款成功；出款中不可再次结算；`SETTLEMENT_LOCK` 即便使用 `ADJUSTMENT` 兼容载体，也不得进入人工调账权限、报表或核销口径。 |
...
资金系统不能只证明顺序执行正确，还要证明同一对象在并发事件下不会双写、重复释放、重复入账或覆盖审计。以下用例在编码阶段可按模块拆到授权、余额控制、清结算、对账和归档专项中，但红线口径必须保持一致。
...
| 领域 | 目标测试资产 | 设计落地要求 |
| --- | --- | --- |
| DSL 契约 | `FundsInstructionSpecContractTests`、`FundsAmountBoundaryContractTests`、`RouteDslContractTests`、`PaymentInstrumentRouteDslContractTests`、`TransactionServiceAbilityDslJsonContractTests`。 | JSON 契约、枚举、金额临界值、route、posting、entry、payment instrument、replay 和指标边界必须保持一致。 |
...
| 钱包账户 | `FundingAccountServiceImplTests`、`CreditAccountServiceImplTests`、`BudgetGroupServiceImplTests`、`PlatformFundingAccountServiceImplTests`、`PaymentInstrumentServiceImplTests`、`SpendSubjectFundingRelationServiceImplTests`、`WalletLayerBoundaryTests`。 | 覆盖账户、支付工具绑定、工具状态和方向、平台账户角色、支出主体资金来源关系、敏感信息脱敏和 wallet 不承载交易命令。 |
| 对账清算 | `ReconciliationTaskServiceTests`、`ReconciliationMatchingRuleTests`、`ReconciliationDifferenceLifecycleTests`、`ClearingSettlementBoundaryTests`。 | 覆盖对账任务、匹配规则、差错闭环、清分前基础对账、清算前置对账、退款时序、批次幂等和重跑不覆盖审计。 |
| 资金数据治理 | `FundsGovernanceTaskBoundaryTests`、`FundsArchiveManifestServiceTests`、`FundsBalanceCheckpointReplayTests`、`FundsProjectionReplayServiceTests`、`FundsGovernanceDifferenceReportTests`。 | 覆盖统一治理流程骨架、归档申请、资金归档 Manifest、余额检查点、水位推进、交易投影重放、差异报告、状态隔离和控制对象互不复用。 |
| 指标和运营边界 | `FundsMetricBoundaryTests`、`FundsOperationPermissionBoundaryTests`、`FundsRailFxBoundaryTests`。 | 指标实现细节由报表指标模块处理；资金底座只验证指标只读、不反写事实、不复用资金域水位或 checkpoint；运营权限和轨道/FX 未完成专项系分前只保留边界测试。 |
...
7. 涉及金额的用例必须从 5.1 中选择相关临界值；涉及同一对象多事件处理的用例必须说明是否需要并发竞争测试。
