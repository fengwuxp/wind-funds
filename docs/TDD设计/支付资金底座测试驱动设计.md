...
| 项目 | 当前值 |
| --- | --- |
| 当前阶段 | `W5-MIG08-CAPTE-BENEFIT-ACTIONFACT-CONSUMER-E4-ASSESSMENT-ENTRY-CARD-001 / E4_BLOCKED_LINEAGE / E4_ASSESSMENT_ENTRY_CARD_INDEPENDENT_CHECKER_PENDING / E4_EXECUTION_GRANT_NO / DOCUMENTATION_ONLY / CODE_FREEZE / plan-r2.260`；Host 行为证据保留，source -> binary 谱系待独立执行 |
| 状态版本 | `plan-r2.260 / 2026-08-25` |
| 权威输入 | [公共能力层产品设计](../产品设计/支付资金公共能力层-产品设计.md)、[公共能力层 DSL 设计](../DSL设计/支付资金公共能力层-DSL设计.md)、[公共能力层系统分析设计](../系分设计/支付资金公共能力层-系分设计.md) |
| 当前授权 | `MIG08_ACTIONFACT_CONSUMER_E4_ASSESSMENT_ENTRY_CARD_GRANT_CONSUMED / E4_EXECUTION_GRANT_NO`；当前不授权构建/install、代码或测试写入、联网、POM、DDL、MIG-09、Git、L4、enable/release/production |
...
该轮测试只证明 Capte library Consumer 能在已解析 `1.0.1-SNAPSHOT` 下使用 ActionFact，并以 seed/recover 两 JVM context 证明零新 transaction/detail/ledger facts；它还缺 Provider source revision 到已发布制品的可审计 lineage，因此不直接记作本切片 E4 PASS。独立 closeout Checker=`PASS / 0 P0 / 0 P1 / 0 P2`；该 closeout 当时只进入 E4 assessment Entry Card，现由 20.43 的 `plan-r2.260` 承接。

### 20.43 `W5-MIG08-CAPTE-BENEFIT-ACTIONFACT-CONSUMER-E4-ASSESSMENT-ENTRY-CARD-001`

#### 20.43.1 评估结论

`ASSEMBLY / SCHEMA_TX / ACTIONFACT_MAIN / UNKNOWN_CONTEXT_REBUILD` 复用 Green closeout 的 fresh 证据；本轮不重跑测试。`LINEAGE` 未通过：本地 `1.0.1-SNAPSHOT` 没有 source revision/build log 绑定，installed impl source/class 与当前 impl 不一致，且 installed impl source 不命中任何 Git revision。评估时两个 workspace target JAR 也与 installed JAR 不同，随后被并发 clean 清除，故只作过程观察。结论固定为 `E4_BLOCKED_LINEAGE`，不得以 sources JAR、mtime、`localCopy=true`、易失 target 或单次 Green 替代 source -> binary 证明。

#### 20.43.2 后续唯一执行卡

| 项目 | 冻结要求 |
| --- | --- |
| 目标 | 只关闭当前 ActionFact Consumer 的 `source -> built -> resolved/loaded` 缺口；不得改变已绿行为 |
| Source | 执行前两次读取两仓 HEAD、default/-uall manifest、staged 与 diff-check；冻结 Provider 完整 dirty fingerprint，构建前后及 Consumer 测试前必须一致 |
| Build | Java 21、Maven 离线、由 Provider HEAD + fingerprint 派生的唯一非 Snapshot 版本；复用已有全 reactor build/install 与七制品 lineage harness，不修改 POM 或构建配置 |
| Artifact | core、transaction-face/impl、wallet-face/impl、ledger-face/impl 的 built/resolved/loaded 路径与 SHA-256 逐项相等；Consumer 必须显式传 `wind-funds.version` 与 repository |
| Tests | 既有 `testCurrentProviderArtifactLineageAndAssembly`、ActionFact 主流程、seed/recover 分阶段 fresh 执行；沿用已冻结业务断言和准确计数，不修改测试以适配结果 |
| Stop | 新文件写入、source/manifest 漂移、共享 Snapshot、缺用例/skip、任一 SHA 不同、需联网或需改生产/测试/POM/schema 时立即停止 |
| Excluded | MySQL、PMD、兼容、Public API/DTO、L4、MIG-09、Git、enable/release/production |

本卡本身不授权 build/install 或测试执行；下一 Human Owner gate=`MIG08_ACTIONFACT_CONSUMER_E4_LINEAGE_EXECUTION_GRANT`。
