...
| `DSL-BALANCE-CONTROL-LIMIT-BUDGET-001` | 信用额度和预算调整。 | `TDD-CTRL-005` 至 `TDD-CTRL-008`、`TDD-CTRL-FLOW-003` 至 `TDD-CTRL-FLOW-005`。 | balance control resolver、credit account service、budget group service、ledger projection。 | `TDD-RED-008`、`TDD-RED-012`。 |
...
| `DSL-SETTLEMENT-RECONCILIATION-ADJUST-001` | 对账差错调账。 | `TDD-RECON-001`、`TDD-RECON-002`、`TDD-CTRL-ERR-005`、`TDD-OPS-001`。 | exception case、approval、evidence、adjust command、ledger posting。 | 无审批调账、直接改历史分录、调账落到余额控制。 |
...
| `AC-AUTH-001` 至 `AC-AUTH-007`、`AC-AUTH-011`、`AC-AUTH-012` | `TDD-AUTH-001` 至 `TDD-AUTH-013`、`TDD-AUTH-FLOW-*`、`TDD-AUTH-ERR-*`、`TDD-ROUTE-003`、`TDD-ROUTE-008`、`TDD-ROUTE-009`。 | 02-交易路由钱包账目与投影系分设计的授权交易状态机、共享卡资金来源和授权回放。 | 授权主链路。 |
| `AC-AUTH-008` 至 `AC-AUTH-010`、`RED-025` 至 `RED-027`、`RED-035` | `TDD-AUTH-EXT-001` 至 `TDD-AUTH-EXT-004`。 | 02-交易路由钱包账目与投影系分设计的发卡授权控制扩展边界。 | 发卡扩展；未启用发卡产品时只做边界测试，不进入默认主链路。 |
| `AC-CTRL-*`、`RED-013` | `TDD-CTRL-*`、`TDD-CTRL-FLOW-*`、`TDD-CTRL-ERR-*`、`TDD-LEDGER-006`、`TDD-RED-006`、`TDD-RED-012`。 | 02-交易路由钱包账目与投影系分设计的余额控制、信用账户、预算组和受控负余额。 | 余额控制与账务主链路；不承接跨主体调账。 |
| `AC-ADJ-001`、`RED-010` | `TDD-RECON-001`、`TDD-RECON-002`、`TDD-CTRL-ERR-005`、`TDD-OPS-001`。 | 03-清结算与对账系分设计的对账差错、调账、审批、凭证、核销和重新对账。 | 调账专项；必须作为直接交易调账或差错调账事实，不落到余额控制。 |
...
| `AC-ARCH-*`、`AC-REPLAY-*`、`RED-016` 至 `RED-019`、`RED-034`、`RED-040` 至 `RED-042` | `TDD-GOV-*`、`TDD-ARCH-*`、`TDD-REPLAY-*`、`TDD-METRIC-004`、`TDD-RACE-008`、`TDD-RACE-010`、`TDD-RED-026` 至 `TDD-RED-028`。 | 04-归档重放与指标治理系分设计的统一治理流程接入、ArchiveManifest、Checkpoint、Watermark、BalanceSnapshotVerifyRef、差异报告和交易投影重放任务。 | 归档重放专项；余额快照只能从账本事实确认，普通指标快照不能替代余额检查点、Manifest 覆盖校验或余额水位推进。 |
...
| `transaction-face` | `FundsDirectTransactionService`、`FundsAuthorizationTransactionService`、`FundsBalanceControlService` | 对外服务方法、请求模型、业务身份、幂等语义和返回流水。 | 当前以 flow 测试覆盖主链路；command service 级测试属于待补目标资产。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、强制完成、授权链退款、无授权退款和发卡控制边界测试。 | 授权拒绝、完成、释放、退款任一语义变化，都必须覆盖三种账户模型和拒绝无 route/entry 红线。 |
| 余额控制 | `FundsBalanceControlFailureFlowTests`、`FundsWithdrawalAfterPartialUnfreezeFlowTests`、`ControlAccountLedgerInitializationTests`。 | 冻结单生命周期、部分冻结、部分解冻、冻结到期释放、信用额度调整、预算调整和 `LIMIT` 红线测试。 | 任何冻结、解冻、额度或预算变更，都必须证明只在同主体同周期 bucket 内控制余额，不表达消费或跨主体价值转移。 |
| 路由回放 | `DefaultRouteReplayServiceTests`、`CompositeRouteResolverTests`、`PaymentInstrumentRouteDslContractTests`。 | 原交易退款、授权撤销、授权完成、授权过期、授权退款、争议类退款和退费的 route replay 专项。 | 逆向、退款、撤销、退费和拒付不得按当前绑定重新选路；快照缺失必须失败。 |
...
| 余额控制服务 | freeze、unfreeze、limit/budget adjust 单独验证。 | 冻结多次解冻、冻结后提现、信用调额后授权、预算调额后授权。 | 冻结只做同主体 `AVAILABLE <-> FROZEN`，`LIMIT` 只能调额触碰，余额控制无 FX；跨主体资金调账不属于余额控制。 | 超额解冻、冻结表达消费、资金调账误落余额控制、错币种余额控制、无策略负余额。 |
...
| 执行授权 | 若进入代码实现，Execution Grant 已说明允许触碰的契约、枚举、请求模型、状态机和表结构范围。 |
