...
| 资金责任解析 | `FundingAllocationDecision` 唯一，最终主体只能是资金账户、信用账户、VCC 关联资金/信用子账户或平台账户角色解析后的平台资金账户；若字段策略选择 `funding-account-only`，最终主体只能是资金账户。 | 资金责任缺失、不唯一、错币种、账户能力不足、缺账户层级快照、支出控制范围或 Spend Rule 被当作最终主体时失败；不得在字段仍只表达 `fundingAccountId` 时声明信用账户、VCC 子账户或平台角色责任主体生产可用。 |
...
授权交易不设计单独的“支出控制范围授权”。支出控制范围是共享卡场景下的预算范围、归属和审计维度，Spend Rule 承接预算额度、MCC、时间、商户、频控等授权前控制；VCC 场景必须先解析到卡绑定的资金子账户或信用子账户，父账户只表达约束、汇总或显式控制。授权单场景按主体组合覆盖：资金账户、共享卡 + 信用子账户、共享卡 + 支出控制范围 / Spend Rule + 信用子账户。
...
| TDD-AUTH-008 | 共享卡 + 信用子账户授权 | `authorize` | 共享卡只作为支付工具和授权快照引用；卡绑定信用子账户 `AVAILABLE -> AUTHORIZATION`；父账户只作为额度池、资金池或汇总约束；不经过支出控制范围 / Spend Rule 控制。 |
| TDD-AUTH-009 | 共享卡 + 支出控制范围 / Spend Rule + 信用子账户授权 | `authorize` | 共享卡挂在支出控制范围下；Spend Rule 至少校验并记录预算额度控制证据；信用子账户承担授权占用，父账户约束和背后资金责任来源可追溯；任一约束失败整体失败。 |
...
8.2 的授权组合场景必须按三种主体模型参数化验证：资金账户、共享卡 + 信用子账户、共享卡 + 支出控制范围 / Spend Rule + 信用子账户。除非某条用例明确写出“不适用”及原因，否则不得只用单一资金账户模型替代组合场景；VCC 模型要断言信用子账户账务占用、父账户约束快照和背后资金责任来源，涉及共享卡时断言工具快照，涉及支出控制范围时断言预算范围和 Spend Rule 控制占用 / 释放证据。
...
| TDD-AUTH-FLOW-002 | 授权批准 -> 部分撤销 -> 剩余过期 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 部分撤销后仅剩余授权继续占用；过期只释放剩余授权；已撤销金额不得再次释放；预算控制释放只更新控制证据。 |
| TDD-AUTH-FLOW-003 | 授权批准 -> 全额完成 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 授权占用后全额完成；授权剩余归零；收款方或清算桶正确增加；支出控制范围不生成 LedgerEntry；父账户汇总不反写分录。 |
...
| TDD-AUTH-FLOW-005 | 授权完成后退款 + 争议裁决结果 + 费用处理 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 普通退款本金通过 `settleRefund` 承接；争议案件只有在裁决需要退款时才进入资金事实，且通过 reason、外部案件引用、裁决结果、投影展示和审计链路区分；用户败诉或无需资金处理时余额和账本事实不变；费用退回走 `refundFee` 或独立退费；退款和退费必须回放原授权模型快照。 |
| TDD-AUTH-FLOW-006 | 授权部分完成 -> 剩余过期 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 完成金额进入收款方或清算桶；仅剩余授权释放回 `AVAILABLE`；共享卡和预算控制模型必须同步释放工具/预算控制占用证据；生命周期关闭。 |
...
| TDD-AUTH-FLOW-008 | 授权完成后多次退款 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 多次退款累计正确；累计退款不超过已完成金额；部分退款后授权状态不得误收口为 `REFUNDED`，仍需保留可继续退款或争议处理的状态和剩余可退金额；三种模型都不得按当前绑定重新选路。 |
| TDD-AUTH-FLOW-009 | 无前置授权强制完成 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 通过 `settle` 强制完成模式按受控策略入账；必须记录 FORCE 模式、受信策略或审批快照、上限、外部原事实引用、凭证、原因、操作者和审计；不得补造授权占用；共享卡和预算控制模型只有在强制完成策略显式允许时才可解析到信用子账户、资金账户、信用账户或平台资金账户并入账，否则必须失败且无账务副作用；支出控制范围和 Spend Rule 不入账。 |
| TDD-AUTH-FLOW-010 | 完成后过期释放请求 | 资金账户；共享卡 + 信用子账户；共享卡 + 支出控制范围 / Spend Rule + 信用子账户 | 已完成金额不得被过期释放；只处理剩余授权，剩余为 0 时不得生成释放分录；共享卡和预算控制模型必须证明工具/Spend Rule 剩余占用不会被重复释放。 |
...
| TDD-WALLET-006 | 支出主体资金责任解析关系 | `SpendSubjectFundingRelationService` | 信用账户、支出控制范围、Spend Rule、工具或使用主体到资金账户、信用账户或平台账户角色的解析关系可查询；服务名可保留历史“FundingRelation”，但断言语义是资金责任解析；不执行扣款、不写分录、不把支出控制范围或 Spend Rule 当资金来源或最终责任主体。 |
...
| TDD-WALLET-010 | 支付工具资金责任决策 | `SpendSubjectFundingRelationService` / route resolver | 多个资金或额度责任主体命中时必须按优先级、默认标识和规则版本确定唯一结果；缺失、不唯一或币种不一致时失败；支出控制范围和 Spend Rule 只能解释命中条件，不能成为决策结果主体。 |
...
| TDD-WALLET-022 | Spend Control Activity 记录和查询 | `SpendControlMovementApplicationService` | 预算预留、可信释放和退款补偿可以记录为控制活动；超时不写控制流水；同一 `tenantId + movementSn` 同摘要重放返回既有活动，不同摘要拒绝；所有路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
| TDD-WALLET-023 | 预算控制投影只读 | `SpendControlMovementService` / budget control projection query | 预算控制投影只能从 Spend Control Movement 聚合 limitAmount、reservedAmount、consumedAmount、releasedAmount、remainingControlAmount、availableControlAmount、lastMovementSn 和 lastMovementAt；查询心智模型是 `controlScopeId + periodId`；同一控制范围跨周期查询必须按 periodId 隔离，传入目标资金账户或信用账户时必须只聚合该账户控制活动，不得混入同控制范围、同 Spend Rule 下其他账户、其他卡或其他周期的活动；不得包含或反写 `AVAILABLE`、`FROZEN`、`AUTHORIZATION` 等账本余额桶，不得声明支出控制范围 ledger、预算余额桶或用 ledger period bucket 替代控制周期生产可用。 |
| TDD-WALLET-024 | 交易消费支出控制活动 | `SpendControlTransactionConsumptionApplicationService` | 交易成功、授权完成、交易失败、交易拒绝、退款或争议退款后，可以回链原控制活动和原资金交易记录消费、释放或补偿型控制活动；超时不是可信完成事实，不写控制流水；交易成功消耗不得复用失败释放语义；退款交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放，非退款交易事实不得被 `refund` 降级为退款控制补偿；退款交易即使引用原资金交易，也必须能找到对应 `CONSUMED` 控制活动后才能记录补偿，只能走对应控制语义；缺原控制活动、缺已消费控制活动、金额超占用、目标账户或币种不一致时 fail-fast；同一活动流水同摘要并发重复写入必须回读既有活动，不得抛出数据库唯一键异常或重复累计投影；交易消费后的预算控制投影按目标账户查询时不得混入其他账户、其他卡或其他周期控制活动；释放类控制活动写入上限必须按目标资金账户或信用账户隔离，历史关联活动必须与原控制活动在业务场景、业务流水、目标账户、币种、Spend Rule、规则版本、控制范围和 periodId 上同源一致；全路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
...
| FR-TARGET-001 | 资金责任目标主体 | 缺资金责任、多个责任命中、错币种、缺账户层级快照、支出控制范围或 Spend Rule 被作为最终责任主体时仍生成 route；资金责任字段策略未选择 `funding-account-only` 或 `targetSubjectType + targetSubjectId` 时直接进入实现。 | 输出唯一 `FundingAllocationDecision` 或等价不可变决策；选择 `funding-account-only` 时仅声明资金账户责任解析，选择目标主体迁移时同步 DTO、DDL/H2、摘要、fixture、route snapshot、账户层级快照和回放断言。 | 不声明 `fundingAccountId` 已支持信用账户、VCC 关联子账户和平台角色目标态；不混用资金责任字段策略。 |
...
| SR-CONTROL-001 | 预算预留和释放控制活动 | 授权预留、撤销、部分完成或退款释放缺幂等、并发保护，或错误把超时当释放事实写入控制活动。 | 只写 Spend Control Activity 和预算控制投影；不写资金交易、route leg 或 ledger entry。 | 不声明支出控制范围 ledger 或预算余额桶生产可用。 |
...
| SR-CONTROL-004 | 交易结果消费控制活动 | 交易成功后无法区分预算控制消耗和失败释放，或交易退款无法回链原控制活动。 | 新增或等价实现交易消费控制活动服务；成功交易记录 `CONSUMED` 或等价消耗语义，失败或拒绝交易事实记录释放，退款记录补偿活动，并证明无资金事实副作用；超时不写控制流水。 | 不改交易 canonical 入参，不新增支付工具交易内核，不做自动事件消费者或生产迁移。 |
...
| 授权交易服务 | authorize、reversal、settle、settleRefund 单独验证；`settle` 同时覆盖普通完成和强制完成模式；`settleRefund` 同时覆盖完成后退款、无授权直接退款和争议类退款原因；授权资金责任覆盖资金账户、共享卡 + 信用子账户、共享卡 + 支出控制范围 / Spend Rule + 信用子账户。 | 授权-撤销、授权-完成-退款、授权-完成-剩余撤销、完成后退款和费用处理、多次完成、多次退款，均按 8.2 三种账户模型参数化覆盖；授权过期只验证无资金事实副作用。 | 授权拒绝无账务、普通完成不触碰 `LIMIT`、支出控制范围只作为共享卡下的预算范围和 Spend Rule 控制维度、信用子账户承接 VCC 发卡账务，父账户汇总不反写分录，预算和信用额度不表达真实资金、过期不形成资金终态、强制完成不伪造授权占用；普通授权链退款继续引用原完成事实，无授权退款以空原授权流水进入 no-auth 语义，并保留 `externalReferenceSn`、退款原因、操作者和审计。 | 撤销超额、完成超额、授权退款超额、授权拒绝误写争议退款、缺原快照、缺账户层级快照、到期自动释放授权占用、无原消费退款、无授权退款缺外部引用/原因/审计、无授权退款携带内部授权流水、强制完成缺策略。 |
