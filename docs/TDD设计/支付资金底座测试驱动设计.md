...
| `AC-CTRL-*`、`RED-013` | `TDD-CTRL-*`、`TDD-CTRL-FLOW-*`、`TDD-CTRL-ERR-*`、`TDD-LEDGER-006`、`TDD-LEDGER-008` 至 `TDD-LEDGER-011`、`TDD-WALLET-001` 至 `TDD-WALLET-004`、`TDD-ROUTE-004`、`TDD-RACE-004`、`TDD-RED-006`、`TDD-RED-011`、`TDD-RED-012`、`TDD-RED-015`、`TDD-RED-033`。 | 02-交易路由钱包账目与投影系分设计的余额控制、信用账户、预算控制范围 / Spend Rule 控制、账本周期、主体解析和受控负余额。 | P1 余额控制能力，P0 账本和余额投影必须同步守住；`AC-CTRL-004` 对应资金账户余额调整，必须覆盖 `TDD-CTRL-009` 和 `TDD-CTRL-ERR-005`；`AC-CTRL-009` 至 `AC-CTRL-011` 必须覆盖账本周期隔离、继承和跨周期查询；`AC-CTRL-012` 必须覆盖资金账户、信用账户和平台账户角色解析，业务主体、卡、VA、银行账户、支付工具、业务单据、预算控制范围和 Spend Rule 不得直接入账；资金账户余额调整必须有来源、审批、凭证和审计，不承接跨主体价值转移。 |
...
| 预算控制范围和 Spend Rule | 预算控制范围创建不初始化 ledger；Spend Rule 拒绝不生成 route/posting/entry；预算预留、释放和规则命中只更新控制证据或只读视图。 | PRD 的预算控制口径、DSL 的 BudgetGroup 上下文和 Spend Rule 快照、系分的预算控制视图和规则决策日志。 |
| 交易投影 | 多维交易投影可以按工具、账户、预算控制范围和 Spend Rule 查询，但投影重放不得生成资金交易、route、posting、ledger entry 或余额修复事实。 | PRD 的统一只读投影口径、DSL 的 projection replay 边界、系分的 transaction view query 和 view index 边界。 |
...
| TDD-BEN-ENTRY-004 | 授权后继事件权益回放 | `reverse` / `expire` / `settle` / `settleRefund` + `FundsBenefitContributionTransactionService#refund` | 后继事件接收当前重新计算的权益结果，或为完成、撤销、过期、退款分别新增权益专用方法。 | 仍使用授权生命周期入口；退款、业务取消或冲回统一走权益 `refund` 型逆向处理；后继事件读取原授权事实、原 route snapshot 和原让利出资交易；`expire` 已作为授权生命周期既有入口补齐，不是权益专用入口。 |
...
| TDD-BEN-REPLAY-003 | 让利出资事实源一致性 | `refund` / `reversal` / `expire` / `settleRefund` | 被设计选定的事实源必须能读取原让利出资交易流水、历史摘要、原订单或原交易引用、退款策略和决策流水；同一交易不能在不同事实源读到冲突摘要。 |
...
| TDD-ROUTE-003 | 授权 route | `AuthorizationFundsInstructionRouteResolver` | authorize/reversal/settle/refund 使用授权语义和原路径；授权过期不生成 route，拒付、扣回或争议只作为完成后退款的 reason 和审计上下文承接。 |
...
| TDD-WALLET-019 | 钱包应用服务边界 | `InstrumentTransactionLifecycleApplicationService` / `FundsAccountCapabilityApplicationService` / boundary tests | 支付工具交易生命周期、账户能力查询、业务入口参数解析和工具准入应通过 wallet application/use-case facade 表达；外部确认资金事件一旦消费为交易事实，应通过 `transaction.application.ExternalFundsEventApplicationService` 表达；账户聚合型 `WalletAccountApplicationService` 仍是目标态候选，未编码落地前不得把它写成当前公共契约；调用方不得绕过 facade 拼接 `PaymentInstrumentService`、`SpendSubjectFundingRelationService` 或账户资源服务完成授权准入、资金责任解析或交易委派；内部余额钱包、信用额度账户、返利钱包和商户钱包不得被强制包装成 `PaymentInstrument`。 |
| TDD-WALLET-020 | VCC 账户层级应用服务边界 | `AccountHierarchyApplicationService` 目标态候选 / VCC contract tests | VCC 不新增 `VCC_ACCOUNT`；每张卡必须绑定一个资金子账户或信用子账户，并固化父账户、账目 profile、币种、状态、背后资金责任来源、绑定工具摘要和账本初始化证据；当前仅保留设计验收口径，未进入 VCC 编码实现；不得保存完整 PAN/CVC，不承接卡生命周期或 issuer 原始协议。 |
| TDD-WALLET-021 | 支出控制准入快照 | `SpendControlAdmissionApplicationService` | 外部 Spend Rule 决策证据、支付工具预交易快照、资金责任目标和账户能力可以组合为准入结论；通过或拒绝会固化 Spend Rule 决策记录；证据缺失不得写决策记录；任何结果均不得创建资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
...
| TDD-WALLET-023 | 预算控制投影只读 | `SpendControlMovementService` / budget control projection query | 预算控制投影只能从 Spend Control Movement 聚合 limitAmount、reservedAmount、consumedAmount、releasedAmount、remainingControlAmount、availableControlAmount、lastMovementSn 和 lastMovementAt；查询心智模型是 `controlScopeId + periodId`，`budgetGroupSn` 仅作为历史兼容字段；同一控制范围跨周期查询必须按 periodId 隔离，传入目标资金账户或信用账户时必须只聚合该账户控制活动，不得混入同控制范围、同 Spend Rule 下其他账户、其他卡或其他周期的活动；不得包含或反写 `AVAILABLE`、`FROZEN`、`AUTHORIZATION` 等账本余额桶，不得声明预算组 ledger、预算余额桶或用 ledger period bucket 替代控制周期生产可用。 |
| TDD-WALLET-024 | 交易消费支出控制活动 | `SpendControlTransactionConsumptionApplicationService` | 交易成功、授权完成、交易失败、撤销、过期、退款或争议退款后，可以回链原控制活动和原资金交易记录消费、释放或补偿型控制活动；交易成功消耗不得复用失败释放语义；退款交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放，非退款交易事实不得被 `refund` 降级为退款控制补偿；退款交易即使引用原资金交易，也必须能找到对应 `CONSUMED` 控制活动后才能记录补偿，只能走对应控制语义；缺原控制活动、缺已消费控制活动、金额超占用、目标账户或币种不一致时 fail-fast；同一活动流水同摘要并发重复写入必须回读既有活动，不得抛出数据库唯一键异常或重复累计投影；交易消费后的预算控制投影按目标账户查询时不得混入其他账户、其他卡或其他周期控制活动；释放类控制活动写入上限必须按目标资金账户或信用账户隔离，历史关联活动必须与原控制活动在业务场景、业务流水、目标账户、币种、Spend Rule、规则版本、控制范围和 periodId 上同源一致；全路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
...
| TDD-TRANSACTION-EXT-001 | 外部资金事件消费入口 | `ExternalFundsEventApplicationService#consume` | ACH、银行文件、渠道回调或第三方钱包回调进入 funds 前必须先归一为外部事件流水、事件类型、目标资金账户、金额、币种、原交易引用、对账差异引用和业务流水；`ACH_CREDIT_CONFIRMED`、`BANK_CREDIT_CONFIRMED`、`EXTERNAL_CREDIT_CONFIRMED` 到资金账户时必须支持大小写 / 横线归一，显式映射出 `ACH_RAIL`、`BANK_RAIL` 或 `EXTERNAL_RAIL` 作为外部 rail/providerCode，并委派标准 `topup` 内核，证明目标资金账户 `AVAILABLE` 增加并生成 TOPUP 交易、route snapshot、posting plan、LedgerTransaction 和 LedgerEntry；未授权的 debit、return、NOC、reversal、外部 accepted/submitted/processing、信用账户目标或其他事件类型必须在交易内核前 fail-fast，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用；后续工程边界必须分别证明扣账、信用账户还款或调额、return/NOC/reversal、补事实、差异单和幂等回放边界。 |
...
| SC-LOOP-04 准入和授权组合回归 | transaction owner、测试 owner | 外部 approve / decline 只作为决策证据；拒绝无 route、posting、LedgerEntry，通过仍继续支付工具、账户能力、资金责任和余额校验。 | `just verify-slice SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests tests`。 | 需要把规则通过等同于授权成功或资金可用。 |
| SC-LOOP-05 控制流水和投影回归 | wallet owner、测试 owner | `SpendControlMovement` 承接预留、消耗、释放、退款补偿；`BudgetControlProjection` 按 `controlScopeId + periodId` 只读重建。 | `just verify-slice SpendControlMovementServiceFlowTests,BudgetControlLimitAdjustmentApplicationServiceTests,SpendControlTransactionConsumptionApplicationServiceTests tests`。 | 需要把预算组或 Spend Rule 建账、入账或展示为账本余额桶。 |
...
| VCC-LOOP-02 账户层级决策 | 架构 owner、wallet owner | `FinancialAccount / Card Account / VCC Account` 统一映射为资金子账户或信用子账户；当前基线已落到 `targetSubjectType + targetSubjectId`、账户层级绑定、route snapshot 账户层级快照和 H2 字段。 | `just verify-slice AccountHierarchyBindingContractTests,AccountHierarchyBindingServiceImplTests,SpendSubjectFundingRelationServiceImplTests,FundingResponsibilityResolutionApplicationServiceTests,FundsAuthorizationTransactionFlowTests,PaymentInstrumentRouteDslContractTests tests`。 | 仍要求 `funding-account-only` 声明 VCC、信用账户或平台角色生产可用，或新增 `VCC_ACCOUNT`。 |
...
| VCC-LOOP-04 授权交易组合 | transaction owner、wallet owner、测试 owner | `authorizeByInstrument` 组合工具准入、绑定版本、资金责任、账户能力、Spend Rule 决策；批准只占用，拒绝无资金事实。 | `just verify-slice AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests tests`。 | 需要把卡激活等同资金可用，或把规则通过等同授权成功。 |
...
| 3 | VCC-PACK-01 授权场景包 | transaction owner、wallet owner、测试 owner | VCC 授权批准、拒绝、支付工具准入、资金责任解析和拒绝无账务副作用的场景包；当前已用等价测试簇验证。 | `just verify-slice AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests,PaymentInstrumentRouteDslContractTests,FundingResponsibilityResolutionApplicationServiceTests tests`；需要 issuer、processor 或 card lifecycle 能力时停止。 |
| 4 | VCC-PACK-02 清算与逆向场景包 | transaction owner、ledger owner、测试 owner | clearing、partial clearing、refund、reversal 按原 route snapshot、binding version 和规则快照回放；当前已用等价测试簇验证。 | `just verify-slice DefaultRouteReplayServiceTests,RouteDslContractTests,PaymentInstrumentRouteDslContractTests,RouteSnapshotJsonSupportTests,FundsDirectTransactionFlowTests,FundsAuthorizationTransactionFlowTests tests`；缺外部 clearing / chargeback 事实模型时停止。 |
...
| 9 | P2-BOUNDARY-04 ACH / 银行转账等价验证 | wallet owner、transaction owner、reconciliation owner、安全 owner、测试 owner | ACH / 银行转账 confirmed credit、accepted 未到账、目标主体拒绝、rail/channel 归一、出款前准入、对账 gate 和敏感边界已用等价测试簇验证。 | `just verify-slice ExternalFundsEventApplicationServiceTests,InstrumentTransactionLifecycleApplicationServiceTests,WalletExternalFundsRailSupportTests,PayoutPreflightServiceTests,ReconciliationGateApplicationServiceTests,SensitiveContextVariablesValidatorTests tests`；缺 ACH 产品 owner、Nacha / ODFI / RDFI / 银行规则、Debit 授权、return / NOC / reversal 解释、文件批次或法域确认时停止。 |

B6/B8 进入交易投影或重放验证时，投影输入必须限定为交易事实、冻结单、route snapshot、`paymentInstrumentRef`、`AccountHierarchySnapshot`、`FundingAllocationDecision`、`SpendRuleDecisionRecord`、`SpendControlMovement`、账本摘要、授权拒绝事实、清结算和对账差错；不得把交易投影通过写成账务事实、余额事实或生产交付完成。

### 12.6 当前证据包归档

`SC-EVIDENCE-01` 只归档当前可复用证据，不声明生产启用、上线审批、外部合规、发卡处理商或卡组织规则已完成。

| 证据域 | 当前可复用证据 | 未覆盖 / 下一步 | 验证入口 |
| --- | --- | --- | --- |
| Spend Controls 文档边界 | 产品设计、系分、接入指南和 TDD 已统一 Highnote 只作外部校准；wallet 承接 evaluator、admission、movement、projection，transaction 只消费最终决策证据。 | 发版前补 `just compile` 和 `just pmd`；生产仍需发布 owner 确认灰度、回滚、告警、Runbook 和人工接管。 | 文档切片 `git diff --check`；代码切片回到 `SC-LOOP-03` / `SC-LOOP-04` / `SC-LOOP-05`。 |
| VCC 授权场景包 | 本轮已验证授权批准、授权拒绝、支付工具准入、资金责任解析、route DSL 和拒绝无账务副作用，覆盖 `VCC-PACK-01` 等价入口。 | 不声明 issuer、processor、card lifecycle 或发卡生产链路完成；后续转入清算与逆向。 | `just verify-slice AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests,PaymentInstrumentRouteDslContractTests,FundingResponsibilityResolutionApplicationServiceTests tests`，63 tests 通过。 |
| VCC 清算与逆向回放 | 本轮已验证 direct refund、authorization settle / reversal / refund、route replay、route snapshot JSON、支付工具原绑定和账户层级快照回放，覆盖 `VCC-PACK-02` 等价入口。 | 外部 clearing 文件和 partial clearing 差错仍需外部清分事实模型后再补专项。 | `just verify-slice DefaultRouteReplayServiceTests,RouteDslContractTests,PaymentInstrumentRouteDslContractTests,RouteSnapshotJsonSupportTests,FundsDirectTransactionFlowTests,FundsAuthorizationTransactionFlowTests tests`，123 tests 通过。 |
| VCC 账户层级 | 当前基线已覆盖 `targetSubjectType + targetSubjectId`、账户层级绑定、资金责任解析和 route snapshot 账户层级快照。 | 不声明 issuer、processor、card lifecycle 或新增 `VCC_ACCOUNT` 已完成。 | `just verify-slice AccountHierarchyBindingContractTests,AccountHierarchyBindingServiceImplTests,SpendSubjectFundingRelationServiceImplTests,FundingResponsibilityResolutionApplicationServiceTests,FundsAuthorizationTransactionFlowTests,PaymentInstrumentRouteDslContractTests tests`。 |
...
| P2 ACH / 银行转账边界 | 本轮已验证外部 confirmed credit 归一入账、accepted 未到账拒绝、信用账户目标拒绝、wallet rail / channel 归一、出款前准入、对账 gate 和敏感上下文阻断，覆盖 `P2-BOUNDARY-04` 等价入口。 | 不声明 ACH 产品、Nacha / ODFI / RDFI / 银行协议、Debit 授权、return / NOC / reversal 解释、文件批次、Same Day ACH、IAT 或法域规则完成。 | `just verify-slice ExternalFundsEventApplicationServiceTests,InstrumentTransactionLifecycleApplicationServiceTests,WalletExternalFundsRailSupportTests,PayoutPreflightServiceTests,ReconciliationGateApplicationServiceTests,SensitiveContextVariablesValidatorTests tests`，38 tests 通过。 |
...
| TDD-RECON-002 | 差错核销 | 核销前后差异可追溯；未重新对账不得关闭差错；当前首切片由 `ReconciliationDifferenceApplicationServiceTests` 证明“未回链处理动作不得仅靠 rerun 关闭差错”的最小红线。 |
| TDD-RECON-003 | 清分前基础对账 | 交易、单据、账本分录、余额桶和来源快照一致或可解释；核对失败则不可清分。 |
| TDD-RECON-004 | 清算前置对账 | 账实、账账、单账和余额一致性满足清算放行要求；重大差错阻断清算候选或清算确认；当前首切片由 `ReconciliationGateApplicationServiceTests` 与 `ClearingSettlementGateConsumerServiceTests` 证明对 CLEARING / SETTLEMENT 的对象级阻断与条件放行消费。 |
...
| TDD-P2-GA-002 | `GA-AC-003` 至 `GA-AC-005` | 全球账户出款、回单终态和退汇。 | 出款前置检查通过后才进入在途；无成功回单不得 PAID；`payOutByRail` 只能在外部终态成功或等价业务确认后关闭内部冻结资金；退汇必须关联原出金并处理费用、责任和幂等。 |
| TDD-P2-GA-003 | 全球账户接入准入卡 | 全球账户、跨境收付款或 ACH/银行转账进入资金底座前的证据校验。 | 必须先证明外部状态是否终态、内部账务主体、金额币种、幂等引用、外部规则确认状态、失败或退回 owner；confirmed credit 才能进入 `ExternalFundsEventApplicationService`，终态成功出款才可进入 `payOutByRail`；accepted、submitted、processing、message sent、退汇、return、NOC、reversal、错币种、FX 未确认或费用不一致只能进入在途、差错或人工复核，不得生成默认 topup、withdraw、refund、route、posting 或 ledger entry。 |
...
| 授权交易 | `FundsAuthorizationTransactionFlowTests`。 | 授权 command service、共享卡预算授权、授权过期不入资金交易、强制完成、授权链退款、无授权退款、争议裁决退款可区分性、发卡控制边界和授权并发竞争测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 初始已把 `B4-CB-RED-001A / TDD-RED-017B` 回归化，`B4-AUTH-RACE` 初始已把 `B4-RACE-RED-001` 回归化。 | 授权拒绝、完成、可信撤销、退款、授权过期不入账或争议裁决资金结果任一语义变化，都必须覆盖三种账户模型、拒绝无 route/entry、原路径回放和完成/撤销/退款并发红线；争议案件不得强制落独立 `chargeback` 公共入口，用户败诉不得产生资金副作用，用户胜诉退款不得被压缩为不可区分的普通退款；完整争议运营、旧 `chargeback` API 物理移除或更深跨节点并发控制仍需新 工程边界。 |
...
| 路由回放 | `DefaultRouteReplayServiceTests`、`CompositeRouteResolverTests`、`PaymentInstrumentRouteDslContractTests`。 | 原交易退款、授权撤销、授权完成、授权退款、争议类退款、退费和支付工具换绑后的全链路 route replay 专项；授权过期不得进入 route replay。 | 逆向、退款、撤销、退费和拒付不得按当前绑定重新选路；快照缺失必须失败；支付工具、外部账户、默认资金责任关系、平台角色和绑定历史不得泄露敏感原文或覆盖历史证据。 |
...
| 对账清算 | 已有最小服务级 H2 首切片：`ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`；清算策略可参考 `SettlementPolicySpecTests`。 | `ReconciliationTaskServiceTests`、`ReconciliationMatchingRuleTests`、`SettlementOrderServiceTests`、`PayoutReceiptMismatchTests`、`PayoutExplainabilityTests`；清结算退款时序、出款前准入绑定真实结算事实、外部非终态、出款金额不一致、出款解释状态防误导、cutoff、月末、闰日、节假日、非默认时区和 DST 专项。 | 当前首切片只证明对账差错、gate、clearing/settlement 准入消费和 payout preflight 最小门禁，不得外推为完整清分、清算、结算、出款生命周期或数据库级闭环完成。 |
...
| 清结算与对账 | 设计阶段先 L1/L5；首切片已进入 L2，整域仍待补 L4/L3。 | 可清分明细、清分批次、清分结果快照、清算候选、清算确认、结算锁定、出款前准入、出款结果、出款解释状态、差错核销。 | 清分对象直接入账、缺前置对账清算、出款受理当成功、差错直接改历史分录、缺解释状态仍展示可操作。 | 仅有局部服务级闭环。现有 `difference + gate + clearing/settlement consumer + payout preflight` 首切片不能替代完整清结算、对账或出款生命周期完成证据，仍缺完整出款生命周期、更多对象表结构和数据库闭环。 | 已有 `ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`；目标补 reconciliation task/match、clearing/settlement/payout 更完整 H2 服务测试、`PayoutReceiptMismatchTests` 和 `PayoutExplainabilityTests`。 |
...
| 授权交易服务 | authorize、reversal、settle、settleRefund 单独验证；`settle` 同时覆盖普通完成和强制完成模式；`settleRefund` 同时覆盖完成后退款、无授权直接退款和争议类退款原因；授权资金责任覆盖资金账户、共享卡 + 信用子账户、共享卡 + 预算组 / Spend Rule + 信用子账户。 | 授权-撤销、授权-完成-退款、授权-完成-剩余撤销、完成后退款和费用处理、多次完成、多次退款，均按 8.2 三种账户模型参数化覆盖；授权过期只验证无资金事实副作用。 | 授权拒绝无账务、普通完成不触碰 `LIMIT`、预算组只作为共享卡下的预算范围和 Spend Rule 控制维度、信用子账户承接 VCC 发卡账务，父账户汇总不反写分录，预算和信用额度不表达真实资金、过期不形成资金终态、强制完成不伪造授权占用；普通授权链退款继续引用原完成事实，无授权退款以空原授权流水进入 no-auth 语义，并保留 `externalReferenceSn`、退款原因、操作者和审计。 | 撤销超额、完成超额、授权退款超额、授权拒绝误写争议退款、缺原快照、缺账户层级快照、到期自动释放授权占用、无原消费退款、无授权退款缺外部引用/原因/审计、无授权退款携带内部授权流水、强制完成缺策略。 |
...
| 路由与回放 | direct、authorization、balance-control resolver 单独验证。 | 原路径退款、授权撤销/完成/退款、退费、解冻、账本周期 replay 组合；授权过期不参与 route replay。 | `supports` 只判断不改状态，外部账户和工具只进引用，平台角色必须解析为具体账户，route snapshot 固化账本周期。 | 缺 snapshot 重新选路、route code 漂移、resolver 选错、外部账户入账、过期生成 route 或 EXPIRE 语义、`settle` 强制完成伪造授权占用。 |
