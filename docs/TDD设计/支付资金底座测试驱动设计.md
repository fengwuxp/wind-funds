...
| VCC 授权场景包 | 本轮已验证授权批准、授权拒绝、支付工具准入、资金责任解析、route DSL 和拒绝无账务副作用，覆盖 `VCC-PACK-01` 等价入口。 | 不声明 issuer、processor、card lifecycle 或发卡生产链路完成；后续转入清算与逆向。 | `just verify-slice AuthorizationAdmissionApplicationServiceTests,FundsAuthorizationTransactionFlowTests,PaymentInstrumentRouteDslContractTests,FundingResponsibilityResolutionApplicationServiceTests tests`。 |
