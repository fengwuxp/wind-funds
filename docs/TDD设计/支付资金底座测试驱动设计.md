...
| `AC-CLR-*`、`AC-SET-*`、`AC-SET-006` 至 `AC-SET-009`、`AC-REC-*`、`RED-030` 至 `RED-033`、`RED-037` 至 `RED-039` | `TDD-CLS-*`、`TDD-CLS-FLOW-*`、`TDD-SETTLE-*`、`TDD-SETTLE-004`、`TDD-SETTLE-005`、`TDD-RECON-*`、`TDD-RED-020` 至 `TDD-RED-025`。 | 03-清结算与对账系分设计的可清分明细、清分批次、清算候选、清算批次、结算单、出款单、出款前准入门禁、出款解释状态、对账批次、差错单和追偿单。 | 清结算与对账专项；不混入交易主链路；出款门禁未知、外部非终态和金额不一致都不能展示为完成或自动放行。 |
| `AC-ARCH-*`、`AC-ARCH-008`、`AC-REPLAY-*`、`RED-016` 至 `RED-019`、`RED-034`、`RED-040` 至 `RED-042` | `TDD-GOV-*`、`TDD-ARCH-*`、`TDD-ARCH-009`、`TDD-REPLAY-*`、`TDD-METRIC-004`、`TDD-RACE-008`、`TDD-RACE-010`、`TDD-RED-018`、`TDD-RED-026` 至 `TDD-RED-028`。 | 04-归档重放与指标治理系分设计的统一治理流程接入、ArchiveManifest、Checkpoint、Watermark、BalanceSnapshotVerifyRef、差异报告、异常人工处理和交易投影重放任务。 | 归档重放专项；余额快照只能从账本事实确认，普通指标快照不能替代余额检查点、Manifest 覆盖校验或余额水位推进；归档或重放异常只能进入差异报告、审批、补证据、缩小范围、重跑或关闭差异，不能直接改交易、账目、余额或投影事实。 |
...
| 清结算与对账 | 设计阶段先 L1/L5；数据库落地后补 L2/L4/L3。 | 可清分明细、清分批次、清算候选、清算确认、结算锁定、出款前准入、出款结果、出款解释状态、差错核销。 | 清分候选直接入账、缺前置对账清算、出款受理当成功、差错直接改历史分录、缺解释状态仍展示可操作。 | Missing。冻结基线只有设计矩阵、模块骨架和 B7-00 出款前准入候选实现；候选实现已覆盖准入解释状态、完整外部规则核验证据和只读无账务事实断言，但未纳入清结算、对账或出款生命周期 Done 证据，且仍缺完整出款生命周期、表结构和数据库闭环。 | 当前以 `SettlementPolicySpecTests`、文档矩阵和 B7-00 候选实现差距为准；目标补 reconciliation/clearing/settlement/payout H2 服务测试、`PayoutPreflightTests`、`PayoutReceiptMismatchTests` 和 `PayoutExplainabilityTests`。 |
| 归档重放与治理 | L2 governance 端口 + L4 范围/水位/Manifest。 | archive request、Manifest、checkpoint、watermark、balance snapshot、projection replay、difference report 和人工处理闭环。 | 无范围重放、缺 Manifest 推进水位、dry-run 推进 checkpoint、普通指标快照替代余额快照、异常人工处理直接改事实。 | Existing Partial。交易投影重放已有局部基线；归档 Manifest、余额快照、差异报告和人工处理闭环是 Missing/Future Target。 | `FundsProjectionReplayServiceTests`，目标补 archive Manifest、balance snapshot、difference report 和 manual resolution 测试。 |
...
| 批次准入 | 02 主链路只按批次 1 至批次 6 逐批授权；03 清结算与对账、04 归档重放与指标治理未开放整体编码准入，必须另起独立 OpenSpec change 或 Execution Grant。 |
