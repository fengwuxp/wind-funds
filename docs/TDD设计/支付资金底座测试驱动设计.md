...
| TDD-RECON-007 | 对账任务生命周期 | 对账任务创建、收数、等待、匹配、差异、阻断、重跑、关闭均有状态和报告；重跑不得覆盖旧运行记录；当前由 `ReconciliationRunResultApplicationServiceTests` 证明完成态运行结果和逐笔匹配结果不可变、状态与计数由明细派生、来源对不可重复计数、输入顺序摘要稳定、并发幂等、写入失败全量回滚和无账务副作用。完整任务生命周期、来源事实绑定、调用权限、来源采集和匹配规则执行仍待实现；当前测试不证明上游提交的数据覆盖完整。 |
...
| 对账清算 | 已有 H2 局部切片：`ReconciliationRunResultApplicationServiceTests`、`ReconciliationDifferenceApplicationServiceTests`、`ReconciliationGateApplicationServiceTests`、`ReconciliationDifferenceReportApplicationServiceTests`、`ClearingSplittableDetailApplicationServiceTests`、`ClearingSettlementGateConsumerServiceTests`、`PayoutPreflightServiceTests`；清算策略可参考 `SettlementPolicySpecTests`。 | `ReconciliationTaskServiceTests`、`ReconciliationMatchingRuleTests`、清分批次/快照测试、`SettlementOrderServiceTests`、`PayoutReceiptMismatchTests`、`PayoutExplainabilityTests`；清结算退款时序、出款前准入绑定真实结算事实、cutoff、时区和 DST 专项。 | 当前只证明不可变完成态运行结果、持久化逐笔匹配事实及派生摘要、对账差错、只读报告、fail-closed gate、单笔可清分准入、clearing/settlement gate consumer 和 payout preflight；不得外推为完整来源采集、归一化、匹配规则执行、任务生命周期、清分批次、清算、结算、出款生命周期或数据库闭环完成。 |
