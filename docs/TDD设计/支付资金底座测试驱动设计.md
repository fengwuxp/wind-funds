...
| TDD-WALLET-024 | 交易消费支出控制活动 | `SpendControlTransactionConsumptionApplicationService` | 交易成功、授权完成、交易失败、撤销、过期、退款或争议退款后，可以回链原控制活动和原资金交易记录消费、释放或补偿型控制活动；交易成功消耗不得复用失败释放语义；退款交易事实不得被 `release` 降级为普通失败释放，只能走退款控制补偿语义；缺原控制活动、金额超占用、目标账户或币种不一致时 fail-fast；同一活动流水同摘要并发重复写入必须回读既有活动，不得抛出数据库唯一键异常或重复累计投影；全路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影事实。 |
