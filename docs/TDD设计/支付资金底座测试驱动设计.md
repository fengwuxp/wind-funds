...
| P2 VCC、全球账户和收单业务支持 | 只作为业务专项 TDD 输入，不进入默认工程范围。 | 外部规则确认状态、敏感数据边界、P0/P1 回归范围、专项验收 ID 和未覆盖条件。 | `TDD-P2-*`、`TDD-RAIL-*`、`TDD-FX-*`、`TDD-OPS-*` 与对应资金底座红线。 | 把资金底座测试通过声明为 VCC、全球账户或收单业务系统整体交付完成。 |
