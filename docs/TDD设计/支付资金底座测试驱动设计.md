...
| 资金责任解析 | `FundingAllocationDecision` 唯一，最终主体只能是资金账户、信用账户、VCC 关联资金/信用子账户或平台账户角色解析后的平台资金账户；字段策略采用 `targetSubjectType + targetSubjectId`。 | 资金责任缺失、不唯一、错币种、账户能力不足、缺账户层级快照、支出控制范围或 Spend Rule 被当作最终主体时失败；不得恢复旧资金账户镜像字段。 |
...
| FR-TARGET-001 | 资金责任目标主体 | 缺资金责任、多个责任命中、错币种、缺账户层级快照、支出控制范围或 Spend Rule 被作为最终责任主体时仍生成 route；字段未同步 `targetSubjectType + targetSubjectId` 就进入实现。 | 输出唯一 `FundingAllocationDecision` 或等价不可变决策；目标主体迁移必须同步 DTO、DDL/H2、摘要、fixture、route snapshot、账户层级快照和回放断言。 | 不恢复旧资金账户镜像字段，不混用资金责任字段策略。 |
...
| 路由回放 | `DefaultRouteReplayServiceTests`、`CompositeRouteResolverTests`、`PaymentInstrumentRouteDslContractTests`。 | 原交易退款、授权撤销、授权完成、授权退款、争议类退款、退费和支付工具换绑后的全链路 route replay 专项；授权过期不得进入 route replay。 | 逆向、退款、撤销、退费和拒付不得按当前绑定重新选路；快照缺失必须失败；支付工具、外部账户、资金责任关系、平台角色和绑定历史不得泄露敏感原文或覆盖历史证据。 |
