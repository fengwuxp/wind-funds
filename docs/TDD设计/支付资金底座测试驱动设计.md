...
| TDD-WALLET-025 | 支付工具收款入口 | `InstrumentTransactionLifecycleApplicationService#receiveByInstrument` | VA、ACH 或外部钱包端点收款必须先完成支付工具状态、方向、币种、绑定版本、资金责任和账户能力准入，再委派账户主体型 `FundsDirectTransactionService#topup`；成功后资金账户 `AVAILABLE` 增加并生成标准 TOPUP 交易、route、posting plan、账本交易和分录；付款-only 工具、缺绑定或能力不匹配时必须在进入交易内核前失败，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用；该入口不改变交易 canonical 入参，不给 VA、卡或外部账户建账。 |
