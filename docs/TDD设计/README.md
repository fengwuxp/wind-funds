...
| 8 | [GSD-2-P0P1-LedgerWalletTransaction推进计划.md](GSD-2-P0P1-LedgerWalletTransaction推进计划.md) | GSD-2 Wave 5 P0/P1 ledger、wallet、transaction 优先推进计划，当前账户层级来源契约、资金责任目标主体、wallet application facade、交易投影解释、余额调账审计、B7 差错闭环、动作守卫、准入消费和出款 preflight 首个消费方接入已完成首轮 Green；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 已补 contract/design-only 任务卡，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成兼容入口最小审计 guard 和验证收口，`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮已完成授权争议退款解释，下一候选转为 B4 投影解释剩余矩阵、wallet facade、ledger guard、清算/结算消费方接入、B7 差异报告或 B5 审计扩展。 |
