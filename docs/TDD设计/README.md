...
| 10.2 | [GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md](GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md) | `GSD2-B5-SR-TRANSACTION-CONSUME-001` 与 `GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` 的确认和消费记录：在支出控制准入、支付工具预交易快照、控制活动记录、预算控制投影和账户主体型交易内核均已形成首轮证据后，已补齐交易成功消耗、失败释放、已有退款资金事实后的控制补偿，以及同流水同摘要并发唯一键冲突回读既有活动的服务层切片。当前状态为 `SR_TRANSACTION_CONSUME_CONCURRENCY_GREEN_VERIFIED_COMMITTED`；本 Grant 不授权完整 Spend Rule 规则引擎、事件消费 / outbox、Controller、HTTP/RPC、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造或 ledger posting。 |
