...
| 全球账户入金 | 上层验签并归一 -> 已确认的 `CONFIRMED` credit event -> 委派 topup -> 目标资金账户入账 -> 上层匹配银行流水与 LedgerTransaction -> 冻结纯对账结果。 | `ACCEPTED/PROCESSING` 不得入账；外部账户只作为引用，不作为 LedgerEntry 主体；纯对账不伪造结算 Gate，`DIFFERENCE_FOUND` 不得回写资金和账本事实。 | 已有 L3：`ExternalFundsEventApplicationServiceTests` 证明已确认事件到 topup/route/ledger，并模拟上层提交匹配结论后验证 `BALANCED / DIFFERENCE_FOUND` 均不改变已入账事实；验签、银行流水核验、匹配执行和运营差错处置仍由上层负责。 |
