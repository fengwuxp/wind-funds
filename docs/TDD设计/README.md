...
本目录是支付资金底座最终版 TDD 设计入口。它承接产品设计、DSL 设计、系分设计和现有代码模块，把“产品必须证明什么”转成“代码测试必须覆盖哪些真实执行路径”。
...
| 顺序 | 文档 | 作用 |
| --- | --- | --- |
| 1 | [支付资金底座最终TDD设计.md](支付资金底座最终TDD设计.md) | 定义测试驱动设计原则、模块测试矩阵、场景用例、红线用例、现有测试资产和执行门禁。 |
...
| `core`、`wallet-*`、`transaction-*`、`ledger-*`、`tests` | 真实代码执行路径和现有测试资产。 |
