...
本次 CR 结论：TDD 入口能够承接 PRD 目标、DSL 契约和系分落点，可以进入测试资产分析和首批 Red 排序。进入编码前仍需 Execution Grant 明确写入范围、禁止范围、目标测试资产、验证命令和 Not Done 条件；未授权时只能产出测试设计、契约草案或 dry-run。
...
| P0 资金内核 | 钱包、账本、账目、余额投影、对账、清分、清算、结算、归档和账本余额快照。 | 统一资金事实和账务对象。 | 账户账本、清结算对象、governance 逻辑边界和横切红线。 | B2/B7/B8 首批 Red、余额桶断言、对账差错、归档水位、只读边界和失败无副作用。 | P0 能力未闭合就先验证 P2 业务特殊路径。 |
