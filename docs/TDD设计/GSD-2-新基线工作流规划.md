...
| 当前补充证据 | `GSD2-B5-SR-TRANSACTION-CONSUME-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006`、`GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009`、`GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010`、`GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013`、`GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014`、`GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` 和本轮 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001` 已形成服务层验证证据。 |
| 设计 baseline | `docs/产品设计`、`docs/DSL设计`、`docs/系分设计`、`docs/TDD设计`、`openspec` 的当前可读状态；Spend Rule 产品语义以 `docs/产品设计/09-SpendRule支出规则产品设计.md` 为权威，系统落点以 `docs/系分设计/06-SpendRule支出规则系分设计.md` 为权威；`openspec` 异常 Git 状态只作为停止条件和只读事实记录。 |
| 活跃未完成编码计划 | 当前没有活跃未完成编码计划；`GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 已按用户确认完成首轮实现、本地 Green / Verify 并提交到 `78f7f008`，`GSD2-B2-BUDGET-GROUP-NON-LEDGER-SUBJECT-001` 已提交到 `a5b12a3f`，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit，并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫、释放上限目标账户隔离守卫和 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001` 已完成服务层 Red / Green / Verify。旧候选不再作为当前编码计划，只能在新 Workflow 中被重新选择、重新编号、重新确认。 |
| Owner | AI Native 流程编排负责状态、Loop、GSD 和门禁；产品架构专家负责业务目标、对象、能力、验收和金融待确认；资深架构师负责系统边界、接口、TDD、验证和编码准入。 |
| 写入范围 | 本文、W5 P0/P1 ledger-wallet-transaction 推进计划、LWT 生产可用能力 Goal、B5 交易消费支出控制活动确认包、Agent Loop / Plan Grant 默认授权策略、AI 代码交付闭环基线、GSD-2 单一 Grant 任务模板、`docs/TDD设计/README.md`、`docs/README.md`、Spend Rule 产品/系分独立分册和 OpenSpec tasks 状态同步。 |
| 写入文件 | `docs/产品设计/09-SpendRule支出规则产品设计.md`、`docs/系分设计/06-SpendRule支出规则系分设计.md`、`docs/产品设计/README.md`、`docs/系分设计/README.md`、`docs/产品设计/02-交易路由钱包账目与投影.md`、`docs/系分设计/02-交易路由钱包账目与投影系分设计.md`、`docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 本轮新增写入补充 | `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001` 已完成服务层契约消费：规则定义、不可变版本、规则挂载和决策日志形成最小持久化边界，已发布版本同摘要可幂等回放、异摘要拒绝覆盖，支付工具和预算组只作为控制 scope 记录，规则拒绝不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本轮另按产品/系分交付标准新增 Spend Rule 独立产品分册和系分分册，将规则配置、准入决策、控制证据、预算控制视图和只读解释从主线交易路由文档中拆出；该 docs-only 拆分不授权继续扩完整规则引擎、规则表达式执行器、控制活动、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| Git 策略 | B7 对象级 Gate、清算 / 结算 consumer、B7 差异报告、支出控制准入快照和 B5 控制活动均已按授权提交固化；本轮释放上限目标账户隔离守卫随本提交固化；后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
...
| 当前活跃编码执行队列 | `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED` | `GSD2-B5-SR-CONTROL-ACTIVITY-001` 已随 `78f7f008` 提交，预算组非建账已随 `a5b12a3f` 提交，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit；并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫已完成服务层 Red / Green / Verify；当前没有 active 编码任务。下一轮必须重新选择新的单一 Grant。 |
...
| 默认建议 | 若继续优先收敛 LWT 生产可用能力，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、wallet 授权准入、账户能力来源、预交易快照、支出控制准入、ledger guard、B5 余额调账、B7 gate / consumer / report、B5 控制活动、预算组非建账、B5 交易消费支出控制活动、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫均已消费；下一轮需重新选择新的单一 Grant。 |
| 当前默认确认包 | 当前没有可复用默认确认包；[GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md](GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md) 已消费并作为证据保留，状态 `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`。 |
...
| Git 策略 | `summary_only`。 |
| 下一 owner | 用户重新选择新的单一 Grant；不再复用 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-B5-SR-CONTROL-ACTIVITY-001` 或 `GSD2-B2-BUDGET-GROUP-NON-LEDGER-SUBJECT-001`；若继续 wallet，建议优先从 Spend Rule 决策消费、投影解释、事件消费 / outbox、VCC facade 或生产迁移准入中择一建立新 Grant。 |
