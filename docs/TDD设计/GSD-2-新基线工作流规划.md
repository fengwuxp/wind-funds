...
| Git / code baseline | 当前已提交 Git/code baseline 包含 `a1397ddf feat: 补齐对账差异报告只读查询` 和 `96938fa8 docs: 回写对账差异报告提交基线`；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已完成 Red / Green / Verify 并提交固化。本轮消费 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`，新增支付工具预交易快照服务、请求、DTO、只读实现和目标服务流测试，作为新的 wallet 预交易快照基线。`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已成为已消费被依赖证据；`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据，`da7d2ea test: 阻断契约夹具承载资金流断言` 和 `b3b9712 feat: 对齐资金底座GSD基线与交易回放能力` 只保留为 GSD-2 历史证据。 |
| 设计 baseline | `docs/产品设计`、`docs/DSL设计`、`docs/系分设计`、`docs/TDD设计`、`openspec` 的当前可读状态；`openspec` 异常 Git 状态只作为停止条件和只读事实记录。 |
| 活跃未完成编码计划 | 已清零。旧候选不再作为当前编码计划，只能在新 Workflow 中被重新选择、重新编号、重新确认；AUTH 兼容 adapter、B4 remaining、wallet 授权准入、wallet route snapshot 回链、账户能力来源准入、预交易快照、ledger guard、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer 和 B7 差异报告已消费；当前仅保留后续新 Grant 选择。 |
| Owner | AI Native 流程编排负责状态、Loop、GSD 和门禁；产品架构专家负责业务目标、对象、能力、验收和金融待确认；资深架构师负责系统边界、接口、TDD、验证和编码准入。 |
| 写入范围 | 本文、W5 P0/P1 ledger-wallet-transaction 推进计划、LWT 生产可用能力 Goal、B7 对账差异报告确认包、Agent Loop / Plan Grant 默认授权策略、AI 代码交付闭环基线、GSD-2 单一 Grant 任务模板、`docs/TDD设计/README.md`、`docs/README.md` 和 OpenSpec tasks 状态同步。 |
| 写入文件 | `docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B7-对账差异报告ExecutionGrant确认包.md`、`docs/TDD设计/README.md`、`docs/README.md`、`openspec/changes/tdd-baseline-reset/tasks.md`。 |
| 本轮新增写入补充 | `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已消费为本地 Green / Verify：新增 wallet 支付工具预交易快照 face / impl / DTO / request 和目标测试，并同步 LWT Goal、W5、README 和 OpenSpec tasks；不授权 Spend Rule、VCC facade、支付工具交易内核、Controller、HTTP/RPC、DDL/H2 schema、生产迁移或 Git 历史重写。 |
...
| Git 策略 | B7 对象级 Gate、清算 / 结算 consumer 和 B7 差异报告均已按提交授权固化；本轮支付工具预交易快照在验证通过后按用户当前 CAD 自动推进和提交授权进入本地提交。 |
...
| 当前活跃编码执行队列 | `EMPTY` | `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已消费为本地 Green / Verify，当前没有可继续编码的 Execution Grant，也没有默认可自动执行的 Java / DDL / 公共契约任务；下一步需改选新的单一 Grant。 |
...
背景：GSD-2 初始重置点为 `b3b9712`，后续已继续提交账户层级快照、信用账户路由闭合、GSD2 路由快照契约、DSL 契约门禁、资金责任目标主体、wallet application facade、支付工具能力准入 facade、交易投影解释、余额调账审计、对账差错闭环、对账差错处理动作守卫、对账准入消费、出款 preflight 消费、AUTH chargeback 兼容 guard、B4 投影解释 remaining 矩阵、wallet 授权准入、route snapshot 回链、ledger guard、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer 和 B7 对账差异报告，当前已提交 Git/code baseline 至 `96938fa8`。`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已完成本地 Red / Green / Verify，作为本轮 wallet 能力来源准入变更记录。当前需要避免继续沿用旧计划或旧基线状态，先以状态载体把最新设计、代码、任务和 OpenSpec 重新对齐。
...
| 架构项 | 当前基线 |
| --- | --- |
| 现状和影响范围 | 当前已提交代码基线为 `96938fa8 docs: 回写对账差异报告提交基线`；B7 对象级 Gate 基座、清算 / 结算 consumer 和 B7 差异报告已提交固化。B5-002 route snapshot 审计回链已在 `da3b4f19` 提交固化，B5-003 独立审计查询已在 `4ef64275` 提交固化；`0b251593` 保留为进入 B5-002 前的历史基线；`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`e81a8a25` 和 `ae8cb8a6` 保留为已消费能力证据，`da7d2ea` 和 `b3b9712` 只保留为 GSD-2 历史证据。当前账户能力来源准入切片影响 core、wallet-face、wallet-impl、tests、docs 和 openspec。 |
...
| 候选队列 | 已完成 `GSD2-B2-ACCOUNT-HIERARCHY-CONTRACT-001`、`GSD2-B2-ACCOUNT-HIERARCHY-SOURCE-CONTRACT-002`、`GSD2-B2-FR-TARGET-001`、`GSD2-B2-WALLET-APPLICATION-FACADE-001`、`GSD2-B2-WALLET-APPLICATION-FACADE-002 / B2-PI-CAP-CAD-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-DIFFERENCE-MVP-001`、`GSD2-B7-RECON-DIFFERENCE-MVP-002`、`GSD2-B7-RECON-GATE-CONSUME-001`、`GSD2-B7-RECON-GATE-CONSUME-002`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 和 `GSD2-LD-LEDGER-GUARD-REGRESSION-001` 首轮 Green；当前以 [GSD-2-LWT-生产可用能力Goal.md](GSD-2-LWT-生产可用能力Goal.md) 作为下一轮 loop 状态载体。 |
| 默认建议 | 若继续优先收敛 LWT 生产可用能力，`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 的 contract/design-only 任务卡已补齐，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；下一步重新选择 Spend Rule 控制闭环、VCC facade、B7 报告扩展等新的单一 Grant。 |
| 当前默认确认包 | 无待确认默认编码包。`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已由 [GSD-2-B7-对账差异报告ExecutionGrant确认包.md](GSD-2-B7-对账差异报告ExecutionGrant确认包.md) 消费并提交；[GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md](GSD-2-B7-清算结算真实消费方ExecutionGrant确认包.md) 已转为 consumer 消费结果记录；`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 和 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 已由本轮 LWT Goal 消费为 wallet Green。下一轮若进入编码，需重新确认新的单一 Grant。 |
...
| Git 策略 | `summary_only`。 |
| 下一 owner | 用户确认是否进入 B7 报告扩展、Spend Rule 控制闭环、VCC facade 或 wallet 后续能力；清算 / 结算真实消费方服务已消费，阻断粒度沿用已完成的对象级 Gate 基座，不再复用 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 或 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`；产品架构专家确认业务用例、展示口径和 Not Done；资深架构师明确接入边界、交易/账本副作用断言、DTO、服务测试、账本护栏和验证命令。 |
...
| 状态一致性 | `rg "GSD2-LWT|LWT-PRODUCTION|单一 Grant 决策账本|结构门禁验证证据账本|FundsAccountCapabilityApplicationServiceTests|PaymentInstrumentPreTransactionSnapshotApplicationServiceTests|GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001|GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001|LedgerNormalBalanceGuard|B7_RECON_DIFFERENCE_REPORT_GREEN_VERIFIED_COMMITTED|GSD2-B7-RECON-DIFFERENCE-REPORT-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001|GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001" docs openspec` | 旧计划不再作为活跃入口，新 Workflow、LWT Goal、W5、README、OpenSpec tasks、wallet 授权准入、route snapshot 回链、账户能力来源准入、支付工具预交易快照、ledger guard、B5-002、B5-003、B7 对象级 Gate 基座、清算 / 结算 consumer 和 B7 差异报告 Green 结果均可追踪，下一 Grant 仍需用户重新确认。 |
