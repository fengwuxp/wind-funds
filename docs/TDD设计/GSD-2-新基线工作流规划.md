...
| Git / code baseline | 当前已提交 Git/code baseline 包含 `a1397ddf feat: 补齐对账差异报告只读查询` 和 `96938fa8 docs: 回写对账差异报告提交基线`；`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001 / scopeDecision=object-scope-schema-backed`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已完成 Red / Green / Verify 并提交固化。本轮消费 `GSD2-B2-SPEND-CONTROL-ADMISSION-001`，新增支出控制准入服务、请求、DTO、枚举、只读实现和目标服务流测试，作为新的 wallet Spend Rule 决策证据准入快照基线。`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 已成为已消费被依赖证据；`4ef64275 feat: 补齐余额调账独立审计查询`、`da3b4f19 feat: 补齐余额调账路由审计回链`、`0b251593 feat: 补齐账本正常余额方向护栏`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`ae8cb8a6` 和 `e81a8a25` 保留为已消费能力证据，`da7d2ea test: 阻断契约夹具承载资金流断言` 和 `b3b9712 feat: 对齐资金底座GSD基线与交易回放能力` 只保留为 GSD-2 历史证据。 |
| 设计 baseline | `docs/产品设计`、`docs/DSL设计`、`docs/系分设计`、`docs/TDD设计`、`openspec` 的当前可读状态；`openspec` 异常 Git 状态只作为停止条件和只读事实记录。 |
| 活跃未完成编码计划 | 已清零。旧候选不再作为当前编码计划，只能在新 Workflow 中被重新选择、重新编号、重新确认；AUTH 兼容 adapter、B4 remaining、wallet 授权准入、wallet route snapshot 回链、账户能力来源准入、预交易快照、支出控制准入快照、ledger guard、B5-002 route snapshot 审计回链、B5-003 独立审计查询、B7 对象级 Gate 基座、B7 清算 / 结算 consumer 和 B7 差异报告已消费；当前仅保留后续新 Grant 选择。 |
...
| 本轮新增写入补充 | `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 已消费为本地 Green / Verify：新增 wallet 支出控制准入 face / impl / DTO / request / enum 和目标测试，并同步 LWT Goal、W5、README 和 OpenSpec tasks；不授权 Spend Rule 规则定义、决策日志持久化、Spend Control Activity、预算控制投影、VCC facade、支付工具交易内核、Controller、HTTP/RPC、DDL/H2 schema、生产迁移或 Git 历史重写。 |
...
| Git 策略 | B7 对象级 Gate、清算 / 结算 consumer 和 B7 差异报告均已按提交授权固化；本轮支出控制准入验证通过后按用户当前 CAD 自动推进和提交授权进入本地提交。 |
...
| 架构项 | 当前基线 |
| --- | --- |
| 现状和影响范围 | 进入本轮支出控制准入切片前的最新已提交代码基线为 `83aaa0ad feat: 补齐支付工具预交易快照`；B7 对象级 Gate 基座、清算 / 结算 consumer 和 B7 差异报告已提交固化。B5-002 route snapshot 审计回链已在 `da3b4f19` 提交固化，B5-003 独立审计查询已在 `4ef64275` 提交固化；`0b251593` 保留为进入 B5-002 前的历史基线；`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`e81a8a25` 和 `ae8cb8a6` 保留为已消费能力证据，`da7d2ea` 和 `b3b9712` 只保留为 GSD-2 历史证据。当前支出控制准入切片影响 core、wallet-face、wallet-impl、tests、docs 和 openspec。 |
...
| 默认建议 | 若继续优先收敛 LWT 生产可用能力，`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 的 contract/design-only 任务卡已补齐，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；下一步重新选择完整 Spend Rule 控制活动 / 预算控制投影、VCC facade、B7 报告扩展等新的单一 Grant。 |
...
| Git 策略 | `summary_only`。 |
| 下一 owner | 用户确认是否进入 B7 报告扩展、完整 Spend Rule 控制活动 / 预算控制投影、VCC facade 或 wallet 后续能力；清算 / 结算真实消费方服务已消费，阻断粒度沿用已完成的对象级 Gate 基座，不再复用 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001`、`GSD2-B7-RECON-DIFFERENCE-REPORT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 或 `GSD2-B2-SPEND-CONTROL-ADMISSION-001`；产品架构专家确认业务用例、展示口径和 Not Done；资深架构师明确接入边界、交易/账本副作用断言、DTO、服务测试、账本护栏和验证命令。 |
