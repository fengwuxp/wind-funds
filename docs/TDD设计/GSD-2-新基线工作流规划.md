...
| 当前补充证据 | `GSD2-B5-SR-TRANSACTION-CONSUME-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006` 和本轮 `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` 已形成服务层验证证据。 |
| 设计 baseline | `docs/产品设计`、`docs/DSL设计`、`docs/系分设计`、`docs/TDD设计`、`openspec` 的当前可读状态；`openspec` 异常 Git 状态只作为停止条件和只读事实记录。 |
| 活跃未完成编码计划 | 当前没有活跃未完成编码计划；`GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 已按用户确认完成首轮实现、本地 Green / Verify 并提交到 `78f7f008`，`GSD2-B2-BUDGET-GROUP-NON-LEDGER-SUBJECT-001` 已提交到 `a5b12a3f`，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit，并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫和失败释放业务流水一致性守卫已完成服务层 Red / Green / Verify。旧候选不再作为当前编码计划，只能在新 Workflow 中被重新选择、重新编号、重新确认。 |
...
| 本轮新增写入补充 | `GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层消费：交易成功记录控制消耗，失败 / 拒绝 / 过期释放控制占用，已有退款资金事实后记录控制补偿，且不调整支付工具 `REFUND` 方向规则；后续补片已证明 `REFUND` 交易事实不得被 `consume` 或 `release` 降级为普通控制活动，非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿，退款交易引用原资金交易但没有对应已消费控制活动时不得生成退款控制补偿，资金交易事实不得跨业务场景消费当前 Spend Rule 控制占用，`consume` 不得串用同场景其他业务流水的成功交易，且 `release` 不得串用同场景其他业务流水的失败、拒绝或过期交易。该切片只覆盖交易成功消耗、失败释放、退款补偿、原控制活动 / 原交易回链、业务场景一致性守卫、成功消费业务流水一致性守卫和失败释放业务流水一致性守卫，不授权继续扩 Spend Rule 规则定义实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| Git 策略 | B7 对象级 Gate、清算 / 结算 consumer、B7 差异报告、支出控制准入快照和 B5 控制活动均已按授权提交固化；本轮释放业务流水一致性守卫随本提交固化；后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
...
| 当前活跃编码执行队列 | `SR_TRANSACTION_RELEASE_BUSINESS_SN_GUARD_GREEN_VERIFIED` | `GSD2-B5-SR-CONTROL-ACTIVITY-001` 已随 `78f7f008` 提交，预算组非建账已随 `a5b12a3f` 提交，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit；并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫和失败释放业务流水一致性守卫已完成服务层 Red / Green / Verify；当前没有 active 编码任务。下一轮必须重新选择新的单一 Grant。 |
...
| 当前默认确认包 | 当前没有可复用默认确认包；[GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md](GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md) 已消费并作为证据保留，状态 `SR_TRANSACTION_RELEASE_BUSINESS_SN_GUARD_GREEN_VERIFIED`。 |
