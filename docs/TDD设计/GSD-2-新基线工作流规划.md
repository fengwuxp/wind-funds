...
| 本轮新增写入补充 | `GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层消费：交易成功记录控制消耗，失败 / 拒绝 / 过期释放控制占用，已有退款资金事实后记录控制补偿，且不调整支付工具 `REFUND` 方向规则；后续补片已证明 `REFUND` 交易事实不得被 `consume` 或 `release` 降级为普通控制活动，非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿，退款交易引用原资金交易但没有对应已消费控制活动时不得生成退款控制补偿，资金交易事实不得跨业务场景消费当前 Spend Rule 控制占用，`consume` 不得串用同场景其他业务流水的成功交易，`release` 不得串用同场景其他业务流水的失败、拒绝或过期交易，`refund` 补偿金额不得超过退款交易引用的已消费控制活动净额，被引用已消费控制活动必须与原控制活动同源一致，同一原控制活动下同一资金交易流水的累计解释金额不得超过资金交易金额，同一控制活动流水和摘要不得被不同业务语义复用，且退款交易引用的原消费资金交易必须真实存在并满足租户、类型、状态、币种、业务场景和业务流水一致性。本轮 `GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` 只补预算控制投影按目标资金账户或信用账户过滤，证明同预算组和同 Spend Rule 下其他账户或其他卡的控制活动不会污染当前账户投影。该切片不授权继续扩 Spend Rule 规则定义实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| Git 策略 | B7 对象级 Gate、清算 / 结算 consumer、B7 差异报告、支出控制准入快照和 B5 控制活动均已按授权提交固化；本轮预算控制投影目标账户隔离守卫随本提交固化；后续新 Grant 默认 `summary_only`，除非用户再次明确授权提交。 |
...
| 当前活跃编码执行队列 | `SR_TRANSACTION_BUDGET_PROJECTION_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED` | `GSD2-B5-SR-CONTROL-ACTIVITY-001` 已随 `78f7f008` 提交，预算组非建账已随 `a5b12a3f` 提交，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层 Green / Verify / Commit；并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫和预算控制投影目标账户隔离守卫已完成服务层 Red / Green / Verify；当前没有 active 编码任务。下一轮必须重新选择新的单一 Grant。 |
...
| 默认建议 | 若继续优先收敛 LWT 生产可用能力，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、wallet 授权准入、账户能力来源、预交易快照、支出控制准入、ledger guard、B5 余额调账、B7 gate / consumer / report、B5 控制活动、预算组非建账、B5 交易消费支出控制活动和预算控制投影目标账户隔离守卫均已消费；下一轮需重新选择新的单一 Grant。 |
| 当前默认确认包 | 当前没有可复用默认确认包；[GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md](GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md) 已消费并作为证据保留，状态 `SR_TRANSACTION_BUDGET_PROJECTION_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`。 |
