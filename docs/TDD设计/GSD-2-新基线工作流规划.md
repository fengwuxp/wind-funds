...
| 本轮新增写入补充 | `GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成服务层消费：交易成功记录控制消耗，失败 / 拒绝 / 过期释放控制占用，已有退款资金事实后记录控制补偿，且不调整支付工具 `REFUND` 方向规则；后续补片已证明 `REFUND` 交易事实不得被 `consume` 或 `release` 降级为普通控制活动，非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿，退款交易引用原资金交易但没有对应已消费控制活动时不得生成退款控制补偿，资金交易事实不得跨业务场景消费当前 Spend Rule 控制占用，`consume` 不得串用同场景其他业务流水的成功交易，`release` 不得串用同场景其他业务流水的失败、拒绝或过期交易，`refund` 补偿金额不得超过退款交易引用的已消费控制活动净额，被引用已消费控制活动必须与原控制活动同源一致，同一原控制活动下同一资金交易流水的累计解释金额不得超过资金交易金额，同一控制活动流水和摘要不得被不同业务语义复用，且退款交易引用的原消费资金交易必须真实存在并满足租户、类型、状态、币种、业务场景和业务流水一致性。`GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` 已补预算控制投影按目标资金账户或信用账户过滤；本轮 `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` 进一步补释放类活动写入上限目标账户过滤，并证明历史脏关联活动跨账户、跨规则或跨预算组时不得参与当前控制活动金额计算。该切片不授权继续扩 Spend Rule 规则定义实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
