...
本文是资金数据治理能力的系统分析设计分册。逻辑模块名为 `governance`，负责说明大数据量下资金事实归档、投影重放、检查点、水位、资金归档 Manifest、差异报告和指标项承接边界如何落到系统对象、数据表、任务状态、核心 API、可观测和准入门禁。`governance` 是逻辑模块名和能力边界名；物理落点必须保持独立逻辑边界和 face/port 依赖，不得把治理对象混入交易或账本事实表。
...
| 候选名称 | 判断 | 结论 |
| --- | --- | --- |
| `governance` | 命名简短；结合资金域上下文和中文名“资金数据治理能力”限定后，可承接归档、重放、检查点、水位、差异报告和生产修复门禁。 | 采用。 |
...
本节保留归档、余额重建和交易投影重放相关表的职责、关键字段组、状态和红线，不再逐字段展开。归档和重放属于生产高危能力，所有任务表必须记录租户、范围、发起人、原因、审批、状态、checkpoint、摘要、差异报告和 traceId。

#### 4.6.1 表归属和字段组

| 表或对象 | 事实归属 | 04 中的作用 | 关键字段组 | 物理落点建议 |
| --- | --- | --- | --- | --- |
| `t_funds_archive_request` | 资金数据治理能力 | 归档控制面。 | 申请号、租户、范围、原因、截止时间、预检查、审批、Manifest 引用、状态、操作者、trace。 | 独立治理模块存在时落 `governance`；否则落归档治理适配实现。 |
| `t_funds_archive_manifest` | 资金数据治理能力 | 归档证据面。 | Manifest 号、申请号、对象类型、范围、记录数、借贷摘要、游标、checkpoint、水位、冷热位置、摘要、状态。 | 独立治理模块存在时落 `governance`；否则落归档治理适配实现。 |
| `t_balance_checkpoint` | 账本模块 | 余额重建、归档门禁和冷热拼接证明。 | 检查点号、主体、账目、币种、周期、检查点时间、last entry、余额、借贷摘要、分录数、覆盖模式、校验报告、Manifest。 | 归属 `ledger`，04 只定义引用要求。 |
| `t_balance_projection_watermark` | 账本模块 | 余额水位推进和冷热边界证明。 | 水位号、主体、账目、币种、周期、水位时间、last entry、检查点、推进任务、推进状态、版本。 | 归属 `ledger`，04 只定义引用要求。 |
| 普通指标快照 | 报表指标模块 | 只读协作对象。 | 指标任务、指标窗口、指标水位、发布状态。 | 归属报表指标模块，04 不设计指标快照表。 |
| `t_projection_replay_task` | 资金数据治理能力或投影实现 | 投影重放控制面。 | 任务号、租户、范围、视图域、模式、幂等键、状态、checkpoint、成功/失败/跳过/差异数量、审批、原因、操作者、trace。 | 若有独立治理模块落 `governance`；否则落投影实现侧并保留治理端口。 |
| `t_funds_governance_difference_report` | 资金数据治理能力 | 归档、余额重建和投影重放的统一差异样本。 | 报告号、任务类型、任务号、差异类型、来源对象、期望/实际摘要、等级、状态、关联差错单、操作者、trace。 | 独立治理模块存在时落 `governance`；否则落治理适配实现。 |

#### 4.6.2 状态和唯一性要求

| 对象 | 状态要求 | 唯一性和索引 |
| --- | --- | --- |
| 归档申请 | `CREATED`、`PRECHECKING`、`PRECHECK_FAILED`、`APPROVING`、`APPROVED`、`RUNNING`、`COMPLETED`、`FAILED`、`CANCELLED`。 | 申请号唯一；按租户、状态、截止时间索引。 |
| 归档 Manifest | `DRAFT`、`READY`、`RUNNING`、`VERIFYING`、`COMPLETED`、`FAILED`、`CANCELLED`。 | Manifest 号唯一；按申请、状态、checkpoint 索引。 |
| 余额检查点 | `CREATED`、`VERIFYING`、`VERIFIED`、`FAILED`。 | 同 bucket 同 checkpoint time 只有一个有效检查点；按检查点号和状态索引。 |
| 余额水位 | `ACTIVE`、`FAILED`、`PAUSED`，推进状态单独表达。 | 同 bucket 只有一个当前水位；按水位时间和 checkpoint 索引。 |
| 投影重放任务 | `CREATED`、`RUNNING`、`PAUSED`、`COMPLETED`、`FAILED`、`CANCELLED`。 | 任务号唯一；租户 + 幂等键唯一；按状态和范围索引。 |
| 差异报告 | `OPEN`、`CONFIRMED`、`RESOLVED`、`IGNORED`。 | 报告号唯一；任务 + 来源 + 差异类型唯一；按任务、状态和等级索引。 |
