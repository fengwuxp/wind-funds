...
本文是资金数据治理能力的系统分析设计分册。逻辑模块名为 `governance`，负责说明大数据量下资金事实归档、投影重放、检查点、水位、资金归档 Manifest、差异报告和指标项承接边界如何落到系统对象、数据表、任务状态、核心 API、可观测和准入门禁。`governance` 是逻辑模块名和能力边界名；是否新增 `governance-face` / `governance-impl` 物理模块，由归档、重放或生产修复能力是否进入编码批次决定。
...
| --- | --- |
| 模块中文名 | 资金数据治理能力 |
| 逻辑模块名 | `governance`。当前先作为独立逻辑能力和跨模块编排边界；当归档、重放或生产修复进入编码批次时，再评估是否新增 `governance/governance-face` 和 `governance/governance-impl` 物理模块。 |
| Java 包建议 | 若新增物理模块，包名建议为 `com.capte.funds.governance`；若暂不新增模块，则契约按归属落在 `ledger`、`transaction` 或对账清算模块，并保留 `governance` 编排语义。 |
...
| 当前阶段优先级 | P0 确认归档申请、预检查、资金归档 Manifest、交易投影重放任务、差异报告和快照校验确认边界；物理模块拆分按编码批次确认；余额检查点和水位通过账本能力协作落地。 |

模块一句话定义：

资金数据治理能力负责把“历史事实能否安全归档、归档后能否证明、投影出错能否有界重放”变成可审批、可执行、可审计、可回滚的资金域治理能力；它只定义资金域规则、编排和证明，不重写资金事实，也不替代通用数据治理或报表指标模块。
...
| 候选名称 | 判断 | 结论 |
| --- | --- | --- |
| `governance` | 短、可扩展；结合资金域上下文和中文名“资金数据治理能力”限定后，可承接归档、重放、检查点、水位、差异报告、生产修复门禁和未来同类数据治理能力。 | 采用。 |
| `archive` | 只覆盖归档，无法表达交易投影重放和差异报告。 | 不采用。 |
| `archive-replay` | 准确但偏窄，后续数据质量、证据链、生产修复门禁等同类能力不易归入。 | 不采用。 |
...
| 普通指标快照 | 指标快照批次和指标水位。 | 指标口径版本、数据来源、质量规则、缺数处理、权限脱敏、发布审批和失败回滚。 | 指标水位、指标查询、报表展示、导出订阅。 | 作为余额证明、归档完成依据、调账依据或清结算状态推进依据。 |
...
| 交易投影重放 | 投影重放任务和重放 checkpoint。 | 范围、事实源位置、Manifest 一致性、语义兼容起点、重放模式、影子结果、差异报告和审批。 | 影子投影或正式只读投影 upsert。 | 重新入账、补写交易事实、修改账本分录、推进余额水位或指标水位。 |
...
| 失败状态 | 指标不发布、停留旧版本或标记质量异常。 | 检查点不得进入 VERIFIED，余额水位停留旧值，归档准入被阻断。 |
...
| 失败影响 | 指标不发布、回退旧版本或标记质量异常。 | 余额水位不推进、归档阻断、重建失败并生成差异报告。 |
...
本节定义资金数据治理能力进入编码前的 Java 契约设计。目标不是提前写实现，也不强制当前批次新增物理模块，而是明确逻辑契约、枚举、Request、Query、DTO、服务接口和内部端口，让后续编码可以按 TDD 批次落地。若编码批次只需要账本检查点、交易投影重放端口或归档 Manifest 的局部能力，可先落在事实归属模块；若归档、重放和生产修复控制面整体进入编码，再新增独立 `governance-face` / `governance-impl`。
...
| 独立物理模块 | 归档申请、Manifest、重放任务、差异报告和生产修复控制面整体进入编码批次。 | `governance-face`。 | `governance-impl`。 | 只能通过 face/port 依赖交易、账本和对账清算能力，不得依赖其他模块 impl。 |
...
| ArchiveRequestQuery | 按租户、状态、对象类型、主体、时间窗口、审批号查询归档申请。 |
| ArchiveRequestDTO | 返回申请号、范围、预检查摘要、审批、状态、主 Manifest、原因和审计。 |
...
| ArchiveManifestQuery | 按申请号、Manifest、状态、对象类型、检查点、水位和范围查询。 |
| ArchiveManifestDTO | 返回 Manifest 范围、对象、冷热位置、摘要、游标、检查点、水位、状态和审计。 |
...
| BalanceRebuildTaskDTO | 返回任务范围、目标时间、检查点、水位、差异摘要、状态和审计。 |
...
| BalanceSnapshotVerifyResultDTO | 返回检查点、覆盖模式、Manifest 覆盖、热区游标、冷区游标、借贷摘要、余额摘要、差异摘要和校验状态。 |
...
| ProjectionReplayTaskQuery | 按租户、状态、视图域、范围、模式和时间查询。 |
| ProjectionReplayTaskDTO | 返回任务范围、模式、状态、checkpoint、成功数、失败数、跳过数、差异数和审计。 |
...
| 类型 | 字段或方法 | 说明 |
| --- | --- | --- |
| CreateDifferenceReportRequest | `requestSn`、`requestDigest`、`tenantId`、`taskType`、`taskSn`、`differenceType`、`sourceType`、`sourceSn`、`expectedValue`、`actualValue`、`severity`、`operatorRef` | 创建差异报告。 |
| LinkDifferenceToExceptionRequest | `requestSn`、`requestDigest`、`reportSn`、`exceptionSn`、`operatorRef` | 将重大差异关联对账差错单。 |
| ResolveDifferenceReportRequest | `requestSn`、`requestDigest`、`reportSn`、`resolution`、`evidenceRefs`、`operatorRef` | 关闭差异报告。 |
| DifferenceReportQuery | 按任务、来源、等级、状态、差错单和时间查询。 |
| DifferenceReportDTO | 返回差异来源、期望值、实际值、等级、状态、差错关联、证据和审计。 |
...
本节定义归档、余额重建和交易投影重放相关表的最终设计要求。归档和重放均属于生产高危能力，所有任务表必须记录租户、范围、发起人、原因、审批、状态、checkpoint、摘要、差异报告和 traceId。

表归属原则：

| 表或对象 | 事实归属 | 04 中的作用 | 物理落点建议 |
| --- | --- | --- | --- |
| 资金归档申请 | 资金数据治理能力 | 归档控制面。 | 独立物理模块存在时落 `governance`；否则落归档治理适配实现。 |
| 资金归档 Manifest | 资金数据治理能力 | 归档证据面。 | 独立物理模块存在时落 `governance`；否则落归档治理适配实现。 |
| 余额检查点 | 账本模块 | 余额重建、归档门禁和冷热拼接证明。 | 归属 `ledger`，04 只定义字段要求和引用方式。 |
| 余额投影水位 | 账本模块 | 余额水位推进和冷热边界证明。 | 归属 `ledger`，04 只定义字段要求和引用方式。 |
| 普通指标快照 | 报表指标模块 | 作为只读协作对象，04 只定义不得复用资金域控制对象。 | 归属报表指标模块，04 不设计指标快照表。 |
| 交易投影重放任务 | 资金数据治理能力或投影实现 | 投影重放控制面。 | 若有独立治理模块落 `governance`；否则落投影实现侧并保留治理端口。 |
| 差异报告 | 资金数据治理能力 | 归档、余额重建和投影重放的统一差异样本。 | 独立物理模块存在时落 `governance`；否则落治理适配实现。 |

#### 4.6.1 资金归档申请表

表名：t_funds_archive_request

业务用途：承载一次资金归档操作的申请、范围、审批、执行策略和结果。它是归档控制面，用于判断这次归档是否允许执行；申请通过后才允许生成资金归档 Manifest 和执行归档。
...
| request_sn | VARCHAR(64) | 是 | 资金归档申请号 | FAR202605180001 |
...
| manifest_sn | VARCHAR(64) | 否 | 首个或主资金归档 Manifest | FAM202605180001 |
| status | VARCHAR(30) | 是 | 状态 | APPROVED |
...
- 名称：uk_funds_archive_request_sn；字段：request_sn

普通索引：

- 名称：idx_funds_archive_request_tenant_status；字段：tenant_id, status
- 名称：idx_funds_archive_request_cutoff；字段：expected_cutoff_time

#### 4.6.2 资金归档 Manifest 表

表名：t_funds_archive_manifest

业务用途：承载一次资金归档实际落地的对象、范围、冷热位置、记录数量、金额摘要、游标、检查点、水位和防篡改摘要。它是归档证据面，用于证明本次归档结果可查询、可校验、可重建；缺资金归档 Manifest 不得标记归档成功。
...
| gmt_create | DATETIME | 是 | 创建时间 | - |
| gmt_modified | DATETIME | 是 | 最后更新时间 | - |
| manifest_sn | VARCHAR(64) | 是 | 资金归档 Manifest 号 | FAM202605180001 |
| tenant_id | BIGINT(20) | 是 | 租户 ID | 1 |
| request_sn | VARCHAR(64) | 是 | 资金归档申请号 | FAR202605180001 |
...
| debit_amount | BIGINT(20) | 是 | 借方金额 | 1000000 |
| credit_amount | BIGINT(20) | 是 | 贷方金额 | 1000000 |
| last_cursor | VARCHAR(128) | 否 | 最后处理游标 | LE1000000 |
| checkpoint_sn | VARCHAR(64) | 是 | 检查点和水位引用 | CP... |
| watermark_sn | VARCHAR(64) | 是 | 检查点和水位引用 | BW... |
| hot_location | VARCHAR(512) | 是 | 热区和冷区位置 | table:t_ledger_entry |
| cold_location | VARCHAR(512) | 是 | 热区和冷区位置 | table:t_ledger_entry |
| digest | VARCHAR(128) | 是 | 防篡改摘要 | sha256... |
| status | VARCHAR(30) | 是 | 状态 | COMPLETED |
| operator_id | VARCHAR(64) | 否 | 操作者 ID | - |
| trace_id | VARCHAR(64) | 否 | 链路追踪 ID | - |

状态说明：`DRAFT` 草稿，`READY` 可执行，`RUNNING` 执行中，`VERIFYING` 校验中，`COMPLETED` 完成，`FAILED` 失败，`CANCELLED` 取消。

唯一索引：

- 名称：uk_funds_archive_manifest_sn；字段：manifest_sn

普通索引：

- 名称：idx_funds_archive_manifest_request；字段：request_sn
- 名称：idx_funds_archive_manifest_status；字段：tenant_id, status
- 名称：idx_funds_archive_manifest_checkpoint；字段：checkpoint_sn

#### 4.6.3 余额检查点表

表名：t_balance_checkpoint

表归属：账本模块拥有；04 仅定义资金数据治理能力引用该表时必须满足的字段、状态和索引要求。

业务用途：保存某主体、账目、币种、账本周期在某时间点的已校验余额、借贷摘要和分录游标，用于冷热拼接、余额重建和归档门禁。

余额检查点是账本余额快照的物理表达。它可以复用统一快照任务的调度和差异报告能力，但表归属、字段口径、状态推进和校验规则必须由账本模块定义和维护。
...
| coverage_mode | VARCHAR(30) | 是 | 余额快照覆盖模式 | HOT_ONLY |
| hot_last_entry_sn | VARCHAR(64) | 否 | 热区覆盖到的最后分录 | LE202605180001 |
| cold_last_entry_sn | VARCHAR(64) | 否 | 冷区覆盖到的最后分录 | LE202512310001 |
| status | VARCHAR(30) | 是 | 状态 | VERIFIED |
| verify_report_sn | VARCHAR(64) | 否 | 余额快照校验差异报告 | DR... |
| manifest_sn | VARCHAR(64) | 否 | 覆盖该边界的资金归档 Manifest | FAM... |

状态说明：`CREATED` 已创建，`VERIFYING` 校验中，`VERIFIED` 已校验，`FAILED` 失败。

确认约束：

1. 状态进入 `VERIFIED` 前必须完成分录数量、借贷金额、余额方向、last entry cursor、digest 和覆盖模式校验。
2. `verify_report_sn` 有阻断差异时不得进入 `VERIFIED`。
3. 同一 bucket 同一 checkpoint time 只能有一个有效检查点；重算必须产生新版本或新 checkpoint，不得覆盖原始审计。
4. 普通指标快照任务不得写入或更新本表。
5. `coverage_mode=HOT_ONLY` 时 `manifest_sn` 可以为空，但 `hot_last_entry_sn` 必须存在。
6. `coverage_mode=COLD_MANIFEST` 时 `manifest_sn` 和 `cold_last_entry_sn` 必须存在，且 Manifest 必须为已完成。
7. `coverage_mode=MIXED` 时 `manifest_sn`、`hot_last_entry_sn` 和 `cold_last_entry_sn` 都必须存在，且冷热摘要合并后必须与检查点摘要一致。

唯一索引：

- 名称：uk_balance_checkpoint_bucket_time；字段：tenant_id, subject_type, subject_id, ledger_subject_code, currency, period_type, period_id, checkpoint_time

普通索引：

- 名称：idx_balance_checkpoint_sn；字段：checkpoint_sn
- 名称：idx_balance_checkpoint_status；字段：status

#### 4.6.4 余额投影水位表

表名：t_balance_projection_watermark

表归属：账本模块拥有；04 仅定义资金数据治理能力引用该表时必须满足的水位语义、字段、状态和索引要求。
...
| advance_task_sn | VARCHAR(64) | 否 | 推进水位的治理任务 | BRT202605180001 |
| advance_status | VARCHAR(30) | 是 | 水位推进状态 | ADVANCED |
| status | VARCHAR(30) | 是 | 状态 | ACTIVE |
| version | INT(11) | 是 | 乐观锁版本 | 0 |

状态说明：`ACTIVE` 当前有效，`FAILED` 最近推进失败，`PAUSED` 暂停推进。

推进约束：

1. `checkpoint_sn` 必须指向 `VERIFIED` 的余额检查点。
2. `advance_status` 只有在快照校验通过、差异报告无阻断项、审批或 Runbook 满足要求后才能进入 `ADVANCED`。
3. 指标水位、指标快照批次、归档 Manifest、交易投影重放 checkpoint 都不能写入 `watermark_sn` 或替代 `checkpoint_sn`。
4. 水位推进失败时保留旧 `watermark_time`、旧 `last_entry_sn` 和失败差异报告。

唯一索引：

- 名称：uk_balance_watermark_bucket；字段：tenant_id, subject_type, subject_id, ledger_subject_code, currency, period_type, period_id

普通索引：

- 名称：idx_balance_watermark_time；字段：watermark_time
- 名称：idx_balance_watermark_checkpoint；字段：checkpoint_sn
...
| tenant_id | BIGINT(20) | 是 | 租户 ID | 1 |
| scope_type | VARCHAR(50) | 是 | 重放范围类型和值 | SINGLE_TRANSACTION |
| scope_value | TEXT | 是 | 重放范围类型和值 | {} |
| view_domain | VARCHAR(50) | 是 | 视图域 | MERCHANT_BILL |
| mode | VARCHAR(30) | 是 | 重放模式 | VERIFY_ONLY |
| idempotency_key | VARCHAR(128) | 是 | 幂等键 | sha256... |
| status | VARCHAR(30) | 是 | 状态 | RUNNING |
| checkpoint | VARCHAR(512) | 否 | 断点续跑位置 | cursor-001 |
| success_count | BIGINT(20) | 是 | 结果摘要 | 100 |
| failed_count | BIGINT(20) | 是 | 结果摘要 | 0 |
| skipped_count | BIGINT(20) | 是 | 结果摘要 | 0 |
| difference_count | BIGINT(20) | 是 | 结果摘要 | 2 |
...
- 名称：uk_projection_replay_task_sn；字段：task_sn
- 名称：uk_projection_replay_idempotency；字段：tenant_id, idempotency_key

普通索引：

- 名称：idx_projection_replay_status；字段：status, gmt_modified
- 名称：idx_projection_replay_scope；字段：scope_type, view_domain
...
| tenant_id | BIGINT(20) | 是 | 租户 ID | 1 |
| task_type | VARCHAR(50) | 是 | 来源任务类型和任务号 | PROJECTION_REPLAY |
| task_sn | VARCHAR(64) | 是 | 来源任务类型和任务号 | PRT... |
...
| expected_value | TEXT | 否 | 期望值和实际值摘要 | {} |
| actual_value | TEXT | 否 | 期望值和实际值摘要 | {} |
| severity | VARCHAR(20) | 是 | 严重等级 | HIGH |
| status | VARCHAR(30) | 是 | 状态 | OPEN |
...
- 名称：uk_difference_report_sn；字段：report_sn
- 名称：uk_difference_report_source；字段：task_sn, source_type, source_sn, difference_type

普通索引：

- 名称：idx_difference_report_task；字段：task_type, task_sn
- 名称：idx_difference_report_status；字段：status, severity
...
4. 账本余额快照测试必须证明普通指标快照不能替代余额检查点、余额水位和 Manifest 覆盖校验。
5. 账本余额快照测试必须覆盖 `HOT_ONLY`、`COLD_MANIFEST`、`MIXED` 三种覆盖模式。
6. 指标项只在报表指标模块承接，不进入交易、路由、钱包、账本主链路，也不复用归档重放的控制表和水位对象。
7. 若复用外部通用数据治理或报表指标模块的任务框架，必须通过端口隔离，不允许 `governance` 直接依赖其内部表或调度实现。
