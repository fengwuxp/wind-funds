...
本节按架构师系分模板描述归档、余额快照、余额水位、交易投影重放和差异报告表结构。归档和重放属于生产高危能力，所有任务表必须记录租户、范围、发起人、原因、审批、状态、checkpoint、摘要、差异报告和 traceId。普通指标快照只作为外部协作对象描述边界，不在本模块落表。

统一约束：

- 归档 Manifest、余额检查点、余额水位、普通指标快照、交易投影重放 checkpoint 是不同治理对象，不得混用字段、流水和水位。
- 余额检查点和余额水位归属 ledger 能力；04 只定义治理引用和校验要求。独立 governance 模块存在时，归档申请、Manifest、投影重放任务和差异报告落 governance；否则落治理适配实现，但端口语义不变。
- 金额字段使用最小货币单位，借贷摘要和余额摘要必须可由 ledger entry、ledger transaction、posting plan 摘要和 normal balance 重算。
- 数据库不建外键，不级联删除；只保存业务流水和摘要。归档、检查点、差异报告和审计证据只追加，不覆盖历史事实。
- 大字段只保存脱敏 JSON、摘要或对象存储引用，不保存归档数据正文、敏感凭证或外部账户原文。
...
- manifest_sn 允许为空用于热区预检查和审批前兼容；COMPLETED 时必须引用已完成 Manifest。
...
业务用途：承载余额重建、归档门禁和冷热拼接证明；余额快照只能从账本事实计算。
...
| subject_id | varchar(30) | [X] | 账务主体 ID | M1001 |
