...
- 当前版本：v1.0
- 文档状态：Review
- owner：governance owner、ledger owner、transaction owner、reconciliation owner、架构 owner、测试 owner、SRE owner
- 更新日期：2026-07-17
- 变更摘要：按最新系统分析设计模板补齐产品语义、运行时规则、数据模板约束和规则落地结构。
- 目标读者：架构、研发、测试、SRE、数据治理、安全、财务和审计。
- 决策状态：逻辑能力边界已确认；物理落点和生产修复控制面仍需单独工程准入。
- 置信度：中高；容量、冷热介质和生产运行参数待实施验证。

## 系分模板追踪索引

| 系分模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 产品语义输入 | 1.1 | 承接归档、重建、重放和可审计验收。 |
| 概要设计与追踪 | 第三节 | 覆盖物理落点、依赖、能力地图和治理流程。 |
| 运行时与详细设计 | 4.1-4.9 | 覆盖归档、重建、重放、接口、数据、补偿和状态规则。 |
| 规则落地 | 4.10 | 汇总事实不可变、水位推进、重放边界和指标只读规则。 |
| Engineering Handoff | 第六节 | 区分最小首切片和目标态切片。 |
...
物理实现时按下列事实归属拆分，避免 governance、ledger 和报表指标模块交叉持有事实。

| 对象 | 系统归属 | 可写边界 | 可读或引用边界 |
| --- | --- | --- | --- |
| 账本余额检查点 | ledger / balance projection。 | 由账本事实、账目、账本周期、entry 游标和金额摘要生成。 | governance 可引用其 checkpointSn、覆盖模式和摘要用于归档门禁、冷热拼接和余额重建。 |
| 余额水位 | ledger / balance projection。 | 由余额检查点校验通过后推进；失败不得推进。 | governance 只读引用，不复用为归档 Manifest 水位或指标水位。 |
| 资金归档 Manifest | governance。 | 由归档任务记录范围、对象、冷热位置、摘要、审批、校验和差异报告。 | ledger、transaction、reconciliation 和报表指标模块只按授权读取 Manifest 证明，不反写 Manifest。 |
| 交易投影重放任务 | governance 编排，投影模块执行只读重建。 | 只写重放任务、checkpoint、difference report 和审计。 | transaction、ledger、reconciliation 只提供事实读取，不允许被重放任务写事实。 |
| 指标项输入 | 报表指标模块拥有计算和发布；wind-funds 只提供可信输入。 | wind-funds 不写指标结果、指标水位或报表发布状态。 | 报表指标模块只读消费资金事实、投影和 Manifest 授权视图。 |
...
| --- | --- | --- | --- | --- |
| REQ-GOV-001 | 核心名相 | Manifest、余额检查点、余额水位、重放 checkpoint 和指标水位职责独立。 | 产品 01、04 | 3.1-3.5、4.1-4.4。 |
| SCN-GOV-001 | 业务场景 | 历史事实归档、余额重建、投影验证/重放、差异报告和人工处理。 | 产品 04、05 | 4.1-4.3、4.7。 |
| AC-GOV-001 | 验收种子 | 先校验后推进水位，重放不重新入账，余额不从交易投影或指标反推。 | 产品 05 | 4.2、4.3、4.8。 |
| QA-GOV-001 | 质量属性种子 | 任务有界、可续跑、可回滚、可审计，导出脱敏且生产 apply 受审批控制。 | 产品 04、05 | 4.7、第五、六节。 |
...
本分册进入工程落地前按下表确认结构完整性，避免把治理能力边界误读成通用数据平台或报表指标模块。
...
| 详细设计、接口设计和数据设计 | 第 4.1 至 4.6 节覆盖归档、余额重建、投影重放、指标边界、Java 模型、服务 API 和表设计。 | 触碰 API、表结构、H2 schema、状态机或服务入口时必须另起工程任务。 |
...
| 测试设计和回归测试 | 第 4.5.10、4.8 节覆盖 TDD API 模拟验证矩阵和测试设计。 | 工程落地前必须明确契约测试、服务级测试、H2 schema、差异报告、幂等并发和红线失败测试。 |
| 工程任务边界 | 第 6 节交付切片与准入门禁。 | 具体负责人、交付范围和验收方式以独立工程边界为准。 |
...
本分册从系统分析视角已具备交付评审状态：治理逻辑边界、物理落点候选、归档 Manifest、余额检查点、余额水位、交易投影重放、差异报告、指标边界、大数据消费承接和生产修复控制面已经承接产品分册。该状态只能说明“系分可交付评审”，不能替代工程任务边界、代码实现、数据库落地或生产上线批准。
...
| TDD 分析 | 可进入 | 可以输出 Red 顺序、治理物理落点、Manifest/H2 范围、dry-run/apply、指标水位隔离、治理导出和大数据消费边界测试矩阵。 |
| 工程边界 | 待工程边界确认 | 必须由独立工程边界逐项确认 `GOV-GATE-*`、写入范围、禁止范围、验证命令和停止条件。 |
| 数据库级落地 | 待实现 | 需补归档任务、运行记录、Manifest、checkpoint、watermark、冷热对象清单、余额快照、差异报告、人工处理、导出快照、消费方登记、审计对象和 H2 schema。 |
| 服务级落地 | 待实现 | 需补真实 H2 服务流程测试、dry-run/apply 幂等、范围锁、回滚续跑、差异报告、只读事实边界、指标水位隔离和失败无副作用测试。 |
| 生产准入 | 待确认 | 需补 NFR、容量、观测告警、Runbook、灰度回滚、外部规则核验、专业确认状态、使用者解释字段、治理导出脱敏、职责分离和证据最小化。 |

工程任务至少需要确认：治理物理落点；是否复用 `governance-face`、`governance-impl`；依赖方向；是否新增公共契约、DTO、Entity、Mapper、DDL/H2 schema；`TDD-B8-RED-001` 至 `TDD-B8-RED-005` Red 顺序；Manifest、checkpoint、watermark、余额快照、差异报告、人工处理、dry-run/apply、指标水位隔离、大数据消费承接、治理导出、回滚续跑和边界测试范围。缺少任一 `GOV-GATE-*` 时，本分册只能继续做 TDD 分析、契约草案或 dry-run，不得进入 apply、正式水位推进、生产修复控制面、生产代码、测试代码、DDL/H2 schema 或运行时配置写入。
...
| Java 包建议 | 工程落地前必须在任务边界中二选一确认物理落点：独立 governance 模块，或由 ledger / transaction / reconciliation 等归属模块承接局部能力。无论采用哪种落点，都必须保留 governance 编排语义、独立 API/port、差异报告、统一治理接入边界和独立表职责，不得把治理对象混入交易、账本、投影、清结算或指标事实表。 |
...
治理能力先按逻辑边界建模，工程落地前再由任务边界确认物理落点。选择物理落点时必须优先保护事实边界、依赖方向、测试可证明性和后续迁移成本，不能因为已有包名或短期实现方便把治理对象混进交易、账本、清结算或指标事实表。

| 落点方案 | 适用条件 | 允许写入范围 | 禁止 | 迁移代价和风险 | 必须先补的测试 |
| --- | --- | --- | --- | --- | --- |
| 复用既有 `governance-face` / `governance-impl` | 工程任务要落归档、重放、差异报告或生产修复控制面中的跨模块能力，且既有治理骨架足以承接。 | governance 契约、任务对象、Manifest、差异报告、审计摘要、编排服务和必要的 H2 schema。 | 依赖 transaction-impl、ledger-impl 或 reconciliation-impl；在 governance 内直接写交易、账本、清结算、对账、余额或指标事实。 | 最符合长期治理边界，但公共契约一旦发布会带来兼容约束；需要补足模块边界测试和 DDL/H2。 | `TDD-B8-RED-001` 至 `TDD-B8-RED-005`、模块依赖边界测试、Manifest 幂等测试、指标水位隔离测试。 |
...
| 新增独立物理模块 | 现有 governance 骨架不足以承载完整治理闭环，或需要单独部署、单独权限、单独审计和跨模块治理生命周期。 | 新增 governance-face / governance-impl、独立表、Mapper、DTO、Service、审计和 H2 schema。 | 未有工程任务边界就创建空壳模块；把指标计算、报表查询、看板、导出订阅放进 governance；跳过 BOM/依赖/CI 影响评估。 | 长期边界最清晰，但初始成本最高；需要调整多模块构建、依赖聚合、测试和发布门禁。 | 模块依赖架构测试、契约兼容测试、表结构 H2 测试、失败回滚测试、权限审计测试和 `just verify-cad`。 |

选择规则：

1. 若工程任务要做正式 apply、生产修复控制面、跨模块 Manifest 或差异报告，优先复用或建设独立 governance 落点，不应散落到事实归属模块。
2. 若只补账本检查点、水位或单一事实读取端口，可暂放归属模块，但必须保留 governance 语义、差异报告接口和后续迁移路径。
3. 若工程任务未确认物理落点、依赖方向、是否新增公共契约、DDL/H2、Mapper/Entity 归属和边界测试，则只能做设计、contract-only 或 dry-run，不得写入生产代码。
...
| 报表指标模块 | 归档、重放、差异和任务摘要的只读事实输入。 | 提供指标项、推荐事实来源和业务口径边界。 | 指标结果反写归档、重放、账本、交易、清结算或对账状态；复用本模块的 Manifest、水位或 checkpoint 表达报表任务进度。 |
...
| 指标项承接边界 | 交易、账本、投影、清结算、对账、外部文件 | 指标项清单、使用者、关注问题、推荐事实来源 | 在本分册设计报表指标模块内部实现、反写资金事实，或复用归档重放控制对象表达指标任务。 |
| 大数据消费边界 | 资金冷归档、Manifest、投影、清结算、对账和治理导出快照 | 治理读取授权、导出快照、Manifest 摘要、脱敏策略、digest、消费方审计记录 | 报表数仓直接扫描在线资金冷归档、反写资金事实、推进资金水位，或替代余额快照和交易重放 checkpoint。 |
...
| 普通指标快照 | 快照任务、指标水位、历史累计值、热数据实时计算、质量报告和发布审批。 | 资金指标项的推荐事实来源和只读边界。 | 指标水位、指标快照批次。 | 报表指标模块拥有；governance 只提供输入边界。 |
...
| 视图域 | 用户账单、商户账单、运营时间线、财务核对视图和指标项输入。 |
...
资金交易指标是报表指标模块承接的只读能力，不是交易、路由、钱包、账本、清结算或对账主链路的一部分。本分册只提供报表指标模块需要理解的业务输入：指标域、可能关心的指标项、使用者、关注问题和推荐事实来源。
...
| 推荐事实来源 | 交易、账本、余额投影、清结算、对账、归档和重放任务。 | 报表指标模块选择具体数据链路、存储方案、计算任务和权限模型。 |

#### 4.4.2 产品和业务关注指标项

| 指标域 | 可能关心的指标项 | 主要使用者 | 推荐事实来源 |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| 指标口径 | 指标编码、名称、口径版本、维度、聚合方式、发布状态。 | 提供资金域指标项、业务问题、推荐事实来源和只读边界。 | 资金数据治理能力提前定义报表指标内部模型。 |
...
governance/
  governance-face/
    src/main/java/com/wind/funds/governance/
      enums/
      model/
      query/
      request/
...
| ProjectionReplayTaskState | 重放任务状态 | CREATED、APPROVING、APPROVED、RUNNING、PAUSED、COMPLETED、FAILED、CANCELLED |
...
| ProjectionReplayTaskQuery | tenantId、state、viewDomain、scopeType、scopeValue、mode、timeWindow | 按租户、状态、视图域、范围、模式和时间查询。 |
| ProjectionReplayTaskDTO | taskSn、scopeType、scopeValue、mode、state、checkpoint、successCount、failedCount、skippedCount、differenceCount、audit | 返回任务范围、模式、状态、checkpoint、成功数、失败数、跳过数、差异数和审计。 |
...
| TDD-ARCH-010 大数据消费边界 | createGovernanceExportSnapshot、authorizeColdFactRead、recordExportAudit | 报表数仓、离线指标或经营分析需要消费资金历史事实。 | 通过治理读取或导出快照承接。 | 必须有范围、权限、审批、Manifest 摘要、脱敏策略、digest、消费方和审计引用；不得直接扫描在线资金冷归档，不得反写交易、账目、余额、清结算、对账或投影事实，不得推进资金水位、替代余额快照或交易重放 checkpoint。 |
...
2. 业务唯一性必须说明唯一索引、索引字段及其保护的业务规则，包括任务范围、运行、Manifest、checkpoint 或差异报告约束。
3. 普通索引必须说明索引字段和查询/排序场景，覆盖任务状态扫描、范围检索、水位定位和人工处理队列。
4. 数据约束与兼容必须说明历史数据迁移、兼容发布、数据校验与回滚、删除与归档；治理表本身的归档和删除必须保留证明链。
...
本节表由账本模块拥有。04 分册只固定 governance 编排、引用和校验时必须依赖的字段、不变量和失败边界；物理落点、Mapper、Entity 和 H2 schema 是否随工程任务落地，必须在对应任务边界中明确。
...
W2 当前只落地交易投影恢复最小切片：`t_projection_replay_task`、`t_projection_replay_difference` 和 `t_funds_transaction_projection`，字段与索引以 `database/mysql/governance/001_create_governance_tables.sql` 为准。下列统一治理差异报告、暂停/取消/失败终态、归档、余额快照、跨冷热 source 和人工处理对象仍是目标态，不得从 W2 切片外推为已实现。
...
| operator_id | varchar(64) | [X] | 发起人或系统来源 | admin |
| trace_id | varchar(128) | [ ] | 链路追踪号 | T100001 |
| state | varchar(50) | [X] | 重放任务状态 | RUNNING |
| version | int(11) | [X] | 乐观锁版本 | 1 |
...
| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_projection_replay_task_state | tenant_id, state | 任务扫描 |
| idx_projection_replay_task_view | view_domain, state | 视图域任务查询 |
...
| --- | --- | --- | --- | --- |
| 归档 | 创建归档申请并执行 dry-run/apply。 | 申请、运行和 Manifest 分离；只有校验通过的 apply 才能推进归档结果。 | 范围、审批、checkpoint、余额证明、对象摘要和冷热位置完整。 | 缺证据或摘要冲突时失败，保留差异报告，不删除热数据。 |
| 余额重建 | 以检查点和增量分录执行重建。 | 先计算影子结果，再比较金额摘要；验证通过后才能确认水位。 | 检查点覆盖模式、entry 游标、币种、周期和范围一致。 | 差异非零时不推进水位，转人工分析或缩小范围重跑。 |
| 投影重放 | VERIFY_ONLY、SHADOW_REBUILD 或 APPLY_PROJECTION。 | 只读消费事实源，模式和范围不可在运行中扩大，不重新入账。 | 指定主体/时间/视图/批次范围、原因、审批和基准摘要。 | 失败保留续跑位置和差异报告；禁止改交易、分录和余额事实。 |
| 人工处理 | 审批差异、确认恢复或取消任务。 | 人工动作只改变治理任务及审计状态，不直接修改资金事实。 | 操作者、复核人、原因、凭证、前后值和权限完整。 | 证据不足时保持挂起，禁止绕过 dry-run 或差异验证。 |

### 4.10 规则落地表

| 规则 ID | 业务表达 | 工程承接 | 验证证据 |
| --- | --- | --- | --- |
| GOV-R-001 | 归档必须先证明范围内资金事实完整且余额可重建。 | archive request、Manifest、ledger checkpoint 引用、difference report。 | dry-run、摘要校验、冷热查询和余额重建测试。 |
| GOV-R-002 | 水位只能在对应检查点验证成功后推进，不能复用归档或指标任务状态。 | ledger watermark port、checkpoint confirmation、类型隔离。 | 失败水位不变、指标任务越界 must-fail。 |
| GOV-R-003 | 投影重放只能重建只读视图，不重新路由、过账或修改历史事实。 | governance replay orchestrator、projection port、事实只读端口。 | VERIFY_ONLY/SHADOW/APPLY 测试和 forbidden facts 断言。 |
| GOV-R-004 | 任何生产修复必须有范围、审批、差异报告、续跑点和回滚口径。 | governance application、审计模型、Runbook 和权限门禁。 | 无范围/无审批失败、人工操作审计和恢复验收。 |
...
| 指标项承接 | 本分册只输出指标项、业务问题、使用者和推荐事实来源；计算、存储、展示、导出、订阅和性能由报表指标模块设计。 |
...
首个工程切片优先交付“归档 dry-run 证明闭环”，不直接进入 apply、冷热切换、全量交易投影重放或指标消费。该切片用于证明归档范围、账本余额检查点引用、Manifest 摘要、差异报告和水位不推进规则可被系统验证。

| 首切片 | 最小数据落点 | 必须证明 | 暂不承接 |
| --- | --- | --- | --- |
| 归档申请 | 归档任务或申请记录。 | 范围、对象类型、原因、审批引用、操作者和幂等键。 | 不移动真实数据，不切换冷热查询。 |
| Manifest 草案 | Manifest 或 dry-run manifest 记录。 | 对象清单、数量摘要、金额摘要、冷热目标、checksum 和差异样本。 | 不把草案当归档完成证明。 |
| 余额检查点引用 | checkpoint/watermark 只读引用。 | 检查点存在、覆盖模式明确、摘要可比对、失败不推进水位。 | 不新增独立余额快照事实，不由 governance 写余额水位。 |
| 差异报告 | difference report。 | 缺 Manifest、摘要不一致、范围冲突、权限不足和水位校验失败都能形成差异并阻断。 | 不自动修复交易、账本、余额或指标结果。 |
| 审计和告警 | 治理操作审计和告警事件。 | dry-run、失败、人工处理、导出和重跑均可追踪。 | 不建设完整数据治理平台或报表后台。 |

该首切片通过后，后续再按 apply、冷热查询、余额重建、交易投影重放和指标项消费扩展。每个扩展切片都必须单独声明公共契约、表结构、H2 schema、范围锁、回滚续跑、目标 Red 和生产停止条件。
...
| 指标项承接 | 指标域、指标项、使用者、业务问题、推荐事实来源和只读边界。 | 不设计报表指标模块内部实现；不进入资金主链路；不反写事实；不复用归档重放控制对象。 |

进入工程落地前检查：

1. 独立工程边界已逐项填写 `GOV-GATE-001` 至 `GOV-GATE-009`，并引用产品 05、TDD 第 13.0.3/13.0.4 和本分册设计锚点。
