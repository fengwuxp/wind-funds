...
| 调用目的 | 推荐入口 | 调用方 | 不适用 |
| --- | --- | --- | --- |
| VCC/卡授权、共享卡授权、支付工具授权。 | `PaymentInstrumentTransactionApplicationService.authorizeByInstrument` | 上层业务、发卡业务、通道适配。 | 不直接调用交易内核替换 canonical 入参；不绕过支付工具、资金责任、账户能力和 Spend Rule 准入。 |
| VA 或银行支付工具收款、ACH/银行已确认入金。 | `PaymentInstrumentTransactionApplicationService.receiveByInstrument` 或 `ExternalFundsEventApplicationService.consume` | 业务包已持有支付工具引用时用支付工具入口；只有外部事件事实时用外部事件入口。 | 不给 VA、外部账户或支付工具建账；不把外部 accepted 当内部成功。 |
...
| PaymentInstrumentTransactionApplicationService | 支付工具交易公共应用服务，当前承接授权、带控制预留的可信授权完成/撤销和已确认收款。授权实现内部使用 `PaymentInstrumentAuthorizationProcessor`，该处理器不是公共契约。 | 支付工具预交易快照应用服务、Spend Control admission / consumption、资金责任解析应用服务、账户能力应用服务、历史 route snapshot 和 canonical 交易服务。 | 首次授权解析当前工具、绑定、资金责任和账户能力；可信完成/撤销只读取原授权快照、原账务主体和原控制预留，在同一本地事务内分别委派资金完成/撤销并写控制消费/释放，不按当前绑定重选路。原交易本金退款继续调用 `FundsAuthorizationTransactionService#refund` 并回放原快照，不新增支付工具退款 facade；`REFUND_COMPENSATED` 是另行策略授权的控制事实。 |
...
| `PaymentInstrumentTransactionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/instrument` | `transaction/impl/src/main/java/com/wind/funds/transaction/application/instrument/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`。 | 支付工具授权、带控制预留的可信授权完成/撤销与已确认收款入口；完成和撤销回放原授权快照，并在同一本地事务中分别消费或释放关联控制预留，再委派账户主体型交易内核。原交易本金退款保持账户主体型 canonical 入口，不在本接口重复建模。 |
...
`PaymentInstrumentTransactionApplicationService#authorizeByInstrument`、`completeAuthorizationByInstrument`、`reverseAuthorizationByInstrument`、`receiveByInstrument` 和其他支付工具型 application facade 的接口位于 wallet-face，实现位于 transaction-impl：首次动作解析业务入口参数、工具能力、绑定、预算控制、资金责任和账户能力，再委派账户主体型交易内核；可信完成和撤销必须读取原授权 `RouteSnapshot` 与交易上下文，不重新读取当前绑定，并在存在 `controlReservationSn` 时分别原子记录 `CONSUMED` / `RELEASED`。`receiveByInstrument` 只把 VA、银行账户或外部钱包端点等收款入口解析为目标资金账户和外部入金引用。三类渠道事实必须分开：`FundsTransaction.channel` 只表达 `BANK_TRANSFER`、`DIGITAL_WALLET` 等交易渠道大类；请求的 `externalRailCode` 表达本次实际使用的 ACH、WIRE、LOCAL_BANK 或 DIGITAL_WALLET rail；`PaymentInstrument.channelCode` 表达稳定的支付工具接入方或提供方。交易实现按 `instrumentType + externalRailCode` 校验工具与 rail 是否匹配，再委派 `FundsDirectTransactionService#topup`；ACH、wire 和 local bank 统一归一为 `BANK_TRANSFER`，外部钱包归一为 `DIGITAL_WALLET`。route snapshot 的 `ExternalAccountRef.channelCode` 固化实际 rail，`providerCode` 固化支付工具快照中的接入方；外部事件本身没有提供方事实时不得伪造 provider。请求必须携带 `expectedBindingVersion`，缺失、未知 rail 或工具与 rail 不匹配时必须在交易内核前失败，不得生成资金交易、route、posting 或账本事实。不得把 `FundsAuthorizationTransactionAuthorizeRequest`、`FundsTransactionPayRequest`、`FundsTransactionTopupRequest` 或余额控制请求整体替换成支付工具引用入参，也不得让交易内核只感知 `PaymentInstrument` 而丢失最终资金账务主体。
...
`receiveByInstrument` 在读取当前支付工具和绑定快照前，先通过现有 `FundsTransactionQueryService` 按外部资金事实键查询已成立交易。命中 `CLOSED` 时读取原 `RouteSnapshot`，核对交易模式、交易类型、金额、币种、支付工具号和原绑定版本后返回原交易号；该路径不读取当前工具状态、当前绑定或当前资金责任。未命中时才执行当前支付工具、绑定、资金责任和账户能力准入。因此，绑定变更、暂停或解绑后的首次延迟入金必须失败且不产生资金副作用；上层只能依据不可变 VA / 收款分配证据确定原目标账户，再调用账户主体型 `topup`，资金底座不扫描绑定历史猜测路由。
...
| --- | --- | --- | --- |
| 授权过期 | 不作为交易层公共入口。 | 到期、超时或通道未返回不是可信资金事实，不生成资金交易、route、posting、LedgerEntry 或余额变化。 | `PaymentInstrumentTransactionApplicationService` 不触发 expire；如需释放占用，必须由可信 reversal、清算剩余释放、余额调整或对账差错补事实承接。 |
...
| 授权准入组合 | 已形成 `PaymentInstrumentTransactionApplicationService` 与支付工具 `RouteSnapshot` 回链初始服务流，并已补齐预交易快照和 Spend Control admission 组合。 | 组合工具准入、绑定快照、Spend Rule 决策证据、资金责任和账户能力后委派账户主体型授权内核；Spend Rule 拒绝必须在交易事实、route、posting 和账本事实前失败。 | wind-funds 后续只闭合 Spend Rule 通用契约、原路径回放和上层接入回归；VCC facade、issuer 事件消费和发卡业务策略在 Fincone 实现，不得替换 canonical 授权请求。 |
...
| 支付工具授权 | `PaymentInstrumentTransactionApplicationService#authorizeByInstrument` | 支付工具授权命令、工具准入快照、规则决策和资金责任决策。 | `FundsAuthorizationTransactionService#authorize`。 | 资金授权交易号或拒绝结果。 |
| 支付工具可信授权完成 | `PaymentInstrumentTransactionApplicationService#completeAuthorizationByInstrument` | 原授权引用、本次可信完成金额币种和稳定 `businessSn`。 | `FundsAuthorizationTransactionService#complete`、`SpendControlTransactionConsumptionApplicationService#consume`、原授权事实和原 `RouteSnapshot`。 | 原授权交易号；存在控制预留时资金完成与控制消费同事务成功或整体回滚。 |
| 支付工具可信授权撤销 | `PaymentInstrumentTransactionApplicationService#reverseAuthorizationByInstrument` | 原授权引用、本次可信撤销金额币种和稳定 `businessSn`。 | `FundsAuthorizationTransactionService#reversal`、`SpendControlTransactionConsumptionApplicationService#release`、原授权事实和原 `RouteSnapshot`。 | 原授权交易号；存在控制预留时资金撤销与控制释放同事务成功或整体回滚。 |
...
`PaymentInstrumentTransactionApplicationService` 承接首笔支付工具授权、已确认收款，以及需要联动 Spend Control 的支付工具可信授权完成和撤销；纯账户主体型完成、撤销和退款继续由 `FundsAuthorizationTransactionService` 承接。所有后授权动作必须引用原授权交易和原主账户，沿原 `RouteSnapshot` 回放，不按当前绑定关系重新选路；支付工具可信完成/撤销 facade 只做历史事实恢复和本地事务编排，不形成平行交易内核。
...
| 接口或组件 | 建议落点 | 职责 | 禁止行为 |
| --- | --- | --- | --- |
| `FundsBenefitContributionTransactionService` | 让利出资记账交易服务，落在 `transaction-face` 的 application 边界。 | 提供 `settle`、`refund` 两类命令；接收业务侧已决策让利出资结果，校验成本承担账务主体、让利承接账务主体、让利承接目标账目、金额和资金性质后，委派标准交易路由、交易事实和账本链路。目标账目必填且只允许 `CLEARING/SETTLEMENT`，不得默认猜测。 | 不新增 `FundsMarketingTransactionService`，不计算优惠金额，不判断券是否可用，不维护券包生命周期，不保存券、活动或规则来源归因，不拥有独立营销交易状态机，不直接写 route、posting、LedgerEntry 或余额投影，不要求调用方传入专业确认状态或事实摘要。 |
...
#### 4.8.4 投影实现选择与 `supports` 边界

`LedgerTransactionPostingService` 保留 `List<LedgerBalanceProjectionService> + supports(...)` 的实现选择机制。其必要性不是为当前资金账户、信用账户分别复制一套余额算法，而是为“投影事实来源相同，但投影实现责任、存储或一致性机制确实不同”的账户体系保留受控扩展点。例如账本存储迁移期间旧、新投影实现短期并存，或未来新增由独立账本模块负责的可入账主体类型。删除该机制会迫使统一过账服务直接依赖具体投影实现，也会让后续真实异构场景只能通过条件分支侵入统一过账主链。

当前默认实现统一承接 `FUNDING_ACCOUNT` 和 `CREDIT_ACCOUNT`。VCC 预付卡绑定资金子账户，VCC 共享卡绑定信用子账户，仍使用默认投影实现；多币种、账本周期、账目 profile、父子账户、Spend Rule 和支付工具差异均由现有账本 bucket、route、posting 和控制层契约承接，不构成新增投影实现的理由。余额重建是独立治理流程，也不能伪装成账户类型对应的在线投影实现。

实现选择契约：

1. `supports(...)` 只判断实现能力，必须基于稳定、不可变且已进入过账上下文的路由事实；不得查询数据库、远程服务、当前余额或账户状态，不得写状态或发布事件。
2. 账户是否存在、账户与账本主体是否匹配、账本状态、币种、周期、正常余额方向和余额约束由 `project(...)` 与账本原子更新负责；`supports(...)` 不替代业务准入和资金安全校验。
3. 每个账务主体必须唯一命中一个投影实现。零命中或多命中必须失败；零命中和实现冲突应在写入账本事实前阻断，投影阶段的账户或余额校验失败必须使同一过账事务整体回滚。
4. 如果未来仅凭 `FundsAccountId.type` 无法区分多个真实投影 provider，必须先在账户 profile 或等价不可变快照中增加明确的投影 provider/version 路由事实，再扩展选择契约；禁止通过“谁能从数据库查到账户”进行探测式路由。
5. 第二个生产实现只有在实现责任、存储或一致性机制已经不同，并具备唯一选择、回滚、重放和对账测试时才允许接入；不能仅因产品名称、国家、币种、卡类型或业务场景不同而新增。

当前代码保留上述扩展机制，但默认实现的 `supports(...)` 仍通过账户查询判断存在性，属于待收敛实现差距。目标态应改为无 I/O 的能力判断，账户存在性继续由 `project(...)` 校验。

| 案例 | 实际场景 | 预期选择与处理 | 验证重点 |
| --- | --- | --- | --- |
| 默认资金账户投影 | 用户钱包、商户资金账户或平台责任账户发生充值、支付、退款、冻结或提现。 | `FUNDING_ACCOUNT` 唯一命中默认实现；按 ledger bucket 更新余额。 | `supports(...)` 无查询；主体、币种、周期和余额约束由投影阶段校验。 |
| 默认信用账户投影 | 共享额度、信用账户授权占用、结算或退款。 | `CREDIT_ACCOUNT` 唯一命中默认实现；额度与授权账目仍按统一 normal balance 规则投影。 | 不因额度产品类型新增实现；余额不足或账目不匹配时整笔事务回滚。 |
| VCC 预付卡与共享卡 | 预付卡交易落资金子账户；共享卡交易落信用子账户，并按 route 显式处理父资金账户控制分录。 | 两类子账户均命中默认实现；不得出现 `VCC_ACCOUNT` 或卡号投影实现。 | 卡、PAN、token、Spend Rule 不参与投影实现选择；父子账户分别按真实 LedgerEntry 投影。 |
| 未支持主体 | `VCC_ACCOUNT`、支付工具、外部账户或未知主体类型被错误写入 posting。 | 零投影实现命中，在账本事实写入前失败。 | 不生成 ledger transaction、posting plan、LedgerEntry 或余额变化。 |
| 实现范围冲突 | 两个实现同时声明支持同一主体类型或同一 provider/version。 | 多实现命中，拒绝过账。 | 错误可定位；不得按 Spring 顺序、首个命中或数据库查询结果静默选择。 |
| 账户不存在 | 主体类型可由默认实现处理，但账户记录不存在或已脱离合法账户域。 | 可完成纯能力选择；`project(...)` 校验账户时失败，同一过账事务整体回滚。 | 不保留半截账本交易、分录或余额投影。 |
| 投影存储迁移 | 旧、新投影存储在受控迁移窗口内并存。 | 只有账户 profile 或不可变快照携带明确 provider/version 后，才允许两个实现按该事实唯一选择。 | 不双写、不探测查库路由；原交易重放沿用原 provider/version，具备差异核对和回滚方案。 |
...
| idx_payment_instrument_binding_status | status | 生命周期状态筛选 |
...
| MAIN-R-005 | 余额投影由账本分录派生；多投影实现只在同一事件需要异构投影策略时使用 `supports` 选择。 | ledger projection service 列表、策略解析和唯一命中门禁。 | `LedgerBalanceProjectionServiceImplTests`、策略选择和重复命中失败测试。 |
