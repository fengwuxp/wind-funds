...
1. `com.capte.funds.transaction.model.request` 下的直接交易、授权交易和余额控制 Request 只以 `ReadonlyContextVariables` 承载 `contextVariables`，表达请求创建时调用方提供的补充上下文读取视图。
...
5. 后续若扩展 Request/DTO 的核心资金语义，应新增一等字段或不可变事实快照，并在 Execution Grant 中明确公共契约范围；不得恢复 request 专属上下文封装来规避公共契约授权。

直接交易权益入口对齐：

1. 含优惠券、代金券或补贴的直接支付仍由 `FundsDirectTransactionService.pay` 承接，不新增 benefitPay、couponPay 或 voucherPay 等权益专用服务方法。
2. 含权益退款仍由 `FundsDirectTransactionService.refund` 承接，不新增 benefitRefund。退款必须基于原交易、原 route snapshot 和原权益快照回放；不得由退款请求携带当前营销规则重新计算后的权益结果作为资金事实来源。
...
4. `FundsTransactionPayRequest` 可作为权益快照进入资金指令的承载入口，目标字段是可选 `benefitSnapshot` 或等价明确字段；核心权益字段不得继续藏入 `contextVariables`。
5. `FundsTransactionRefundRequest` 更适合承载原交易、原 route snapshot、原权益快照或 replay 策略引用，而不是承载一份新的权益计算结果；若缺原权益快照，应失败或进入人工处理。
6. 幂等摘要必须纳入权益快照稳定摘要；同一业务流水、不同权益结果不能复用同一直接交易结果。
7. 券能不能退由业务层、订单层、营销权益系统或运营审批链路决策；直接退款请求若涉及权益处置，应承载业务层给出的本次退款决策引用、决策来源、规则版本和审计上下文。资金底座只校验原快照引用、剩余可处理金额和资金侧处置，不判断券可退性。

#### 4.1.2 交易应用入口分层

交易服务需要区分“业务入口形态”和“资金事实内核”。`transaction-face` 中直接交易、授权交易和余额控制的内核请求以已解析账户主体为准；外部支付工具、外部钱包端点、通道 token、内部钱包入口、外部账户和预算控制上下文属于 route 准入、快照和应用编排输入。
...
| 交易内核入口 | 资金交易编排。 | 资金账户、信用账户或平台账户角色解析结果、金额、币种、幂等键、原事实引用和审计上下文。 | 资金交易流水、授权流水、冻结/调整流水。 | 负责幂等、生命周期、route、posting、entry、projection；不得重新解释当前工具绑定来覆盖历史路径。 |
| 回放和逆向入口 | 退款、撤销、过期、拒付、退费和解冻。 | 原交易、原授权、原完成、原 route snapshot 或冻结来源。 | 逆向资金事实或失败原因。 | 优先使用原快照，不能按当前支付工具绑定或当前资金责任关系重新选路。 |

授权交易是双层入口的首要适用场景：上层业务自然持有卡、VA、外部钱包端点、共享卡、内部钱包入口和外部授权引用。外部工具入口必须先校验工具状态、方向、币种、绑定版本、使用人、预算组、Spend Rule、资金责任唯一性和账户能力；内部钱包、信用额度或权益入口必须先解析为主体或快照。只有授权批准时，内核才基于解析后的资金账户、信用账户或平台账户角色生成 `AVAILABLE -> AUTHORIZATION` 路径。授权拒绝、支付工具不可用、入口解析失败、规则拒绝、资金责任缺失或不唯一时，只记录拒绝事实和原因，不生成 route、posting 或 ledger entry。

当前代码中的 `FundsAuthorizationTransactionService.authorize(FundsAuthorizationTransactionAuthorizeRequest)` 仍属于账户主体型内核入口；后续如要开放支付工具型授权入口，应新增或调整应用层契约并通过 Execution Grant 明确公共契约、Request/DTO、幂等摘要、敏感上下文、路由快照和回归测试范围，不得在未授权时直接替换现有内核请求字段。
...
| 结算后退款 | settleRefund | FundsAuthorizationTransactionRefundRequest | 普通授权链退款返回授权交易流水号、退款明细流水号、累计退款金额；无授权退款返回退款资金事实和外部原事实审计摘要 | 普通退款引用原完成；无授权退款必须显式 `NO_AUTH` 或等价模式，并引用外部原消费、原完成或差错凭证、外部原事实类型、退款原因、退款凭证、操作者和审计 | 普通退款使用原完成引用 + 退款业务流水 + 请求摘要；无授权退款使用外部原事实引用 + 外部原事实类型 + 退款业务流水 + 请求摘要 | 累计退款超过已完成金额失败；无授权退款缺模式、缺外部原事实类型、缺原因、缺凭证或缺审计失败；不得补造授权占用，不得携带或查询内部授权流水。 |
| 结算后拒付承接 | settleRefund | FundsAuthorizationTransactionRefundRequest 携带拒付原因、凭证、外部引用和审计上下文 | 授权交易流水号、拒付承接明细流水号、争议/拒付展示标识 | 仅用于已结算授权争议、扣回或差错追偿，不用于授权拒绝 | 原完成引用 + 拒付或争议业务流水 + 请求摘要 | 不要求落到独立 chargeback 方法；不得把授权拒绝写成拒付；不得把拒付压缩成不可区分的普通退款。 |
...
4. settleRefund 同时承接普通授权链退款、无授权直接退款和拒付承接；普通授权链退款继续要求原完成引用，无授权退款必须显式 `NO_AUTH` 或等价模式，携带外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者和审计，且不得携带或查询内部授权流水；资金路径可以复用退款路径，但原因、凭证、外部引用和投影展示语义必须保留。
5. 服务返回值只表达资金底座已经接受并处理的资金事实，不承诺通道、卡组织或上层业务事件最终状态。

`B4-DISPUTE-SEMANTIC-ALIGNMENT` 当前只进入准入候选态，未授权编码。若后续确认该 Grant，系统实现优先补 `settleRefund` 争议语义的查询、投影、审计和幂等摘要可区分性；既有 `FundsAuthorizationTransactionService#chargeback` 只能作为兼容、显式事件或内部适配资产复用，不在首轮扩展为公共目标态主入口。需要完整 dispute case、representment、清结算追偿、VCC processor 或外部卡组织规则时，必须另起专项 Grant。
...
4. reversal、expire 和普通 settle 必须引用原授权事实；普通 settleRefund 必须引用原完成事实；settleRefund 无授权退款模式必须显式 `NO_AUTH` 或等价模式，引用外部原消费、外部原完成或差错凭证、外部原事实类型、退款原因、退款凭证、操作者和审计，不得补造内部授权占用，不得携带或查询内部授权流水；拒付通过 settleRefund 携带原因、凭证和审计上下文承接；settle 强制完成模式必须有受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计，不能把普通完成的原授权流水当作强制完成凭证。
...
2. `FundsAuthorizationTransactionAuthorizeRequest` 可在后续授权工程任务中新增可选 `benefitSnapshot` 或等价明确字段，用于固化 `HOLD_ONLY` 占用引用；授权拒绝、支付工具准入失败或余额不足时，不核销权益，不生成 route、posting 或 entry。
3. `reversal`、`expire`、`settle` 和 `settleRefund` 原则上读取原授权事实、原 route snapshot 和原权益快照，不接收当前重新计算的权益结果；完成核销、撤销释放、过期释放和退款冲回必须沿原快照处理。
4. `expire` 是授权生命周期既有入口，已由 B4-TRX-EXPIRE 补齐事件和服务入口；它不是优惠券或权益设计引入的新增入口，后续不得借权益任务再次扩大公共契约。
5. `settle` 强制完成、`settleRefund` 无授权退款、拒付承接和代码中既有 `chargeback` 方法属于授权目标态的独立对齐问题；权益设计只要求这些路径能引用原权益快照并保留原因、凭证和审计，不要求新增权益专用方法。
6. 授权后续事件中的释放、核销、返券、不返券或补贴冲回决策来自业务层或营销权益系统；资金底座只消费决策结果和原授权权益快照，不在 reversal、expire、settle 或 settleRefund 内重新判断券是否可退。
...
| 资金账户余额调整 / 信用账户额度调整 / 预算控制额度调整 | adjust | FundsBalanceAdjustRequest；编码落地时可按资金账户、信用账户、预算控制对象补充专用字段或专用 Request | 调整流水号、调整后目标账目余额摘要、LIMIT / AVAILABLE 摘要或预算控制视图摘要 | 具备调整对象、审批、凭证或规则版本、原因、目标账目或控制窗口和操作者；资金账户余额调整必须有差错、运营修正或财务调整来源 | 调整业务流水 + 请求摘要 | 缺审批、缺来源、错币种、破坏已授权或已使用约束、违反负余额策略时不生成分录；预算控制调整不得生成预算组分录。 |
...
钱包账户是能力域，不是单一实体。系统层以 `FundingAccount`、`CreditAccount`、`BudgetGroup`、`PlatformFundingAccountRole`、`PaymentInstrument`、`SpendSubjectFundingRelation` 和余额查询服务共同承接钱包产品能力；其中 `FundingAccount` 只表示真实资金账户或平台责任资金账户。统一路由和入账时使用 `FundsSubjectType` / `SubjectRef`；外部支付工具和工具快照使用 `PaymentInstrumentRef`，内部余额钱包、信用额度、返利钱包或商户钱包等业务入口先解析为 `SubjectRef`、`BenefitSnapshot`、`FundingAllocationDecision` 或等价不可变快照，不得用 `FundingAccount` 兼容所有主体，也不得把所有业务入口强行包装成 `PaymentInstrument`。
...
应用层可以通过 Request 字段承接内部余额钱包、信用额度、返利钱包、商户钱包、银行卡、VA、外部账户和通道 token 等业务入口参数。但该参数只属于 application/use-case facade，不是交易内核主体，也不要求内部账户都包装成 `PaymentInstrument`。facade 的职责是把入口解析为 `SubjectRef`、`PaymentInstrumentRef`、`BenefitSnapshot`、`FundingAllocationDecision` 或等价不可变快照，再委派交易、路由和账务内核。
...
| FundingResponsibilityResolutionApplicationService | 将工具、使用主体、预算组、Spend Rule 和平台角色解析成唯一资金或额度责任主体。 | SpendSubjectFundingRelationService、PlatformFundingAccountService、账户服务。 | 输出 `FundingAllocationDecision` 或等价不可变决策；预算组和 Spend Rule 不得成为最终责任主体。 |
| AuthorizationAdmissionApplicationService | 面向卡、VCC、共享卡、外部钱包端点、通道 token 或内部支付方式选择等入口完成授权前准入。 | 支付工具能力应用服务、资金责任解析应用服务、Spend Rule 控制、账户能力查询。 | 输出批准/拒绝和快照输入；批准后委派授权交易内核，拒绝无 route/posting/entry。 |

新增 application facade 的 Java 落点必须遵循钱包承接用例、交易承接内核的依赖方向。首选落包如下；具体写入仍以 Execution Grant 为准。

| 接口 | face 包 | impl 包 | Request/DTO 包 | 说明 |
| --- | --- | --- | --- | --- |
| `PaymentInstrumentCapabilityApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/instrument` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/instrument/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`，必要时加 `instrument` 子包。 | 支付工具能力准入、绑定版本和敏感字段边界。 |
| `FundingResponsibilityResolutionApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/funding` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/funding/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`，必要时加 `funding` 子包。 | 解析最终资金或额度责任主体；B2-FR 必须先裁决字段策略。 |
| `AuthorizationAdmissionApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/instrument` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/instrument/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`。 | `authorizeByInstrument` 或等价授权准入入口；只委派交易内核。 |
| `InstrumentTransactionLifecycleApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/instrument` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/instrument/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`。 | 清算、释放、退款、拒付的外层用例入口；按原交易和原快照委派交易层。 |
| `VccPrepaidFundingApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/vcc` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/vcc/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`，必要时加 `vcc` 子包。 | 预付卡入金确认、系统内充值和退回；P2 独立切片。 |
| `VccSharedCardTransactionApplicationService` | `wallet/wallet-face/src/main/java/com/capte/funds/wallet/application/vcc` | `wallet/wallet-impl/src/main/java/com/capte/funds/wallet/application/vcc/impl` | `com.capte.funds.wallet.model.request`、`com.capte.funds.wallet.model.dto`，必要时加 `vcc` 子包。 | 共享卡授权、清算和逆向的 VCC 场景编排；不拥有账本主体。 |

不新增顶层 `com.capte.funds.instrument` 包。原因是支付工具入口需要读取钱包侧工具、绑定、资金责任、账户能力和 Spend Rule 控制上下文，再委派 `transaction-face` 的 canonical 服务；若落到 `transaction-impl`，会反向依赖钱包资源服务并破坏模块边界。

若后续新增 `authorizeByInstrument` 或其他支付工具型 application facade，它们只能位于 application/use-case facade：先解析业务入口参数、工具能力、绑定、预算控制、资金责任和账户能力，再委派账户主体型交易内核。不得把 `FundsAuthorizationTransactionAuthorizeRequest`、`FundsTransactionPayRequest` 或余额控制请求整体替换成支付工具引用入参，也不得让交易内核只感知 `PaymentInstrument` 而丢失最终资金账务主体。
...
交易层需要继续完善账户主体型能力，而不是新增支付工具型交易内核。后续 B3/B4/B5/B6 可以在 `transaction/transaction-face/src/main/java/com/capte/funds/transaction/application` 下扩展或完善 canonical 服务、请求、事件和状态机；但这些契约的主体仍必须是已解析的资金账户、信用账户或平台账户角色解析后的平台资金账户。

| 交易层能力 | 允许完善的包 | 必须保持的边界 | 对支付工具场景的承接方式 |
| --- | --- | --- | --- |
| 授权过期 | `com.capte.funds.transaction.application`、`com.capte.funds.transaction.model.request`、`transaction-impl` 对应实现。 | 基于原授权和账户主体释放；不接收卡作为余额主体。 | `InstrumentTransactionLifecycleApplicationService` 可触发 expire 或 release 命令。 |
...
| 无授权直接退款 | 同上，必要时扩展 refund 请求。 | 必须显式 `NO_AUTH` 或等价模式，有外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者和审计；不得补造内部授权，不得携带或查询内部授权流水。 | 支付工具引用只用于定位外部原事实和投影归因。 |
| 拒付和争议扣回 | 同上。 | 必须与授权拒绝、普通退款可区分；不得重复损失。 | 支付工具 facade 可使用 `DisputeInstrumentRefund` 等候选命名表达争议退款，底层默认委派 `settleRefund` 并回放原交易路径；独立 chargeback 公共入口需另行 Grant。 |
| 余额控制调账 | `FundsBalanceControlService`、余额控制请求和实现。 | 调整主体是账户或控制视图；必须有审批、差错单、凭证和审计。 | 共享卡调额多数落绑定、预算或 Spend Rule；只有真实资金/额度变化才委派余额控制。 |
| 原路径回放和查询解释 | route replay、transaction projection、查询 DTO。 | 重放只读；缺快照进入差错或人工处理。 | 卡账单、使用人账单和预付资金历史由支付工具引用过滤投影生成。 |

| P2 场景能力包 | 适配层职责 | 委派的 P1/P0 能力 | 不进入资金底座内核的内容 |
| --- | --- | --- | --- |
| VCC 预付卡 | 解释卡授权、外部充值、预付资金模式和清算结果；输出工具快照、外部引用和资金责任主体。 | 授权交易、直接交易、清结算、对账、归档。 | 卡组织协议、PAN/CVC、处理商账户状态、卡内余额。 |
| VCC 共享卡 | 解释共享卡绑定、成员/卡组权限和调额控制。 | 支付工具绑定、资金责任解析、Spend Rule、必要时授权准入。 | 共享卡余额、共享卡账本、调额即入账。 |
...
当前代码可作为资源管理和契约基线，但不能直接声明支付工具交易准入或 Spend Rule 控制生产可用。系统层必须把“资源服务存在”和“应用用例闭环”分开裁决。

| 系统面 | 当前代码观察 | 目标态要求 | 编码准入影响 |
| --- | --- | --- | --- |
| 支付工具资源服务 | 已有 `PaymentInstrumentService`、支付工具表、绑定表、绑定历史和服务级测试。 | 继续作为管理后台和内部协作能力。 | 可作为 B2 局部基线，不等于交易准入 Done。 |
| 支付工具能力准入 | 尚未形成 `PaymentInstrumentCapabilityApplicationService` 或等价 application facade。 | 一次性校验状态、方向、动作能力、币种、有效期、绑定版本和敏感字段，输出不可变准入快照。 | B2 Round 0 首批 Red 必须证明失败无 route/posting/entry。 |
| 资金责任解析 | 现有关系仍以 `fundingAccountId` 为主要目标字段。 | 解析结果必须是资金账户、信用账户或平台角色解析后的平台资金账户；预算组和 Spend Rule 不能成为结果主体。 | B2 必须先决定 `targetSubjectType + targetSubjectId` 或明确 funding-account-only。 |
| Spend Rule 控制 | 未形成独立 Spend Rule 定义、版本、决策日志、控制活动和预算控制投影。 | 规则定义、规则版本、决策结果、拒绝原因、预留、释放、调整和审计必须可追溯。 | B2/B5 需独立 Execution Grant，不得混入 A1。 |
| 授权准入组合 | 尚未形成 `AuthorizationAdmissionApplicationService` 或 `authorizeByInstrument` 契约。 | 组合工具准入、绑定快照、Spend Rule、资金责任和账户能力后委派账户主体型授权内核。 | B4 独立授权；不得替换 canonical 授权请求。 |
...
| --- | --- | --- |
| 适用条件 | 本轮只验证现有支出主体与资金账户关系，不声明信用账户、平台资金账户或组合资金责任已经生产可用。 | B2-FR 需要正式支持信用账户、平台角色解析后的平台资金账户，或未来多资金主体责任分配。 |
...
| `Spend Control Activity` | Spend Rule 执行过程中的命中、拒绝、预留、占用、释放、调整等活动事实。 | 可作为交易投影输入和审计证据，不替代资金交易或账本交易。 |

建议的 Round 0 顺序：

1. 先补支付工具能力准入 facade 契约和失败无副作用 Red，不写 route、posting 或 entry。
2. 再补资金责任解析的目标主体裁决，明确是否迁移为 `targetSubjectType + targetSubjectId`。
3. 再补 Spend Rule 控制事实：规则定义、版本、决策日志、Spend Control Activity、预算预留和释放。
4. 最后补授权准入组合 facade：从支付工具引用进入，批准后委派账户主体型授权内核，拒绝时只留下可审计拒绝事实。

本节仅确立系统分层和能力方向，不授权新增 Java 接口、包名、表字段或测试代码。实际落地时应先由 B2 或 B4 的 Execution Grant 指定是否新增 application 包、是否保留现有 service 命名、是否新增 Request/DTO 和是否调整交易模块依赖方向。
...
| --- | --- | --- | --- | --- |
| 资金账户 | 管理真实资金、待清算、平台责任、手续费、预收待付或差错责任主体。 | `FundingAccountService`、`FundsSubjectBalanceQueryService`、交易/清结算服务。 | owner、币种、账户能力、账本 profile、余额桶、负余额策略和审计来源。 | 先创建资金账户并初始化账本；交易、清结算或调账通过 route/posting 使用该主体。 |
...
| 支付工具 | 管理卡、VA、外部钱包端点、通道 token、虚拟卡和共享卡工具快照。 | `PaymentInstrumentService`、PaymentInstrumentCapabilityApplicationService、route resolver。 | 状态、方向、动作能力、币种、有效期、绑定版本、脱敏引用、敏感字段和资金责任唯一性。 | 外部工具先注册为支付工具，再绑定到使用主体或内部责任主体；内部余额钱包、返利钱包和商户钱包先解析为主体或权益快照，不注册成工具。交易发生时固化 `PaymentInstrumentRef` 或入口解析快照。 |
| 支出主体资金责任解析关系 | 连接信用账户、预算组、Spend Rule、工具或使用主体与最终资金责任主体；兼容现有服务名时仍对应 `SpendSubjectFundingRelationService`。 | `SpendSubjectFundingRelationService`、route resolver。 | 关系类型、优先级、默认标识、币种、有效期、规则版本和多命中冲突。 | 配置某个使用主体或工具如何解析到资金账户、信用账户或平台账户角色；预算组和 Spend Rule 只参与控制和解释，不承担资金责任。 |

VCC 场景中，调用方不能直接要求“创建预付卡资金账户”“创建共享卡信用账户”或“创建预算组账本”。正确顺序是：注册卡工具，建立绑定、预算 scope、Spend Rule 和资金责任解析关系，必要时创建资金账户或信用账户作为内部责任主体，再由授权交易解析出唯一 `FundingAllocationDecision`。

Highnote 模式对本系统的工程映射如下：`FinancialAccount` 对应 wind-funds 的资金账户、信用账户或平台角色解析后的平台资金账户；`Ledger` 和 `LedgerEntry` 对应账本 bucket 和不可变分录；`PaymentCard` 对应 `PaymentInstrumentRef` 和支付工具绑定；`FinancialAccountActivity` / transaction feed 对应交易投影和使用者账单。该映射只确认分层，不引入 Highnote 对象名或外部 SDK 依赖。共享卡、多卡共享账户和 authorized user 场景在 wind-funds 中通过同一内部责任主体 + 多个工具绑定 + 控制快照 + 交易投影过滤表达；只有独立余额、独立授信、独立账期、独立还款或独立资金责任成立时，才创建独立内部责任主体。
...
VCC 卡、虚拟卡、卡 token、预付虚拟卡和共享卡在系统层都先进入 `PaymentInstrument` 体系，而不是进入账户或账本体系。预付卡只改变资金责任解析和责任账户选择，不改变工具不记余额的原则；共享卡只改变绑定、使用人、预算组和资金责任快照，不新增“共享卡账户”或“共享卡账本主体”。
...
| --- | --- | --- | --- |
| VCC 普通授权 | `PaymentInstrumentRef` + `FundingAllocationDecision`。 | 卡工具、脱敏展示号、绑定版本、资金账户/信用账户或平台账户角色、预算组上下文、Spend Rule 快照、规则版本和外部授权引用。 | 卡、token、持卡人、预算组、Spend Rule 或外部卡账户成为 `LedgerEntry.subject`。 |
| prepaid virtual card | 工具仍在 `PaymentInstrument`；预付资金责任由 `SpendSubjectFundingRelationService` 解析到内部主体。 | 预付资金责任、责任账户、财务确认引用、退款处置和规则版本。 | 把预付卡写成内部余额账户，或把储值券、礼品卡和预付代金券混入 VCC 工具模型。 |
| shared card | 一个工具可绑定使用人、部门、项目、预算组和兜底资金责任主体。 | 使用人、绑定角色、绑定版本、预算组、资金责任解析关系、优先级、默认标识和规则版本。 | 新增共享卡资金账户，或在逆向事件中按当前绑定重新解释历史授权。 |
...
以上命名是系统语义，不冻结 Java 包名或 DTO。进入编码前必须在 Execution Grant 中确认包、接口、Request/DTO、错误码和依赖方向。
...
| `DisputeInstrumentRefund`（候选命名） | 拒付、争议扣回、追偿。 | originalTransactionSn、disputeRef、disputeAmount、stage、reason、voucherRef、requestDigest。 | 默认委派 `settleRefund` 保留争议语义；是否生成独立扣回、费用或追偿事实由后续清结算/争议专项确认。 | 不与普通 refund 混成不可区分结果，不允许重复损失；不把 B4 首轮语义反推为独立 chargeback 公共入口。 |
...
stateDiagram-v2
    ...
    SETTLED --> CHARGEBACK: dispute case candidate
    CHARGEBACK --> REPRESENTED: representment
    REPRESENTED --> SETTLED: dispute won
    REPRESENTED --> CHARGEBACK: dispute lost
    DECLINED --> [*]
    RELEASED --> [*]
    REFUNDED --> [*]
```
...
| 状态 | 共享卡口径 | 预付卡口径 | 账务要求 |
| --- | --- | --- | --- |
| ADMITTED | 工具、绑定、使用人、预算组和规则上下文已固化。 | 工具、预付责任、确认引用和责任主体已固化。 | 无账务副作用，可进入授权内核。 |
| DECLINED | 拒绝原因可按卡、使用人、预算组和规则版本解释。 | 拒绝原因可按余额不足、未确认入金或责任缺失解释。 | 不生成 route、posting、LedgerEntry。 |
| AUTHORIZED | 同一账户可被多张共享卡占用，卡侧按投影拆分。 | 预付责任主体 AUTHORIZATION 占用。 | 只做授权占用，不代表清算消费。 |
| SETTLED | 按原绑定快照归因，不按当前绑定更新。 | 按原预付责任主体消费。 | 核销授权占用，生成实际消费和费用。 |
| RELEASED | 原主体释放剩余占用。 | 原预付责任主体释放剩余占用。 | 同主体释放，不跨主体转移。 |
| REFUNDED | 原路径回补，当前卡状态只影响展示。 | 原预付责任主体回补。 | 必须引用原 route snapshot。 |
| CHARGEBACK | 按原交易和责任主体扣回或追偿。 | 原预付责任主体承担争议扣回或追偿。 | 与 refund 独立，防重复损失。 |

账务矩阵：

| 业务动作 | 借方/贷方语义 | 余额桶变化 | 分录主体 | 投影维度 |
| --- | --- | --- | --- | --- |
| 共享卡授权批准 | 同一内部责任主体内授权占用。 | `AVAILABLE -> AUTHORIZATION`。 | 资金账户或信用账户。 | cardRef、cardholderRef、departmentRef、budgetGroupRef、spendRuleRef。 |
| 共享卡授权拒绝 | 无资金事实。 | 无。 | 无。 | 拒绝原因、规则版本、工具快照。 |
| 共享卡清算 | 授权占用核销为消费、待清算或费用。 | `AUTHORIZATION -> CLEARING/SETTLEMENT/FEE`。 | 原资金账户或信用账户。 | 原工具和绑定快照。 |
| 共享卡退款 | 原路径回补。 | 按原交易语义回补 AVAILABLE 或冲减应收。 | 原资金账户或信用账户。 | 原卡、原使用人、当前展示提示。 |
| 预付入金确认 | 外部确认或内部转账增加责任主体可用余额。 | 目标 `AVAILABLE` 增加。 | 资金账户或平台责任账户。 | fundingRef、source、confirmationRef。 |
| 预付卡授权批准 | 预付责任主体授权占用。 | `AVAILABLE -> AUTHORIZATION`。 | 预付资金责任账户。 | cardRef、prepaidResponsibilityRef。 |
| 预付卡清算 | 预付责任主体消费。 | `AUTHORIZATION -> CLEARING/SETTLEMENT/FEE`。 | 原预付责任账户。 | clearingRef、merchant、fee。 |
| 预付资金退回 | 原责任主体资金转出或提现。 | `AVAILABLE` 减少，目标账户增加或出款在途。 | 原预付责任账户和目标主体。 | unloadRef、approvalRef、externalConfirmationRef。 |
...
| refund 与 chargeback 重复 | 冻结后续自动处理，标记重复损失风险。 | 争议运营和财务确认。 |
...
| 权益组件消费 | 读取 `FundsInstructionSpec.benefitSnapshot` 中已决策的权益结果，只按 `ledgerEffect`、`fundingNature` 和承担方生成或跳过权益 route leg。 |
...
4. 含权益交易目标态应保存 `benefitSnapshot` 或等价权益快照摘要；如果第一阶段暂不改 `RouteSnapshotSpec`，必须至少能在交易事实、指令快照或 route context 中追溯 `benefitSnapshotId`、组件摘要和退款处置。

#### 4.5.3 权益快照能力落点

权益快照属于资金指令事实输入，不属于路由自有模型。路由只消费它，不拥有营销规则。

| 能力层 | 工程落点 | 允许做什么 | 不做什么 |
| --- | --- | --- | --- |
| 契约承载 | `core` 的 `FundsInstructionSpec`、权益 Spec、不可变模型和枚举；DSL 契约测试。 | 让资金指令可携带 `benefitSnapshot`；无权益交易遵循空值语义；验证金额闭合、组件类型和退款处置。 | 不要求 RouteResolver、PostingAssembler、交易表或 route snapshot 立即完整消费。 |
| 路由账务消费 | `RouteResolver`、`RouteSnapshotFactory`、`LedgerPostingAssembler`、`DefaultRouteReplayService`。 | 商户让利 `NO_LEDGER` 不生成 leg；平台补贴、储值券和合作方补贴按资金性质生成独立资金影响；退款和授权释放读取原权益快照。 | 不把营销规则、券可用性判断或券生命周期推进放进资金底座。 |
| 清结算对账投影 | 清分明细、清算候选、对账差错、交易投影、运营时间线、财务核对视图和指标项输入。 | 展示和核对商户让利、平台补贴、代金券核销、补贴冲回、不退券和权益差异。 | 不用清结算、指标结果或报表模块输出反写交易事实、账本分录或营销权益状态。 |

能力门禁：

1. 契约承载只表示 DSL 和 JSON 契约可承载权益快照，不得宣称含权益交易、退款、授权后续事件、清结算或对账已具备生产能力。
2. 路由账务消费落地前，必须确定权益快照持久化位置：route snapshot、交易事实快照、独立权益快照表或等价不可变存储，至少一个作为 replay 和对账的事实来源。
3. 路由账务消费的幂等摘要必须包含权益快照稳定摘要；稳定摘要至少覆盖 `benefitSnapshotId`、`benefitGroupSn`、组件序列、组件金额、`closureRole`、`ledgerEffect`、`fundingNature`、规则版本和退款处置。
4. 路由账务消费不得只在 `contextVariables` 写入权益摘要；生产长期回放必须依赖 route snapshot、交易事实快照、独立权益快照表或等价不可变存储。
5. 清结算对账投影不能从交易投影、清结算汇总或报表反推权益组件；缺原权益快照时只能失败、跳过或进入人工处理。
...
| 权益快照事实源 | `RouteSnapshotSpec`、交易事实快照、独立权益快照表或等价不可变存储之一。 | `transaction-impl`、`ledger-impl`、归档重放、清结算、对账。 | 只能停留在 contract-only，不进入 route/posting 生产消费。 |
...
| 历史无权益快照处理 | 失败、人工处理、迁移补录或只读差错，并记录触发人、处理依据、补录来源、复核人、影响范围、是否可撤销和重放校验。 | 逆向交易、归档重放、对账差错、运营后台、审计查询。 | 默认失败或人工处理。 |
| 补充权益事实模型 | 是否允许追加 supplemental benefit fact；必须声明载体、补录来源、审批、复核、digest、版本、适用范围、撤销关系和 apply 前校验。 | 逆向交易、归档读取、治理重放、对账差错、运营后台和审计查询。 | 未确认时不得补录，只能失败、跳过或人工处理；补充事实不得覆盖原始事实。 |
...
| compensation handler | 处理主成功伴随失败、伴随成功主失败、超时未知和补偿失败，生成冲正、重试、差错或人工处理证据。 | 部分成功后静默展示普通成功、清结算 Done 或归档 apply Done。 |
...
| supplemental fact verifier | 在退款、差错、对账、归档读取或治理重放 apply 前校验证据、digest、范围和撤销状态。 | 把补充事实当成当前营销规则结果，或在证据失效时继续 apply。 |
...
2. `ImmutableFundsInstructionSpec` 若增加 record 字段，会影响直接构造、序列化和测试夹具；进入编码前必须扫描调用点。
3. 只在指令上保留权益快照不足以支撑长期 replay；目标态应进入 route snapshot、交易事实快照或等价不可变存储。
4. 幂等摘要必须包含权益快照稳定摘要；同一业务流水、不同权益结果不能被当作同一请求复用。
5. 储值券、礼品卡、预付券可能涉及负债、预收待付、客户资金、沉淀收益或合同口径，编码前必须有财务、会计、合规或业务确认。
6. `contextVariables` 只能作为兼容迁移通道，且只能放可追溯引用或稳定摘要；组件金额、资金责任、退款处置和当前营销规则不得作为长期回放依据。
7. 历史权益快照迁移补录只能生成追加型 supplemental benefit fact 或等价补充事实，必须记录补录来源、审批、复核、digest、版本、关联原交易和撤销关系；不得覆盖原资金交易、原 route snapshot 或原账本事实。
8. 专业确认状态必须在最终写入、退款、清结算确认、归档 apply 或治理重放 apply 前重新校验；预检通过后确认过期、撤销或范围变更时必须阻断或转人工，不得沿用旧预检结果。
...
| RouteSnapshot / `paymentInstrumentRef` | 支付工具类型、脱敏引用、绑定版本、路由版本和交易发生时的入口解释。 | 按当前支付工具状态重新选路、读取完整敏感字段、把支付工具当作账务主体。 |
...
| period_id | varchar(30) | [X] | 预算账本周期 ID；用于精确定位预算组余额 bucket | 2026-05 |
...
- 共享卡绑定必须能表达工具、使用主体、预算组、资金账户或信用账户之间的关系；多关系命中时必须由资金责任解析关系和优先级决策出唯一结果。
- prepaid virtual card 绑定不得以卡工具自身作为资金责任主体；必须关联内部预付资金责任主体或待专业确认后阻断。
...
目标态说明：当前实现和 H2 schema 仍以 `funding_account_id` 命名目标字段；进入工程落地前需要由 `资深架构师` 联动确认是否迁移为 `target_subject_type + target_subject_id`，或保留兼容字段并在 service/DTO/DDL/TDD 中显式声明只能解析到资金账户。本文档先固化产品和系统目标口径，不视为已授权修改生产代码、测试 schema 或数据库迁移。

工程裁决必须在以下两个切片中二选一，不能混用：
...
| funding_account_id | varchar(30) | [X] | 兼容字段：当前实现中的目标资金账户 ID 或业务号；在 `funding-account-only` 切片中不得承载信用账户、平台角色或预算组；目标态如支持信用账户或平台账户角色，需迁移为目标主体类型 + 目标主体 ID。 | FA100001 |
| currency | varchar(10) | [X] | 币种 | CNY |
...
| uk_spend_subject_funding_rel_subject | spend_subject_type, spend_subject_id, funding_account_id, currency, relation_type | 同主体、目标资金责任主体、币种、关系类型唯一；目标态迁移后应改为 target subject 唯一约束。 |
...
| idx_spend_subject_funding_rel_funding | funding_account_id | 查询当前资金账户被引用关系；目标态迁移后应改为 target subject 查询索引。 |
...
- `funding_account_id` 按字符串保存；若目标态改为 `funding_account_sn` 或 `target_subject_type + target_subject_id`，必须同步表结构、Entity、Mapper、测试数据、查询契约和迁移兼容策略。
...
- source_type：RECONCILIATION、OPERATION、FINANCE、RISK、SYSTEM_RULE。
...
| 含权益交易缺稳定权益快照 | 正向生产准入失败；逆向、清结算、对账或投影重放只能失败、跳过或进入人工处理，不按当前营销规则补算。 |
| 权益快照摘要与业务幂等记录不一致 | 拒绝复用原交易结果，返回摘要冲突或进入差错；不得覆盖原交易事实。 |
...
| 伴随指令部分成功 | 必须生成冲正、补偿、差错单或人工处理证据；不得展示普通成功，不得进入清结算 Done 或归档 apply Done。 |
| 补充权益事实缺证据或已撤销 | 退款、对账、归档读取或治理重放必须失败、跳过或进入人工处理；不得覆盖原事实或继续 apply。 |
| 审计证据包失效 | 最终入账、退款、清结算确认、归档 apply 或治理重放 apply 前阻断或降级；dry-run 不能替代 apply 证据。 |
...
| 权益金额组件 | 无权益空值语义、商户券 no-ledger、平台补贴 posting-required、平台券不补足商户、储值券资金性质、授权占券、不退券但冲补贴、部分退款分摊、独立伴随指令原子性、补充权益事实、审计证据包 TOCTOU、缺原权益快照失败或人工处理。 |
...
| P1 | 权益快照契约 | `FundsInstructionSpec.benefitSnapshot`、权益 Spec、不可变模型、枚举和 DSL 契约样例。 | `AC-BEN-*`、`RED-050` 至 `RED-057`、`DSL-BENEFIT-*` 和 `TDD-BEN-*` 已映射；工程任务明确允许公共契约和枚举变更；不要求本切片修改 route snapshot、表结构或完整 route/posting 消费；交付结论只能写“契约承载完成”，不得写“含权益生产链路完成”。 |
| P1 | 权益路由账务消费 | 商户券不入账、平台补贴独立 leg、储值券负债口径、授权占券、退款补贴冲回、posting context。 | route snapshot、交易事实快照、独立权益快照表或等价不可变存储至少一个可用于 replay；幂等摘要覆盖权益快照稳定摘要；posting plan 独立平衡；不退券和不冲补贴分开；零实付、储值券和平台补贴账户角色的待确认项已关闭或明确不在工程任务范围。 |
...
5. 含权益生产链路必须能证明原权益快照可长期回放，不能只依赖请求对象内存态、当前营销系统或投影汇总。
