...
| 层级 | 职责 | 接口契约口径 | 事务边界 |
| --- | --- | --- | --- |
| `wallet.application` | 承接支付工具、VA、VCC、全球账户、ACH 等业务入口，完成准入、绑定、账户能力、Spend Rule、外部引用和预交易快照组合。 | 已有 `authorizeByInstrument`、`receiveByInstrument`、`payOutByRail` 和 `ExternalFundsEventApplicationService#consume` 最小入口；`payOutByRail` 当前仅完成服务层契约和未接内核 fail-fast guard，不代表真实出款 Done；`consume` 当前仅完成外部资金事件消费契约和未接归一资金事实内核 fail-fast guard，不代表真实 ACH/银行事件消费 Done。入参必须包含业务流水、场景、金额、币种、工具 / rail / 外部事件引用和可解析账户主体，收款工具和出款 rail 入口必须携带调用方期望的绑定版本。 | 失败应停在交易事实前；通过后委派 transaction canonical service；收款工具入口应在 route snapshot 固化脱敏 `PaymentInstrumentRef` 和外部账户引用；出款 rail 和外部事件真实接入必须另行证明提现/出款、在途、费用、入账、扣账、退款、撤销、调账、清结算和对账边界。 |
