...
3. 授权拒绝无 route、posting、LedgerTransaction、LedgerEntry 和余额变化。
...
| `RouteParticipant` / `RouteLeg` | 最终资金或额度主体、金额、账目、路径以及平台角色解析结果；参与方可附带直接父账户关系快照。 | 把账户层级、支出控制范围、Spend Rule 或支付工具当作隐式资金分配结果。 |
...
| MAIN-R-002 | 授权批准占用 AVAILABLE，拒绝不生成 route、posting、LedgerEntry 或余额变化。 | authorization application、route resolver、ledger posting。 | 授权成功/拒绝流程测试和 forbidden facts 断言。 |
