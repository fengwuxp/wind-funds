...
- 当前版本：v1.0
- 文档状态：Review
- owner：wallet owner、transaction owner、ledger owner、架构 owner、测试 owner
- 更新日期：2026-07-17
- 变更摘要：按最新系统分析设计模板补齐产品语义、数据模板约束、规则落地和运行时追踪结构；保留现有接口与表设计事实。
- 目标读者：架构、研发、测试、SRE、安全和资金运营。
- 决策状态：主链路边界已确认；目标态表进入工程前仍需逐切片准入。
- 置信度：高；未落库能力以本文工程门禁为准。
...
| P0 | 余额投影 | ledger-impl | Ledger 内部唯一 projector、LedgerBalanceChangedEvent。 |
...
| --- | --- | --- | --- | --- |
| REQ-MAIN-001 | 核心名相 | 钱包账户承载资金或信用责任，支付工具只提供访问能力，Spend Rule 只提供非现金控制。 | 产品 02、06、09 | 4.4 钱包账户、4.10 Spend Rule 证据消费。 |
| SCN-MAIN-001 | 业务场景 | 充值、付款、转账、退款、提现、授权、撤销、清算、冻结和解冻。 | 产品 02、05 | 4.1-4.9 的服务入口、状态机和投影。 |
| AC-MAIN-001 | 验收种子 | 资金动作生成稳定 route snapshot、平衡 posting plan、不可变分录和正确余额桶。 | 产品 05 | 4.6-4.9、4.13 测试设计。 |
| QA-MAIN-001 | 质量属性种子 | 幂等冲突拒绝、失败无账务副作用、历史路径可回放、查询不扫描全量分录。 | 产品 01、05 | 4.12、第五节和第六节。 |
...
flowchart LR
    ...
    Entry --> Balance["Ledger internal projector\n余额投影"]
...
| 直接退款 | refund | FundsTransactionRefundRequest | 资金交易流水号 | 原交易引用或显式退款路径 + 退款业务流水 + 请求摘要 | 有原交易引用时只按原 route snapshot 回放；无引用时必须显式给出到账账户、出资账户和出资账目；累计退款不超过原交易可退金额。 |
...
5. `topup`、`transfer`、`pay`、`refund`、`withdraw` 请求统一以可选 `feeChargeSpec` 承载本次交易新增收费，字段类型当前保持 `FeeSpec`；费用腿在主 route 解析后追加，并与主交易在同一本地事务入账。扣费账户必须是主路径中唯一真实 FundingAccount：充值取到账账户，转账取付款账户，付款取实际资金付款账户，退款取唯一真实资金受益账户，提现取提现账户；均从 `AVAILABLE` 扣，提现本金仍从 `FROZEN` 扣。
6. `feeChargeSpec` 不表示历史手续费退回，也不允许从 CreditAccount 额度、外部账户或多个候选账户推断扣款；`contextVariables` 中的同名保留键必须拒绝。扣费账户必须处于允许借记的 `ACTIVE` 状态，退款入账允许 `FROZEN` / `SUSPENDED` 不代表允许同步扣费；历史退费继续通过 `refundFee` 回放原费用腿。
...
4. 权益让利资金影响由 Consumer Benefit adapter 先完成场景准入和责任分摊，再通过 generic direct pay/refund 形成独立资金交易并以 ActionFact 恢复；主交易、route snapshot、posting context 或投影只引用稳定资金事实，不承载旧重型权益快照。
...
4. 携带 `referenceTransactionSn` 时，`accountId`、`payerId`、`payerLedgerSubjectCode` 必须为空；未携带时三者必须完整提供，且 `payerLedgerSubjectCode` 不设默认值。
5. 缺原交易、缺原 `routeSnapshot`、错币种、累计退款超额或原 leg 不允许部分回放时，必须在账务入账前失败；失败不得生成 route、posting、LedgerEntry、余额投影或交易投影副作用。
6. route resolver 中直接付款本金 leg 和费用 leg 需要显式声明可部分回放，确保部分退款、退费和后续 replay 共享同一不可变路径；仍不得按当前支付工具绑定、当前资金责任、当前支出控制范围或当前 Spend Rule 重新选路。
...
| 回放和逆向入口 | 退款、可信撤销、拒付、退费和解冻。 | 原交易、原授权、原完成、原 route snapshot 或冻结来源。 | 逆向资金事实或失败原因。 | 优先使用原快照，不能按当前支付工具绑定或当前资金责任关系重新选路；到期或 expired 仅作观察，不进入资金回放。 |
...
`FundsAuthorizationTransactionService.authorize(FundsAuthorizationTransactionAuthorizeRequest)` 属于账户主体型 canonical 入口。当前没有支付工具交易编排的真实生产 Consumer，宿主或 adapter 必须先归一账户主体、资金责任和外部事实，再调用 canonical 服务；Provider 内部可以组合支付工具快照与 Spend Control 事实，但不新增或保留支付工具型 Public facade。

wallet application 调用面选择：

| 调用目的 | 推荐入口 | 调用方 | 不适用 |
| --- | --- | --- | --- |
| VCC/卡授权、共享卡授权、支付工具授权。 | 上层归一账户主体后调用 `FundsAuthorizationTransactionService.authorize`。 | 上层业务、发卡业务、通道适配。 | 不把支付工具引用直接替换 canonical 入参；不绕过支付工具、资金责任、账户能力和 Spend Rule 准入。 |
| VA 或银行支付工具收款、ACH/银行已确认入金。 | `ExternalFundsEventApplicationService.consume`，或上层归一目标账户后调用 `FundsDirectTransactionService.topup`。 | 业务包或外部事实 adapter。 | 不给 VA、外部账户或支付工具建账；不把外部 accepted 当内部成功。 |
| 全球账户出款、SWIFT/local/ACH payout。 | 上层 P2 出款业务确认外部成功后调用 `FundsDirectTransactionService.withdraw`。 | 全球账户或出款业务包。 | 必须携带原冻结流水并由 canonical withdraw 校验原主体、金额和累计消费；资金底座不解释 provider 状态，不在完成阶段按当前支付工具绑定重新选路。 |
| 只读解析支付工具、绑定、资金责任、账户能力和准入快照。 | `PaymentInstrumentCapabilityApplicationService`、`FundingResponsibilityResolutionApplicationService`、`FundsAccountCapabilityApplicationService`、`PaymentInstrumentPreTransactionSnapshotApplicationService` | 支付工具交易入口内部组合、风控预检、运营解释。 | 不创建交易事实、route、posting、账本交易、分录或余额投影。 |
| 授权前 Spend Rule 准入。 | `SpendControlAdmissionApplicationService` | 支付工具授权入口或需要规则证据的上层业务。 | 不执行完整规则引擎，不替代资金交易。 |
| 交易成功、退款补偿后的控制额度变动。 | `SpendControlMovementService`；交易事实到控制事实的转换由 transaction-impl 内部编排。 | Wallet 控制事实查询方；Consumer 不调用内部编排。 | 不创建或修改资金交易、ledger posting、支付工具资金流向或 canonical 入参。 |
...
| 完成 | complete | FundsAuthorizationTransactionCompleteRequest | 授权交易流水号、完成明细流水号、累计完成金额和剩余授权金额 | 普通完成必须引用原授权；强制完成必须显式 FORCE 模式、受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计，初始 FORCE 模式不得依赖原授权流水 | 普通完成使用原授权流水 + 完成业务流水 + 请求摘要；强制完成使用外部原事实引用 + 受信策略/审批快照 + 完成业务流水 + 请求摘要 | 完成超过剩余授权失败；强制完成缺策略、超上限、缺外部原事实、缺凭证或缺审计失败。 |
| 完成后退款 | refund | FundsAuthorizationTransactionRefundRequest | 普通授权链退款返回授权交易流水号、退款明细流水号、累计退款金额；无授权退款返回退款资金事实和外部引用审计摘要 | 普通退款引用原完成；无授权退款以空原授权流水进入 no-auth 语义，并携带 `externalReferenceSn`、退款原因、操作者和审计 | 普通退款使用原完成引用 + 退款业务流水 + 请求摘要；无授权退款使用外部引用 + 退款业务流水 + 请求摘要 | 累计退款超过已完成金额失败；无授权退款缺外部引用、缺原因或缺审计失败；不得补造授权占用，不得携带或查询内部授权流水。 |
| 上层已确认的争议本金退款 | refund | `FundsAuthorizationTransactionRefundRequest` 携带原完成交易、退款金额、退款业务流水、安全外部案件引用、原因、凭证引用和审计上下文 | 授权交易流水号、退款明细流水号和累计退款金额 | 仅用于上层已确认需要退回已完成授权本金的结果；无需资金处理时不调用资金交易入口 | 原完成引用 + 退款业务流水 + 请求摘要 | 不要求落到独立 chargeback 方法；不得把授权拒绝写成拒付，不得把案件状态或裁决过程写入资金交易；上层必须先将追偿、准备金抵扣或后续结算扣减归一为明确资金指令。 |
...
3. complete 同时承接普通完成和强制完成；普通完成继续要求原授权交易流水，强制完成必须显式声明 FORCE 模式、受信策略或审批快照、上限、原因、外部原事实引用、凭证和操作者；初始 FORCE 模式不得携带或依赖内部原授权流水，不得构造 `AUTHORIZATION` reference 或查询原授权账本交易。
4. refund 同时承接普通授权链退款、无授权直接退款和争议裁决后的退款结果；普通授权链退款继续要求原完成引用，无授权退款以空原授权流水进入 no-auth 语义，携带 `externalReferenceSn`、退款原因、操作者和审计，且不得携带或查询内部授权流水；争议裁决结果必须携带案件引用、裁决结果、原因、凭证和审计；资金路径可以复用退款路径，但原因、外部引用和投影展示语义必须保留。用户败诉或无需资金处理时，不生成资金交易事实。
5. 服务返回值只表达资金底座已经接受并处理的资金事实，不承诺通道、卡组织或上层业务事件最终状态。

争议语义应先在 dispute case 层得出裁决，再由 `refund / AUTH_REFUND` 的查询、投影、审计和幂等摘要承接退款结果；裁决无资金影响时只保留案件、审计和投影解释，不进入交易内核。2026-06-26 已裁决独立 `FundsAuthorizationTransactionService#chargeback` 公共方法、请求模型、事件枚举和 replay 分支退出目标态，与 `expire` 一起移除；需要完整争议运营、representment、清结算追偿、VCC processor、外部卡组织规则、跨仓库兼容或历史数据只读解释时，必须另起专项工程任务，不恢复交易内核 `chargeback` 主入口。
...
```mermaid
flowchart TD
    A["上层归一后的授权事件\nreversal / complete / refund"] --> B["校验幂等键和请求摘要"]
    B --> C{"是否需要原事实?"}
    ...
    F -- "否" --> G["失败或进入差错\n无 ledger entry"]
    F -- "是" --> H{"事件类型"}
    H -- "reversal" --> I["AUTHORIZATION -> AVAILABLE\n累计释放"]
    H -- "complete" --> K["AUTHORIZATION 或受控来源 -> 收款方 / CLEARING\n累计完成"]
    H -- "refund" --> L["原完成路径反向\n累计退款或拒付承接"]
...
授权后续事件必须先判定原事实、剩余可处理金额和幂等摘要，再生成账务计划。资金聚合派生 `remainingAuthorizedAmount = authorizedAmount - completedAmount - reversedAmount`；部分完成或部分释放后剩余金额大于 0 时，授权聚合保持 OPEN，不增加部分完成状态；剩余金额为 0 时聚合转为 CLOSED。完成后的到期提示、撤销后的完成、重复完成、重复退款等异常组合必须由生命周期累计金额和请求摘要共同拦截；到期、超时或通道未返回不是可信资金事实，不生成 route、posting、LedgerEntry 或余额变化。
...
stateDiagram
    ...
    HELD --> HELD: 部分完成 / 部分释放，remainingAuthorizedAmount > 0
    HELD --> COMPLETED: remainingAuthorizedAmount = 0 且 completedAmount > 0
    HELD --> REVERSED: remainingAuthorizedAmount = 0 且 completedAmount = 0
    COMPLETED --> COMPLETED: 部分本金退款，refundedAmount < completedAmount
    COMPLETED --> REFUNDED: 本金全退，refundedAmount = completedAmount
    REFUNDED --> COMPLETED: 受信 late clearing / overcapture 已独立入账
    DECLINED --> [*]
    REVERSED --> [*]
```

状态收口规则：授权交易不提供独立 close 服务入口。`FundsTransactionState.OPEN/CLOSED` 只表达资金授权聚合是否仍可执行后续资金动作，不是任何上游业务的主状态。`remainingAuthorizedAmount = amount - completedAmount - reversedAmount` 仅作为资金准入和查询解释派生值；上游业务状态、事件状态和展示标签由对应业务域维护。授权超时或 expired 不是可信资金结果，不自动释放占用；只有可信 reversal、void、清算剩余释放或差错补事实可改变资金聚合。

授权红线：

1. 授权拒绝不得生成 RouteLeg、PostingPlan、LedgerEntry、declinedAmount 或 chargeback 事件。
2. 授权成功不是最终消费；用户账单和商户账单要区分占用和结算。
3. 授权完成不新增含义重复的完成金额账目，不触碰 `LIMIT`；VCC PREPAID 的 FundingAccount 执行 `AUTHORIZATION -> SETTLEMENT`，SHARED 的 CreditAccount 执行 `AUTHORIZATION -> OUTSTANDING`。父 FundingAccount 只有作为原 RouteSnapshot 的显式参与方时才承担一次真实资金结算责任，账户层级关系本身不触发资金变化。
4. reversal 和普通 complete 必须引用原授权事实；普通 refund 必须引用原完成事实；refund 无授权退款模式以空原授权流水进入 no-auth 语义，引用 `externalReferenceSn`、退款原因、操作者和审计，不得补造内部授权占用，不得携带或查询内部授权流水；拒付通过 refund 携带原因、凭证和审计上下文承接；complete 强制完成模式必须有受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计，不能把普通完成的原授权流水当作强制完成凭证。
...
3. `reversal`、`complete` 和 `refund` 原则上读取原授权事实、原 route snapshot、原权益资金事实或历史摘要，不接收当前重新计算的权益结果；完成核销、可信撤销释放和退款冲回必须沿原事实处理。
4. `expire` 已从授权交易目标态移除；权益设计不得借过期释放恢复公共契约。授权占券到期只能由权益系统或运营侧形成控制释放、差错候选或可信撤销/补事实后再进入资金链路。
5. `complete` 强制完成、`refund` 无授权退款和拒付承接属于授权目标态的独立对齐问题；权益设计只要求这些路径能引用原权益资金事实或历史摘要并保留原因、凭证和审计，不要求新增权益专用方法。
6. 授权后续事件中的释放、核销、返券、不返券或补贴冲回决策来自业务层或营销权益系统；资金底座只消费决策结果和原授权权益资金事实，不在 reversal、complete 或 refund 内重新判断券是否可退。
...
钱包账户是能力域，不是单一实体。系统层以 `FundingAccount`、`CreditAccount`、`AccountHierarchyRelation`、支出控制范围 `SpendControlScope`、`PlatformFundingAccountRole`、`PaymentInstrument`、`SpendSubjectFundingRelation` 和余额查询服务共同承接钱包产品能力；其中 `FundingAccount` 只表示真实资金账户、平台责任资金账户或经明确 profile 标识的资金子账户，`CreditAccount` 可承接信用子账户，`SpendControlScope` 只表示 Spend Rule 可引用的支出控制 scope。统一路由和入账时使用 `FundsSubjectType` / `SubjectRef`；外部支付工具和工具快照使用 `PaymentInstrumentRef`，内部余额钱包、信用额度、返利钱包或商户钱包等业务入口先解析为 `SubjectRef`、`BenefitSnapshot` 或显式 route participant/leg，不得用普通 `FundingAccount` 兼容所有主体，也不得把所有业务入口强行包装成 `PaymentInstrument`。
...
应用层可以通过 Request 字段承接内部余额钱包、信用额度、返利钱包、商户钱包、VCC 卡、银行卡、VA、外部账户和通道 token 等业务入口参数。但该参数只属于 application/use-case facade，不是交易内核主体，也不要求内部账户都包装成 `PaymentInstrument`。facade 的职责是把入口解析为 `SubjectRef`、`PaymentInstrumentRef`、`BenefitSnapshot` 或显式 route participant/leg 等不可变事实，再委派交易、路由和账务内核。
...
| 层次 | 当前服务 | 完成度 | 继续扩展原则 |
| --- | --- | --- | --- |
| 基础服务 | `FundingAccountService`、`CreditAccountService`、`SpendControlScopeService`、`PlatformFundingAccountService`、`PaymentInstrumentService`、`PaymentInstrumentBindingService`、`PaymentInstrumentBindingHistoryService`、`SpendSubjectFundingRelationService`、`AccountHierarchyRelationService`、`FundsSubjectBalanceQueryService`、`SpendRuleDefinitionService`、`SpendRuleVersionService`、`SpendRuleBindingService`、`SpendRuleDecisionRecordService`、`SpendControlMovementService`。 | 条件可用：wallet 实体和控制事实均已有基础服务或明确组合服务承接，Mapper 访问收敛在 `wallet-impl/services/impl`；账户层级服务只维护不可换绑的直接父子结构事实；支出控制范围创建不建账，Spend Rule 控制事实不入 ledger。 | 新增单对象能力优先并入基础服务；不得新增旧式 `DomainService` / `DomainQueryService`、兼容 facade 或只做委派的浅服务。 |
| 场景 application service | `FundingResponsibilityResolutionApplicationService`、`FundsAccountCapabilityApplicationService`、`PaymentInstrumentCapabilityApplicationService`、`PaymentInstrumentPreTransactionSnapshotApplicationService`、`ExternalFundsEventApplicationService`、`SpendControlAdmissionApplicationService`、`BudgetControlLimitAdjustmentApplicationService`；支付工具交易与交易后控制消费只保留 transaction-impl 内部编排。 | 条件可用：已覆盖授权准入、预交易快照、外部确认入金和预算额度调额；支付工具收款、完成/撤销与控制消费作为 Provider 内部路径验证。失败路径要求停在资金交易、route、posting 和账本事实前。 | 只有真实跨模块 Consumer 证明稳定复用时才扩展 Public application service；内部编排不得上推为 facade。 |
| 未完成能力 | 支付工具型出款发起与完成 facade、外部扣账、return/NOC/reversal、信用账户还款或调额、VCC 预付 / 共享卡专项 facade、事件消费 / outbox、运营后台和生产迁移。 | 未完成交付：当前冻结请求尚未固化原支付工具绑定快照，因此不得在出款完成阶段按当前绑定补做解析；外部出款状态机仍归 P2 业务包。 | 只有当出款发起契约能够在冻结时保存原工具、绑定和资金主体快照，才评估新增支付工具型完成入口；此前复用 canonical `withdraw(referenceFreezeSn)`。 |

账目 profile 与 required ledger 初始化归 Ledger Owner。Wallet 账户创建只调用 `ledger-face.LedgerService.initializeRequiredLedgers(...)` 的受控 `void` 命令；profile/version、内部 catalog、bucket 创建与完整性复验均留在 `ledger-impl`。Wallet 不再拥有 `LedgerProfileService`、`SubjectLedgerInitializer` 或 ledger id 结果映射。
...
| FundingResponsibilityResolutionApplicationService | 按租户、源资金/信用账户、币种和关系类型读取唯一资金责任关系，输出资金账户或信用账户目标快照。 | SpendSubjectFundingRelationService。 | 源端和目标端都受 `FundsSubjectType` 约束；不解析工具绑定、账户层级、Spend Rule 或平台角色，不校验余额和额度，不写 route。 |
| Transaction Provider 内部支付工具编排 | 承接授权、带控制预留的可信授权完成/撤销和已确认收款的 Provider 内部组合；`PaymentInstrumentAuthorizationProcessor` 与六个内部命令均不是公共契约。 | 支付工具预交易快照应用服务、Spend Control admission、内部控制事实编排、资金责任解析应用服务、账户能力应用服务、历史 route snapshot 和 canonical 交易服务。 | 首次授权解析当前工具、绑定、资金责任和账户能力；可信完成/撤销只读取原授权快照、原账务主体和原控制预留，在同一本地事务内分别委派资金完成/撤销并写控制消费/释放，不按当前绑定重选路。Consumer 只调用 canonical Funds Service；不新增支付工具退款或交易 facade。 |
...
| Transaction Provider 内部控制事实编排 | 交易成功、授权完成、可信授权撤销、退款或争议退款后，将交易结果翻译为 Spend Rule 控制活动的消耗、释放或补偿活动；可信完成/撤销必须回链原授权资金交易和原控制预留，累计 `CONSUMED` / `RELEASED` 分别不得超过资金聚合 `completedAmount` / `reversedAmount`；失败或拒绝通过同一资金事务回滚控制预留，超时不写控制流水。 | SpendControlMovementService、资金交易事实查询、route snapshot、原控制活动、支付工具快照和预算 scope。 | 只桥接交易结果和控制活动；不创建资金交易，不改交易 canonical 入参，不写 route、posting、LedgerEntry、ledger transaction 或账本余额投影，也不向 Consumer 暴露。 |
...
| --- | --- | --- | --- | --- |
| `PaymentInstrumentCapabilityApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/instrument` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/instrument/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`，必要时加 `instrument` 子包。 | 支付工具能力准入、绑定版本和敏感字段边界。 |
| `FundingResponsibilityResolutionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/funding` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/funding/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`，必要时加 `funding` 子包。 | 解析最终资金或额度责任主体；B2-FR 必须先裁决字段策略。 |
| Transaction Provider 内部支付工具编排 | 无 face/Public 接口 | `transaction/impl/src/main/java/com/wind/funds/transaction/application/instrument` | `com.wind.funds.transaction.application.instrument` 内部命令与 Wallet 只读事实 DTO。 | 组合支付工具准入、原授权快照和控制预留后委派账户主体型交易内核；只服务 Provider 内部调用，不向 Consumer 暴露替代 facade。 |
| `SpendControlAdmissionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/spend` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/spend/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`。 | 消费外部 Spend Rule 决策证据，组合支付工具预交易快照，输出授权前支出控制准入结论；不拥有规则引擎或预算投影。 |
| `SpendControlMovementService` | `wallet/face/src/main/java/com/wind/funds/wallet/service` | `wallet/impl/src/main/java/com/wind/funds/wallet/services/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.query`、`com.wind.funds.wallet.model.dto`。 | 记录 `RESERVED`、`CONSUMED`、`REFUND_COMPENSATED`、`RELEASED` 和调额类控制活动，按活动聚合只读预算控制投影；准入和拒绝证据进入决策记录，交易失败或拒绝依赖同一资金事务回滚，超时不写控制流水。 |
| Transaction Provider 内部控制事实编排 | 无 face/Public 接口 | `transaction/impl/src/main/java/com/wind/funds/transaction/application/spend` | `com.wind.funds.transaction.application.spend` 内部命令与 Wallet `SpendControlMovement` 事实契约。 | 把已成立交易事实转换为消费、可信释放或退款补偿控制活动；归属 transaction-impl，不由 Consumer 直接调用。 |
| `ExternalFundsEventApplicationService` | `transaction/face/src/main/java/com/wind/funds/transaction/application` | `transaction/impl/src/main/java/com/wind/funds/transaction/application/external/impl` | `com.wind.funds.transaction.model.request`。 | ACH、银行文件或银行渠道回调的外部资金事件消费入口；当前只消费已确认 ACH/银行入金事件并委派账户主体型 `topup`，因此公共契约和实现均归属交易层。外部钱包、外部扣账、return/NOC/reversal、信用账户还款或调额、差异单、补事实和事件消费者需另起工程边界。 |
| `VccPrepaidFundingApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/vcc` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/vcc/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`，必要时加 `vcc` 子包。 | 预付卡入金确认、系统内充值和退回；写入卡绑定资金子账户，不写卡号账本或 `VCC_ACCOUNT`。 |
| `VccSharedCardTransactionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/vcc` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/vcc/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`，必要时加 `vcc` 子包。 | 共享卡授权、清算和逆向的 VCC 场景编排；使用卡绑定信用子账户作为账务主体，父账户提供约束和汇总，卡维度来自投影。 |

不新增顶层 `com.wind.funds.instrument` 包。wallet-face 只保留支付工具资源、绑定、准入快照、资金责任和控制事实契约；交易编排与内部命令归 transaction-impl，通过 wallet-face 消费稳定事实，不直接依赖 wallet-impl、钱包 DAL 或 Mapper。没有真实 Consumer 前，不在 wallet-face、transaction-face 或 core 提供替代 facade。

transaction-impl 内部支付工具编排负责首次动作的工具能力、绑定、预算控制、资金责任和账户能力组合，再委派账户主体型交易内核；可信完成和撤销必须读取原授权 `RouteSnapshot` 与交易上下文，不重新读取当前绑定，并在存在 `controlReservationSn` 时分别原子记录 `CONSUMED` / `RELEASED`。内部收款编排只把已归一的 VA、银行账户或外部钱包端点事实解析为目标资金账户和外部入金引用。三类渠道事实必须分开：`FundsTransaction.channel` 只表达 `BANK_TRANSFER`、`DIGITAL_WALLET` 等交易渠道大类；内部命令的 `externalRailCode` 表达本次实际使用的 ACH、WIRE、LOCAL_BANK 或 DIGITAL_WALLET rail；`PaymentInstrument.channelCode` 表达稳定的支付工具接入方或提供方。交易实现按 `instrumentType + externalRailCode` 校验工具与 rail 是否匹配，再委派 `FundsDirectTransactionService#topup`；ACH、wire 和 local bank 统一归一为 `BANK_TRANSFER`，外部钱包归一为 `DIGITAL_WALLET`。route snapshot 的 `ExternalAccountRef.channelCode` 固化实际 rail，`providerCode` 固化支付工具快照中的接入方；外部事件本身没有提供方事实时不得伪造 provider。内部命令必须携带 `expectedBindingVersion`，缺失、未知 rail 或工具与 rail 不匹配时必须在交易内核前失败，不得生成资金交易、route、posting 或账本事实。不得把 `FundsAuthorizationTransactionAuthorizeRequest`、`FundsTransactionPayRequest`、`FundsTransactionTopupRequest` 或余额控制请求整体替换成支付工具引用入参，也不得让交易内核只感知 `PaymentInstrument` 而丢失最终资金账务主体。

外部已确认入金采用两层幂等。第一层仍以 `tenantId + businessScene + businessSn` 识别一次业务命令；第二层以 `tenantId + externalSourceCode + externalFundsFactSn + externalFundsEffectType` 识别一次外部资金效果。`externalSourceCode` 必须由可信上游适配层按真实资金事实命名空间归一后显式传入，不接受终端客户直接控制，资金底座也不从 provider、rail 或支付工具推导。`externalEventSn`、`channelTransactionSn`、provider 和 rail 属于交付、渠道或追踪事实，不组成通用资金事实唯一键。同一第二层键顺序再次到达时，若由目标账户、金额、币种、原金额和适用汇率组成的 `externalFundsFactDigest` 一致且原交易已 `CLOSED`，直接返回原交易号；摘要不一致或原交易未成功闭合时拒绝，不重新 route 或 posting。自动入口缺稳定外部来源或资金事实号时必须在交易内核前失败。数据库联合唯一键承担并发最终裁决，不新增进程锁；并发竞争失败方允许直接收到唯一键异常，不要求资金底座捕获后返回胜者交易号。

外部事实处理发生纯技术异常时，资金交易、route、posting、账本事实和外部事实唯一键必须在同一事务中整体回滚，调用方可以使用同一外部事实重试。若某笔资金交易已经以 `FAILED` 状态提交，则它已成为稳定的失败或不确定事实，不能作为技术重试覆盖或自动重新入账，应交由差错核对链路处理。

Provider 内部收款编排在读取当前支付工具和绑定快照前，先通过现有 `FundsTransactionQueryService` 按外部资金事实键查询已成立交易。命中 `CLOSED` 时读取原 `RouteSnapshot`，核对交易模式、交易类型、金额、币种、支付工具号和原绑定版本后返回原交易号；该路径不读取当前工具状态、当前绑定或当前资金责任。未命中时才执行当前支付工具、绑定、资金责任和账户能力准入。因此，绑定变更、暂停或解绑后的首次延迟入金必须失败且不产生资金副作用；上层只能依据不可变 VA / 收款分配证据确定原目标账户，再调用账户主体型 `topup`，资金底座不扫描绑定历史猜测路由。

授权交易入口采用公共应用服务与 canonical 内核两层系统边界：
...
交易层需要继续完善账户主体型能力，而不是新增支付工具型交易内核。后续 B3/B4/B5/B6 可以在 `transaction/face/src/main/java/com/wind/funds/transaction/application` 下扩展或完善 canonical 服务、请求、事件和状态机；但这些契约的主体仍必须是已解析的资金账户、信用账户、VCC 资金/信用子账户或平台账户角色解析后的平台资金账户。

| 交易层能力 | 允许完善的包 | 必须保持的边界 | 对支付工具场景的承接方式 |
| --- | --- | --- | --- |
| 授权过期 | 不作为交易层公共入口。 | 到期、超时或通道未返回不是可信资金事实，不生成资金交易、route、posting、LedgerEntry 或余额变化。 | Provider 内部支付工具编排不触发 expire；如需释放占用，必须由可信 reversal、清算剩余释放、余额调整或对账差错补事实承接。 |
| 强制完成 | 同上。 | 必须有 FORCE 模式、受信策略或审批快照、原因、上限、外部原事实引用、凭证和审计；初始不得依赖原授权流水，不得伪造授权占用。 | VCC clearing forced post 或差错完成先解释业务事实，再委派 complete；带原授权的 overcapture 或 late clearing 需要另行扩展切片。 |
...
| 拒付和争议资金结果 | 同上。 | 只承接上层已确认的资金结果，必须与授权拒绝、普通退款可区分并防止重复损失。 | Fincone 或其他上层业务完成案件处理和资金决策；需要退回本金时委派 `refund` 并回放原交易路径，无资金影响时不调用 wind-funds。 |
...
| 原路径回放和查询解释 | route replay、transaction projection、查询 DTO。 | 重放只读；缺快照进入差错或人工处理。 | wind-funds 按子账户、父账户和支付工具引用提供资金事实解释；Fincone 组合 VCC 事件和业务状态生成卡账单、使用人视图和预付卡业务历史。 |
...
| `wallet.application` | 承接支付工具、VA、VCC、全球账户、ACH 等业务入口，完成准入、绑定、账户能力、Spend Rule、外部引用和预交易快照组合。 | 已有 `authorizeByInstrument` 和 `receiveByInstrument`；`receiveByInstrument` 按 `instrumentType + externalRailCode` 校验并归一为交易渠道大类；支付工具 `channelCode` 只表达稳定接入方。外部事件事实本身消费为交易事实时走 `transaction.application.ExternalFundsEventApplicationService#consume`。入参必须包含业务流水、场景、金额、币种、工具 / rail / 外部引用和可解析账户主体，支付工具入口必须携带调用方期望的绑定版本。 | 失败应停在交易事实前；通过后委派 transaction canonical service；route snapshot 固化脱敏 `PaymentInstrumentRef`、实际 rail、可确认的 provider 和外部流水引用；出款 rail 状态机、完成判定和原工具快照固化必须由 P2 业务包另行闭环。 |
...
| `clearing/reconciliation` | 匹配内部事实和外部证据，形成差异并记录人工处理、结算锁定和出款结果引用；需要追加资金事实时调用 transaction-face 的明确业务命令。 | 接收内部交易引用、账本交易引用、外部事件引用、规则版本和差异指纹。 | 外部 accepted 与内部 posting 不在同一事务，通过对账、受控资金命令和人工处理闭合；reconciliation 不直接写资金事实。 |
...
| 系统面 | 设计输入 | 目标态要求 | 工程影响 |
| --- | --- | --- | --- |
| 账户层级与 VCC 关联子账户 | `AccountHierarchyRelation` 保存不可换绑的直接父子结构事实；`RouteParticipant.accountHierarchySnapshot` 只保存 `relationSn + parentAccountRef`。当前不提供 root、状态、有效期或自动多级树解析。 | VCC 预付卡必须绑定资金子账户，共享卡必须绑定信用子账户；账户层级不推导父账户出资，父账户真实资金影响必须由显式 participant/leg 表达；卡、工具、支出控制范围和 Spend Rule 不得入账。 | 账户层级是支付工具、VCC prepaid funding 和 lifecycle 的结构证据；正向路由固化当前直接父账户，逆向/重放只复制原快照。 |
...
| 授权准入组合 | 已形成 transaction-impl 内部支付工具编排与 `RouteSnapshot` 回链服务流，并已补齐预交易快照和 Spend Control admission 组合。 | 组合工具准入、绑定快照、Spend Rule 决策证据、资金责任和账户能力后委派账户主体型授权内核；Spend Rule 拒绝必须在交易事实、route、posting 和账本事实前失败。 | wind-funds 只闭合 Provider 内部编排、Spend Rule 通用契约和原路径回放；VCC/issuer adapter 负责上游事件归一，不新增支付工具交易 Public facade。 |
...
1. 先补账户层级和 VCC 关联子账户准入：确认子账户 profile、不可换绑的直接父账户关系、参与方快照、账目 profile 和“层级不自动入账” Red。
...
| AccountHierarchyRelationService | 创建和查询资金账户、信用账户之间不可换绑的直接父子关系。 | 一个子账户最多一个直接父账户；拒绝自关联、跨币种、CLOSED 账户和已存在环路；不承载 root、状态、有效期、出资责任、PAN/CVC、issuer 原始协议或卡生命周期。当前版本不引入跨节点锁，同一租户并发构建多条拓扑关系必须由调用方串行提交。 |
...
| SpendSubjectFundingRelationService | 维护源资金/信用账户在指定币种和关系类型下到目标资金/信用账户的唯一责任关系。 | 源端和目标端类型都只能是 `FUNDING_ACCOUNT` / `CREDIT_ACCOUNT`；创建时校验目标账户存在、可扣和币种一致，不执行扣款、不写分录。 |
| LedgerService | 按已准入的 subject、currency、profile/version 与 effective period 受控初始化 required ledger。 | 只在主体初始化流程通过 `void initializeRequiredLedgers(...)` 使用；profile catalog 与 bucket 完整性复验由 Ledger Owner 内部完成，交易执行路径不得自动建账。 |
...
| 信用账户 | 管理授信额度、可用额度、授权占用和已用额度。 | `CreditAccountService`、授权交易、余额控制服务。 | LIMIT、AVAILABLE、AUTHORIZATION、OUTSTANDING、额度周期、调额来源、授信责任和规则版本。 | 先创建信用账户并设置额度；VCC 或授权交易将其作为显式 route participant，不把卡工具写入信用账户。 |
| VCC 关联子账户 | 管理 VCC 场景中可记账子账户的余额、授权占用、清算、退款、费用和已确认争议资金结果。 | `AccountHierarchyRelationService`、`FundsSubjectBalanceQueryService`、交易/清结算服务。 | owner、币种、账目 profile、状态、绑定工具、直接父账户、背后资金责任来源、敏感字段边界和审计来源。 | 先创建资金子账户或信用子账户并初始化账本；卡工具绑定到该子账户；账户层级只保存结构关系，资金动作通过 route/posting 使用显式主体。wind-funds 不管理卡生命周期或卡账单，不新增 `VCC_ACCOUNT`。 |
...
| 支出主体资金责任解析关系 | 将一个已经确定的资金账户或信用账户映射到唯一出资或结算责任账户。 | `SpendSubjectFundingRelationService`、`FundingResponsibilityResolutionApplicationService`。 | `spendSubjectType + spendSubjectId + currency + relationType` 唯一；源端和目标端都只能是资金账户或信用账户，目标必须可用、同币种。 | 不提供优先级、默认项、有效期或兜底链；工具绑定、账户层级、Spend Rule 和平台角色由各自服务先行处理。 |

VCC 场景中，调用方不能直接要求“创建卡号账本”“创建支出控制范围账本”或“多张卡共用同一子账户”。正确顺序是：确认卡产品资金模式和账户层级策略，创建或引用资金子账户/信用子账户，注册卡工具，建立工具到子账户、直接父账户、预算 scope、Spend Rule 和资金责任解析关系，必要时创建资金账户或信用账户作为背后责任来源，再由授权交易把最终主体和金额显式写入 route participant/leg。账户层级关系本身不得自动创建父账户资金路径。
...
```mermaid
flowchart TD
    A["创建 FundingAccount / CreditAccount"] --> B["提交 subject、currency、profile/version、effective period"]
    B --> C["LedgerService.initializeRequiredLedgers"]
    C --> D["Ledger internal catalog 解析 required bucket set"]
    D --> E{"完整 bucket set 已存在且一致?"}
    E -- "是" --> F["回读逐字段复验，void 幂等完成"]
    E -- "否" --> G["同一 REQUIRED 事务创建 t_ledger bucket"]
    G --> H["void 完成"]
    F --> I["如需观察 durable identities，使用既有 Ledger query"]
    H --> I
```
...
6. 支出控制范围不得调用 `LedgerService.initializeRequiredLedgers(...)`，不得初始化 ledger bucket；预算周期、可用、预留和释放只进入预算型 Spend Rule 控制视图、规则决策日志和审计证据。
...
| `LedgerTransaction` | 表示一次资金事实或控制事实的完整过账单元；可同时包含子账户明细分录、父账户控制分录或父子真实划拨分录。通过 `fundsTransactionSn` 关联资金交易持有的原 route snapshot，各 posting plan 通过 `routeLegId` 对应本次路径 leg，逆向回放再通过 `replayRefLegId` 定位原 leg。 | 不能重复保存 route snapshot 或账户层级摘要，也不能由余额投影或父账户聚合结果补造新的账本交易。 |
| `LedgerEntry` | 主体必须是具体资金账户或信用账户节点；必须固化 `postingRole`、账本周期、账目、金额和父交易/计划引用。账户层级与支付工具信息由原资金交易的 route snapshot 统一持有。 | 不能重复保存账户层级快照或不可解析摘要，不能把父账户聚合视图、支付工具、支出控制范围、Spend Rule、卡号、PAN、token 或 `VCC_ACCOUNT` 写成分录主体。 |
...
1. 资金交易的 route snapshot 必须固化原始 `accountRef` 和可选直接 `parentAccountRef`，用于退款、撤销、拒付和重放；当前不提供 root 或多级树运行时解析。`postingRole` 由 posting plan 明确并固化在 LedgerEntry，不写回 route snapshot。
2. 父子账户关系创建后不可换绑；工具改绑到另一个子账户只影响新交易。历史 LedgerTransaction 和 LedgerEntry 不重写、不迁移，逆向处理只读取原 route snapshot。
3. VCC 预付卡默认写卡绑定资金子账户；共享卡写卡绑定信用子账户，并必须在同一 route/posting 中显式加入父 `FundingAccount` 的 `PARENT_CONTROL` 授权占用，防止父账户在完成时透支。这是共享卡模式的准入约束，仍由显式 posting plan 承载，不得从 `AccountHierarchyRelation` 自动推导。
...
flowchart TD
    ...
    B --> D["AccountHierarchyRelationService<br/>子账户 / 直接父账户"]
...
4. `FundingResponsibilityResolutionApplicationService` 必须按支出主体、币种和关系类型解析出唯一资金账户或信用账户；0 条或多条关系都失败。平台角色必须先由 `PlatformFundingAccountService` 解析成具体资金账户，支出控制范围和 Spend Rule 不能作为最终资金责任主体。
5. 最终内部账户必须具备当前动作能力，例如 PAY、RECEIVE、WITHDRAW；支付工具能力不得绕过账户能力。
6. PaymentInstrumentRef、ExternalAccountRef、RoutingDecision、RouteParticipant、参与方账户层级快照和 RouteLeg 必须按需进入 route snapshot；LedgerEntry 主体不得出现工具、卡、VA、外部账户、外部钱包端点或通道 token。
7. 完整 PAN、CVV、密钥、token secret、银行账户敏感号和身份凭证不得进入普通快照、日志、导出或报表。
8. 支付工具状态、绑定、方向、动作能力和绑定优先级变更必须记录操作者、时间、原因、版本和前后值；资金责任关系当前只创建和查询，删除或变更需另立受控工程入口。
...
| --- | --- | --- | --- |
| VCC 普通授权 | `PaymentInstrumentRef` + `RouteParticipant(SubjectRef + AccountHierarchySnapshot)` + `RouteLeg`。 | 卡工具、脱敏展示号、绑定版本、资金/信用子账户、直接父账户关系、显式资金或额度路径、支出控制范围上下文、Spend Rule 快照、规则版本和外部授权引用。 | 卡、PAN、token、持卡人、支出控制范围、Spend Rule、`VCC_ACCOUNT` 或外部卡凭证成为 `LedgerEntry.subject`。 |
...
| shared card | 每张工具绑定一个信用子账户，多张卡可受同一主账户约束，并绑定使用人、部门、项目和支出控制范围；资金责任关系独立解析唯一目标。 | 信用子账户、直接父账户、使用人、绑定角色、绑定版本、支出控制范围、Spend Rule 快照和唯一资金责任关系。 | 新增共享卡号账本、多张卡绑定同一子账户，在资金责任关系内做优先级兜底，或在逆向事件中按当前绑定重新解释历史授权。 |
...
| 4. 资金责任解析关系 | 用已确定的支出主体、币种和关系类型解析唯一资金账户或信用账户；平台角色在调用前解析为具体资金账户。 | 形成可供路由使用的唯一主体，最终进入 `RouteParticipant` / `RouteLeg`。 | 0 条、多条、目标不可用或币种不一致时失败无账务副作用。 |
| 5. 账户能力与余额 | 内部主体具备动作能力、账本周期、余额/额度满足，且预算规则控制通过。 | 进入授权 route 和 posting。 | 余额不足、额度不足、预算规则拒绝、周期缺失或能力不匹配时拒绝，无账务路径。 |
| 6. 快照回放 | 后续可信撤销、完成、退款或争议资金结果引用原 route snapshot；过期只作观察。 | 可信资金动作按原路径处理；过期观察不生成资金动作。 | 缺快照不得按当前绑定重选路，进入异常或人工处理；超时或 expired 不得触发资金回放。 |

#### 4.4.3.1 支付工具场景资金契约

本节只设计 `wind-funds` 拥有的支付工具准入、资金责任解析和 canonical 资金动作。VCC 只是其中一个上游场景；`VccTransaction`、`VccTransactionEvent`、issuer 协议、事件应用状态、VCC 主状态和卡账单归 `fincone` VCC 发卡业务所有，不进入本模块数据模型或状态机。
...
| route resolver | 固化 `PaymentInstrumentRef`、binding snapshot、参与方账户层级快照、显式 RouteLeg、规则版本和外部引用。 | 在逆向交易中按当前绑定或当前账户层级重新选路。 |
| ledger | 只记录账户主体型分录和余额桶变化。 | 创建卡账本、支出控制范围账本、Spend Rule 账本或 issuer 账户账本。 |
| transaction projection | 生成可重建的资金交易解释，并提供稳定资金引用给上层组合。 | 生成 VCC 事件时间线或卡账单，反写交易事实、route snapshot、posting 或余额。 |
...
| 资金责任解析 | `FundingResponsibilityResolutionApplicationService` | tenantId、spendSubjectType、spendSubjectId、currency、relationType。 | `SpendSubjectFundingRelationService`。 | 返回包含 relationSn 和目标主体的唯一决策；0 条或多条失败，路由负责将结果写入 participant/leg。 |
| 支付工具授权 | 宿主归一后调用 `FundsAuthorizationTransactionService#authorize`；Provider 可复用内部编排。 | 账户主体型授权命令、工具准入快照、规则决策和资金责任决策。 | `FundsAuthorizationTransactionService#authorize`。 | 资金授权交易号或拒绝结果。 |
| 支付工具可信授权完成 | `FundsAuthorizationTransactionService#complete`；控制联动由 Provider 内部编排。 | 原授权引用、本次可信完成金额币种和稳定 `businessSn`。 | 原授权事实、原 `RouteSnapshot` 与 `SpendControlMovementService`。 | 原授权交易号；存在控制预留时资金完成与控制消费同事务成功或整体回滚。 |
| 支付工具可信授权撤销 | `FundsAuthorizationTransactionService#reversal`；控制联动由 Provider 内部编排。 | 原授权引用、本次可信撤销金额币种和稳定 `businessSn`。 | 原授权事实、原 `RouteSnapshot` 与 `SpendControlMovementService`。 | 原授权交易号；存在控制预留时资金撤销与控制释放同事务成功或整体回滚。 |
| 账户主体授权后续动作及退款 | `FundsAuthorizationTransactionService#complete/reversal/refund` | 原资金授权引用、原授权主账户、本次金额币种和稳定 `businessSn`。 | 原授权交易事实和原 `RouteSnapshot`。 | 资金交易引用、更新后的授权金额摘要或稳定失败。 |

支付工具首笔授权、已确认收款、可信授权完成和撤销只在 transaction-impl 内部保留 Provider 编排；Consumer 必须先归一并调用 `FundsDirectTransactionService` 或 `FundsAuthorizationTransactionService`。所有后授权动作必须引用原授权交易和原主账户，沿原 `RouteSnapshot` 回放，不按当前绑定关系重新选路；内部编排只做历史事实恢复和本地事务协调，不形成平行交易内核或 Public facade。

上游业务持有外部事件、业务主交易和展示状态；`wind-funds` 只持有授权资金聚合、幂等、防超额、RouteSnapshot 和账务事实。当前只在同进程、同数据源、同事务管理器和本地 Bean 调用前提下允许共享本地短事务。前提不成立时必须单独评审跨库/远程一致性，本文档不预埋 MQ、Outbox 或 Saga。

资金动作语义：

| 资金动作 | 必填事实 | 成功副作用 | 失败副作用 |
| --- | --- | --- | --- |
| 支付工具授权 | instrumentRef、amount、currency、actor、bindingVersion、businessSn、requestDigest。 | 委派账户主体型授权内核，生成 AUTHORIZATION 占用、RouteSnapshot、资金交易和账务事实。 | 返回拒绝或失败；无 route、posting、LedgerTransaction、LedgerEntry。 |
| 授权完成 | authorizationTransactionSn、completionAmount、currency、businessSn、originalRouteSnapshotRef、requestDigest。 | 核销本次授权占用，累计 `completedAmount`，生成独立资金和账务事实。 | 原授权、金额、币种、快照或摘要不一致时失败。 |
| 可信授权释放 | authorizationTransactionSn、releaseAmount、businessSn、reason、requestDigest。 | 同主体 `AUTHORIZATION -> AVAILABLE`，累计 `reversedAmount`。 | 超时、expired 或本地猜测不得调用；超过未使用授权时失败。 |
| 原交易本金退款 | originalTransactionSn、principalRefundAmount、currency、businessSn、reason、actor、requestDigest。 | 沿原 RouteSnapshot 和原汇率快照回放，累计 `refundedAmount`。 | 缺原事实、币种不同或累计超过 `completedAmount` 时失败。 |
| 业务确认的无原路由贷记 | 明确出资/到账账户、金额、币种、原因、操作人、审批、businessSn 和 requestDigest。 | 通过适用的直接退款或转账入口形成独立资金事实。 | 资金底座不推断原消费、责任方或业务合法性；事实不完整时失败。 |

`wind-funds` 只保证单个 canonical 资金动作的本地事务、幂等和账务完整性。它不接收 `totalCreditAmount` 作为本金退款，不提供 `overRefund`、复合退款或上层组件编排服务。本金 100、额外 credit 10 必须分别形成 `refund(100)` 和经业务确认的独立资金动作；额外 credit 不写入 `refundedAmount`。

幂等和摘要规则：

| 维度 | 摘要字段 | 冲突处理 |
| --- | --- | --- |
| 业务身份 | businessScene、businessSn、transactionType、eventType。 | 同键同摘要返回原资金结果；同键不同摘要冲突失败。外部事件 ID 是上层证据，不替代资金业务唯一性。 |
| 金额币种 | amount、currency、completionAmount、refundAmount、feeAmount、FxAppliedRate。 | 金额、币种或已应用汇率变化必须作为摘要冲突，不得覆盖原资金事实或跨币种直接加总。 |
| 工具与路由 | instrumentRef、bindingVersion、RouteParticipant、参与方层级快照、RouteLeg、RouteSnapshot 引用。 | 逆向动作只比对原快照，不读取当前绑定、当前账户层级、当前 Spend Rule 或当前资金责任重算。 |
| 本金退款 | originalTransactionSn、refundAmount、originalRouteSnapshotRef、originalFxAppliedRate。 | 只累计原完成交易本金，不超过 `completedAmount`；补偿、运费和 FX credit 不进入 `refundedAmount`。 |

`wind-funds` 不定义上游业务主状态或事件应用状态，也不将自身 `FundsTransactionState.OPEN/CLOSED` 直接翻译为 VCC 、收单或全球账户状态。资金层只保证：

1. `remainingAuthorizedAmount = amount - completedAmount - reversedAmount` 不为负数。
2. 本金退款后 `refundedAmount <= completedAmount`。
3. 授权拒绝无 route leg、posting、LedgerTransaction、LedgerEntry 和余额变化；允许保留不含 legs、不可回放的 RouteSnapshot 作为拒绝解释证据。
4. 超时或 expired 不生成资金释放；只有可信 reversal、void、清算剩余释放或差错补事实可调用 `reversal`。
5. 多个 canonical 资金动作各自使用独立稳定 `businessSn`；组件进度、部分成功恢复和人工复核属于上层业务。

账务矩阵：

| 业务动作 | 借方/贷方语义 | 余额桶变化 | 分录主体 | 投影维度 |
| --- | --- | --- | --- | --- |
| 共享卡授权批准 | 卡绑定信用子账户和父 `FundingAccount` 同步授权占用。 | 信用子账户和父资金账户均执行 `AVAILABLE -> AUTHORIZATION`。 | 信用子账户写 `DETAIL`；父资金账户写显式 `PARENT_CONTROL`，不由层级关系隐式生成。 | cardRef、cardholderRef、departmentRef、spendControlScopeRef、spendRuleRef、childAccountRef、parentAccountRef、fundingSourceRef。 |
| 共享卡授权拒绝 | 无资金事实。 | 无。 | 无。 | 拒绝原因、规则版本、工具快照。 |
| 共享卡授权完成 | 信用额度控制与真实资金责任分别闭合。 | 信用子账户 `AUTHORIZATION -> OUTSTANDING`；父 FundingAccount 只有作为原 RouteSnapshot 显式参与方时才承担一次真实资金结算责任。 | 原信用子账户、原快照中显式选择的 FundingAccount 和已确认收款主体。 | 原工具、原子账户、参与方层级快照、原 RouteLeg 和绑定快照。 |
| 共享卡退款 | 原路径分别回补信用额度和真实资金。 | 信用子账户 `OUTSTANDING -> AVAILABLE`；真实结算责任沿原路径冲回父 FundingAccount AVAILABLE。 | 原信用子账户、原父 FundingAccount 和原收款主体。 | 原卡、原使用人、原 RouteSnapshot。 |
...
| 预付卡授权完成 | 资金子账户完成金额进入 VCC 内部结算口径。 | `AUTHORIZATION -> SETTLEMENT`。 | 原资金子账户。 | completionRef、merchant、feeRef。 |
...
flowchart TD
    ...
    F --> G{"资金责任唯一且账户能力满足?"}
    G -- "否" --> H["路由失败或人工处理\n无 posting"]
    G -- "是" --> I["生成 RoutingDecision\nmatchedRules / selected accounts / reason"]
    I --> J["RouteSnapshotFactory\n固化工具、参与方、层级快照和 RouteLeg"]
...
3. 同一工具绑定的默认关系和优先级更新必须有版本；资金责任关系当前没有更新入口。route snapshot 保存实际采用的绑定和责任结果，便于审计和重放。
...
| 主体解析 | 将用户、商户、平台角色、信用账户、支付工具绑定和预算控制上下文解析成资金路径主体；支出控制范围和 Spend Rule 只参与控制、规则快照和查询维度，不作为 route leg、posting 或 ledger entry 主体。 |
| 路径证据 | RouteLeg 只固化 source/target、角色、金额、顺序和回放引用；账目、账期、阶段、余额效果和约束由 `LedgerPostingAssembler` 从资金指令、账户 Profile 或原账本事实派生。 |
| 权益资金事实消费 | 读取权益让利资金交易、route context 或等价不可变事实中已决策的权益结果；`complete` 入口只消费需要入账的出资事实，并按 `fundingNature`、成本承担主体和让利承接账务主体生成权益 route leg。 |
...
| 能力层 | 工程落点 | 允许做什么 | 不做什么 |
| --- | --- | --- | --- |
| 契约承载 | Consumer Benefit adapter + `FundsDirectTransactionService.pay/refund` + `FundsActionFact`；Consumer boundary 与 ActionFact 契约测试。 | adapter 校验 tenant、成本承担主体、让利承接账务主体、Money、Capte-owned funding nature 和业务引用，再归一为通用资金命令；完成与恢复只认 ActionFact。 | Benefit 场景 service/request/enum 不进入 Provider Public API；不把专业确认、确认凭证、摘要、营销来源归因、账务效果或券规则塞入通用请求/context。 |
| 路由账务消费 | generic direct command 复用既有 `RouteResolver`、`RouteSnapshotFactory`、`LedgerPostingAssembler`、`DefaultRouteReplayService`。 | 平台补贴、商户承担和合作方补贴按逐出资方 Money 生成独立资金影响；退款、业务取消、人工纠错或反向冲销读取原 ActionFact/交易和 route snapshot 后回放。 | 不把营销规则、券可用性判断或券生命周期推进放进资金底座；无资金转移解释事实、`HOLD_ONLY`、`RELEASE_ONLY`、`REVERSAL_REQUIRED` 在未确认独立事实或伴随指令策略前不生成资金交易。 |
...
| 权益资金事实源 | generic direct action 形成的 `FundsActionFact`、交易事实快照、route snapshot 摘要、独立上游权益事实或等价不可变存储之一。 | Consumer adapter、`transaction-impl`、`ledger-impl`、归档重放、清结算、对账。 | 只能停留在 request/contract-only，不进入 route/posting 生产消费。 |
...
| benefit contribution accounting transaction | 根据上游已经决策完成的成本承担账务主体、让利承接账务主体、让利承接目标账目、金额和资金性质生成标准资金交易。 | 计算优惠金额、判断券是否可用、读取当前券包状态、保存券、活动或规则来源归因。 |
...
| 接口或组件 | 建议落点 | 职责 | 禁止行为 |
| --- | --- | --- | --- |
| Consumer Benefit adapter | 落在 Capte/Host 的资金集成边界；Provider 只保留 generic direct command 与 ActionFact。 | 接收业务侧已决策让利出资结果，校验 tenant、成本承担账务主体、让利承接账务主体、显式账目、Money 和 Capte-owned funding nature 后，调用标准交易路由、交易事实和账本链路；当前真实 Consumer 固定商户 `CLEARING`。 | 不新增同义 Benefit/Marketing facade，不计算优惠金额，不判断券是否可用，不维护券包生命周期，不把券、活动或规则来源归因写入 Funds context，也不直接写 route、posting、LedgerEntry 或余额投影。 |
...
4. 幂等摘要必须包含让利出资记账交易核心字段；同一业务流水、不同出资方、承接主体、承接目标账目、金额或资金性质不能被当作同一请求复用。
...
Ledger 模块的系统定位是账务事实层和余额投影事实源。跨模块生产写入口使用标准 `LedgerTransactionPostingService` 过账网关；跨模块只读账务证据查询按需收敛到既有基础服务：账本交易和账目分录事实由 `LedgerTransactionService` 按租户和稳定流水号读取，账本 bucket 与余额信息由 `LedgerService` 承接。`getLedgerTransactionBySn`、`getLedgerEntryBySn` 属于强存在查询，查不到必须抛业务异常；对账需要的分录集合按 tenant 与稳定账本交易引用查询，允许返回空页。公共能力不提供按数据库 ID 读取或通用账本交易分页，运营检索应从宿主业务引用、ActionFact 或受控管理面定位稳定账本流水。`ledger-impl` 使用唯一内部 projector 落地余额投影并发布余额观察事件，不向宿主暴露投影实现选择。Ledger 模块不承接业务交易入口、路由决策、支付工具解析、Spend Rule 决策、清结算批次状态机或对账差错生命周期。
...
| 统一过账 | `LedgerTransactionPostingService` | 校验 `LedgerTransactionSpec`、`PostingPlan`、`LedgerEntry` 和幂等摘要，原子写入账本交易、posting plan、不可变分录和余额投影；跨模块写入只走标准过账网关，`LedgerTransactionService` 只提供事实查询，不暴露写方法。 | 重新解析 route、重选工具绑定、解释业务订单生命周期、接收业务方直接指定的借贷方向，或绕过 posting gateway 直接调用 ledger-impl 持久化实现。 |
| 账务证据查询 | `LedgerTransactionService`、`LedgerEntryQuery` | 按 tenant 与稳定账本交易/分录流水读取不可变账务事实；`getLedgerTransactionBySn` 和 `getLedgerEntryBySn` 是强存在查询，`queryLedgerEntries` 只读承接对账所需的分录集合与主体、账目、币种、周期、业务引用过滤。跨模块查询禁止省略 tenant、禁止以数据库 ID 作为事实引用，也不提供无 Consumer 的通用账本交易分页。 | 查询时修复状态、补写分录、重算余额、跨租户兜底查询、扫表推断业务完成，或推进清结算和对账状态机。 |
| 余额投影 | `LedgerService`、ledger-impl 内部唯一 projector | 从 `LedgerEntry` 派生余额，支持当前余额、周期余额、检查点和重建输入；按账务主体、账目、币种和周期 bucket 查询，不为透传读取另设 application facade。 | 从交易投影、余额日志、报表或人工汇总反写账本事实；由宿主选择 projector。 |
| 不可变账务事实 | `LedgerTransactionPostingService`、`LedgerTransactionService`、`LedgerEntryQuery` | `LedgerTransaction` 创建即代表 transaction、posting plan、entry 和余额投影原子过账成功；posting gateway 只追加，查询服务只读，不提供状态更新或物理删除。 | 保存 PENDING/FAILED 半成品账本交易，修改或删除原交易、原 posting plan、原分录。 |

目标态公共语义是：生产资金流只有追加型账务事实能改变余额。`LedgerTransaction`、`PostingPlan` 和 `LedgerEntry` 不提供更新或删除接口；错账、冲正、补记、调账、追偿和差错核销必须形成新的资金事实或控制事实，并重新经过 route、posting 和 ledger 链路。
...
2. 通过 Consumer adapter 归一为 generic direct pay 的真实入账让利出资必须形成独立 posting plan 或独立 posting phase。
...
| 承接项 | 系分要求 |
| --- | --- |
| 参与方和账户类型 | route leg 解析后必须能落到内部资金主体和账户类型；账目由资金指令、账户 Profile 或原账本事实确定，外部账户、支付工具、用户 ID、商户 ID 只能作为引用或归属信息。 |
...
5. LedgerEntry 只通过稳定 `sn` 作为对账清算域的内部证据；批次、匹配关系、差错和处理状态由对账清算模块独立保存，不得反写分录。
...
| 产品对象 | 系统承接 | 状态口径 | 边界 |
| --- | --- | --- | --- |
| 余额投影任务 | Ledger 内部唯一 projector 的一次处理上下文，可由实现记录为任务或处理日志。 | WAITING -> CALCULATING -> VERIFYING -> APPLIED；失败为 FAILED 或 NEEDS_REBUILD。 | 只从 ledger entry 和账本累计派生余额，不读取交易投影或报表。 |
...
`LedgerTransactionPostingService` 在 Ledger 本地事务内直接使用唯一内部 projector。Funding、Credit、VCC 资金子账户、退款、撤销、外部入金和显式调账都按同一 `LedgerEntry -> normal side -> period -> balance constraint -> optimistic version` 规则派生余额；账户不存在、主体或账目不匹配、状态不可入账、币种或周期冲突、余额下限失败时整笔回滚。

当前没有第二个生产实现、异构存储或不同一致性算法，因此不保留 `List + supports`、Bean 顺序、first-supported composite 或 provider/version 预留。未来若出现真实异构投影责任，必须重新经过产品、DSL、系分、TDD 与公共契约裁决；不得直接增加第二个 Bean 分叉现有资金不变量。
...
| 层次 | 写入时机 | 一致性与用途 | 当前实现 |
| --- | --- | --- | --- |
| 交易生命周期事实 | route 解析后、账务过账前后，与账务事实处于同一本地事务。 | 强一致；承接幂等、累计金额、后续 replay 准入和事实查询。 | `DefaultFundsInstructionLifecycleSaver` 已实现。 |
| 单笔当前解释 | 查询时读取持久化交易、明细和原 RouteSnapshot。 | 只读；用于详情、客服和运营下钻，必须反映聚合剩余授权和累计金额。 | `DefaultFundsTransactionProjectionExplainApplicationService` 已实现。 |
| 正常投影通知 | 主事务提交后触发。 | 最终一致；失败不回滚资金事实。publisher 应按 `transactionSn` 读取持久化事实，不能把内存上下文当作新的事实源。 | after-commit 触发和端口已实现；无正式 publisher。 |
| 账单投影存储 | publisher 幂等写入，或治理重放写入。 | 可丢弃、可重建；面向主体和视图域分页查询。 | W2 已建最小投影表并实现治理重放 writer；正常 publisher 和公共分页查询未实现。 |
| 治理重放 | 有界任务显式触发。 | 只修复投影；必须有范围、差异报告和独立 checkpoint。 | W2 已实现持久任务、热区交易/冻结事实 source、双高水位 checkpoint、差异摘要、影子与正式 writer；批次、跨冷热和宿主控制面未实现。 |

单笔资金解释只回答“当前资金聚合是否仍可执行后续动作，以及已累计多少完成、释放和退款本金”。授权 100、部分完成 60 时，解释必须暴露剩余授权 40；已完成 80、已退本金 30 时，解释必须暴露 `completedAmount=80`、`refundedAmount=30` 和可退本金 50。上游事件时间线、部分/全额退款标签和业务主状态由对应业务域基于这些资金事实组合，不由资金投影越界生成。

当前阶段采用最小实现：单笔解释直接查询交易事实；W2 仅为受控恢复落地 `t_funds_transaction_projection`，不因此新增正式 publisher、公共分页查询或异步基础设施。只有上游出现按用户、商户、卡或账户分页查询的真实入口时，才补正常发布和查询服务。投影发布需要跨进程可靠投递时再引入 Outbox；当前进程内 after-commit 回调不声明可靠投递，也不会自动创建重放任务。
...
| RouteSnapshot / `paymentInstrumentRef` | `instrumentSn`、`state`、支付工具类型、脱敏引用、绑定号、绑定版本、绑定角色、绑定主体、准入决策、路由版本和交易发生时的入口解释。 | 使用旧 `instrumentId/status`、按当前支付工具状态重新选路、读取完整敏感字段、把支付工具当作账务主体。 |
| `RouteParticipant` / `RouteLeg` | 最终资金或额度主体、金额、路径、回放引用以及平台角色解析结果；参与方可附带直接父账户关系快照。 | 在 route 中承载账目、账期、阶段、余额效果或约束，或把账户层级、支出控制范围、Spend Rule、支付工具当作隐式资金分配结果。 |
...
7. `paymentInstrumentRef` 和交易摘要只使用 `instrumentSn/state`；本次为无兼容硬切，不读取旧 `instrumentId/status`。宿主若仍需回放旧快照，必须在发布前完成离线迁移。
...
| 交易投影重放任务 | 由 04 governance 的 t_projection_replay_task 承载；02 只定义正常交易投影生成、只读投影端口和触发边界。 | W2 持久状态为 CREATED -> COMPLETED；批次未完成保持 CREATED，失败事务回滚并保留原状态/checkpoint。暂停、取消和失败终态尚未实现。 | 只用于校验、影子重建或应用只读投影，必须有范围和差异报告；不得把重放任务表落入交易主链路表。 |
...
2. 业务唯一性必须说明唯一索引、索引字段及其保护的业务规则；技术主键不能替代业务唯一约束。
3. 普通索引必须说明索引字段和查询/排序场景；不得为未出现的查询模式提前建索引。
4. 数据约束与兼容必须说明历史数据迁移、兼容发布、数据校验与回滚、删除与归档；目标态表未落库时明确标记为新建，不虚构迁移完成状态。
...
| state | varchar(50) | [X] | 账户状态，见状态说明 | ACTIVE |
...
| idx_funding_account_state | state | 运营筛选账户状态 |
...
| ledger_profile_version | int(11) | [X] | 账本 profile 版本 | 1 |
| state | varchar(50) | [X] | 信用账户状态 | ACTIVE |
| version | int(11) | [X] | 乐观锁版本 | 0 |
...
| idx_credit_account_state | state | 状态运营查询 |
...
| state | varchar(50) | [X] | 支出控制范围状态 | ACTIVE |
| version | int(11) | [X] | 乐观锁版本 | 0 |
...
| idx_spend_control_scope_state | state | 状态运营查询 |
...
| flow_direction | varchar(50) | [X] | 工具资金流向 | OUTBOUND |
...
| currency | varchar(10) | [X] | 币种 | CNY |
| state | varchar(50) | [X] | 工具状态 | ACTIVE |
...
| idx_payment_instrument_state | state | 状态筛选 |
...
- 共享卡的 `PaymentInstrumentBinding` 只表达工具到信用子账户的绑定；使用主体、支出控制范围和 Spend Rule 由授权准入上下文、决策记录及快照承接。账户层级关系表达信用子账户的直接父账户；资金责任关系按联合业务键直接解析唯一资金账户或信用账户，不在关系内部做优先级选择。
...
业务用途：承载支出主体在指定币种和关系类型下到资金账户或信用账户的唯一资金责任关系。该表不选择工具绑定、不解释账户层级或 Spend Rule、不执行扣款、不写分录；最终主体和金额由 route participant/leg 承载。
...
| tenant_id | bigint(20) | [X] | 归属租户 | 1 |
| spend_subject_id | varchar(64) | [X] | 源资金账户或信用账户 ID。 | CA100001 |
| spend_subject_type | varchar(50) | [X] | 源主体类型，只允许 `FUNDING_ACCOUNT`、`CREDIT_ACCOUNT`。 | CREDIT_ACCOUNT |
| target_subject_type | varchar(50) | [X] | 资金责任目标主体类型，只允许 `FUNDING_ACCOUNT`、`CREDIT_ACCOUNT`。 | FUNDING_ACCOUNT |
...
- 支出控制范围、Spend Rule、支付工具、业务使用人和平台角色都不能直接作为该表的源端或目标端；调用方必须先把它们解析为具体资金账户或信用账户。
...
| external_source_code | varchar(128) | [ ] | 外部资金事实号的稳定来源命名空间 | BANK_A:ACCOUNT_TOKEN_001 |
| external_funds_fact_sn | varchar(128) | [ ] | 外部系统中一次可入账资金变动的稳定事实号 | BANK_LEDGER_ENTRY_001 |
| external_funds_effect_type | varchar(50) | [ ] | 该外部事实产生的资金效果；当前自动入金使用 DIRECT | DIRECT |
| external_funds_fact_digest | varchar(64) | [ ] | 目标账户、金额币种和适用汇率等不可变资金载荷 SHA-256 | 5e504d... |
| reference_transaction_sn | varchar(64) | [ ] | 引用原交易号 | FT000001 |
| state | varchar(50) | [X] | 聚合状态 | PROCESSING |
...
| completed_amount | bigint(20) | [X] | 累计完成金额，非负 | 1000 |
...
| uk_funds_transaction_external_fact | tenant_id, external_source_code, external_funds_fact_sn, external_funds_effect_type | 外部资金事实幂等；普通交易四字段为空，不受影响 |
...
| idx_funds_transaction_state | state | 状态运营查询 |
...
| currency | varchar(10) | [X] | 币种 | CNY |
| state | varchar(50) | [X] | 明细状态 | SUCCEEDED |
...
| idx_funds_transaction_detail_state | state | 状态查询 |
...
| currency | varchar(10) | [X] | 币种 | CNY |
| state | varchar(50) | [X] | 冻结单状态 | FROZEN |
...
| idx_funds_frozen_order_state | state | 状态查询 |
| idx_funds_frozen_order_expire | expire_time, state | 到期释放扫描 |
...
| state | varchar(20) | [X] | 账本状态 | ACTIVE |
| version | int(11) | [X] | 乐观锁版本 | 0 |
...
事实语义：`LedgerTransaction` 只有成功过账后才落库，不保存生命周期状态。业务处理中、失败、结算和冲正分别由资金交易、授权单、结算单、出款单或新增的引用交易表达。
...
- 已落库的账本交易不得更新或删除；错账通过追加引用原交易的冲正或调账交易处理。
...
- 记账计划没有独立生命周期状态；有效性由所属不可变账本交易、借贷控制总额和摘要校验共同证明。
...
- `LedgerEntry.sn` 是对账清算域引用内部账务事实的稳定标识，不代表每条分录都必须参加外部对账。
- 对账批次、匹配结果、部分匹配、差错类型、处理状态和完成时间保存在对账清算模块；同一分录可以被不同对账范围独立引用。
- 清算确认或差错修正需要改变资金时，必须追加新的资金交易和 LedgerEntry，不得更新历史分录。
...
> W2 当前实现采用恢复专用最小表，字段和索引以 `database/mysql/governance/001_create_governance_tables.sql` 为准；下列完整分页投影模型仍是目标态，未实现字段不得作为当前接入契约。
...
6. 授权累计金额统一使用 `completedAmount/completed_amount`；生产切换前必须提供 `settled_amount -> completed_amount` 迁移脚本、dry-run、数据校验和回滚方案，不长期双写或保留同义字段。
...
| 授权交易 | 授权批准、授权拒绝、撤销、过期、普通完成、complete 强制完成、完成后退款、争议裁决资金结果承接。 |
...
| --- | --- | --- | --- |
| MAIN-R-001 | 支付工具必须先解析到有效账户主体，不能直接成为交易或账本主体。 | wallet payment instrument facade、资金责任解析、transaction canonical request。 | 支付工具准入测试、路由契约测试、ledger 主体护栏测试。 |
| MAIN-R-002 | 授权批准占用 AVAILABLE，拒绝不生成 route leg、posting、LedgerEntry 或余额变化；可保留空 RouteSnapshot。 | authorization application、route resolver、ledger posting。 | 授权成功/拒绝流程测试和 forbidden facts 断言。 |
| MAIN-R-003 | 冻结和解冻只表达同主体 AVAILABLE 与 FROZEN 转换，不表达消费或价值转移。 | `FundsBalanceControlService`、余额控制 route 和 posting。 | 冻结、解冻和跨主体 must-fail 流程测试。 |
| MAIN-R-004 | 逆向动作按原路径和累计处理上限执行；找不到原路由的退款只能走上层明确选择的无原路由入口。 | transaction 原路径回放、no-original-route refund 契约、累计金额守卫。 | 普通退款、无原路由退款、超额退款和幂等冲突测试。 |
| MAIN-R-005 | 余额投影由账本分录派生，全部可入账主体使用 Ledger 内部唯一 projector。 | posting service 在本地事务内完成支持性校验、账务写入与余额投影。 | `LedgerBalanceProjectionServiceImplTests`、posting 原子性和架构边界测试。 |
...
| P1 | 让利出资记账交易契约 | Consumer Benefit adapter、generic direct pay/refund、`FundsActionFact`、通用请求摘要和 DSL 契约样例。 | `AC-BEN-*`、`RED-050` 至 `RED-057`、`DSL-BENEFIT-*` 和 `TDD-BEN-*` 已映射；Provider Benefit service/request/funding enum 与旧 `FundsInstructionSpec.benefitSnapshot` 均退出目标契约；交付结论只能按 ActionFact、route/posting 和 Consumer E4 实际证据分层声明。 |
