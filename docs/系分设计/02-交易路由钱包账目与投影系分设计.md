...
| PaymentInstrumentApplicationService | 注册工具、变更绑定、生成工具快照、校验工具状态/方向/币种/敏感信息。 | PaymentInstrumentService、绑定历史、敏感字段校验器。 | 不返回完整敏感凭证；不表达内部余额。 |
...
| AuthorizationAdmissionApplicationService | 面向卡、VCC、共享卡、钱包标识等入口完成授权前准入。 | 支付工具应用服务、资金责任解析应用服务、Spend Rule 控制、账户能力查询。 | 输出批准/拒绝和快照输入；批准后委派授权交易内核，拒绝无 route/posting/entry。 |
