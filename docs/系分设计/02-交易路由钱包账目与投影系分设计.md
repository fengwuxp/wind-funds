...
| wallet-face | 暴露真实资金账户、信用账户、预算组、平台账户角色、支付工具、支出主体资金来源关系和余额查询契约。 | 承载交易命令或账本过账命令；把 `FundingAccount` 暴露成信用、预算和支付工具的通用父类。 |
...
3. 资金账户余额调整、信用账户额度调整和预算组额度调整由余额控制服务的 adjust 承接；对账差错调账必须先经过差错单、审批、凭证和审计闭环，再触发余额控制 adjust 或批次授权的直接交易调账事实。
...
| 资金账户余额调整 / 信用账户额度调整 / 预算组额度调整 | adjust | FundsBalanceAdjustRequest；编码落地时可按资金账户、信用账户、预算组补充专用字段或专用 Request | 调整流水号、调整后目标账目余额摘要或 LIMIT / AVAILABLE 摘要 | 具备调整对象、审批、凭证或规则版本、原因、目标账目和操作者；资金账户余额调整必须有差错、运营修正或财务调整来源 | 调整业务流水 + 请求摘要 | 缺审批、缺来源、错币种、破坏已授权或已使用约束、违反负余额策略时不生成分录。 |
...
4. adjust 支持资金账户余额调整、信用账户额度调整和预算组额度调整；资金账户余额调整必须是同主体、同币种、同账本周期内的受控余额修正，并有差错、运营修正或财务调整来源。
...
| 信用账户 | 管理授信额度、可用额度和授权占用。 | `CreditAccountService`、授权交易、余额控制服务。 | LIMIT、AVAILABLE、AUTHORIZATION、额度周期、调额来源、授信责任和规则版本。 | 先创建信用账户并设置额度；VCC 或授权交易通过资金来源决策引用它，不把卡工具写入信用账户。 |
| 预算组 | 管理预算总量、可用预算、预算授权占用和周期控制。 | `BudgetGroupService`、授权交易、余额控制服务。 | 预算 owner、periodType、periodId、LIMIT、AVAILABLE、AUTHORIZATION、规则版本和预算周期。 | 先创建预算组并配置周期；共享卡、员工卡或项目费用通过绑定关系引用它。 |
| 支付工具 | 管理卡、VA、钱包标识、通道 token、虚拟卡和共享卡工具快照。 | `PaymentInstrumentService`、route resolver。 | 状态、方向、币种、绑定版本、脱敏引用、敏感字段和资金来源唯一性。 | 先注册支付工具，再绑定到使用主体或内部责任主体；交易发生时固化 `PaymentInstrumentRef`。 |
| 支出主体资金来源关系 | 连接信用账户、预算组、工具或使用主体与真实资金来源。 | `SpendSubjectFundingRelationService`、route resolver。 | 关系类型、优先级、默认标识、币种、有效期和多命中冲突。 | 配置谁可以从哪个资金账户、信用账户、预算组或平台账户角色承担资金责任。 |

VCC 场景中，调用方不能直接要求“创建预付卡资金账户”或“创建共享卡信用账户”。正确顺序是：注册卡工具，建立绑定和资金来源关系，必要时创建资金账户、信用账户或预算组作为内部责任主体，再由授权交易解析出唯一 `FundingAllocationDecision`。
...
5. `t_funding_account`、信用账户表和预算组表可以共享 ledger 初始化机制，但不得共享同一实体语义；主体类型必须在 `SubjectRef`、route leg、posting plan 和 ledger entry 中保持可区分。

#### 4.4.3 支付工具状态与路由准入

支付工具只提供路由输入、绑定关系和审计快照，不表达内部余额。交易、授权、提现、入金识别和退款进入路由前，必须先完成工具状态、方向、币种、绑定和资金来源校验。钱包标识属于支付工具引用或外部端点引用，不是钱包账户域本身；使用钱包余额支付时，路由必须先解析出内部 `SubjectRef`。

VCC 卡、虚拟卡、卡 token、预付虚拟卡和共享卡在系统层都先进入 `PaymentInstrument` 体系，而不是进入账户或账本体系。预付卡只改变资金来源解析和责任账户选择，不改变工具不记余额的原则；共享卡只改变绑定、使用人、预算组和资金来源快照，不新增“共享卡账户”或“共享卡账本主体”。
...
8. 如果 `PaymentInstrumentRef` 表示钱包标识，route leg 仍必须使用解析后的资金账户、信用账户、预算组或平台账户角色；不得把钱包标识、工具号或 token 作为 ledger subject。
...
| --- | --- | --- | --- |
| VCC 普通授权 | `PaymentInstrumentRef` + `FundingAllocationDecision`。 | 卡工具、脱敏展示号、绑定版本、资金账户/信用账户/预算组、规则版本和外部授权引用。 | 卡、token、持卡人或外部卡账户成为 `LedgerEntry.subject`。 |
| prepaid virtual card | 工具仍在 `PaymentInstrument`；预付资金责任由 `SpendSubjectFundingRelationService` 解析到内部主体。 | 预付资金来源、责任账户、财务确认引用、退款处置和规则版本。 | 把预付卡写成内部余额账户，或把储值券、礼品卡和预付代金券混入 VCC 工具模型。 |
| shared card | 一个工具可绑定使用人、部门、项目、预算组和兜底资金来源。 | 使用人、绑定角色、绑定版本、预算组、资金来源关系、优先级、默认标识和规则版本。 | 新增共享卡资金账户，或在逆向事件中按当前绑定重新解释历史授权。 |
...
| 2. 业务模式识别 | 普通 VCC、prepaid virtual card、shared card、credit/charge/debit 模式可被唯一识别。 | 选择对应资金来源解析策略。 | 进入待确认或 contract-only；不得猜测资金责任主体。 |
| 3. 绑定快照 | 找到唯一有效绑定、绑定版本、使用人上下文和预算约束。 | 固化到 route snapshot。 | 多绑定、缺版本或已失效时失败无账务副作用。 |
| 4. 资金来源关系 | 解析到唯一资金账户、信用账户、预算组或平台账户角色。 | 生成 `FundingAllocationDecision`。 | 多资金来源、缺预付资金确认或责任主体不明确时失败无账务副作用。 |
| 5. 账户能力与余额 | 内部主体具备动作能力、账本周期、余额/额度/预算满足。 | 进入授权 route 和 posting。 | 余额不足、额度不足、周期缺失或能力不匹配时拒绝，无账务路径。 |
...
| 平台补贴表达 | 同一指令额外 leg，或独立伴随指令；选择独立伴随指令时必须声明主交易和伴随指令的事务边界、业务组幂等键、失败补偿、冲正、投影合并和部分成功处理。 | `RouteResolver`、`RouteSnapshotFactory`、`PostingAssembler`、幂等摘要、交易投影、审计。 | 不把平台补贴与用户本金净额混记；缺资金来源或伴随指令原子性策略时失败或人工处理。 |
...
- prepaid virtual card 的预付资金余额不存放在本表，必须通过资金来源关系解析到经确认的内部资金责任主体。
- 共享卡不要求新增工具表字段；使用人、部门、项目、预算组和资金来源通过绑定、资金来源关系和 route snapshot 表达。
...
- 共享卡绑定必须能表达工具、使用主体、预算组、资金账户或信用账户之间的关系；多关系命中时必须由资金来源关系和优先级决策出唯一结果。
- prepaid virtual card 绑定不得以卡工具自身作为资金来源；必须关联内部预付资金责任主体或待专业确认后阻断。
...
- funding_account_id 按字符串保存；若目标态改为 funding_account_sn，必须同步表结构、Entity、Mapper、测试数据和查询契约。
...
业务用途：目标态表，记录 FundsBalanceControlService.adjust 产生的资金账户余额调整、信用账户额度调整和预算组额度调整动作。它是调整流水号、来源、审批、凭证、审计和账本引用的事实载体；现有测试 DDL 尚未落表，编码实现前必须补充 DDL、Entity、Mapper 和服务层测试。
...
| account_sn | varchar(64) | [ ] | 资金账户、信用账户或预算组编号 | FA100001 |
| ledger_subject_code | varchar(50) | [ ] | 资金账户目标账目或额度账目 | AVAILABLE |
...
| operator_id | varchar(64) | [ ] | 操作者或系统来源 | system |
| funds_transaction_sn | varchar(64) | [ ] | 关联直接交易调账资金事实；同主体余额调整可为空 | FT100001 |
| ledger_transaction_sn | varchar(64) | [ ] | 关联账本交易号 | LT100001 |
| status | varchar(50) | [X] | 动作状态 | SUCCEEDED |
...
- adjust_object_type：FUNDING_ACCOUNT、CREDIT_ACCOUNT、BUDGET_GROUP。
- adjust_type：FUNDING_BALANCE、CREDIT_LIMIT、BUDGET_LIMIT。
...
- 信用账户额度调整和预算组额度调整只影响 LIMIT / AVAILABLE 控制口径，不表达现金流、外部通道或清结算结果。
...
| 余额控制 | 冻结、部分解冻、全量解冻、超额解冻、冻结后追偿、资金账户余额调整、信用账户额度调整、预算组额度调整、adjust 绕过差错闭环红线。 |
...
| 优先级 | 切片 | 交付内容 | 准入门禁 |
| --- | --- | --- | --- |
| P0 | 账户与账本初始化 | 资金账户、信用账户、预算组、支付工具、绑定关系、账本 bucket。 | 表结构、唯一键、显式建账、周期红线和账户查询测试通过。 |
| P0 | 账务与余额投影 | posting plan、ledger transaction、ledger entry、余额投影、余额日志。 | 独立平衡、分录不可变、余额投影一致、日志失败不回滚测试通过；余额投影可从分录重建。 |
| P0 | 支付工具与资金来源 | 支付工具、绑定关系、外部账户引用、支出主体资金来源关系、工具准入守卫和快照。 | 工具状态/方向/币种/账户能力/资金来源唯一性测试通过；敏感信息不得入普通快照、日志、导出或报表；工具换绑后原路径回放测试通过。 |
