...
- 金额字段统一使用最小货币单位，字段类型优先 bigint(20)，不使用浮点数；币种字段使用 varchar(10)，取 ISO 4217 或内部等价编码。
- 所有跨模块引用使用业务流水号，不暴露数据库主键；数据库不建外键，不使用级联删除，逻辑关联由应用服务和测试保证。
- 事实表、历史表、审计表只追加或状态迁移，不覆盖金额、主体、账目、周期、借贷方向和业务引用。
- 状态字段必须有明确枚举、终态和非法流转；新增必填字段必须给默认值，破坏性字段变更先做兼容发布。
- route_snapshot、context_variables、before_snapshot、after_snapshot 等大字段只保存脱敏 JSON 或摘要，不参与高频过滤，不保存完整卡号、完整外部账户、密钥或敏感凭证。

#### 4.11.1 通用字段约定

| 字段组 | 字段名称 | 字段类型 | 是否必填 | 字段说明 | 值示例 |
| --- | --- | --- | --- | --- | --- |
| 主键 | id | bigint(20) | [X] | 自增主键，仅模块内部持久化使用 | 10001 |
| 审计 | gmt_create | datetime(3) | [X] | 创建时间，默认当前数据库时间 | 2026-05-20 10:00:00.000 |
| 审计 | gmt_modified | datetime(3) | [X] | 最后更新时间，默认当前数据库时间并随更新刷新 | 2026-05-20 10:00:01.000 |
| 租户 | tenant_id | bigint(20) | [X] | 归属租户，必须参与幂等或范围查询 | 1 |
| 标识 | sn | varchar(64) | [X] | 业务流水号；表内全局唯一，跨模块引用优先使用 | FA202605200001 |
| 状态 | status | varchar(50) | [X] | 当前业务状态；枚举见各表状态说明 | ACTIVE |
| 乐观锁 | version | int(11) | [ ] | 并发控制字段；当前态表必填，事实明细表可不使用 | 1 |
| 扩展 | description | varchar(512) | [ ] | 脱敏描述，不参与核心判断 | 主账户 |
| 扩展 | context_variables | text | [ ] | 脱敏 JSON 上下文；不得保存敏感原文，不参与高频查询 | {"traceId":"T1"} |
