...
| 权益快照契约 | `FundsInstructionSpec.benefitSnapshot`、权益 Spec、不可变模型、枚举和 DSL 契约样例。 | `AC-BEN-*`、`RED-050` 至 `RED-057`、`DSL-BENEFIT-*` 和 `TDD-BEN-*` 已映射；Execution Grant 明确允许公共契约和枚举变更；不要求本切片修改 route snapshot、表结构或完整 route/posting 消费。 |
| 权益路由账务消费 | 商户券不入账、平台补贴独立 leg、储值券负债口径、授权占券、退款补贴冲回、posting context。 | route snapshot 或等价快照可用于 replay；posting plan 独立平衡；不退券和不冲补贴分开；零实付、储值券和平台补贴账户角色的待确认项已关闭或明确不在本批次。 |
