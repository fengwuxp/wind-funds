...
钱包账户是能力域，不是单一实体。系统层以 `FundingAccount`、`CreditAccount`、`BudgetGroup`、`PlatformFundingAccountRole`、`PaymentInstrument`、`SpendSubjectFundingRelation` 和余额查询服务共同承接钱包产品能力；其中 `FundingAccount` 只表示真实资金账户或平台责任资金账户。统一路由和入账时使用 `FundsSubjectType` / `SubjectRef`，统一支付入口和工具快照时使用 `PaymentInstrumentRef`，不得用 `FundingAccount` 兼容所有主体。
...
| PaymentInstrumentCapabilityApplicationService | 注册工具、变更绑定、生成工具快照、校验工具状态、方向、动作能力、有效期、币种和敏感信息。 | PaymentInstrumentService、绑定历史、敏感字段校验器。 | 不返回完整敏感凭证；不表达内部余额；也可在实现命名中保留 `PaymentInstrumentApplicationService` 作为兼容 facade，但语义必须是工具能力准入。 |
...
若后续新增 `authorizeByInstrument` 或 `payByInstrument`，它们只能位于 application/use-case facade：先解析工具能力、绑定、预算控制、资金责任和账户能力，再委派账户主体型交易内核。不得把 `FundsAuthorizationTransactionAuthorizeRequest`、`FundsTransactionPayRequest` 或余额控制请求整体替换成支付工具引用入参。
