...
| 层级 | 职责 | 接口契约口径 | 事务边界 |
| --- | --- | --- | --- |
| `wallet.application` | 承接支付工具、VA、VCC、全球账户、ACH 等业务入口，完成准入、绑定、账户能力、Spend Rule、外部引用和预交易快照组合。 | 已有 `authorizeByInstrument` 和 `receiveByInstrument` 最小入口；后续可以新增 `payOutByRail`、`consumeExternalFundsEvent` 等应用层入口。入参必须包含业务流水、场景、金额、币种、工具或 rail 引用和可解析账户主体，收款工具入口必须携带调用方期望的绑定版本。 | 失败应停在交易事实前；通过后委派 transaction canonical service；收款工具入口应在 route snapshot 固化脱敏 `PaymentInstrumentRef` 和外部账户引用。 |
