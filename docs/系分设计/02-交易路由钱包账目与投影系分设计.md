...
| PaymentInstrumentCapabilityApplicationService | 注册工具、变更绑定、生成工具快照、校验工具状态/方向/币种/敏感信息。 | PaymentInstrumentService、绑定历史、敏感字段校验器。 | 不返回完整敏感凭证；不表达内部余额；也可在实现命名中保留 `PaymentInstrumentApplicationService` 作为兼容 facade，但语义必须是工具能力准入。 |
...
| 支付工具 | 管理卡、VA、钱包标识、通道 token、虚拟卡和共享卡工具快照。 | `PaymentInstrumentService`、route resolver。 | 状态、方向、币种、绑定版本、脱敏引用、敏感字段和资金责任唯一性。 | 先注册支付工具，再绑定到使用主体或内部责任主体；交易发生时固化 `PaymentInstrumentRef`。 |
...
支付工具只提供路由输入、绑定关系和审计快照，不表达内部余额。交易、授权、提现、入金识别和退款进入路由前，必须先完成工具状态、方向、币种、绑定和资金责任校验。钱包标识属于支付工具引用或外部端点引用，不是钱包账户域本身；使用钱包余额支付时，路由必须先解析出内部 `SubjectRef`。
...
3. 资金责任解析关系必须解析为唯一的资金账户、信用账户或平台账户角色；预算组和 Spend Rule 只提供控制条件、规则版本和审计证据，不能作为最终资金责任主体；缺失、不唯一、优先级冲突或默认关系冲突时失败。
...
7. 支付工具状态、绑定、方向、优先级和资金责任解析关系变更必须记录操作者、时间、原因、版本和前后值。
8. 如果 `PaymentInstrumentRef` 表示钱包标识，route leg 仍必须使用解析后的资金账户、信用账户或平台账户角色；不得把钱包标识、工具号、token、预算组或 Spend Rule 作为 ledger subject。
...
| 步骤 | 系统判断 | 通过结果 | 失败结果 |
| --- | --- | --- | --- |
| 1. 工具合法性 | 工具存在、状态可用、方向匹配、币种匹配、脱敏引用合法。 | 进入绑定解析。 | 授权拒绝或工具准入失败；无 route、posting、entry。 |
