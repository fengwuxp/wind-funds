...
| 账单投影存储 | publisher 幂等写入，或治理重放写入。 | 可丢弃、可重建；面向主体和视图域分页查询。 | 目标设计，当前未建表、未实现查询。 |
| 治理重放 | 有界任务显式触发。 | 只修复投影；必须有范围、差异报告和独立 checkpoint。 | 服务骨架已实现；真实 source/writer 未实现。 |
...
当前阶段采用最小实现：单笔解释直接查询交易事实，不新增投影表或异步基础设施；只有上游出现按用户、商户、卡或账户分页查询的真实入口时，才落地 `t_funds_transaction_projection`、正式 publisher 和查询服务。投影发布需要跨进程可靠投递时再引入 Outbox；当前进程内 after-commit 回调不声明可靠投递。
