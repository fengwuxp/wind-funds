...
状态收口规则：授权交易不提供独立 close 服务入口。`FundsTransactionStatus.OPEN/CLOSED` 只表达资金授权聚合是否仍可执行后续资金动作，不是任何上游业务的主状态。`remainingAuthorizedAmount = amount - completedAmount - reversedAmount` 仅作为资金准入和查询解释派生值；上游业务状态、事件状态和展示标签由对应业务域维护。授权超时或 expired 不是可信资金结果，不自动释放占用；只有可信 reversal、void、清算剩余释放或差错补事实可改变资金聚合。
...
`wind-funds` 不定义上游业务主状态或事件应用状态，也不将自身 `FundsTransactionStatus.OPEN/CLOSED` 直接翻译为 VCC 、收单或全球账户状态。资金层只保证：
