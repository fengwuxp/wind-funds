...
sequenceDiagram
    ...
    Control->>Control: 校验同主体、同币种、终局证据、审批和负余额策略
...
3. 纠偏后若 `AVAILABLE < 0`，后续支付、授权、冻结、提现、出款、清算放行和报表展示必须读取负余额策略或出款门禁，不能把负可用解释为可继续使用额度。
...
| AuthorizationAdmissionApplicationService | 面向卡、VCC、共享卡、外部钱包端点、通道 token 或内部支付方式选择等入口完成授权前准入。 | 支付工具能力应用服务、账户层级应用服务、资金责任解析应用服务、Spend Rule 控制、账户能力查询。 | 输出批准/拒绝和快照输入；批准后委派授权交易内核，拒绝无 route/posting/entry。 |
...
| 资金责任解析 | 现有关系仍以 `fundingAccountId` 为主要目标字段。 | 解析结果必须是资金账户、信用账户、VCC 关联资金/信用子账户或平台角色解析后的平台资金账户；预算组和 Spend Rule 不能成为结果主体。 | B2 必须先决定 `targetSubjectType + targetSubjectId` 或明确 funding-account-only；涉及 VCC 子账户时不得继续用 funding-account-only 声明目标态完成。 |
| Spend Rule 控制 | 目标态需要独立 Spend Rule 定义、版本、决策日志、控制活动和预算控制投影。 | 规则定义、规则版本、决策结果、拒绝原因、预留、释放、调整和审计必须可追溯。 | 需独立工程任务承接，不得混入直接交易主链路。 |
...
4. 再补 Spend Rule 控制事实：规则定义、版本、决策日志、Spend Control Activity、预算预留和释放。
5. 最后补授权准入组合 facade：从支付工具引用进入，批准后委派账户主体型授权内核，拒绝时只留下可审计拒绝事实。
...
路由层在系统分层中是确定性路径解析器和快照生成器，不是清结算对象服务、对账服务、账本写入器或交易生命周期管理器。交易编排层可以保存 route snapshot 并把它交给账务计划装配；`RouteResolver` 本身只消费 `FundsInstructionSpec`、原 route snapshot 和已固化的规则/支付工具/权益快照。
...
| 营销账户承接 | 平台营销成本账户、商户营销让利账户、权益负债账户、合作方补贴账户和营销留置账户是否纳入本期，以及它们对应的资金账户、平台责任账户或账户 profile。 | 钱包账户、route resolver、posting assembler、清结算、对账和交易投影。 | 未选择时只能保留权益快照契约，不得让平台补贴、储值负债释放、合作方补贴或营销留置进入生产资金流。 |
...
| benefit funding resolver | 从权益组件的 `ledgerEffect`、`fundingNature`、承担方、受益方、资金来源角色和专业确认状态解析营销账户引用。 | 计算优惠金额、判断券是否可用、读取当前券包状态或替代营销系统。 |
| marketing account guard | 在 route/posting 前校验有资金影响组件是否具备账户引用、账户 profile、专业确认状态、核销引用和可回放摘要。 | 缺账户、缺确认或 `NO_LEDGER` 组件时静默放行。 |
...
| 接口或组件 | 建议落点 | 职责 | 禁止行为 |
| --- | --- | --- | --- |
| `FundsBenefitFundingApplicationService` | 应用门面，建议落在 `transaction-face` 的 application 边界。 | 接收业务侧已决策权益快照或等价快照，编排资金责任解析、营销账户 guard、幂等摘要输入和既有交易服务委派。 | 不新增独立交易状态机，不直接写 route、posting、LedgerEntry、余额投影，不绕过 `FundsDirectTransactionService` 和 `FundsAuthorizationTransactionService`。 |
...
| `MarketingAccountGuard` | route/posting 前置 guard。 | 校验账户引用、账户 profile、专业确认状态、核销或占用引用、原权益快照和可回放摘要。 | 不把 `NO_LEDGER` 组件转成资金分录，不在缺证据时继续入账，不替代清结算或对账差错处理。 |

`FundsMarketingTransactionService` 不作为目标态接口。权益交易授权、权益交易结算、权益交易退款在工程上分别映射到授权生命周期、直接交易/授权完成和退款/撤销回放，不允许形成一套平行的 marketing transaction lifecycle。若确有独立权益资金事实需要伴随指令承接，必须以 `companion instruction group`、原子性模式、补偿策略、投影合并键和失败无副作用测试证明它没有替代原交易生命周期。
...
Ledger 模块的系统定位是账务事实层和余额投影事实源。`ledger-face` 对外暴露账本 bucket、账本交易、posting plan、ledger entry、余额投影和账务证据查询能力；`ledger-impl` 负责落地账务事实、维护余额投影和发布余额观察事件。Ledger 模块不承接业务交易入口、路由决策、支付工具解析、Spend Rule 决策、清结算批次状态机或对账差错生命周期。
...
| 统一过账 | `LedgerTransactionPostingService`、`LedgerTransactionService.postLedgerTransaction` | 校验 `LedgerTransactionSpec`、`PostingPlan`、`LedgerEntry` 和幂等摘要，原子写入账本交易、posting plan、不可变分录和余额投影。 | 重新解析 route、重选工具绑定、解释业务订单生命周期、接收业务方直接指定的借贷方向。 |
| 账务证据查询 | `LedgerTransactionService` 查询方法、`LedgerEntryQuery`、`LedgerTransactionQuery` | 按主体、账目、币种、周期、业务引用、交易引用、清结算标记和对账标记查询账务证据。 | 查询时修复状态、补写分录、重算余额或推进清结算和对账状态机。 |
| 余额投影 | `LedgerBalanceProjectionService` | 从 `LedgerEntry` 派生余额，支持当前余额、周期余额、检查点和重建输入。 | 从交易投影、余额日志、报表或人工汇总反写账本事实。 |
...
目标态说明：兼容字段可继续以 `funding_account_id` 命名目标字段；进入工程落地前需要由 `资深架构师` 联动确认是否迁移为 `target_subject_type + target_subject_id`，或保留兼容字段并在 service/DTO/DDL/TDD 中显式声明只能解析到资金账户。本文档先固化产品和系统目标口径，不视为已授权修改生产代码、测试 schema 或数据库迁移。
...
| funding_account_id | varchar(30) | [X] | 兼容字段：目标资金账户 ID 或业务号；在 `funding-account-only` 切片中不得承载信用账户、平台角色或预算组；目标态如支持信用账户或平台账户角色，需迁移为目标主体类型 + 目标主体 ID。 | FA100001 |
| currency | varchar(10) | [X] | 币种 | CNY |
...
- 外部余额异常纠偏允许把同主体 AVAILABLE 调整为负数，但必须同步负余额策略、风险状态、追偿或人工处理引用；后续支付、授权、冻结、提现、出款或清算放行不得绕过该负余额状态。
...
| P1 | 权益路由账务消费 | 商户券不入账、平台补贴独立 leg、营销账户承接、储值券负债口径、授权占券、退款补贴冲回、posting context。 | route snapshot、交易事实快照、独立权益快照表或等价不可变存储至少一个可用于 replay；幂等摘要覆盖权益快照稳定摘要；posting plan 独立平衡；不退券和不冲补贴分开；零实付、储值券、平台补贴账户角色和营销账户 profile 的待确认项已关闭或明确不在工程任务范围。 |
