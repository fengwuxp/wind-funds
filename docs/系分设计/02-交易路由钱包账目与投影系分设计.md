...
| `LedgerEntry` | 主体必须是具体资金账户或信用账户节点；建议固化 `parentAccountRef`、`rootAccountRef`、`accountPath`、`accountLevel`、`hierarchyVersion`、`postingRole` 和 `sourceInstrumentRef` 的脱敏引用。 | 不能把父账户聚合视图、支付工具、预算组、Spend Rule、卡号、PAN、token 或 `VCC_ACCOUNT` 写成分录主体。 |
...
1. route snapshot 必须固化原始 `accountRef`、`parentAccountRef`、`rootAccountRef`、`accountPath`、`hierarchyVersion` 和 `postingRole`，用于退款、撤销、拒付和重放。
