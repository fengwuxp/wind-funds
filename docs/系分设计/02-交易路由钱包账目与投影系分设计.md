...
本分册进入编码前按下表确认结构完整性，避免把目标态设计误读成单批次实现授权。

| 完整性项 | 本分册落点 | 准入要求 |
| --- | --- | --- |
| 需求背景和问题 | 第 1 节说明交易、路由、钱包、账目和投影要回答的资金事实问题。 | Execution Grant 必须映射产品验收 ID、DSL caseId、系分章节和 TDD 用例。 |
| 目标、非目标和边界 | 第 2.1 至 2.3 节定义业务目标、技术目标和非目标；第 3.2 节定义模块职责边界。 | 未授权的公共契约、表结构、状态机和模块边界不得在编码批次中顺手修改。 |
| 概要设计和关键依赖 | 第 3.1 节主链路架构、第 3.2 节模块职责、第 4 节各能力详细设计。 | 交易编排、路由、账务、钱包、投影和查询依赖方向必须符合仓库模块边界。 |
| 详细设计、接口设计和数据设计 | 第 4 节服务契约、状态机、表设计、投影和审计章节。 | 触碰 Request、DTO、枚举、状态机或 H2 schema 时必须在 Execution Grant 中显式授权。 |
...
| 研发计划、负责人和验收方式 | 第 6 节实施切片与准入门禁。 | 本分册只给切片级计划，具体负责人、里程碑和验收方式以 Harness Plan 与批次 Execution Grant 为准。 |
...
| 差错调账入口 | 对账差错链路触发 FundsBalanceControlService.adjust 或直接交易调账事实，具体公共契约由编码批次授权确认 | FundsBalanceAdjustRequest 或批次授权的调账 Request | 调整流水号或调账资金交易流水号 | 差错单或调整来源 + 调整业务流水 + 请求摘要 | 必须有审批、凭证、责任主体、原因和审计；不得绕过差错闭环或直接改余额。 |
...
3. 后续 B3-05 或 B6-06 编码批次如需落地含权益直接交易，应在人工确认的 Execution Grant 下扩展 `FundsTransactionPayRequest`、`FundsTransactionRefundRequest`、指令转换和 route replay 所需字段；B1-10 不修改直接交易服务入口和 Request/DTO。
...
2. `FundsAuthorizationTransactionAuthorizeRequest` 可在后续 B4-09 编码授权中新增可选 `benefitSnapshot` 或等价明确字段，用于固化 `HOLD_ONLY` 占用引用；授权拒绝、支付工具准入失败或余额不足时，不核销权益，不生成 route、posting 或 entry。
3. `reversal`、`expire`、`settle` 和 `settleRefund` 原则上读取原授权事实、原 route snapshot 和原权益快照，不接收当前重新计算的权益结果；完成核销、撤销释放、过期释放和退款冲回必须沿原快照处理。
4. `expire` 是授权生命周期既有缺口，需要在 B4-03 经人工确认后新增事件和服务入口；它不是优惠券或权益设计引入的新增入口。
...
8. 如编码批次沿用现有通用 adjust 命名，任务说明必须写明其产品语义限制和准入来源；不得把 AC-ADJ-001 简化成无差错、无审批、无凭证、无重新对账的余额直接修改。
...
| 阶段 | 工程落点 | 允许做什么 | 不做什么 |
| --- | --- | --- | --- |
| Phase 1 契约承载 | `core` 的 `FundsInstructionSpec`、权益 Spec、不可变模型和枚举；DSL 契约测试。 | 让资金指令可携带 `benefitSnapshot`；无权益交易遵循空值语义；验证金额闭合、组件类型和退款处置。 | 不要求 RouteResolver、PostingAssembler、交易表或 route snapshot 立即完整消费。 |
| Phase 2 路由账务消费 | `RouteResolver`、`RouteSnapshotFactory`、`LedgerPostingAssembler`、`DefaultRouteReplayService`。 | 商户让利 `NO_LEDGER` 不生成 leg；平台补贴、储值券和合作方补贴按资金性质生成独立资金影响；退款和授权释放读取原权益快照。 | 不把营销规则、券可用性判断或券生命周期推进放进资金底座。 |
| Phase 3 清结算对账投影 | 清分明细、清算候选、对账差错、交易投影、运营时间线、财务核对视图和指标项输入。 | 展示和核对商户让利、平台补贴、代金券核销、补贴冲回、不退券和权益差异。 | 不用清结算、指标结果或报表模块输出反写交易事实、账本分录或营销权益状态。 |

阶段门禁：

1. Phase 1 只表示 DSL 和 JSON 契约可承载权益快照，不得宣称含权益交易、退款、授权后续事件、清结算或对账已具备生产能力。
2. Phase 2 开始前，必须确定权益快照持久化位置：route snapshot、交易事实快照、独立权益快照表或等价不可变存储，至少一个作为 replay 和对账的事实来源。
3. Phase 2 的幂等摘要必须包含权益快照稳定摘要；稳定摘要至少覆盖 `benefitSnapshotId`、`benefitGroupSn`、组件序列、组件金额、`closureRole`、`ledgerEffect`、`fundingNature`、规则版本和退款处置。
4. Phase 2 不得只在 `contextVariables` 写入权益摘要；生产长期回放必须依赖 route snapshot、交易事实快照、独立权益快照表或等价不可变存储。
5. Phase 3 不能从交易投影、清结算汇总或报表反推权益组件；缺原权益快照时只能失败、跳过或进入人工处理。

编码准入决策卡：

| 决策项 | 必须选择的内容 | 影响模块 | 未选择时默认处理 |
| --- | --- | --- | --- |
| Phase/Batch 边界 | Phase 1 contract-only、Phase 2 route/posting/replay 或 Phase 3 replay/清结算/对账/投影/归档/冷热读取/治理重放消费，并绑定具体 Harness 批次。 | OpenSpec、Harness、TDD、目标测试资产。 | 不得把历史 B1/B1-10 契约承载当作后续生产消费 Done。 |
| JSON 夹具级别 | `CONTRACT_ONLY` 或资金流夹具；Phase 2 资金流夹具必须声明 route、posting、余额或失败无副作用断言；Phase 3 资金流夹具还必须按范围声明 replay、projection、settlement、reconciliation 或 archive 断言。 | DSL JSON verifier、TDD 契约测试、服务级测试、清结算/对账/归档专项测试。 | 只能声明字段和 validation 可解析，不声明生产资金流。 |
| 权益快照事实源 | `RouteSnapshotSpec`、交易事实快照、独立权益快照表或等价不可变存储之一。 | `transaction-impl`、`ledger-impl`、归档重放、清结算、对账。 | 只能停留在 Phase 1 contract-only，不进入 route/posting 生产消费。 |
...
| 审计证据包 | 确认记录、有效期、适用范围、脱敏证据引用、审计责任人和撤销/变更处理必须齐备；证据引用不得保存敏感原文。 | Execution Grant、运营后台、审计查询、清结算/对账/归档和生产验收。 | 缺失、过期、范围不匹配或含敏感原文时阻断或降级为 contract-only。 |
...
| 外部规则核验状态 | 税务、会计、合同、卡组织、银行、通道、KYC/KYB/AML、客户资金、跨境或外汇口径的规则来源、版本或发布日期、生效日期、适用主体或适用范围、适用法域、核验日期、确认方和确认状态。 | Execution Grant、自动入账、退款、清结算、出款、归档、治理重放和审计放行。 | 未核验时只能降级、阻断或转人工，不得作为自动资金处理依据。 |

Phase 2 或 Phase 3 进入编码时，Execution Grant 必须同时写明以上决策项、允许修改的公共契约、表结构或快照结构、目标测试清单和禁止越界项。没有决策卡的批次只能做 contract-only 或文档准备。
...
1. 当前资金指令主金额要求正数；零实付订单不能在未授权修改金额语义前直接作为用户付款主指令表达，需拆为补贴或代金券资金事实，或在后续编码授权中明确调整零金额规则。
...
6. `contextVariables` 只能作为短期迁移通道，且只能放可追溯引用或稳定摘要；组件金额、资金责任、退款处置和当前营销规则不得长期放在上下文里。
...
`LedgerPostingAssembler` 和账本写入实现必须承接 DSL 的 [资金场景借贷平衡与账务期望表](../DSL设计/支付资金底座DSL承载层设计.md#51-资金场景借贷平衡与账务期望表)。该表只定义场景期望，不替代实现中的 `normalBalanceSide` 推导、表字段约束和服务级测试。
...
| P1 | 权益快照契约 | `FundsInstructionSpec.benefitSnapshot`、权益 Spec、不可变模型、枚举和 DSL 契约样例。 | `AC-BEN-*`、`RED-050` 至 `RED-057`、`DSL-BENEFIT-*` 和 `TDD-BEN-*` 已映射；Execution Grant 明确允许公共契约和枚举变更；不要求本切片修改 route snapshot、表结构或完整 route/posting 消费；交付结论只能写“契约承载 Done”，不得写“含权益生产链路 Done”。 |
| P1 | 权益路由账务消费 | 商户券不入账、平台补贴独立 leg、储值券负债口径、授权占券、退款补贴冲回、posting context。 | route snapshot、交易事实快照、独立权益快照表或等价不可变存储至少一个可用于 replay；幂等摘要覆盖权益快照稳定摘要；posting plan 独立平衡；不退券和不冲补贴分开；零实付、储值券和平台补贴账户角色的待确认项已关闭或明确不在本批次。 |
