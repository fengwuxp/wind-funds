...
| --- | --- | --- | --- |
| Phase/Batch 边界 | Phase 1 contract-only、Phase 2 route/posting/replay 或 Phase 3 清结算/对账/投影消费，并绑定具体 Harness 批次。 | OpenSpec、Harness、TDD、目标测试资产。 | 不得把历史 B1/B1-10 契约承载当作后续生产消费 Done。 |
| JSON 夹具级别 | `CONTRACT_ONLY` 或资金流夹具；资金流夹具必须声明 route、posting、余额或失败无副作用断言。 | DSL JSON verifier、TDD 契约测试、服务级测试。 | 只能声明字段和 validation 可解析，不声明生产资金流。 |
...
| 平台补贴表达 | 同一指令额外 leg，或独立伴随指令。 | `RouteResolver`、`RouteSnapshotFactory`、`PostingAssembler`、幂等摘要。 | 不把平台补贴与用户本金净额混记；缺资金来源时失败或人工处理。 |
...
| 历史无权益快照处理 | 失败、人工处理、迁移补录或只读差错。 | 逆向交易、归档重放、对账差错、运营后台。 | 默认失败或人工处理。 |
| 专业确认状态 | 财务、税务、会计、合规、法务或业务确认状态；未确认时必须写明降级处理。 | 平台补贴、储值/预付、礼品卡、负债释放、转损益、退款和清结算。 | 只能停留在契约或设计验证，不得自动入账、退款或结算。 |
...
| 权益金额组件 | 无权益空值语义、商户券 no-ledger、平台补贴 posting-required、平台券不补足商户、储值券资金性质、授权占券、不退券但冲补贴、部分退款分摊、缺原权益快照失败或人工处理。 |
