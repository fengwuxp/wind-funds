...
| 层次 | 当前服务 | 完成度 | 继续扩展原则 |
| --- | --- | --- | --- |
| 基础服务 | `FundingAccountService`、`CreditAccountService`、`SpendControlScopeService`、`PlatformFundingAccountService`、`PaymentInstrumentService`、`PaymentInstrumentBindingService`、`PaymentInstrumentBindingHistoryService`、`SpendSubjectFundingRelationService`、`AccountHierarchyBindingService`、`AccountHierarchyService`、`FundsSubjectBalanceQueryService`、`LedgerProfileService`、`SubjectLedgerInitializer`、`SpendRuleDefinitionService`、`SpendRuleVersionService`、`SpendRuleAssignmentService`、`SpendRuleDecisionRecordService`、`SpendControlMovementService`。 | 条件可用：wallet 实体和控制事实均已有基础服务或明确组合服务承接，Mapper 访问收敛在 `wallet-impl/services/impl`；支出控制范围创建不建账，Spend Rule 控制事实不入 ledger。 | 新增单对象能力优先并入基础服务；不得新增旧式 `DomainService` / `DomainQueryService`、兼容 facade 或只做委派的浅服务。 |
...
| 资金责任解析 | 已选择 `targetSubjectType + targetSubjectId`，并保留 `fundingAccountId` 作为资金账户目标的兼容字段。 | 解析结果必须是资金账户、信用账户、VCC 关联资金/信用子账户或平台角色解析后的平台资金账户；支出控制范围和 Spend Rule 不能成为结果主体。 | 当前只闭合资源关系目标主体表达和服务层校验，平台角色解析、application facade、完整 route snapshot 决策和回放仍由后续工程边界承接。 |
...
| 兼容要求 | 禁止把信用账户、平台角色或支出控制范围编码进 `funding_account_id`。 | `funding_account_id` 只能作为兼容字段或派生字段，不再作为资金责任的唯一语义来源。 |
...
业务用途：承载信用账户、支出控制范围、Spend Rule、工具或使用主体到资金账户、信用账户或平台账户角色的资金责任解析关系；历史曾称“资金来源关系”，当前口径统一按资金责任解析理解。该表仅作为资金责任主体候选和 `FundingAllocationDecision` 输入，不执行 Spend Rule 决策、不扣款、不写分录，也不得把支出控制范围或 Spend Rule 输出为最终资金责任主体。

工程裁决：已采用 `targetSubjectType + targetSubjectId` 的 `ddl-backed` 切片。`funding_account_id` 只作为兼容字段保留：当目标主体是 `FUNDING_ACCOUNT` 时与 `target_subject_id` 一致；当目标主体是 `CREDIT_ACCOUNT` 时可以为空。后续若要支持平台角色解析后的平台资金账户，需要在 application facade 和 route snapshot 决策中继续扩展，不得把平台角色或支出控制范围直接写入 `funding_account_id`。

历史切片关系：

| 裁决切片 | 字段口径 | 服务口径 | 验收口径 |
| --- | --- | --- | --- |
| `funding-account-only` | 保留 `funding_account_id`，字段语义固定为资金账户 ID 或业务号。 | 解析结果只能是 `FUNDING_ACCOUNT`，信用账户、平台角色和支出控制范围不得通过该字段变相表达。 | 只验支付工具或支出主体到资金账户的解析和快照，不声明信用账户或平台资金账户责任解析生产可用。 |
...
- `target_subject_type + target_subject_id` 是目标主体写入事实；`funding_account_id` 仅保留兼容读取和资金账户目标镜像，不得承载信用账户、平台角色、支出控制范围、Spend Rule、支付工具或卡。
