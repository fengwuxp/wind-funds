...
| 阶段 | 工程落点 | 允许做什么 | 不做什么 |
| --- | --- | --- | --- |
| Phase 1 契约承载 | `core` 的 `FundsInstructionSpec`、权益 Spec、不可变模型和枚举；DSL 契约测试。 | 让资金指令可携带 `benefitSnapshot`；无权益交易保持兼容；验证金额闭合、组件类型和退款处置。 | 不要求 RouteResolver、PostingAssembler、交易表或 route snapshot 立即完整消费。 |
...
4. Phase 2 如果仍只在 `contextVariables` 写入权益摘要，只能作为过渡兼容，不得作为生产长期回放依据。
...
| 权益金额组件 | 无权益兼容、商户券 no-ledger、平台补贴 posting-required、平台券不补足商户、储值券资金性质、授权占券、不退券但冲补贴、部分退款分摊、缺原权益快照失败或人工处理。 |
