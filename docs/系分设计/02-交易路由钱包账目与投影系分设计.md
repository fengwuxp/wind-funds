...
争议语义应先在 dispute case 层得出裁决，再由 `settleRefund / AUTH_REFUND` 的查询、投影、审计和幂等摘要承接退款结果；裁决无资金影响时只保留案件、审计和投影解释，不进入交易内核。既有 `FundsAuthorizationTransactionService#chargeback` 只能作为历史兼容或内部适配资产复用，不默认扩展为公共目标态主入口。需要完整 dispute case、representment、清结算追偿、VCC processor、外部卡组织规则、独立 `chargeback` 一等目标 API 或移除既有 `chargeback` 公共方法时，必须另起专项工程任务。
...
| transaction application | 承接已解析账户主体型直接交易、授权交易、余额控制、退款、清算和拒付命令。 | 接收未解析的卡号、卡 token、预算组或 Spend Rule 作为账务主体。 |
