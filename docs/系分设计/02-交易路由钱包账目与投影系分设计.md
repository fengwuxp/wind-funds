...
3. 授权完成不新增含义重复的完成金额账目，不触碰 `LIMIT`；VCC PREPAID 的 FundingAccount 执行 `AUTHORIZATION -> SETTLEMENT`，SHARED 的 CreditAccount 执行 `AUTHORIZATION -> OUTSTANDING`，父 FundingAccount 沿原 RouteSnapshot 只承担一次真实资金结算责任。
...
| `AccountHierarchyApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/account` | `wallet/impl/src/main/java/com/wind/funds/wallet/application/account/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`，必要时加 `account` 子包。 | 父子账户建立、子账户 profile、父账户约束、账目 profile、状态、资金来源和绑定摘要；B2-ACCOUNT-HIERARCHY 独立切片。 |
...
| 系统面 | 设计输入 | 目标态要求 | 工程影响 |
| --- | --- | --- | --- |
| 账户层级与 VCC 关联子账户 | 账户类型、账本初始化、可选直接父账户、层级版本、账目 profile、`AccountHierarchySnapshot` 和 `PostingRole`；当前不自动解析 root 或多级账户树。 | VCC 预付卡必须绑定资金子账户，共享卡必须绑定信用子账户；父账户默认只做约束和汇总，只有显式 `PARENT_CONTROL` 或真实父子划拨才写父账户分录；卡、工具、支出控制范围和 Spend Rule 不得入账。 | 账户层级是支付工具、VCC prepaid funding 和 lifecycle 的前置能力；未完成前不得声明 VCC 子账户、父账户快照或父子账户账务生产可用。 |
...
| 信用账户 | 管理授信额度、可用额度、授权占用和已用额度。 | `CreditAccountService`、授权交易、余额控制服务。 | LIMIT、AVAILABLE、AUTHORIZATION、OUTSTANDING、额度周期、调额来源、授信责任和规则版本。 | 先创建信用账户并设置额度；VCC 或授权交易通过资金责任决策引用它，不把卡工具写入信用账户。 |
| VCC 关联子账户 | 管理 VCC 场景中可记账子账户的余额、授权占用、清算、退款、费用和已确认争议资金结果。 | `AccountHierarchyApplicationService`、`FundsSubjectBalanceQueryService`、交易/清结算服务。 | owner、币种、账目 profile、状态、绑定工具、父账户、背后资金责任来源、敏感字段边界和审计来源。 | 先创建资金子账户或信用子账户并初始化账本；卡工具绑定到该子账户；资金动作通过 route/posting 使用该子账户。wind-funds 不管理卡生命周期或卡账单，不新增 `VCC_ACCOUNT`。 |
...
| 工具与路由 | instrumentRef、bindingVersion、FundingAllocationDecision 引用、RouteSnapshot 引用。 | 逆向动作只比对原快照，不读取当前绑定、当前 Spend Rule 或当前资金责任重算。 |
...
| 共享卡授权完成 | 信用额度控制与真实资金责任分别闭合。 | 信用子账户 `AUTHORIZATION -> OUTSTANDING`；父 FundingAccount 沿原 RouteSnapshot 只承担一次真实资金结算责任。 | 原信用子账户、原父 FundingAccount 和已确认收款主体。 | 原工具、原子账户、原父账户、原 FundingAllocationDecision 和绑定快照。 |
