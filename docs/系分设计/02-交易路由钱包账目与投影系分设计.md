...
| 拒付和争议扣回 | 同上。 | 必须与授权拒绝、普通退款可区分；不得重复损失。 | `ChargebackInstrumentTransaction` 只做 facade 命令语义，底层仍回放原交易路径。 |
...
| VCC 清算/逆向 | `InstrumentTransactionLifecycleApplicationService` | clearing、reversal、refund、chargeback 命令和原授权引用。 | 授权交易 settle/reversal/refund/chargeback 服务。 | 后续交易号、差错单或人工处理任务。 |
...
| `ChargebackInstrumentTransaction` | 拒付、争议扣回、追偿。 | originalTransactionSn、disputeRef、chargebackAmount、stage、reason、requestDigest。 | 生成争议扣回、费用或追偿事实。 | 不与 refund 合并，不允许重复损失。 |
