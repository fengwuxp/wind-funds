...
| 产品能力 | 系统模块 | 核心契约 |
| --- | --- | --- |
| 直接交易 | transaction-face、transaction-impl | FundsDirectTransactionService、直接交易 Request 契约、指令转换器。 |
| 授权交易 | transaction-face、transaction-impl | FundsAuthorizationTransactionService、授权交易 Request 契约、授权路由解析器。 |
...
| 权益快照契约 | `FundsInstructionSpec.benefitSnapshot`、权益 Spec、不可变模型、枚举和 DSL 契约样例。 | `AC-BEN-*`、`RED-050` 至 `RED-057`、`DSL-BENEFIT-*` 和 `TDD-BEN-*` 已映射；Execution Grant 明确允许公共契约和枚举变更；不要求本切片修改 route snapshot、表结构或完整 route/posting 消费；交付结论只能写“契约承载 Done”，不得写“含权益生产链路 Done”。 |
| 权益路由账务消费 | 商户券不入账、平台补贴独立 leg、储值券负债口径、授权占券、退款补贴冲回、posting context。 | route snapshot、交易事实快照、独立权益快照表或等价不可变存储至少一个可用于 replay；幂等摘要覆盖权益快照稳定摘要；posting plan 独立平衡；不退券和不冲补贴分开；零实付、储值券和平台补贴账户角色的待确认项已关闭或明确不在本批次。 |
