...
| 系分模板项 | 本文落点 | 裁剪说明 |
| --- | --- | --- |
| 产品语义输入 | 1.1 | 关联产品需求、场景、验收与质量属性。 |
| 概要设计与追踪 | 第三节 | 覆盖主链路架构、模块职责和依赖方向。 |
| 运行时与详细设计 | 4.1-4.13 | 覆盖入口、状态机、路由、账务、投影、数据和补偿。 |
| 规则落地 | 4.14 | 汇总跨模块资金红线的工程承接与验证证据。 |
| Engineering Handoff | 第六节 | 明确实施切片、写入范围、验证和停止条件。 |
...
| FundingResponsibilityResolutionApplicationService | 将工具、子账户、父账户、使用主体、支出控制范围、Spend Rule 和平台角色解析成唯一资金或额度候选主体。 | SpendSubjectFundingRelationService、PlatformFundingAccountService、账户服务、AccountHierarchyRelationService。 | 输出可供路由使用的主体选择；最终主体、账目和金额由 route participant/leg 显式表达，支出控制范围和 Spend Rule 不得成为最终责任主体。 |
...
| 4. 资金责任解析关系 | 解析到唯一资金账户、信用账户或平台账户角色；支出控制范围和 Spend Rule 只作为控制上下文。 | 形成可供路由选择的主体，最终进入 `RouteParticipant` / `RouteLeg`。 | 多责任主体、缺预付资金确认或责任主体不明确时失败无账务副作用。 |
...
| 资金责任解析 | `FundingResponsibilityResolutionApplicationService` | instrumentRef、spendSubject、spendControlScopeRef、spendRuleContext、prepaidResponsibilityContext、servicePlanSnapshot。 | `SpendSubjectFundingRelationService`、平台账户角色解析、账户能力查询。 | 唯一候选主体或不唯一/缺失失败；路由负责将其写入 participant/leg。 |
...
业务用途：承载信用账户、支出控制范围、Spend Rule、工具或使用主体到资金账户或信用账户的资金责任解析关系；历史曾称“资金来源关系”，当前口径统一按资金责任解析理解。该表仅提供资金责任主体候选和路由选择依据，不执行 Spend Rule 决策、不扣款、不写分录，也不得把支出控制范围或 Spend Rule 输出为最终资金责任主体；最终主体和金额由 route participant/leg 承载。
