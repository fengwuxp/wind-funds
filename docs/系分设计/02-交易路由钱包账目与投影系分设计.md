...
交易服务需要区分“业务入口形态”和“资金事实内核”。`transaction-face` 中直接交易、授权交易和余额控制的内核请求以已解析账户主体为准；支付工具、钱包标识、外部账户和预算控制上下文属于 route 准入、快照和应用编排输入。

| 分层 | 面向对象 | 可接收输入 | 输出 | 设计约束 |
| --- | --- | --- | --- | --- |
| 业务应用入口 | 上层业务、通道适配、VCC/共享卡/钱包产品。 | `PaymentInstrumentRef`、外部授权引用、商户/MCC/国家、使用主体、预算组、Spend Rule、金额和业务流水。 | 通过、拒绝或解析后的资金事实请求。 | 负责校验工具、绑定、规则、资金责任和权限，并固化工具/绑定/规则快照；不得写账本事实。 |
...
授权交易是双层入口的首要适用场景：上层业务自然持有卡、VA、钱包标识、共享卡和外部授权引用，授权准入必须先从支付工具边界校验状态、方向、币种、绑定版本、使用人、预算组、Spend Rule、资金责任唯一性和账户能力；只有授权批准时，内核才基于解析后的资金账户、信用账户或平台账户角色生成 `AVAILABLE -> AUTHORIZATION` 路径。授权拒绝、支付工具不可用、规则拒绝、资金责任缺失或不唯一时，只记录拒绝事实和原因，不生成 route、posting 或 ledger entry。
...
| AuthorizationAdmissionApplicationService | 面向卡、VCC、共享卡、钱包标识等入口完成授权前准入。 | 支付工具能力应用服务、资金责任解析应用服务、Spend Rule 控制、账户能力查询。 | 输出批准/拒绝和快照输入；批准后委派授权交易内核，拒绝无 route/posting/entry。 |
...
| 支付工具 | 管理卡、VA、钱包标识、通道 token、虚拟卡和共享卡工具快照。 | `PaymentInstrumentService`、PaymentInstrumentCapabilityApplicationService、route resolver。 | 状态、方向、动作能力、币种、有效期、绑定版本、脱敏引用、敏感字段和资金责任唯一性。 | 先注册支付工具，再绑定到使用主体或内部责任主体；交易发生时固化 `PaymentInstrumentRef`。 |
...
支付工具只提供路由输入、绑定关系和审计快照，不表达内部余额。交易、授权、提现、入金识别和退款进入路由前，必须先完成工具状态、方向、动作能力、有效期、币种、绑定和资金责任校验。钱包标识属于支付工具引用或外部端点引用，不是钱包账户域本身；使用钱包余额支付时，路由必须先解析出内部 `SubjectRef`。
...
6. PaymentInstrumentRef、ExternalAccountRef、RoutingDecision 和 FundingAllocationDecision 必须进入 route snapshot；LedgerEntry 主体不得出现工具、卡、VA、外部账户或通道 token。
7. 完整 PAN、CVV、密钥、token secret、银行账户敏感号和身份凭证不得进入普通快照、日志、导出或报表。
8. 支付工具状态、绑定、方向、动作能力、优先级和资金责任解析关系变更必须记录操作者、时间、原因、版本和前后值。
9. 如果 `PaymentInstrumentRef` 表示钱包标识，route leg 仍必须使用解析后的资金账户、信用账户或平台账户角色；不得把钱包标识、工具号、token、预算组或 Spend Rule 作为 ledger subject。
...
| account_sn | varchar(64) | [ ] | 资金账户、信用账户、预算组或 Spend Rule 编号，按 adjust_object_type 解释 | FA100001 |
