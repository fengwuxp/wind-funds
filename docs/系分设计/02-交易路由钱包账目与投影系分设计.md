...
| 结算后退款 | `settleRefund` | `FundsAuthorizationTransactionRefundRequest` | 基于已结算路径退款，不超过已结算金额。 |
...
4. `reversal`、`expire`、`settle` 和 `settleRefund` 必须引用原授权或原完成事实；拒付通过 `settleRefund` 携带原因、凭证和审计上下文承接；`settle` 强制完成模式必须有策略、上限、原因和审计。
...
唯一索引：`uk_payment_instrument_binding_sn(sn)`；`uk_payment_instrument_binding_subject(instrument_sn, binding_role, subject_type, subject_id, currency)`。
