...
| PaymentInstrumentTransactionApplicationService | 支付工具交易公共应用服务，当前承接授权和已确认收款。授权实现内部使用 `PaymentInstrumentAuthorizationProcessor`，该处理器不是公共契约。 | 支付工具预交易快照应用服务、Spend Control admission、资金责任解析应用服务、账户能力应用服务和 canonical 交易服务。 | 先解析工具能力、绑定、资金责任和账户能力；授权可消费 Spend Rule 决策证据；通过后委派账户主体型交易内核，拒绝停在交易内核前且无账务 route leg/posting/entry。 |
...
| `PaymentInstrumentTransactionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/instrument` | `transaction/impl/src/main/java/com/wind/funds/transaction/application/instrument/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`。 | 支付工具授权与已确认收款入口；先消费钱包准入和预交易快照，授权按需消费 Spend Control 准入，再委派账户主体型交易内核。 |
...
| 授权后续资金动作 | `FundsAuthorizationTransactionService#complete/reversal/refund` | 原资金授权引用、原授权主账户、本次金额币种和稳定 `businessSn`。 | 原授权交易事实和原 `RouteSnapshot`。 | 资金交易引用、更新后的授权金额摘要或稳定失败。 |

`PaymentInstrumentTransactionApplicationService` 只承接首笔支付工具授权和已确认收款；完成、可信撤销和退款已有账户主体型 `FundsAuthorizationTransactionService` 承接。后授权动作必须引用原授权交易和原主账户，沿原 `RouteSnapshot` 回放，不新增支付工具 facade，也不按当前绑定关系重新选路。
...
| 接口或组件 | 建议落点 | 职责 | 禁止行为 |
| --- | --- | --- | --- |
| `FundsBenefitContributionTransactionService` | 让利出资记账交易服务，落在 `transaction-face` 的 application 边界。 | 提供 `complete`、`refund` 两类命令；接收业务侧已决策让利出资结果，校验成本承担账务主体、让利承接账务主体、金额和资金性质后，委派标准交易路由、交易事实和账本链路。 | 不新增 `FundsMarketingTransactionService`，不计算优惠金额，不判断券是否可用，不维护券包生命周期，不保存券、活动或规则来源归因，不拥有独立营销交易状态机，不直接写 route、posting、LedgerEntry 或余额投影，不要求调用方传入专业确认状态或事实摘要。 |
