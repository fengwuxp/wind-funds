...
| `AuthorizationAdmissionApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/instrument` | `transaction/impl/src/main/java/com/wind/funds/transaction/application/instrument/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`。 | 支付工具授权入口实现；先消费钱包准入、预交易快照和 Spend Control 准入，再委派账户主体型授权交易内核。 |
...
| `InstrumentTransactionLifecycleApplicationService` | `wallet/face/src/main/java/com/wind/funds/wallet/application/instrument` | `transaction/impl/src/main/java/com/wind/funds/transaction/application/instrument/impl` | `com.wind.funds.wallet.model.request`、`com.wind.funds.wallet.model.dto`。 | 支付工具交易生命周期统一外层入口；实现先调用钱包预交易快照和授权准入，再委派交易内核创建资金事实。 |
...
| 支付工具授权 | `InstrumentAuthorizationApplicationService` | 支付工具授权命令、工具准入快照、规则决策和资金责任决策。 | `FundsAuthorizationTransactionService.authorize`。 | 资金授权交易号或拒绝结果。 |
| 授权后续资金动作 | `InstrumentTransactionLifecycleApplicationService` | 原资金授权引用、本次金额币种、稳定 `businessSn`、原责任和原 RouteSnapshot 引用。 | `complete`、`reversal`、`refund` 等已确认的单个资金动作。 | 资金交易引用、更新后的资金金额摘要或稳定失败。 |

以上命名只表达 `wind-funds` 目标服务语义，不冻结 Java 包名或 DTO。当前公开的 `InstrumentTransactionLifecycleApplicationService` 尚未提供完成和释放入口；现有账户主体型授权交易服务已支持 `complete` / `reversal` 的金额闭合。跨模块 facade 需 wallet、transaction 和上游 owner 共同签收接口、Request/DTO、错误码、幂等和依赖方向；未签收前不以文档候选命名生成生产公共接口。
