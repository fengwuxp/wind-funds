...
`InstrumentTransactionLifecycleApplicationService#authorizeByInstrument`、`receiveByInstrument` 和其他支付工具型 application facade 只能位于 application/use-case facade：先解析业务入口参数、工具能力、绑定、预算控制、资金责任和账户能力，再委派账户主体型交易内核。`receiveByInstrument` 只把 VA、ACH 或外部钱包端点等收款入口解析为目标资金账户和外部入金引用，最终委派 `FundsDirectTransactionService#topup`，并把脱敏支付工具引用固化到 route snapshot；请求必须携带 `expectedBindingVersion`，缺失时在交易内核前失败且不得生成资金交易、route、posting 或账本事实。不得把 `FundsAuthorizationTransactionAuthorizeRequest`、`FundsTransactionPayRequest`、`FundsTransactionTopupRequest` 或余额控制请求整体替换成支付工具引用入参，也不得让交易内核只感知 `PaymentInstrument` 而丢失最终资金账务主体。
...
| 层级 | 职责 | 接口契约口径 | 事务边界 |
| --- | --- | --- | --- |
| `wallet.application` | 承接支付工具、VA、VCC、全球账户、ACH 等业务入口，完成准入、绑定、账户能力、Spend Rule、外部引用和预交易快照组合。 | 已有 `authorizeByInstrument`、`receiveByInstrument`、`payOutByRail` 和 `ExternalFundsEventApplicationService#consume` 最小入口；`payOutByRail` 当前仅完成服务层契约和未接内核 fail-fast guard，不代表真实出款 Done；`consume` 当前已支持 confirmed credit 到资金账户的标准 `topup` 消费，但不代表外部扣账、return/NOC/reversal、信用账户还款或调额、差异单、补事实、清结算或对账 Done。入参必须包含业务流水、场景、金额、币种、工具 / rail / 外部事件引用和可解析账户主体，收款工具和出款 rail 入口必须携带调用方期望的绑定版本。 | 失败应停在交易事实前；通过后委派 transaction canonical service；收款工具入口应在 route snapshot 固化脱敏 `PaymentInstrumentRef` 和外部账户引用；出款 rail 和外部事件扩展必须另行证明提现/出款、在途、费用、扣账、退款、撤销、调账、清结算和对账边界。 |
...
Ledger 模块的系统定位是账务事实层和余额投影事实源。跨模块生产默认入口应优先使用 `ledger-face` 的 application facade：`LedgerPostingApplicationService` 负责标准账本过账，`LedgerFactQueryApplicationService` 负责账本交易和账目分录事实查询，`LedgerBalanceProjectionApplicationService` 负责账本余额投影查询。`LedgerService`、`LedgerTransactionService` 等资源型服务保留为模块内部协作、初始化、测试夹具或兼容治理候选，不作为普通业务资金修正主入口；`ledger-impl` 负责落地账务事实、维护余额投影和发布余额观察事件。Ledger 模块不承接业务交易入口、路由决策、支付工具解析、Spend Rule 决策、清结算批次状态机或对账差错生命周期。
...
| 统一过账 | `LedgerPostingApplicationService`、`LedgerTransactionPostingService`、`LedgerTransactionService.postLedgerTransaction` | 校验 `LedgerTransactionSpec`、`PostingPlan`、`LedgerEntry` 和幂等摘要，原子写入账本交易、posting plan、不可变分录和余额投影；跨模块调用优先走 application facade。 | 重新解析 route、重选工具绑定、解释业务订单生命周期、接收业务方直接指定的借贷方向。 |
| 账务证据查询 | `LedgerFactQueryApplicationService`、`LedgerTransactionService` 查询方法、`LedgerEntryQuery`、`LedgerTransactionQuery` | 按租户、主体、账目、币种、周期、业务引用、交易引用、清结算标记和对账标记查询账务证据；跨模块查询优先走 application facade，禁止省略租户边界。 | 查询时修复状态、补写分录、重算余额、跨租户兜底查询或推进清结算和对账状态机。 |
| 余额投影 | `LedgerBalanceProjectionApplicationService`、`LedgerBalanceProjectionService` | 从 `LedgerEntry` 派生余额，支持当前余额、周期余额、检查点和重建输入；跨模块读取优先走 application facade。 | 从交易投影、余额日志、报表或人工汇总反写账本事实。 |
