...
4. settleRefund 同时承接普通授权链退款、无授权直接退款和拒付承接；普通授权链退款继续要求原完成引用，无授权退款以空原授权流水进入 no-auth 语义，携带 `externalReferenceSn`、退款原因、操作者和审计，且不得携带或查询内部授权流水；资金路径可以复用退款路径，但原因、外部引用和投影展示语义必须保留。
5. 服务返回值只表达资金底座已经接受并处理的资金事实，不承诺通道、卡组织或上层业务事件最终状态。

`B4-DISPUTE-SEMANTIC-ALIGNMENT` 首轮已闭合。系统实现已优先补齐 `settleRefund / AUTH_REFUND` 争议语义的查询、投影、审计和幂等摘要可区分性；既有 `FundsAuthorizationTransactionService#chargeback` 只能作为兼容、显式事件或内部适配资产复用，不在首轮扩展为公共目标态主入口。需要完整 dispute case、representment、清结算追偿、VCC processor、外部卡组织规则或独立 `chargeback` 一等目标 API 时，必须另起专项 Grant。
