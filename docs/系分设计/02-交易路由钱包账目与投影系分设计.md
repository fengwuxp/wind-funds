...
交易投影生成用户账单、商户账单、运营时间线、财务核对视图和指标项输入。它不是现有写入链路的事实源，应由资金交易、冻结单、账本分录、对账清算模块对象和对账差错派生。
...
| funding_account_id | varchar(30) | [X] | 兼容字段：当前实现中的目标资金账户 ID 或业务号；目标态如支持信用账户或平台账户角色，需迁移为目标主体类型 + 目标主体 ID。 | FA100001 |
| currency | varchar(10) | [X] | 币种 | CNY |
...
| 优先级 | 切片 | 交付内容 | 准入门禁 |
| --- | --- | --- | --- |
| P0 | 账户与账本初始化 | 资金账户、信用账户、预算组、Spend Rule、支付工具、绑定关系、账本 bucket。 | 资金账户和信用账户显式建账；预算组和 Spend Rule 只初始化控制视图、规则快照和审计证据，不初始化 ledger bucket；表结构、唯一键、周期红线和账户查询测试通过。 |
