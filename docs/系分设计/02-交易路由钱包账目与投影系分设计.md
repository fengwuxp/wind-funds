...
| Spend Rule 控制 | 目标态需要独立 Spend Rule 定义、版本、决策日志、控制活动和预算控制投影；首轮已形成 `SpendControlAdmissionApplicationService` 消费外部决策证据。 | 规则定义、规则版本、决策结果、拒绝原因、预留、释放、调整和审计必须可追溯；准入快照只证明外部决策证据可被交易前链路消费。 | 完整规则定义、决策日志持久化、Spend Control Activity、预算控制投影和并发预留释放仍需独立工程任务承接。 |
| 授权准入组合 | 已形成 `AuthorizationAdmissionApplicationService` 与支付工具 route snapshot 回链首轮服务流。 | 组合工具准入、绑定快照、Spend Rule 决策证据、资金责任和账户能力后委派账户主体型授权内核。 | 后续仍需 VCC facade、完整业务策略准入和控制活动闭环；不得替换 canonical 授权请求。 |
...
| 预算控制投影 | 从控制活动聚合控制口径的 reservedAmount、releasedAmount、remainingControlAmount、lastActivitySn 和 lastActivityAt。 | 不包含 ledger balance、available balance、frozen balance 或 authorization balance；不得反写账本余额。 |
...
| 能力层 | 工程落点 | 允许做什么 | 不做什么 |
| --- | --- | --- | --- |
| 契约承载 | `transaction-face` 的 `FundsBenefitFundingApplicationService`、权益让利资金交易请求、来源引用 DTO 和权益资金枚举；契约测试。 | 让资金底座可接收 `apply/refund/reverse` 权益资金交易命令；验证让利方、受益方、金额、来源引用、原订单或原交易引用等最小资金事实。 | 不要求 RouteResolver、PostingAssembler、交易表或 route snapshot 立即完整消费；不把专业确认、确认凭证或摘要作为公共请求字段。 |
| 路由账务消费 | `FundsBenefitFundingApplicationServiceImpl` 委派 `FundsDirectTransactionService`，复用既有 `RouteResolver`、`RouteSnapshotFactory`、`LedgerPostingAssembler`、`DefaultRouteReplayService`。 | `POSTING_REQUIRED` 平台补贴、储值券和合作方补贴按资金性质生成独立资金影响；退款和撤销读取原权益资金交易 route snapshot 后回放。 | 不把营销规则、券可用性判断或券生命周期推进放进资金底座；`NO_LEDGER`、`HOLD_ONLY`、`RELEASE_ONLY`、`REVERSAL_REQUIRED` 在未确认独立事实或伴随指令策略前不生成资金交易。 |
...
7. 历史权益事实迁移补录只能生成追加型 supplemental benefit fact 或等价补充事实，必须记录补录来源、审批、复核、digest、版本、关联原交易和撤销关系；不得覆盖原资金交易、原 route snapshot 或原账本事实。
