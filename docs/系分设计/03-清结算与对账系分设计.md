...
| SettlementOrderState | 结算单状态 | DRAFT、REVIEWING、APPROVED、LOCKED、FAILED、CANCELLED；FAILED 只表达锁定资金的明确账务拒绝，出款状态不进入本枚举 |
| PayoutOrderState | 出款单状态 | CREATED、SUBMITTED、ACCEPTED、PROCESSING、SUCCEEDED、FAILED、RETURNED、MISMATCHED |
...
| PayoutOrderDTO | sn、tenantId、settlementOrderSn、settlementSubjectType、settlementSubjectId、amount、currency、factStatus、displayStatus、operationStatus、payoutAccountRef、payeeEndpointRef、channelRef、externalReference、Gate/准入证据摘要、成功/失败资金交易流水、最后回单摘要、脱敏失败信息和时间 | 当前 W2 单笔查询的统一解释模型；不得只返回技术 status。 |
| PayoutOrderQuery | tenantId、settlementSn、payoutSn、status、factStatus、displayStatus、operationStatus、manualReviewRequired、externalRuleVerificationStatus、payeeSubjectRef、channelRef、amountRange、createdRange | 目标态出款处理台查询条件；当前 W2 不提供列表或分页查询。 |
