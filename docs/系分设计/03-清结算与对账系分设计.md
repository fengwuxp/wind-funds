...
7. 重跑创建新批次并以 `previousBatchSn` 引用已完成批次；准入对象和窗口必须一致，允许使用新的规则版本，不覆盖旧来源和旧结果。新批次创建后，上一批次及其结果仍可审计查询，但立即失去门禁消费资格；重跑未完成时失败关闭。
...
1. `createBatch` 按内部 `batchDigest` 与业务唯一键幂等；重跑必须生成新批次并引用已完成批次，不提供覆盖或关闭旧批次接口。门禁通过 `previousBatchSn` 反向检查结果是否已被替代，已被引用的旧结果只能审计追溯，不能继续准入。
...
| RecordReconciliationDifferenceRerunRequest | tenantId、differenceSn、rerunSn、rerunBatchSn、ruleVersion、balanced、evidenceRef、resultDigest、description | 登记处理后的重跑结果；不得覆盖旧差错、旧审批或旧处理动作。 |
...
| TDD-RACE-007 对账重跑与差错核销并发 | 目标态并发契约，未落地：createBatch(previousBatchSn)、recordRerunResult 的竞争控制 | 同一差错正在核销，同时对账批次被重跑。 | 并发执行重跑和核销。 | 不覆盖审批和核销结果；已关闭差错不得无新证据自动重开；重跑报告独立。当前版本仅保证单请求幂等和事实不可覆盖，不声明并发线性化能力。 |
...
| previous_batch_sn | varchar(64) | [ ] | 重跑引用的上一已完成批次。 |
...
唯一键：`tenant_id + sn`、`tenant_id + batch_digest`。状态推进采用当前态条件更新；运行结果写入与 `DATA_READY -> COMPLETED` 绑定在同一事务完成。
