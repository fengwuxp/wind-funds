...
外部规则核验引用必须保存规则来源、版本或发布日期、生效日期、适用法域或适用范围、核验日期、确认方和确认状态。系统只校验字段完整性和确认状态，不判断外部规则真实性或最新性；正式生产准入仍由法务、合规、财务、通道或持牌机构确认。
...
| 里程碑 | 负责人 | 交付物 | 验收方式 |
| --- | --- | --- | --- |
| 设计准入 | 产品、架构、资金域研发、测试 | PRD、DSL、系分和 TDD 对齐，出款前门禁、外部非终态和金额不一致口径一致。 | 文档 CR、结构检查和 `git diff --check` 通过。 |
...
4. 出款成功后退款、拒付、费用或差错不得回滚历史出款，必须进入 RecoveryOrderService。

出款前准入模型：

| 模型 | 字段 | 说明 |
| --- | --- | --- |
| CheckPayoutPreflightRequest | tenantId、settlementSn、payoutSn、currency、amount、payoutAccountRef、payeeEndpointRef、channelRef、idempotencyKey、ruleEvidenceRef、approvalRef | 出款提交前的准入检查请求；允许在创建出款单前只传 settlementSn，也允许在提交前带 payoutSn 复核。 |
| PayoutPreflightResultDTO | passed、blockingLevel、blockingReasons、manualReviewRequired、externalRuleVerificationStatus、checkedAt、checkedBy、expiresAt、evidenceRefs | 准入结果必须能说明是否通过、阻断等级、阻断原因、是否转人工、外部规则核验状态、检查时间、有效期和证据引用。 |
