...
- 所有金额使用最小货币单位，字段类型优先 bigint(20)，方向通过 direction、item_type、状态和账本分录表达，不使用负数偷换业务方向。
- 所有批次、单据和差错对象必须保留租户、范围、规则版本、状态、摘要、操作者或系统来源、trace 和审计线索。
- 数据库不建外键，不使用级联删除；跨模块只引用资金交易号、交易明细号、账本分录号、账本交易号、对账号、批次号等业务流水。
- 对账、差错、审计、追偿和核销只追加动作或状态迁移，不得覆盖历史分录、外部证据或资金事实。
- 清算账期、结算周期、账本周期、报表周期必须分字段表达，不得互相替代。
...
| sn | varchar(64) | [X] | 业务流水号，表内全局唯一 | RB100001 |
| tenant_id | bigint(20) | [X] | 归属租户 | 1 |
| status | varchar(50) | [X] | 对象状态，枚举见各表状态说明 | CREATED |
| version | int(11) | [ ] | 乐观锁版本；批次、当前态单据必填 | 1 |
| rule_code | varchar(64) | [ ] | 规则编码 | MERCHANT_CLEARING |
| rule_version | int(11) | [ ] | 规则版本 | 3 |
...
| currency | varchar(10) | [X] | 币种 | CNY |
| external_account_mask | varchar(128) | [X] | 脱敏外部收款账户 | 6222****1234 |
