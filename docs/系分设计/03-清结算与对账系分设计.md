...
AbortReconciliationBatchRequest
  -> 终止当前无效批次并固化操作人、时间和原因
  -> 调用方以该批次为 previousBatchSn 显式创建替代批次
```
...
8. 绑定 Gate 的同一租户、`reconciliationScopeRef` 和准入对象由 `ReconciliationBatchLineage` 唯一记录当前批次头。根批次创建抢占唯一血缘，重跑必须以 `previousBatchSn` 引用已完成或已终止的当前头；正常重跑前，差错以 `COALESCE(lastRerunBatchSn, reconciliationBatchSn)` 表达当前批次锚点，当前头运行差异必须全部物化，且每项未关闭差错都已进入 `ADJUSTING` 并存在对应动作事实。范围、准入对象和窗口必须一致，允许使用新的规则版本，不覆盖旧来源和旧结果。Gate 只接受当前头绑定的 `COMPLETED` 运行结果；当前头为 `ABORTED`、缺失、漂移、差错动作未完成或重跑未完成时均失败关闭。纯对账批次不创建 Gate 血缘头。
...
stateDiagram
    ...
    DATA_READY --> ABORTED
    COMPLETED --> ABORTED: 无差错状态锚定
    COMPLETED --> [*]
    ABORTED --> [*]
```

`ReconciliationBatchStatus` 只描述已落地批次的工程进度：`CREATED` 已创建，`DATA_COLLECTING` 已冻结一侧来源，`DATA_READY` 两侧来源已冻结，`COMPLETED` 已绑定不可变运行结果，`ABORTED` 表示操作人已确认该批次证据不可继续使用。`ABORTED` 不是解析失败或任务重试状态，不删除来源、运行结果或差错事实，也不自动创建替代批次；替代批次必须显式引用它并保持范围、准入对象和窗口一致。已完成且存在差错状态锚点的批次不得终止，避免隐式反转差错处置。`BALANCED/DIFFERENCE_FOUND` 属于 `ReconciliationRunResultStatus`；当前 `PASSED/BLOCKED` 属于 gate 决策。上层任务编排可另行表达 `WAITING_DATA`、解析失败、匹配执行失败和重试，但不得把这些任务态混入当前批次、运行结论或准入决策。`CONDITIONALLY_PASSED` 只有在审批、阈值、责任人、风险承接和到期重查契约落地后才能引入。
...
| ReconciliationDifferenceStatus | 当前对象级 Gate 差错状态 | BLOCKED、ADJUSTING、RECONCILING、RESOLVED |
...
1. `createBatch` 按内部 `batchDigest` 与业务唯一键幂等；正常重跑引用已完成批次，替代批次引用已终止批次，均生成新批次且不覆盖历史。`abortBatch` 只允许终止当前血缘头，固化 `abortedBy/abortedTime/abortReason`；相同事实重放幂等，不同终止事实冲突。已完成且有差错状态锚点的批次拒绝终止。服务锁定上一批次后以 `FOR UPDATE` current read 读取唯一子批次并比较摘要；`tenantId + previousBatchSn` 唯一键并发兜底，禁止同父分叉。门禁通过 `previousBatchSn` 反向检查结果是否已被替代，旧结果只能审计追溯。绑定 Gate 且运行结果存在差异时，必须先证明当前未关闭差异均已物化且当前动作事实完整，才允许创建正常重跑批次；纯对账批次不套用该门禁。
...
| ClearingBatchServiceImpl | 管理清算批次，确认时触发交易层清算资金事实。 | 交易层、宿主审计能力、候选锁定。 |
...
| --- | --- | --- | --- | --- |
| TDD-RECON-007 对账批次生命周期 | createBatch、recordSourceSnapshot、recordRunResult、abortBatch、checkGate | 存在租户、准入对象、对账窗口、规则版本和来源证据引用。 | 创建批次、依次冻结两侧来源、提交完整匹配结果并检查准入；对无效当前头执行受控终止并创建替代批次。 | 正向状态按 CREATED -> DATA_COLLECTING -> DATA_READY -> COMPLETED 推进；ABORTED 固化操作人、时间和原因且不能准入，替代批次显式引用旧批次；运行结论和门禁决策独立表达，同一 Gate 对象只允许一个当前血缘头。 |
...
本节按架构师系分模板描述清结算与对账表结构。对账批次、Gate 批次血缘头、来源快照/成员、运行结果、逐笔匹配、差错和可清分明细已落地 H2 测试 schema、Entity、Mapper、服务层测试和 MySQL 首次建表基线；宿主迁移登记、生产同版本演练与验收尚未完成。其余清分、清算、结算、出款和追偿仍是目标态。字段命名若在实现时调整，需要回到本节和 TDD 同步校准。
...
| previous_batch_sn | varchar(64) | [ ] | 重跑引用的上一已完成或已终止批次；同一租户内非空值唯一，禁止同父分叉。 |
...
| batch_digest | varchar(64) | [X] | 对账范围、可选准入对象、规则、窗口和重跑关系的 SHA-256。 |
...
状态说明：`BLOCKED` 表示待处理；`ADJUSTING` 表示已登记一项处理动作且必须先重跑；`RECONCILING` 表示最近一次重跑仍未对平，可以登记下一项动作；`RESOLVED` 只允许由后继线性批次的 `BALANCED` 运行结果关闭。
...
| source_transaction_version | int(11) | [X] | 识别候选时冻结的来源交易乐观锁版本；最终清分前必须重新核对 | 0 |
...
- 最终清分命令必须在同一事务内重新核对 `source_transaction_version`、退款累计和当前对象级 Gate；任一变化都失败关闭。当前仓内尚无最终清分命令，因此该宿主原子复核未验收前清分执行保持生产 `NO-GO`。
...
| 门禁 | 当前状态 | 生产验收证据 |
| --- | --- | --- |
| 生产 DDL 迁移 | 库内基线已提供，宿主未验收 | `database/mysql/reconciliation` 已提供 MySQL 8.0+ 首次建表和部署回读资产，统一使用 `utf8mb4_bin` 保持身份字段精确匹配，不发布危险删表 rollback，并由 `ReconciliationMysqlDdlContractTests` 守卫其与 H2 测试 schema 的结构漂移；宿主仍须纳入自身迁移体系，完成 checksum、历史数据判断、同版本预发演练、备份恢复和失败前向修复。 |
...
| 最终命令复核 | 未完成 | 清分、清算、结算和出款最终命令在自身事务内重新检查当前 Gate、来源版本和逆向事实。 |
| 观测和恢复 | 未完成 | 指标、告警、Runbook、任务重启、数据库故障、备份恢复、容量压测和慢查询计划。 |
