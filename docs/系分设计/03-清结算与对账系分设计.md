...
| PayoutPreflightResultDTO | passed、blockingLevel、blockingReasons、manualReviewRequired、decisionResult、displayStatus、action、externalRuleVerificationStatus、checkedAt、checkedBy、expiresAt、evidenceRefs | 当前结果只说明证据预检是否通过；通过时 `displayStatus=PREFLIGHT_PASSED`、`action=SUBMISSION_REVALIDATION_REQUIRED`，不得作为出款提交授权。 |
...
| PayoutOrderQuery | tenantId、settlementSn、payoutSn、state、displayStatus、nextAction、manualReviewRequired、externalRuleVerificationStatus、payeeSubjectRef、channelRef、amountRange、createdRange | 目标态出款处理台查询条件；当前 W2 不提供列表或分页查询。 |
