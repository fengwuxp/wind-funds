...
| 数据库级落地 | 首切片和可清分明细扩展切片已具备最小落点，整域仍待实现 | 已有 `t_reconciliation_difference` 和 `t_clearing_splittable_detail`、对应 Entity/Mapper、唯一键和 H2 schema；清分批次、清分结果快照、清算、结算、出款、对账任务和匹配结果仍需继续补 DDL、DTO、Converter、fixture 和表结构测试。 |
| 服务级落地 | 局部最小闭环，整域仍待实现 | 已有对账差错登记/调账回链/重跑、reconciliation gate、可清分单笔准入、clearing/settlement gate consumer 和 payout preflight 的真实 H2 服务流程测试；完整清分批次、清算、结算、出款生命周期、并发竞争、权限审计和退款时序仍需继续补齐。 |
...
目标态表结构覆盖对账、清分、清算、结算、出款和追偿。首个工程切片不得一次性铺开全部对象，优先落“对账差错闭环”，用最小数据落点证明差异识别、阻断、处理、审计和重跑。

| 首切片 | 最小数据落点 | 必须证明 | 暂不承接 |
| --- | --- | --- | --- |
| 对账批次 | `funds_reconciliation_batch` 或等价批次表。 | 范围、来源、规则版本、状态、幂等重跑、失败审计。 | 不生成清分批次、清算批次、结算单或出款单。 |
| 匹配结果 | `funds_reconciliation_match_result` 或等价匹配结果表。 | 内部事实、外部证据、差异类型、差异指纹、阻断级别和重复匹配幂等。 | 不直接改交易状态、ledger entry 或余额投影。 |
