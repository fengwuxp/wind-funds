...
| 数据库级落地 | `GO`（已声明范围） | 对账证据、差错、可清分明细、清分批次、清算候选和清算批次已落地 Entity/Mapper、唯一键、生产 DDL 与 H2 schema；完整结算单、出款单和追偿单不在当前能力范围。 |
| 服务级落地 | `GO`（已声明范围） | 已有对账、差错、Gate、可清分准入、清分结果快照、清算候选、清算批次原子编排和 `CLEARING -> AVAILABLE` 的真实 H2 服务流程测试；外部收数与解析、业务归一化、任务调度、完整结算和外部出款由上层或后续能力承接。 |
| 生产准入 | `GO`（已声明范围） | `tests` 模块 H2 全量集成测试作为项目生产验收；观测、Runbook、Web 审计、数据治理和目标数据库运行评估属于宿主责任。 |

工程任务需要确认复用模块、写入边界、表与状态机、资金 Red、上层业务决策和处理事实引用。`CLS-GATE-001` 至 `CLS-GATE-004` 约束 wind-funds 工程落地；`CLS-GATE-005` 至 `CLS-GATE-008` 约束宿主接入、展示、权限、审计和运行治理，不反向阻塞本仓已声明能力。完整结算、外部出款、争议/风控权威查询和按原分配退款未来纳入时，必须重新建立对应工程门禁。
...
| 出款前准入门禁 | 当前 `PayoutPreflightService.checkPayoutPreflight` 只做证据预检和对象级 Gate；目标态 `PayoutOrderService.submitPayoutOrder` 才读取结算锁定、账户、端点、通道、余额、幂等等权威事实并最终复核。 | DSL-SETTLEMENT-PAYOUT-RESULT-001 的出款提交前准入；任一最终门禁缺失、失败或未知时不生成 FUND_OUT / IN_TRANSIT。 | TDD-SETTLE-004、TDD-RED-023；宿主审批使用 HOST-OPS-001。 |
...
本节按架构师系分模板描述清结算与对账表结构。对账批次、Gate 批次血缘头、来源快照/成员、运行结果、逐笔匹配、差错、可清分明细、清分批次、清算候选和清算批次已落地 H2 测试 schema、Entity、Mapper、服务层测试和 MySQL 首次建表基线，并通过项目 H2 生产验收。宿主迁移登记和目标数据库演练属于接入兼容与发布责任，不阻塞 wind-funds 准出。结算、出款和追偿仍是目标态。字段命名若在实现时调整，需要回到本节和 TDD 同步校准。
...
Owner 已确认 `tests` 模块 H2 全量集成测试作为 wind-funds 生产验收。当前已声明的对账、清分和内部清算能力准出结论为 `GO`；下表用于区分仓内验收、未提供能力和宿主接入责任，不再把接入方环境证据作为本项目阻塞项。

| 能力项 | 当前结论 | 验收或责任边界 |
| --- | --- | --- |
| 数据库与 DDL | `GO` | H2 全量集成测试、生产 DDL 静态契约、测试 schema 对齐和命令保护构成仓内验收。`ReconciliationMysqlMigrationIntegrationTests` 与 `just test-mysql-reconciliation` 保留为接入方选择 MySQL 时的可选兼容验证。 |
| 数据库并发 | `GO` | H2 双事务用例验证批次血缘、差错动作、Gate 和最终清分复核的竞争与失败关闭。其他数据库压测由接入方按部署环境决定。 |
| 可信来源接入 | 宿主责任 | 上层适配器负责来源验签、授权、去重、归一化和原文保管；reconciliation 只冻结稳定引用与摘要。 |
| 最终命令复核 | `GO`（已声明范围） | 清分和内部清算确认在自身事务内复核当前 Gate、来源交易与退款事实。争议/风控权威查询、完整结算和外部出款不在当前能力范围。 |
