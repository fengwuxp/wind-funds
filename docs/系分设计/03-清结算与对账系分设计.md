...
| 数据库级落地 | 完成态运行结果、逐笔匹配、差错和可清分明细切片已具备最小落点，整域仍待实现 | 已有 `t_reconciliation_run_result`、`t_reconciliation_match_result`、`t_reconciliation_difference` 和 `t_clearing_splittable_detail`、对应 Entity/Mapper、唯一键和 H2 schema；完整对账任务批次、来源记录、清分批次、清分结果快照、清算、结算和出款仍需继续补 DDL、DTO、Converter、fixture 和表结构测试。 |
| 服务级落地 | 完成态证据准入局部闭环，整域仍待实现 | 已有逐笔匹配结果固化和批次摘要派生、对账差错登记/调账回链/重跑、fail-closed reconciliation gate、可清分单笔准入、clearing/settlement gate consumer 和 payout preflight 的真实 H2 服务流程测试；任务调度、外部收数、归一化和匹配规则执行、完整清分批次、清算、结算、出款生命周期、权限审计和退款时序仍需继续补齐。 |
| 生产准入 | 待确认 | 需补 NFR、容量、观测告警、Runbook、灰度回滚、外部规则核验、专业确认状态、使用者解释字段、职责分离和证据最小化。 |

工程任务至少需要确认：是否复用 `reconciliation-face`、`reconciliation-impl`；是否扩展出款前准入设计；表命名和状态机；`TDD-B7-RED-001` 至 `TDD-B7-RED-009` Red 顺序；运营补事实命令白名单；出款回单、金额不一致、差错核销、追偿和敏感导出的审计字段；事实状态、展示状态、操作状态、不可操作原因、后续处理动作、责任方、到期重查和脱敏证据引用；高危动作职责分离；NFR 假设；以及财务、合规、法务、通道或持牌机构的确认状态。缺少任一 `CLS-GATE-*` 时，本分册只能继续做 TDD 分析、契约草案或实现差距复核，不得进入生产代码、测试代码、DDL/H2 schema 或运行时配置写入。
...
完整 `t_reconciliation_batch` 任务生命周期和匹配引擎尚未落地前，当前实现使用不可变 `t_reconciliation_run_result` 承接完成态批次头，使用 `t_reconciliation_match_result` 承接逐笔结果，避免 gate 把“差错表为空”或调用方自报汇总误判为对账通过。

```text
RecordReconciliationRunResultRequest
  -> ReconciliationRunResultApplicationService.recordRunResult
  -> 校验并规范化逐笔匹配结果
  -> 从逐笔结果派生状态和计数
  -> 同一事务固化 t_reconciliation_run_result + t_reconciliation_match_result
...
1. 运行结果 `sn`、逐笔结果 `sn`、组合 `sourceDigest`、逐笔 `matchIdentityDigest` / `matchDigest`、最终 `resultDigest`、状态和计数均由服务内部生成；调用方只提供批次、对象、规则、两侧来源集合摘要、逐笔匹配事实和证据引用。
2. 业务唯一键为 `tenant_id + reconciliation_batch_sn + gate_object_type + gate_object_sn`；相同事实重放返回原结果，事实漂移拒绝覆盖。
3. `EXACT_MATCH` 和 `RULE_MATCH` 只有在来源已验证且内部、外部引用均存在时才计入匹配；其他匹配强度计入差异，缺失类差异只允许对应一侧引用为空。
4. 只有派生出的 `BALANCED` 可继续差错检查；记录不存在、对象不匹配或任何非 `BALANCED` 结果全部失败关闭。
5. 已核销差错只有在最后重跑批次和规则版本与本次运行结果一致时才允许条件通过。
6. 该切片不采集外部文件、不执行归一化或匹配规则、不自动创建差错，也不创建清分、结算、出款或账本事实。
7. 差错报告仅在 `includeGateDecision=true` 且差错具备对象级定位时要求调用方显式提供 `reconciliationRunResultSn`；纯差错事实查询不要求该参数，报告服务不得自行选择“最新运行结果”。
8. 当前记录服务不读取已持久化的归一化来源事实，不能独立证明来源内容和覆盖范围；只能由受控内部对账适配层调用。来源事实绑定和调用权限未完成前，该能力不得作为生产自动放行已完备的声明。
...
| 契约 | 关键字段 | 说明 |
| --- | --- | --- |
| `RecordReconciliationRunResultRequest` | tenantId、reconciliationBatchSn、gateObjectType、gateObjectSn、ruleVersion、internalSourceDigest、externalSourceDigest、matchResults、evidenceRefs | 受控内部对账适配层提交完成态逐笔结果；状态、计数、流水号和摘要由服务内部生成。 |
| `ReconciliationRunResultDTO` | sn、批次、准入对象、状态、规则、来源摘要、结果摘要、匹配计数、证据引用、记录人和时间 | 作为后续 gate 的明确正向或阻断证据。 |
...
| ApplyExceptionActionRequest | requestSn、requestDigest、actionSn、exceptionSn、actionType、whitelistCommand、whitelistAuthorityRef、approvalSn、evidenceRefs、idempotencyKey、originalFactRef、fundsTransactionSn、adjustActionSn、ledgerTransactionSn、operatorRef、reason、description | 登记补事实、冲正、调账、挂账、追偿、核销等动作；只有白名单命令且证据齐备时才允许记录资金事实引用。 |
...
| TDD-RECON-004 清算前置对账 | runReconciliationMatching、createExceptionFromMatch、assertNoBlockingException | 清分结果快照已固化，存在清算候选输入。 | 执行清算前置对账并生成差异。 | 错主体、错币种、金额不平、余额公式不成立时生成差错并阻断候选或批次确认。 |
...
| TDD-RECON-008 对账数据分段 | createReconciliationBatch、recordMatchResult、createExceptionFromMatch | 同一对账批次同时包含交易数据、账务数据、清分清算数据和外部资金数据。 | 生成匹配结果和差错单。 | 差错必须标明数据分段、对账类型、来源记录和责任方；不得只用自由文本说明差异。 |
...
| internal_source_digest / external_source_digest / source_digest | varchar(64) | [X] | 两侧归一化事实集合摘要和内部生成的组合摘要。 |
...
唯一键：`tenant_id + sn`，以及 `tenant_id + reconciliation_batch_sn + gate_object_type + gate_object_sn`。该表只追加，不提供更新或删除业务接口。

##### 当前对账匹配结果明细表

表名：`t_reconciliation_match_result`

业务用途：追加保存一次完成态运行的逐笔匹配事实。每项保存内部/外部稳定引用、来源质量、匹配强度、可选差错字段、证据引用、来源对 `match_identity_digest` 和完整结果 `match_digest`；一对多或多对一场景可让稳定引用指向已固化的事实集合，不在本表复制业务明细。
