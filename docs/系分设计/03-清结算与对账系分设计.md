...
| 数据库级落地 | 既有切片、W1、W2 与 W3 已完成本地验收 | W1 结算单/金额项、W2 出款单/回单和 W3 追偿单/结果均已落地 Entity/Mapper、唯一键、生产 MySQL DDL、H2 schema 和精确表结构守卫。目标 MySQL 迁移与 RR 并发仍须专用环境证据。 |
...
| 生产准入 | 按切片证据独立裁决 | H2、DDL 静态契约、资金红线和独立 Checker 构成本地门禁；W2 目标 MySQL 迁移与 `REPEATABLE-READ` 并发用例是部署前外部证据。观测、Runbook 和 Web 审计仍由宿主负责。 |
...
当前已实现 `ClearingBatchApplicationServiceImpl`、清算批次表写入和候选 `CLEARED/BLOCKED/LOCKED` 推进。资金底座复核当前交易、退款累计、route snapshot 摘要、候选经济摘要和对象级 Gate；上述声明范围以 `tests` 模块 H2 全量集成测试完成项目生产验收。争议/风控权威判断和按原资金分配处理部分退款不在当前能力范围，不得由现有结果外推。
...
本节按架构师系分模板描述清结算与对账表结构。对账证据、差错、Gate、清分、清算、W1 结算单/金额项、W2 出款单/回单和 W3 追偿责任/结果引用已落地 H2 测试 schema、Entity、Mapper、服务层测试和 MySQL 首次建表基线。H2 与静态 DDL 构成仓内本地验收，W1/W2/W3 的目标 MySQL 迁移及 `REPEATABLE-READ` 并发仍须专用环境证据。字段命名若在实现时调整，需要回到本节和 TDD 同步校准。
...
| 当前来源与结果封版 | 单侧来源最多 1000 项、单次匹配结果最多 2000 项；超限在批次查询和加锁前失败，H2 边界用例作为项目生产验收。该限制不能解释为吞吐承诺；目标数据库压测由宿主按自身流量决定。 |
...
`tests` 模块 H2 全量集成测试作为 wind-funds 仓内本地验收；目标 MySQL 迁移与隔离级别相关并发语义必须由专用数据库门禁补证，不能由 H2 结果替代。下表用于区分仓内验收、目标数据库证据和宿主接入责任。

| 能力项 | 当前结论 | 验收或责任边界 |
| --- | --- | --- |
| 数据库与 DDL | 仓内 `GO`；目标数据库 `PENDING` | H2 全量集成测试、生产 DDL 静态契约、测试 schema 对齐和命令保护构成仓内验收。部署前必须在专用 `wind_funds_reconciliation_test` 库运行 `ReconciliationMysqlMigrationIntegrationTests` 与 `just test-mysql-reconciliation`。 |
| 数据库并发 | H2 `GO`；目标 MySQL `PENDING` | H2 双事务用例验证仓内竞争与失败关闭；目标 MySQL 用例必须在 `REPEATABLE-READ` 下验证唯一键失败后的 current read 可见性。其他容量和压测由接入方按部署环境决定。 |
| 可信来源接入 | 宿主责任 | 上层适配器负责来源验签、授权、去重、归一化和原文保管；reconciliation 只冻结稳定引用与摘要。 |
| 最终命令复核 | 仓内 `GO`（已声明范围） | 清分、内部清算、W1 结算锁定和 W2 出款提交均在自身事务内复核当前 Gate 与对应权威事实；W3 只登记宿主已确认责任与已完成资金交易引用。出款提交不调用外部通道；争议/风控权威查询、复杂结算、具体通道执行和追偿策略/资金执行不在当前能力范围。 |
...
五项契约用于说明 wind-funds 之外的接入责任：`SOURCE` 负责外部数据可信接入，`OBS` 负责监控告警，`RUNBOOK` 负责故障处置，`AUDIT` 负责 Web/宿主操作审计，`GOV` 负责数据治理。实际环境证据由接入方按自身发布制度管理，不属于 wind-funds 交付物，也不阻塞本项目生产准出。
...
| W2 最小出款资金事实 | `t_payout_order` + `t_payout_receipt`；当前已落地生产 DDL、H2 schema、Entity/Mapper、应用服务和资金原语。 | 锁定外部端点结算单、提交时 Gate/宿主权威准入、外部回单与 reference 唯一归属、非终态不动账、成功 `SETTLEMENT -> PREPAYMENT -> CASH`、失败 `SETTLEMENT -> AVAILABLE`、RETURNED/MISMATCHED 不自动动账，以及跨单/并发冲突失败关闭。 | 不调用具体通道，不启用 IN_TRANSIT，不做 FAILED 重试、拆分出款、来源采集/匹配、正式差错物化或追偿。目标 MySQL RR 并发仍须专用环境验证。 |
| W3 最小追偿资金事实 | `t_recovery_order` + `t_recovery_result`；当前已落地生产 DDL、H2 schema、Entity/Mapper、应用服务和 H2 并发红线；目标 MySQL RR 门禁已定义但尚未在专用环境执行。 | 已确认责任创建幂等、来源漂移拒绝、CREATED -> PARTIALLY_RECOVERED -> RECOVERED、只引用已关闭 RECOVERY 资金交易、责任主体/币种/金额校验、结果 append-only、同交易唯一认领和不超额。 | 不判断退款/拒付责任，不执行准备金、结算抵扣、负余额或人工收款，不创建资金交易，不做核销/关闭、来源采集匹配、运营台、IAM、审计、监控或 Runbook。目标 MySQL 迁移和 RR 并发仍须专用环境证据。 |
