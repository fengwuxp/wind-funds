...
| 数据库级落地 | 对账批次、冻结来源、完成态运行结果、逐笔匹配、差错和可清分明细已具备最小落点，整域仍待实现 | 已有 `t_reconciliation_batch`、`t_reconciliation_source_snapshot`、`t_reconciliation_source_item`、`t_reconciliation_run_result`、`t_reconciliation_match_result`、`t_reconciliation_difference` 和 `t_clearing_splittable_detail`、对应 Entity/Mapper、唯一键和 H2 schema；清分批次、清分结果快照、清算、结算和出款仍需继续补齐。 |
| 服务级落地 | 冻结来源到完成态证据准入已局部闭环，整域仍待实现 | 已有批次创建、两侧来源冻结、完整覆盖校验、逐笔结果与批次完成原子绑定、重跑引用、对账差错登记/调账回链、fail-closed reconciliation gate、可清分单笔准入、clearing/settlement gate consumer 和 payout preflight 的真实 H2 服务流程测试；任务调度、外部收数与解析、业务归一化、匹配规则执行、完整清分批次、清算、结算、出款生命周期、宿主权限审计和退款时序仍需继续补齐。 |
...
| 资金账务口径 | 资金域研发负责人、账务负责人。 | 复核 route、posting、ledger entry、余额桶、账本周期、调账、冲正、追偿和失败无副作用断言。 | 回到 DSL 或系分补齐账务矩阵、状态机和资金事实白名单。 |
...
| --- | --- | --- | --- |
| 交易模块 | 资金交易、交易明细、退款、调账、出款结果等资金事实。 | 仅通过资金事实白名单命令追加清算确认、结算锁定、出款结果、调账、冲正、补事实等标准资金动作。 | 直接改交易状态、直接插入交易明细、绕过交易层补账，或用泛化 actionType 生成资金事实。 |
...
| 出款前准入门禁 | PayoutOrderService.checkPayoutPreflight、submitPayoutOrder 共享结算锁定、出款账户、收款端点、通道额度、cutoff、名单筛查、外部规则核验、负余额、准备金、对账差错、幂等和审批守卫。 | DSL-SETTLEMENT-PAYOUT-RESULT-001 的出款提交前准入；任一门禁缺失、失败或未知时不生成 FUND_OUT / IN_TRANSIT。 | TDD-SETTLE-004、TDD-RED-023、TDD-OPS-001。 |
...
| 结算模式快照 | 结算单或结算策略快照记录 settlementMode、settlementDestination、triggerMode。 | 账单模式不生成 SETTLEMENT 锁定；冻结模式和中间户模式按对应资金事实白名单推进。 | TDD-SETTLE-006、TDD-B7-RED-008。 |
...
当前实现使用 `t_reconciliation_batch` 冻结准入范围，使用两张来源表冻结 `REFERENCE/COMPARISON` 成员集合，再由不可变 `t_reconciliation_run_result` 和 `t_reconciliation_match_result` 承接完成态结论，避免 gate 把“差错表为空”、局部匹配或调用方自报摘要误判为对账通过。

```text
CreateReconciliationBatchRequest
  -> 创建批次并冻结准入对象、规则版本、窗口和时区
...
4. 逐笔匹配可表达一对一、一对多或多对一，但所有冻结来源引用必须至少被覆盖一次，且任何引用都必须属于对应快照；缺失或越界均不得生成运行结果。
...
7. 重跑创建新批次并以 `previousBatchSn` 引用已完成批次；准入对象和窗口必须一致，允许使用新的规则版本，不覆盖旧来源和旧结果。同一租户内每个上一批次只允许一个直接重跑批次，血缘保持线性；新批次创建后，上一批次及其结果仍可审计查询，但立即失去门禁消费资格，重跑未完成时失败关闭。
8. 只有与 `COMPLETED` 批次正确绑定且派生为 `BALANCED` 的结果可继续差错检查；记录缺失、绑定不一致、对象不匹配或任何非 `BALANCED` 结果全部失败关闭。
9. 该切片不采集或解析外部文件、不执行来源业务归一化或匹配规则、不自动创建差错，也不创建清分、结算、出款或账本事实；宿主应用负责调用权限和证据访问控制。
...
`ReconciliationBatchStatus` 只描述已落地批次的工程进度：`CREATED` 已创建，`DATA_COLLECTING` 已冻结一侧来源，`DATA_READY` 两侧来源已冻结，`COMPLETED` 已绑定不可变运行结果。`BALANCED/DIFFERENCE_FOUND` 属于 `ReconciliationRunResultStatus`；`PASSED/CONDITIONAL_PASSED/BLOCKED` 属于 gate 决策。上层任务编排可另行表达 `WAITING_DATA`、解析失败、匹配执行失败和重试，但不得把这些任务态混入当前批次、运行结论或准入决策。
...
| AdjustmentAction | 运营补事实命令、调账/冲正/挂账/核销/追偿动作和审批链。 | 必须经过白名单、权限、审批、幂等和审计后生成后续动作。 | adjustmentActionRef、commandType、approvalRef、idempotencyKey。 | 不允许绕过白名单直接写账或关闭差错。 |
| AuditTrail | 资金操作审计表、运营后台操作记录和附件引用。 | 记录操作者、时间、前后状态、依据、附件、审批和复核。 | auditId、operatorRef、operationType、beforeAfterDigest、attachmentRef。 | 不保存敏感原文，不用审计备注替代事实证据。 |
...
| 挂账 | 暂存无法归属差异。 | 默认不生成资金事实；只有转为白名单调账命令且审批、凭证、重新对账齐备时，才可追加 ADJUSTMENT 或等价事实。 |
| 追偿 | 对已出款后退款、拒付、费用或差错发起追回。 | 进入准备金、后续抵扣、负余额或人工收款。 |
| 核销 | 经证据确认关闭差错。 | 不等同于改账，必须关联重新对账或凭证。 |

#### 4.3.2.1 运营补事实命令白名单

`actionType` 只表达运营动作分类，不是生成资金事实的授权字段。涉及资金变化时，系统必须同时满足 `whitelistCommand`、工程任务边界、审批、证据、幂等键、原事实引用和失败无副作用测试；未列入白名单的动作只能生成处理单、差异报告、人工复核记录或审计记录。

| 白名单命令 | 触发场景 | 允许生成的资金事实 | 必填证据 | 禁止和测试口径 |
| --- | --- | --- | --- | --- |
...
1. `whitelistCommand` 必须进入动作幂等摘要；同一 `actionSn` 或 `idempotencyKey` 重放只能返回同一事实引用。
2. `actionType` 与 `whitelistCommand` 不一致、白名单命令缺工程任务边界、缺审批或缺证据时，动作只能停留在差错处理历史，不得生成 `FundsInstruction`、route、posting plan 或 ledger entry。
3. 白名单命令必须显式声明可撤销边界；撤销只能追加 `REVERSAL` 或新的差错闭环动作，不能物理删除或覆盖历史事实。
4. 工程落地必须先补 Red：未列入白名单的 `ApplyExceptionActionRequest` 生成资金事实必须失败；白名单命令缺来源单据、审批、证据、原事实引用或幂等键必须失败。
...
| merchantSubjectRef | 商户、合作方或收益参与方的可记账主体引用。 |
| currency | 币种。 |
| amountItems | 本金、费用、退款、争议、权益、补贴、分润、调整和准备金等金额项。 |
...
| ruleCode/ruleVersion | 清分规则编码和版本。 |
| reconciliationSn/admissionStatus | 清分前基础对账引用和准入结论。 |
| snapshotDigest | 参与方、来源事实、金额项、规则快照和对账结论的 sha256 摘要。 |

系统约束：

1. 生成快照不得调用交易层、账本过账或余额投影写入能力。
2. 快照摘要必须覆盖来源事实、金额项、规则版本和对账准入结论，用于幂等和重跑解释。
...
| ReconciliationBatchStatus | 对账批次执行进度 | CREATED、DATA_COLLECTING、DATA_READY、COMPLETED |
...
| 契约 | 关键字段 | 说明 |
| --- | --- | --- |
| `CreateReconciliationBatchRequest` | tenantId、gateObjectType、gateObjectSn、ruleVersion、windowStart、windowEnd、timezoneId、previousBatchSn | 创建批次并冻结准入范围；`previousBatchSn` 仅用于引用已完成重跑来源。 |
| `RecordReconciliationSourceSnapshotRequest` | tenantId、reconciliationBatchSn、sourceRole、sourceType、sourceItemRefs、evidenceRefs | 为 `REFERENCE` 或 `COMPARISON` 冻结不可变来源成员集合；不携带调用方生成的摘要。 |
| `RecordReconciliationRunResultRequest` | tenantId、reconciliationBatchSn、matchResults | 提交已完成的逐笔匹配结论；准入对象、规则、来源摘要和来源证据全部从冻结批次读取。 |
| `ReconciliationBatchDTO` | sn、准入对象、规则、窗口、previousBatchSn、status、runResultSn、batchDigest、创建信息 | 返回批次范围、执行进度和完成态结果绑定。 |
| `ReconciliationSourceSnapshotDTO` | sn、批次、角色、类型、sourceDigest、recordCount、sourceItemRefs、evidenceRefs、创建信息 | 返回一侧冻结来源事实及内部摘要。 |
| `ReconciliationRunResultDTO` | sn、批次、准入对象、status、ruleVersion、两侧摘要、组合摘要、结果摘要、匹配计数、证据引用 | 作为后续 gate 的明确正向或阻断证据。 |
...
1. `createBatch` 按内部 `batchDigest` 与业务唯一键幂等；重跑必须生成新批次并引用已完成批次，不提供覆盖或关闭旧批次接口。服务锁定上一批次后以 `FOR UPDATE` current read 读取唯一子批次并比较摘要；`tenantId + previousBatchSn` 唯一键并发兜底，禁止同父分叉。摘要唯一键竞争也必须用 current read 加载赢家，不能依赖 `REPEATABLE READ` 的旧快照。门禁通过 `previousBatchSn` 反向检查结果是否已被替代，已被引用的旧结果只能审计追溯，不能继续准入。
...
5. 当前不提供对账批次公共查询服务、调度服务或匹配引擎接口；宿主应用可以围绕已落地命令服务编排，但不能绕过来源冻结和 gate。
...
| 模型 | 关键字段 | 说明 |
| --- | --- | --- |
| `ReconciliationMatchResultItem` | referenceSourceRef、comparisonSourceRef、sourceQuality、matchStrength、differenceType、severity、currency、differenceAmount、evidenceRef | 表达一项匹配结论；至少有一侧引用。 |
...
| 其他差错项 | 两侧或单侧引用、非自动匹配强度、差错类型和等级 | 可携带非负差异金额；金额大于 0 时必须有币种。 |
...
| Request | 关键字段 | 说明 |
| --- | --- | --- |
| CreateReconciliationDifferenceRequest | tenantId、differenceSn、reconciliationBatchSn、sourceRecordSn、sourceQuality、matchStrength、differenceType、severity、currency、differenceAmount、responsiblePartyRef、blockingScope、blockingObjectType、blockingObjectSn、ruleVersion、evidenceRef、description | 登记上层已确认的差错运营对象，不直接发起补事实、冲正、调账或追偿。 |
...
4. `lastRerunSn` 和 `lastRerunEvidenceRef` 均保存 `ReconciliationRunResult.sn`，其余最后重跑字段是运行结果事实快照，不接受调用方覆盖。
...
1. 差错单不得直接改账；涉及资金变化时，ApplyExceptionActionRequest 只能记录白名单命令下由交易层、余额控制层或账本层返回的事实引用。
...
5. actionSn、releaseSn、whitelistCommand 和 idempotencyKey 必须用于动作级幂等，避免重复调账、重复放行、重复补事实或重复核销。
6. actionType 未列入白名单、whitelistAuthorityRef 缺失或白名单命令缺必要证据时，applyExceptionAction 必须失败或仅登记非资金动作，不得生成 FundsInstruction、route、posting plan 或 ledger entry。
...
| 模型 | 字段 | 说明 |
| --- | --- | --- |
| CheckPayoutPreflightRequest | tenantId、settlementSn、payoutSn、currency、amount、payoutAccountRef、payeeEndpointRef、channelRef、idempotencyKey、ruleEvidenceRef、externalRuleVerificationEvidence、approvalRef | 出款提交前的准入检查请求；允许在创建出款单前只传 settlementSn，也允许在提交前带 payoutSn 复核；`ruleEvidenceRef` 只作审计引用，准入放行必须以结构化外部规则核验证据摘要为准。 |
| PayoutPreflightResultDTO | passed、blockingLevel、blockingReasons、manualReviewRequired、factStatus、displayStatus、operationStatus、externalRuleVerificationStatus、checkedAt、checkedBy、expiresAt、evidenceRefs | 准入结果必须能说明是否通过、阻断等级、阻断原因、是否转人工、事实状态、展示状态、操作状态、外部规则核验状态、检查时间、有效期和证据引用。 |
| PayoutPreflightBlockingReasonDTO | code、message、guardName、severity、recoverable、evidenceRef、confirmationOwner | 单个门禁失败或未知原因；不得保存完整外部账户、敏感凭证或原始文件。 |
...
blockingReasons 至少覆盖 SETTLEMENT_NOT_LOCKED、PAYOUT_ACCOUNT_INVALID、PAYEE_ENDPOINT_INVALID、CHANNEL_UNAVAILABLE、CHANNEL_LIMIT_EXCEEDED、CUTOFF_CLOSED、WATCHLIST_BLOCKED、EXTERNAL_RULE_UNVERIFIED、NEGATIVE_BALANCE_BLOCKED、RESERVE_INSUFFICIENT、RECONCILIATION_BLOCKED、IDEMPOTENCY_CONFLICT、APPROVAL_REQUIRED。submitPayoutOrder 必须重新执行同一组门禁，不能只信任过期的 checkPayoutPreflight 结果。factStatus、displayStatus 和 operationStatus 不得由前端或运营台自行推断，必须由服务端按同一准入结果生成；缺任一状态时视为不可提交外部出款。
...
| 实现类或组件 | 职责 | 依赖 |
| --- | --- | --- |
| ReconciliationBatchApplicationServiceImpl | 创建批次并冻结 `REFERENCE/COMPARISON` 来源快照，内部生成流水号和摘要；当前已落地。 | 批次、来源快照和来源成员 Mapper。 |
| ReconciliationRunResultApplicationServiceImpl | 校验冻结来源完整覆盖，固化完成态运行结果和逐笔结果，并原子完成批次；当前已落地。 | 批次、来源、运行结果和逐笔结果 Mapper。 |
| ReconciliationGateApplicationServiceImpl | 校验批次完成绑定、运行结论和差错闭环，输出只读准入决策；当前已落地。 | 批次、运行结果和差错 Mapper。 |
...
| ClearingSplittableDetailApplicationServiceImpl | 按资金交易、交易明细和 LedgerEntry 稳定引用执行单笔可清分准入并幂等记录结果；当前已落地。 | 交易 face 查询、账本 face 查询、对账 gate。 |
| ClearingSplitServiceImpl | 后续管理清分批次，并在确认后固化清分结果快照。 | 可清分明细、基础对账结论；当前未实现。 |
| ClearingPeriodResolver | 解析清算账期、cutoff、节假日和可清算时间。 | 清算策略快照。 |
| ClearingCandidateServiceImpl | 生成和管理清算候选。 | 清分结果快照、对账结论、退款/争议/风控状态。 |
...
| --- | --- | --- | --- | --- |
| TDD-RECON-007 对账批次生命周期 | createBatch、recordSourceSnapshot、recordRunResult、checkGate | 存在租户、准入对象、对账窗口、规则版本和来源证据引用。 | 创建批次、依次冻结两侧来源、提交完整匹配结果并检查准入。 | 批次只按 CREATED -> DATA_COLLECTING -> DATA_READY -> COMPLETED 推进；运行结论和门禁决策独立表达；重跑创建新批次且旧结果立即失去门禁资格。 |
| TDD-RECON-003 清分前基础对账 | createBatch、recordSourceSnapshot、recordRunResult、checkGate、ClearingSplittableDetailApplicationService#identifySplittableDetail | 交易、单据、账本分录、余额桶和 route snapshot 均存在。 | 上层完成来源采集和匹配，资金底座冻结来源、登记结果，并在可清分识别前检查准入。 | 完全一致时允许识别；存在阻断差异时不生成可清分明细。 |
...
| TDD-RECON-001 对账差错单 | createDifference、linkAdjustmentResult、createBatch(previousBatchSn)、recordRerunResult | 匹配结果存在金额不一致。 | 登记差错、回链上层已完成的处理事实、创建重跑批次并登记重新对账结果。 | 差错单有来源、责任、审批、凭证、资金事实引用和重新对账结果；不得直接修改历史分录或余额。 |
...
| TDD-RACE-007 对账重跑与差错核销并发 | createBatch(previousBatchSn)、recordRerunResult | 同一差错正在核销，同时对账批次被重跑。 | 并发执行重跑和核销。 | 上一批次行锁内以 current read 读取子批次并比较摘要，保证相同请求复用同一事实；行锁与 `tenantId + previousBatchSn` 唯一键禁止不同请求形成重跑分叉。差错绑定和门禁检查均锁定候选批次，再以 current read 检查后继，与后继创建按批次行串行。不覆盖审批和核销结果；已关闭差错不得被新结果自动重开。H2 用例验证并发 API 行为，生产交付还必须在真实 MySQL `REPEATABLE READ` 下复核锁等待和 current-read 语义。 |
| TDD-CLS-001 交易进入可清分 | ClearingSplittableDetailApplicationService#identifySplittableDetail | 成功或仍 OPEN 但当前明细已成功的交易、商户 CLEARING 分录、可解析 route snapshot、posting plan、账本交易和对账 gate。 | 识别单笔可清分明细。 | 生成 SPLIT_READY 且不改变 route、posting、ledger transaction、entry 或余额；同一 LedgerEntry 重跑幂等。当前由 `ClearingSplittableDetailApplicationServiceTests` 承接。 |
...
| TDD-RECON-008 对账数据分段 | createBatch、recordSourceSnapshot、recordRunResult | 批次来源分别为交易、账本、余额、外部流水或清结算报表。 | 冻结来源并生成匹配结果。 | 来源快照必须保留 `sourceType`、成员稳定引用和证据；差错不得只用自由文本说明来源。 |
...
2. 基础对账 API 不依赖外部 `requestSn/requestDigest`；必须用服务端流水号、业务唯一键、稳定摘要和状态前置条件验证幂等与并发。
3. 清分、清算、结算、出款和追偿 API 必须通过阻断守卫和红线测试证明不会绕过基础对账准入。
4. 如果实现时发现某个用例需要直接调用 Mapper 才能断言，则说明 API 或查询 DTO 设计不足，应先补契约再写实现。

### 4.12 数据表设计

本节按架构师系分模板描述清结算与对账表结构。对账批次、来源快照/成员、运行结果、逐笔匹配、差错和可清分明细已落地 DDL/H2 schema、Entity、Mapper 和服务层测试；其余清分、清算、结算、出款和追偿仍是目标态，进入编码前必须同步补齐表结构与测试。字段命名若在实现时调整，需要回到本节和 TDD 同步校准。
...
业务用途：追加保存一次已完成对账运行的范围、规则、来源摘要、派生匹配摘要和证据引用，供清分、结算和出款准入显式消费。
...
| gate_object_type / gate_object_sn | varchar | [X] | 本次结果适用的准入对象。 |
...
| evidence_refs | text | [X] | 来源文件、报表或匹配报告引用 JSON。 |
| created_by / gmt_create | varchar / datetime | [X] | 记录人和记录时间。 |

唯一键：`tenant_id + sn`，以及 `tenant_id + reconciliation_batch_sn`。准入对象和规则由批次继承，调用方不能用同一批次写入另一对象或规则；该表只追加，不提供更新或删除业务接口。

##### 当前对账匹配结果明细表

表名：`t_reconciliation_match_result`

业务用途：追加保存一次完成态运行的逐笔匹配事实。每项保存基准侧/核对侧稳定引用、来源质量、匹配强度、可选差错字段、证据引用、来源对 `match_identity_digest` 和完整结果 `match_digest`；一对多或多对一通过多条匹配项覆盖同一稳定引用，不在本表复制业务明细。
...
业务用途：冻结一次对账的准入对象、规则版本、时间窗口、时区和重跑关系，并在完成时绑定唯一运行结果。

| 字段名称 | 字段类型 | 是否必填 | 字段说明 |
| --- | --- | --- | --- |
| sn / tenant_id | varchar(64) / bigint(20) | [X] | 内部生成的批次流水号和租户。 |
| gate_object_type / gate_object_sn | varchar | [X] | 本批次结果可被消费的准入对象。 |
| rule_version | varchar(64) | [X] | 匹配或对账规则版本。 |
| window_start / window_end / timezone_id | datetime / varchar | [X] | 左闭右开对账窗口及 IANA 时区。 |
| previous_batch_sn | varchar(64) | [ ] | 重跑引用的上一已完成批次；同一租户内非空值唯一，禁止同父分叉。 |
| status | varchar(50) | [X] | CREATED、DATA_COLLECTING、DATA_READY、COMPLETED。 |
| run_result_sn | varchar(64) | [ ] | COMPLETED 时绑定的唯一运行结果。 |
| batch_digest | varchar(64) | [X] | 范围、规则、窗口和重跑关系的 SHA-256。 |
...
| evidence_refs | text | [X] | 文件、报表或采集结果的稳定证据引用 JSON。 |
...
业务用途：保存来源快照中每个不可变事实的稳定引用和内部身份摘要，供运行结果验证引用归属与完整覆盖。

唯一键：`tenant_id + sn`、`tenant_id + source_snapshot_sn + source_item_ref`。`item_digest` 由来源角色、类型和规范化引用生成，读取时重新计算校验；成员引用允许指向交易、账本、余额、外部流水或报表的稳定事实标识，但本表不复制业务原文。

五张表共同形成 `batch -> source snapshot/item -> run result -> match result` 证据链。批次及其来源只描述核对范围和输入，运行结果只描述完成态结论，逐笔结果只描述匹配事实；任何一层都不得直接修改交易、账本或余额。
...
| currency/principal_amount | varchar(10)/bigint(20) | [X] | 币种与本金最小单位金额 | USD/1000 |
| refund_amount | bigint(20) | [X] | 清分前已退款金额；争议只有形成退款资金事实后才在此体现 | 0 |
| clearing_period | varchar(30) | [X] | 上层策略已解析的清分周期 | 2026-05-20 |
| rule_code/rule_version | varchar(64) | [X] | 清分规则编码与版本 | MERCHANT_CLEARING/1 |
...
| reconciliation_evidence_refs | text | [X] | 对账证据引用 JSON，不保存完整外部报文 | [] |
| source_digest | varchar(64) | [X] | 来源事实和规则快照 SHA-256 | 64 位 hex |
| created_by | varchar(64) | [X] | 创建操作人 | 10001 |

状态说明：

- SPLIT_READY：来源事实与清分前准入通过，可以进入后续清分批次。
- EXCLUDED：当前来源事实被稳定原因排除；不表示资金交易或账本事实失败。
- 当前切片不定义 WAITING、SPLIT_LOCKED 或 SPLIT_CONFIRMED；这些状态只有在清分扫描调度和清分批次实现时再引入。
...
| idx_clearing_splittable_detail_subject | tenant_id, subject_type, subject_id, clearing_period | 主体周期扫描 |
...
- 当前切片只固化本金和清分前退款金额；费用、税费、补贴、权益、准备金和争议案件金额项由后续清分规则/结果快照切片定义，争议只有形成退款资金事实后才通过 refund_amount 体现。
- 当前切片尚无锁定或入批能力；同一明细只能进入一个未取消批次的约束由后续清分批次表和并发测试承接。
...
| merchant_subject_type | varchar(50) | [X] | 商户、合作方或收益参与方主体类型 | MERCHANT |
| merchant_subject_id | varchar(30) | [X] | 商户、合作方或收益参与方主体 ID | M1001 |
| currency | varchar(10) | [X] | 币种 | CNY |
| clearing_period | varchar(30) | [X] | 清分周期 | 2026-05-20 |
| amount_items_json | varchar(2048) | [X] | 金额项 JSON 摘要，含本金、费用、退款、争议、权益、补贴、分润、调整和准备金 | {"principal":1000} |
| funds_transaction_sn | varchar(64) | [X] | 来源资金交易号 | FT100001 |
| funds_transaction_detail_sn | varchar(64) | [X] | 来源资金交易明细号 | FTD100001 |
| ledger_transaction_sn | varchar(64) | [X] | 来源账本交易号 | LT100001 |
| ledger_entry_sn | varchar(64) | [X] | 来源账本分录号 | LE100001 |
| route_snapshot_digest | varchar(128) | [X] | route snapshot 摘要 | sha256:route |
| rule_code | varchar(64) | [X] | 清分规则编码 | MERCHANT_CLEARING |
| rule_version | int(11) | [X] | 清分规则版本 | 1 |
| reconciliation_sn | varchar(64) | [ ] | 清分前基础对账号 | RB100001 |
| admission_status | varchar(50) | [X] | 对账准入结论 | ADMITTED |
| snapshot_digest | varchar(128) | [X] | 参与方、来源事实、金额项、规则快照和对账结论摘要 | sha256:snapshot |
| status | varchar(50) | [X] | 快照状态 | ACTIVE |

唯一索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| uk_clearing_split_result_snapshot_sn | sn | 快照号唯一 |
| uk_clearing_split_result_snapshot_digest | tenant_id, split_batch_sn, splittable_detail_sn, snapshot_digest | 同一来源同一摘要重复确认幂等 |

普通索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_clearing_split_result_snapshot_subject | merchant_subject_type, merchant_subject_id, clearing_period | 主体周期查询 |
| idx_clearing_split_result_snapshot_detail | splittable_detail_sn | 按来源明细追溯 |

数据约束与目标态校验：

- 快照内容字段生成后不得覆盖；错分、漏项或补差通过新快照、差错单或候选排除承接。
- snapshot_digest 使用 sha256，必须覆盖参与方、金额项、来源事实、规则快照和对账准入结论。
...
| 对账批次 | `funds_reconciliation_batch` 或等价批次表。 | 范围、来源、规则版本、状态、幂等重跑、失败审计。 | 不生成清分批次、清算批次、结算单或出款单。 |
| 匹配规则执行 | 规则、字段映射和任务执行服务；当前未落地。 | 从可信归一化来源生成逐笔匹配事实，保留规则版本、阈值和失败样本。 | 不直接改交易状态、ledger entry 或余额投影，不与已落地的结果固化服务混为一体。 |
| 差错单 | `funds_reconciliation_exception_case` 或等价差错表。 | 差错生命周期、责任方、处理动作、审批引用、凭证、重新对账和关闭条件。 | 不覆盖全量调账、核销、追偿和出款后逆向。 |
| 操作审计 | `funds_operation_audit` 或等价高危操作审计表。 | 操作者、审批号、原因、证据引用、脱敏查看、导出和告警可追踪。 | 不建设完整运营后台和全量报表导出。 |

首切片通过后，再按可清分明细、清分批次、清分结果快照、清算候选、清算批次、结算单、出款单和追偿闭环逐步扩展。每个扩展切片都必须重新声明新增表、唯一键、幂等键、状态迁移、H2 schema、目标 Red 和回归范围。
