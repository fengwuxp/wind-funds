...
| 待处理对象发现 | 当前应用服务支持按状态和时间上界分页查询清分批次、清算候选与清算批次，固定返回只读 DTO；生产 DDL 已为批次状态+账龄、候选状态+可清算时间、候选状态+状态变更时间及所属批次回查提供复合索引。生产宿主应固定本轮时间上界后按小页扫描并保存自己的执行检查点，不得把查询结果当作确认或释放授权；准出时必须在峰值等量数据上用 `EXPLAIN` 证明查询命中对应索引且估算扫描行数受分页容量约束。 |
...
五项契约必须由宿主对应 Owner 提供实际环境证据；接口存在、H2 测试通过、文档签字或资金底座保存了 operator/摘要字段，都不能单独替代宿主验收。
