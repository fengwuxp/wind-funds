...
PayoutOrderDTO 的 state、displayStatus 和 nextAction 必须与出款单状态、前置门禁、外部回单、金额一致性、规则核验和差错状态共同推导。外部 accepted、submitted、message sent、processing、IN_TRANSIT、MISMATCHED、EXTERNAL_RULE_UNVERIFIED 或 MANUAL_REVIEW_REQUIRED 都不得映射为到账成功、可出款完成或可自动放行。
...
| W2 最小出款资金事实 | `t_payout_order` + `t_payout_receipt`；当前已落地生产 DDL、H2 schema、Entity/Mapper、应用服务和资金原语。 | 锁定外部端点结算单、提交时 Gate/宿主权威准入、外部回单与 reference 唯一归属、非终态不动账、成功 `SETTLEMENT -> PREPAYMENT -> CASH`、失败 `SETTLEMENT -> AVAILABLE`、RETURNED/MISMATCHED 不自动动账，以及跨单/并发冲突失败关闭。 | 不调用具体通道，不启用 IN_TRANSIT，不做 FAILED 重试、拆分出款、来源采集/匹配、正式差错物化或追偿；实际数据库并发兼容由宿主验证。 |
