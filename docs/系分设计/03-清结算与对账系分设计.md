...
| 对账差错调账 | 差错单、审批、凭证、调账事实和重新对账。 | `DSL-SETTLEMENT-RECONCILIATION-ADJUST-001`、`DIRECT_TRANSACTION / ADJUST`。 | `TDD-RECON-001`、`TDD-RECON-002`、`TDD-OPS-001`。 |
...
本节保留清结算与对账表的职责、关键字段组、状态、唯一性和资金红线，不再逐字段展开类型示例。字段类型和长度以实现 DDL 与测试 schema 为准；所有金额使用最小货币单位，所有批次和单据必须保留租户、范围、规则版本、状态、摘要、操作者或系统来源、trace 和审计线索。

#### 4.12.1 通用字段组

| 字段组 | 适用对象 | 设计要求 |
| --- | --- | --- |
| 批次和单据标识 | 对账、清分、清算、结算、出款、追偿 | 保留唯一业务号；跨模块引用使用业务号，不暴露主键。 |
| 来源证据 | 对账匹配、可清分、候选、差错 | 引用交易明细、ledger entry、外部流水、文件摘要、规则版本和差异指纹。 |
| 金额摘要 | 批次、单据、明细 | 拆分本金、费用、退款、扣减、准备金、追偿等金额项；不得只存总额。 |
| 状态和幂等 | 全部命令对象 | 状态由命令推进；同范围、同规则版本、同摘要重复扫描或确认必须幂等。 |
| 审计和审批 | 高危动作 | 清算确认、结算锁定、出款回单、差错核销、调账、敏感导出必须记录审批、原因、凭证和 trace。 |

#### 4.12.2 对账和差错表

| 表 | 业务用途 | 关键字段组 | 唯一性和红线 |
| --- | --- | --- | --- |
| `t_reconciliation_batch` | 对账任务、范围、规则版本、数据源和运行结果。 | 对账号、类型、范围、规则、内部/外部来源、状态、匹配摘要、差异数量、操作者、trace。 | 对账号唯一；同范围同规则同文件摘要幂等；对账任务不得改交易或账本事实。 |
| `t_reconciliation_match_result` | 账实、账账、单账和余额匹配结果。 | 结果号、对账号、内部来源、外部来源、差异类型、严重等级、阻断范围、差异摘要、状态。 | `reconciliationSn + differenceDigest` 去重；只引用证据，不承载调账结果。 |
| `t_exception_case` | 差错单、责任方、处理动作和关闭结果。 | 差错号、来源、责任主体、金额、等级、阻断范围、状态、动作摘要、审批、凭证、重新对账。 | 同来源、差异类型、责任主体只允许一个未关闭差错；差错单不得直接改账。 |
| `t_funds_operation_audit` | 高危操作、审批动作、敏感导出和人工复核审计。 | 审计号、操作者、操作类型、对象、前后摘要、原因、审批、凭证、trace、结果。 | 审计号唯一；审计失败不得静默吞掉，高危资金动作应失败或进入待补审计。 |

#### 4.12.3 清分和清算表

| 表 | 业务用途 | 关键字段组 | 状态和约束 |
| --- | --- | --- | --- |
| `t_clearing_splittable_detail` | 可清分明细池。 | 明细号、交易明细、ledger entry、商户主体、币种、金额项、清分周期、规则版本、状态、排除原因。 | 只来自成功交易和商户 `CLEARING` 分录；失败交易、缺快照、缺分录进入排除。 |
| `t_clearing_split_batch` | 清分批次。 | 批次号、商户、币种、清分周期、规则版本、明细摘要、金额摘要、状态、操作者。 | 确认只固化分类结果，不触发 `CLEARING -> AVAILABLE`；已确认批次不得删除或重写。 |
| `t_clearing_candidate` | 清算候选池。 | 候选号、来源清分明细、清算账期、可清算时间、金额、状态、阻断原因、规则版本。 | 清分确认、前置对账通过、账期到达、退款/争议/风控守卫均满足后才可入池。 |
| `t_clearing_batch` | 清算批次。 | 批次号、商户、币种、清算账期、规则版本、候选摘要、金额摘要、状态、清算资金交易号。 | `confirmClearingBatch` 是清算阶段唯一触发资金事实的动作；重复确认不得重复入账。 |
| `t_clearing_batch_detail` | 清算批次和候选明细绑定关系。 | 明细号、批次号、候选号、来源清分批次和明细、交易明细、ledger entry、金额、状态、失败原因。 | 批次号 + 候选号唯一；必须能回溯到原交易和分录。 |

#### 4.12.4 结算、出款和追偿表

| 表 | 业务用途 | 关键字段组 | 状态和红线 |
| --- | --- | --- | --- |
| `t_settlement_order` | 结算净额确认对象。 | 结算单号、结算主体、币种、结算周期、总额、加项、扣减、准备金、净额、状态、审批、锁定资金交易、规则版本、摘要。 | 锁定时触发 `AVAILABLE -> SETTLEMENT`；不代表外部出款成功。 |
| `t_settlement_order_item` | 结算金额项拆分。 | 金额项号、结算单号、类型、方向、金额、来源单据、规则版本、审批、描述。 | 结算净额必须可拆解到本金、费用、退款、争议、准备金、追偿、汇差或调账项。 |
| `t_payout_order` | 出款执行状态、外部回单和失败回退入口。 | 出款单号、结算单、收款主体、金额、脱敏外部账户、通道、外部 reference、状态、受理/完成时间、在途资金交易、失败原因、回单摘要。 | 外部受理不等于到账成功；成功、失败、退回、金额不一致必须分开。 |
| `t_recovery_order` | 已出款后退款、拒付、费用、差错或负余额的追偿责任。 | 追偿单号、来源对象、责任主体、应追/已追金额、追偿方式、状态、账本交易引用、审批、凭证。 | 出款后发生损失不得回滚历史出款；必须进入追偿、后续抵扣、负余额、人工收款或核销。 |
