...
| 里程碑 | 负责人 | 交付物 | 验收方式 |
| --- | --- | --- | --- |
| 设计准入 | 产品、架构、资金域研发、测试 | PRD、DSL、系分和 TDD 对齐，出款前门禁、外部非终态、金额不一致和出款解释状态口径一致。 | 文档 CR、结构检查和 `git diff --check` 通过。 |
| 数据库级落地 | 资金域研发、DBA、测试 | DDL、H2 schema、Entity、Mapper、DTO、Converter 和 fixture 对齐。 | 表结构测试、Mapper/Converter 测试和边界测试通过。 |
| 服务级落地 | 资金域研发、测试、SRE | 对账、清分、清算、结算、出款、差错、追偿和审计服务可运行。 | H2 服务流程测试、幂等并发测试、权限审计测试和失败路径回归通过。 |
| 生产准入 | 产品、财务、风控、合规、SRE | Runbook、监控告警、权限矩阵、外部规则核验和灰度/回滚方案齐备。 | 生产交付 CR、演练记录、外部规则确认和上线审批通过。 |

### 2.7 系分交付状态与编码准入门禁

本分册从系统分析视角已具备交付评审状态：模块定位、依赖关系、对象关系、子模块边界、接口入口、状态机、表设计、运营后台、测试对象和金融红线已经承接产品分册。该状态只能说明“系分可交付评审”，不能替代编码授权。
...
| TDD 分析 | 可进入 | 可以输出首批 Red、DDL/H2 范围、服务级 H2 流程、并发重跑、权限审计、使用者解释、外部规则和 Not Done 矩阵。 |
| 编码准入 | 未打开 | 必须由用户确认独立 Execution Grant，逐项填写 `CLS-GATE-*`，明确写入范围、禁止范围、验证命令和停止条件。 |
| 数据库级落地 | 未完成 | 需补 DDL、H2 schema、Entity、Mapper、DTO、Converter、fixture 和表结构测试。 |
| 服务级落地 | 未完成 | 需补真实 H2 服务流程测试、幂等重跑、并发竞争、权限审计和失败无副作用测试。 |
| 生产准入 | 未完成 | 需补 NFR、容量、观测告警、Runbook、灰度回滚、外部规则核验、专业确认状态、使用者解释字段、职责分离和证据最小化。 |

Execution Grant 至少需要确认：是否复用 `reconciliation-face`、`reconciliation-impl`；是否扩展出款前准入候选能力；表命名和状态机；`TDD-B7-RED-001` 至 `TDD-B7-RED-007` 首批 Red；运营补事实命令白名单；出款回单、金额不一致、差错核销、追偿和敏感导出的审计字段；事实状态、展示状态、操作状态、不可操作原因、下一步动作、责任方、到期重查和脱敏证据引用；高危动作职责分离；NFR 假设；以及财务、合规、法务、通道或持牌机构的确认状态。缺少任一 `CLS-GATE-*` 时，本分册只能继续做 TDD 分析、契约草案或实现差距复核，不得进入生产代码、测试代码、DDL/H2 schema 或运行时配置写入。
...
| --- | --- | --- | --- |
| 交易模块 | 资金交易、交易明细、退款、调账、出款结果等资金事实。 | 仅通过 Execution Grant 明确的运营补事实命令白名单追加清算确认、结算锁定、出款结果、调账、冲正、补事实等标准资金动作。 | 直接改交易状态、直接插入交易明细、绕过交易层补账，或用泛化 actionType 生成资金事实。 |
...
`actionType` 只表达运营动作分类，不是生成资金事实的授权字段。涉及资金变化时，系统必须同时满足 `whitelistCommand`、Execution Grant、审批、证据、幂等键、原事实引用和失败无副作用测试；未列入白名单的动作只能生成处理单、差异报告、人工复核记录或审计记录。
...
2. `actionType` 与 `whitelistCommand` 不一致、白名单命令缺 Execution Grant、缺审批或缺证据时，动作只能停留在差错处理历史，不得生成 `FundsInstruction`、route、posting plan 或 ledger entry。
3. 白名单命令必须显式声明可撤销边界；撤销只能追加 `REVERSAL` 或新的差错闭环动作，不能物理删除或覆盖历史事实。
4. 编码准入必须先补首批 Red：未列入白名单的 `ApplyExceptionActionRequest` 生成资金事实必须失败；白名单命令缺来源单据、审批、证据、原事实引用或幂等键必须失败。
5. `EARLY_AVAILABILITY_RELEASE` 不属于默认生产能力。只有业务 PRD、系分、TDD、外部规则确认和 Execution Grant 同时明确风险承担、准备金或授信、追偿、额度冻结、到期复核、对账阻断和回滚策略时，才允许进入编码。
