...
| 数据库级落地 | 首切片已具备最小落点，整域仍待实现 | 已有对账差错 `t_reconciliation_difference`、对应 Entity/Mapper 和首批表结构验证语义；清分、清算、结算、出款、对账任务和匹配结果等整域对象仍需继续补 DDL、H2 schema、DTO、Converter、fixture 和表结构测试。 |
| 服务级落地 | 首切片已具备最小闭环，整域仍待实现 | 已有对账差错登记/调账回链/重跑、reconciliation gate、clearing/settlement gate consumer 和 payout preflight 的首批真实 H2 服务流程测试；完整清分、清算、结算、出款生命周期、并发竞争、权限审计和失败无副作用闭环仍需继续补齐。 |
...
| ClearingSplitServiceImpl | 识别可清分明细，管理清分批次，并在确认后固化清分结果快照。 | 交易查询、账本查询、基础对账结论。 |
