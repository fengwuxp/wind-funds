...
| 边界 | 设计约束 | 越界风险 |
| --- | --- | --- |
| 系统边界 | 对账清算模块只维护对账、清分、清算、结算、出款、差错、追偿等业务对象及宿主审计引用；不维护操作审计对象。资金变化必须经交易层或账本层追加标准资金事实。 | 对账清算模块直接写账、改交易、绕过路由或重复建设宿主操作审计，导致资金事实与责任边界不可解释。 |
...
| PRD/TDD 门禁映射 | 通过 | 资金侧绑定产品 05 第 5.6.1 节的 `CLS-GATE-001` 至 `CLS-GATE-004` 和对应 Red；`CLS-GATE-005` 至 `CLS-GATE-008` 记录宿主接入责任，不作为 wind-funds 工程或准出阻塞项。 |
| TDD 分析 | 通过 | 已覆盖声明范围的 Red 顺序、DDL/H2、服务流程、并发重跑和未覆盖范围；宿主权限审计、使用者视图和 Runbook 由接入方自行验收。 |
| 工程边界 | 通过（已声明范围） | 写入范围、禁止范围、验证命令和停止条件已确认；未提供能力不得由当前结果外推。 |
| 数据库级落地 | 既有切片、W1、W2 与 W3 已完成仓内验收 | W1 结算单/金额项、W2 出款单/回单和 W3 追偿单/结果均已落地 Entity/Mapper、唯一键、生产 MySQL DDL、H2 schema 和精确表结构守卫；实际数据库迁移与并发兼容由宿主验证。 |
| 服务级落地 | 既有切片、W1、W2 与 W3 已完成本地验收 | W1 完成 `AVAILABLE -> SETTLEMENT`；W2 完成出款创建、提交意图、归一回单及成功关闭/失败回退；W3 只登记已确认责任和已完成追偿资金结果，不执行追偿策略或资金命令。 |
| 生产准入 | 按切片证据独立裁决 | H2、DDL 静态契约、资金红线和独立 Checker 构成仓库门禁；实际数据库迁移与隔离级别验证是宿主部署证据。观测、Runbook 和 Web 审计仍由宿主负责。 |

工程任务需要确认复用模块、写入边界、表与状态机、资金 Red、上层业务决策和处理事实引用。`CLS-GATE-001` 至 `CLS-GATE-004` 约束 wind-funds 工程落地；`CLS-GATE-005` 至 `CLS-GATE-008` 约束宿主接入、展示、权限、审计和运行治理，不反向阻塞本仓已声明能力。当前 W1 纳入本金中间户结算锁定，W2 纳入最小出款资金事实，W3 纳入最小追偿责任与结果事实；复杂净额、追偿策略执行、争议/风控权威查询和按原分配退款未来纳入时，必须重新建立对应工程门禁。
...
| 安全审计和外部规则 | 宿主安全负责人、风控负责人、合规或通道确认方。 | 复核权限、审批、职责分离、脱敏、导出审计、外部规则核验字段和确认状态。 | 影响宿主业务放行时阻断、降级或转人工；不作为 wind-funds 仓内准出证据。 |
| 生产运行 | 宿主 SRE 负责人、研发负责人、测试负责人。 | 复核容量假设、批处理窗口、重跑策略、告警、Runbook、灰度回滚和恢复验收。 | 由宿主发布制度决定是否上线，不反向改变 wind-funds H2 验收结论。 |
...
| 出款前准入门禁 | `PayoutPreflightService.checkPayoutPreflight` 只做证据预检和对象级 Gate。当前 `PayoutOrderApplicationService.submitOrder` 在锁内复核结算单与 PAYOUT Gate，并要求宿主唯一 `PayoutSubmissionAuthority` 对账户、端点、通道、审批和外部规则证据作权威准入；任一事实缺失、拒绝、非法或过期均失败关闭。提交只持久化宿主可投递意图，不调用外部通道。 | DSL-SETTLEMENT-PAYOUT-RESULT-001 的出款提交前准入；任一最终门禁缺失、失败或未知时不生成出款资金事实；W2 不启用 IN_TRANSIT。 | TDD-SETTLE-004、TDD-RED-023；宿主审批使用 HOST-OPS-001。 |
...
| 出款解释状态 | 当前 `PayoutOrderApplicationService.getOrder` 与各命令结果统一返回服务端生成的事实状态、展示状态和操作状态；列表、处理台和导出属于目标态或宿主能力。 | DSL-SETTLEMENT-PAYOUT-RESULT-001 的解释状态；非终态、门禁待补、规则待确认和金额不一致不得展示为到账成功或可自动放行。 | `PayoutOrderApplicationServiceTests`、`PayoutPublicContractTests`。 |
| 对账差错调账 | 差错单、审批、凭证、调账事实和重新对账。 | DSL-SETTLEMENT-RECONCILIATION-ADJUST-001、BALANCE_CONTROL / BALANCE_ADJUST 或批次授权 DIRECT_TRANSACTION / ADJUSTMENT。 | TDD-RECON-001、TDD-RECON-002、TDD-CTRL-ERR-005；宿主审批使用 HOST-OPS-001。 |
...
7. 当前同步原子接口限定单侧 normalized facts `<= 1000`、单次 Provider 计算的逐笔结果 `<= 2000`。Bean Validation 与服务层必须使用同一上限，并在批次查询、加锁、流水号生成和写库前校验；超限不得截断、分页后多次调用同一快照接口或留下部分事实。
...
`ReconciliationBatchState` 只描述已落地批次的工程进度：`CREATED` 已创建，`DATA_COLLECTING` 已冻结一侧来源，`DATA_READY` 两侧来源已冻结，`COMPLETED` 已绑定不可变运行结果，`ABORTED` 表示操作人停止一个尚未完成的当前作业。`ABORTED` 不是解析失败或任务重试状态，不删除已冻结来源；`COMPLETED` 不允许进入 `ABORTED`。完成态来源、解析或匹配证据被确认无效时，`replaceBatch` 创建携带 `replacementReason/replacementEvidenceRef` 的新 `CREATED` 批次，原批次继续保持 `COMPLETED`，原子推进 Gate 血缘并使原批次锚定差错进入 `INVALIDATED`。`BALANCED/DIFFERENCE_FOUND` 属于 `ReconciliationRunOutcome`；当前 `PASSED/BLOCKED` 属于 gate 决策。上层任务编排可另行表达 `WAITING_DATA`、解析失败、匹配执行失败和重试，但不得把这些任务态混入当前批次、运行结论或准入决策。`CONDITIONALLY_PASSED` 只有在审批、阈值、责任人、风险承接和到期重查契约落地后才能引入。
...
以下复杂状态机是未来运营工单目标，不是当前 `ReconciliationDifferenceState`。它需要独立的工单、分派、误报、条件放行和到期重查能力后才能启用，不得由调用方把这些状态直接写入当前差错表。
...
| `PAYOUT_RESULT` | 外部 executor 回单到达，且结果可归一。 | executor 成功且内部 Funds/Ledger/Balance 闭合时只形成一次 payout effect 并关闭 `SETTLEMENT` 或 `IN_TRANSIT`；失败按一次性回退事实或差错闭环处理；beneficiary arrival、rail finality 另行取证。 | `payoutSn`、`externalReceiptRef`、`receiptStatus`、`amount/currency`、`approvalSn` 或自动回单证据、`idempotencyKey`、`operatorRef`。 | `accepted`、`submitted`、`processing` 不得当成功；executor success 不得当到账/finality；金额不一致进入差错或挂账；覆盖 `TDD-SETTLE-002` 至 `TDD-SETTLE-005`。 |
...
| ruleVersion | 清分规则版本。 |
| admissionResult | 当前切片为 SPLIT_READY、EXCLUDED；锁定和确认状态由后续清分批次切片定义。 |
...
| state | WAITING_PERIOD、BLOCKED、READY、LOCKED、CLEARED、EXCLUDED。 |
...
当前已实现 `ClearingBatchApplicationServiceImpl`、清算批次表写入和候选 `CLEARED/BLOCKED/LOCKED` 推进。资金底座复核当前交易、退款累计、route snapshot 摘要、候选经济摘要和对象级 Gate；上述声明范围以 `tests` 模块 H2 全量集成测试完成仓内本地验收。争议/风控权威判断和按原资金分配处理部分退款不在当前能力范围，不得由现有结果外推。
...
6. 结算单状态只表达净额复核和结算资金锁定结果；`FAILED` 仅表达锁定时的明确账务拒绝，不承载 `PAYOUT_CREATED`、`PAYOUTING` 或 `PAID`，出款生命周期只属于 PayoutOrder。
7. 当前 `W1-SETTLEMENT-LOCK-001` 只接受中间户模式和已确认清算批次的 `PRINCIPAL / ADD` 金额项；账单模式、冻结模式及其他金额项失败关闭。
...
flowchart TD
    ...
    F1 --> G{"外部结果"}
    F2 --> G
    G -- "executor 终态成功" --> H["出款单 SUCCEEDED\n内部 payout effect 关闭 SETTLEMENT 或 IN_TRANSIT\n到账/finality 另行确认"]
...
9. executor `SUCCEEDED` 只表示执行结果和内部资金 effect；只有独立 bank/beneficiary 证据对平后才能声明 beneficiary arrival，rail finality 由外部 Owner 按版本化规则独立声明；Gate/BALANCED/余额不得替代二者。

### 4.10 追偿单

当前 W3 追偿单只承载上层已确认的责任事实和已完成资金结果引用。退款、拒付、费用、差错或负余额如何转成责任、采用准备金抵扣、后续结算抵扣、负余额还是人工收款，均由宿主业务决定；reconciliation 不执行这些策略，也不创建资金交易。

| 字段 | 说明 |
| --- | --- |
| sn | 追偿单号。 |
| sourceType/sourceSn/sourceDigest | 宿主已确认的责任来源、来源流水与不可变摘要。 |
| responsibleSubjectType/responsibleSubjectId | 应承担追偿结果的资金主体。 |
| expectedAmount/recoveredAmount/currency | 应追、已追金额和币种；未追金额由两者计算。 |
| state | CREATED、PARTIALLY_RECOVERED、RECOVERED。 |
| approvalRef/evidenceRef | 责任确认的宿主审批与受控证据引用。 |
| lastFundsTransactionSn/recoveredTime | 最近登记的已完成资金交易与全额追回时间。 |

`RecoveryResult` 以 append-only 方式绑定追偿单和已关闭的 `RECOVERY` 资金交易，保存金额、币种、登记幂等键、摘要、审批和证据引用。同一资金交易和同一幂等键在租户内只能登记一次；累计金额不得超过应追金额。资金交易自己的 `businessSn` 仍是宿主单次资金动作幂等键，不强制等于追偿单号，否则会阻断一张追偿单的多次部分追回。
...
| ReconciliationBatchState | 对账批次执行进度 | CREATED、DATA_COLLECTING、DATA_READY、COMPLETED、ABORTED |
...
| ReconciliationRunOutcome | 完成态运行结论 | BALANCED、DIFFERENCE_FOUND |
...
| ReconciliationDifferenceState | 当前对象级 Gate 差错状态 | BLOCKED、ADJUSTING、RECONCILING、RESOLVED、INVALIDATED |
...
| ClearingSplittableAdmissionResult | 可清分明细准入状态 | 当前为 SPLIT_READY、EXCLUDED；不表达已清分、已清算或已结算 |
| ClearingSplitBatchState | 清分批次状态 | DRAFT、REVIEWING、CONFIRMED、CANCELLED |
| ClearingCandidateState | 清算候选状态 | WAITING_PERIOD、BLOCKED、READY、LOCKED、CLEARED、EXCLUDED |
| ClearingBatchState | 清算批次状态 | DRAFT、REVIEWING、CONFIRMED、CANCELLED、FAILED；不允许部分成功 |
| SettlementOrderState | 结算单状态 | DRAFT、REVIEWING、APPROVED、LOCKED、FAILED、CANCELLED、RELEASED；FAILED 只表达锁定资金的明确账务拒绝，CANCELLED 只表达草稿/复核阶段取消，RELEASED 表达锁定资金已安全释放，出款状态不进入本枚举 |
| PayoutOrderState | 出款单状态 | CREATED、SUBMITTED、ACCEPTED、PROCESSING、SUCCEEDED、FAILED、RETURNED、MISMATCHED、CANCELLED |
| RecoveryOrderState | 追偿责任事实状态 | CREATED、PARTIALLY_RECOVERED、RECOVERED；抵扣中、催收中属于宿主流程状态，核销/取消尚未纳入 W3 |
...
| `RecordReconciliationSourceSnapshotRequest` | tenantId、reconciliationBatchSn、sourceRole、sourceNamespace、snapshotIdentity、snapshotVersion、coverage、facts（最多 1000 项）、evidenceRefs（最多 100 项） | 为 `REFERENCE` 或 `COMPARISON` 原子冻结 carrier-neutral 的不可变 normalized facts；事实身份、版本、覆盖范围和证据由来源适配器提供，集合摘要由资金底座生成；不是分段上传接口，也不接收 raw carrier payload。 |
| `RecordReconciliationRunResultRequest` | tenantId、reconciliationBatchSn | 请求 Provider 对指定批次执行一次 strict-exact；逐笔结果、运行结果和摘要全部从冻结的两侧 normalized facts 计算，不接收调用方逐笔结果，也不是逐页追加接口。 |
...
| `ReconciliationSourceSnapshotDTO` | sn、批次、角色、sourceNamespace、snapshotIdentity、snapshotVersion、coverage、facts、sourceDigest、recordCount、evidenceRefs、创建信息 | 返回一侧冻结的 carrier-neutral normalized facts 及内部摘要。 |
...
| `ReconciliationMatchResultDTO` | sn、运行结果、批次、referenceFactRef、comparisonFactRef、comparisonIdentity、resultKind、差额事实、证据、摘要、创建信息 | 表达 Provider 从冻结 facts 计算出的不可变逐笔结果；只允许通过运行结果服务分页读取，不提供独立写入、更新或删除入口。 |
...
public interface ReconciliationRunResultApplicationService {
    ...
    ReconciliationRunResultDTO executeStrictExact(
...
上述 `PayoutOrderService` 代码块仍是包含分页处理台、拆分与重试的完整目标态。当前 W2 已落地更小的公共契约 `PayoutOrderApplicationService`，覆盖从 `LOCKED + EXTERNAL_ENDPOINT` 结算单创建唯一出款单、持久化提交意图、接收宿主归一回单，以及成功关闭或失败回退资金事实。`PayoutPreflightService.checkPayoutPreflight` 仍只是调用方证据预检，不读取账户、端点、通道、额度、cutoff、名单、余额或准备金权威事实；预检通过不得直接解释为可提交。当前 `submitOrder` 在锁内重验 Settlement、PAYOUT Gate 和宿主注入的唯一 `PayoutSubmissionAuthority`，未配置、配置不唯一、拒绝、证据非法或过期均失败关闭；它不调用外部通道，`SUBMITTED` 只表示宿主可投递的意图已持久化。
...
2. `recordSourceSnapshot` 只登记已归一来源的稳定身份、版本、覆盖范围、facts 和证据，不下载文件、不解析内容、不执行匹配；同一批次同一角色事实不可变。
3. `executeStrictExact` 必须从两侧冻结 facts 验证 comparison identity、Money、comparison status、claim semantics、rule reference、成员唯一性和完整 coverage，再由 Provider 派生每项 `ReconciliationMatchResultKind`、计数与 `BALANCED/DIFFERENCE_FOUND`。
...
逐笔匹配由 Provider 从 `RecordReconciliationRunResultRequest` 指定批次的两侧冻结 facts 计算并一次性固化，不提供 caller 写入、独立写入、更新或删除的碎片化公共服务。

| 模型 | 关键字段 | 说明 |
| --- | --- | --- |
| Provider-computed match result | referenceFactRef、comparisonFactRef、comparisonIdentity、resultKind、absoluteDifferenceCurrency、absoluteDifferenceAmount、largerSide、matchIdentityDigest、resultDigest、evidenceRefs | Provider 对冻结 facts 的一次计算结果；不接受 caller 自报 `sourceQuality`、`matchStrength` 或 result kind。 |
| `MATCHED` | 两侧 facts、同 comparison identity、同币种 Money、同 comparison status/claim semantics/rule 且均已证明 | 只计入严格精确对平；不得携带差错类型或差额。 |
| 缺失或不可比较 | 单侧缺失或 comparison proof 不成立，结果为 `REFERENCE_MISSING`、`COMPARISON_MISSING` 或 `NOT_COMPARABLE` | 由 Provider 根据 facts 计算；不得用空侧、总额相等或 caller assertion 伪造对平。 |
| 其他差异 | `CURRENCY_MISMATCH`、`MONEY_MISMATCH`、`STATUS_MISMATCH`、`SEMANTICS_MISMATCH`、`RULE_MISMATCH` 或 `IDENTITY_CONFLICT` | 差额、币种和较大侧只在对应结果 kind 下由 Provider 计算；差异进入 Difference/人工路径，不自动记为 Balanced。 |
...
wind-funds 不提供 `FundsOperationAuditService`、审计 Request/Query/DTO 或审计表。Web/宿主统一审计能力应在入口层记录 `tenantId`、操作类型、对象类型与流水号、操作者、审批引用、原因、证据引用、traceId 和结果，并能按 reconciliation 对象回查。对账关闭、差错动作、清算确认、结算锁定、出款确认、敏感证据查看和导出属于宿主审计责任；该责任不由 wind-funds 重复实现，也不作为本仓代码准出证据。`t_reconciliation_difference_action` 只证明业务动作事实，不替代访问与操作审计。
...
1. returnToDraft 只允许 REVIEWING -> DRAFT，不释放来源；修正金额或来源必须先取消原单再重新创建。
2. cancelOrder 只允许 DRAFT/REVIEWING -> CANCELLED，并在同一事务内释放全部 active_source_claim 和 active_order_digest；APPROVED/LOCKED 不得人工取消。锁定时明确账务拒绝则自动进入 FAILED，保留失败资金事实并释放两类活动占用，允许原样重建新单。
3. approveOrder 在 REVIEWING -> APPROVED 时一次性写入 settlementApprovalRef；它不是策略审批引用，不参与创建幂等摘要。
4. lockOrder 触发 AVAILABLE -> SETTLEMENT，成功或明确失败都固化 reconciliationRunResultSn、resultDigest 和 evidenceDigest；LOCKED 重放只接受首次消费的同一运行结果号。
5. checkPayoutPreflight 只做解释性预检；submitOrder 必须在锁内重验结算锁定、PAYOUT Gate，并调用宿主提供的 `PayoutSubmissionAuthority` 校验账户、端点、通道、规则、审批等权威准入证据。任一事实缺失、失败、未知或过期时不得进入 SUBMITTED，也不得生成资金事实。
6. handleReceipt 必须区分外部受理、处理中、成功、失败、退回和金额不一致；W2 的 ACCEPTED/PROCESSING 不生成 `IN_TRANSIT`，成功执行 `SETTLEMENT -> PREPAYMENT -> CASH`，失败只执行一次 `SETTLEMENT -> AVAILABLE`。RETURNED/MISMATCHED 不自动动账，交由宿主采集、匹配并物化正式差错。
7. 当前 getOrder 返回服务端生成的事实状态、展示状态和操作状态；分页处理台 queryPayoutOrders 仍是目标态。前端、运营台、商户账单和审计导出不得自行按 status 推断可操作性。
8. W2 不支持失败后重试或换渠道，也不覆盖历史终态。出款成功后的退款、拒付、费用或差错由宿主确认责任后，可进入 W3 `RecoveryOrder`。
9. createOrder 只接受宿主已确认的责任主体、金额、币种、来源摘要、审批和证据；相同来源键只能重放完全一致的事实。
10. recordResult 只登记已关闭、同租户同币种、`businessScene=RECOVERY` 且包含责任主体成功明细的既有资金交易；同一交易不得跨单认领，累计金额不得超过 expectedAmount。
11. W3 不执行准备金抵扣、后续结算抵扣、负余额或人工收款，不调用交易、余额或账本命令，也不提供核销、关闭、分页处理台、来源采集匹配和运营流程。
...
| PayoutPreflightResultDTO | passed、blockingLevel、blockingReasons、manualReviewRequired、decisionResult、displayStatus、action、externalRuleVerificationResult、checkedAt、checkedBy、expiresAt、evidenceRefs | 当前结果只说明证据预检是否通过；通过时 `displayStatus=PREFLIGHT_PASSED`、`action=SUBMISSION_REVALIDATION_REQUIRED`，不得作为出款提交授权。 |
...
| SettlementPolicySnapshotDTO | policyCode、policyVersion、settlementMode、settlementDestination、triggerMode、settlementPeriod、timezone、cutoff、policyApprovalRef | W1 结算策略快照；周期与模式、去向、触发方式、时区、cutoff、规则和策略审批共同进入策略快照摘要，结算复核使用独立 settlementApprovalRef。 |
| PayoutOrderDTO | sn、tenantId、settlementOrderSn、settlementSubjectType、settlementSubjectId、amount、currency、state、displayStatus、nextAction、payoutAccountRef、payeeEndpointRef、channelRef、externalReference、Gate/准入证据摘要、成功/失败资金交易流水、最后回单摘要、脱敏失败信息和时间 | 当前 W2 单笔查询的统一解释模型；不得只返回技术 status。 |
| PayoutOrderQuery | tenantId、settlementSn、payoutSn、state、displayStatus、nextAction、manualReviewRequired、externalRuleVerificationResult、payeeSubjectRef、channelRef、amountRange、createdRange | 目标态出款处理台查询条件；当前 W2 不提供列表或分页查询。 |

外部规则证据统一使用 `verificationResult` 和 `ExternalRuleVerificationResult`。出款回单规范摘要使用 `state`，不再读取或比较旧 `status` 键；真实宿主必须在升级前确认不存在仍需幂等复用的旧摘要，或先完成离线迁移。
| CreateRecoveryOrderRequest | tenantId、sourceType/sourceSn、responsibleSubjectType/responsibleSubjectId、expectedAmount、currency、sourceDigest、approvalRef、evidenceRef | W3 已确认责任事实输入；不表达追偿策略或执行方式。 |
| RecordRecoveryResultRequest | tenantId、recoveryOrderSn、fundsTransactionSn、idempotencyKey、approvalRef、evidenceRef | W3 只引用已完成追偿资金交易并追加结果。 |
| RecoveryOrderDTO | sn、tenantId、来源、责任主体、expectedAmount、recoveredAmount、remainingAmount、currency、status、lastFundsTransactionSn、recoveredTime | W3 单笔责任与累计结果事实；不返回催收、抵扣或核销流程状态。 |

目标态 blockingReasons 至少覆盖 SETTLEMENT_NOT_LOCKED、PAYOUT_ACCOUNT_INVALID、PAYEE_ENDPOINT_INVALID、CHANNEL_UNAVAILABLE、CHANNEL_LIMIT_EXCEEDED、CUTOFF_CLOSED、WATCHLIST_BLOCKED、EXTERNAL_RULE_UNVERIFIED、NEGATIVE_BALANCE_BLOCKED、RESERVE_INSUFFICIENT、RECONCILIATION_BLOCKED、IDEMPOTENCY_CONFLICT、APPROVAL_REQUIRED。当前预检只生成引用缺失等解释原因，并使用不加锁的 `inspectGate`；通过时仍固定要求 `SUBMISSION_REVALIDATION_REQUIRED`。当前 `submitOrder` 已在事务中调用 `checkGate`，并消费宿主 `PayoutSubmissionAuthority` 的准入决策摘要、证据引用和有效期；wind-funds 不伪造账户、通道、合规或准备金权威事实。

PayoutOrderDTO 的 state、displayStatus 和 nextAction 必须与出款单状态、前置门禁、外部回单、金额一致性、规则核验和差错状态共同推导。`SUCCEEDED` 只可解释为 executor/internal payout effect；外部 accepted、submitted、message sent、processing、IN_TRANSIT、MISMATCHED、EXTERNAL_RULE_UNVERIFIED 或 MANUAL_REVIEW_REQUIRED 都不得映射为 beneficiary arrival、rail finality、可出款完成或可自动放行。
...
| SettlementOrderApplicationServiceImpl | 从已确认清算批次生成不可变金额项和策略快照，管理 DRAFT -> REVIEWING -> APPROVED -> LOCKED/FAILED，固化最终 Gate 证据，并按成功、明确失败、结果未知三类资金结果原子收敛。 | 清算批次、结算单及金额项 Mapper、对账 Gate、结算资金服务。 |
| PayoutPreflightServiceImpl | 只检查调用方提供的出款证据和对象级对账 Gate，返回非授权性质的预检结果；不管理提交、回单或资金动作。 | 对账 Gate。 |
| PayoutOrderApplicationServiceImpl | 从锁定的外部端点结算单创建唯一出款单；锁内重验 Gate 与宿主准入；固化提交意图和归一回单；以数据库唯一键裁决回单及外部 reference 归属，成功关闭或失败回退一次，退回/冲突保留复核事实。 | 结算单与出款 Mapper、对账 Gate、`PayoutSubmissionAuthority`、出款资金服务。 |
| RecoveryOrderApplicationServiceImpl | 幂等创建已确认追偿责任，并只读校验、追加登记已完成 `RECOVERY` 资金结果；不执行抵扣、负余额、人工收款或核销。 | 追偿单/结果 Mapper、交易 face 查询。 |
...
5. `reconciliation-impl` 可以依赖 `transaction-face` 完成清算、结算与出款资金编排；仓内只有对应应用服务可调用这些资金原语。`transaction` 不得依赖 reconciliation，也不得自行判断候选、账期、退款、争议、清算或出款回单规则。
...
本矩阵用来从测试用例倒推 API 是否可用。若某一行无法写出 Given/When/Then，说明 API 设计需要回炉。对账证据、差错、Gate、清分、内部清算、W1 结算锁定、W2 出款资金事实和 W3 最小追偿责任/结果引用使用当前已实现接口；尚未交付的调度、匹配、复杂结算和追偿策略/资金执行等目标态能力必须显式标注“目标态，未落地”，不得与现行契约混用。

| TDD 用例 | 触发 API | Given | When | Then |
| --- | --- | --- | --- | --- |
| TDD-RECON-007 对账批次生命周期 | createBatch、recordSourceSnapshot、executeStrictExact、abortBatch、replaceBatch、checkGate | 存在租户、准入对象、对账窗口、规则版本和来源证据引用。 | 创建批次、依次冻结两侧 normalized facts、执行 strict-exact 并检查准入；未完成作业可受控终止，完成态证据失效必须显式替代。 | 正向状态按 CREATED -> DATA_COLLECTING -> DATA_READY -> COMPLETED 推进；ABORTED 只关闭未完成作业并固化操作人、时间和原因；完成批次保持不可变，替代批次显式引用旧批次并失效旧差错；运行结论和门禁决策独立表达，同一 Gate 对象只允许一个当前血缘头。 |
| TDD-RECON-003 清分前基础对账 | createBatch、recordSourceSnapshot、executeStrictExact、checkGate、ClearingSplittableDetailApplicationService#identifySplittableDetail | 交易、单据、账本分录、余额桶和 route snapshot 均存在。 | 来源适配器先完成归一，资金底座冻结 facts、由 Provider 计算结果，并在可清分识别前检查准入。 | 完全一致时允许识别；存在阻断差异时只返回无流水号的临时排除结论，不写可清分明细、不占用来源分录唯一键，阻断解除后可重试。 |
| TDD-RECON-004 清算前置对账 | executeStrictExact、createDifference、checkGate | 清分结果快照已固化，存在清算候选输入。 | Provider 从冻结 facts 计算运行结果，差异由独立服务登记，再检查清算准入。 | 错主体、错币种、金额不平、余额公式不成立时生成差错并阻断候选或批次确认。 |
...
| TDD-RECON-006 余额桶一致 | executeStrictExact、createDifference | 提供 CLEARING、AVAILABLE、SETTLEMENT、IN_TRANSIT、FROZEN 期初、本期入出和期末。 | Provider 按冻结 normalized facts 执行严格比较，资金底座登记计算出的结果和差错。 | 公式不成立时生成 BALANCE_FORMULA 差异和 S0/S1 差错；不能登记为对平。 |
...
| TDD-SETTLE-001 结算锁定 | createOrder、submitOrder、approveOrder、lockOrder | 同租户、同主体、同币种的清算批次已确认，PRINCIPAL / ADD 金额项、策略快照和 settlementApprovalRef 完整。 | 复核并锁定结算单。 | 成功只生成一次 AVAILABLE -> SETTLEMENT 并固化 Gate 证据；明确失败进入 FAILED、释放活动占用且可原样重建；技术异常整体回滚保持 APPROVED。 |
| TDD-SETTLE-002 executor 出款结果和失败回退 | createPayoutOrder、submitPayoutOrder、handlePayoutReceipt | 结算单已锁定，存在出款单和外部 reference。 | 处理外部受理、executor 成功、失败或退回回单。 | 外部受理不关闭内部 effect；executor 成功仅在内部闭合后关闭 SETTLEMENT/IN_TRANSIT；beneficiary arrival、rail finality 另行取证；失败或退回只回退一次并保留回单摘要。 |
...
| TDD-SETTLE-006 结算模式快照 | createOrder、lockOrder | 结算策略选择中间户、冻结或账单模式。 | 生成结算单并尝试锁定。 | 当前切片只允许中间户模式；冻结和账单模式均失败关闭且不得生成 SETTLEMENT 资金事实，中间户模式必须能解释 SETTLEMENT 余额桶。 |
| TDD-SETTLE-007 结算退回、取消与失败退场 | returnToDraft、cancelOrder、lockOrder | 结算单处于 DRAFT、REVIEWING、APPROVED、LOCKED 或 FAILED。 | 尝试退回、取消或处理明确账务拒绝。 | REVIEWING 可退回 DRAFT 但不释放来源；DRAFT/REVIEWING 可取消；明确账务拒绝进入 FAILED；两者都原子释放来源与活动幂等占用，LOCKED 永不释放。 |
| W3 追偿责任与结果引用 | createOrder、recordResult、getOrder | 宿主已确认追偿责任，并已通过现有资金能力完成一笔 `RECOVERY` 资金交易。 | 创建追偿单并登记部分或全部追回结果。 | 只累计引用已关闭、同租户/币种且包含责任主体成功明细的资金交易；来源、结果幂等键和交易认领均唯一，不超额、不重复动账；责任判断和追偿执行仍由宿主负责。 |
| TDD-RECON-008 对账数据分段 | createBatch、recordSourceSnapshot、executeStrictExact | 批次来源分别为交易、账本、余额、外部流水或清结算报表。 | 来源适配器对 carrier-neutral normalized facts 生成内容摘要，资金底座冻结来源并由 Provider 生成匹配结果。 | 来源快照必须保留来源命名空间、快照身份/版本、覆盖范围、成员稳定引用、内容摘要和证据；同一引用内容变化不得复用旧快照，差错不得只用自由文本说明来源。 |
| TDD-B7-RED-008 账单模式误入 SETTLEMENT | createOrder、lockOrder | settlementMode 为账单模式。 | 尝试锁定 SETTLEMENT 或生成出款成功状态。 | 必须失败或保持账单待处理；不得生成 AVAILABLE -> SETTLEMENT 或出款完成事实。 |
| TDD-B7-RED-009 对账数据分段缺失仍关闭差错 | resolveExceptionCase | 差错缺数据分段、对账类型或来源记录。 | 尝试关闭差错。 | 必须失败；不能仅凭备注、审批意见或人工确认关闭。 |
| HOST-OPS-001 宿主高危操作审批 | Web/宿主命令入口 | 存在清算确认、结算锁定、差错核销或敏感导出动作。 | 缺审批或缺原因执行高危动作。 | Web/宿主必须失败或进入待补审批，并统一记录操作审计；wind-funds 不提供 `recordAudit` 或审计 API。 |
| TDD-OPS-006 后台直接改账红线 | 所有对账清算 API | 运营后台发起差错处理、排除、恢复、核销。 | 尝试直接修改历史分录、余额投影、交易投影或清结算金额。 | 没有任何 API 暴露直接改账入口；修正只能记录差错动作和资金事实引用。 |

模拟结论：

1. 对账任务、对账匹配、差错闭环和资金安全边界可以推导出 wind-funds TDD 用例；操作审计由 Web/宿主使用 `HOST-OPS-*` 自行验收。
...
本节按架构师系分模板描述清结算与对账表结构。对账证据、差错、Gate、清分、清算、W1 结算单/金额项、W2 出款单/回单和 W3 追偿责任/结果引用已落地 H2 测试 schema、Entity、Mapper、服务层测试和 MySQL 首次建表基线。H2 与静态 DDL 构成仓内验收；实际数据库迁移、锁和隔离级别语义由采用该数据库的宿主补证。字段命名若在实现时调整，需要回到本节和 TDD 同步校准。
...
| outcome | varchar(50) | [X] | 当前完成态记录由逐笔结果派生 `BALANCED` 或 `DIFFERENCE_FOUND`；任务等待和失败不由该记录入口创建。 |
...
| state | varchar(50) | [X] | CREATED、DATA_COLLECTING、DATA_READY、COMPLETED、ABORTED。 |
...
| idx_reconciliation_difference_state | tenant_id, state | 差错队列 |
| idx_reconciliation_difference_blocking_object | tenant_id, blocking_object_type, blocking_object_sn, state | Gate 精确阻断查询 |
...
reconciliation 不建设 `t_funds_operation_audit` 或模块内审计服务。人工处理、审批、敏感证据查看、导出、结算锁定和出款操作由 Web/宿主统一审计能力记录，并通过对象类型、对象流水号、操作者、审批引用、原因、证据引用和 traceId 关联本模块事实。对账差错动作表只承担业务动作事实链，不替代访问审计、查看审计或导出审计；宿主审计可回查由接入方自行验收，不阻塞 wind-funds 准出。
...
| admission_result | varchar(50) | [X] | 当前准入结果 | SPLIT_READY |
| exclusion_reason | varchar(64) | [ ] | 稳定排除原因枚举 | SOURCE_FACT_INCOMPLETE |
| reconciliation_decision_result | varchar(50) | [X] | 清分前对账 gate 结论 | PASSED |
...
| idx_clearing_splittable_detail_admission_result | tenant_id, admission_result | 待清分扫描 |

数据约束与目标态校验：

- 交易、交易明细或账本分录等根引用不存在时快速失败；只有根引用可读取但 route snapshot、posting plan 等关联事实不完整时，才记录 EXCLUDED 和稳定原因。
- 只有状态为 `CLOSED` 的 `DIRECT/PAY` 资金交易、`PAY` 明细和 `DIRECT_TRANSACTION/PAY` 账本交易，且来源分录为 `CLEARING + CLEARING category + CREDIT + INCREASE + DETAIL` 时才是正向待清分事实；授权完成进入 `SETTLEMENT/COMPLETION`，`OPEN` 直接支付、退款、冲正、借方或减少分录不得进入清分。
...
| state | varchar(50) | [X] | 清分批次状态 | REVIEWING |
...
| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_clearing_split_batch_scope | tenant_id, subject_type, subject_id, currency, business_line, split_period, state | 单主体批次查询 |
...
| state_changed_time | datetime(3) | [X] | 状态变更时间 | 2026-05-21 00:00:00.000 |
...
| idx_clearing_candidate_state | tenant_id, state | 按状态扫描候选 |
...
| state | varchar(50) | [X] | 清算批次状态 | CONFIRMED |
...
| 字段名称 | 字段类型 | 是否必填 | 字段说明 | 值示例 |
| --- | --- | --- | --- | --- |
| sn | varchar(64) | [X] | 结算单号，租户内唯一 | SO100001 |
| tenant_id | bigint(20) | [X] | 归属租户 | 1 |
| settlement_subject_type | varchar(50) | [X] | 结算主体类型 | MERCHANT |
| settlement_subject_id | varchar(64) | [X] | 结算主体 ID | M1001 |
| currency | varchar(10) | [X] | 币种 | CNY |
| settlement_period | varchar(30) | [X] | 结算周期 | 2026-05 |
| settlement_mode | varchar(30) | [X] | 结算模式；当前只支持 INTERMEDIARY_ACCOUNT | INTERMEDIARY_ACCOUNT |
| settlement_destination | varchar(50) | [X] | 结算去向快照 | INTERNAL_ACCOUNT |
| trigger_mode | varchar(30) | [X] | 触发方式快照 | HOST_COMMAND |
| timezone | varchar(50) | [X] | 策略时区 | Asia/Shanghai |
| cutoff | varchar(30) | [X] | cutoff 规则快照 | 23:00 |
| total_amount | bigint(20) | [X] | 总额 | 20000 |
| add_amount | bigint(20) | [X] | 加项金额；当前等于本金总额 | 20000 |
| deduct_amount | bigint(20) | [X] | 扣减金额；当前固定 0 | 0 |
| reserve_amount | bigint(20) | [X] | 准备金金额；当前固定 0 | 0 |
| net_amount | bigint(20) | [X] | 净结算金额；当前等于 add_amount | 20000 |
| state | varchar(50) | [X] | 结算单状态 | LOCKED |
| settlement_approval_ref | varchar(128) | [ ] | REVIEWING -> APPROVED 时写入的宿主结算审批引用 | AP100001 |
| lock_funds_transaction_sn | varchar(64) | [ ] | 锁定资金交易号 | FT300001 |
| rule_code | varchar(64) | [X] | 结算规则编码 | MERCHANT_SETTLEMENT |
| rule_version | varchar(64) | [X] | 结算规则版本 | v1 |
| policy_approval_ref | varchar(128) | [ ] | 策略自身审批引用，属于创建时策略快照 | POLICY_AP100001 |
| amount_digest | varchar(64) | [X] | 金额项摘要 | 64 位小写 sha256 |
| source_digest | varchar(64) | [X] | 排序后清算来源集合摘要 | 64 位小写 sha256 |
| policy_snapshot_digest | varchar(64) | [X] | 模式、去向、触发、周期、时区、cutoff、规则和 policyApprovalRef 摘要 | 64 位小写 sha256 |
| order_digest | varchar(64) | [X] | 来源、金额和策略快照组合摘要 | 64 位小写 sha256 |
| created_by | varchar(64) | [X] | 创建人 | operator:1001 |
| submitted_by / submitted_time | varchar(64) / datetime | [ ] | 提交复核人和时间 | operator:1001 / 2026-07-29 10:00:00 |
| approved_by / approved_time | varchar(64) / datetime | [ ] | 审批记录人和时间 | operator:1002 / 2026-07-29 10:05:00 |
| locked_by / locked_time | varchar(64) / datetime | [ ] | 锁定人和时间 | operator:1002 / 2026-07-29 10:06:00 |
| returned_by / returned_time / return_reason | varchar(64) / datetime / varchar(512) | [ ] | 退回审计字段 | operator:1002 / 2026-07-29 10:03:00 / 补充证据 |
| cancelled_by / cancelled_time / cancel_reason | varchar(64) / datetime / varchar(512) | [ ] | 取消审计字段 | operator:1001 / 2026-07-29 10:04:00 / 重建策略快照 |
| reconciliation_run_result_sn / reconciliation_result_digest / reconciliation_evidence_digest | varchar(64) / varchar(64) / varchar(64) | [ ] | 锁定成功或明确失败时固化的 Gate 运行结果、结果摘要和证据引用摘要 | RRR100001 / 64 位小写 sha256 |
| active_order_digest | varchar(64) | [ ] | 活动创建幂等占用；CANCELLED/FAILED 置 NULL | 64 位小写 sha256 |
| failed_by / failed_time / failure_reason | varchar(64) / datetime / varchar(512) | [ ] | 明确账务拒绝审计字段 | operator:1002 / 2026-07-29 10:06:00 / 账本余额不足 |
| version | int(11) | [X] | 乐观锁版本 | 1 |

状态说明：

- DRAFT：草稿；REVIEWING：复核中；APPROVED：已审批；LOCKED：资金已锁定；FAILED：锁定被账本明确拒绝且无账本事实；CANCELLED：DRAFT 或 REVIEWING 已取消。出款状态仍由 PayoutOrder 表达。

唯一索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| uk_settlement_order_sn | tenant_id, sn | 结算单号租户内唯一 |
| uk_settlement_order_active_digest | tenant_id, active_order_digest | 有效结算单创建幂等；CANCELLED/FAILED 释放后可原样重建 |

普通索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_settlement_order_subject | tenant_id, settlement_subject_type, settlement_subject_id, settlement_period | 主体结算查询 |
| idx_settlement_order_state | tenant_id, state, gmt_modified | 结算状态扫描 |
| idx_settlement_order_lock_transaction | tenant_id, lock_funds_transaction_sn | 锁定交易追溯 |
...
- 锁定必须固化本次 Gate 运行结果和证据摘要；LOCKED 幂等重放携带不同运行结果号时失败关闭。
- 明确账务拒绝必须先确认资金交易为 FAILED，再原子释放 active_order_digest 和 active_source_claim；技术异常或结果未知整体回滚并保持 APPROVED。
- `settlement_mode + settlement_destination + trigger_mode + rule_code/version + settlement_period + timezone + cutoff + policy_approval_ref` 构成创建时不可变策略快照输入；摘要不替代原始字段。`settlement_approval_ref` 在 REVIEWING -> APPROVED 时一次性写入并保持不可变，不参与创建幂等摘要。
- 当前切片仅允许 `INTERMEDIARY_ACCOUNT` 进入锁定；账单模式和冻结模式失败关闭。
...
| 字段名称 | 字段类型 | 是否必填 | 字段说明 | 值示例 |
| --- | --- | --- | --- | --- |
| sn | varchar(64) | [X] | 金额项号，租户内唯一 | SOI100001 |
| tenant_id | bigint(20) | [X] | 归属租户 | 1 |
| settlement_order_sn | varchar(64) | [X] | 结算单号 | SO100001 |
| item_type | varchar(30) | [X] | 金额项类型；当前固定 PRINCIPAL | PRINCIPAL |
| direction | varchar(20) | [X] | 金额方向；当前固定 ADD | ADD |
| amount | bigint(20) | [X] | 金额，最小货币单位，必须大于 0 | 1000 |
| currency | varchar(10) | [X] | 币种 | CNY |
| source_type | varchar(30) | [X] | 来源类型；当前固定 CLEARING_BATCH | CLEARING_BATCH |
| source_sn | varchar(64) | [X] | 来源流水 | CB100001 |
| source_amount_digest | varchar(64) | [X] | 创建时清算批次金额摘要 | 64 位小写 sha256 |
| active_source_claim | tinyint(1) | [ ] | 有效来源占用标记；有效时固定为 1，释放后为 NULL | 1 |
| created_by | varchar(64) | [X] | 创建人 | operator:1001 |

状态说明：

- 当前切片 direction 固定使用 ADD；金额必须大于 0。

唯一索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| uk_settlement_order_item_sn | tenant_id, sn | 金额项号租户内唯一 |
| uk_settlement_order_item_source | tenant_id, settlement_order_sn, source_type, source_sn | 同结算单同来源唯一 |
| uk_settlement_item_active_source | tenant_id, source_type, source_sn, active_source_claim | 防止同一清算来源被不同有效结算单重复占用 |

普通索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_settlement_order_item_order | tenant_id, settlement_order_sn | 查询结算单金额项 |

数据约束与目标态校验：

- 当前结算净额只由已确认清算本金构成；费用、退款、争议、准备金、追偿、汇差和调账项待各自权威来源契约就绪后扩展。
- 金额项只解释净额，不直接触发出款或账本分录。
- 金额项业务字段不更新；修正通过追加调整项表达。`active_source_claim` 只在允许取消时释放，不改变金额事实。
- 当前切片只接受 `PRINCIPAL / ADD / CLEARING_BATCH`；创建时活动来源占用为 1。DRAFT/REVIEWING 取消或 APPROVED 锁定明确失败时在同一事务内置 NULL，LOCKED 永不释放。
- `order_digest` 保留不可变创建事实，`active_order_digest` 负责有效请求幂等，活动来源唯一键负责跨结算单业务唯一；服务校验不能替代数据库约束。
...
| state | varchar(50) | [X] | 出款状态 | ACCEPTED |
...
- CREATED：已创建；SUBMITTED：已提交；ACCEPTED：外部已受理；PROCESSING：外部处理中；SUCCEEDED：终态成功；FAILED：终态失败；RETURNED：终态退回；MISMATCHED：终态金额不一致或待差错处理；CANCELLED：尚未提交外部出款前取消。
...
| idx_payout_order_state | state | 出款状态扫描 |
...
业务用途：承载宿主已经确认的追偿责任及累计结果，不表达追偿策略或执行流程。
...
| responsible_subject_type | varchar(50) | [X] | 责任资金主体类型 | FUNDING_ACCOUNT |
| responsible_subject_id | varchar(64) | [X] | 责任资金主体 ID | FA1001 |
| expected_amount | bigint(20) | [X] | 应追金额，最小货币单位，大于 0 | 1000 |
| recovered_amount | bigint(20) | [X] | 已追金额，最小货币单位，非负 | 200 |
| currency | varchar(10) | [X] | 币种 | CNY |
| state | varchar(30) | [X] | CREATED、PARTIALLY_RECOVERED、RECOVERED | PARTIALLY_RECOVERED |
| source_digest/order_digest | varchar(64) | [X] | 来源与创建事实 SHA-256 | 64 位十六进制摘要 |
| approval_ref | varchar(128) | [X] | 宿主责任确认审批引用 | AP100001 |
| evidence_ref | varchar(256) | [X] | 宿主责任确认受控证据引用 | EV100001 |
| last_funds_transaction_sn | varchar(64) | [ ] | 最近登记的已完成资金交易 | FT500001 |
| created_by/recovered_time | varchar(64)/datetime | [X]/[ ] | 创建人和全额追回时间 | OP100001 |
| version | int(11) | [X] | 乐观锁版本 | 1 |

状态说明：

- CREATED：责任已登记且尚无结果；PARTIALLY_RECOVERED：累计结果小于应追金额；RECOVERED：累计结果等于应追金额。抵扣中、催收中属于宿主流程状态，核销和取消不在 W3。

唯一索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| uk_recovery_order_sn | tenant_id, sn | 租户内追偿单号唯一 |
| uk_recovery_order_source | tenant_id, source_type, source_sn, responsible_subject_type, responsible_subject_id, currency | 同来源、责任主体和币种只生成一个追偿责任 |

普通索引：

| 索引名称 | 索引字段 | 索引用途 |
| --- | --- | --- |
| idx_recovery_order_subject_state | tenant_id, responsible_subject_type, responsible_subject_id, state, gmt_modified | 责任主体状态扫描 |
| idx_recovery_order_last_transaction | tenant_id, last_funds_transaction_sn | 最近资金结果追溯 |

##### 追偿结果表

表名：t_recovery_result

业务用途：append-only 绑定追偿单和宿主已经完成的资金交易；登记动作本身不动账。

| 字段名称 | 字段类型 | 是否必填 | 字段说明 | 值示例 |
| --- | --- | --- | --- | --- |
| sn/tenant_id | varchar(64)/bigint(20) | [X] | 结果流水与租户 | RR100001 / 1 |
| recovery_order_sn | varchar(64) | [X] | 追偿单流水 | RO100001 |
| funds_transaction_sn | varchar(64) | [X] | 已关闭的 RECOVERY 资金交易 | FT500001 |
| amount/currency | bigint(20)/varchar(10) | [X] | 从资金交易读取的本次金额币种 | 200 / CNY |
| idempotency_key/result_digest | varchar(128)/varchar(64) | [X] | 登记幂等键与请求摘要 | RECOVERY:RO100001:1 |
| approval_ref/evidence_ref | varchar(128)/varchar(256) | [X] | 结果确认审批与受控证据引用 | AP200001 / EV200001 |
| recorded_by | varchar(64) | [X] | 登记人 | OP100001 |

唯一索引：`uk_recovery_result_sn(tenant_id, sn)`、`uk_recovery_result_transaction(tenant_id, funds_transaction_sn)`、`uk_recovery_result_idempotency(tenant_id, idempotency_key)`；普通索引：`idx_recovery_result_order(tenant_id, recovery_order_sn, id)`。

数据约束与当前校验：

- 创建追偿单和登记结果都不回滚历史出款，也不直接发起资金交易。
- 结果交易必须 CLOSED、同租户同币种、`businessScene=RECOVERY`，且包含责任主体、全额对应本次交易金额的成功明细。
- recovered_amount 不得大于 expected_amount；追偿单行锁、version 和结果唯一键共同约束并发。
- 同一资金交易不得被第二张追偿单认领；同一幂等键重放必须与首次请求摘要一致。
...
| 待处理对象发现 | 当前应用服务支持按状态和时间上界分页查询清分批次、清算候选与清算批次，固定返回只读 DTO；生产 DDL 已为批次状态+账龄、候选状态+可清算时间、候选状态+状态变更时间及所属批次回查提供复合索引。生产宿主应固定本轮时间上界后按小页扫描并保存自己的执行检查点，不得把查询结果当作确认或释放授权；峰值等量数据的 `EXPLAIN` 与扫描行数评估由宿主按自身数据库和流量完成，不作为 wind-funds 准出门禁。 |
| 批次确认 | 批次明细按摘要锁定，确认过程可重试但不得重复入账。 |
| 当前来源与结果封版 | 单侧来源最多 1000 项、单次匹配结果最多 2000 项；超限在批次查询和加锁前失败，H2 边界用例作为仓内本地边界验收。该限制不能解释为吞吐承诺；目标数据库压测由宿主按自身流量决定。 |
...
`tests` 模块 H2 全量集成测试、生产 DDL 静态契约和 schema 对齐作为 wind-funds 仓内验收。H2 不等价证明任一宿主数据库的锁和隔离级别语义；这类实际环境证据由接入方按自身数据库补证，不阻断仓库级 `REPOSITORY_VERIFIED`。下表用于区分仓内验收和宿主接入责任。

| 能力项 | 当前结论 | 验收或责任边界 |
| --- | --- | --- |
| 数据库与 DDL | `REPOSITORY_VERIFIED`（已声明范围） | H2 全量集成测试、生产 DDL 静态契约、测试 schema 对齐和命令保护构成仓内验收；采用 MySQL 的宿主可运行 `ReconciliationMysqlMigrationIntegrationTests` 与 `just test-mysql-reconciliation` 作为部署兼容证据。 |
| 数据库并发 | `REPOSITORY_VERIFIED`（仓内语义） | H2 双事务用例验证仓内竞争与失败关闭；宿主必须在实际数据库和隔离级别下验证唯一键失败后的 current read、容量和锁行为。 |
| 可信来源接入 | 宿主责任 | 上层适配器负责来源验签、授权、去重、归一化和原文保管；reconciliation 只冻结稳定引用与摘要。 |
| 最终命令复核 | `REPOSITORY_LOCAL_VERIFIED`（已声明范围） | 清分、内部清算、W1 结算锁定和 W2 出款提交均在自身事务内复核当前 Gate 与对应权威事实；W3 只登记宿主已确认责任与已完成资金交易引用。出款提交不调用外部通道；争议/风控权威查询、复杂结算、具体通道执行和追偿策略/资金执行不在当前能力范围。 |
| 观测和恢复 | 宿主责任 | wind-funds 提供状态/时间分页发现入口；指标、告警、检查点和 Runbook 由宿主实现。 |
| 操作审计与数据治理 | 宿主责任 | Web/宿主负责操作审计；接入方负责证据保留、归档、脱敏、访问审批和销毁。 |
...
| 长时间锁定候选 | `queryCandidates(LOCKED, stateChangedTimeMax=cutoff)`，可用 `lockedClearingBatchSn` 收敛范围 | 回查所属清算批次和资金事实；只有所属批次确定撤回且无资金事实时调用受控释放。 | 按超时自动释放。 | 候选与批次状态一致，不出现重复清算或资金事实悬空。 |
...
| 契约 ID | 宿主责任 | 建议验证 | 不可接受的接入方式 |
| --- | --- | --- | --- |
| `REC-HOST-SOURCE-001` | 可信来源适配器必须在调用 reconciliation 前完成来源授权、验签、去重、规范化和敏感原文保管；输出稳定 `sourceItemRef`、规范化版本、64 位小写 SHA-256 `contentDigest` 与受控 `evidenceRef`。 | 适配器接口/配置版本、IAM 授权清单、验签与重复输入测试报告、同一原文重放摘要一致性报告、证据引用受控回读记录。 | 未验签或未授权来源可写入、摘要由调用方任意自报、原文直接写入普通查询表、同一事实重放摘要漂移。 |
| `REC-HOST-OBS-001` | 以固定时间上界和小页检查点调用清分批次、清算候选、清算批次发现接口；产出各状态数量、最老账龄、扫描耗时/失败数、未知结果数和重复命令拦截数，并为每个告警绑定 Owner、阈值和 Runbook URI。 | 仪表盘截图或导出、告警规则版本、测试告警通知、查询检查点样本、峰值数据 `EXPLAIN` 与慢查询报告。 | 告警无 Owner/阈值/处置入口，直接扫表绕过租户服务，发现查询全表扫描，结果未知被自动释放。 |
| `REC-HOST-RUNBOOK-001` | 覆盖批次长期 `REVIEWING`、候选长期 `LOCKED`、`BLOCKED/FAILED`、MySQL 故障、迁移中断、备份恢复和重复命令；每条流程明确事实回查、允许动作、禁止动作、升级路径和恢复验收。 | 版本化 Runbook、演练工单与时间线、故障注入或桌面演练记录、恢复后交易/账本/批次一致性报告。 | 仅按超时改状态或释放候选，未知资金结果继续处理，没有升级 Owner 或恢复验收。 |
| `REC-HOST-AUDIT-001` | Web/宿主入口统一记录清分/清算提交、退回、确认、取消、候选排除/恢复/释放、差错动作和敏感证据访问；审计包含租户、对象引用、操作者、审批引用、原因、证据引用、traceId、结果和失败原因。 | 按任一 reconciliation 对象反查完整操作链、权限与审批测试、审计写入失败演练、脱敏导出样本。 | 高危命令不可回查、审计失败被静默吞掉、资金模块另建不一致审计事实、普通查询暴露敏感原文。 |
| `REC-HOST-GOV-001` | 为表内事实和外部证据引用登记数据分级、保留期、归档、删除、法务保全、加密、访问审批、备份恢复和销毁责任；资金事实保留与证据原文保留分别治理，删除证据原文不能改写历史摘要。 | 数据目录与责任人、保留策略版本、归档/法务保全/删除演练、密钥与访问复核、备份恢复报告、到期销毁记录。 | 未定义保留和法务保全规则，删除或归档破坏事实引用，普通运营账号可读取原文，备份无法恢复或无法证明销毁。 |

五项契约用于说明 wind-funds 之外的接入责任：`SOURCE` 负责外部数据可信接入，`OBS` 负责监控告警，`RUNBOOK` 负责故障处置，`AUDIT` 负责 Web/宿主操作审计，`GOV` 负责数据治理。实际环境证据由接入方按自身发布制度管理，不属于 wind-funds 仓库级交付物，也不阻断 `REPOSITORY_VERIFIED`；缺失时必须阻断对应 `HOST_INTEGRATION_VERIFIED` 或 `PRODUCTION_VERIFIED` 结论。
...
目标态表结构覆盖对账、清分、清算、结算、出款和追偿。工程按依赖切片推进；当前对账证据至 W3 最小追偿资金事实已逐步落地，更完整的追偿策略和其他目标态能力仍须独立门禁。
...
| W1 本金结算锁定 | `t_settlement_order` + `t_settlement_order_item`；当前已落地生产 DDL、H2 schema、Entity/Mapper 和应用服务。 | 已确认清算来源、同租户/主体/币种、策略周期摘要、活动幂等与来源唯一占用、DRAFT -> REVIEWING -> APPROVED -> LOCKED/FAILED、Gate 证据固化和一次 `AVAILABLE -> SETTLEMENT`。 | 不执行复杂净额、冻结/账单模式、外部出款、追偿、调度、宿主审批流或运营台。 |
| W2 最小出款资金事实 | `t_payout_order` + `t_payout_receipt`；当前已落地生产 DDL、H2 schema、Entity/Mapper、应用服务和资金原语。 | 锁定外部端点结算单、提交时 Gate/宿主权威准入、外部回单与 reference 唯一归属、非终态不动账、executor 成功且内部闭合时一次性 `SETTLEMENT -> PREPAYMENT -> CASH`、失败 `SETTLEMENT -> AVAILABLE`、RETURNED/MISMATCHED 不自动动账，以及跨单/并发冲突失败关闭；beneficiary arrival、rail finality 与 reconciliation 分开。 | 不调用具体通道，不启用 IN_TRANSIT，不做 FAILED 重试、拆分出款、来源采集/匹配、正式差错物化或追偿；实际数据库并发兼容由宿主验证。 |
| W3 最小追偿资金事实 | `t_recovery_order` + `t_recovery_result`；当前已落地生产 DDL、H2 schema、Entity/Mapper、应用服务和 H2 并发红线；MySQL 兼容专项资产已提供。 | 已确认责任创建幂等、来源漂移拒绝、CREATED -> PARTIALLY_RECOVERED -> RECOVERED、只引用已关闭 RECOVERY 资金交易、责任主体/币种/金额校验、结果 append-only、同交易唯一认领和不超额。 | 不判断退款/拒付责任，不执行准备金、结算抵扣、负余额或人工收款，不创建资金交易，不做核销/关闭、来源采集匹配、运营台、IAM、审计、监控或 Runbook；实际数据库迁移和并发兼容由宿主验证。 |
| 操作审计 | Web/宿主统一审计；不在 reconciliation 建表。 | 操作者、审批号、原因、证据查看、导出和告警可通过对象引用回查。 | 不重复建设模块审计服务。 |

后续按原分配退款、争议/风控准入端口、复杂结算金额项和追偿核销/策略协作逐步扩展。每个扩展切片都必须重新声明新增表、唯一键、幂等键、状态迁移、H2 schema、目标 Red 和回归范围。
