...
| 当前编码准备基线 | 最新已提交设计和任务对齐输入以确认时 Git HEAD 为准；`270122e docs: 刷新 CAD 准入基线` 是上一完整 CAD 验证证据提交；`77bc9f4 fix: 阻断钱包上下文权益核心事实` 保留为上一冻结代码能力基线，当前准入复核输入包含 `98ec7cc..81a7ecb` 的代码、测试和文档收敛，以及 B4-TRX-EXPIRE / B4-FORCE-SETTLE / B4-NO-AUTH-REFUND 后续提交。`9456ab6 docs: 对齐 A0 准入与代码基线` 仍作为上一 A0/code baseline 冻结点；`4a7ef12 docs: 固化代码准入 CR 基线` 和 `f99800b docs: 对齐代码 CR 任务基线` 保留为历史准入证据。2026-05-31 的资金责任解析、支付工具投影、授权支付工具应用入口、钱包 DDD 应用层、资金责任目标字段、BudgetGroup/Spend Rule 控制上下文、核心设计骨架、代码 CR 差异、B2/B4 准入口径和钱包入口收敛已进入新的设计/任务对齐输入。 | 以 `30b1a00` 设计基线、`eabd672` 编码准备包、`6be9c99` B1-10 契约承载、`5bad4b7` 权益资金流编码基线、`42100dc` 权益生产准入门禁、`555a943` 至 `db766fa` 的工程约规、测试门禁、治理重放门禁、CAD 完整验证门禁、出款准入规则核验证据，`f386ed0a` 至 `5901265` 的编码准入设计冻结、出款准入设计冻结、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要和治理重放范围/来源/差异项校验，`5901265..77bc9f4` 的资金事实红线、敏感上下文阻断、上下文不可变、防御性拷贝、外部账户原文阻断、MVP/归档边界和设计交付口径收敛，`9456ab6` 的 A0 准入与代码基线对齐，`3ef522c`、`a7d3fc9` 的授权支付工具入口和钱包应用层 CR 结论，`8e4a801` 的最新设计任务基线索引，`5a78f02` 的核心设计骨架修复，`f99800b` 的代码 CR 任务基线，`4a7ef12` 的代码准入 CR 基线，`98ec7cc..81a7ecb` 的后续资金红线、支付工具约束和文档准入口径收敛，`270122e` 的 CAD 准入刷新和完整门禁通过证据，`b0666ba` / `f99f3a3` 的授权过期释放证据，`616dac1` / `3825466` 的授权强制完成首轮能力与策略红线证据，`006bcaa` 的无授权退款 canonical 能力证据作为当前准备基线输入；当前设计基线不得自动等同为授权支付工具应用入口代码完成、钱包 application facade 代码完成、资金责任目标字段迁移完成、预算组账务主体化兼容缺口关闭、含权益生产链路、清结算对账、资金数据治理、指标治理或任一未授权 MVP 编码 Done 证据。 |
...
| 本次设计变更提交范围 | 本次提交只刷新 docs、OpenSpec/Harness 的 CAD Round 0 准入基线、授权卡引用和任务计划口径；不包含生产代码、测试代码、DDL/H2 schema 或运行时配置写入。 | 未纳入本次提交或未在 Execution Grant 中显式纳入的变更只能作为草稿参考，不能作为冻结编码基线、生产 Done 证据或可写范围授权；Execution Grant 必须列出允许读取和允许写入的未提交文件。 |
...
| 代码能力基线 | 截至 `006bcaa`，B1-10 权益快照 DSL 契约、契约测试、JSON 夹具、请求摘要、稳定摘要、账务计划装配器长 ID、预算组默认周期 `LIFETIME`、账务事实断言、钱包边界、交易稳定摘要、治理投影重放边界、出款前准入候选实现、CAD 完整验证门禁、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要、治理重放差异校验、资金事实红线、敏感上下文阻断、上下文不可变性、失败无副作用、费用幂等、外部账户主体阻断、支付工具绑定对象约束、钱包入口文档口径、授权过期释放、授权强制完成和无授权退款首轮 canonical 能力已纳入局部基线；`270122e` 进一步刷新 CAD 准入引用并完成完整门禁复核。 | 共享承载已有 DSL 契约测试和 B1-10 权益快照契约承载基线，基础事实已有支付工具、绑定历史、资金责任解析关系（历史表名、服务名或字段仍可能使用 funding relation 命名）、账务计划装配器、预算组默认周期 `LIFETIME`、余额投影、上下文敏感字段阻断、支付工具绑定对象约束和主链路事实红线局部基线；交易与读模型已有直接交易、授权过期释放、授权强制完成、无授权退款、局部边界测试、解释摘要和重放差异校验；出款前准入候选实现只作为清结算与对账差距复核输入，清结算与对账、资金数据治理、拒付/争议增强、授权并发竞争和支付工具授权应用入口仍是独立待落地域。 |
| B4-DISPUTE 当前恢复入口 | 2026-06-03 只读复核确认：代码已有 `chargeback` 独立入口、`CHARGEBACK` event 和授权交易 flow 回归测试，但目标设计仍要求拒付/争议默认可由 `settleRefund` 携带原因、凭证、外部引用和审计上下文承接，不强制独立 `chargeback` 服务入口作为目标态主入口。`B4-DISPUTE-SEMANTIC-ALIGNMENT` 已形成可确认但未授权的单一 Grant 候选包。 | 当前只允许作为 `B4-DISPUTE-SEMANTIC-ALIGNMENT` 确认输入；若要编码，必须由用户确认该 Execution Grant，并沿首批 Red 验证争议退款与普通授权链退款、NO_AUTH 退款、授权拒绝的查询、投影、审计和幂等摘要可区分性。 |
...
| 2026-06-02/03 B4 授权后继能力代码基线 | B4-TRX-EXPIRE 已由 `b0666ba` 闭合；B4-FORCE-SETTLE 首轮账户主体型 canonical 能力已由 `616dac1` 闭合，策略红线已由 `3825466` 加固；B4-NO-AUTH-REFUND 已闭合并经 CR 收缩为资金层最小契约。普通完成与 FORCE 分支已切开；无授权退款以空原授权流水进入 no-auth 语义，通过 `externalReferenceSn`、退款原因和审计摘要形成独立退款事实，不构造内部 `AUTHORIZATION` reference，不查询原授权账本交易。B4-DISPUTE-CHARGEBACK 已完成只读语义裁决，并沉淀为 `B4-DISPUTE-SEMANTIC-ALIGNMENT` Grant 候选：现有 `chargeback` 能力可作为实现资产和兼容资产，首轮默认由 `settleRefund` 承接目标态争议语义。 | B4-FORCE-SETTLE 和 B4-NO-AUTH-REFUND 只作为回归基线；B4-DISPUTE-SEMANTIC-ALIGNMENT 当前可进入用户确认态但不开放默认编码准入；B4-AUTH-RACE、授权支付工具 application facade、授权占券和权益生命周期、VCC、Spend Rule、清结算对账和治理 apply 仍不开放默认编码准入。 |
...
| 本轮代码写入 | 不授权新切片。 | 当前任务只允许同步 `006bcaa` 之后的设计和任务账本闭环状态；生产代码、测试代码、DDL/H2 schema、运行时配置均不纳入本轮写入范围。 |

首轮编码推荐路径：先做 A0 基线核验，确认钱包账户、支付工具、账本、账目、余额投影和现有测试资产可承接，再由用户按 A1 至 A4 中的单一 MVP 任务切片确认 Execution Grant。B1 至 B8 只作为覆盖索引和历史任务编号，不作为新的默认开工授权；B7/B8 仍需独立 TDD 分析和 Execution Grant。A0 标准输出页见 `docs/TDD设计/A0-编码准入基线核验.md`，后续 Execution Grant 应引用该页的 `authorityBaseline`、`codeBaseline`、`targetAssets`、`schemaNeed`、`firstRedCandidateSet` 和 `nextExecutionGrantRequest`。截至 2026-06-03，B4-TRX-EXPIRE、B4-FORCE-SETTLE 和 B4-NO-AUTH-REFUND 已进入代码基线；下一轮 GSD-CAD 不再默认恢复 B4-NO-AUTH-REFUND，必须重新选择并确认新的单一 Execution Grant。
