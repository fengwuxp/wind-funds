...
| 2026-06-02/03 B4 授权后继能力代码基线 | B4-TRX-EXPIRE 已由 `b0666ba` 闭合；B4-FORCE-SETTLE 首轮账户主体型 canonical 能力已由 `616dac1` 闭合，策略红线已由 `3825466` 加固；B4-NO-AUTH-REFUND 已由 `006bcaa` 闭合。普通完成与 FORCE 分支已切开；无授权退款通过 `NO_AUTH` 模式、外部原事实和审计摘要形成独立退款事实，不构造内部 `AUTHORIZATION` reference，不查询原授权账本交易。B4-DISPUTE-CHARGEBACK 已完成只读语义裁决：现有 `chargeback` 能力可作为实现资产和兼容资产，但目标态主入口仍需在 `settleRefund` 争议承接和独立 `chargeback` 之间明确。 | B4-FORCE-SETTLE 和 B4-NO-AUTH-REFUND 只作为回归基线；B4-DISPUTE-CHARGEBACK 当前只开放 R0 语义裁决，不开放默认编码准入；B4-AUTH-RACE、授权支付工具 application facade、授权占券和权益生命周期、VCC、Spend Rule、清结算对账和治理 apply 仍不开放默认编码准入。 |
