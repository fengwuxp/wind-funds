...
| 2026-06-04 B4-AUTH-PI Grant 候选基线刷新 | `docs/TDD设计/B2B4-支付工具与SpendRule生产可用性Round0准入卡.md#82-authinstrumentgrantcandidate2026-06-03` 已补齐 `B4-AUTH-PI-CAD-001` 可确认包，并已把确认基线刷新到包含 `88d80c7` 索引基线及后续基线校准提交的 Git HEAD；候选包列明 Owner、authority baseline、写入范围、只读范围、首批 Red、验证命令、Git 策略、禁止事项和停止条件。 | 该包仍只是 `READY_TO_CONFIRM_NOT_CODE_AUTHORIZED`，不等于用户已确认 Execution Grant。后续只有用户明确确认 `Execution Grant：B4-AUTH-PI` 后，才允许写授权 application facade Red 和最小实现；否则继续保持 Round 0 / summary_only。 |
