...
| 当前编码准备基线 | 代码能力基线截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`；本轮文档与任务计划提交后，才作为新的设计和任务冻结基线。 | 以 `30b1a00` 设计基线、`eabd672` 编码准备包、`6be9c99` B1-10 契约承载、`5bad4b7` 权益资金流编码基线、`42100dc` 权益生产准入门禁、`555a943` 至 `db766fa` 的工程约规、测试门禁、治理重放门禁、CAD 完整验证门禁、出款准入规则核验证据，`f386ed0a` 至 `5901265` 的编码准入设计冻结、出款准入设计冻结、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要和治理重放范围/来源/差异项校验，以及 `5901265..77bc9f4` 的资金事实红线、敏感上下文阻断、上下文不可变、防御性拷贝、外部账户原文阻断、MVP/归档边界和设计交付口径收敛作为当前准备基线；不得自动等同为含权益生产链路、清结算对账、归档治理或指标治理 Done 证据。 |
...
| 项 | 结论 | 执行口径 |
| --- | --- | --- |
| 设计基线 | 可冻结。 | 本轮文档、OpenSpec 和 Harness Plan 对齐后应先形成提交点；未提交前不得作为 MVP 编码任务的基线提交点。 |
