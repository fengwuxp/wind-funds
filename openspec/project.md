...
| 当前编码准备基线 | `9456ab6 docs: 对齐 A0 准入与代码基线` 已冻结为当前 PRD、DSL、系分、TDD、OpenSpec 和 Harness 任务基线；代码能力基线仍截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`。 | 以 `30b1a00` 设计基线、`eabd672` 编码准备包、`6be9c99` B1-10 契约承载、`5bad4b7` 权益资金流编码基线、`42100dc` 权益生产准入门禁、`555a943` 至 `db766fa` 的工程约规、测试门禁、治理重放门禁、CAD 完整验证门禁、出款准入规则核验证据，`f386ed0a` 至 `5901265` 的编码准入设计冻结、出款准入设计冻结、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要和治理重放范围/来源/差异项校验，`5901265..77bc9f4` 的资金事实红线、敏感上下文阻断、上下文不可变、防御性拷贝、外部账户原文阻断、MVP/归档边界和设计交付口径收敛，以及 `9456ab6` 的 A0 准入与代码基线对齐作为当前准备基线；不得自动等同为含权益生产链路、清结算对账、归档治理或指标治理 Done 证据。 |
| 当前 A0/A1 准入证据 | `4f0a43e docs: 记录 A0 只读验证证据`、`26144cc docs: 补齐 A1 直接交易准入卡`、`4dbad21 docs: 记录 CAD 完整验证证据`、`52f116f docs: 记录 A1 现有覆盖扫描`。 | 这些提交只作为 A1 直接交易事实红线 Execution Grant 的确认输入；其中 `52f116f` 记录既有测试覆盖扫描和目标测试通过证据；未获用户确认前，不授权生产代码、测试代码、DDL/H2 schema 或运行时配置写入。 |
| 未冻结设计变更 | 当前无未冻结 docs、OpenSpec 或 Harness Plan 变更。 | 若后续工作树出现未提交变更，提交或在 Execution Grant 中显式纳入前只作为评审输入，不能作为冻结编码基线、生产 Done 证据或可写范围授权。 |
...
| 代码能力基线 | 截至 `77bc9f4`，B1-10 权益快照 DSL 契约、契约测试、JSON 夹具、请求摘要、稳定摘要、账务计划装配器长 ID、预算组默认周期 `LIFETIME`、账务事实断言、钱包边界、交易稳定摘要、治理投影重放边界、出款前准入候选实现、CAD 完整验证门禁、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要、治理重放差异校验、资金事实红线、敏感上下文阻断和上下文不可变性已纳入当前局部基线。 | 共享承载已有 DSL 契约测试和 B1-10 权益快照契约承载基线，基础事实已有支付工具、绑定历史、资金来源关系、账务计划装配器、预算组默认周期 `LIFETIME`、余额投影、上下文敏感字段阻断和主链路事实红线局部基线，交易与读模型仍只具备局部流程、边界测试、解释摘要和重放差异校验；出款前准入候选实现只作为清结算与对账差距复核输入，清结算与对账、归档重放与指标治理仍是独立待落地域。 |
...
| 项 | 结论 | 执行口径 |
| --- | --- | --- |
| 设计基线 | 已冻结。 | `9456ab6 docs: 对齐 A0 准入与代码基线` 是后续 MVP 编码任务的当前设计和任务基线提交点；若后续再产生未提交设计变更，必须先提交或显式纳入 Execution Grant。 |
...
7. 当前设计基线、OpenSpec 基线、Harness Plan 和旧测试清理结果已作为独立检查点冻结；若存在本轮未提交的设计、OpenSpec 或 Harness Plan 变更，必须先提交或在 Execution Grant 中显式纳入本任务基线附件，未冻结前不得把后续编码实现混入同一轮交付。
...
17. 若本任务涉及 VCC、全球账户或收单业务能力包，Execution Grant 必须声明业务分册、业务验收 ID、场景 pack 范围、归一资金动作、允许新增或修改的公共契约、外部引用脱敏字段、外部规则核验状态、P0/P1 回归测试、业务专项 TDD 用例和 Not Done 红线；未声明时只能做产品、DSL、系分或 contract-only 验证，不得进入生产资金流 Done 结论。
...
| 9. 运营和治理高危门禁是否显式 | 清结算对账触发资金事实时是否列出运营补事实命令白名单；治理任务是否确认 `GOV-GATE-*`、物理落点、依赖方向、公共契约、DDL/H2、Mapper/Entity、边界测试、指标水位隔离测试和大数据归档承接边界。 | 补 Execution Grant 或缩小到只读/contract-only 范围。 |
