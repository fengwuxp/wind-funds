...
| 当前编码准备基线 | 最新已提交设计和任务对齐输入以确认时 Git HEAD 为准；`270122e docs: 刷新 CAD 准入基线`、`9456ab6 docs: 对齐 A0 准入与代码基线`、`4a7ef12 docs: 固化代码准入 CR 基线` 和 `f99800b docs: 对齐代码 CR 任务基线` 只保留为历史准入证据，不再作为当前恢复入口。当前有效输入聚焦 B1-10 契约承载、A0/A1 准入证据、B2/B4 支付工具与 Spend Rule Round 0、B4 授权后继能力闭环提交，以及 `openspec/changes/tdd-baseline-reset/tasks.md` 的最新任务账本。 | B4 已闭合的局部代码能力包括 `b0666ba` / `f99f3a3` 授权过期释放、`616dac1` / `3825466` 授权强制完成、`006bcaa` / `818da34` / `967586c` 无授权退款最小契约与路由回退、`949b24a` 授权争议退款可区分性和 `47c5269` 授权后继并发竞争；`4d5f9c2` 与本轮任务账本已回写对应 GSD-CAD handoff。当前设计基线不得自动等同为授权支付工具应用入口代码完成、钱包 application facade 代码完成、资金责任目标字段迁移完成、预算组账务主体化兼容缺口关闭、完整 dispute/chargeback case、含权益生产链路、清结算对账、资金数据治理、指标治理或任一未授权 MVP 编码 Done 证据。 |
...
| B4 授权后继当前恢复入口 | B4-DISPUTE-SEMANTIC-ALIGNMENT 已由 `949b24a` 闭合并由 `4d5f9c2` 回写任务账本；B4-AUTH-RACE 已由 `47c5269` 闭合；B4-NO-AUTH-REFUND、B4-DISPUTE 旧恢复入口和 Grant 候选包都不再作为当前恢复入口。当前没有可直接恢复的 B4 默认编码入口。 | 下一轮若选择授权支付工具应用入口、授权权益生命周期、完整 dispute/chargeback case、余额控制调账、原路径回放或其他候选，必须另起 Round 0 或新的单一 Execution Grant。 |
...
| 2026-06-04 P2-ACQ-CAPTURE Grant 候选补齐 | `docs/TDD设计/P2-业务能力包Round0准入卡.md#14-p2-acq-capture-round-0-扫描2026-06-04` 已把收单 capture 归一、商户 `CLEARING` 和敏感数据边界切片收敛为 `P2-ACQ-CAPTURE-CAD-001`；代码扫描确认当前只有商户信息、商户上下文、清算账目、授权 capture 概念和敏感上下文阻断局部基线，尚未形成 `AcquiringCaptureApplicationService`、收单 capture Request/DTO、capture 幂等摘要、商户资金账户解析、merchant CLEARING 断言、支付成功不等于可提现和 PCI 原文阻断目标 Red。 | 当前状态为 `READY_TO_CONFIRM_NOT_CODE_AUTHORIZED`。后续只有用户明确确认 `Execution Grant：P2-ACQ-CAPTURE`、选择 `contract-only` 或 `canonical-clearing-backed`、确认 clearingDecision、settlementDecision、sensitiveDecision、外部规则和 PCI 安全边界状态后，才允许写收单 capture application facade Red 和最小实现；未确认前不得写 Java、测试、DDL/H2 schema、公共契约或运行时配置，也不得把 capture/payment success 直接写成可提现、可结算、出款成功、普通退款或拒付事实。 |
...
首轮编码推荐路径：先做 A0 基线核验，确认钱包账户、支付工具、账本、账目、余额投影和现有测试资产可承接，再由用户按 A1 至 A4 中的单一 MVP 任务切片确认 Execution Grant。B1 至 B8 只作为覆盖索引和历史任务编号，不作为新的默认开工授权；B7/B8 仍需独立 TDD 分析和 Execution Grant。A0 标准输出页见 `docs/TDD设计/A0-编码准入基线核验.md`，后续 Execution Grant 应引用该页的 `authorityBaseline`、`codeBaseline`、`targetAssets`、`schemaNeed`、`firstRedCandidateSet` 和 `nextExecutionGrantRequest`。截至 2026-06-04，B4-TRX-EXPIRE、B4-FORCE-SETTLE、B4-NO-AUTH-REFUND、B4-DISPUTE-SEMANTIC-ALIGNMENT 和 B4-AUTH-RACE 已进入代码基线；下一轮 GSD-CAD 不再默认恢复这些已消费的 B4 任务包。`B4-AUTH-PI-CAD-001`、`B2-PI-CAP-CAD-001`、`B2-FR-FAO-CAD-001`、`B5-SR-CONTROL-CAD-001` 和 `B6-B8-PI-VIEW-CAD-001` 已具备可确认的 Grant 候选包，但尚未获得编码授权；`P2-VCC-PREPAID-CAD-001`、`P2-VCC-LIFECYCLE-CAD-001`、`P2-GA-INBOUND-CAD-001`、`P2-GA-OUTBOUND-CAD-001`、`P2-GA-FX-FEE-CAD-001` 与 `P2-ACQ-CAPTURE-CAD-001` 也已具备 P2 业务专项 Grant 候选包，但不属于默认 P0/P1 开工路径。后续若继续编码，必须先由用户确认 `Execution Grant：B4-AUTH-PI`、`Execution Grant：B2-PI-CAP`、`Execution Grant：B2-FR-FAO`、`Execution Grant：B5-SR-CONTROL`、`Execution Grant：B6-B8-PI-VIEW` 之一，或确认 `Execution Grant：P2-VCC-PREPAID` / `Execution Grant：P2-VCC-LIFECYCLE` / `Execution Grant：P2-GA-INBOUND` / `Execution Grant：P2-GA-OUTBOUND` / `Execution Grant：P2-GA-FX-FEE` / `Execution Grant：P2-ACQ-CAPTURE` 作为 P2 业务专项切片。B5-SR-CONTROL 必须同步选择 `contract-only` 或 `ddl-backed`；B6-B8-PI-VIEW 必须同步选择 `query-contract-only` 或 `projection-store-backed`；P2-VCC-PREPAID 必须同步选择 `contract-only` 或 `funding-flow-backed`、责任主体策略和外部规则核验状态；P2-VCC-LIFECYCLE 必须同步选择 `contract-only` 或 `canonical-lifecycle-backed`、原快照策略、disputeDecision 和外部规则核验状态；P2-GA-INBOUND 必须同步选择 `contract-only` 或 `canonical-funds-backed`、账户解析决策、FX 决策和外部规则核验状态；P2-GA-OUTBOUND 必须同步选择 `preflight-contract-only` 或 `canonical-transit-backed`、preflightDecision、transitDecision、returnDecision、FX 决策和外部规则核验状态；P2-GA-FX-FEE 必须同步选择 `contract-only` 或 `attribution-backed`、`no-fx-execution` 或 `quote-validation-only`、`fee-attribution-only` 或 `fee-ledger-backed`、外部规则和专业确认状态；P2-ACQ-CAPTURE 必须同步选择 `contract-only` 或 `canonical-clearing-backed`、`no-clearing-batch-write` 或 `clearing-batch-backed`、`no-settlement-release`、`masked-reference-only`、外部规则和 PCI 安全边界状态。
