...
| 03 清结算与对账 | 设计准入通过，编码准入未打开。 | 独立 OpenSpec change、模块和表确认、DDL/H2、服务契约、状态机、服务级 H2 流程测试和外部规则确认。 |
| 04 归档、重放与指标边界 | 设计准入通过，只有交易投影重放局部代码基线。 | 归档 Manifest、账本余额快照、普通指标快照隔离、范围锁、差异报告、回滚/续跑和边界测试。 |
