...
| 当前编码准备基线 | 当前已提交设计和任务对齐输入已推进至 `8e4a801 docs: 对齐最新设计任务基线`；代码能力基线仍截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`。`9456ab6 docs: 对齐 A0 准入与代码基线` 仍作为上一 A0/code baseline 冻结点。2026-05-31 的资金责任解析、支付工具投影、授权支付工具应用入口、钱包 DDD 应用层、资金责任目标字段和 BudgetGroup/Spend Rule 控制上下文调整已进入新的设计/任务对齐输入；本轮核心设计骨架修复提交后继续作为更新后的设计/任务输入。 | 以 `30b1a00` 设计基线、`eabd672` 编码准备包、`6be9c99` B1-10 契约承载、`5bad4b7` 权益资金流编码基线、`42100dc` 权益生产准入门禁、`555a943` 至 `db766fa` 的工程约规、测试门禁、治理重放门禁、CAD 完整验证门禁、出款准入规则核验证据，`f386ed0a` 至 `5901265` 的编码准入设计冻结、出款准入设计冻结、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要和治理重放范围/来源/差异项校验，`5901265..77bc9f4` 的资金事实红线、敏感上下文阻断、上下文不可变、防御性拷贝、外部账户原文阻断、MVP/归档边界和设计交付口径收敛，`9456ab6` 的 A0 准入与代码基线对齐，`3ef522c`、`a7d3fc9` 的授权支付工具入口和钱包应用层 CR 结论，以及 `8e4a801` 的最新设计任务基线索引作为当前准备基线输入；当前设计基线不得自动等同为授权支付工具应用入口代码完成、钱包 application facade 代码完成、资金责任目标字段迁移完成、预算组账务主体化兼容缺口关闭、含权益生产链路、清结算对账、资金数据治理、指标治理或任一 MVP 编码 Done 证据。 |
...
| 项 | 结论 | 执行口径 |
| --- | --- | --- |
| 设计基线 | 当前已提交设计和任务对齐输入为 `8e4a801`；`9456ab6` 仍是上一 A0/code baseline 冻结点。 | 后续 MVP 编码任务若要采用资金责任解析、支付工具投影、授权支付工具应用入口、钱包 DDD 应用层、资金责任目标字段和 BudgetGroup/Spend Rule 控制上下文调整，必须引用 `8e4a801` 或更新后的设计提交点，或在 Execution Grant 的 `authorityBaseline` 中显式列为基线附件。 |
