...
| 当前任务账本状态 | 已消费的 B4-NO-AUTH-REFUND、B4-DISPUTE-SEMANTIC-ALIGNMENT、过期恢复入口和旧候选包只作为历史证据，不再作为当前默认任务计划。 | 后续编码必须重新选择一个单一 Execution Grant；未纳入 Grant 的文档变更、代码变更或草稿参考，不能作为冻结编码基线、生产 Done 证据或可写范围授权。 |
...
| B4 授权后继当前恢复入口 | B4-DISPUTE-SEMANTIC-ALIGNMENT 已由 `949b24a` 闭合并由 `4d5f9c2` 回写任务账本；B4-NO-AUTH-REFUND、B4-DISPUTE 旧恢复入口和 Grant 候选包都不再作为当前恢复入口。 | 下一轮只能重新确认 B4-AUTH-RACE、授权支付工具应用入口、授权权益生命周期、完整 dispute/chargeback case 或其他经过 Round 0 的单一 Execution Grant。 |
