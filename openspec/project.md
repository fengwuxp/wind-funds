...
| 当前编码准备基线 | 截至 `9fc2e5d test: 对齐 CAD 完整验证入口` | 以 `30b1a00` 设计基线、`eabd672` 编码准备包、`6be9c99` B1-10 契约承载、`5bad4b7` 权益资金流编码基线、`42100dc` 权益生产准入门禁和 `555a943` 至 `9fc2e5d` 的工程约规、测试门禁、治理重放门禁、CAD 完整验证门禁作为当前准备基线；不得自动等同为含权益生产链路 Done 证据。 |
...
| 代码能力基线 | 截至 `9fc2e5d`，B1-10 权益快照 DSL 契约、契约测试、JSON 夹具、请求摘要、稳定摘要、账务计划装配器长 ID、预算组默认周期 `LIFETIME`、账务事实断言、钱包边界、交易稳定摘要、治理投影重放边界和 CAD 完整验证门禁已纳入当前局部基线。 | 批次 1 已有 DSL 契约测试和 B1-10 权益快照契约承载基线，批次 2 已有支付工具、绑定历史、资金来源关系、账务计划装配器、预算组默认周期 `LIFETIME` 和余额投影局部基线，批次 3 至 6 仍只具备局部流程或边界测试，批次 7/8 仍是独立待落地域。 |
...
14. 若本批次涉及含权益交易的 route/posting/replay、清结算、对账、投影、归档、冷热读取或治理重放消费，Execution Grant 必须声明 Phase 能力边界、JSON 夹具级别、权益快照事实源、零实付表达、平台补贴表达、独立伴随指令原子性、储值预付口径、退款分摊粒度、历史无权益快照处理策略、补充权益事实模型、专业确认状态、审计证据包、使用者解释视图、证据最小化和外部规则核验状态；缺任一项时不得进入生产资金流 Done 结论。
...
| 6. 权益准入是否完整 | 含权益批次是否已声明 Phase 能力边界、JSON 夹具级别、事实源、零实付表达、平台补贴表达、独立伴随指令原子性、储值预付口径、退款分摊、历史无快照处理、补充权益事实模型、专业确认状态、审计证据包、使用者解释视图、证据最小化和外部规则核验状态；涉及投影、归档、冷热读取或治理重放时是否同步声明对应消费边界和最终重校验点。 | 降级为 contract-only 或补齐决策卡。 |
