...
| 名称 | 任务规划落点 | 边界 |
| --- | --- | --- |
| Route Replay | 批次 1、批次 6 | 只解决后续资金事件沿原 route snapshot 回放，缺快照必须失败或进入人工处理。 |
| Transaction Projection Replay | 批次 6、批次 8 | 只修复只读交易视图、账单、时间线或批次视图，不重新入账、不修改余额。 |
| Balance Rebuild | 批次 8 | 只从账本分录、检查点、水位和 Manifest 重建余额投影，不从余额日志、交易投影或报表反推余额。 |
| Archive Resume | 批次 8 | 只用于归档或重放任务断点续跑，不替代账本周期、余额水位或资金路径选择。 |
...
4. Route Replay、交易投影重放、余额重建和归档续跑是否被放入同一批次，以及是否会产生边界混用。
5. 清结算、对账、归档和重放是否进入本轮实现。
