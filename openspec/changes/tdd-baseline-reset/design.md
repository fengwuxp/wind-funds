...
| `settleRefund` 无授权退款模式首轮 canonical 能力已闭合 | `006bcaa`、`818da34` 和 `967586c` 已证明无前置授权但有可追溯外部引用时，可在不补造授权占用、不暴露请求 `refundMode` 的前提下形成独立退款事实；普通授权链退款仍依赖 `authorizationTransactionSn`、`AUTHORIZATION` reference 和原授权/原完成 route replay，no-auth refund 可由 `EXTERNAL_TRANSACTION` reference 推断专用路由。 | 后续只作为 B4 授权交易回归基线；运营审批、人工差错单、累计退款跨请求聚合控制、查询投影解释、外部规则或完整 dispute/chargeback case 必须另起单一 MVP Execution Grant。 |
