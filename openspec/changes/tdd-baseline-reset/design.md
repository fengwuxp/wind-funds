...
| 清结算、对账、归档、指标已有骨架或候选实现但不得混入主链路 | 容易误以为 03、04 已获准抢跑，或把治理能力写回交易主链路。 | 先完成 02 阶段；03、04 分别独立处理。冻结基线中 `reconciliation-*` 仅具备模块骨架和 B7-00 出款前准入候选实现，只视为候选输入；`governance-*` 只视为候选落点和局部基线。 |
