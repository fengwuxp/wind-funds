...
| `settleRefund` 无授权退款模式引用和审计字段需复核 | 无前置授权但有外部原消费、原完成或差错凭证时，需要可证明不补造授权占用且可追溯；当前扫描确认普通授权链退款仍依赖 `authorizationTransactionSn`、`AUTHORIZATION` reference 和原授权/原完成 route replay。 | 归入 B4 授权交易覆盖索引；进入编码时按单一 MVP 任务授权，并显式确认 `refundMode` 或等价模式、外部原事实引用、外部原事实类型、退款原因、退款凭证、操作者/审计字段和必要原事实金额摘要；普通授权链退款继续要求内部授权流水，NO_AUTH 模式不得携带或查询内部授权流水。 |
