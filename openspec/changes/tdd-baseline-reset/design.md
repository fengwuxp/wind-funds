...
| `settle` 强制完成模式缺少契约字段 | 无前置授权强制完成无法证明 FORCE 模式、受信策略或审批快照、上限、原因、外部原事实引用、凭证和审计；现有普通完成仍依赖内部原授权流水。 | 归入 B4 授权交易覆盖索引；进入编码时按单一 MVP 任务授权。Execution Grant 必须明确 `authorizationTransactionSn` 条件化规则：普通完成必填，首轮 FORCE 模式不得依赖内部原授权流水。 |
...
3. 强制完成和无授权直接退款对历史数据、幂等键、外部凭证和对账解释的影响；授权过期已在 B4-TRX-EXPIRE 中补齐账户主体型 canonical 能力，后续若扩展并发、投影解释或业务 facade 仍需单独授权。
