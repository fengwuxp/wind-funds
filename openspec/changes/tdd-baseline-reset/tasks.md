...
| 本轮新增权威补充 | `docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md` 已新增 `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 消费记录；B7 差异报告已提交到 `a1397ddf`，本轮账户能力来源准入已完成本地 Green / Verify。 |
...
| 本轮新增写入补充 | 账户能力来源准入 face / impl / DTO / request / core 字段 / 目标测试、TDD README、docs README、LWT Goal、W5 推进计划、GSD-2 工作流入口和本 Harness tasks 的状态同步；不授权完整预交易快照、Spend Rule、VCC facade、支付工具交易内核、Controller、HTTP/RPC、生产迁移或 Git 历史重写。 |
...
| 活跃未完成编码计划 | `EMPTY`。旧候选不再是当前编码计划，只能作为 backlog reference；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮争议退款、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；下一轮不得复用 B5-003、B7 对象级 Gate、B7 consumer、B7 report 或 account capability Grant，需重新选择 wallet 完整预交易快照、Spend Rule 控制闭环、B7 报告扩展或其他单一 Grant。 |
...
| 下一候选 | 若继续编码，只能在完整预交易快照、Spend Rule 控制闭环、B7 报告扩展或 VCC facade 中重新确认单一 Grant。Spend Rule、VCC、全球账户和 P2 业务仍后置。 |
