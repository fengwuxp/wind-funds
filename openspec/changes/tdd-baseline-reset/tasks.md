...
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片和控制活动幂等语义一致性守卫补片，新增 `SpendControlTransactionConsumptionApplicationService`、`CONSUMED` / `REFUND_COMPENSATED` 控制活动语义、原活动 / 原交易回链、`consumedAmount` 投影解释，并证明同流水同摘要并发唯一键冲突可回读既有活动；补充证明 `REFUND` 交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放，非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿，退款交易没有对应已消费控制活动时不得生成退款控制补偿，跨业务场景资金交易不得消费当前 Spend Rule 控制占用，同业务场景但不同业务流水的成功交易不得消费当前控制活动，同业务场景但不同业务流水的失败、拒绝或过期交易也不得释放当前控制活动，退款补偿金额不得超过退款交易引用的已消费控制活动净额，且退款引用的已消费控制活动必须与原控制活动在业务场景、业务流水、目标账户和币种上同源一致；同一原控制活动下同一资金交易流水的 `CONSUMED`、`RELEASED` 或 `REFUND_COMPENSATED` 累计解释金额不得超过资金交易金额；同一控制活动流水和摘要不得被不同活动类型或不同业务语义复用。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| 当前默认确认包 | 当前无可复用默认确认包；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫和控制活动幂等语义一致性守卫状态为 `SR_TRANSACTION_CONSUME_IDEMPOTENCY_SEMANTIC_GUARD_GREEN_VERIFIED`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`。 |
