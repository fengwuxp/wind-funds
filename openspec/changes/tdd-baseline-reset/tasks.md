...
- [x] 2026-06-21 消费 `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 服务层补片：在角色协作 Loop 中把本轮结论聚合到 LWT Goal，收敛预算组作为 LedgerEntry/posting 主体的高风险缺口。Red 阶段证明旧 `DefaultLedgerTransactionPostingServiceImpl` 会让 `BUDGET_GROUP` 分录越过主体白名单进入余额约束；Green 后 posting 入口只允许 `FUNDING_ACCOUNT` 和 `CREDIT_ACCOUNT`，即使预算组存在显式控制账本，外部 `LedgerTransactionSpec` 携带预算组分录也会在 ledger transaction、posting plan、LedgerEntry 和余额投影前失败。已验证非沙箱 `just test-one DefaultLedgerTransactionPostingServiceImplTests tests` 12 tests、`just test-one ControlAccountLedgerInitializationTests tests` 11 tests、`just test-one LedgerBalanceProjectionServiceImplTests tests` 6 tests、`just test-ledger` 28 tests 通过，`just pmd` 和 `git diff --check` 通过。本补片不删除 `FundsSubjectType.BUDGET_GROUP`，不改 route resolver、route replay、余额查询、余额控制兼容路径、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC 或生产迁移。
...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片；LWT Goal、OpenSpec spec 和本 tasks 已把当前状态从释放上限目标账户隔离守卫推进到预算组 LedgerEntry/posting 主体阻断。 |
...
| 当前状态 | `LEDGER_BUDGET_GROUP_POSTING_GUARD_GREEN_VERIFIED` |
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片、控制活动幂等语义一致性守卫补片、退款引用原交易业务一致性守卫补片、预算控制投影目标账户隔离守卫补片和释放上限目标账户隔离守卫补片；本轮新增 ledger posting 主体白名单守卫，证明 `BUDGET_GROUP` 即使存在显式控制账本，也不得作为外部 `LedgerTransactionSpec` 的 posting / LedgerEntry 主体，失败发生在 ledger transaction、posting plan、LedgerEntry 和余额投影前。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、预算组 route / 余额查询 / 余额控制兼容退出、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
