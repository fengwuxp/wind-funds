...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片和兼容补偿；最新 ledger / wallet / transaction 生产可用 CR 结论已落成角色 Loop 任务队列；预算组控制账本兼容退出 Phase 1 已补控制视图替代入口，Phase 2 已硬禁任意 `BUDGET_GROUP` LedgerEntry 主体，Phase 3 已清理旧 profile、显式初始化、余额查询、route replay 和测试夹具旧语义；本轮 `GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 已补齐 wallet 授权准入对预交易快照和 Spend Control admission 的组合消费。LWT Goal、OpenSpec spec 和本 tasks 已把当前状态推进到 `WALLET_AUTH_ADMISSION_COMPOSE_GREEN_VERIFIED`。 |
...
| 当前状态 | `LEDGER_APPLICATION_FACADE_GREEN_VERIFIED` |
