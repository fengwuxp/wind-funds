...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片和兼容补偿；最新 ledger / wallet / transaction 生产可用 CR 结论已落成角色 Loop 任务队列；预算组控制账本兼容退出 Phase 1 已补控制视图替代入口，Phase 2 已硬禁任意 `BUDGET_GROUP` LedgerEntry 主体，Phase 3 已清理旧 profile、显式初始化、余额查询、route replay 和测试夹具旧语义；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 已补齐 wallet 授权准入对预交易快照和 Spend Control admission 的组合消费；`GSD2-LD-APPLICATION-FACADE-001` 已补齐 ledger 跨模块生产默认 application facade，并提交到 `e6006888`；`GSD2-WALLET-SPECIAL-BUSINESS-CAPABILITY-MAP-001` 已完成 design-only 能力地图，把 VCC 预付卡、VCC 共享卡、VA 收款、全球账户付款、ACH 或银行转账事件的入口解释、系统边界、DSL 承载和 TDD Red 种子回写到 PRD / 系分 / DSL / TDD / LWT Goal；`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001` 已消费支付工具生命周期授权入口委派切片；`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 已消费支付工具收款 route snapshot 回链切片，`receiveByInstrument` 委派账户主体型 `topup` 时固化脱敏 `PaymentInstrumentRef`；`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 已消费收款工具 expectedBindingVersion 必填护栏，缺版本在交易内核前失败且无资金事实副作用；`GSD2-GA-PAYOUT-RAIL-CONTRACT-001` 已消费全球账户出款 rail 服务层契约和未接内核 fail-fast guard，当前不生成资金事实、在途、费用、清结算或对账事实；`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 已消费外部 confirmed credit 入金到资金账户的最小真实消费，生成 TOPUP 资金交易、route snapshot、posting plan、LedgerTransaction 和 LedgerEntry，未授权 debit 事件和信用账户目标失败无资金 / 账本副作用。LWT Goal、OpenSpec spec 和本 tasks 已把当前状态推进到 `WALLET_EXTERNAL_FUNDS_EVENT_CREDIT_CONSUME_GREEN_VERIFIED`。 |
...
| 当前状态 | `WALLET_EXTERNAL_FUNDS_EVENT_CREDIT_CONSUME_GREEN_VERIFIED` |
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片、控制活动幂等语义一致性守卫补片、退款引用原交易业务一致性守卫补片、预算控制投影目标账户隔离守卫补片和释放上限目标账户隔离守卫补片；ledger posting 主体白名单守卫与兼容补偿已证明 `BUDGET_GROUP` 资金价值类分录不得作为外部 `LedgerTransactionSpec` 的 posting / LedgerEntry 主体；预算组控制账本兼容退出已消费 Phase 1 控制视图替代、Phase 2 posting 硬禁和 Phase 3 旧账本兼容清理；wallet 授权准入组合已消费预交易快照与 Spend Control admission；`GSD2-LD-APPLICATION-FACADE-001` 已补齐 `LedgerPostingApplicationService`、`LedgerFactQueryApplicationService` 和 `LedgerBalanceProjectionApplicationService`；`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001` 已消费支付工具生命周期授权入口委派切片；`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 已新增直接充值请求可选支付工具快照承载位、指令转换器透传和 `receiveByInstrument` route snapshot 归因断言；`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 已补齐收款工具 expectedBindingVersion 必填契约和失败无资金事实副作用断言；`GSD2-GA-PAYOUT-RAIL-CONTRACT-001` 已新增 `payOutByRail` 契约和未接内核 guard；`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 已让 `ExternalFundsEventApplicationService#consume` 支持 confirmed credit 到资金账户的标准 `topup` 消费。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、真实提现/出款、外部扣账、return、NOC、reversal、信用账户还款或调额、在途、费用、清结算、对账、Controller、HTTP/RPC、事件消费者、outbox、生产历史数据迁移、公共 API 删除、枚举删除或 Git 历史重写。 |
...
| 活跃未完成编码计划 | 当前没有可复用的活跃编码 Grant；`GSD2-B5-SR-CONTROL-ACTIVITY-001` 已完成首轮本地 Green / Verify 并提交到 `78f7f008`，预算组非建账已提交到 `a5b12a3f`，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成本地 Green / Verify / Commit，`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` 已完成本地 Red / Green / Verify，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003`、`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005`、`GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006`、`GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009`、`GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010`、`GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012`、`GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013`、`GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014`、`GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015`、`GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001`、`GSD2-LD-BUDGET-GROUP-CONTROL-COMPAT-EXIT-001` Phase 1、`GSD2-LD-BUDGET-GROUP-CONTROL-POSTING-FORBID-001` Phase 2、`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002`、`GSD2-LD-APPLICATION-FACADE-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`、`GSD2-GA-PAYOUT-RAIL-CONTRACT-001`、`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001` 和 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 已完成本地目标回归和状态回写。旧候选不再是当前编码计划，只能作为 backlog reference；若继续推进，必须从外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA 收款场景、VCC 单场景、真实提现/出款内核、transaction 兼容债、旧 ledger 公共 CRUD API 治理、完整 Spend Rule 或生产迁移中重新选择新的单一 Grant。 |
| 写入范围 | 本轮写入范围为 `wallet-impl` 外部事件 confirmed credit 消费实现、外部事件服务流测试、系分、TDD Goal、特殊业务能力确认包和本 Harness tasks；不写 DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 账户主体、ledger posting、运行时配置、统一支付工具交易内核、外部扣账、return、NOC、reversal、信用账户还款或调额、清结算、对账、事件消费者、outbox 或旧公共 CRUD API 删除。 |
...
| 当前默认确认包 | 当前没有可复用编码确认包；`docs/TDD设计/GSD-2-Wallet特殊业务能力地图ExecutionGrant确认包.md` 已作为 design-only 能力地图完成，并已消费 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001` 支付工具生命周期授权入口委派切片、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 收款 route snapshot 回链切片、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 收款绑定版本必填护栏切片、`GSD2-GA-PAYOUT-RAIL-CONTRACT-001` 出款 rail 契约 guard 切片、`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001` 外部资金事件消费契约 guard 切片和 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` confirmed credit 入金消费切片，不再授权继续扩真实出款、外部扣账 / return / NOC / reversal、信用账户还款或调额、事件消费者 / outbox 或完整 VCC 生命周期。`docs/TDD设计/GSD-2-LD-预算组控制账本兼容退出ExecutionGrant确认包.md` 已消费 Phase 1、Phase 2 和 Phase 3，状态 `BUDGET_GROUP_LEDGER_COMPAT_CLEANUP_GREEN_VERIFIED`；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫状态为 `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`；`GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已消费并提交到 `a613eff5` / `80cd1461`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002`、`GSD2-LD-APPLICATION-FACADE-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`、`GSD2-GA-PAYOUT-RAIL-CONTRACT-001`、`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001` 和 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 均已消费，当前不得继续复用这些 Grant。 |
| 下一候选 | 按被依赖方优先，预算组兼容退出、wallet 授权准入组合、ledger application facade、wallet 特殊业务能力地图 design-only、`authorizeByInstrument` 推荐入口契约、`receiveByInstrument` 收款快照回链、收款绑定版本必填护栏、`payOutByRail` 契约 guard、外部资金事件消费契约 guard 和 confirmed credit 入金消费均已完成；推荐下一个单一候选为外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA/VCC 单场景、真实提现/出款内核衔接、`GSD2-TRX-COMPAT-DEBT-001`、旧 ledger 公共 CRUD API 治理、完整 Spend Rule 规则引擎或事件消费 / outbox 专项中的一个。 |
