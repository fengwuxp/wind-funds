...
| 6 | 支付工具支持 | `B2-PI-CAP-CAD-001`、`B4-AUTH-PI-CAD-001`、`B5-SR-CONTROL-CAD-001`、`B6-B8-PI-VIEW-CAD-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-CONTRACT-002`。 | 放到最后的支持队列；只处理工具动作能力准入、授权工具 application facade、Spend Rule 控制、支付工具收款入口和只读解释，不替换交易 canonical 请求。`receiveByInstrument` 已完成最小服务层入口，VA、ACH 或外部钱包端点收款必须先经上层业务或通道适配层解释为归一入金事实，再由 wallet application 解析工具、绑定、资金责任和账户能力后委派 `FundsDirectTransactionService#topup`；不授权出款、退款、ACH return、全球账户付款、VCC 专项生命周期或统一支付工具交易内核。 |
...
| 当前默认确认包 | 当前没有可复用编码确认包；`docs/TDD设计/GSD-2-Wallet特殊业务能力地图ExecutionGrant确认包.md` 已作为 design-only 能力地图完成，并在本轮消费 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-CONTRACT-001` 首个授权入口契约，不再授权继续扩 `receiveByInstrument`、`payOutByRail`、外部事件消费或完整 VCC 生命周期。`docs/TDD设计/GSD-2-LD-预算组控制账本兼容退出ExecutionGrant确认包.md` 已消费 Phase 1、Phase 2 和 Phase 3，状态 `BUDGET_GROUP_LEDGER_COMPAT_CLEANUP_GREEN_VERIFIED`；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫状态为 `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`；`GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已消费并提交到 `a613eff5` / `80cd1461`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002`、`GSD2-LD-APPLICATION-FACADE-001` 和 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-CONTRACT-001` 首个授权入口契约均已消费，当前不得继续复用这些 Grant。 |
| 下一候选 | 按被依赖方优先，预算组兼容退出、wallet 授权准入组合、ledger application facade、wallet 特殊业务能力地图 design-only 和 `authorizeByInstrument` 推荐入口契约均已完成；推荐下一个单一候选为 `receiveByInstrument`、`payOutByRail` 或外部事件消费中的一个服务层契约 / Red 切片。也可改选 VCC 预付卡充值提现、VA 收款、`GSD2-TRX-COMPAT-DEBT-001`、旧 ledger 公共 CRUD API 治理、完整 Spend Rule 规则引擎或事件消费 / outbox 专项。 |
