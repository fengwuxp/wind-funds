...
| 本轮新增写入补充 | 在已消费交易消费支出控制活动、预算控制投影、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard、外部 confirmed credit 入金消费和 Spend Rule docs-only 设计闭环等历史证据基础上，本轮消费 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`：新增 `SpendRuleDefinition`、`SpendRuleVersion`、`SpendRuleAssignment` 和 `SpendRuleDecisionLog` 的最小服务层契约、Request/DTO、枚举、Entity、Mapper、H2 schema 和目标测试；`SpendControlActivity` 保留为既有控制事实能力，本轮不删除、不回滚、不继续扩展。当前不授权完整规则引擎、规则表达式执行器、运营后台、审批流、Controller、HTTP/RPC、VCC facade、支付工具交易内核、真实提现/出款、外部扣账、return、NOC、reversal、信用账户还款或调额、在途、费用、清结算、对账、事件消费者、outbox、生产历史数据迁移、公共 API 删除、枚举删除或 Git 历史重写。 |
...
| 活跃未完成编码计划 | 当前没有可复用的活跃编码 Grant；`GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`、`GSD2-B5-SR-CONTROL-ACTIVITY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-001` 及其并发、退款、业务场景、目标账户隔离等补片、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard 、外部 confirmed credit 入金消费和 Spend Rule 决策消费均已完成目标回归和状态回写。旧候选不再是当前编码计划，只能作为 backlog reference；若继续推进 Spend Rule，优先从 `GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001`、完整规则引擎、事件消费 / outbox 或生产迁移中重新选择单一 Grant；若不选 Spend Rule，则可在外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA 收款场景、VCC 单场景、真实提现/出款内核、transaction 兼容债、旧 ledger 公共 CRUD API 治理、事件消费 / outbox 或生产迁移中另选单一 Grant。 |
| 写入范围 | 本轮写入范围为 core 枚举、wallet-face application / Request / DTO、wallet-impl application / Entity / Mapper、tests 服务流测试、H2 schema、TDD 任务卡、TDD README、LWT Goal、P0/P1 推进计划、docs README 和本 Harness tasks；不写 Controller、HTTP/RPC、交易 canonical 账户主体、ledger posting、运行时配置、统一支付工具交易内核、外部扣账、return、NOC、reversal、信用账户还款或调额、清结算、对账、事件消费者、outbox、生产迁移或旧公共 CRUD API 删除。 |
