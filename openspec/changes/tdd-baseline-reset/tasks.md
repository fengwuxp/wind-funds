...
- [x] 2026-06-20 将 `com.wind.funds.wallet.application` 只读 CR 结论补入 LWT Goal、W5 推进计划和 `GSD2-B5-SR-TRANSACTION-CONSUME-001` 确认包：当前 wallet application 包具备资金责任解析、账户能力来源、支付工具能力、预交易快照、授权准入、支出控制准入和控制活动服务流证据，但只能判定为条件基础能力。下一 Grant 进入 Red 前必须吸收以下准入条件：交易结果消费控制活动必须补 `CONSUMED / consumedAmount` 或等价语义；`AuthorizationAdmissionApplicationService` 只能视为支付工具授权内核准入 baseline，不能误读为完整 Spend Rule 编排入口；退款首轮只处理已有资金退款事实后的控制补偿，不修改支付工具 `REFUND` 能力方向；控制活动幂等需要覆盖并发唯一键冲突读取回放。本记录仍为 docs-only 状态维护；未确认 `Execution Grant：GSD2-B5-SR-TRANSACTION-CONSUME-001`、`schemaDecision：activity-schema-backed` 和 `refundActionDecision：control-compensation-only` 前，不授权 Java、测试、DDL/H2 schema、公共契约、Entity、Mapper、Controller、HTTP/RPC、运行时配置、Git 或生产迁移。
- [x] 2026-06-20 为 `GSD2-B5-SR-TRANSACTION-CONSUME-001` 补齐 Grant 消费运行卡：`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已新增 Pick / Red / Contract / Green / Extend / Review / Verify / Handoff 顺序，明确首个 Red 应证明交易成功消费控制活动缺口，Green 应复用现有 `t_spend_control_activity`、Entity、Mapper 和活动服务基础能力，并新增 `CONSUMED` 或等价成功消耗语义、`consumedAmount` 或等价投影、原活动与原交易回链。该记录同时校准旧“控制活动服务不存在”的历史预检已被 `78f7f008` 消费；当前缺口只剩交易消费服务层闭环。本记录仍为 docs-only 交接增强；未确认 `Execution Grant：GSD2-B5-SR-TRANSACTION-CONSUME-001`、`schemaDecision：activity-schema-backed` 和 `refundActionDecision：control-compensation-only` 前，不授权 Java、测试、DDL/H2 schema、公共契约、Entity、Mapper、Controller、HTTP/RPC、运行时配置、Git 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-001 / schemaDecision=activity-schema-backed / refundActionDecision=control-compensation-only` 服务层切片：新增 `SpendControlTransactionConsumptionApplicationService`、`SpendControlTransactionConsumptionRequest` 和 wallet-impl 实现；补 `CONSUMED` 与 `REFUND_COMPENSATED` 控制活动类型、原活动 / 交易回链字段、`consumedAmount` 投影口径、H2 `t_spend_control_activity` 测试字段和 `SpendControlTransactionConsumptionApplicationServiceTests`。目标测试覆盖成功交易消费、失败释放、退款控制补偿、超额拒绝和摘要冲突幂等边界，并证明失败路径不写资金交易、route、posting、LedgerEntry、账本交易或余额投影。已验证 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests`、`just test-one SpendControlActivityApplicationServiceTests tests`、`just test-one SpendControlAdmissionApplicationServiceTests tests`、`just compile`、`just pmd` 和 `git diff --check` 通过。本 Grant 不得复用为 Controller、HTTP/RPC、支付工具 `REFUND` 方向重裁决、交易 canonical 入参改造、统一支付工具交易内核、完整 Spend Rule 规则引擎、事件消费 / outbox、生产迁移、VCC facade、route replay、posting assembler 或 ledger posting 授权；自动告警和补偿重试仍需后续专项 Grant。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` 服务层切片：在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加并发同流水同摘要消费用例，首次非沙箱目标测试 Red 为 `DuplicateKeyException`；Green 后 `SpendControlActivityApplicationServiceImpl` 捕获并发唯一键冲突，按 `tenantId + activitySn` 回读既有控制活动并比较 `activityDigest`，同摘要返回既有活动，不同摘要仍沿用摘要冲突拒绝。该测试断言只保留一条 `CONSUMED` 控制活动、一条资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证 `just test-one SpendControlTransactionConsumptionApplicationServiceTests,SpendControlActivityApplicationServiceTests,SpendControlAdmissionApplicationServiceTests tests` 服务层组合回归 15 tests、`just compile`、`just pmd`、`git diff --check` 和边界关键词扫描通过。本切片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` 服务层切片：在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加失败 `REFUND` 交易误走 `release` 的无副作用用例，首次非沙箱目标测试 Red 为 `Expecting code to raise a throwable`，证明旧服务会把失败退款交易写成 `RELEASED` 控制活动；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#release` 拒绝退款交易事实，退款相关控制影响只能通过 `refund` 记录 `REFUND_COMPENSATED`。已验证 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 7 tests、服务层组合回归 16 tests、`just compile`、`just pmd`、`git diff --check` 和边界扫描通过。本切片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加已关闭 `REFUND` 交易误走 `consume` 的无副作用用例，证明退款交易事实不得被降级为普通成功消费；结合已完成 release 守卫，退款相关控制影响只能通过 `refund` 记录 `REFUND_COMPENSATED`。本轮只改目标测试和状态文档，生产实现沿用既有服务层守卫；已验证非沙箱 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 8 tests、`just compile`、`just pmd` 和 `git diff --check` 通过，沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-COMPENSATION-GUARD-003` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加非 `REFUND` 交易误走 `refund` 的无副作用用例，证明普通成功交易事实不得被降级为 `REFUND_COMPENSATED` 控制补偿；失败路径不新增退款补偿控制活动，不修改既有消费控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。本轮只改目标测试和状态文档，生产实现沿用既有服务层守卫；已验证非沙箱 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 9 tests 通过，沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-REFERENCE-GUARD-004` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加退款交易引用原资金交易但原交易没有对应 `CONSUMED` 控制活动的无副作用用例，证明退款控制补偿必须基于已消费控制事实，不能只凭退款交易引用生成 `REFUND_COMPENSATED`。本轮只改目标测试和状态文档，生产实现沿用既有服务层守卫；已验证非沙箱 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 10 tests 通过，沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SCENE-GUARD-005` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加跨业务场景资金交易不得消费当前 Spend Rule 控制占用的无副作用用例。Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl` 校验原控制活动与请求的业务场景、业务流水一致，并校验资金交易事实与请求业务场景一致；不强制资金交易 `businessSn` 等于控制活动业务流水，以保留失败释放和退款交易自身业务流水语义。目标测试断言失败路径不新增控制活动，不修改资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 11 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-BUSINESS-SN-GUARD-006` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加同业务场景但不同业务流水的成功资金交易不得消费当前 Spend Rule 控制占用的无副作用用例。Red 证明旧服务会串用同场景其他业务流水的成功交易生成 `CONSUMED` 控制活动；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#consume` 在控制活动、请求和资金交易业务场景一致的基础上，额外要求成功消费交易 `businessSn` 与请求业务流水一致。该补片当时只约束 `consume`，不改变 `refund` 的退款交易引用已消费控制活动语义；后续 `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` 已补齐 `release` 失败释放业务流水一致性守卫。目标测试断言失败路径不新增控制活动，不修改资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 12 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-RELEASE-BUSINESS-SN-GUARD-007` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加同业务场景但不同业务流水的失败资金交易不得释放当前 Spend Rule 控制占用的无副作用用例。Red 证明旧服务会串用同场景其他业务流水的失败交易生成 `RELEASED` 控制活动；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#release` 在控制活动、请求和资金交易业务场景一致的基础上，额外要求失败、拒绝或过期交易 `businessSn` 与请求业务流水一致。目标测试断言失败路径不新增控制活动，不修改资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 13 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-NET-GUARD-008` 服务层补片：按用户指定在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加同一原占用下多笔已消费控制活动时，退款补偿金额不得超过退款交易引用的已消费控制活动剩余净额的无副作用用例。Red 证明旧服务会借用同一原占用下其他消费活动净额放行退款补偿；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#refund` 在已消费引用存在的基础上，额外按退款交易 `referenceTransactionSn` 聚合被引用已消费活动和历史退款补偿活动，拒绝超过被引用净消费金额的补偿请求。目标测试断言失败路径不新增控制活动，不修改资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 14 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCED-CONSUMED-CONSISTENCY-GUARD-009` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加退款引用的已消费控制活动与原 `RESERVED` 控制活动不同源时拒绝补偿的无副作用用例。Red 证明旧服务只要找到被引用交易对应的 `CONSUMED` 控制活动就会放行，即使历史脏事实或绕过入口留下的已消费活动业务流水与原占用不一致；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#refund` 在引用净额守卫前校验被引用 `CONSUMED` 控制活动与原控制活动的业务场景、业务流水、目标账户和币种一致。目标测试断言失败路径不新增退款补偿控制活动，不修改既有控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 15 tests 通过，组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlActivityApplicationServiceTests` 21 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONTROL-TRANSACTION-AMOUNT-GUARD-010` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加同一原控制活动下同一资金交易流水被多条控制活动累计解释超过交易金额的无副作用用例，覆盖 `CONSUMED`、`RELEASED` 和 `REFUND_COMPENSATED` 三类控制活动。Red 证明旧服务允许同一交易流水被多次控制消费、释放或退款补偿累计解释超过交易金额；Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl` 以 `tenantId + originalActivitySn + transactionSn + activityType` 查询既有控制活动，排除当前幂等 `activitySn` 后校验既有金额加本次请求金额不得超过资金交易金额。目标测试断言失败路径不新增控制活动，不修改既有控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 18 tests 通过，组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlActivityApplicationServiceTests` 24 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-CONSUME-IDEMPOTENCY-SEMANTIC-GUARD-011` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 和 `SpendControlActivityApplicationServiceTests` 增加同一控制活动流水和摘要被不同活动类型或不同关键业务语义复用时拒绝回放的无副作用用例。Red 证明旧 `SpendControlActivityApplicationServiceImpl` 只比较 `activityDigest`，会把同一 `activitySn + activityDigest` 的 `RELEASED` 活动返回给 `consume` 或把 `RESERVED` 活动返回给 `RELEASED` 记录请求；Green 后控制活动幂等回放还会校验活动类型、业务场景、业务流水、原活动、交易流水、支付工具、目标账户、金额、币种、Spend Rule 和决策证据一致。目标测试断言失败路径不新增控制活动，不修改既有控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 19 tests 通过，组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlActivityApplicationServiceTests` 26 tests 通过；沙箱内 embedded Redis 端口限制已记录为环境问题。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-GUARD-012` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加历史脏 `CONSUMED` 控制活动引用不存在原消费资金交易时拒绝退款补偿的无副作用用例。Red 证明旧 `SpendControlTransactionConsumptionApplicationServiceImpl#refund` 只要找到已消费控制活动就会放行，即使退款交易 `referenceTransactionSn` 指向的原消费资金交易事实不存在；Green 后 `refund` 还会校验引用原消费资金交易存在、租户一致、不是另一笔 `REFUND`、状态为 `CLOSED`、币种与请求一致。目标测试断言失败路径不新增 `REFUND_COMPENSATED` 控制活动，不修改既有控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证非沙箱先复现 Red 为 `Expecting code to raise a throwable`，Green 后 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 20 tests 通过，组合回归 `SpendControlTransactionConsumptionApplicationServiceTests,SpendControlActivityApplicationServiceTests` 27 tests 通过；收口执行 `just compile`、`just pmd` 和 `git diff --check`。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-REFUND-REFERENCE-FACT-BUSINESS-GUARD-013` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加退款控制补偿引用的原消费资金交易业务流水不一致时拒绝的无副作用用例。Green 后 `SpendControlTransactionConsumptionApplicationServiceImpl#refund` 在引用原消费资金交易存在、租户、类型、状态和币种校验之后，还校验原交易 `businessScene`、`businessSn` 与当前请求一致，避免跨订单或历史脏交易事实被退款控制补偿复用。目标测试断言失败路径不新增 `REFUND_COMPENSATED` 控制活动，不修改既有控制活动或资金交易事实，且 route、posting、LedgerEntry、账本交易和余额投影不变化。已验证沙箱内目标测试因 embedded Redis 端口限制失败；非沙箱复跑 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 21 tests 通过；收口执行 `just compile`、`just pmd` 和 `git diff --check`。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-BUDGET-PROJECTION-TARGET-ACCOUNT-GUARD-014` 服务层补片：按用户指定继续在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加同预算组和同 Spend Rule 下多个目标账户时，成功交易消费只影响当前目标账户预算控制投影的回归；同时在 `SpendControlActivityApplicationServiceTests` 补控制活动投影按目标账户过滤和拒绝非资金 / 信用账户目标主体。Green 后 `BudgetControlProjectionQuery` / `BudgetControlProjectionDTO` 增加可选 `targetAccountId`，`SpendControlActivityApplicationServiceImpl` 聚合控制活动时按目标资金账户或信用账户过滤，避免其他账户或其他卡控制活动污染当前账户投影。已验证沙箱内目标测试因 embedded Redis 端口限制失败；非沙箱复跑 `just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 22 tests 通过。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-20 消费 `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` 服务层补片：按用户指定继续在 `SpendControlActivityApplicationServiceTests` 增加释放类控制活动写入上限按目标账户隔离的回归，并在 `SpendControlTransactionConsumptionApplicationServiceTests` 增加历史脏关联活动目标账户不一致时拒绝释放且无副作用的用例。Green 后 `SpendControlActivityApplicationServiceImpl` 在校验释放金额时按 `targetAccountId` 查询可释放占用，避免同预算组和同 Spend Rule 下其他账户的占用抬高当前账户释放上限；`SpendControlTransactionConsumptionApplicationServiceImpl` 在计算关联活动使用前校验历史关联活动与原 `RESERVED` 控制活动的业务场景、业务流水、目标账户、币种、Spend Rule、规则版本和预算组一致。已验证非沙箱复跑 `just test-one SpendControlActivityApplicationServiceTests,SpendControlTransactionConsumptionApplicationServiceTests tests` 33 tests 通过。本补片不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting、DDL/H2 schema、Entity、Mapper 或生产迁移。
- [x] 2026-06-21 消费 `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 服务层补片：在角色协作 Loop 中把本轮结论聚合到 LWT Goal，收敛预算组作为 LedgerEntry/posting 主体的高风险缺口。首轮 Red 阶段证明旧 `DefaultLedgerTransactionPostingServiceImpl` 会让 `BUDGET_GROUP` 分录越过主体白名单进入余额约束；硬护栏提交后发现会误伤余额控制预算额度调整兼容路径，本轮补偿 Green 改为拒绝预算组资金价值类分录，同时允许预算组 `CONTROL` 类 `LIMIT`、`AVAILABLE`、`AUTHORIZATION` 控制账本入账。已验证非沙箱 `just test-one DefaultLedgerTransactionPostingServiceImplTests tests` 13 tests、`just test-one FundsBalanceControlFailureFlowTests tests` 19 tests、`just test-ledger` 28 tests、`just test-balance-control` 44 tests、`just compile`、`just pmd` 和 `git diff --check` 通过。本补片不删除 `FundsSubjectType.BUDGET_GROUP`，不改 route resolver、route replay、余额查询、余额控制请求语义、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC 或生产迁移。
- [x] 2026-06-21 消费 `GSD2-LD-BUDGET-GROUP-CONTROL-COMPAT-EXIT-001 / compatDecision=staged-control-view-backed / phase=control-view-replacement-first` 服务层补片：按确认结论先补预算组控制账本退出前的控制视图替代路径，新增 `BudgetControlLimitAdjustmentApplicationService`、预算额度调额 Request/Result、`LIMIT_INCREASED` / `LIMIT_DECREASED` 控制活动类型、调额原因 / 操作者 / 审计引用字段和预算控制投影额度字段。Green 后预算额度调增 / 调减只写 Spend Control Activity 并派生 Budget Control Projection，不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影；同 `activitySn` 同摘要回放返回既有事实，异摘要冲突拒绝，调减后额度低于当前已占用控制额时 fail-fast 且无控制活动或资金事实副作用。已验证沙箱内目标 Spring 测试因 embedded Redis 端口限制失败；非沙箱复跑 `just test-one BudgetControlLimitAdjustmentApplicationServiceTests tests` 4 tests、`just test-one SpendControlActivityApplicationServiceTests tests` 10 tests、`just test-one SpendControlTransactionConsumptionApplicationServiceTests tests` 23 tests 通过，并执行 `just compile`、`just pmd` 通过；收口执行 `git diff --check`。本补片不删除 `FundsSubjectType.BUDGET_GROUP`，不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、route replay、posting assembler、ledger posting 控制账本兼容例外或生产迁移；Phase 2 已由 `GSD2-LD-BUDGET-GROUP-CONTROL-POSTING-FORBID-001` 消费，后续不得复用该 Grant 直接执行 Phase 3。
- [x] 2026-06-21 消费 `GSD2-LD-BUDGET-GROUP-CONTROL-POSTING-FORBID-001` 服务层补片：按用户授权在 Phase 1 控制视图替代入口通过后，移除 `DefaultLedgerTransactionPostingServiceImpl` 中预算组 `CONTROL` 类 `LIMIT` / `AVAILABLE` / `AUTHORIZATION` 入账例外，`POSTABLE_SUBJECT_TYPES` 只允许 `FUNDING_ACCOUNT` 和 `CREDIT_ACCOUNT`，任意 `BUDGET_GROUP` LedgerEntry 主体均在 ledger transaction、posting plan、LedgerEntry 和余额投影落库前拒绝。同步在 `FundsBalanceControlInstructionConverter` 前置拒绝预算组旧余额控制调额入口，`LIMIT_ADJUST` 只保留信用账户；预算额度调增 / 调减的可用入口是 `BudgetControlLimitAdjustmentApplicationService`。目标测试覆盖预算组控制分录拒绝且账本事实快照不变、预算组旧余额控制调增 / 调减入口失败无资金事实和账本事实副作用、替代入口继续只写控制活动和预算控制投影；相邻交易回归同步去除预算组旧调额铺底，证明预算组作为直接交易或授权交易主体仍拒绝且无资金或账本副作用。已验证非沙箱 `just test-one DefaultLedgerTransactionPostingServiceImplTests tests` 13 tests、`just test-one FundsBalanceControlFailureFlowTests tests` 19 tests、`just test-one BudgetControlLimitAdjustmentApplicationServiceTests tests` 4 tests 通过；沙箱内 Spring 测试曾因 embedded Redis 端口限制失败，按项目约规以非沙箱结果作为代码验证；并验证 `just test-one FundsDirectTransactionFlowTests tests` 51 tests、`just test-one FundsAuthorizationTransactionFlowTests tests` 32 tests、`just compile`、`just pmd` 和 `git diff --check` 通过。本补片不删除 `FundsSubjectType.BUDGET_GROUP`，不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、route resolver、posting assembler 结构、公共 API 删除或生产迁移；Phase 3 已由 `GSD2-B2-BUDGET-GROUP-LEDGER-COMPAT-CLEANUP-001` 消费。
- [x] 2026-06-21 消费 `GSD2-B2-BUDGET-GROUP-LEDGER-COMPAT-CLEANUP-001` 服务层补片：在 Phase 1 控制视图替代入口和 Phase 2 任意预算组 LedgerEntry 主体硬禁均完成后，清理预算组旧账本兼容语义。`BUDGET_BASIC` 不再作为 active ledger profile，`SubjectLedgerInitializer` 显式拒绝预算组初始化账本，资金账户查询聚合不再把预算组解析成 initialized balance subject，route leg、route replay 和 DSL JSON 合约均拒绝预算组作为核心资金路由 / 回放参与方；预算组仅保留为控制上下文、范围和审计维度。目标测试覆盖旧 profile 不可用、显式初始化失败、余额查询返回未初始化、legacy budget group route replay 失败、DSL route leg 拒绝、直接交易 / 授权交易预算组失败无账本副作用、旧余额控制调额入口失败无资金或账本副作用。已验证 `just test-one FundsDslJsonContractTests tests` 36 tests、`just test-one ControlAccountLedgerInitializationTests tests` 11 tests、`just test-one FundsSubjectBalanceQueryServiceImplTests tests` 12 tests、`just test-one DefaultRouteReplayServiceTests tests` 11 tests、`just test-one FundsDirectTransactionFlowTests tests` 51 tests、`just test-one FundsAuthorizationTransactionFlowTests tests` 32 tests、`just test-one FundsBalanceControlFailureFlowTests tests` 19 tests、非沙箱 `just test-one DefaultLedgerTransactionPostingServiceImplTests tests` 13 tests、非沙箱 `just test-one BudgetControlLimitAdjustmentApplicationServiceTests tests` 4 tests、`just compile`、`just pmd` 和 `git diff --check`。本补片不删除 `FundsSubjectType.BUDGET_GROUP`，不做生产历史数据迁移，不改 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、公共 API 删除或 Git 历史重写。
...
| 6 | 支付工具支持 | `B2-PI-CAP-CAD-001`、`B4-AUTH-PI-CAD-001`、`B5-SR-CONTROL-CAD-001`、`B6-B8-PI-VIEW-CAD-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-CONTRACT-002`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`。 | 放到最后的支持队列；只处理工具动作能力准入、授权工具 application facade、Spend Rule 控制、支付工具收款入口和只读解释，不替换交易 canonical 请求。`receiveByInstrument` 已完成最小服务层入口、收款 route snapshot 支付工具归因回链和 expectedBindingVersion 必填护栏，VA、ACH 或外部钱包端点收款必须先经上层业务或通道适配层解释为归一入金事实，再由 wallet application 解析工具、绑定、资金责任和账户能力后委派 `FundsDirectTransactionService#topup`；不授权出款、退款、ACH return、全球账户付款、VCC 专项生命周期或统一支付工具交易内核。 |
...
| B2-07 | 收敛预算组账务主体化兼容缺口。 | `wallet-*`、`ledger-*`、`transaction-*`、`tests/src/test/java`，具体写入范围由 Execution Grant 决定 | 产品预算组 / Spend Rule 边界、DSL BudgetGroup 上下文、系分账户服务边界、`GAP-WALLET-003`、本轮代码 CR。 | `FundsSubjectType.BUDGET_GROUP`、预算组余额查询或预算组 ledger 初始化被当作目标态可记账主体；预算组成为 route leg 或余额控制主体；预算控制额度调整绕过 Spend Rule 控制视图；`FundsAmountBoundaryContractTests`、`PaymentInstrumentRouteDslContractTests`、`ControlAccountLedgerInitializationTests` 或交易 flow support 继续把预算组入账路径当目标态保护。 | 已完成预算组创建不初始化 ledger bucket、预算组不得作为 LedgerEntry/posting 主体的 Red/Green 证据；后续必须继续明确 `BUDGET_GROUP` 是兼容枚举、迁移别名、只读查询过滤条件还是待删除目标，并同步收敛 `DefaultFundsAccountQueryServiceImpl`、`DefaultLedgerProfileServiceImpl`、`RouteSubjectSupport`、`DefaultRouteReplayService`、`FundsBalanceControlInstructionConverter` 和相关测试中的旧语义，或在 Execution Grant 中明确哪些路径只是兼容验证。 | `just test-ledger`、`just test-boundary` |
...
| B4-03 | 授权过期不入资金交易。 | `transaction-*`、`core`、`tests/src/test/java` | `GAP-AUTH-001`、`TDD-AUTH-011`、`TDD-ROUTE-008`、`TDD-RED-016`。 | `EXPIRE` 与 `REVERSAL` 混用，过期状态释放已完成金额或生成资金事实。 | `b0666ba` 的授权过期释放实现只保留为历史痕迹；2026-06-26 目标态裁决要求移除 `EXPIRE` 事件、`EXPIRED` 状态、`expire` 服务入口、请求模型、route replay 过期分支、生命周期过期终态和授权过期释放测试。授权到期、超时或通道未返回只进入提醒、差错候选或运营处理，释放占用必须由可信撤销、余额调整或对账差错补事实承接。 | `just test-one FundsAuthorizationTransactionFlowTests tests`、`just test-one DefaultRouteReplayServiceTests tests`、`just test-transaction`、`just test-boundary`、`just compile`、`just pmd` |
...
| B4-07 | 明确 chargeback 不落 `FundsAuthorizationTransactionService#chargeback`。 | `transaction-*`、`tests/src/test/java`，后续完整 case 仍需独立授权 | `GAP-AUTH-003`、拒付业务事实、B4 准入卡 `disputeSemanticAlignmentGrantCandidate`。 | 测试或任务强制要求调用 `chargeback` 服务入口，或把拒付结果只保存成无法区分的普通退款。 | 已由 `949b24a` 完成首轮争议资金结果可区分性：默认以 `settleRefund / AUTH_REFUND` 为拒付/争议承接目标态主入口。2026-06-26 目标态进一步裁决移除独立 `chargeback` 公共 API、请求模型、事件枚举和 route replay 分支；争议退款的原因、凭证、外部引用、内部 `DISPUTE` 标签、审计上下文、请求摘要和 route/ledger/posting/entry 事实继续由 `settleRefund` 承接。完整 dispute case、清结算追偿和外部规则仍需另起 Grant。 | `just test-one FundsAuthorizationTransactionFlowTests tests`、`just test-transaction`、`just test-business-flow`、`just test-boundary`、`just compile`、`just pmd` |
| B4-08 | 固化授权并发竞争红线。 | `transaction-*`、`core`、`tests/src/test/java` | TDD 13.5、授权生命周期。 | 同一授权的完成、可信撤销、退款并发导致重复入账、重复释放或剩余为负；过期状态不得参与资金迁移竞争。 | 已由 `47c5269` 完成首轮授权后继并发竞争红线；其中 `expire` 相关路径已被 2026-06-26 目标态移除裁决覆盖，后续回归只保留 settle / reversal / settleRefund 等可信资金后继事件的竞争边界。跨节点锁、数据库唯一约束、版本字段或更深退款并发扩展需另起 Grant。 | `just test-one FundsAuthorizationTransactionFlowTests tests`、`just test-transaction`、`just test-business-flow`、`just test-boundary`、`just compile`、`just pmd` |
| B4-09 | 固化授权占券和权益生命周期边界。 | `transaction-*`、`core`、`tests/src/test/java` | `DSL-BENEFIT-AUTH-HOLD-001`、`TDD-BEN-AUTH-*`、`TDD-BEN-ENTRY-003`、`TDD-BEN-ENTRY-004`、`RED-053`。 | 授权拒绝、工具准入失败或余额不足后仍核销权益；授权完成和可信撤销并发导致同一权益重复核销或释放；完成时按当前券规则重算；新增授权权益专用服务入口或授权后续事件接收当前权益结果。 | 不新增授权权益专用服务入口；`authorize` 如需表达授权占券，应在对应 Execution Grant 中新增权益占用引用、原权益资金事实引用或等价一等字段，不恢复旧 `benefitSnapshot`；`reversal`、`settle` 和 `settleRefund` 读取原授权事实、原权益资金事实、占用引用或历史摘要；授权阶段只固化占用引用，完成按原事实核销，可信撤销释放剩余占用；授权过期不入资金交易，也不触发权益释放或核销。失败事件无 route/posting/entry 副作用。 | `just test-business-flow` |
...
16. 直接交易和授权交易不因权益资金事实新增权益专用服务方法；后续如需扩展 `FundsTransactionPayRequest`、`FundsTransactionRefundRequest`、`FundsAuthorizationTransactionAuthorizeRequest` 或授权后续事件 Request，必须由对应 MVP 任务的 Execution Grant 明确允许。授权过期 `expire` 已由 2026-06-26 目标态裁决移除：到期、超时或通道未返回不是可信资金事实，不得借权益任务恢复 `expire` 公共契约、事件、状态或过期释放路径。
...
16D. 预算组兼容缺口进入编码前，必须确认 `FundsSubjectType.BUDGET_GROUP` 是兼容枚举、迁移别名、只读查询过滤条件还是待删除目标。预算组 posting / LedgerEntry 主体阻断已形成拒绝护栏证据；未确认前仍不得把预算组 ledger 初始化、预算组 route leg、预算组余额查询或余额控制路径作为目标态 Done 证据。
...
16F. 本轮代码 CR 发现的预算组旧语义必须作为 B2-07 的首要准入风险处理：`FundsSubjectType.BUDGET_GROUP` 枚举、`BUDGET_BASIC` profile、预算组 ledger 初始化、预算组余额查询、route participant、route replay、余额控制 limit adjust 和测试 fixture 中的预算组主体断言都不得在未确认兼容策略时继续扩大；posting postable subject 已由 `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 收敛为拒绝护栏。若因兼容保留，Execution Grant 必须写明兼容场景、禁止新写入、迁移退出条件和回归测试。
16G. 支付工具 application facade 的新增接口默认落 `wallet-face` 的 `com.wind.funds.wallet.application.instrument`、`com.wind.funds.wallet.application.funding` 或 `com.wind.funds.wallet.application.vcc`，实现落 `wallet-impl` 对应 `application/*/impl` 包，Request/DTO 默认落 `com.wind.funds.wallet.model.request` 和 `com.wind.funds.wallet.model.dto`。不得新增顶层 `com.wind.funds.instrument`，不得让 `transaction-impl` 依赖钱包资源服务。交易层若要完善无授权退款、拒付/争议、余额控制调账和原路径回放，必须作为账户主体型 canonical 内核补强切片独立授权，不得包装成统一支付工具交易内核；授权过期释放不再作为目标态能力，强制完成首轮 canonical 能力已作为 B4-FORCE-SETTLE 闭合。
...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片和兼容补偿；最新 ledger / wallet / transaction 生产可用 CR 结论已落成角色 Loop 任务队列；预算组控制账本兼容退出 Phase 1 已补控制视图替代入口，Phase 2 已硬禁任意 `BUDGET_GROUP` LedgerEntry 主体，Phase 3 已清理旧 profile、显式初始化、余额查询、route replay 和测试夹具旧语义；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 已补齐 wallet 授权准入对预交易快照和 Spend Control admission 的组合消费；`GSD2-LD-APPLICATION-FACADE-REBASE-001` 已回收首轮透传型 ledger application facade 和独立 fact query 服务，账本交易 / 账目分录强存在流水查询合并回 `LedgerTransactionService`，余额调账审计和授权原账本交易查询消费该基础服务；`GSD2-WALLET-SPECIAL-BUSINESS-CAPABILITY-MAP-001` 已完成 design-only 能力地图，把 VCC 预付卡、VCC 共享卡、VA 收款、全球账户付款、ACH 或银行转账事件的入口解释、系统边界、DSL 承载和 TDD Red 种子回写到 PRD / 系分 / DSL / TDD / LWT Goal；`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`、`GSD2-GA-PAYOUT-RAIL-CONTRACT-001` 和 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 均已消费并作为历史证据保留。`GSD2-B5-SPEND-RULE-DESIGN-CLOSURE-001`、`GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`、`GSD2-B5-SPEND-RULE-DECISION-CONSUME-001`、`GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001`、`GSD2-B5-SPEND-RULE-DECISION-MOVEMENT-SEPARATION-001`、`GSD2-B5-SPEND-RULE-EVIDENCE-SEMANTIC-GUARD-001`、`GSD2-B5-SPEND-RULE-DOC-BASELINE-ALIGN-001`、`SPEND-RULE-PRD-SD-TEMPLATE-ALIGN-076`、`GSD2-SERVICE-LAYER-RESPONSIBILITY-ALIGN-001`、`GSD2-SR-COMPAT-FACADE-DELEGATION-001`、`GSD2-SR-DEFINITION-VERSION-ASSIGNMENT-SERVICE-SPLIT-001` 和 `GSD2-SR-CONTROL-ACTIVITY-SERVICE-SPLIT-001` 均已形成历史证据；当前 wallet 实体基础服务标准化已完成支付工具绑定、账户层级绑定、账户读取契约、账户查询投影和闭环 CR，状态推进到 `WALLET_ENTITY_SERVICE_STANDARDIZATION_BASE_SERVICE_CLOSURE_VERIFIED`。 |
| 权威文档 | `docs/TDD设计/GSD-2-新基线工作流规划.md`、`docs/TDD设计/GSD-2-W1-基线差距审计.md`、`docs/TDD设计/GSD-2-W2-单一Grant选择卡.md`、`docs/TDD设计/GSD-2-W3-B2账户层级CAD准入草案.md`、`docs/TDD设计/GSD-2-W4-B2账户层级ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-P0P1-LedgerWalletTransaction推进计划.md`、`docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md`、`docs/TDD设计/GSD-2-B5-余额调账审计扩展ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-AUTH-Chargeback目标语义对齐任务卡.md`、`docs/TDD设计/GSD-2-AUTH-Chargeback兼容入口ExecutionGrant确认包.md` |
| 当前状态 | `LD_APPLICATION_FACADE_REBASE_GREEN_VERIFIED` |
| 本轮验证补充 | `LD-APPLICATION-FACADE-REBASE-089` 已执行 `just test-one LedgerTransactionServiceFactQueryTests tests`，2 tests 通过；受影响交易回归 `just test-one FundsBalanceAdjustAuditFlowTests,FundsAuthorizationTransactionFlowTests tests` 首次沙箱因嵌入式 Redis socket 被拒失败，非沙箱重跑 36 tests 通过；`just compile`、`just pmd` 和 `git diff --check` 通过。 |
| 本轮恢复说明 | 当前真实活跃切片以 `LD-APPLICATION-FACADE-REBASE-089` 为准；下方既有“本轮新增写入补充”保留历史恢复上下文，不作为继续扩 wallet、Spend Rule、DDL、Controller、HTTP/RPC、生产迁移或 Git 操作的授权。 |
| 本轮新增写入补充 | 在已消费交易消费支出控制活动、预算控制投影、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard、外部 confirmed credit 入金消费和 Spend Rule 服务层证据基础上，上一轮已完成 `GSD2-SERVICE-LAYER-RESPONSIBILITY-ALIGN-001`、`GSD2-SR-COMPAT-FACADE-DELEGATION-001`、`GSD2-SR-DEFINITION-VERSION-ASSIGNMENT-SERVICE-SPLIT-001`、`GSD2-SR-CONTROL-ACTIVITY-SERVICE-SPLIT-001`、`GSD2-WALLET-ENTITY-SERVICE-BINDING-SPLIT-001`、`GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-HIERARCHY-SPLIT-001` 与 `GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-READ-CONTRACT-SPLIT-001`。本轮消费 `GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-QUERY-PROJECTION-SPLIT-001`：`DefaultFundsAccountQueryServiceImpl` 通过 `FundingAccountService`、`CreditAccountService` 的 DTO / Query 契约解析资金 / 信用账户，不再直接持有 Funding / Credit Mapper、Entity 或 NameRefs，同时保持账户查询投影编排职责和 ledger 余额投影边界不变。当前不授权 DDL/H2 schema、Entity / Mapper 字段、完整规则引擎、规则表达式执行器、运营后台、审批流、Controller、HTTP/RPC、VCC facade、支付工具交易内核、真实提现/出款、外部扣账、return、NOC、reversal、信用账户还款或调额、在途、费用、清结算、对账、事件消费者、outbox、生产历史数据迁移、公共 API 删除、枚举删除或 Git 历史重写。 |
| Git / code baseline | 当前已提交 Git/code baseline 包含 `e6006888 feat: 补齐账本应用服务入口` 和 `700afeae docs: 同步账本应用服务状态`；上一批服务层基线已提交到 `4ba6cd6a feat: 补齐资金底座服务层基线`。本轮 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 和 `GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 为待提交工作树证据：新增 lifecycle 推荐入口 `InstrumentTransactionLifecycleApplicationService#authorizeByInstrument`，保留授权准入兼容入口 `authorizeByPaymentInstrument`；收款入口在委派账户主体型 `topup` 时固化脱敏支付工具快照，并要求调用方显式提供 expectedBindingVersion。`GSD2-B5-SR-CONTROL-ACTIVITY-001 / schemaDecision=ddl-backed` 的服务层最小实现已提交到 `78f7f008`，`GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已提交到 `a1397ddf`，`a5b12a3f`、`3b31d6e0`、`021ee2ce`、`4ef64275`、`da3b4f19`、`0b251593`、`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`e81a8a25` 和 `ae8cb8a6` 保留为已消费能力证据。 |
| 活跃未完成编码计划 | wallet 实体基础服务标准化 Goal 已完成当前基础服务闭环 CR：支付工具绑定当前态 / 历史、账户层级绑定当前态、资金 / 信用账户读取契约和账户查询投影切片均已收敛；预算组基础服务当前无新增代码缺口；支出主体资金责任关系的 ACTIVE 目标跨表过滤按生产分页一致性暂保留，不改为内存过滤。`GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`、`GSD2-B5-SR-CONTROL-ACTIVITY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-001` 及其并发、退款、业务场景、目标账户隔离等补片、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard 、外部 confirmed credit 入金消费、Spend Rule 决策消费、投影解释、文档基线对齐和 Spend Rule 实体基础服务标准化均已完成状态回写。旧候选不再是当前编码计划，只能作为 backlog reference。 |
| 写入范围 | 本轮写入范围为 `DefaultFundsAccountQueryServiceImpl` 账户读取依赖委派、相关服务流测试 import、LWT Goal 和本 Harness tasks；不写 DDL/H2 schema、Entity、Mapper 字段、Controller、HTTP/RPC、交易 canonical 账户主体、ledger posting、余额投影算法、运行时配置、统一支付工具交易内核、外部扣账、return、NOC、reversal、信用账户还款或调额、清结算、对账、事件消费者、outbox、生产迁移或旧公共 CRUD API 删除。 |
| 只读范围 | PRD、DSL、系分、TDD、OpenSpec、源码、测试、Justfile、最近 Git 提交和旧 GSD/Grant 历史材料。 |
| 下一 Workflow | `GSD2-B2-ACCOUNT-HIERARCHY-CONTRACT-001`、`GSD2-B2-ACCOUNT-HIERARCHY-SOURCE-CONTRACT-002`、`GSD2-B2-FR-TARGET-001`、`GSD2-B2-WALLET-APPLICATION-FACADE-001`、`GSD2-B2-WALLET-APPLICATION-FACADE-002 / B2-PI-CAP-CAD-001`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮争议退款、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B5-BALANCE-ADJUST-AUDIT-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-DIFFERENCE-MVP-001`、`GSD2-B7-RECON-DIFFERENCE-MVP-002`、`GSD2-B7-RECON-GATE-CONSUME-001`、`GSD2-B7-RECON-GATE-CONSUME-002` 和 `GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成本地 Green 和门禁收口；`GSD2-AUTH-CHARGEBACK-TARGET-ALIGN-001` 与 `GSD2-TRX-DISPUTE-RESULT-SEMANTIC-ALIGN-001` 已完成 contract/design-only 对齐。 |
| 当前默认确认包 | 当前没有可复用编码确认包；`docs/TDD设计/GSD-2-B5-SpendRule设计闭环ExecutionGrant确认包.md` 已记录 docs-only 设计闭环和 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001` 消费结果，当前不得继续复用该 Grant 扩完整规则引擎、控制活动、Controller、HTTP/RPC、事件消费 / outbox、生产迁移或 Git。`docs/TDD设计/GSD-2-Wallet特殊业务能力地图ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-LD-预算组控制账本兼容退出ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md`、`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 以及 `GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002`、`GSD2-LD-APPLICATION-FACADE-001`、`GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`、`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`、`GSD2-GA-PAYOUT-RAIL-CONTRACT-001`、`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001` 和 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 均已消费，当前不得继续复用这些 Grant。 |
| 下一候选 | 按被依赖方优先，预算组兼容退出、wallet 授权准入组合、ledger application facade、wallet 特殊业务能力地图 design-only、`authorizeByInstrument` 推荐入口契约、`receiveByInstrument` 收款快照回链、收款绑定版本必填护栏、`payOutByRail` 契约 guard、外部资金事件消费契约 guard、confirmed credit 入金消费、Spend Rule 定义契约、Spend Rule 决策消费、投影解释、服务层职责基线、兼容 facade 委派、规则定义 / 版本 / 挂载服务层拆分、控制额度变动流水服务层拆分和旧 chargeback 公共 API 删除 readiness 评估均已完成；若继续 Spend Rule，推荐从完整规则引擎、多规则决策明细持久化、公共类名 / 表名迁移、事件消费 / outbox、生产迁移或运营后台中独立确认新的单一 Grant；若改选其他方向，可在外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA/VCC 单场景、真实提现/出款内核衔接、旧 ledger 公共 CRUD API 治理、旧 chargeback 公共 API 跨仓库依赖扫描或事件消费 / outbox 专项中择一。 |
| 禁止范围 | 未确认新的单一 Execution Grant 前，不继续扩展 wallet 授权 route snapshot 回链、交易投影、余额调账查询、Spend Rule、VCC facade、清结算、P2 业务、旧 ledger 公共 CRUD API 删除 / 改签 / 权限化、transaction 公共兼容 API 改签、运行时配置、下一 Grant 的 Git add/commit/push、联网、依赖安装、生产配置或不可逆操作；B5-003 不得复用为审批、补事实、清结算或生产权限授权。 |
| 验证命令 | 已完成代码基线验证包括 `verify-cad`、B4/B5/B7 目标测试、`test-reconciliation`、`compile`、`pmd` 和 `git diff --check`；AUTH 兼容 adapter 已执行授权目标测试、transaction 分组、compile、pmd 和 diff；B4 remaining 已执行目标解释矩阵、主写投影回归、compile、pmd 和 diff；wallet 授权准入与 route snapshot 回链已执行 `just test-one AuthorizationAdmissionApplicationServiceTests tests`、`just test-one AuthorizationAdmissionApplicationServiceTests,PaymentInstrumentCapabilityApplicationServiceTests,RouteSnapshotJsonSupportTests,FundsAuthorizationTransactionFlowTests tests`、`just compile`、`just pmd`、`git diff --check` 和结构检查；账户能力来源已执行 `just test-one FundsAccountCapabilityApplicationServiceTests tests`、`just test-one FundsAccountCapabilityApplicationServiceTests,PaymentInstrumentCapabilityApplicationServiceTests,FundingResponsibilityResolutionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests`、`just compile`、`just pmd` 和 `git diff --check`；wallet 授权准入组合已执行 `just test-one AuthorizationAdmissionApplicationServiceTests tests` 4 tests、`just test-one SpendControlAdmissionApplicationServiceTests,PaymentInstrumentPreTransactionSnapshotApplicationServiceTests,PaymentInstrumentCapabilityApplicationServiceTests,FundingResponsibilityResolutionApplicationServiceTests,FundsAccountCapabilityApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 18 tests、`just compile`、`just pmd` 和 `git diff --check`；本轮 ledger 服务层职责回正计划执行 `just test-one LedgerTransactionServiceFactQueryTests tests`、`just compile`、`just pmd` 和 `git diff --check`；ledger guard 已执行 `just test-one DefaultLedgerTransactionPostingServiceImplTests,LedgerBalanceProjectionServiceImplTests,LedgerServiceImplTests tests`、`just test-ledger`、`just compile`、`just pmd` 和结构检查；预算组 posting 主体护栏已执行 `just test-one DefaultLedgerTransactionPostingServiceImplTests tests` 13 tests、`just test-one FundsBalanceControlFailureFlowTests tests` 19 tests、`just test-ledger` 28 tests、`just test-balance-control` 44 tests、`just compile`、`just pmd` 和 `git diff --check`；B5-002 已执行目标 Red/Green、`just test-one FundsBalanceAdjustAuditFlowTests tests`、`just test-balance-control`、`just compile`、`just pmd` 和 `git diff --check`；B5-003 已执行 `just test-one LedgerDtoContextVariablesContractTests tests`、`just test-one FundsBalanceAdjustAuditFlowTests tests`、`just test-balance-control`、`just test-boundary`、`just test-transaction`、`just compile` 和 `just pmd`；B7 consumer 已执行 `just test-one ClearingSettlementGateConsumerServiceTests tests`、`just test-reconciliation`、`just compile` 和 `just pmd`；B7 report 已执行 `just test-one ReconciliationDifferenceReportApplicationServiceTests tests`、`just test-reconciliation`、`just compile`、`just pmd`、`git diff --check` 和结构检查。 |
...
2026-06-21 消费 `GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002`：`AuthorizationAdmissionApplicationServiceImpl` 复用 `PaymentInstrumentPreTransactionSnapshotApplicationService` 解析工具能力、资金责任和账户能力；`AuthorizeByPaymentInstrumentRequest` 携带 Spend Rule 决策证据时显式消费 `SpendControlAdmissionApplicationService`，`REJECTED` 在账户主体型授权内核前失败。新增授权准入目标测试证明 Spend Rule 拒绝路径不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影；沙箱内目标测试因 embedded Redis 本地端口限制失败，已在非沙箱复跑 `just test-one AuthorizationAdmissionApplicationServiceTests tests` 4 tests 通过。本 Grant 不改变交易 canonical 入参，不写 Controller、HTTP/RPC、DDL/H2 schema、ledger posting、完整 Spend Rule 规则引擎、控制活动自动消费、VCC facade 或统一支付工具交易内核。

2026-06-21 消费 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001`：`InstrumentTransactionLifecycleApplicationService#authorizeByInstrument` 作为支付工具交易生命周期统一授权入口，当前只委派 `AuthorizationAdmissionApplicationService#authorizeByInstrument`，不复制授权准入、route、posting 或 ledger 逻辑；`InstrumentTransactionLifecycleApplicationServiceTests` 新增服务层委派 Red / Green，Red 先因 lifecycle 接口缺少方法编译失败，Green 后沙箱内目标测试因 embedded Redis 端口限制失败，非沙箱复跑 `just test-one InstrumentTransactionLifecycleApplicationServiceTests tests` 3 tests 通过。本切片不改变 transaction canonical 入参，不写 Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、route resolver、ledger posting、支付工具 `REFUND` 方向、VCC facade 或统一支付工具交易内核。

2026-06-21 消费 `GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001`：`InstrumentTransactionLifecycleApplicationService#receiveByInstrument` 保持委派账户主体型 `FundsDirectTransactionService#topup`，但在 `FundsTransactionTopupRequest` 上携带可选脱敏 `PaymentInstrumentRef`，由直接交易指令转换器透传到 persisted route snapshot。目标测试先 Red 证明收款 route snapshot 的 `paymentInstrumentRef` 为空，Green 后非沙箱复跑 `just test-one InstrumentTransactionLifecycleApplicationServiceTests tests` 3 tests 通过，断言工具号、工具类型、币种、状态、绑定号、绑定版本、绑定角色、内部主体和 `RECEIVE` 准入动作可审计，且不泄露外部原始工具号；收口验证 `just compile`、`just pmd` 和 `git diff --check` 均通过。本切片不改变交易 canonical 账户主体、不写 Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、ledger posting、支付工具交易内核、出款、退款、ACH return、全球账户付款或 VCC 生命周期。

2026-06-21 消费 `GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001`：`ReceiveByInstrumentRequest` 将 `expectedBindingVersion` 升为必填，`InstrumentTransactionLifecycleApplicationService#receiveByInstrument` 在解析支付工具预交易快照和委派账户主体型 `topup` 之前做前置拒绝。目标测试先 Red 证明缺 expectedBindingVersion 时会继续进入收款交易并生成资金事实；Green 后非沙箱复跑 `just test-one InstrumentTransactionLifecycleApplicationServiceTests tests` 4 tests 通过，缺版本路径拒绝且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本切片不改变交易 canonical 账户主体、不写 Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、ledger posting、支付工具交易内核、出款、退款、ACH return、全球账户付款或 VCC 生命周期。

2026-06-21 消费 `GSD2-GA-PAYOUT-RAIL-CONTRACT-001`：新增 `PayOutByRailRequest` 和 `InstrumentTransactionLifecycleApplicationService#payOutByRail`，用于承载全球账户出款 rail 的服务层业务入口字段，包括支付工具引用、出款账户主体、金额、币种、rail、收款人引用、外部出款流水、业务流水和期望绑定版本。目标测试先 Red 证明缺 `PayOutByRailRequest` 时编译失败；Green 后非沙箱复跑 `just test-one InstrumentTransactionLifecycleApplicationServiceTests tests` 5 tests 通过，当前 `payOutByRail` 在账户主体型提现/出款内核前 fail-fast，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本切片不接真实提现/出款、不生成在途、费用、清结算或对账事实，不实现 SWIFT/local/ACH 协议，不改变交易 canonical 账户主体、不写 Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、ledger posting、支付工具交易内核、ACH return 或 VCC 生命周期。

2026-06-21 消费 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001`：新增 `ExternalFundsEventApplicationService#consume` 和 `ConsumeExternalFundsEventRequest`，用于承载 ACH、银行文件、渠道回调或第三方钱包回调进入资金域前的外部事件流水、事件类型、目标资金 / 信用账户主体、金额、币种、原交易引用、对账差异引用和业务流水。目标测试先 Red 证明缺 `ExternalFundsEventApplicationService`、`ConsumeExternalFundsEventRequest` 和 impl 时编译失败；Green 后非沙箱复跑 `just test-one ExternalFundsEventApplicationServiceTests tests` 1 test 通过，当前 `consume` 在归一资金事实内核前 fail-fast，且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本切片不接真实 ACH/银行事件消费、不生成入账、扣账、退款、撤销、调账、差异单、清结算或对账事实，不实现 Nacha/ODFI/RDFI/银行文件协议，不改变交易 canonical 账户主体、不写 Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、ledger posting、事件消费者、outbox 或生产迁移。

2026-06-22 消费 `GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001`：在既有 `ExternalFundsEventApplicationService#consume` 契约上，支持 `ACH_CREDIT_CONFIRMED`、`BANK_CREDIT_CONFIRMED`、`EXTERNAL_CREDIT_CONFIRMED` 到资金账户的 confirmed credit 消费，内部委派标准 `FundsDirectTransactionService#topup`。目标测试先 Red 证明旧 guard 会在交易内核前失败；Green 后非沙箱复跑 `just test-one ExternalFundsEventApplicationServiceTests tests` 3 tests 通过，confirmed credit 入金会增加目标资金账户 `AVAILABLE`，并生成 TOPUP 资金交易、route snapshot、posting plan、LedgerTransaction 和 LedgerEntry；未授权 debit 事件和信用账户目标在交易内核前失败且无资金 / 账本副作用。本切片不接外部扣账、return、NOC、reversal、信用账户还款或调额、差异单、补事实、清结算、对账、事件消费者、outbox、银行文件协议、Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、交易 canonical 入参或 ledger posting 改造。

2026-06-25 消费 `GSD2-WALLET-EXTERNAL-FUNDS-RAIL-SEMANTICS-001`：新增 wallet application 层 rail 语义解析支持，`receiveByInstrument` 不再直接用 `FundsTransactionChannel.valueOf` 解释业务 `channelCode`，而是支持 `BANK_RAIL`、`ACH`、`wire-transfer` 等钱包支持的 rail / channel 别名并归一为当前交易层稳定渠道 `WIRE_TRANSFER`；`ExternalFundsEventApplicationService#consume` 对 confirmed credit 事件类型做大小写 / 横线归一，并显式映射出 `ACH_RAIL`、`BANK_RAIL` 或 `EXTERNAL_RAIL` 写入 route snapshot `externalAccountRef.providerCode`。目标测试覆盖未知 rail 友好拒绝、未授权事件拒绝、收款 route snapshot providerCode 和 confirmed credit 入金 route snapshot providerCode；`WalletExternalFundsRailSupportTests` 4 tests、`InstrumentTransactionLifecycleApplicationServiceTests` 6 tests、`ExternalFundsEventApplicationServiceTests` 3 tests 通过。本切片不接真实外部扣账、return、NOC、reversal、信用账户还款或调额、真实出款、清结算、对账、事件消费者、outbox、Controller、HTTP/RPC、DDL/H2 schema、Entity、Mapper、交易 canonical 入参或 ledger posting 改造。
...
| 现有覆盖 | 当时 `FundsAuthorizationTransactionFlowTests` 已覆盖部分完成后过期、过期超剩余失败、撤销超剩余失败、退款/chargeback 超已完成失败，以及 reversal、settle、refund、chargeback 幂等摘要冲突；这些只证明顺序路径和幂等，不证明并发竞争。2026-06-26 后，过期和独立 chargeback 覆盖只作为历史证据，不再属于目标态回归面。 |
...
| 首批 Red | `B4-RACE-RED-001`：同一授权的 settle 与可信 reversal，必要时 settle 与 settleRefund 并发竞争时，只能有一个合法金额迁移获胜，失败方无 route、posting、ledger entry、projection 或余额变化；历史版本曾包含 settle 与 expire 竞争，已被 2026-06-26 目标态移除裁决覆盖。 |
...
| 完成内容 | 新增 `B4-RACE-RED-001` 对应的授权后继并发 flow 覆盖，证明同一授权的 settle / reversal 等可信后继事件并发竞争只有一个赢家；失败方无 route、posting、ledger entry、projection 或余额变化。历史版本曾覆盖 settle / expire / reversal 竞争；2026-06-26 后 expire 路径已退出目标态。实现层在授权后继命令进入编排前按原授权流水串行化，并在事务完成前持有 JVM 锁；读取授权原交易时使用 `FOR UPDATE` 行锁，完成和撤销继续校验剩余可迁移金额。 |
...
- [x] 2026-06-22 完成 `GSD2-B5-SPEND-RULE-DESIGN-CLOSURE-001` docs-only 任务落地：新增 `docs/TDD设计/GSD-2-B5-SpendRule设计闭环ExecutionGrant确认包.md`，并同步 PRD、DSL、系分、TDD README、LWT Goal、P0/P1 推进计划和 docs README。当前 Spend Rule 设计闭环收敛为 `SpendRuleDefinition`、`SpendRuleVersion`、`SpendRuleAssignment` 和 `SpendRuleDecisionLog` 四层，字段命名采用 `ruleId` / 历史 `sn` 作为稳定系统标识、`ruleCode` 作为业务可读配置编码。`SpendControlActivity` 保留为既有控制事实能力，本轮不继续扩展、不删除、不回滚；本任务不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、VCC facade、事件消费 / outbox、生产迁移或 Git。后续若进入编码，推荐重新确认 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`，并先证明规则版本不可原地覆盖、规则挂载不得输出控制对象为资金责任主体、规则拒绝不得生成 route/posting/LedgerEntry。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`：新增 Spend Rule 定义、版本、挂载和决策日志最小服务层契约、Request/DTO、core 枚举、wallet impl Entity/Mapper/Service、H2 schema 和 `SpendRuleDefinitionApplicationServiceTests`。目标测试证明已发布版本同摘要幂等、异摘要拒绝覆盖，挂载支付工具 / 预算组只记录控制 scope 且不输出资金责任主体，规则拒绝只记录决策日志且不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本 Grant 不授权完整规则引擎、运营后台、控制活动扩展、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、VCC facade、事件消费 / outbox、生产迁移或 Git push；后续若继续 Spend Rule，推荐重新确认 `GSD2-B5-SPEND-RULE-DECISION-CONSUME-001` 或 `GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001`。
- [x] 2026-06-22 完成 `GSD2-B5-SPEND-RULE-DESIGN-DOC-SPLIT-001` docs-only 任务落地：新增 `docs/产品设计/09-SpendRule支出规则产品设计.md` 和 `docs/系分设计/06-SpendRule支出规则系分设计.md`，并同步 docs README、产品 README、系分 README、TDD README、GSD-2 工作流、LWT Goal、P0/P1 推进计划和 OpenSpec tasks。Spend Rule 产品语义正式从主线交易路由文档拆出，独立承接规则定义、不可变版本、规则挂载、决策日志、控制活动、预算控制视图、生产启用门禁和验收；主线 02 文档只保留交易、路由、钱包、账本和投影如何消费规则证据。本任务不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、VCC facade、事件消费 / outbox、生产迁移或 Git push。
- [x] 2026-06-22 完成 `GSD2-B5-SPEND-RULE-SPEC-LOCK-001` docs-only 自主交付控制卡落地：新增 `docs/TDD设计/GSD-2-B5-SpendRule自主交付控制卡.md`，并同步产品设计 09、系分设计 06、DSL README、TDD README 和 Spend Rule 设计闭环确认包。Spend Rule DSL v1.1 锁定 `SpendRuleVersionSpec`、`SpendRuleAssignmentSpec`、`SpendRuleDecisionEvidenceSpec` 三类契约，字段组包括 `display`、`matchSpec`、`counterSpec`、`limitSpec`、`decisionSpec`、`safetySpec`、`evaluatedRules`、`decisionPolicy` 和 forbidden facts，覆盖单笔限额、周期金额限额、次数限额、MCC / 商户 / 国家地区 / 时间窗口和多规则裁决可解释性。本任务不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、事件消费、生产迁移、Git 提交或完整规则引擎；后续进入编码前必须重新确认 `GSD2-B5-SPEND-RULE-DECISION-CONSUME-001`、`GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001` 或等价单一 Grant。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-DECISION-CONSUME-001`：`SpendControlAdmissionApplicationService` 已将 Spend Rule 决策证据接入 `SpendRuleDefinitionApplicationService#recordDecision`，固化 rule、version、assignment、scope、instrument、action、amount、currency、business、decision result、reject reason 和 decision digest；`AuthorizationAdmissionApplicationService` 已透传支付工具授权请求中的 assignment / scope / decision evidence。目标测试覆盖准入通过写决策日志、准入拒绝写拒绝决策日志、同决策流水同摘要幂等复用、同决策流水异摘要拒绝，以及授权入口 Spend Rule 拒绝时无资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。沙箱内目标测试因 embedded Redis 端口绑定受限失败，沙箱外复跑 `just test-one SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 9 tests。本 Grant 不授权完整规则引擎、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push；后续不得复用本 Grant 扩展投影解释、规则计算或生产发布。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001`：交易投影解释采用“自解释已固化事实”方案，不接入 `wind-script`。`AuthorizationAdmissionApplicationService` 在 Spend Rule 准入通过后把最小 `spendRuleDecision` 快照写入授权交易上下文；`FundsTransactionProjectionExplanationSource` 只读解析 allow-list 字段，输出 `spendRule:*`、`spendRuleVersion:*`、`spendRuleAssignment:*`、`spendRuleDecision:*`、`spendRuleDecisionLog:*` 证据和解释 payload。目标测试先在非沙箱环境复现 Red：交易成功但投影解释缺少 Spend Rule 决策证据；Green 后 `just test-one AuthorizationAdmissionApplicationServiceTests tests` 通过 5 tests，且证明解释查询不新增资金、route、posting、LedgerEntry、账本交易或余额事实，不输出 ruleSpec 或 script。本 Grant 不授权完整规则引擎、`wind-script` 接入、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push；后续若要执行规则 DSL 或接入 `wind-script`，必须另起单一 Grant。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-MODULE-OWNERSHIP-GUARD-001`：按产品和架构结论固化 Spend Rule 模块归属：规则定义、版本、挂载、决策日志、准入、控制活动和预算控制视图归属于 `wallet` 支出控制域；`transaction` 只消费已固化 `spendRuleDecision` 快照、控制活动引用和 route snapshot 做历史投影解释；`ledger` 只接受可入账账户主体。本轮同步 PRD、系分、DSL 和 TDD 控制卡，并新增架构边界守卫，扫描 `transaction-*` 生产源码不得 import / 注入 wallet Spend Rule application service、规则 Entity 或 Mapper。验证证据：`just test-one FundsModuleDependencyBoundaryTests tests` 通过 6 tests；`just test-boundary` 通过 174 tests；`git diff --check` 通过。本 Grant 不授权完整规则引擎、`wind-script` 接入、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-SERVICE-HARDEN-001`：按角色自主推进控制，从已完成定义契约、决策消费、投影解释和模块归属守卫之后选择低风险服务层硬化切片。新增支付工具范围一致性校验：当 `RecordSpendRuleDecisionLogRequest.scopeType = PAYMENT_INSTRUMENT` 时，`instrumentSn` 必须存在并等于 `scopeId`，避免投影、对账和审计出现“控制范围为支付工具但工具引用缺失或错配”的决策证据。本轮补 `SpendRuleDefinitionApplicationServiceTests` Red/Green，证明缺失或错配支付工具号会拒绝，且失败不产生资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。验证证据：沙箱内目标测试因 embedded Redis 端口绑定受限失败；沙箱外先复现 Red，再复跑 `just test-one SpendRuleDefinitionApplicationServiceTests tests` 通过 4 tests；`just test-one SpendRuleDefinitionApplicationServiceTests,SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 14 tests。本 Grant 不授权完整规则引擎、`wind-script` 接入、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-SERVICE-HARDEN-002`：按角色自主推进控制，继续选择 Spend Rule 定义服务的低风险硬化切片。新增决策结果一致性校验：当 `RecordSpendRuleDecisionLogRequest.decisionResult != REJECTED` 时，不允许携带 `rejectReason`，避免交易投影解释、客服审计和对账报告出现“通过但有拒绝原因”的矛盾证据。本轮补 `SpendRuleDefinitionApplicationServiceTests` Red/Green，先证明原实现允许矛盾证据，再修复为 fail closed，并断言失败不写决策日志、不产生资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。验证证据：沙箱内目标测试因 embedded Redis 端口绑定受限失败；沙箱外先复现 Red，再复跑 `just test-one SpendRuleDefinitionApplicationServiceTests tests` 通过 5 tests；`just test-one SpendRuleDefinitionApplicationServiceTests,SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 15 tests。本 Grant 不授权 `assignmentSn` 强必填公共契约收紧、完整规则引擎、`wind-script` 接入、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-ASSIGNMENT-CONTRACT-002`：按角色自主推进控制，补齐 Spend Rule 挂载生产合同。`AssignSpendRuleVersionRequest`、`SpendRuleAssignmentDTO`、`SpendRuleAssignment` 和 H2 `t_spend_rule_assignment` 已新增 `conflictPolicy`、`effectiveFrom`、`effectiveTo`；`SpendRuleDefinitionApplicationServiceImpl` 强制挂载必须携带冲突策略和有效期、结束时间必须晚于开始时间，决策日志引用 `assignmentSn` 时必须命中当前有效挂载。目标测试覆盖缺冲突策略、缺生效开始时间、缺生效结束时间、非法时间窗口、未来生效挂载和已过期挂载均被拒绝，且失败不写决策日志、不产生资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。验证证据：非沙箱先复现 Red：缺少 `SpendRuleConflictPolicy` 导致 `SpendRuleDefinitionApplicationServiceTests` 编译失败；Green 后 `just test-one SpendRuleDefinitionApplicationServiceTests tests` 通过 7 tests；`just test-one SpendRuleDefinitionApplicationServiceTests,SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 17 tests。本 Grant 不授权完整规则引擎、`wind-script` 接入、冲突裁决执行器、运营后台、控制活动重构、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-ASSIGNMENT-QUERY-EXPLAIN-001`：补齐 Spend Rule 挂载查询和挂载可用性解释能力。`SpendRuleDefinitionApplicationService` 新增 `queryAssignments` 和 `explainAssignment`；wallet-face 新增 `SpendRuleAssignmentQuery`、`SpendRuleAssignmentExplainQuery`、`SpendRuleAssignmentExplanationDTO`；core 新增 `SpendRuleAssignmentExplanationStatus`；wallet-impl 只读实现按 tenant、assignment、rule、version、scope、status 和有效窗口过滤，并解释 `EFFECTIVE`、`NOT_YET_EFFECTIVE`、`EXPIRED`、`DISABLED` 状态及 evidenceRefs。目标测试覆盖当前有效挂载查询、未来生效 / 已过期挂载解释，并证明查询 / 解释不写决策日志、不产生资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。验证证据：非沙箱先复现 Red：缺少 Query / DTO 导致 `SpendRuleDefinitionApplicationServiceTests` 编译失败；Green 后 `just test-one SpendRuleDefinitionApplicationServiceTests tests` 通过 9 tests。本 Grant 不授权完整规则引擎、`wind-script` 接入、冲突裁决执行器、交易 canonical 入参、ledger posting、Controller、HTTP/RPC、事件消费 / outbox、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-DECISION-MOVEMENT-SEPARATION-001`：按角色自主推进控制，收紧 Spend Rule 决策记录和控制额度变动流水的新写入边界。`SpendControlActivityApplicationService#recordActivity` 已拒绝新的 `ADMISSION_RECORDED` / `REJECTED_RECORDED` 写入，准入通过和拒绝证据必须通过 `SpendRuleDefinitionApplicationService#recordDecision` 固化；`SpendControlActivityApplicationServiceTests` 不再依赖准入服务构造活动输入，目标测试证明历史兼容决策活动类型误入控制额度变动流水入口时 fail-fast，且不产生资金交易、route、posting、LedgerEntry、账本交易或余额投影副作用。本 Grant 不删除历史枚举、表字段或存量数据，不做公共类名 / 表名 / DTO / Mapper 迁移，不授权完整规则引擎、Controller、HTTP/RPC、事件消费 / outbox、交易 canonical 入参、ledger posting、生产迁移、VCC facade 或 Git push。
- [x] 2026-06-22 消费 `GSD2-B5-SPEND-RULE-EVIDENCE-SEMANTIC-GUARD-001`：按角色自主推进控制，将 Spend Rule 决策记录和控制额度变动流水的兼容期语义分类固化到 `SpendControlActivityType` 枚举契约。枚举统一声明 `productSemantic`、预算投影参与性、调额类、释放类和决策记录兼容类；`SpendControlActivityApplicationServiceImpl` 消费枚举分类方法，不再在实现类中维护私有类型集合；`SpendControlTransactionConsumptionApplicationServiceTests` 并发夹具改为代理 application service，避免测试越过服务层直接依赖 DAL mapper。新增 `SpendControlActivityTypeContractTests` 证明 `ADMISSION_RECORDED` / `REJECTED_RECORDED` 是决策记录兼容类型且不参与预算控制投影，其余控制额度变动类型统一解释为 `SpendControlMovement`，调额类和释放类仅是控制额度变动流水子集。本轮验证发现默认 Maven in-process 编译可能产出 `Unresolved compilation` 错误桩，可信验证采用 forked javac reactor 目标测试和 classfile 扫描；后续需单独固化构建产物完整性门禁。本 Grant 不做公共类名、表名、DTO、Mapper、枚举删除或历史数据迁移，不授权 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-BUILD-CLASSFILE-INTEGRITY-GUARD-001`：按角色自主推进控制，将构建产物完整性风险固化为本地验证门禁。`Justfile` 新增 `verify-classfiles`，扫描所有 Maven 模块的 `target/classes` 和 `target/test-classes`，若发现 `Unresolved compilation` 错误桩则失败；`compile`、`test-one`、`test-module`、`verify-fast`、`verify-cad` 和 `install-snapshots` 统一使用 forked javac 并在构建或测试后执行 classfile 扫描。TDD README 和 LWT Goal Ledger 同步该门禁，后续 Spend Rule 和资金主链路代码切片不得只凭 Maven 成功状态声明编译产物可信。本 Grant 不改业务服务层、公共契约、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移或 Git push。
- [x] 2026-06-23 完成 `GSD2-B5-SPEND-RULE-DOC-BASELINE-ALIGN-001` docs-only 基线对齐：同步 PRD、系分、DSL 和 TDD，将 Spend Rule 最终交付口径收敛为产品语义 `SpendRuleDecisionRecord` / `SpendControlMovement`，当前代码兼容名 `SpendRuleDecisionLog` / `SpendControlActivity`；系分按现有 Entity、Request 和 H2 schema 校准 `ruleSpec / ruleDigest`、`assignmentSn`、`decisionSn`、`activitySn`、单条决策记录、控制额度变动流水和预算控制投影；DSL 明确 v1.1 JSON 仍为 `DOC_ONLY`，`evaluatedRules`、`decisionPolicy`、`finalDecision`、`requestDigest` 尚未作为独立机器契约或数据库字段落地；TDD 补当前服务层测试证据矩阵和 Not Done。本文档对齐不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费、生产迁移或 Git push。
- [x] 2026-06-23 完成 `GSD2-SERVICE-LAYER-RESPONSIBILITY-ALIGN-001` docs-only 基线对齐：同步 Spend Rule PRD、系分、TDD README、LWT Goal 和 OpenSpec tasks，将产品能力对象与工程服务类型解耦。当前目标服务层按基础服务和场景 application service 收敛：基础服务承接单对象事实、Mapper 访问、幂等和只读解释；application service 只承接跨对象用例编排。后续不再新增旧式 `DomainService` / `DomainQueryService`、兼容 facade 或只做委派的浅服务。本任务不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费、生产迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-SR-COMPAT-FACADE-DELEGATION-001`：按当前服务层基线完成 Spend Rule 决策记录服务层收敛。新增 `SpendRuleVersionService`、`SpendRuleAssignmentService`、`SpendRuleDecisionLogService` 及实现；只有基础服务直接访问 Mapper，application service 不直接访问 Mapper。`SpendControlAdmissionApplicationService` 改为依赖决策记录基础服务，避免准入场景继续依赖规则定义大 facade。新增 `SpendRuleDecisionLogServiceTests` 覆盖决策记录幂等、窄查询和无资金副作用；回归 `SpendRuleDefinitionApplicationServiceTests`、`SpendControlAdmissionApplicationServiceTests` 和 `AuthorizationAdmissionApplicationServiceTests`。验证证据已在 LWT Goal 记录。本 Grant 不授权 DDL/H2 schema、Entity、Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-SR-DEFINITION-VERSION-ASSIGNMENT-SERVICE-SPLIT-001`：按当前服务层基线继续把 Spend Rule 规则定义、版本发布、规则挂载和挂载查询 / 解释收敛到标准基础服务。新增 `SpendRuleDefinitionService`，`SpendRuleVersionService`、`SpendRuleAssignmentService` 扩展基础创建 / 查找 / 查询能力；只有基础服务直接访问 Mapper，application service 不直接访问 Mapper。`SpendRuleDefinitionApplicationService` 保留公共兼容契约并委派规则定义、版本发布、规则挂载、挂载查询 / 解释和决策记录能力。新增 `SpendRuleDefinitionServiceTests` 覆盖版本幂等、摘要漂移拒绝、挂载幂等、挂载查询 / 解释只读和非法挂载失败无资金副作用；回归准入和授权组合测试。验证证据已在 LWT Goal 记录。本 Grant 不授权 DDL/H2 schema、Entity / Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-LD-FACE-MUTATION-BOUNDARY-GUARD-001`：Ledger face 仍保留余额直改、账本删除、账本交易更新 / 删除等历史资源型写接口；本轮新增 `FundsModuleDependencyBoundaryTests` 架构边界守卫，禁止 `transaction`、`wallet`、`reconciliation`、`governance` 生产源码直接调用这些危险方法。首次 `just test-one FundsModuleDependencyBoundaryTests tests` 中 JUnit 7 tests 已通过，但 `verify-classfiles` 命中旧 `target/classes` 的 `Unresolved compilation` 残留；随后执行 forked javac `clean compile` 重新生成产物，复跑目标测试 7 tests、`verify-classfiles` 和 `git diff --check` 均通过。后续如需删除或迁移 public API，必须另起兼容退出 Grant。本任务不授权 Controller、HTTP/RPC、交易 canonical 入参、ledger posting 语义重写、DDL/H2 schema 或生产迁移。
- [x] 2026-06-23 消费 `GSD2-SR-CONTROL-ACTIVITY-SERVICE-SPLIT-001`：按当前服务层基线完成控制额度变动流水服务层收敛。新增 `SpendControlActivityService` 及实现，由基础服务承接控制额度变动流水写入、查询和预算控制投影；`BudgetControlLimitAdjustmentApplicationService` 与 `SpendControlTransactionConsumptionApplicationService` 组合该基础服务完成预算调额和交易后控制消费。验证证据：目标组合回归 44 tests、`compile`、`pmd` 和 `git diff --check` 通过。本 Grant 不授权 DDL/H2 schema、Entity / Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名 / 表名迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-WALLET-ENTITY-SERVICE-BINDING-SPLIT-001`：按项目标准基础服务统一 `PaymentInstrumentBinding` 与 `PaymentInstrumentBindingHistory` 的服务层职责。新增 `PaymentInstrumentBindingService`、`PaymentInstrumentBindingHistoryService`、`UpdatePaymentInstrumentBindingRequest`、`RecordPaymentInstrumentBindingHistoryRequest` 及实现；`PaymentInstrumentServiceImpl` 不再直接访问绑定 Mapper 或绑定历史 Mapper，只保留支付工具实体管理、绑定业务校验和兼容编排。验证证据：`PaymentInstrumentServiceImplTests` 28 tests、`verify-classfiles` 和 `just pmd` 均在隔离工作副本通过；主工作区存在 IDE/JDT 旧 `target/classes` 错误桩污染，已通过 forked javac `clean compile` 确认生产代码可编译。本 Grant 不授权 DDL/H2 schema、Entity / Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、统一支付工具交易内核、VCC facade、完整规则引擎、事件消费 / outbox、生产迁移、公共类名 / 表名迁移或 Git push。
- [x] 2026-06-24 消费 `GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-HIERARCHY-SPLIT-001`：新增 `AccountHierarchyBindingDTO`、`AccountHierarchyBindingService`、`AccountHierarchyBindingServiceImpl` 和 converter，把 `AccountHierarchyBinding` 当前态创建、ACTIVE 查询和唯一性判断收敛到标准基础服务；`AccountHierarchyServiceImpl` 不再直接访问 `AccountHierarchyBindingMapper`，继续保留账户存在性 / 状态 / 币种校验和 route snapshot 解析职责。本切片不改 Funding/Credit account 解析语义、不新增历史窗口、不改 DDL/H2 schema、Entity、Mapper 字段、Controller、HTTP/RPC、交易 canonical 入参或 ledger posting。验证证据：`just test-one AccountHierarchyBindingServiceImplTests tests` 在隔离工作副本通过 2 tests；`just test-one FundsAuthorizationTransactionFlowTests tests` 在隔离工作副本通过 32 tests；`just pmd` 在隔离工作副本通过；主工作区同一命令测试本体通过但 `verify-classfiles` 命中既有 IDE/JDT 旧 `target/classes` 错误桩。
- [x] 2026-06-24 消费 `GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-READ-CONTRACT-SPLIT-001`：按项目标准基础服务统一资金账户 / 信用账户读取契约和账户解析依赖。`FundingAccountService`、`CreditAccountService` 新增按租户和账号读取方法；`FundingAccountQuery` 新增平台账户过滤；`PlatformFundingAccountServiceImpl` 通过 `FundingAccountService` 查询平台资金账户，并在返回路由主体时使用 `FundsSubjectType.FUNDING_ACCOUNT`；`SpendSubjectFundingRelationServiceImpl` 和 `AccountHierarchyServiceImpl` 不再直接持有 Funding / Credit Mapper。本切片不改 DDL/H2 schema、Entity、Mapper 字段、Controller、HTTP/RPC、交易 canonical 入参、ledger posting 或生产迁移；`DefaultFundsAccountQueryServiceImpl` 查询投影和资金责任关系跨表过滤另起切片评估。验证证据：隔离工作副本 `just test-one FundingAccountServiceImplTests,PlatformFundingAccountServiceImplTests,SpendSubjectFundingRelationServiceImplTests,AccountHierarchyBindingServiceImplTests tests` 通过 30 tests；隔离工作副本 `just test-one FundsTransferPayWithdrawChainFlowTests tests` 通过 1 test；隔离工作副本 `just pmd` 通过；主工作区 `git diff --check` 通过，`just compile` javac reactor 成功但最终 classfile 扫描命中既有 IDE/JDT 旧 `target/classes` 错误桩。
- [x] 2026-06-24 消费 `GSD2-WALLET-ENTITY-SERVICE-ACCOUNT-QUERY-PROJECTION-SPLIT-001`：按项目标准基础服务统一账户查询投影服务的账户读取边界。`DefaultFundsAccountQueryServiceImpl` 不再直接依赖 Funding / Credit Mapper、Entity 或 NameRefs，改为通过 `FundingAccountService`、`CreditAccountService` 的 DTO / Query 契约解析资金账户和信用账户；该服务仍保留账户主体到余额投影查询的编排职责，不改账本余额投影算法和 ledger 查询边界。本切片不改 DDL/H2 schema、Entity、Mapper 字段、Controller、HTTP/RPC、交易 canonical 入参、ledger posting 或生产迁移；预算组基础服务和资金责任关系跨表过滤另起切片评估。验证证据：主工作区 `just test-one FundsSubjectBalanceQueryServiceImplTests tests` 测试本体通过 12 tests，但最终 classfile 扫描命中既有 IDE/JDT 旧 `target/classes` 错误桩；隔离工作副本 `just test-one FundsSubjectBalanceQueryServiceImplTests tests` 通过 12 tests；隔离工作副本 `just test-one FundsSubjectBalanceQueryServiceImplTests,FundsAccountCapabilityApplicationServiceTests,LedgerBalanceProjectionServiceImplTests,DefaultLedgerTransactionPostingServiceImplTests,FundingAccountServiceImplTests tests` 通过 43 tests；隔离工作副本 `just pmd` 通过；主工作区 `git diff --check` 通过。
- [x] 2026-06-24 完成 `GSD2-WALLET-ENTITY-SERVICE-STANDARDIZATION-CLOSURE-001`：对 `com.wind.funds.wallet.dal.entities` 到服务层的基础服务映射做闭环 CR。13 个 wallet 实体均已有对应基础服务或明确组合服务承接；领域写 / 读服务不直接持有 Mapper。`BudgetGroupServiceImpl` 当前已符合预算控制对象基础服务边界，不建账、不做预算重算；`SpendSubjectFundingRelationServiceImpl` 保留 Funding / Credit 账户 ACTIVE 目标跨表子查询，是为了维持分页 count、排序和 route 候选过滤的一致性，不在本轮改成内存过滤。本闭环不授权 BudgetGroup public API 删除、Spend Rule 公共命名迁移、DDL/H2 schema、Entity、Mapper 字段、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、生产迁移或 Git。
- [x] 2026-06-24 消费 `GSD2-WALLET-ENTITY-CONTRACT-HARDEN-001`：按本轮 wallet 实体 CR 修复生产契约坏味道。H2 schema 和系分将支付工具绑定、资金责任关系、交易明细、冻结单、账本和余额投影相关账户主体引用统一到 `varchar(64)`；`PaymentInstrumentServiceImpl` 支持支付工具绑定创建 / 更新按 `requestSn` 同请求幂等重放回读既有绑定，字段漂移时拒绝；Spend Rule 定义、决策记录、控制额度变动流水和相关 application 入口统一校验 digest 必须使用 `sha256:` 前缀；`AccountHierarchyBinding`、DTO 和测试移除无实质并发或历史意义的 `version` 字段。验证证据：`just compile` 通过；`just test-one AccountHierarchyBindingServiceImplTests,PaymentInstrumentServiceImplTests,SpendSubjectFundingRelationServiceImplTests,SpendRuleDigestValidatorTests tests` 通过 52 tests。本 Grant 不授权 Controller、HTTP/RPC、交易 canonical 入参、支付工具 `REFUND` 方向、ledger posting、生产迁移、public API 删除、完整规则引擎或 Git push；clean-build 生成源码链路仍建议另起 Build/Codegen guard。
- [x] 2026-06-24 消费 `GSD2-BUILD-CODEGEN-CLEAN-COMPILE-GUARD-001`：补齐 clean build 与注解生成链路本地门禁。`Justfile` 新增 `clean-compile` 和 `verify-codegen`，从空 `target` 执行 forked javac reactor 编译后，串联 classfile 错误桩扫描和代表性 MyBatis-Flex `NameRefs` / MapStruct converter 生成类存在性检查；`verify-cad` 改为先跑 `clean-compile`，避免只验证增量编译。本任务不改业务代码、公共契约、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎或生产迁移。验证证据：`just clean-compile`、`just pmd`、`just verify-cad` 和 `git diff --check` 通过。
- [ ] 2026-06-23 计划 `GSD2-SR-PUBLIC-NAME-MIGRATION-001`：若要把当前代码兼容名 `SpendRuleDecisionLog` / `SpendControlActivity` 迁移为产品语义名 `SpendRuleDecisionRecord` / `SpendControlMovement`，需另起兼容迁移 Grant，覆盖公共类名、DTO / Request、表名或字段名、H2 schema、调用方清单、回滚策略和发布窗口；不得复用本轮服务层标准化 Grant。
- [x] 2026-06-24 消费 `GSD2-CORE-FUNDS-SUBJECT-TYPE-SEMANTIC-GUARD-001`：`FundsSubjectType.BUDGET_GROUP` 保留兼容期枚举，但通过 `isLedgerPostable` / `isLedgerPostableName` 明确为不可入账控制视图；RouteLeg、PostingPlan、DSL JSON verifier、route amount closure 和 ledger posting 已统一消费该语义，补充 core DSL 与 ledger posting 回归测试。本任务未删除枚举、未改历史数据、未触碰 Controller、HTTP/RPC、交易 canonical 入参或生产迁移。
- [x] 2026-06-24 消费 `GSD2-WALLET-APPLICATION-FACADE-CALLING-SURFACE-001`：docs-only 整理 wallet application 调用面。系分 4.1.2 已明确新业务优先走 `InstrumentTransactionLifecycleApplicationService` 或 `ExternalFundsEventApplicationService`；支付工具能力、资金责任、账户能力、预交易快照、Spend Rule 准入、交易后控制消费和预算额度调整等专项 application service 只用于内部组合、只读解析或交易后控制事实消费。当前不新增更窄 facade，不处理 Controller、HTTP/RPC、交易 canonical 入参、ledger posting 或生产迁移。
- [x] 2026-06-24 完成 `GSD2-WALLET-SERVICE-LAYER-COMPLETION-ALIGN-001` docs-only 对齐：系分和 LWT Goal 已明确 wallet 基础服务与 application service 完成度。基础服务覆盖资金账户、信用账户、预算组、平台账户、支付工具、绑定、绑定历史、资金责任、账户层级、余额查询、账目 profile、Spend Rule 定义 / 版本 / 挂载 / 决策记录 / 控制额度变动流水；application service 覆盖资金责任解析、账户能力、支付工具能力、预交易快照、授权准入、支付工具生命周期、外部确认入金、Spend Rule 准入、交易后控制消费和预算额度调额。当前不新增旧式 `DomainService` / `DomainQueryService` 或更窄 facade；真实出款、外部扣账、return/NOC/reversal、VCC 专项 facade、事件消费 / outbox 和生产迁移仍需另起单一 Grant。
- [x] 2026-06-24 消费 `GSD2-LWT-ENTITY-SERVICE-SQL-ALIGN-CR-001`：对 ledger / wallet / transaction 实体层、基础服务和 H2 SQL 做 CR。源码扫描未发现 `DomainService` / `DomainQueryService` / `InMemory` / `Fake` / `Mock` 生产服务残留；wallet 基础服务分层保持当前完成态；实体和 SQL 字段复扫后补齐 `t_ledger.status` 到 `Ledger`、`LedgerDTO`、`LedgerQuery`、查询条件和 MapStruct 映射，并在 `LedgerServiceImplTests` 断言默认 `ACTIVE`。验证证据：实体 / SQL 字段复扫无输出；`just test-one LedgerServiceImplTests tests` 通过 4 tests；`git diff --check`、`just verify-classfiles` 和 `just pmd` 通过。本任务不新增 `LedgerStatus` 枚举，不删除 Ledger face 历史 update / delete 公共契约，不改 DDL/H2 schema、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、生产迁移或 Git push。
- [x] 2026-06-25 对齐 `GSD2-TRX-DISPUTE-RESULT-SEMANTIC-ALIGN-001`（旧口头别名：`GSD2-TRX-COMPAT-DEBT-001`）：撤销“处理 chargeback 兼容债 = 新增或完善 chargeback 交易能力”的读法。chargeback / dispute 只作为案件过程；交易层只消费裁决后的资金结果：需要退款时走 `settleRefund / AUTH_REFUND` 并保留争议原因、凭证、外部案件引用和审计，无资金影响时不调用资金交易入口。既有 `FundsAuthorizationTransactionService#chargeback` 曾只作为历史兼容方法保留，该口径已被 2026-06-26 移除裁决覆盖；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 只保留为历史 guard 证据。
- [x] 2026-06-25 消费 `GSD2-TRX-CHARGEBACK-ENTRY-DEPRECATION-001` 首轮退役治理：曾仅对旧 public API `FundsAuthorizationTransactionService#chargeback` 和 `FundsAuthorizationTransactionChargebackRequest` 增加弃用标注，并把 OpenSpec 口径收敛为交易层只消费争议裁决后的资金结果。该首轮 deprecated 兼容态已被 2026-06-26 目标态移除裁决覆盖；完整争议运营、representment、清结算追偿、VCC processor 和外部规则仍归属 P2/B7 专项。
- [x] 2026-06-25 消费 `GSD2-TRX-QUERY-OVERLOAD-FAILFAST-001`：收敛 `FundsTransactionQueryService#sumConsumedReplayLegAmount` 6 参数 default overload 的误用风险，默认实现遇到 `excludedBusinessScene / excludedBusinessSn` 时 fail-fast，避免未显式支持排除能力的实现静默忽略排除参数；生产实现 `DefaultFundsTransactionQueryService` 保持显式支持排除参数，未改 route replay、账务行为、DDL/H2 schema、Controller、HTTP/RPC 或运行时配置。
- [x] 2026-06-25 锁定 `GSD2-TRX-CHARGEBACK-PUBLIC-API-EXIT-PLAN-001` design-only 退出计划：当时旧 `chargeback` 公共 API 保持 deprecated 兼容，不物理删除；后续计划分三步推进。该计划已被 2026-06-26 用户裁决覆盖：独立 `chargeback` 公共 API、请求模型、事件枚举和 route replay 分支与 `expire` 一起进入移除；跨仓库兼容、发布窗口或历史解释如有需要另起迁移/发布专项。
- [x] 2026-06-25 完成 `GSD2-TRX-CHARGEBACK-USAGE-SCAN-001` 只读扫描：旧 `chargeback` 公共 API 的生产调用集中在 `transaction` 内部公共契约、实现、converter、route replay、posting、lifecycle 和 projection explanation 兼容链路；未发现 `wallet`、`ledger`、`reconciliation`、`governance` 生产代码直接调用 `authorizationTransactionService.chargeback(...)`，也未发现 Controller / HTTP / RPC / OpenAPI 暴露文件。测试仍通过 `FundsAuthorizationTransactionFlowTests`、`FundsTransactionProjectionExplainApplicationServiceTests` 和请求上下文契约测试保留兼容入口回归。下一步若继续治理，只能先确认 `GSD2-TRX-CHARGEBACK-TEST-MIGRATION-001`；物理删除仍被阻断，直到测试迁移、历史解释回归、发布兼容和单独删除 Grant 完成。
- [x] 2026-06-25 消费 `GSD2-TRX-CHARGEBACK-TEST-MIGRATION-001` 首轮：将授权链“超额争议资金结果失败无副作用”测试从旧 `chargeback` 公共入口迁移到目标态 `settleRefund` 争议字段，继续断言失败不得借用其他交易沉淀在 `SETTLEMENT` 的余额、不得生成资金交易、route、posting、LedgerEntry 或余额变化。历史兼容 `chargeback` 正向、缺外部争议引用 guard、投影解释和请求上下文契约测试当时继续保留；2026-06-26 移除裁决后，这些兼容回归不再作为目标态测试资产。
- [x] 2026-06-25 完成 `GSD2-TRX-CHARGEBACK-TEST-MIGRATION-001` 剩余测试裁决：当时剩余直接调用旧 `chargeback` 的测试统一命名为 `CompatChargeback`，明确保留为历史兼容正向、最小审计 guard、幂等摘要冲突和投影解释回归。2026-06-26 移除裁决覆盖该裁决：不再保留独立 `chargeback` 入口测试，争议资金结果测试统一走 `settleRefund` 争议字段。
- [x] 2026-06-25 完成 `GSD2-TRX-CHARGEBACK-PUBLIC-API-REMOVE-READINESS-001` design-only 评估：当时旧 `chargeback` 公共 API 删除 readiness 结论为 `DELETE_READINESS_NOT_READY`。2026-06-26 用户重新裁决后，该 readiness 结论作废，当前状态改为 `SUPERSEDED_BY_20260626_REMOVAL_DECISION`：独立 `chargeback` 公共 API、请求模型、`CHARGEBACK` eventType、route replay phase、lifecycle saver、ledger posting assembler 和 projection explain 兼容链路进入移除；争议资金结果保留 `settleRefund / AUTH_REFUND` 可区分审计语义。跨仓库兼容、发布窗口、历史数据只读解释或迁移公告如有需要另起专项。
- [x] 2026-06-26 消费 `GSD2-AUTH-EXPIRE-CHARGEBACK-REMOVAL-001`：用户确认 `expire` 是不可信错误状态，系统不需要支持；独立 `chargeback` 与 `expire` 一起进入移除。目标态删除 `FundsAuthorizationTransactionService#expire`、`FundsAuthorizationTransactionService#chargeback`、对应请求模型、`EXPIRE` / `CHARGEBACK` 事件、`EXPIRED` 终态、route replay 过期 / chargeback 分支、ledger posting / lifecycle / projection 兼容分支和目标测试中的旧入口回归。保留 `settleRefund` 争议字段作为 chargeback / dispute 裁决后资金结果承接入口；授权过期不生成 route、posting、LedgerEntry、余额或交易事实。代码闭环已在 `811d7501 refactor: 移除授权过期和拒付独立入口` 提交固化；验证证据为 `just compile`、`just pmd`、`just test-one FundsAuthorizationTransactionFlowTests tests`、`just test-one DefaultRouteReplayServiceTests tests`、`just test-one FundsTransactionProjectionExplainApplicationServiceTests tests`、`just test-one FundsTransactionRequestContextVariablesContractTests tests` 和 `git diff --check` 通过，旧符号残留扫描无输出。
- [x] 2026-06-25 完成 `GSD2-WALLET-CAPABILITY-SURFACE-CLEANUP-001` docs-only 任务基线：将 wallet application 能力闭环 CR 结论回写到 LWT Goal、P0/P1 推进计划和 Wallet 特殊业务能力确认包。当前 wallet application 是条件可用基线：业务入口优先 `InstrumentTransactionLifecycleApplicationService` 和 `ExternalFundsEventApplicationService`，专项 admission / snapshot / control service 只用于内部组合或交易后控制消费；`payOutByRail` 仍是 guard-only，`ExternalFundsEventApplicationService#consume` 当前只支持 confirmed credit 到资金账户。随后已消费 `GSD2-WALLET-EXTERNAL-FUNDS-RAIL-SEMANTICS-001`，下一轮不再把 rail 语义作为默认推荐 Grant，改为评估支付工具绑定唯一性、真实出款、外部扣账 / return / NOC / reversal、信用账户事件、VCC / VA 单场景。本轮 cleanup 本身未改 Java、测试、DDL/H2 schema、公共契约、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、事件消费者、outbox 或生产迁移。
- [x] 2026-06-25 消费 `GSD2-WALLET-EXTERNAL-FUNDS-RAIL-SEMANTICS-001`：收敛 `receiveByInstrument` 与 `ExternalFundsEventApplicationService#consume` 的 rail / channel / confirmed credit 事件映射语义，钱包层负责业务 rail 和事件类型解释，交易层继续只消费账户主体和稳定 `FundsTransactionChannel`；目标测试 13 tests 通过，收口继续以 `compile`、`pmd` 和 `git diff --check` 为门禁。
- [x] 2026-06-25 消费 `GSD2-WALLET-INSTRUMENT-BINDING-UNIQUENESS-001` 第一阶段：支付工具绑定创建 / 变更 `requestSn` 升级为 face 必填契约和服务层强校验，避免调用方绕过绑定历史唯一键与 replay 逻辑。新增 `PaymentInstrumentServiceImplTests` 缺流水创建 / 变更失败用例，断言失败不写绑定当前态、不追加绑定历史、不产生账本事实副作用；目标测试 `just test-one PaymentInstrumentServiceImplTests tests` 通过 34 tests。本阶段不授权默认绑定 DB 级并发唯一键、H2 schema、MySQL 迁移、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、真实出款、外部扣账 / return / NOC / reversal、信用账户事件、VCC facade、事件消费者或 outbox；后续若继续并发唯一硬化必须另起单一 Grant。
- [x] 2026-06-25 消费 `GSD2-WALLET-INSTRUMENT-BINDING-DEFAULT-GUARD-001`：默认支付工具绑定补齐 DB 级并发保护。新增 `t_payment_instrument_binding_guard`、`PaymentInstrumentBindingGuard`、`PaymentInstrumentBindingGuardMapper` 和内部并发 guard 组件；创建或变更 ACTIVE 默认绑定时按 tenant、instrument、binding role、currency 和 guard type 获取唯一 guard 行 `FOR UPDATE`，再执行有效窗口重叠判断和当前态 / 历史写入；并发下只允许一个默认绑定成功，非重叠未来默认绑定继续允许。目标测试 `just test-one PaymentInstrumentServiceImplTests tests` 通过 35 tests。本阶段不改 Controller、HTTP/RPC、交易 canonical 入参、ledger posting、真实出款、外部扣账 / return / NOC / reversal、信用账户事件、VCC facade、事件消费者、outbox 或生产 MySQL 迁移。
- [x] 2026-06-25 纠偏默认绑定 guard 表 DDL 落点：项目约定 `tests/src/test/resources/jdbc-schema.sql` 是 H2/MySQL Mode 测试表结构来源，不在系分目录单独保留生产 SQL。已删除误加的 `docs/系分设计/production-ddl` 目录和引用，`t_payment_instrument_binding_guard`、主键和 `uk_payment_instrument_binding_guard_scope` 继续以测试 schema、Entity、Mapper 和服务层 guard 口径为准。本阶段不执行生产数据库迁移，不改 Java、测试、H2 schema、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、真实出款、外部扣账 / return / NOC / reversal、信用账户事件、事件消费者或 outbox；真实生产迁移必须另起 DBA / 发布 Grant。
