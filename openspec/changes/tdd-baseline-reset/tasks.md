...
| 活跃未完成编码计划 | 当前没有可复用的活跃编码 Grant；`GSD2-B5-SR-CONTROL-ACTIVITY-001` 已完成首轮本地 Green / Verify 并提交到 `78f7f008`，预算组非建账已提交到 `a5b12a3f`，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成本地 Green / Verify / Commit，`GSD2-B5-SR-TRANSACTION-CONSUME-CONCURRENCY-001` 已完成本地 Red / Green / Verify，`GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-RELEASE-GUARD-001` 和 `GSD2-B5-SR-TRANSACTION-CONSUME-REFUND-FACT-GUARD-002` 已完成本地目标回归和状态回写。旧候选不再是当前编码计划，只能作为 backlog reference；若继续推进，必须重新选择新的单一 Grant。 |
