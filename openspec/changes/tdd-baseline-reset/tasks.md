...
- [x] 已冻结设计基线、OpenSpec 基线、Harness Plan 和旧测试清理结果已作为独立检查点冻结；上一设计冻结点为 30b1a00 docs: 冻结权益快照设计基线。当前代码能力基线截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`，文档与任务准备基线以本文件当前版本为准；后续生产编码仍需用户按 MVP 任务切片单独授予 Execution Grant。
...
1. 设计基线、OpenSpec 基线、Harness Plan 和旧测试清理结果必须先冻结为独立检查点；上一设计冻结点为 30b1a00 docs: 冻结权益快照设计基线，当前代码能力基线截至 `77bc9f4 fix: 阻断钱包上下文权益核心事实`，文档与任务准备基线以本文件当前版本为准；后续具体 MVP 编码任务仍必须通过 Execution Grant 明确授权。
...
| 后续基线收敛 | 请求摘要、稳定摘要支撑、账务计划装配器长 ID、预算组默认周期 `LIFETIME`、账务事实断言、钱包/交易/治理边界、CAD 完整验证门禁、出款准入规则核验证据、编码准入设计冻结、提现/解冻红线、余额日志证据、路由事实边界、交易投影解释、权益回放摘要、治理重放差异校验、资金事实红线、敏感上下文阻断、上下文不可变、防御性拷贝、外部账户原文阻断、MVP/归档边界和设计交付口径已在后续提交中收敛，当前仓库代码基线截至 `77bc9f4`。 | `75b46ef` 至 `5901265` 的历史基线证据，以及 `5901265..77bc9f4` 的资金事实红线、敏感上下文和设计交付口径收敛证据。 |
...
基线是否已冻结：代码能力基线截至 77bc9f4；当前 docs/OpenSpec/Harness 工作树变更提交或被 Execution Grant 明确列为本任务附件后，才能作为 B2 冻结开工基线；本任务启动前必须复核工作树状态
工作树状态：执行前必须复核；dirty 时未列入允许纳入范围的变更不得作为 Done 证据
