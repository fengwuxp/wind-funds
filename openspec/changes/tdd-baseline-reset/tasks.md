...
当前执行优先级按模块或能力划分：账本账目 > 钱包 > 交易层。支付工具、VCC 和全球账户只作为最后一组支持能力；收单能力仅做设计、边界复核和差距登记，不做实现。B1 至 B8 只保留为 TDD 覆盖索引，不表示任一编号优先于上述队列，也不表示任何索引可以绕过独立 Execution Grant。
...
| 顺位 | 队列 | 任务范围 | 当前口径 |
| --- | --- | --- | --- |
| 1 | 账本账目 | ledger account、账目、posting、ledger entry、余额投影、清结算对账对象、账本余额快照和治理证据。 | 最高优先级；下一轮优先做 Round 0 或单一 Execution Grant 准备。 |
| 2 | 钱包 | 钱包账户、内部钱包入口、资金责任解析、钱包 application facade、账户能力和失败无副作用。 | 排在交易层之前；支付工具只作为钱包的后置支持能力，不代表钱包主优先级。 |
| 3 | 交易层 | A1 直接交易、账户主体型授权交易、余额控制、route replay、交易投影和交易投影重投影。 | 已闭合 B4 任务只作为回归基线；新增交易层能力仍需单一 Execution Grant。 |
| 4 | 支付工具支持 | `B2-PI-CAP-CAD-001`、`B4-AUTH-PI-CAD-001`、`B5-SR-CONTROL-CAD-001`、`B6-B8-PI-VIEW-CAD-001`。 | 放到最后的支持队列；越级选择时必须说明 P0/P1 依赖已满足。 |
| 5 | VCC 与全球账户支持 | `P2-VCC-PREPAID-CAD-001`、`P2-VCC-LIFECYCLE-CAD-001`、`P2-GA-INBOUND-CAD-001`、`P2-GA-OUTBOUND-CAD-001`、`P2-GA-FX-FEE-CAD-001`。 | P2 业务专项支持队列；不属于默认编码入口。 |
| 6 | 收单 | `P2-ACQ-CAPTURE`、`P2-ACQ-DISPUTE`、商户清结算和 PCI/外部规则边界。 | `DESIGN_ONLY_NOT_CODE_CANDIDATE`；不写实现、不写 Red 测试、不确认 Execution Grant。 |
...
| 2026-06-04 B2-PI-CAP 后置支持候选 | 支付工具资源服务已能证明工具、绑定、绑定历史、状态、方向、币种、生效窗口、敏感字段阻断和无账务副作用局部基线；`PaymentInstrumentDirection` 当前只有 `RECEIVE`、`PAYMENT`、`BOTH`，未固化 RECEIVE、PAY、AUTHORIZE、REFUND、WITHDRAW 五类动作能力，且没有 `PaymentInstrumentCapabilityApplicationService` 或等价 application facade。 | `B2-PI-CAP-CAD-001` 只作为支付工具支持队列的单一 Grant 确认输入，整体排在账本账目、钱包基础能力和交易层之后；首批 Red 为 `R0-PI-001`，只处理工具能力准入和不可变准入快照。未确认前不得写 application facade、Request/DTO、动作能力枚举、DDL/H2 schema、测试或实现；不得混入 B4 授权准入、B2-FR 目标主体迁移、Spend Rule、预算控制、投影、治理或 P2。 |
