...
| 当前状态 | `WALLET_AUTH_ADMISSION_COMPOSE_GREEN_VERIFIED` |
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片、控制活动幂等语义一致性守卫补片、退款引用原交易业务一致性守卫补片、预算控制投影目标账户隔离守卫补片和释放上限目标账户隔离守卫补片；ledger posting 主体白名单守卫与兼容补偿已证明 `BUDGET_GROUP` 资金价值类分录不得作为外部 `LedgerTransactionSpec` 的 posting / LedgerEntry 主体，失败发生在 ledger transaction、posting plan、LedgerEntry 和余额投影前；预算组控制账本兼容退出已消费 Phase 1 控制视图替代、Phase 2 posting 硬禁和 Phase 3 旧账本兼容清理，任意 `BUDGET_GROUP` LedgerEntry 主体均拒绝，旧余额控制预算组调额入口前置拒绝且无资金或账本事实副作用，旧 profile / 显式初始化 / 余额查询 / route replay 不再把预算组当 ledger subject；wallet 授权准入组合已消费预交易快照与 Spend Control admission，Spend Rule 拒绝时在交易内核前失败且无资金、route、posting、LedgerEntry、账本交易或余额投影副作用。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产历史数据迁移、公共 API 删除、枚举删除或 Git 历史重写。 |
