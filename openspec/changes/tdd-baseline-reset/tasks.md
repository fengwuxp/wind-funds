...
- [x] 2026-06-25 对齐 `GSD2-TRX-DISPUTE-RESULT-SEMANTIC-ALIGN-001`（旧口头别名：`GSD2-TRX-COMPAT-DEBT-001`）：撤销“处理 chargeback 兼容债 = 新增或完善 chargeback 交易能力”的读法。chargeback / dispute 只作为案件过程；交易层只消费裁决后的资金结果：需要退款时走 `settleRefund / AUTH_REFUND` 并保留争议原因、凭证、外部案件引用和审计，无资金影响时不调用资金交易入口。既有 `FundsAuthorizationTransactionService#chargeback` 只作为历史兼容方法保留，`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001` 已完成最小审计 guard，不再作为后续默认计划。
- [x] 2026-06-25 消费 `GSD2-TRX-CHARGEBACK-ENTRY-DEPRECATION-001` 首轮退役治理：仅对旧 public API `FundsAuthorizationTransactionService#chargeback` 和 `FundsAuthorizationTransactionChargebackRequest` 增加弃用标注，并把 OpenSpec 口径收敛为交易层只消费争议裁决后的资金结果。未删除公共方法、未改 `CHARGEBACK` event、route replay、投影解释、账务行为、DDL/H2 schema、Controller、HTTP/RPC 或运行时配置；完整争议运营、representment、清结算追偿、VCC processor 和外部规则仍归属 P2/B7 专项。
...
- [x] 2026-06-25 锁定 `GSD2-TRX-CHARGEBACK-PUBLIC-API-EXIT-PLAN-001` design-only 退出计划：旧 `chargeback` 公共 API 当前保持 deprecated 兼容，不物理删除；后续分三步推进，先确认 `GSD2-TRX-CHARGEBACK-USAGE-SCAN-001` 扫描生产源码、测试、文档、OpenSpec 和外部暴露面，再做目标态测试迁移和历史 `CHARGEBACK` 事实解释回归，最后才允许以单独 Grant 删除、隐藏或移动旧入口。本轮未改 Java、测试、DDL/H2 schema、事件语义、route replay、投影解释、Controller、HTTP/RPC 或运行时配置。
...
- [x] 2026-06-25 消费 `GSD2-TRX-CHARGEBACK-TEST-MIGRATION-001` 首轮：将授权链“超额争议资金结果失败无副作用”测试从旧 `chargeback` 公共入口迁移到目标态 `settleRefund` 争议字段，继续断言失败不得借用其他交易沉淀在 `SETTLEMENT` 的余额、不得生成资金交易、route、posting、LedgerEntry 或余额变化。历史兼容 `chargeback` 正向、缺外部争议引用 guard、投影解释和请求上下文契约测试继续保留；公共 API 删除仍被剩余测试裁决、历史解释回归、发布兼容和显式删除 Grant 阻断。
- [x] 2026-06-25 完成 `GSD2-TRX-CHARGEBACK-TEST-MIGRATION-001` 剩余测试裁决：剩余直接调用旧 `chargeback` 的测试统一命名为 `CompatChargeback`，明确保留为历史兼容正向、最小审计 guard、幂等摘要冲突和投影解释回归，不再作为目标态交易能力入口。公共 API 删除仍需单独评估发布兼容、跨模块依赖影响、回滚策略和显式删除 Grant。
- [x] 2026-06-25 完成 `GSD2-TRX-CHARGEBACK-PUBLIC-API-REMOVE-READINESS-001` design-only 评估：旧 `chargeback` 公共 API 删除 readiness 结论为 `DELETE_READINESS_NOT_READY`。阻断原因是 `transaction-face` 仍是跨模块公共 Java 契约，fincone、fincone-issuing、nobe、SDK、RPC/HTTP 适配层等外部依赖尚未扫描；历史 `CHARGEBACK` event、route replay、lifecycle saver、ledger posting assembler 和 projection explain 仍需保留解释链路；版本策略、废弃窗口、迁移公告和回滚步骤尚未确认。后续若继续治理，优先选择 `GSD2-TRX-CHARGEBACK-PUBLIC-API-DEPENDENCY-SCAN-001`；物理删除必须另起显式删除 Grant。
