...
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片，新增 `SpendControlTransactionConsumptionApplicationService`、`CONSUMED` / `REFUND_COMPENSATED` 控制活动语义、原活动 / 原交易回链和 `consumedAmount` 投影解释；当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| 活跃未完成编码计划 | 当前没有可复用的活跃编码 Grant；`GSD2-B5-SR-CONTROL-ACTIVITY-001` 已完成首轮本地 Green / Verify 并提交到 `78f7f008`，预算组非建账已提交到 `a5b12a3f`，`GSD2-B5-SR-TRANSACTION-CONSUME-001` 已完成本地 Green / Verify / Commit。旧候选不再是当前编码计划，只能作为 backlog reference；若继续推进，必须重新选择新的单一 Grant。 |
...
| 当前默认确认包 | 当前无可复用默认确认包；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，状态 `SR_TRANSACTION_CONSUME_GREEN_VERIFIED_COMMITTED`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`。 |
