...
| 本轮新增权威补充 | `docs/TDD设计/GSD-2-LWT-生产可用能力Goal.md` 已新增 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 消费记录；B7 差异报告已提交到 `a1397ddf`，本轮支出控制准入快照已完成本地 Green / Verify。 |
...
| 本轮新增写入补充 | 支出控制准入 face / impl / DTO / request / enum / 目标测试、TDD README、docs README、LWT Goal、W5 推进计划、GSD-2 工作流入口和本 Harness tasks 的状态同步；不授权 Spend Rule 规则定义、决策日志持久化、Spend Control Activity、预算控制投影、VCC facade、支付工具交易内核、Controller、HTTP/RPC、DDL/H2 schema、生产迁移或 Git 历史重写。 |
...
| 活跃未完成编码计划 | `EMPTY`。旧候选不再是当前编码计划，只能作为 backlog reference；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮争议退款、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001`、`GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001`、`GSD2-B2-SPEND-CONTROL-ADMISSION-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002`、`GSD2-B5-BALANCE-ADJUST-AUDIT-003`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、`GSD2-B7-RECON-CLEARING-SETTLEMENT-CONSUMER-SERVICE-001` 和 `GSD2-B7-RECON-DIFFERENCE-REPORT-001` 已消费；下一轮不得复用 B5-003、B7 对象级 Gate、B7 consumer、B7 report、account capability、pre-transaction snapshot 或 spend control admission Grant，需重新选择完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或其他单一 Grant。 |
...
| 下一候选 | 若继续编码，只能在完整 Spend Rule 控制活动 / 预算控制投影、B7 报告扩展、VCC facade 或其他 wallet 后续能力中重新确认单一 Grant。Spend Rule、VCC、全球账户和 P2 业务仍后置。 |
