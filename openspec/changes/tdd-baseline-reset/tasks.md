...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片和兼容补偿；最新 ledger / wallet / transaction 生产可用 CR 结论已落成角色 Loop 任务队列；预算组控制账本兼容退出 Phase 1 已补控制视图替代入口，Phase 2 已硬禁任意 `BUDGET_GROUP` LedgerEntry 主体，Phase 3 已清理旧 profile、显式初始化、余额查询、route replay 和测试夹具旧语义；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 已补齐 wallet 授权准入对预交易快照和 Spend Control admission 的组合消费；`GSD2-LD-APPLICATION-FACADE-001` 已补齐 ledger 跨模块生产默认 application facade，并提交到 `e6006888`；本轮 `GSD2-WALLET-SPECIAL-BUSINESS-CAPABILITY-MAP-001` 已完成 design-only 能力地图，把 VCC 预付卡、VCC 共享卡、VA 收款、全球账户付款、ACH 或银行转账事件的入口解释、系统边界、DSL 承载和 TDD Red 种子回写到 PRD / 系分 / DSL / TDD / LWT Goal。LWT Goal、OpenSpec spec 和本 tasks 已把当前状态推进到 `WALLET_SPECIAL_BUSINESS_CAPABILITY_MAP_READY_TO_CONFIRM`。 |
...
| 当前默认确认包 | 当前没有可复用编码确认包；`docs/TDD设计/GSD-2-Wallet特殊业务能力地图ExecutionGrant确认包.md` 已作为 design-only 能力地图完成，不授权 Java、测试、DDL/H2 schema 或公共契约。`docs/TDD设计/GSD-2-LD-预算组控制账本兼容退出ExecutionGrant确认包.md` 已消费 Phase 1、Phase 2 和 Phase 3，状态 `BUDGET_GROUP_LEDGER_COMPAT_CLEANUP_GREEN_VERIFIED`；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫状态为 `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`；`GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已消费并提交到 `a613eff5` / `80cd1461`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 与 `GSD2-LD-APPLICATION-FACADE-001` 均已消费，当前不得继续复用这些 Grant。 |
| 下一候选 | 按被依赖方优先，预算组兼容退出、wallet 授权准入组合、ledger application facade 和 wallet 特殊业务能力地图 design-only 均已完成；推荐下一个单一候选为 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-CONTRACT-001`，仅选择 `authorizeByInstrument`、`receiveByInstrument`、`payOutByRail` 或外部事件消费中的一个服务层契约 / Red 切片。也可改选 VCC 预付卡充值提现、VA 收款、`GSD2-TRX-COMPAT-DEBT-001`、旧 ledger 公共 CRUD API 治理、完整 Spend Rule 规则引擎或事件消费 / outbox 专项。 |
