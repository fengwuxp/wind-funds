...
| Git / code baseline | 当前已提交 Git/code baseline 为 `4ef64275 feat: 补齐余额调账独立审计查询`；`GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已完成本地 Red / Green / Verify 并提交固化；`da3b4f19 feat: 补齐余额调账路由审计回链` 是进入 B5-003 前的已提交基线；`0b251593 feat: 补齐账本正常余额方向护栏` 是进入 B5-002 route snapshot 审计回链切片前的已提交基线；`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 已完成本地 Red / Green 并提交固化为能力证据；`dd442888`、`ea8f8800`、`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`e81a8a25` 和 `ae8cb8a6` 保留为已消费能力证据，`da7d2ea test: 阻断契约夹具承载资金流断言` 和 `b3b9712 feat: 对齐资金底座GSD基线与交易回放能力` 只保留为 GSD-2 历史证据。 |
| 活跃未完成编码计划 | `EMPTY`。旧候选不再是当前编码计划，只能作为 backlog reference；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮争议退款、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001`、`GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001`、`GSD2-LD-LEDGER-GUARD-REGRESSION-001`、`GSD2-B5-BALANCE-ADJUST-AUDIT-002` 和 `GSD2-B5-BALANCE-ADJUST-AUDIT-003` 已消费；下一轮不得复用 B5-003，需重新确认 B7 清算/结算 gate、B7 差异报告、wallet 完整预交易快照或其他单一 Grant。 |
...
| 下一候选 | 默认建议从 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 或 B7 差异报告中重新确认单一 Grant。若选择清算/结算 gate，需先在 `docs/TDD设计/GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md` 确认阻断粒度：沿用当前 `blockingScope` 类型级消费，即 `scopeDecision=type-scope-no-schema`，还是把差错侧升级为 `blockingObjectType / blockingObjectSn` 对象级命中键，即 `scopeDecision=object-scope-schema-backed`。2026-06-19 源码 Gap Audit 已确认 `gateObjectSn` 当前只存在于 gate request / decision，差错登记、DTO、Entity、H2 schema 和 Mapper 仍没有阻断对象字段；生产可用方向推荐对象级 schema-backed，类型级方案只能作为 `CONSERVATIVE_MVP_NOT_FULLY_USABLE`。对象级命中需要新的公共契约、DDL/H2 schema、Entity、Mapper 和 Red 授权；兼容规则必须同时证明对象精确命中、同类型不同对象不误阻断、历史类型级差错保守阻断。若继续 ledger guard，只能另起新的 guard 扩展 Grant；若继续 wallet，只能在完整预交易快照、账户能力来源组合、Spend Rule 控制闭环或 VCC facade 中重新确认单一 Grant。Spend Rule、VCC、全球账户和 P2 业务仍后置。 |
