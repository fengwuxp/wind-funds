...
| 本轮验证补充 | `LD-APPLICATION-FACADE-TENANT-CONSUME-088` 已执行 `just test-one LedgerApplicationFacadeTests tests` 2 tests 通过、`just verify-classfiles`、交易模块 ledger 资源服务直接依赖扫描、`git diff --check` 和 `just pmd`。同步复跑 `just test-one LedgerServiceImplTests tests` 4 tests 通过，验证共享 H2 schema 初始化和 ledger status 对齐仍可用。 |
| 本轮恢复说明 | 当前真实活跃切片以 `LD-APPLICATION-FACADE-TENANT-CONSUME-088` 为准；下方既有“本轮新增写入补充”保留历史恢复上下文，不作为继续扩 wallet、Spend Rule、DDL、Controller、HTTP/RPC、生产迁移或 Git 操作的授权。 |
