...
| 本轮新增权威补充 | `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 已完成服务层补片和兼容补偿；LWT Goal、OpenSpec spec 和本 tasks 已把当前状态从释放上限目标账户隔离守卫推进到预算组资金价值分录阻断与控制账本兼容入账。 |
...
| 当前状态 | `LEDGER_BUDGET_GROUP_CONTROL_POSTING_COMPAT_GREEN_VERIFIED` |
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片、控制活动幂等语义一致性守卫补片、退款引用原交易业务一致性守卫补片、预算控制投影目标账户隔离守卫补片和释放上限目标账户隔离守卫补片；本轮新增 ledger posting 主体白名单守卫与兼容补偿，证明 `BUDGET_GROUP` 资金价值类分录不得作为外部 `LedgerTransactionSpec` 的 posting / LedgerEntry 主体，失败发生在 ledger transaction、posting plan、LedgerEntry 和余额投影前；预算组 `CONTROL` 类 `LIMIT`、`AVAILABLE`、`AUTHORIZATION` 控制账本仍可用于余额控制兼容路径。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、预算组 route / 余额查询 / 余额控制兼容退出、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
...
| 当前默认确认包 | 当前无可复用默认确认包；`docs/TDD设计/GSD-2-B5-交易消费支出控制活动ExecutionGrant确认包.md` 已被消费并新增执行记录，交易消费、并发幂等、退款释放守卫、退款事实守卫、退款补偿入口守卫、退款已消费引用守卫、业务场景一致性守卫、成功消费业务流水一致性守卫、失败释放业务流水一致性守卫、退款引用净消费上限守卫、退款引用已消费一致性守卫、交易事实累计解释金额守卫、控制活动幂等语义一致性守卫、退款引用原交易业务一致性守卫、预算控制投影目标账户隔离守卫和释放上限目标账户隔离守卫状态为 `SR_TRANSACTION_RELEASE_TARGET_ACCOUNT_GUARD_GREEN_VERIFIED`；本轮 `GSD2-LD-BUDGET-GROUP-POSTING-GUARD-001` 状态为 `LEDGER_BUDGET_GROUP_CONTROL_POSTING_COMPAT_GREEN_VERIFIED`；`docs/TDD设计/GSD-2-B5-SpendRule控制活动与预算投影ExecutionGrant确认包.md` 已从确认包进入首轮实现并完成本地 Green / Verify / Commit，状态 `SR_CONTROL_ACTIVITY_GREEN_VERIFIED_COMMITTED`。 |
| 下一候选 | 暂无默认推荐；若要推进 B7 报告扩展、VCC facade、完整 Spend Rule 规则引擎、事件消费 / outbox、生产迁移或其他 wallet 后续能力，必须另起新的单一 Grant。 |
