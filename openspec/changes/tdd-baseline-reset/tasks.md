...
| 当前状态 | `SPEND_RULE_PRD_SD_TEMPLATE_ALIGNED` |
| 本轮新增写入补充 | 在已消费交易消费支出控制活动、预算控制投影、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard、外部 confirmed credit 入金消费和 Spend Rule 服务层证据基础上，上一轮已消费 `GSD2-B5-SPEND-RULE-DOC-BASELINE-ALIGN-001`：同步 PRD、系分、DSL 和 TDD，将产品语义收敛为 `SpendRuleDecisionRecord` / `SpendControlMovement`，当前代码兼容名保留 `SpendRuleDecisionLog` / `SpendControlActivity`；系分按当前 Entity、Request 和 H2 schema 校准 `ruleSpec / ruleDigest`、`assignmentSn`、`decisionSn`、`activitySn`、单条决策记录和控制额度变动流水；DSL 明确 v1.1 JSON 仍为 `DOC_ONLY`，`evaluatedRules`、`decisionPolicy`、`finalDecision`、`requestDigest` 未作为独立机器契约或数据库字段落地；TDD 补当前服务层测试证据矩阵。本轮继续补强模板合规：09 PRD 新增文档状态、PRD 模板追踪、背景问题证据、需求链、可衡量成功指标、金融四流、专业确认和产品到架构 / TDD 交接；06 系分新增文档状态、系分模板追踪、产品语义输入、业务驱动追踪、设计视图、方案取舍、方法级接口契约、数据设计模板项、质量属性、兼容迁移 / 灰度回滚、Runbook、业务驱动验证和参考资料。当前不授权 Java、测试、DDL/H2 schema、完整规则引擎、规则表达式执行器、运营后台、审批流、Controller、HTTP/RPC、VCC facade、支付工具交易内核、真实提现/出款、外部扣账、return、NOC、reversal、信用账户还款或调额、在途、费用、清结算、对账、事件消费者、outbox、生产历史数据迁移、公共 API 删除、枚举删除或 Git 历史重写。 |
...
| 活跃未完成编码计划 | 当前没有可复用的活跃编码 Grant；`GSD2-B5-SPEND-RULE-DEFINITION-CONTRACT-001`、`GSD2-B5-SR-CONTROL-ACTIVITY-001`、`GSD2-B5-SR-TRANSACTION-CONSUME-001` 及其并发、退款、业务场景、目标账户隔离等补片、预算组兼容退出、wallet 授权准入组合、ledger application facade、支付工具生命周期入口、收款快照、出款 rail 契约 guard 、外部 confirmed credit 入金消费、Spend Rule 决策消费、投影解释和本轮文档基线对齐均已完成状态回写。旧候选不再是当前编码计划，只能作为 backlog reference；若继续推进 Spend Rule，必须从完整规则引擎、多规则决策明细持久化、事件消费 / outbox、生产迁移或运营后台等新候选中重新选择单一 Grant；若不选 Spend Rule，则可在外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA 收款场景、VCC 单场景、真实提现/出款内核、transaction 兼容债、旧 ledger 公共 CRUD API 治理、事件消费 / outbox 或生产迁移中另选单一 Grant。 |
| 写入范围 | 本轮写入范围为 Spend Rule PRD、系分、DSL、TDD README、LWT Goal 和本 Harness tasks；不写 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 账户主体、ledger posting、运行时配置、统一支付工具交易内核、外部扣账、return、NOC、reversal、信用账户还款或调额、清结算、对账、事件消费者、outbox、生产迁移或旧公共 CRUD API 删除。 |
...
| 下一候选 | 按被依赖方优先，预算组兼容退出、wallet 授权准入组合、ledger application facade、wallet 特殊业务能力地图 design-only、`authorizeByInstrument` 推荐入口契约、`receiveByInstrument` 收款快照回链、收款绑定版本必填护栏、`payOutByRail` 契约 guard、外部资金事件消费契约 guard、confirmed credit 入金消费、Spend Rule 定义契约和 Spend Rule 决策消费均已完成；若继续 Spend Rule，推荐下一个单一候选为 `GSD2-B5-SPEND-RULE-PROJECTION-EXPLAIN-001`、完整规则引擎、事件消费 / outbox 或生产迁移；若改选其他方向，可在外部扣账 / return / NOC / reversal、信用账户还款或调额、新 VA/VCC 单场景、真实提现/出款内核衔接、`GSD2-TRX-COMPAT-DEBT-001`、旧 ledger 公共 CRUD API 治理或事件消费 / outbox 专项中择一。 |
