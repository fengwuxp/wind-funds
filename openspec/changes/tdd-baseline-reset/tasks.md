...
| 下一候选 | 默认建议从 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001` 或 B7 差异报告中重新确认单一 Grant。若选择清算/结算 gate，需先在 `docs/TDD设计/GSD-2-B7-清算结算Gate消费ExecutionGrant确认包.md` 确认阻断粒度：沿用当前 `blockingScope` 类型级消费，即 `scopeDecision=type-scope-no-schema`，还是把 `gateObjectSn` 升级为对象级差错命中键，即 `scopeDecision=object-scope-schema-backed`；对象级命中需要新的公共契约、DDL/H2 schema、Entity、Mapper 和 Red 授权。若继续 ledger guard，只能另起新的 guard 扩展 Grant；若继续 wallet，只能在完整预交易快照、账户能力来源组合、Spend Rule 控制闭环或 VCC facade 中重新确认单一 Grant。Spend Rule、VCC、全球账户和 P2 业务仍后置。 |
