...
| Git / code baseline | `ea8f8800 docs: 补齐授权 route snapshot 准入包` 是进入本切片前的已提交基线；`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 和 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已完成本地 Green，并随本轮提交固化为新的能力证据；`632bd2f6`、`ca603eab`、`873e5f8c`、`a38776c5`、`bc7ffc0f`、`10853e2d`、`e81a8a25` 和 `ae8cb8a6` 保留为已消费能力证据，`da7d2ea test: 阻断契约夹具承载资金流断言` 和 `b3b9712 feat: 对齐资金底座GSD基线与交易回放能力` 只保留为 GSD-2 历史证据。 |
| 活跃未完成编码计划 | `EMPTY`。旧候选不再是当前编码计划，只能作为 backlog reference；`GSD2-AUTH-CHARGEBACK-COMPAT-ADAPTER-001`、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002` 首轮争议退款、`GSD2-B4-TRANSACTION-PROJECTION-EXPLAIN-002-REMAINING`、`GSD2-B2-WALLET-AUTHORIZATION-ADMISSION-001` 和 `GSD2-B2-WALLET-AUTHORIZATION-ROUTE-SNAPSHOT-001` 已消费，下一轮默认重新确认 `GSD2-LD-LEDGER-GUARD-REGRESSION-001`；若不做 ledger guard，再改选 B7 清算/结算 gate、B7 差异报告、B5 审计扩展或其他单一 Grant。 |
...
| 下一候选 | 默认建议重新确认 `GSD2-LD-LEDGER-GUARD-REGRESSION-001`，为后续资金变化切片补 posting、LedgerEntry、账目、余额桶、幂等和失败无副作用护栏回归；若用户不做 ledger guard，再从 `GSD2-B7-RECON-CLEARING-SETTLEMENT-GATE-CONSUME-001`、B7 差异报告或 `GSD2-B5-BALANCE-ADJUST-AUDIT-002` 中重新确认单一 Grant。若继续 wallet，只能在完整预交易快照、账户能力来源组合、Spend Rule 控制闭环或 VCC facade 中重新确认单一 Grant。Spend Rule、VCC、全球账户和 P2 业务仍后置。 |
