...
| 当前状态 | `WALLET_ENTITY_SERVICE_STANDARDIZATION_BASE_SERVICE_CLOSURE_VERIFIED` |
...
- [x] 2026-06-23 完成 `GSD2-SERVICE-LAYER-RESPONSIBILITY-ALIGN-001` docs-only 基线对齐：同步 Spend Rule PRD、系分、TDD README、LWT Goal 和 OpenSpec tasks，将产品能力对象与工程服务类型解耦。当前 `SpendRuleDefinitionApplicationService` 被定义为兼容 facade，不再作为目标态继续扩大的规则大服务；目标服务层按基础服务、领域写服务、领域读服务和场景服务分层，后续代码收敛需先补分层接口 / 服务测试，再由兼容 facade 委派，最后再评估公共类名迁移。本任务不授权 Java、测试、DDL/H2 schema、Entity、Mapper、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费、生产迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-SR-COMPAT-FACADE-DELEGATION-001`：按服务层四类分层基线完成 Spend Rule 兼容 facade 委派首个代码切片。新增 `SpendRuleVersionService`、`SpendRuleAssignmentService`、`SpendRuleDecisionLogService`、`SpendRuleDecisionLogDomainService` 和 `SpendRuleDecisionLogDomainQueryService` 及实现；只有基础服务直接访问 Mapper，领域写 / 读服务和 application service 不直接访问 Mapper。`SpendRuleDefinitionApplicationService` 保留公共兼容契约并委派决策记录写入、查询和解释；`SpendControlAdmissionApplicationService` 改为依赖决策日志领域写服务，避免准入场景继续依赖规则定义大 facade。新增 `SpendRuleDecisionLogDomainServiceTests` 覆盖决策记录幂等、窄查询和无资金副作用；回归 `SpendRuleDefinitionApplicationServiceTests`、`SpendControlAdmissionApplicationServiceTests` 和 `AuthorizationAdmissionApplicationServiceTests`。验证证据：`just test-one SpendRuleDecisionLogDomainServiceTests tests` 通过 3 tests；`just test-one SpendRuleDefinitionApplicationServiceTests,SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 23 tests。本 Grant 不授权 DDL/H2 schema、Entity、Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名迁移或 Git push。
- [x] 2026-06-23 消费 `GSD2-SR-DEFINITION-VERSION-ASSIGNMENT-SERVICE-SPLIT-001`：按服务层四类分层基线继续拆分 Spend Rule 规则定义、版本发布、规则挂载和挂载查询 / 解释。新增 `SpendRuleDefinitionService`、`SpendRuleDefinitionDomainService` 和 `SpendRuleAssignmentDomainQueryService` 及实现；`SpendRuleVersionService`、`SpendRuleAssignmentService` 扩展基础创建 / 查找 / 查询能力；只有基础服务直接访问 Mapper，领域写 / 读服务和 application service 不直接访问 Mapper。`SpendRuleDefinitionApplicationService` 保留公共兼容契约并委派规则定义、版本发布、规则挂载、挂载查询 / 解释和决策记录能力。新增 `SpendRuleDefinitionDomainServiceTests` 覆盖版本幂等、摘要漂移拒绝、挂载幂等、挂载查询 / 解释只读和非法挂载失败无资金副作用；回归 `SpendRuleDefinitionApplicationServiceTests`、`SpendRuleDecisionLogDomainServiceTests`、`SpendControlAdmissionApplicationServiceTests` 和 `AuthorizationAdmissionApplicationServiceTests`。验证证据：`just test-one SpendRuleDefinitionDomainServiceTests tests` 通过 4 tests；`just test-one SpendRuleDefinitionApplicationServiceTests,SpendRuleDecisionLogDomainServiceTests,SpendControlAdmissionApplicationServiceTests,AuthorizationAdmissionApplicationServiceTests tests` 通过 26 tests；`just compile`、`just pmd`、`git diff --check` 通过。本 Grant 不授权 DDL/H2 schema、Entity / Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名迁移或 Git push。
...
- [x] 2026-06-23 消费 `GSD2-SR-CONTROL-ACTIVITY-SERVICE-SPLIT-001`：按服务层四类分层基线完成控制额度变动流水服务层拆分。新增 `SpendControlActivityService`、`SpendControlActivityDomainService`、`SpendControlActivityDomainQueryService`、`SpendControlActivityConverter` 及实现；`SpendControlActivityApplicationService` 保留公共兼容契约并委派控制额度变动流水写入、查询和预算控制投影；`BudgetControlLimitAdjustmentApplicationService` 与 `SpendControlTransactionConsumptionApplicationService` 改为直接依赖领域写 / 读服务。验证证据：`just test-one SpendControlActivityApplicationServiceTests,SpendControlAdmissionApplicationServiceTests,SpendControlTransactionConsumptionApplicationServiceTests,BudgetControlLimitAdjustmentApplicationServiceTests tests` 通过 44 tests；`just compile`、`just pmd` 和 `git diff --check` 通过。本 Grant 不授权 DDL/H2 schema、Entity / Mapper 字段变更、Controller、HTTP/RPC、交易 canonical 入参、ledger posting、完整规则引擎、事件消费 / outbox、生产迁移、公共类名 / 表名迁移或 Git push。
...
- [ ] 2026-06-23 计划 `GSD2-WALLET-APPLICATION-FACADE-CALLING-SURFACE-001`：整理 wallet application 调用面，明确支付工具准入、预交易快照、支出控制、外部资金事件、收款 / 打款等特殊入口的推荐组合关系；先做能力地图和服务边界收敛，不处理 HTTP/RPC。
