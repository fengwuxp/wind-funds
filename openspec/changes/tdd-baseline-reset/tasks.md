...
- [x] 2026-06-07 落地权益营销账户设计基线：PRD、DSL、系分、TDD 和 OpenSpec 已把“营销账户”重新定性为已决策权益资金影响的账户化承接，MVP 先按资金账户、平台责任资金账户或等价账户 profile 表达，不新增支付工具主体，不接管营销规则、券包库存、活动生命周期或最优券计算。2026-06-07 追加交易入口裁决：保留 `FundsBenefitFundingApplicationService` 作为唯一权益资金责任应用门面命名，不新增 `FundsMarketingTransactionService` 或权益授权/结算/退款平行生命周期；`FundsBenefitFundingApplicationService` 只能作为 application 门面，编排权益资金责任解析、营销账户 guard、快照固化并委派既有直接交易、授权交易和退款入口。首个后续候选为 `BEN-MKT-PLATFORM-SUBSIDY-MVP`：只处理平台补贴补足商户，需补 `TDD-BEN-MKT-001`、`TDD-BEN-MKT-002`、`TDD-BEN-ENTRY-006` 和 `TDD-BEN-RED-031` 至 `TDD-BEN-RED-034`，并确认账户 profile、专业确认状态、事实源、独立 leg 或伴随指令原子性和退款回放策略。本轮为 docs-only 设计落地，不授权写 Java、测试、DDL/H2 schema、公共契约或运行时配置。
...
- [x] 2026-06-10 固化 `FundsBalanceControlService` 外部余额异常纠偏设计：第三方钱包、VCC 发卡行、发卡处理商或外部余额系统出现终局余额事实与我侧同主体余额不一致时，余额控制只承接已终局或经差错单确认的受控纠偏，不打开普通消费透支。纠偏必须具备外部终局事件、外部余额快照、差错单或审批、凭证、责任方、操作者和重新对账引用；允许同主体目标账目进入受控负可用，但负余额只能作为风险、追偿、抵扣或人工处理状态，后续支付、授权、冻结、提现、出款和清算放行必须重新过负余额策略。该条只同步 PRD、DSL、系分、TDD 和任务基线，不授权 Java、测试代码、DDL/H2 schema、公共契约或运行时配置。
...
| 5 | 权益营销账户 | `BEN-MKT-PLATFORM-SUBSIDY-MVP`。 | 权益资金流的最小账户化承接候选；只处理平台补贴补足商户，不一次性实现储值负债、合作方补贴、商户让利账户化、授权占券或全部权益生命周期；默认不新增 `FundsMarketingTransactionService`，保留 `FundsBenefitFundingApplicationService` 作为应用门面命名，用于编排解析、guard、快照和委派；未确认前不写 Java、测试、DDL/H2 schema、公共契约或运行时配置。 |
...
| B5-06 | 固化外部余额异常纠偏和受控负可用。 | `transaction-*`、`tests/src/test/java`；若落表则需独立确认 DDL/H2 范围 | `AC-CTRL-013`、`DSL-BALANCE-CONTROL-EXTERNAL-DEFICIT-ADJUST-001`、`TDD-CTRL-012`、`TDD-CTRL-ERR-007`、`TDD-RECON-016` | 上游未终局、无外部余额快照、无差错单或审批、无凭证、跨主体转移损失、纠偏后绕过负余额策略继续交易。 | 通过 `BALANCE_CONTROL / BALANCE_ADJUST` 和 B7 差错闭环承接终局外部余额差异；保留外部事件、快照、审批、凭证、责任和重新对账引用。 | `just test-balance-control`、`just test-reconciliation` |
...
本节记录当前 GSD + Goal 推进基线。它只建立目标、依赖顺序、Wave、Execution Grant 队列和生产可用证据门禁，不替代任何单一编码授权。
...
| 授权前收口 | 当前 Goal 卡、交付雷达、Wave 队列、Grant 队列、默认入口、切换规则和验证矩阵已齐备；后续不再为同一批候选重复扩写 Round 0 文档，只在发现事实错误、索引漂移或用户新增约束时修正文档。 |
| 禁止范围 | Plan Grant 之外不得写测试代码；未确认单一 Execution Grant 前不得写生产代码、DDL/H2 schema、公共契约或运行时配置；`B3-DIRECT-REFUND-REFERENCE-REPLAY` 已消费的公共契约例外只包括 `FundsTransactionRefundRequest.referenceTransactionSn`，不得继续扩展等价 reference DTO 或其他交易请求字段；不得把 Goal、GSD Wave、Round 0、Plan Grant 覆盖、B3 局部闭环或文档 CR 写成生产 Done。 |

#### 9.2.1 Agent Loop Engineering 恢复口径（2026-06-11）

本节记录当前 `GSD + Goal + Loop` 的恢复口径。该 Loop 管理状态读取、低风险文档同步、低风险目标测试覆盖、验证和失败回写，不替代完整单一 Execution Grant。

| 字段 | 内容 |
| --- | --- |
| Loop ID | `GSD-GOAL-MVP-VCC-GLOBAL-FUNDS-LOOP-2026-06-11` |
| Loop 状态 | `LOOP_ACTIVE_PLAN_GRANT_B4_CURRENT_BINDING_REPLAY_FLOW_VERIFIED_SUMMARY_ONLY` |
