...
| 本轮新增权威补充 | `GSD2-B5-SR-TRANSACTION-RELEASE-TARGET-ACCOUNT-GUARD-015` 已完成服务层补片；LWT Goal、W5、GSD-2 入口、TDD README、docs README、TDD 清单和本 tasks 已把当前状态从预算控制投影目标账户隔离守卫推进到释放上限目标账户隔离守卫。 |
...
| 本轮新增写入补充 | 已消费交易消费支出控制活动服务层切片、并发幂等硬化切片、退款释放守卫切片、退款事实守卫补片、退款补偿入口守卫补片、退款已消费引用守卫补片、业务场景一致性守卫补片、成功消费业务流水一致性守卫补片、失败释放业务流水一致性守卫补片、退款引用净消费上限守卫补片、退款引用已消费一致性守卫补片、交易事实累计解释金额守卫补片、控制活动幂等语义一致性守卫补片、退款引用原交易业务一致性守卫补片、预算控制投影目标账户隔离守卫补片和释放上限目标账户隔离守卫补片，新增 `SpendControlTransactionConsumptionApplicationService`、`CONSUMED` / `REFUND_COMPENSATED` 控制活动语义、原活动 / 原交易回链、`consumedAmount` 投影解释，并证明同流水同摘要并发唯一键冲突可回读既有活动；补充证明 `REFUND` 交易事实不得被 `consume` 降级为普通成功消费，也不得被 `release` 降级为普通失败释放，非 `REFUND` 交易事实不得被 `refund` 降级为退款控制补偿，退款交易没有对应已消费控制活动时不得生成退款控制补偿，跨业务场景资金交易不得消费当前 Spend Rule 控制占用，同业务场景但不同业务流水的成功交易不得消费当前控制活动，同业务场景但不同业务流水的失败、拒绝或过期交易也不得释放当前控制活动，退款补偿金额不得超过退款交易引用的已消费控制活动净额，退款引用的已消费控制活动必须与原控制活动在业务场景、业务流水、目标账户和币种上同源一致，同一原控制活动下同一资金交易流水的 `CONSUMED`、`RELEASED` 或 `REFUND_COMPENSATED` 累计解释金额不得超过资金交易金额，同一控制活动流水和摘要不得被不同活动类型或不同业务语义复用，退款交易引用的原消费资金交易必须真实存在并满足租户、交易类型、状态、币种、业务场景和业务流水一致性，交易消费后的预算控制投影按目标账户查询时不得混入同预算组或同 Spend Rule 下其他账户或其他卡的控制活动，且释放类控制活动写入上限也必须按目标账户隔离，历史关联活动必须与原占用在业务场景、业务流水、目标账户、币种、Spend Rule、规则版本和预算组上同源一致。当前不授权 Spend Rule 规则定义、决策日志持久化实现、VCC facade、支付工具交易内核、Controller、HTTP/RPC、事件消费者、生产迁移或 Git 历史重写。 |
