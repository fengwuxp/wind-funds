...
| B4 覆盖索引：授权交易 | 已存在 `FundsAuthorizationTransactionFlowTests`，覆盖授权拒绝、撤销、部分完成、全额完成、退款超额和授权过期释放等局部场景；`b0666ba feat: 补齐授权过期释放 canonical 能力` 已补齐 `EXPIRE` 事件、`expire` 服务入口、请求模型、route replay、生命周期和授权过期释放测试。 | 授权批准、拒绝、撤销、完成、完成后退款和授权过期释放已有局部基线；强制完成、无授权直接退款、拒付承接口径、发卡控制扩展边界和并发竞争仍需按 B4 覆盖索引闭合。 |
...
| GAP-AUTH-002 | `settle` 支持强制完成模式，必须有 FORCE 模式、受信策略或审批快照、原因、上限、外部原事实引用、凭证和审计；首轮 FORCE 模式不得依赖内部原授权流水。 | `FundsAuthorizationTransactionSettleRequest` 当前以原授权交易号为必填，未表达强制完成策略字段、受信策略来源、外部原事实引用、凭证或审计最小集。 | 先补强制完成失败测试，再设计目标态请求契约方案；Execution Grant 必须显式声明 `authorizationTransactionSn` 在普通完成和 FORCE 模式下的条件化规则、请求摘要字段和兼容策略。 |
