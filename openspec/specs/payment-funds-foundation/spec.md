...
| 域 | 规格范围 |
| --- | --- |
| DSL 契约 | `FundsInstruction`、`eventType`、`transactionType`、金额、引用、route、posting、entry、JSON 契约。 |
...
| 钱包账户 | 资金账户、信用账户、预算组、平台账户角色、支付工具、主体余额查询。 |

### 2.2 独立批次范围

| 域 | 规格范围 |
| --- | --- |
| 清结算与对账 | 清算候选、清算批次、结算单、出款单、对账批次、差错单、调账核销。 |
...
| 1 | DSL 契约与枚举红线 | 重建 `FundsInstruction`、event、transactionType、route、posting、JSON 契约测试。 |
| 2 | 钱包账户与账本基础 | 重建账户、账本初始化、平台账户角色、账本周期、posting 平衡和余额投影测试。 |
...
| 6 | Route Replay 与投影 | 重建 Route Replay、缺快照失败、交易投影只读、余额日志边界测试。 |
| 7 | 清结算与对账 | 按独立系分重建清算候选、批次、结算单、出款单、对账差错测试。 |
