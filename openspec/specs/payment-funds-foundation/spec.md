...
| 归档与重放 | 归档门禁、Manifest、checkpoint、watermark、余额重建、交易投影重放。 |
| 指标项 | 只列产品和业务关心的指标项、使用者、业务问题和建议事实来源；具体计算、存储和展示由报表指标模块承接。 |
...
| GAP-ARCH-001 | 归档、重放、指标边界需按 04 系分独立落地。 | 指标实现不是资金底座重点。 | 归档/重放只做事实治理；指标只输出报表指标模块输入。 |
...
| 8 | 归档重放与指标边界 | 重建归档门禁、Manifest、水位、余额重建、交易投影重放、指标只读和指标水位隔离测试。 |
