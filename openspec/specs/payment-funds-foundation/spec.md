...
| 权益让利资金事实 | `FundsBenefitFundingApplicationService` 及其 `settle/refund` 请求，只承接业务侧、订单侧或营销权益系统已决策的优惠券、代金券、支付立减、平台、商户或合作方给用户让利的出资责任结果；请求模型仅保留业务流水、原订单或原交易引用、成本承担主体、让利承接账务主体、金额和资金性质等最小资金事实；`settle` 即表示本次让利出资需要入账，`benefitReceiverSubjectRef` 是当前资金路径的目标账务主体，不等同于营销系统中的用户实体；多方出资按出资方拆成多笔 `settle`，退款分别引用原权益资金交易调用 `refund`；不计算券规则，不判断券是否可用，不维护券生命周期，不处理返利、佣金、分润、储值负债释放或用户余额入账。 |
