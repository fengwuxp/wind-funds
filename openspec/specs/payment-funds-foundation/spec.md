...
| P1 | 权益金额组件 | `FundsBenefitFundingApplicationService`、让利出资记账交易请求、商户券、平台补贴、储值券、授权占券、不退券、部分退款和缺权益资金事实逆向处理；历史 `benefitSnapshotId` 和 `stableDigest` 仅作为只读摘要追溯字段。 |
...
| 预付卡、共享卡交易服务能力包 | 预付卡能力必须遵循“资金先确认、后授权使用”：外部入金、系统内充值或退卡转出只有在确认引用、幂等摘要和内部责任主体唯一时才能委派到账户主体型交易；共享卡能力必须遵循“账户入账、工具归因”：授权、清算、释放、退款和争议裁决资金结果都固化工具快照、绑定版本、预算组上下文、Spend Rule 决策和资金责任决策。 | 首期只能作为 P2 独立 Execution Grant 切片进入，建议拆为 `P2-VCC-PREPAID` 与 `P2-VCC-LIFECYCLE`；不得与 B2/B4/B5/B6/B8 一次性混合授权。 |
...
| 让利出资记账交易 | `FundsBenefitFundingApplicationService` 及其 `settle/refund` 请求，只承接业务侧、订单侧或营销权益系统已决策的优惠券、代金券、支付立减、平台、商户或合作方给用户让利的出资责任结果；请求模型仅保留业务流水、原订单或原交易引用、成本承担主体、让利承接账务主体、金额和资金性质等最小资金事实；`settle` 即表示本次让利出资需要入账，`benefitReceiverSubjectRef` 是当前资金路径的目标账务主体，目标态语义更接近 `benefitSettlementSubjectRef`，不等同于营销系统中的用户实体；多方出资按出资方拆成多笔 `settle`，退款分别引用原让利出资交易调用 `refund`；不计算券规则，不判断券是否可用，不维护券生命周期，不保存券、活动或规则来源归因，不处理返利、佣金、分润、储值负债释放或用户余额入账。 |
...
| GAP-BEN-002 | 直接交易需按原让利出资记账交易处理商户券、平台补贴、储值券、零实付权益和叠加权益。 | 真实入账结算和退款型逆向处理已通过 `FundsBenefitFundingApplicationServiceImpl` 接到标准直接交易、route、posting 和账本链路；非入账权益事实、零实付主指令、伴随指令和储值负债口径仍未闭合。 | B3 覆盖索引继续补商户让利 no-ledger、平台补贴独立资金影响、储值券资金性质、零实付表达、独立伴随指令原子性、部分成功补偿、专业确认、审计证据包、使用者解释视图、证据最小化、外部规则核验和退款分摊确定性测试。 |
...
| GAP-BEN-008 | 权益让利资金入口应从重型权益快照契约重基线为交易级 application service。 | 已由 `GSD2-BENEFIT-FUNDING-TRANSACTION-REBASE-001`、`GSD2-BENEFIT-FUNDING-TRANSACTION-IMPL-001`、`GSD2-BENEFIT-LEGACY-SNAPSHOT-REMOVE-001` 和 `GSD2-BENEFIT-FUNDING-SETTLE-REFUND-REBASE-001` 关闭首轮缺口：`transaction-face` 暴露 `settle/refund`，真实入账让利出资已接到标准交易路由、交易事实和账本分录链路，旧重型权益 DSL 已物理删除，独立 `reverse` 入口已收敛为 `refund` 型逆向处理。 | 后续只作为让利出资记账交易回归基线维护；若要扩展非入账权益事实、独立权益事实表、清结算对账投影或授权占券，需要新的单一 Execution Grant。 |
