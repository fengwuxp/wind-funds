...
| GAP-WALLET-003 | 预算组目标态不是核心资金账务主体，现有 `BUDGET_GROUP` 兼容路径必须收敛。 | 当前代码仍保留 `FundsSubjectType.BUDGET_GROUP`、预算组余额查询解析、显式 `SubjectLedgerInitializer` 兼容初始化和余额控制等兼容路径；`BudgetGroupService#createBudgetGroup` 已不再初始化 ledger bucket，目标测试覆盖 LIFETIME、MONTHLY 和 CUSTOM_CYCLE 预算组创建无账本事实副作用。 | B2/B5 Execution Grant 必须继续明确 `BUDGET_GROUP` 是兼容枚举、迁移别名、只读查询过滤条件还是待删除目标，并收敛预算组 route leg、posting、LedgerEntry、余额查询和余额控制兼容语义；不得把预算组 ledger 初始化或余额 bucket 作为目标态 Done。 |
