...
| GAP-ACQ-001 | 收单 capture 或支付成功必须作为 P2 业务 capability pack 归一事件进入商户 `CLEARING` 或待清算边界，不得直接释放可提现、结算或出款状态。 | 当前已有 `MerchantInfoSpec`、商户上下文、`LedgerSubjectCode.CLEARING/SETTLEMENT`、授权 capture/settlement/refund/chargeback 概念、敏感上下文阻断和出款前准入局部基线，但尚未形成 `AcquiringCaptureApplicationService`、收单 capture Request/DTO、capture 幂等摘要、商户资金账户解析、支付成功不等于可提现 Red、PCI 敏感数据阻断 Red 或 merchant CLEARING 服务级闭环。 | `P2-ACQ-CAPTURE` Execution Grant 必须先补 `R0-ACQ-CAP-001`，证明 capture/payment success 不直接增加 `AVAILABLE`、`SETTLEMENT`、可提现余额或 payout-ready；完整 PAN/CVC/token secret/3DS 原文/PSP 原始报文不进入资金底座；非终态、缺 capture confirmation、缺商户资金账户决策、缺幂等或同外部引用不同摘要不得写 route、posting、LedgerEntry、projection、清分候选或可提现结果。 |
