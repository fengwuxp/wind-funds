...
| P1 | 授权交易 | 授权批准、授权拒绝、授权撤销、授权完成、授权过期、授权链退款、无授权直接退款、争议裁决资金结果承接、`settle` 强制完成模式。 |
...
| B4 覆盖索引：授权交易 | 已存在 `FundsAuthorizationTransactionFlowTests`，覆盖授权拒绝、撤销、部分完成、全额完成、退款超额、授权过期释放、强制完成和无授权直接退款等局部场景；`b0666ba feat: 补齐授权过期释放 canonical 能力` 已补齐 `EXPIRE` 事件、`expire` 服务入口、请求模型、route replay、生命周期和授权过期释放测试；`616dac1 feat: 补齐授权强制完成能力` 和 `3825466 fix: 收紧授权强制完成策略红线` 已补齐 FORCE 模式、受信策略、上限、原因、外部原事实、凭证、审计、普通完成分支隔离和失败无副作用；B4-NO-AUTH-REFUND 已补齐 no-auth 退款模式、`externalReferenceSn`、退款原因和失败无副作用测试。 | 授权批准、拒绝、撤销、普通完成、完成后退款、授权过期释放、强制完成、无授权直接退款和争议裁决资金结果首轮已有局部基线；后续按 B4 覆盖索引闭合发卡控制扩展边界、授权占券和权益生命周期、并发竞争。 |
...
| GAP-AUTH-003 | chargeback / dispute 只作为已完成授权后的争议案件过程；交易层只消费裁决后的资金结果。需要退款或扣回时默认通过 `settleRefund` 携带原因、凭证、外部引用和审计上下文承接，无资金影响时不调用交易入口，不强制独立 `chargeback` 服务入口。 | 代码已有 `FundsAuthorizationTransactionService#chargeback`、`FundsAuthorizationTransactionChargebackRequest`、`CHARGEBACK` eventType、route replay `CHARGEBACK` phase 和授权交易 flow 成功/失败/幂等测试；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 已由 `949b24a` 闭合首轮 canonical 可区分性：`settleRefund / AUTH_REFUND` 通过 `disputeMode`、`disputeReason`、`disputeVoucherRef` 和 `externalDisputeRef` 保留争议审计上下文，请求侧不恢复 `refundMode`。现有 `chargeback` 方法和请求模型已标注为历史兼容入口，不能直接反推为目标态主入口；公共 API 退出计划已锁定为 `GSD2-TRX-CHARGEBACK-PUBLIC-API-EXIT-PLAN-001`，调用方扫描已完成，目标态超额争议资金结果红线已改走 `settleRefund` 争议字段，剩余旧入口测试已统一 `CompatChargeback` 命名并保留为兼容回归；`GSD2-TRX-CHARGEBACK-PUBLIC-API-REMOVE-READINESS-001` 已评估删除 readiness，当前结论为 `DELETE_READINESS_NOT_READY`。 | 后续只把首轮争议退款可区分性、目标态资金红线和历史 `CHARGEBACK` 事实解释作为 B4 回归基线维护；公共 API 物理退出必须继续完成跨仓库依赖扫描、发布兼容、版本废弃窗口、回滚策略和显式删除 Grant；若要处理完整争议运营、representment、清结算追偿、VCC processor 或外部规则，必须另起 P2/B7 专项。 |
