...
| 授权支付工具应用入口 | 面向 VCC、卡、VA、共享卡、钱包标识或通道 token 的授权入口，可以接收支付工具引用和业务上下文；应用层必须先解析工具状态、绑定版本、使用主体、预算组、Spend Rule、资金责任和账户能力，再委派账户主体型授权内核。当前账户主体型 `authorize` 请求不得在未授权时被直接替换为工具引用字段。 |
| 支付工具 | 卡、VA、外部账户、虚拟卡、钱包标识或通道 token 的路由输入和审计快照；不表达内部余额，不作为账本主体。钱包标识只表示前台支付方式、外部钱包端点或工具引用，必须解析为资金账户、信用账户或平台账户角色解析后的平台资金账户后才能进入 route leg、posting 或 ledger entry。预算组和 Spend Rule 只作为控制上下文或规则快照。 |
...
| GAP-AUTH-005 | 授权支付工具应用入口必须支持从工具引用、绑定和规则上下文解析到账户主体型授权内核。 | 当前 `FundsAuthorizationTransactionAuthorizeRequest` 以 `FundsAccountId accountId` 作为内核入参，支付工具和绑定快照主要由 route snapshot/PaymentInstrumentRef 承载；尚未形成明确的 application facade 契约。 | B4 或 B2+B4 联合 Execution Grant 必须先定义应用层服务、Request/DTO、幂等摘要、拒绝事实、快照字段和目标测试资产；新增入口必须证明工具准入失败无 route/posting/entry，批准后 route leg 只使用资金账户、信用账户或平台账户角色解析结果。 |
