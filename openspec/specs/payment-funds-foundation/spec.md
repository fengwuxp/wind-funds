...
| 授权交易 | 授权批准、授权拒绝、授权撤销、授权完成、授权过期、授权链退款、拒付承接口径、`settle` 强制完成模式。 |
| 余额控制 | 冻结、解冻、资金调账、信用额度调额、预算调额。 |
| 路由与回放 | route resolver、route snapshot、原路径 replay、缺快照失败、账本周期继承。 |
...
| GAP-TDD-001 | 测试必须按最终 TDD 重建。 | 旧测试源码已作废并移除，resources 保留。 | 从 DSL 契约和核心红线开始分批恢复测试，不恢复旧版过渡断言。 |
...
| 4 | 授权交易 | 重建授权、拒绝、撤销、完成、过期、强制完成、退款、拒付承接口径和 VCC 扩展边界测试。 |
| 5 | 余额控制 | 重建冻结、解冻、调账、信用额度、预算额度和冻结提现组合测试。 |
| 6 | 路由回放与投影 | 重建原路径 replay、缺快照失败、交易投影只读、余额日志边界测试。 |
| 7 | 清结算与对账 | 按独立系分重建清算候选、批次、结算单、出款单、对账差错测试。 |
| 8 | 归档重放与指标边界 | 重建归档门禁、Manifest、水位、余额重建、交易投影重放和指标只读边界测试。 |
