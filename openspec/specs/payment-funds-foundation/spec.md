...
| 业务专项 | P2 能力包 | VCC 和全球账户业务只通过专项 PRD/系分/TDD 使用资金底座能力；收单业务当前只保留 design-only 设计和边界复核。 |

VCC 和全球账户 P2 业务专项进入编码前必须单独补 Execution Grant。授权中至少写明：对应业务分册、业务验收 ID、场景 pack 范围、归一资金动作、外部引用脱敏字段、外部适配证据包、允许新增或修改的公共契约、是否触碰 DDL/H2 schema、P0/P1 回归测试、外部规则核验状态、`TDD-P2-*` 专项用例和 Not Done 红线。VCC 还必须写明 SupplierBill、AuthorizationEvent、ClearingRecord、FeeRecord、FundingStatement、LedgerEntry、AccountingVoucher、ReconciliationCase、MatchResult、DifferenceItem、AdjustmentAction 和 AuditTrail 的采纳或引用范围；缺任一项时，P2 业务只能停留在产品、DSL、系分、TDD 或 contract-only 验证，不得声明业务生产资金流 Done。收单当前不适用上述编码授权，只允许产品、DSL、系分、TDD 和 OpenSpec 的 design-only 差距复核。

ACH 或银行转账不作为 P2 新增业务专项默认进入资金底座。若 VCC、全球账户或平台出款后续使用 ACH 轨道，Execution Grant 必须证明 ACH 业务或通道适配层已经完成业务归属、指令、授权、文件、批次、return、NOC、reversal、外部受理状态、敏感数据最小化和外部规则解释；资金底座只允许承接归一资金事实、外部引用、对账差错、追偿、调账核销和审计，不得在 `core`、`transaction-*`、`wallet-*` 或 `ledger-*` 中实现 ACH 协议、Nacha 规则或银行账户敏感数据能力。收单涉及 ACH、银行出款或通道清算时当前只登记设计规则，不进入实现。
...
| 营销账户 | 已决策权益资金影响的账户化承接，用于平台营销成本、商户让利归因、权益负债、合作方补贴和营销留置。MVP 先按资金账户、平台责任资金账户或等价账户 profile 表达，不新增支付工具主体，不接管营销规则、券包库存或活动生命周期。 |
...
| B3 覆盖索引：直接交易 | 已存在 `FundsDirectTransactionFlowTests`、`FundsTransferPayWithdrawChainFlowTests`、`FundsTransactionFeeFlowTests`；`B3-DIRECT-REFUND-REFERENCE-REPLAY` 已补 `FundsTransactionRefundRequest.referenceTransactionSn`、原 route snapshot 部分回放、独立退款事实和缺原交易/累计超额失败无副作用测试；2026-06-11 Plan Grant 已补原交易存在但 route snapshot 缺失时直接退款全链路失败无副作用测试。 | 直接交易主链路、退款、手续费和链式流程已有局部流程基线；原交易引用退款回放已闭合为 B3 局部能力；缺原快照直接退款失败无副作用已作为 B4 replay 红线覆盖；仍需按 TDD 覆盖全部 `AC-IN/OUT/PAY/MER/FEE`、权益资金影响、提现异常和红线失败。 |
...
| B6 覆盖索引：Route Replay 与投影 | 已存在 `DefaultRouteReplayServiceTests`、`CompositeRouteResolverTests`、`DefaultRoutedFundsInstructionOrchestratorProjectionTests`；2026-06-11 已复跑 `DefaultRouteReplayServiceTests` 9 tests，通过原支付工具快照、外部账户快照和资金责任决策复用覆盖；同日新增 `FundsDirectTransactionFlowTests#testRefundWithMissingReferenceRouteSnapshotShouldRejectAndLeaveNoSideEffects`，通过原交易存在但 route snapshot 缺失时 fail-fast 且无新资金或账务副作用覆盖；本轮新增 `FundsDirectTransactionFlowTests#testRefundWithReferenceTransactionShouldReuseOriginalInstrumentAndFundingSnapshot`，通过原支付快照固化旧支付工具和旧资金责任后直接退款仍沿历史快照回放覆盖，直接交易 flow 51 tests 通过。 | Route Replay、resolver 无副作用、交易投影 afterCommit 和投影失败不回滚事实已有局部基线；纯 route replay 边界已证明当前请求工具、外部账户或资金责任上下文不会覆盖原 `RouteSnapshot`；缺原快照直接退款 flow 已证明失败无副作用；支付工具换绑或资金责任变化后的直接退款交易 flow 子场景已证明余额、账本事实和退款 route snapshot 归因仍沿原快照；授权/争议/VCC lifecycle 更大组合回放、敏感快照、绑定历史审计、余额日志边界和投影重放全量覆盖仍需按 B6 覆盖索引闭合。 |
...
| GAP-TRX-CAP-001 | 交易层仍需完善账户主体型授权生命周期、退款/拒付、余额控制调账和原路径回放能力；这些能力不是支付工具交易入口。 | 当前直接交易、授权交易、余额控制和投影已有局部基线；B3 直接退款原交易引用回放、授权过期释放、强制完成和 B4-NO-AUTH-REFUND 首轮 canonical 能力已形成 Red/Green 证据；route replay resolver 边界已验证当前工具、外部账户或资金责任变化不覆盖原快照；原交易存在但 route snapshot 缺失时直接退款交易全链路失败无副作用已补覆盖；当前绑定或资金责任变化后的直接退款交易 flow 子场景已证明沿历史快照回放；完整拒付 case、授权/争议/VCC lifecycle 更大组合 replay flow、调账审计和卡维度解释尚未形成完整 Red/Green 证据。 | 后续 B3/B4/B5/B6 Execution Grant 可以在 `transaction-face`、`transaction-impl`、route replay 或 transaction projection 中扩展 canonical 请求、事件、状态机、DTO 和测试；主体仍必须是已解析资金账户、信用账户或平台账户角色解析后的平台资金账户。禁止把这些能力合并成统一 `InstrumentTransactionService`，也禁止让交易 canonical 请求直接接收支付工具作为账务主体。 |
...
| GAP-GA-001 | 全球账户入金、VA/银行流水匹配和外部受理在途必须通过 P2 业务 capability pack 归一后进入资金底座。 | 当前已有 `ExternalAccountRefSpec`、外部账户敏感值校验、FX 端口和出款前准入局部基线，但尚未形成 `GlobalAccountInboundApplicationService`、全球账户入金 Request/DTO、外部状态非终态阻断、VA/银行流水匹配、错币种和外部账户不入账的目标 Red。 | `P2-GA-INBOUND` Execution Grant 必须先补 `R0-GA-IN-001`，证明外部 accepted/message sent/processing/pending/in transit 不增加 AVAILABLE、未匹配 VA 不入账、外部银行账户/VA/Nostro/Vostro 不作为账务主体、错币种无有效 FX quote 阻断、敏感原文阻断和失败无副作用。 |
| GAP-GA-002 | 全球账户出款、外部受理在途、成功回单和退汇必须与出款前准入、解释状态和原出款引用绑定。 | 当前已有 `PayoutOrderService#checkPayoutPreflight`、`CheckPayoutPreflightRequest`、`PayoutPreflightResultDTO` 和 `PayoutPreflightServiceTests` 候选基线，但尚未形成 `GlobalAccountOutboundApplicationService`、全球账户出款 Request/DTO、外部非终态不误展示、成功回单终态、退汇关联原出金、费用和责任处理的目标 Red。 | `P2-GA-OUTBOUND` Execution Grant 必须先补 `R0-GA-OUT-001`，证明 preflight 缺失/失败/未知不得提交外部出款或生成资金事实，submitted/accepted/message sent/processing/pending 不得展示为 `PAID` 或关闭 `SETTLEMENT/IN_TRANSIT`，退汇不得当普通 refund，错币种无有效 FX quote 阻断，失败无副作用。 |
| GAP-GA-003 | 全球账户 FX quote、费用拆分和错币种边界必须以外部决策快照、结构化费用归因和专业确认状态进入资金底座。 | 当前已有 `FxService` / `DefaultFxServiceImpl`、`FundsInstruction.originalAmount`、`FundsInstruction.exchangeRate` 和直接交易手续费/退费局部能力，但尚未形成 `GlobalAccountFxFeeApplicationService`、全球账户 FX quote approval snapshot、费用组件归因、FX P&L 专业确认、错币种无 quote 阻断、费用净额混淆阻断和不执行 FX 的目标 Red。 | `P2-GA-FX-FEE` Execution Grant 必须先补 `R0-GA-FX-001`，证明错币种无有效已批准 quote 不入账、不静默换汇，平台费/银行费/中间行费/本金/FX P&L 不得净额混淆，资金服务不得调用 FX 执行或外部汇率交易系统，外部规则和专业确认缺失时不得声明生产 Done，失败无资金事实副作用。 |
| GAP-ACQ-001 | 收单 capture 或支付成功必须作为 P2 业务 capability pack 归一事件进入商户 `CLEARING` 或待清算边界，不得直接释放可提现、结算或出款状态。 | 当前已有 `MerchantInfoSpec`、商户上下文、`LedgerSubjectCode.CLEARING/SETTLEMENT`、授权 capture/settlement/refund/chargeback 概念、敏感上下文阻断和出款前准入局部基线，但尚未形成 `AcquiringCaptureApplicationService`、收单 capture Request/DTO、capture 幂等摘要、商户资金账户解析、支付成功不等于可提现 Red、PCI 敏感数据阻断 Red 或 merchant CLEARING 服务级闭环。 | 当前按用户优先级降级为 `DESIGN_ONLY_NOT_CODE_CANDIDATE`：只保留收单 capture、dispute、商户清结算、PCI/外部规则和待清算边界设计差距，不作为实现候选；不得确认或消费 P2-ACQ-CAPTURE 实现授权，不写 Java、测试、DDL/H2 schema、公共契约或运行时配置，除非用户后续明确重新打开收单实现优先级。 |
...
| GAP-BEN-MKT-001 | 有资金影响权益组件需要营销账户或等价资金责任账户承接，才能进入 route/posting、清结算、对账和交易投影。 | 当前已具备权益快照契约，但代码尚未形成营销账户 profile、benefit funding resolver、营销账户 guard、posting context 和清结算/对账消费闭环；`FundsBenefitFundingApplicationService` 命名已确认为唯一保留的权益资金责任应用门面命名，具体 Java 公共契约尚未落地。 | 首个候选为 `BEN-MKT-PLATFORM-SUBSIDY-MVP`：只覆盖平台补贴补足商户，要求补 `TDD-BEN-MKT-001`、`TDD-BEN-MKT-002`、`TDD-BEN-ENTRY-006`、`TDD-BEN-RED-031` 至 `TDD-BEN-RED-034`，并明确账户 profile、专业确认状态、事实源、独立 leg 或伴随指令原子性和退款回放策略。未确认前不得声明含权益营销账户生产资金流 Done；不得新增 `FundsMarketingTransactionService` 或权益授权/结算/退款平行生命周期。 |
