...
| GAP-TDD-001 | 测试必须按最终 TDD 重建。 | 历史过渡测试已作废；当前已有服务层流程、Route Replay、余额投影和交易投影重放等局部目标态测试，但批次 1 至批次 6 尚未按 TDD 覆盖索引完整闭环。 | 继续按批次从 DSL 契约和核心红线开始补齐测试，不恢复已废弃断言；已重建测试只作为对应批次的局部基线。 |
