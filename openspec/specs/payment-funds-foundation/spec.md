...
| 外部规则核验状态 | 依赖税务、会计、合同、卡组织、银行、通道、KYC/KYB/AML、客户资金、跨境或外汇口径时，必须保存规则来源、版本或发布日期、生效日期、适用主体或适用范围、核验日期和确认方；缺失时只能阻断、降级或转人工。 |
...
| GAP-CLS-002 | 出款前准入门禁、外部非终态和金额不一致必须有服务契约与测试资产。 | 当前只有 PRD、DSL、系分和 TDD 设计；代码尚无 `PayoutOrderService#checkPayoutPreflight`、`CheckPayoutPreflightRequest`、`PayoutPreflightResultDTO`、出款门禁表/字段或 `PayoutPreflightTests`。 | 批次 7 必须把 `AC-SET-006` 至 `AC-SET-008`、`TDD-SETTLE-004`、`RED-032` 和 `RED-037` 纳入 Execution Grant；先补失败测试，再落出款 preflight、状态守卫、审计和外部规则字段完整性校验。 |
...
| 7 | 03-清结算与对账 | 按独立系分重建可清分准入、清分批次、清算候选、清算批次、结算单、出款单、出款前准入门禁、外部非终态、金额不一致、对账差错、权益拆分、退款时序和追偿测试。 |
