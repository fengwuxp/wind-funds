...
| B4 覆盖索引：授权交易 | 已存在 `FundsAuthorizationTransactionFlowTests`，覆盖授权拒绝、撤销、部分完成、全额完成、退款超额、授权过期释放、强制完成和无授权直接退款等局部场景；`b0666ba feat: 补齐授权过期释放 canonical 能力` 已补齐 `EXPIRE` 事件、`expire` 服务入口、请求模型、route replay、生命周期和授权过期释放测试；`616dac1 feat: 补齐授权强制完成能力` 和 `3825466 fix: 收紧授权强制完成策略红线` 已补齐 FORCE 模式、受信策略、上限、原因、外部原事实、凭证、审计、普通完成分支隔离和失败无副作用；`006bcaa feat: 补齐无授权退款 canonical 能力` 已补齐 `NO_AUTH` 退款模式、外部原事实和失败无副作用测试。 | 授权批准、拒绝、撤销、普通完成、完成后退款、授权过期释放、强制完成和无授权直接退款首轮已有局部基线；后续按 B4 覆盖索引闭合拒付承接口径、发卡控制扩展边界、授权占券和权益生命周期、并发竞争。 |
...
| GAP-AUTH-003 | 拒付只作为已完成授权后的争议、扣回或追偿语义，不等同于授权拒绝；目标态默认可通过 `settleRefund` 携带原因、凭证、外部引用和审计上下文承接，不强制独立 `chargeback` 服务入口。 | 代码已有 `FundsAuthorizationTransactionService#chargeback`、`FundsAuthorizationTransactionChargebackRequest`、`CHARGEBACK` eventType、route replay `CHARGEBACK` phase 和授权交易 flow 成功/失败/幂等测试；同时也已有 `settleRefund` 争议退款保留审计上下文的测试。现有 `chargeback` 是实现资产和兼容资产，不能直接反推为目标态主入口；`B4-DISPUTE-SEMANTIC-ALIGNMENT` 已形成可确认但未授权的 Grant 候选包。 | 后续若用户确认 `B4-DISPUTE-SEMANTIC-ALIGNMENT`，首批 Red 聚焦 `TDD-RED-017B`：查询、投影、审计和幂等摘要上可区分普通退款、NO_AUTH 退款、拒付承接和授权拒绝；若确认 `chargeback` 升级为一等目标态 API，必须另行说明公共契约、状态、投影、查询、清结算追偿和兼容边界。未确认前只能继续只读 CR、文档对齐或任务计划。 |
| GAP-AUTH-004 | `settleRefund` 无授权退款模式必须可表达外部原消费、外部原完成或差错凭证。 | 已由 `006bcaa feat: 补齐无授权退款 canonical 能力` 关闭首轮缺口：`FundsAuthorizationTransactionRefundRequest` 新增 `refundMode=NO_AUTH`、外部原事实引用、外部原事实类型、退款原因、退款凭证和原事实金额币种；converter 在 NO_AUTH 模式构造 `EXTERNAL_TRANSACTION` reference 并禁止携带内部授权流水；route resolver 显式支持 `AUTHORIZATION_NO_AUTH_REFUND_STANDARD`；生命周期保存独立退款事实。 | 后续只作为 B4 回归基线维护；若要扩展运营审批、人工差错单、累计退款跨请求聚合控制、查询投影解释、外部规则核验或敏感上下文专项矩阵，必须另起独立 Execution Grant。 |
