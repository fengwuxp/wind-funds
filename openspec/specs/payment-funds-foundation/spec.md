...
| P0 | 钱包账户 | 资金账户、信用账户、预算控制范围（BudgetGroup 兼容名）、Spend Rule、平台账户角色、支付工具、支付工具绑定、支出主体资金责任解析关系、主体余额查询。 |
...
| P1 | 授权交易 | 授权批准、授权拒绝、可信授权撤销、授权完成、授权链退款、无授权直接退款、争议裁决资金结果承接、`settle` 强制完成模式；授权过期不作为资金交易事实。 |
| P1 | 余额控制 | 冻结、解冻、资金账户余额调整、信用账户额度调整和预算控制额度 / 窗口调整；预算控制调整只写控制额度变动和预算控制投影，不生成 route、posting、LedgerEntry 或账本余额；对账差错调账必须经过差错单、审批、凭证、审计和重新对账闭环，不得被简化成无来源的余额直接修改。 |
| P1 | 路由、回放和交易投影 | route resolver、route snapshot、Route Replay、缺快照失败、账本周期继承、交易投影和交易投影重放只读边界。 |
| P1 | 权益金额组件 | `FundsBenefitContributionTransactionService`、让利出资记账交易请求、商户券、平台补贴、储值券、授权占券、不退券、部分退款和缺权益资金事实逆向处理；历史 `benefitSnapshotId` 和 `stableDigest` 仅作为只读摘要追溯字段。 |
...
| --- | --- | --- | --- |
| VCC 发卡 | 授权交易、支付工具、route snapshot、账本账目、清结算、对账、归档和审计。 | 卡、token、持卡人和 processor account 只作为脱敏工具或外部引用；授权拒绝无账务副作用；clearing、refund 和争议裁决资金结果基于原授权或原 route。 | 完整 PAN/CVC 进入资金底座；VCC 卡被当作 ledger subject；授权拒绝生成 route、posting 或 ledger entry。 |
...
| 收单业务 | capture 归一资金动作、商户 CLEARING、清分、清算、结算、出款、退款、争议、对账和归档。 | capture 只进入待清算；清分、清算、结算和出款分层；refund 与争议扣回防重复损失；敏感卡数据最小化。 | 支付成功直接展示可提现；清分确认释放可结算；争议扣回被普通退款吞掉；完整 PAN/CVC 进入日志、投影、导出或测试夹具。 |
...
| 授权过期 | 到期、超时或通道未返回是不可信运营状态，不提供 `expire` 服务入口、不生成 `EXPIRE` 事件、不形成 `EXPIRED` 终态；释放占用必须由可信撤销、余额调整或对账差错补事实承接。 |
...
| 授权链争议裁决资金结果 | 表达已完成授权后的争议、扣回或追偿裁决结果；资金底座目标态不要求落到 `FundsAuthorizationTransactionService#chargeback`，默认通过 `settleRefund` 携带争议原因、凭证、上下文和审计承接。即使底层终态复用退款终态，也必须保留可查询、可投影、可审计的争议裁决语义，不能只留下普通退款结果。 |
| 授权支付工具应用入口 | 面向 VCC、卡、VA、共享卡、外部钱包端点或通道 token 的授权入口，可以接收支付工具引用和业务上下文；应用层必须先解析工具状态、绑定版本、使用主体、预算控制范围（BudgetGroup）、Spend Rule、资金责任和账户能力，再委派账户主体型授权内核。内部钱包、信用额度或权益入口先解析为 `SubjectRef`、权益资金事实引用或历史摘要。当前账户主体型 `authorize` 请求不得在未授权时被直接替换为工具引用字段。 |
...
| 预付卡、共享卡交易服务能力包 | 预付卡能力必须遵循“资金先确认、后授权使用”：外部入金、系统内充值或退卡转出只有在确认引用、幂等摘要和内部责任主体唯一时才能委派到账户主体型交易；共享卡能力必须遵循“账户入账、工具归因”：授权、清算、释放、退款和争议裁决资金结果都固化工具快照、绑定版本、预算控制范围（BudgetGroup）上下文、Spend Rule 决策和资金责任决策。 | 首期只能作为 P2 独立 Execution Grant 切片进入，建议拆为 `P2-VCC-PREPAID` 与 `P2-VCC-LIFECYCLE`；不得与 B2/B4/B5/B6/B8 一次性混合授权。 |
| 支付工具 | 卡、VA、外部账户、虚拟卡、外部钱包端点或通道 token 的路由输入和审计快照；不表达内部余额，不作为账本主体。内部余额钱包、平台钱包、商户钱包、返利钱包或信用额度入口不是支付工具，必须解析为资金账户、信用账户、平台账户角色解析后的平台资金账户、让利出资记账事实或资金责任决策后才能进入 route leg、posting 或 ledger entry。预算组和 Spend Rule 只作为控制上下文或规则快照。 |
...
| 支出主体资金责任解析关系 | 支出主体、支付工具、信用账户、预算控制范围（BudgetGroup）或 Spend Rule 上下文到最终内部资金或额度责任主体的解析关系；最终结果只能是资金账户、信用账户或平台账户角色解析后的平台资金账户。不计算 spend rules，不执行扣款，不写分录；预算控制范围和 Spend Rule 只提供 scope、规则条件、规则版本和审计解释，不能作为资金池或 `LedgerEntry` 主体。`FundingAccount` 只表示真实资金账户，不得泛化承载信用账户、预算控制范围、支付工具或钱包标识。历史代码、表名或服务名可继续使用 funding relation 兼容命名，但规格语义以资金责任解析为准。 |
...
| 平台补贴 / 储值代金券 | 平台补贴可能形成平台自有资金支出；储值、预付或礼品卡代金券可能涉及负债、预收待付或用户权益余额，不能复用当前优惠让利结算入口，需要独立能力和专业口径确认后再入账。 |
...
| 让利出资记账交易 DSL | 设计权威已统一到 `docs/DSL设计/支付资金底座DSL承载层设计.md`；目标态为 `FundsBenefitContributionTransactionService` 的 `settle/refund` 和通用请求摘要。`GSD2-BENEFIT-LEGACY-SNAPSHOT-REMOVE-001` 已删除旧 `FundsBenefitSnapshotSpec`、组件、引用、退款策略、稳定摘要对象、JSON 支撑和旧夹具；JSON 契约拒绝 `instruction.benefitSnapshot`，历史 `benefitSnapshotId` 与 `stableDigest` 仅可作为只读摘要。 | 当前只能按验证结果声明让利出资记账交易契约和首个真实入账资金流切片；不能声明非入账权益事实、授权占券、清结算、对账、投影、归档、冷热读取或治理重放生产链路已闭合。后续含权益 MVP 任务必须补 Phase/Batch、`fixtureLevel`、零实付表达、事实源、独立伴随指令原子性、储值/预付口径、退款分摊粒度、退款分摊确定性规则、历史无权益资金事实处理、补充权益事实模型、专业确认状态、审计证据包、使用者解释视图、证据最小化和外部规则核验状态准入。 |
| B2 覆盖索引：钱包账户与账本基础 | 已存在 `FundingAccountServiceImplTests`、`ControlAccountLedgerInitializationTests`、`PaymentInstrumentServiceImplTests`、`SpendSubjectFundingRelationServiceImplTests`、`LedgerBalanceProjectionServiceImplTests`、`DefaultLedgerPostingAssemblerTests`。 | 支付工具、绑定历史、资金责任解析关系（历史服务名仍为 funding relation）、资金账户/信用账户显式账本初始化、账务计划装配器长 ID 追溯和投影 afterCommit 已有局部基线；`BudgetGroupService#createBudgetGroup` 已收敛为只创建预算控制范围，不自动初始化 ledger bucket；账本周期、全量余额断言和组合交易仍需后续 MVP 任务继续闭合。 |
...
| B4 覆盖索引：授权交易 | 已存在 `FundsAuthorizationTransactionFlowTests`，覆盖授权拒绝、撤销、部分完成、全额完成、退款超额、强制完成、无授权直接退款和争议退款等局部场景；`b0666ba feat: 补齐授权过期释放 canonical 能力` 只保留为历史实现痕迹，2026-06-26 目标态已裁决移除 `EXPIRE` 事件、`expire` 服务入口、请求模型、route replay 分支、生命周期过期终态和授权过期释放测试；`616dac1 feat: 补齐授权强制完成能力` 和 `3825466 fix: 收紧授权强制完成策略红线` 已补齐 FORCE 模式、受信策略、上限、原因、外部原事实、凭证、审计、普通完成分支隔离和失败无副作用；B4-NO-AUTH-REFUND 已补齐 no-auth 退款模式、`externalReferenceSn`、退款原因和失败无副作用测试。 | 授权批准、拒绝、可信撤销、普通完成、完成后退款、强制完成、无授权直接退款和争议裁决资金结果首轮已有局部基线；授权过期不再作为资金交易能力；后续按 B4 覆盖索引闭合发卡控制扩展边界、授权占券和权益生命周期、并发竞争。 |
| B5 覆盖索引：余额控制 | 已存在 `FundsBalanceControlFailureFlowTests`、`FundsWithdrawalSuccessFlowTests`、`FundsWithdrawalAfterPartialUnfreezeFlowTests`、`FundsWithdrawalRejectionFlowTests`、`BudgetControlLimitAdjustmentApplicationServiceTests`。 | 冻结、解冻、提现、失败无副作用和部分组合路径已有局部基线；预算控制额度调整已具备控制额度变动 / 预算控制投影替代入口，不再把新增能力目标落到预算组 ledger control entry；资金账户余额调整、信用账户额度调整、adjust 红线、冻结关闭并发和全量金额临界值仍需补齐。 |
...
| --- | --- | --- | --- |
| GAP-LEDGER-001 | Ledger 跨模块生产入口必须区分真实用例职责，不为透传调用强行增加 application facade；写入走标准过账网关，只读账务证据走稳定流水查询，余额投影按账务 bucket 查询。 | `GSD2-LD-APPLICATION-FACADE-REBASE-001` 已回收首轮浅 application facade：删除透传型 posting / fact query / balance projection application 服务，保留 `LedgerTransactionPostingService` 作为唯一标准写入口；账本交易和账目分录只读证据查询按需合并回 `LedgerTransactionService`，通过 `getLedgerTransactionBySn`、`getLedgerEntryBySn` 按 tenant 和稳定 `sn` 强存在查询，查不到抛业务异常；分页查询仍可返回空页。交易侧余额调账审计和授权原账本交易查询已改为消费该基础服务。`ledger-face` 仍保留 `LedgerService#updateLedgerBalance/deleteLedgerByIds`、`LedgerTransactionService#updateLedgerTransaction/deleteLedgerTransactionByIds` 等公共兼容 API。 | 当前完成服务层职责回正和目标测试；后续若要删除、改签、隐藏或权限化现有 public service，必须单独给兼容策略、调用方影响清单、生产迁移和回归测试。受控运维审批、运营补账、历史数据清理和清结算差错补事实不得复用本 rebase Grant。 |
| GAP-AUTH-001 | 授权过期不应成为资金交易事实。 | `b0666ba feat: 补齐授权过期释放 canonical 能力` 只保留为历史实现痕迹；2026-06-26 目标态裁决要求移除 `FundsTransactionEventType.EXPIRE`、`FundsTransactionStatus.EXPIRED`、`FundsAuthorizationTransactionExpireRequest`、`FundsAuthorizationTransactionService#expire`、route replay 过期分支和授权过期释放测试。 | 后续不得把 `GAP-AUTH-001` 当待实现缺口；到期、超时或通道未返回只能进入提醒、差错候选或运营处理，释放占用必须由可信撤销、余额调整或对账差错补事实承接。 |
...
| GAP-AUTH-003 | chargeback / dispute 只作为已完成授权后的争议案件过程；交易层只消费裁决后的资金结果。需要退款或扣回时默认通过 `settleRefund` 携带原因、凭证、外部引用和审计上下文承接，无资金影响时不调用交易入口，不保留独立 `chargeback` 服务入口。 | `B4-DISPUTE-SEMANTIC-ALIGNMENT` 已由 `949b24a` 闭合首轮 canonical 可区分性：`settleRefund / AUTH_REFUND` 通过 `disputeMode`、`disputeReason`、`disputeVoucherRef` 和 `externalDisputeRef` 保留争议审计上下文，请求侧不恢复 `refundMode`。2026-06-26 目标态裁决进一步要求移除 `FundsAuthorizationTransactionService#chargeback`、`FundsAuthorizationTransactionChargebackRequest`、`CHARGEBACK` eventType 和 route replay `CHARGEBACK` phase；历史公共 API 退出计划只保留审计过程，不再作为删除阻断。 | 后续只把首轮争议退款可区分性和目标态资金红线作为 B4 回归基线维护；若要处理完整争议运营、representment、清结算追偿、VCC processor、外部规则或历史数据只读解释，必须另起 P2/B7/治理专项。 |
...
| GAP-AUTH-005 | 授权支付工具应用入口必须支持从工具引用、绑定和规则上下文解析到账户主体型授权内核。 | `InstrumentTransactionLifecycleApplicationService#authorizeByInstrument` 已成为支付工具交易生命周期统一授权入口，当前只委派 `AuthorizationAdmissionApplicationService#authorizeByInstrument`；授权准入专项服务复用 `PaymentInstrumentPreTransactionSnapshotApplicationService` 组合支付工具能力、绑定、资金责任和账户能力，请求携带 Spend Rule 决策证据时显式消费 `SpendControlAdmissionApplicationService`，`REJECTED` 会在账户主体型授权内核前拒绝并保持无资金事实副作用；批准后仍委派账户主体型 `FundsAuthorizationTransactionService#authorize`，route snapshot 承载支付工具引用和绑定快照。 | `GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 和 `GSD2-WALLET-INSTRUMENT-LIFECYCLE-AUTH-FACADE-001` 已消费为首轮 Green；仍不声明完整 Spend Rule 规则引擎、决策记录持久化、控制额度变动自动消费、VCC facade、统一支付工具交易内核或交易 canonical 入参改造完成。 |
| GAP-TRX-CAP-001 | 交易层仍需完善账户主体型授权生命周期、退款、余额控制调账和原路径回放能力；这些能力不是支付工具交易入口，也不是完整争议运营 case。 | 当前直接交易、授权交易、余额控制和投影已有局部基线，强制完成、B4-NO-AUTH-REFUND 首轮 canonical 能力、支付工具授权入口 route snapshot 回链和 `paymentInstrumentRef` 投影解释已形成 Red/Green 证据；授权过期释放与独立 chargeback 入口已退出目标态；争议裁决资金结果的 canonical 承接已有首轮基线，完整调账审计、缺快照差错、支付工具换绑后全链路 replay 和 projection store 仍未形成完整 Red/Green 证据。 | 后续 B3/B4/B5/B6 Execution Grant 可以在 `transaction-face`、`transaction-impl`、route replay 或 transaction projection 中扩展 canonical 请求、事件、状态机、DTO 和测试；主体仍必须是已解析资金账户、信用账户或平台账户角色解析后的平台资金账户。禁止把这些能力合并成统一 `InstrumentTransactionService`，也禁止让交易 canonical 请求直接接收支付工具作为账务主体；不得恢复 `expire` 或独立 `chargeback` 入口。 |
| GAP-TRX-CAP-002 | ledger / wallet / transaction 生产准出必须先证明交易主链服务层测试可运行，wallet-face 的只读账本事实契约应有可装配实现，且不破坏 `transaction-impl -> wallet-face`、`wallet-impl -> ledger-face` 的依赖方向。 | 生产可用性 CR 暴露 `just test-transaction` 因交易测试上下文缺少 `LedgerFactQueryService` Bean 失败；该服务契约位于 `wallet-face`，默认实现位于 `wallet-impl` 并只读消费 `ledger-face`，生产模块不得因此让 `transaction-impl` 依赖 `wallet-impl`。 | 本轮最小修复只允许补齐测试上下文装配并回归 `just test-transaction`；若后续生产启动装配仍缺 Bean，应通过应用启动模块或 Spring 扫描配置解决，不得新增同义空壳服务、不得把账本 Mapper 暴露给 transaction，也不得改变依赖方向。 |
| GAP-WALLET-001 | 钱包模块需要 application/use-case facade 承接跨模块用例，资源服务只作为内部协作或管理能力。 | 首轮已由 `GSD2-B2-WALLET-APPLICATION-FACADE-001` 补齐 `FundingResponsibilityResolutionApplicationService`，由 `GSD2-B2-WALLET-APPLICATION-FACADE-002` 补齐 `PaymentInstrumentCapabilityApplicationService`，由 `GSD2-B2-ACCOUNT-CAPABILITY-SOURCE-001` 补齐账户能力来源准入，由 `GSD2-B2-WALLET-PRE-TRANSACTION-SNAPSHOT-001` 补齐支付工具预交易快照，由 `GSD2-B2-SPEND-CONTROL-ADMISSION-001` 补齐支出控制准入快照，由 B5 切片补齐 Spend Control Movement 和交易消费控制额度变动；`GSD2-WALLET-AUTH-ADMISSION-COMPOSE-002` 已让授权准入复用预交易快照并可消费 Spend Rule 决策证据；`GSD2-WALLET-SPECIAL-BUSINESS-CAPABILITY-MAP-001` 已形成 VCC / VA / 全球账户 / ACH 入口能力地图，`InstrumentTransactionLifecycleApplicationService` 已具备授权、收款和出款 rail 契约 guard，`ExternalFundsEventApplicationService#consume` 已具备外部资金事件消费契约。`GSD2-WALLET-INSTRUMENT-RECEIVE-SNAPSHOT-001` 已让 `receiveByInstrument` 在委派账户主体型 `topup` 时把脱敏支付工具引用固化到 route snapshot；`GSD2-WALLET-INSTRUMENT-RECEIVE-BINDING-VERSION-GUARD-001` 已让收款请求强制携带 expectedBindingVersion，缺版本在交易内核前失败且无资金事实副作用；`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CONTRACT-001` 已证明外部事件消费未接归一资金事实内核时失败且无资金 / 账本副作用；`GSD2-WALLET-EXTERNAL-FUNDS-EVENT-CREDIT-CONSUME-001` 已证明 `ACH_CREDIT_CONFIRMED`、`BANK_CREDIT_CONFIRMED`、`EXTERNAL_CREDIT_CONFIRMED` 到资金账户可委派标准 `topup`，并生成 TOPUP 交易、route snapshot、posting plan、LedgerTransaction 和 LedgerEntry；`GSD2-WALLET-EXTERNAL-FUNDS-RAIL-SEMANTICS-001` 已让 wallet application 负责 `BANK_RAIL`、`ACH`、`wire-transfer` 等业务 rail / channel 别名和 confirmed credit 事件类型归一，交易层继续只消费账户主体和稳定 `FundsTransactionChannel`，route snapshot 外部账户 providerCode 保留 `ACH_RAIL`、`BANK_RAIL` 或 `EXTERNAL_RAIL` 等外部 rail 解释；`GSD2-WALLET-INSTRUMENT-BINDING-UNIQUENESS-001` 已让支付工具绑定创建 / 变更 `requestSn` 成为强幂等流水，并通过默认绑定 guard 表唯一行和 `FOR UPDATE` 证明并发下同一 scope 只能留下一个 ACTIVE 默认绑定，缺流水或重复默认绑定在写绑定当前态 / 绑定历史 / 账本事实前失败；guard 表已在 `tests/src/test/resources/jdbc-schema.sql` 的 H2/MySQL Mode 测试 schema、Entity、Mapper 和服务层 guard 中对齐，但尚未执行生产数据库迁移。调用方已可通过 wallet application 契约只读解析工具能力、资金责任、账户能力、外部 Spend Rule 决策证据和最终资金账户或信用账户目标主体快照；资源服务仍保留管理和内部协作职责。 | 后续 B2/B4/B5/P2 Execution Grant 必须继续决定钱包账户聚合、完整 Spend Rule 控制闭环、预算控制投影、VCC facade、真实 `payOutByRail`、外部扣账 / return / NOC / reversal、信用账户还款或调额、事件消费者 / outbox、生产数据库迁移执行或业务场景准入的 facade 命名、Request/DTO、依赖方向和边界测试；跨模块调用不得长期由调用方拼装多个资源服务来完成工具准入、账户能力校验、Spend Rule 决策证据校验或授权准入；业务入口参数不得为了统一入口而强制把内部余额钱包、信用额度账户、返利钱包或商户钱包建模为 `PaymentInstrument`。 |
...
| GAP-WALLET-001B | 跨模块调用不得绕过 wallet application facade 直接组合资源服务完成工具准入或交易委派。 | `FundsModuleDependencyBoundaryTests#testNonWalletModulesShouldNotComposeWalletResourceServicesDirectly` 已把 `PI-CAP-04` 落为边界守卫，非 wallet 生产模块不得直接依赖 `PaymentInstrumentService`、`SpendSubjectFundingRelationService`、`FundingAccountService`、`CreditAccountService`、`BudgetGroupService` 等资源服务拼装准入链路。 | 资源服务继续保留管理和内部协作职责；若新增 VCC、全球账户、支付工具、外部资金事件或账户聚合跨模块用例，应先补 application/use-case facade 契约、Request/DTO、失败无副作用测试和边界测试，而不是让调用方拼装资源服务。 |
...
| GAP-WALLET-003 | 预算组目标态是预算控制范围（Spend Control Scope），不是核心资金账务主体，现有 `BUDGET_GROUP` 兼容路径必须收敛。 | `BudgetGroupService#createBudgetGroup` 已不再初始化 ledger bucket，目标测试覆盖 LIFETIME、MONTHLY 和 CUSTOM_CYCLE 预算控制范围创建无账本事实副作用；Phase 1 已按 `staged-control-view-backed / control-view-replacement-first` 新增 `BudgetControlLimitAdjustmentApplicationService`，预算额度调增 / 调减写 Spend Control Movement 并派生 Budget Control Projection，不生成资金交易、route、posting、LedgerEntry、账本交易或余额投影；Phase 2 已消费 `GSD2-LD-BUDGET-GROUP-CONTROL-POSTING-FORBID-001`，任意 `BUDGET_GROUP` LedgerEntry 主体均在账本交易、posting plan、LedgerEntry 和余额投影落库前拒绝，预算组旧余额控制调额入口前置拒绝且无资金或账本事实副作用；Phase 3 已消费 `GSD2-B2-BUDGET-GROUP-LEDGER-COMPAT-CLEANUP-001`，`BUDGET_BASIC` 不再作为 active ledger profile，显式账本初始化、资金账户查询聚合、route leg / replay 和 DSL 合约均不再把预算组当作核心资金账务主体。 | `BUDGET_GROUP_LEDGER_COMPAT_CLEANUP_GREEN_VERIFIED`。不得把预算组 ledger 初始化、资金价值余额 bucket、控制账本兼容或任意 LedgerEntry 主体作为目标态 Done；`FundsSubjectType.BUDGET_GROUP` 仍作为历史枚举 / 控制上下文保留。生产历史数据清理、公共 API 删除、枚举删除、运营审批、权限模型或真实迁移脚本必须另起 Execution Grant。 |
...
| GAP-BEN-002 | 直接交易需按原让利出资记账交易处理商户券、平台补贴、储值券、零实付权益和叠加权益。 | 真实入账结算和退款型逆向处理已通过 `FundsBenefitContributionTransactionServiceImpl` 接到标准直接交易、route、posting 和账本链路；非入账权益事实、零实付主指令、伴随指令和储值负债口径仍未闭合。 | B3 覆盖索引继续补商户让利 no-ledger、平台补贴独立资金影响、储值券资金性质、零实付表达、独立伴随指令原子性、部分成功补偿、专业确认、审计证据包、使用者解释视图、证据最小化、外部规则核验和退款分摊确定性测试。 |
| GAP-BEN-003 | 授权交易需表达占券、完成核销、可信撤销释放，以及拒绝不处理权益；授权过期不作为资金层权益释放事实。 | 当前授权链路未见权益占用、核销或释放语义。 | B4 覆盖索引补授权占券、完成核销、可信撤销释放、拒绝无副作用和并发竞争测试；过期只进入提醒、差错候选或运营处理，不触发权益资金事实。 |
| GAP-BEN-004 | 退款、业务取消、可信撤销、拒付/争议结果和 Route Replay 必须使用原权益资金事实、历史摘要或经审批补充事实，不得按当前营销规则重算。 | `refund` 已要求引用原权益资金交易，并统一承接退款、业务取消、人工纠错或反向冲销；route replay 只读取历史 `benefitSnapshotId + stableDigest` 摘要且不依赖当前请求旧 DSL。仍缺补充权益事实模型、授权占券、部分退款累计和缺事实人工处理边界。 | B6 覆盖索引补原权益资金事实 digest、事实源一致性、缺事实失败、补充权益事实只追加、部分退款累计闭合、不退券和补贴冲回/保留分离测试；授权过期不得触发资金层权益重算。 |
...
| GAP-BEN-008 | 让利出资入口应从重型权益快照契约重基线为交易级 application service。 | 首轮缺口已关闭：`transaction-face` 暴露 `FundsBenefitContributionTransactionService#settle/refund`，真实入账让利出资已接到标准交易路由、交易事实和账本分录链路，旧重型权益 DSL 已物理删除，独立 `reverse` 入口已收敛为 `refund` 型逆向处理。 | 后续只作为让利出资记账交易回归基线维护；若要扩展非入账权益事实、独立权益事实表、清结算对账投影或授权占券，需要新的单一 Execution Grant。 |
...
| 4 | P1 交易入口：授权交易 | 重建授权、拒绝、撤销、完成、过期、强制完成、完成后退款、无授权直接退款、争议裁决资金结果承接、授权占券和 VCC 扩展边界测试。 |
