...
- Spring 服务层流程测试必须优先使用真实内部 Spring Bean 和 H2 表结构，可继承：AbstractFundsServiceTest；账户、平台账户、路由基础数据等测试依赖由 `setup` 或用例显式准备，Mock/Fake/Recording 只用于外部系统、不可控环境或明确的端口边界。
