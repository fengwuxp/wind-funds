...
> Skill 是专题流程层，不替代本文件的最低约束；通用架构、编码、测试、Review、Git 与交付纪律优先按 `资深架构师` Skill 展开，支付资金产品口径优先按 `支付产品与资金系统专家` Skill 展开。
...
通用 Java/Spring 编码规约、Clean Code、异常日志、安全、集合、金额、时间、空值、注释和禁用项由 `资深架构师` Skill 的编码能力执行。

## 6. 资金测试红线

测试用例设计、测试分层、命名、结构、注释和补测流程由 `资深架构师` Skill 执行。本项目只保留资金域不可丢失的断言红线：
...
交付说明的通用结构、Git 提交建议和验证结论表达由 `资深架构师` Skill 执行。本项目交付时必须额外说明：
