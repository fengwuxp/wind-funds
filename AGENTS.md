...
本仓库默认开启已授权 Skill 的本地协作学习机制。若本机 `~/.skill-learning/consent.md` 存在且 `status: enabled`，`资深架构师` 与 `产品架构专家` 可按各自 Skill 的混合、协作学习规则记录长期偏好、团队约规、业务背景和待确认观察。

学习记录只允许写入本机学习目录，不得写入本仓库、Skill 安装目录或远程系统；不得记录客户、商户、用户、交易、账号、证件、密钥、生产配置等敏感信息。影响长期行为、跨 Skill 复用、业务/合规/隐私边界或强约束偏好的内容，必须先向用户确认。

如果 `consent.md` 仍指向已替换的旧 Skill，应先迁移到 `product-architecture-expert` 或全局授权后再写入对应学习记录。
