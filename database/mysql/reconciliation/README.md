...
`WIND_FUNDS_TEST_MYSQL_EXPECTED_VERSION_PREFIX` 必须与宿主目标小版本一致；迁移测试同时要求 MySQL 产品身份、库名和 `REPEATABLE-READ` 隔离级别匹配。该命令是采用 MySQL 的宿主可选兼容证据，不是 wind-funds 仓库级准出硬门禁，也不替代宿主的迁移记录、checksum、备份恢复和预发验收。

仓库级数据库证据由 H2 全量集成测试和 `ReconciliationMysqlDdlContractTests` 提供，证明已声明功能、事务回滚、schema 与生产 DDL 静态契约。它们不等价证明真实 MySQL 并发、性能、备份恢复和宿主事务接入验收；这些证据由采用 MySQL 的宿主负责。
